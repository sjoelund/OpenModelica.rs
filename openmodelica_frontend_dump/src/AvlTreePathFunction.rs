    ...
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        ...
        Deref @ Tree::NODE { key, .. } => {
            ...
            if key_comp.clone() == -1 {
                ...
                if !((match (&(var_field!((*tree).value, Tree::NODE).clone()), &(value.clone())) { (None, None) => true, (Some(__refeq_l), Some(__refeq_r)) => referenceEq(__refeq_l, __refeq_r), _ => false })) {
        ...
        Deref @ Tree::LEAF { .. } => {
            ...
            if key_comp.clone() == -1 {
                outTree = Arc::new(Tree::NODE { key: var_field!((*tree).key, Tree::LEAF).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: Arc::new(Tree::LEAF { key: inKey.clone(), value: inValue.clone() }), right: Arc::new(crate::AvlTreePathFunction::Tree::EMPTY) });
            } else if key_comp.clone() == 1 {
                outTree = Arc::new(Tree::NODE { key: var_field!((*tree).key, Tree::LEAF).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: Arc::new(crate::AvlTreePathFunction::Tree::EMPTY), right: Arc::new(Tree::LEAF { key: inKey.clone(), value: inValue.clone() }) });
            } else {
                value = conflictFunc(inValue.clone(), var_field!((*tree).value, Tree::LEAF).clone(), var_field!((*tree).key, Tree::LEAF).clone())?;
                if !((match (&(var_field!((*tree).value, Tree::LEAF).clone()), &(value.clone())) { (None, None) => true, (Some(__refeq_l), Some(__refeq_r)) => referenceEq(__refeq_l, __refeq_r), _ => false })) {
...
pub fn addUpdate(mut tree: Arc<Tree>, mut key: Key, mut r#fn: Arc<dyn ::std::ops::Fn(Option<Option<DAE::Function>>) -> Result<Option<DAE::Function>> + 'static>) -> Result<Arc<Tree>> {
    ...
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        ...
        Deref @ Tree::LEAF { .. } => {
            ...
            if key_comp.clone() == -1 {
                new_tree = Arc::new(Tree::NODE { key: var_field!((*tree).key, Tree::LEAF).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: Arc::new(Tree::LEAF { key: key.clone(), value: r#fn(None)? }), right: Arc::new(crate::AvlTreePathFunction::Tree::EMPTY) });
            } else if key_comp.clone() == 1 {
                new_tree = Arc::new(Tree::NODE { key: var_field!((*tree).key, Tree::LEAF).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: Arc::new(crate::AvlTreePathFunction::Tree::EMPTY), right: Arc::new(Tree::LEAF { key: key.clone(), value: r#fn(None)? }) });
            } else {
...
pub fn fold<FT: Clone + 'static>(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(Arc<Absyn::Path>, Option<DAE::Function>, FT) -> Result<FT> + 'static>, mut inStartValue: FT) -> Result<FT> {
...
pub fn foldCond<FT: Clone + 'static>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(Arc<Absyn::Path>, Option<DAE::Function>, FT) -> Result<(FT, bool)> + 'static>, mut value: FT) -> Result<FT> {
...
pub fn fold_2<FT1: Clone + 'static, FT2: Clone + 'static>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(Arc<Absyn::Path>, Option<DAE::Function>, FT1, FT2) -> Result<(FT1, FT2)> + 'static>, mut foldArg1: FT1, mut foldArg2: FT2) -> Result<(FT1, FT2)> {
...
pub fn forEach(mut tree: Arc<Tree>, mut func: Arc<dyn ::std::ops::Fn(Arc<Absyn::Path>, Option<DAE::Function>) -> Result<()> + 'static>) -> Result<()> {
...
pub fn map(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(Arc<Absyn::Path>, Option<DAE::Function>) -> Result<Option<DAE::Function>> + 'static>) -> Result<Arc<Tree>> {
    ...
    outTree = (::match_deref::match_deref! { match &(outTree.clone()) {
        Deref @ Tree::NODE { value, key, .. } => {
            ...
            if !(referenceEq(&*(new_left.clone()),&*(var_field!((*outTree).left, Tree::NODE).clone()))) || !((match (&(value.clone()), &(new_value.clone())) { (None, None) => true, (Some(__refeq_l), Some(__refeq_r)) => referenceEq(__refeq_l, __refeq_r), _ => false })) || !(referenceEq(&*(new_right.clone()),&*(var_field!((*outTree).right, Tree::NODE).clone()))) {
        ...
        Deref @ Tree::LEAF { value, key } => {
            let mut new_value: Value = None;
            new_value = inFunc(key.clone(), value.clone())?;
            if !((match (&(value.clone()), &(new_value.clone())) { (None, None) => true, (Some(__refeq_l), Some(__refeq_r)) => referenceEq(__refeq_l, __refeq_r), _ => false })) {
...
pub fn mapFold<FT: Clone + 'static>(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(Arc<Absyn::Path>, Option<DAE::Function>, FT) -> Result<(Option<DAE::Function>, FT)> + 'static>, mut inStartValue: FT) -> Result<(Arc<Tree>, FT)> {
...
pub fn new() -> Arc<Tree> {
    let mut outTree: Arc<Tree> = Arc::new(crate::AvlTreePathFunction::Tree::EMPTY);
    outTree
}
...
fn rotateLeft(mut inNode: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    outNode = (::match_deref::match_deref! { match &(outNode.clone()) {
        ...
        Deref @ Tree::NODE { right: child @ Deref @ Tree::LEAF { .. }, .. } => {
            let mut node: Arc<Tree> = Arc::new(Tree::EMPTY);
            node = setTreeLeftRight(outNode.clone(), var_field!((*outNode).left, Tree::NODE).clone(), Arc::new(crate::AvlTreePathFunction::Tree::EMPTY))?;
            setTreeLeftRight(child.clone(), node.clone(), Arc::new(crate::AvlTreePathFunction::Tree::EMPTY))?
        },
        _ => {
            inNode.clone()
        },
...
fn rotateRight(mut inNode: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    outNode = (::match_deref::match_deref! { match &(outNode.clone()) {
        ...
        Deref @ Tree::NODE { left: child @ Deref @ Tree::LEAF { .. }, .. } => {
            let mut node: Arc<Tree> = Arc::new(Tree::EMPTY);
            node = setTreeLeftRight(outNode.clone(), Arc::new(crate::AvlTreePathFunction::Tree::EMPTY), var_field!((*outNode).right, Tree::NODE).clone())?;
            setTreeLeftRight(child.clone(), Arc::new(crate::AvlTreePathFunction::Tree::EMPTY), node.clone())?
        },
        _ => {
            inNode.clone()
        },
