...
pub fn add(mut inTree: Arc<Tree>, mut inKey: Key) -> Result<Arc<Tree>> {
    ...
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        ...
        Deref @ Tree::LEAF { key } => {
            ...
            key_comp = keyCompare(inKey.clone(), key.clone())?;
            if key_comp == -1 {
                outTree = Arc::new(Tree::NODE { key: var_field!((*tree).key, Tree::LEAF).clone(), height: 2, left: Arc::new(Tree::LEAF { key: inKey }), right: crate::AvlSetCR::Tree::interned_EMPTY() });
            } else if key_comp == 1 {
                outTree = Arc::new(Tree::NODE { key: var_field!((*tree).key, Tree::LEAF).clone(), height: 2, left: crate::AvlSetCR::Tree::interned_EMPTY(), right: Arc::new(Tree::LEAF { key: inKey }) });
            } else {
                outTree = tree;
            }
            outTree
...
pub(crate) fn addList(mut tree: Arc<Tree>, mut inValues: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>) -> Result<Arc<Tree>> {
...
pub(crate) fn intersection(mut tree1: Arc<Tree>, mut tree2: Arc<Tree>) -> Result<(Arc<Tree>, Arc<Tree>, Arc<Tree>)> {
    let mut intersect: Arc<Tree> = crate::AvlSetCR::Tree::interned_EMPTY();
    let mut rest1: Arc<Tree> = crate::AvlSetCR::Tree::interned_EMPTY();
    let mut rest2: Arc<Tree> = crate::AvlSetCR::Tree::interned_EMPTY();
    let mut keylist1: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
    let mut keylist2: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
    let mut k1: Key;
    let mut k2: Key;
    let mut key_comp: i32;
...
pub(crate) fn new() -> Arc<Tree> {
    let mut outTree: Arc<Tree> = crate::AvlSetCR::Tree::interned_EMPTY();
    outTree
}
...
fn rotateLeft(mut inNode: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    outNode = (::match_deref::match_deref! { match &(outNode.clone()) {
        ...
        Deref @ Tree::NODE { right: child @ Deref @ Tree::LEAF { .. }, .. } => {
            ...
            setTreeLeftRight(child.clone(), node, crate::AvlSetCR::Tree::interned_EMPTY())?
        },
        _ => {
            inNode
        },
...
fn rotateRight(mut inNode: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    outNode = (::match_deref::match_deref! { match &(outNode.clone()) {
        ...
        Deref @ Tree::NODE { left: child @ Deref @ Tree::LEAF { .. }, .. } => {
            ...
            setTreeLeftRight(child.clone(), crate::AvlSetCR::Tree::interned_EMPTY(), node)?
        },
        _ => {
            inNode
        },
