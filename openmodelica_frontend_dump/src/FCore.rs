...
impl metamodelica::gc::MMTrace for Data {
    fn mm_accept(&self, __mmv: &mut dyn metamodelica::gc::MMVisitor) -> Result<(), ()> {
        match self {
            ...
            Data::CL { e, pre, r#mod, kind, status } => {
...
pub mod RefTree {
    ...
    pub(crate) fn addList(mut tree: Arc<Tree>, mut inValues: Arc<metamodelica::List<(ArcStr, metamodelica::Array<Node>)>>, mut conflictFunc: Arc<dyn ::std::ops::Fn(metamodelica::Array<Node>, metamodelica::Array<Node>, ArcStr) -> Result<metamodelica::Array<Node>> + 'static>) -> Result<Arc<Tree>> {
    ...
    pub(crate) fn addUpdate(mut tree: Arc<Tree>, mut key: Key, mut r#fn: Arc<dyn ::std::ops::Fn(Option<metamodelica::Array<Node>>) -> Result<metamodelica::Array<Node>> + 'static>) -> Result<Arc<Tree>> {
    ...
    pub(crate) fn foldCond<FT: Clone + 'static + metamodelica::gc::MMTrace>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(ArcStr, metamodelica::Array<Node>, FT) -> Result<(FT, bool)> + 'static>, mut value: FT) -> Result<FT> {
    ...
    pub(crate) fn fold_2<FT1: Clone + 'static + metamodelica::gc::MMTrace, FT2: Clone + 'static + metamodelica::gc::MMTrace>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(ArcStr, metamodelica::Array<Node>, FT1, FT2) -> Result<(FT1, FT2)> + 'static>, mut foldArg1: FT1, mut foldArg2: FT2) -> Result<(FT1, FT2)> {
    ...
    pub(crate) fn forEach(mut tree: Arc<Tree>, mut func: Arc<dyn ::std::ops::Fn(ArcStr, metamodelica::Array<Node>) -> Result<()> + 'static>) -> Result<()> {
    ...
    pub(crate) fn fromList(mut inValues: Arc<metamodelica::List<(ArcStr, metamodelica::Array<Node>)>>, mut conflictFunc: Arc<dyn ::std::ops::Fn(metamodelica::Array<Node>, metamodelica::Array<Node>, ArcStr) -> Result<metamodelica::Array<Node>> + 'static>) -> Result<Arc<Tree>> {
    ...
    pub(crate) fn getOpt(mut tree: Arc<Tree>, mut key: Key) -> Option<metamodelica::Array<Node>> {
        '__tco: loop {
            let mut k: Key;
            k = ((::match_deref::match_deref! { match &(tree.clone()) {
        Deref @ Tree::NODE { .. } => var_field!((*tree).key, Tree::NODE).clone(),
    ...
    pub fn map(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, metamodelica::Array<Node>) -> Result<metamodelica::Array<Node>> + 'static>) -> Result<Arc<Tree>> {
    ...
    pub fn new() -> Arc<Tree> {
        let mut outTree: Arc<Tree> = crate::FCore::RefTree::Tree::interned_EMPTY();
        outTree
    }
    ...
    fn rotateLeft(mut inNode: Arc<Tree>) -> Result<Arc<Tree>> {
        ...
        Deref @ Tree::NODE { right: child @ Deref @ Tree::LEAF { .. }, .. } => {
            let mut node: Arc<Tree>;
            node = setTreeLeftRight(outNode.clone(), var_field!((*outNode).left, Tree::NODE).clone(), crate::FCore::RefTree::Tree::interned_EMPTY())?;
    ...
    fn rotateRight(mut inNode: Arc<Tree>) -> Result<Arc<Tree>> {
        ...
        Deref @ Tree::NODE { left: child @ Deref @ Tree::LEAF { .. }, .. } => {
            let mut node: Arc<Tree>;
            node = setTreeLeftRight(outNode.clone(), crate::FCore::RefTree::Tree::interned_EMPTY(), var_field!((*outNode).right, Tree::NODE).clone())?;
    ...
    pub(crate) fn toList(mut inTree: Arc<Tree>, mut lst: Arc<metamodelica::List<(ArcStr, metamodelica::Array<Node>)>>) -> Arc<metamodelica::List<(ArcStr, metamodelica::Array<Node>)>> {
    ...
    pub(crate) fn update(mut tree: Arc<Tree>, mut key: Key, mut value: Value) -> Result<Arc<Tree>> {
        let mut outTree: Arc<Tree> = add(tree.clone(), (key.clone()).clone(), value.clone(), (std::sync::Arc::new(fnptr!(addConflictReplace, metamodelica::Array<Node>, metamodelica::Array<Node>, ArcStr)) as std::sync::Arc<dyn ::std::ops::Fn(metamodelica::Array<Node>, metamodelica::Array<Node>, ArcStr) -> Result<metamodelica::Array<Node>> + 'static>))?;
        Ok(outTree)
    }
...
pub fn getFunctionTree(mut cache: Cache) -> Arc<AvlTreePathFunction::Tree> {
    ...
    ft = (match cache {
        ...
        _ => {
            crate::AvlTreePathFunction::Tree::interned_EMPTY()
        },
    });
    ft
}
...
pub(crate) fn getRecordConstructorPath(mut inPath: Arc<Absyn::Path>) -> Result<Arc<Absyn::Path>> {
