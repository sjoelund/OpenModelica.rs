...
pub(crate) fn removeReplacement(mut repl: VariableReplacements, mut inSrc: Arc<DAE::ComponentRef>) -> Result<()> {
    let mut dst_opt: Option<Arc<DAE::Exp>>;
...
pub(crate) fn addReplacements(mut iRepl: VariableReplacements, mut inSrcs: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, mut inDsts: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut inFuncTypeExpExpToBooleanOption: Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>) -> Result<VariableReplacements> {
    '__tco: loop {
        ...
        (Deref @ metamodelica::List::Cons { head: cr, tail: crlst }, Deref @ metamodelica::List::Cons { head: exp, tail: explst }) => {
            ...
            { (iRepl, inSrcs, inDsts, inFuncTypeExpExpToBooleanOption) = (repl, crlst.clone(), explst.clone(), inFuncTypeExpExpToBooleanOption); continue '__tco; }
        },
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (src, dst) => {
                    ...
                    let mut iv: Arc<metamodelica::List<ArcStr>>;
                    let mut derConst: Option<Arc<UnorderedMap::UnorderedMap<Arc<DAE::ComponentRef>, Option<Arc<DAE::Exp>>>>>;
...
pub(crate) fn performReplacementsEqSystem(mut inEqs: Arc<BackendDAE::EqSystem>, mut inRepl: VariableReplacements) -> Result<Arc<BackendDAE::EqSystem>> {
...
fn addReplacementInv2(mut invHt: CrefCrefListTable, mut dst: Arc<DAE::ComponentRef>, mut src: Arc<DAE::ComponentRef>) -> Result<CrefCrefListTable> {
    let mut invHt: CrefCrefListTable = invHt;
    let mut srcs_opt: Option<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>;
...
fn makeTransitive1(mut repl: VariableReplacements, mut src: Arc<DAE::ComponentRef>, mut dst: Arc<DAE::Exp>, mut inFuncTypeExpExpToBooleanOption: Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>) -> (VariableReplacements, Arc<DAE::ComponentRef>, Arc<DAE::Exp>) {
    let mut outRepl: VariableReplacements;
    let mut outSrc: Arc<DAE::ComponentRef>;
    let mut outDst: Arc<DAE::Exp>;
...
fn makeTransitive2(mut repl: VariableReplacements, mut src: Arc<DAE::ComponentRef>, mut dst: Arc<DAE::Exp>, mut inFuncTypeExpExpToBooleanOption: Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>) -> (VariableReplacements, Arc<DAE::ComponentRef>, Arc<DAE::Exp>) {
    let mut outRepl: VariableReplacements;
    let mut outSrc: Arc<DAE::ComponentRef>;
    let mut outDst: Arc<DAE::Exp>;
    (outRepl, outSrc, outDst) = (match inFuncTypeExpExpToBooleanOption.clone() {
...
fn addExtendReplacement(mut extendrepl: CrefSet, mut cr: Arc<DAE::ComponentRef>, mut preCr: Option<Arc<DAE::ComponentRef>>) -> Result<CrefSet> {
    ...
    let mut wcr: Arc<DAE::ComponentRef>;
    let mut wpre: Option<Arc<DAE::ComponentRef>>;
    while !(worklist.clone().is_empty()) {
        (wcr, wpre) = listHead(worklist.clone())?;
        worklist = listRest(worklist.clone())?;
        let _ = (::match_deref::match_deref! { match &((wcr.clone(), wpre.clone())) {
        ...
        (Deref @ DAE::ComponentRef::CREF_IDENT { ident, identType: ty @ Deref @ DAE::Type::T_ARRAY { .. }, .. }, Some(pcr)) => {
            let mut precr: Arc<DAE::ComponentRef>;
            let mut precr1: Arc<DAE::ComponentRef>;
            precr = ComponentReferenceBasics::makeCrefIdent((ident.clone()).clone(), ty.clone(), metamodelica::nil());
        ...
        (Deref @ DAE::ComponentRef::CREF_QUAL { ident, identType: ty, subscriptLst, componentRef: subcr }, None) => {
            let mut precr: Arc<DAE::ComponentRef>;
            let mut precrn: Arc<DAE::ComponentRef>;
            precr = ComponentReferenceBasics::makeCrefIdent((ident.clone()).clone(), ty.clone(), metamodelica::nil());
            UnorderedSet::add(precr.clone(), extendrepl.clone())?;
        ...
        (Deref @ DAE::ComponentRef::CREF_QUAL { ident, identType: ty, subscriptLst, componentRef: subcr }, Some(pcr)) => {
            ...
            let mut precr1: Arc<DAE::ComponentRef>;
            let mut precrn: Arc<DAE::ComponentRef>;
            let mut precrn1: Arc<DAE::ComponentRef>;
            precr = ComponentReferenceBasics::makeCrefIdent((ident.clone()).clone(), ty.clone(), metamodelica::nil());
            precr1 = ComponentReference::joinCrefs(pcr.clone(), precr.clone())?;
            UnorderedSet::add(precr1.clone(), extendrepl.clone())?;
            precrn = ComponentReferenceBasics::makeCrefIdent((ident.clone()).clone(), ty.clone(), subscriptLst.clone());
...
pub(crate) fn addDerConstRepl(mut inComponentRef: Arc<DAE::ComponentRef>, mut inExp: Arc<DAE::Exp>, mut repl: VariableReplacements) -> Result<VariableReplacements> {
    let mut repl: VariableReplacements = repl;
...
pub(crate) fn getAllReplacements(mut repl: VariableReplacements) -> Result<(Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::Exp>>>)> {
    let mut crefs: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
...
fn avoidDoubleHashLookup(mut inExp: Arc<DAE::Exp>, mut inType: Arc<DAE::Type>) -> Arc<DAE::Exp> {
    let mut outExp: Arc<DAE::Exp>;
    outExp = 'mc: {
...
pub(crate) fn addConstantRecordReplacements(mut ty: Arc<DAE::Type>, mut expl: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut repl: VariableReplacements, mut inFuncTypeExpExpToBooleanOption: Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>) -> Result<VariableReplacements> {
    let mut repl: VariableReplacements = repl;
...
pub(crate) fn replaceCref(mut crefIn: Arc<DAE::ComponentRef>, mut replIn: VariableReplacements) -> Result<(Arc<DAE::Exp>, bool)> {
...
fn replaceCrefSubs(mut inCref: Arc<DAE::ComponentRef>, mut repl: VariableReplacements, mut cond: Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>) -> Result<(Arc<DAE::ComponentRef>, bool)> {
    ...
    (outCr, replacementPerformed) = (::match_deref::match_deref! { match &(inCref.clone()) {
        Deref @ DAE::ComponentRef::CREF_QUAL { ident: name, identType: ty, subscriptLst: subs, componentRef: cr } => {
            ...
            cr = if (c1 || c2) {Arc::new(DAE::ComponentRef::CREF_QUAL { ident: (name.clone()).clone(), identType: ty.clone(), subscriptLst: subs.clone(), componentRef: cr.clone() })} else {inCref};
            (cr.clone(), c1 || c2)
        },
        Deref @ DAE::ComponentRef::CREF_IDENT { ident: name, identType: ty, subscriptLst: subs } => {
            ...
            cr = if (c1) {Arc::new(DAE::ComponentRef::CREF_IDENT { ident: (name.clone()).clone(), identType: ty.clone(), subscriptLst: subs.clone() })} else {inCref};
            (cr.clone(), c1)
        },
        _ => {
            (inCref, false)
...
fn replaceCrefSubs2(mut isubs: Arc<metamodelica::List<Arc<DAE::Subscript>>>, mut repl: VariableReplacements, mut cond: Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>) -> Result<(Arc<metamodelica::List<Arc<DAE::Subscript>>>, bool)> {
    ...
    outSubs = ({
        ...
        Deref @ DAE::Subscript::SLICE { exp } => {
            ...
            if (c1) {Arc::new(DAE::Subscript::SLICE { exp: exp.clone() })} else {sub.clone()}
        },
        Deref @ DAE::Subscript::INDEX { exp } => {
            ...
            (exp, c1) = replaceExp(exp.clone(), repl.clone(), cond.clone());
            replacementPerformed = replacementPerformed || c1;
            if (c1) {Arc::new(DAE::Subscript::INDEX { exp: exp.clone() })} else {sub.clone()}
        },
        Deref @ DAE::Subscript::WHOLE_NONEXP { exp } => {
            ...
            (exp, c1) = replaceExp(exp.clone(), repl.clone(), cond.clone());
            replacementPerformed = replacementPerformed || c1;
            if (c1) {Arc::new(DAE::Subscript::WHOLE_NONEXP { exp: exp.clone() })} else {sub.clone()}
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
            __acc = cons(__x, __acc);
...
pub(crate) fn replaceExpList(mut iexpl: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut repl: VariableReplacements, mut cond: Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>) -> (Arc<metamodelica::List<Arc<DAE::Exp>>>, bool) {
...
pub(crate) fn replaceExpList1(mut iexpl: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut repl: VariableReplacements, mut cond: Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>) -> (Arc<metamodelica::List<Arc<DAE::Exp>>>, Arc<metamodelica::List<bool>>) {
    let mut outExpl: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    let mut replacementPerformed: Arc<metamodelica::List<bool>>;
...
fn replaceExpIters(mut inIters: Arc<metamodelica::List<Arc<DAE::ReductionIterator>>>, mut repl: VariableReplacements, mut cond: Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>) -> (Arc<metamodelica::List<Arc<DAE::ReductionIterator>>>, bool) {
    let mut outIter: Arc<metamodelica::List<Arc<DAE::ReductionIterator>>>;
    let mut replacementPerformed: bool = false;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ BackendDAE::Equation::IF_EQUATION { conditions: expl, eqnstrue: eqnslst, eqnsfalse: eqns, source, attr: eqAttr }, repl) => {
                    ...
                    (eqnslst, b2) = List::map3Fold(eqnslst.clone(), (std::sync::Arc::new(fnptr!(replaceEquations2, Arc<metamodelica::List<Arc<BackendDAE::Equation>>>, VariableReplacements, Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>, Arc<metamodelica::List<Arc<BackendDAE::Equation>>>, bool)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<metamodelica::List<Arc<BackendDAE::Equation>>>, VariableReplacements, Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>, Arc<metamodelica::List<Arc<BackendDAE::Equation>>>, bool) -> Result<(Arc<metamodelica::List<Arc<BackendDAE::Equation>>>, bool)> + 'static>), repl.clone(), inFuncTypeExpExpToBooleanOption.clone(), metamodelica::nil(), false)?;
...
fn replaceWhenEquation(mut whenEqn: Arc<BackendDAE::WhenEquation>, mut repl: VariableReplacements, mut inFuncTypeExpExpToBooleanOption: Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>, mut isource: Arc<DAE::ElementSource>) -> Result<(Arc<BackendDAE::WhenEquation>, Arc<DAE::ElementSource>, bool)> {
    ...
    (outWhenEqn, osource, replacementPerformed) = (::match_deref::match_deref! { match &(whenEqn.clone()) {
        Deref @ BackendDAE::WhenEquation { condition: cond, whenStmtLst, elsewhenPart: oelsewhenPart } => {
            ...
            source = ElementSource::addSymbolicTransformationSubstitution(b1, isource, cond.clone(), cond2.clone())?;
            ...
            b4 = b1 || b2 || b3;
            weqn = if (b4) {Arc::new(BackendDAE::WhenEquation { condition: cond2, whenStmtLst: whenStmtLst.clone(), elsewhenPart: oelsewhenPart.clone() })} else {whenEqn};
            (weqn, source, b4)
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn replaceWhenOperator(mut inReinitStmtLst: Arc<metamodelica::List<BackendDAE::WhenOperator>>, mut repl: VariableReplacements, mut inFuncTypeExpExpToBooleanOption: Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>, mut replacementPerformed: bool, mut iAcc: Arc<metamodelica::List<BackendDAE::WhenOperator>>) -> Result<(Arc<metamodelica::List<BackendDAE::WhenOperator>>, bool)> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: wop @ BackendDAE::WhenOperator::ASSIGN { left: cre @ Deref @ DAE::Exp::CREF { componentRef: cr, .. }, right: exp, source }, tail: res } => {
            ...
            { (inReinitStmtLst, repl, inFuncTypeExpExpToBooleanOption, replacementPerformed, iAcc) = (res.clone(), repl, inFuncTypeExpExpToBooleanOption, replacementPerformed || b, metamodelica::cons(wop1, iAcc)); continue '__tco; }
        },
        ...
        Deref @ metamodelica::List::Cons { head: wop @ BackendDAE::WhenOperator::REINIT { stateVar: cr, value: cond, source }, tail: res } => {
            ...
            source = ElementSource::addSymbolicTransformationSubstitution(b1, source.clone(), cre.clone(), cre1)?;
...
fn replaceExpWrapper(mut inExp: Arc<DAE::Exp>, mut inTpl: (VariableReplacements, Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>, bool)) -> (Arc<DAE::Exp>, (VariableReplacements, Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>, bool)) {
    pub type FuncTypeExp_ExpToBoolean = std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>;
...
fn validLhsArrayAssignSTMT(mut oldCr: Arc<DAE::ComponentRef>, mut lhs: Arc<DAE::Exp>, mut rhs: Arc<DAE::Exp>, mut type_: Arc<DAE::Type>, mut source: Arc<DAE::ElementSource>, mut inStatementLst: Arc<metamodelica::List<Arc<DAE::Statement>>>) -> Arc<metamodelica::List<Arc<DAE::Statement>>> {
    let mut outStatementLst: Arc<metamodelica::List<Arc<DAE::Statement>>>;
    outStatementLst = 'mc: {
...
fn replaceElse(mut inElse: Arc<DAE::Else>, mut inVariableReplacements: VariableReplacements, mut inFuncTypeExpExpToBooleanOption: Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>) -> (Arc<DAE::Else>, bool) {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Else::ELSE { statementLst }, repl) => {
                    let mut statementLst_1: Arc<metamodelica::List<Arc<DAE::Statement>>>;
                    let __pa0 = ::match_deref::match_deref! { match &(replaceStatementLst(statementLst.clone(), repl.clone(), inFuncTypeExpExpToBooleanOption.clone(), metamodelica::nil(), false)) {
                        (__pa0, true) => __pa0.clone(),
                        _ => bail!("pattern mismatch"),
                    } };
...
fn replaceElse1(mut inExp: Arc<DAE::Exp>, mut inStatementLst: Arc<metamodelica::List<Arc<DAE::Statement>>>, mut inElse: Arc<DAE::Else>, mut inVariableReplacements: VariableReplacements, mut inFuncTypeExpExpToBooleanOption: Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>) -> (Arc<DAE::Else>, bool) {
    let mut outElse: Arc<DAE::Else>;
    let mut replacementPerformed: bool;
    (outElse, replacementPerformed) = 'mc: {
...
pub(crate) fn replaceBindingExp(mut varIn: BackendDAE::Var, mut repl: VariableReplacements) -> Result<BackendDAE::Var> {
...
pub(crate) fn replaceVariableAttributes(mut attrIn: Arc<DAE::VariableAttributes>, mut repl: VariableReplacements) -> Result<Arc<DAE::VariableAttributes>> {
...
pub(crate) fn replaceOptionExp(mut optIn: Option<Arc<DAE::Exp>>, mut repl: VariableReplacements) -> Result<Option<Arc<DAE::Exp>>> {
...
pub(crate) fn replaceVariableAttributesInVar(mut varIn: BackendDAE::Var, mut repl: VariableReplacements) -> Result<BackendDAE::Var> {
    let mut varOut: BackendDAE::Var;
    varOut = (::match_deref::match_deref! { match &(varIn.clone()) {
...
pub(crate) fn replaceEventInfo(mut eInfoIn: BackendDAE::EventInfo, mut inVariableReplacements: VariableReplacements, mut inFuncTypeExpExpToBooleanOption: Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<bool> + 'static>>) -> Result<BackendDAE::EventInfo> {
