...
//
// =============================================================================
pub(crate) fn calculateStateSetsJacobians(mut inDAE: Arc<BackendDAE::BackendDAE>) -> Result<Arc<BackendDAE::BackendDAE>> {
    let mut outDAE: Arc<BackendDAE::BackendDAE>;
    outDAE = BackendDAEUtil::mapEqSystem(inDAE, (std::sync::Arc::new(calculateEqSystemStateSetsJacobians) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::EqSystem>, Arc<BackendDAE::Shared>) -> Result<(Arc<BackendDAE::EqSystem>, Arc<BackendDAE::Shared>)> + 'static>))?;
    Ok(outDAE)
}
...
// =============================================================================
pub(crate) fn calculateStrongComponentJacobians(mut inDAE: Arc<BackendDAE::BackendDAE>) -> Arc<BackendDAE::BackendDAE> {
    let mut outDAE: Arc<BackendDAE::BackendDAE>;
    match '__try0: {
...
// =============================================================================
pub(crate) fn constantLinearSystem(mut inDAE: Arc<BackendDAE::BackendDAE>) -> Result<Arc<BackendDAE::BackendDAE>> {
    let mut outDAE: Arc<BackendDAE::BackendDAE>;
    (outDAE, _) = BackendDAEUtil::mapEqSystemAndFold(inDAE, (std::sync::Arc::new(constantLinearSystem0) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::EqSystem>, Arc<BackendDAE::Shared>, (bool, i32)) -> Result<(Arc<BackendDAE::EqSystem>, Arc<BackendDAE::Shared>, (bool, i32))> + 'static>), (false, 1))?;
    Ok(outDAE)
}
...
fn detectSparsePatternODE(mut inBackendDAE: Arc<BackendDAE::BackendDAE>) -> Result<Arc<BackendDAE::BackendDAE>> {
    ...
    let mut coloredCols: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>;
...
// =============================================================================
pub(crate) fn symbolicJacobianDAE(mut inBackendDAE: Arc<BackendDAE::BackendDAE>) -> Result<Arc<BackendDAE::BackendDAE>> {
    let mut outBackendDAE: Arc<BackendDAE::BackendDAE>;
    let mut DAE: Arc<BackendDAE::BackendDAE>;
    let mut eqs: Arc<metamodelica::List<Arc<BackendDAE::EqSystem>>>;
    let mut shared: Arc<BackendDAE::Shared>;
    ...
    let mut inDepVars: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut depVars: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut v: BackendDAE::Variables;
    ...
    let mut symjac: Option<(Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>;
...
fn generateSymbolicJacobianPast(mut inBackendDAE: Arc<BackendDAE::BackendDAE>) -> Result<Arc<BackendDAE::BackendDAE>> {
    ...
    let mut eqs: Arc<metamodelica::List<Arc<BackendDAE::EqSystem>>>;
    let mut shared: Arc<BackendDAE::Shared>;
    let mut symJacA: Option<(Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>;
    let mut sparsePattern: (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32);
    let mut sparseColoring: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>;
    let mut nonlinearPattern: (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32);
    let mut funcs: Arc<AvlTreePathFunction::Tree>;
    ...
    functionTree = AvlTreePathFunction::join(functionTree, funcs, (std::sync::Arc::new(fnptr!(AvlTreePathFunction::addConflictDefault, _, _, _)) as std::sync::Arc<dyn ::std::ops::Fn(_, _, _) -> Result<_> + 'static>))?;
    shared = BackendDAEUtil::setSharedFunctionTree(shared, functionTree)?;
    outBackendDAE = Arc::new(BackendDAE::BackendDAE { eqs: eqs, shared: shared });
...
fn createSymbolicJacobianforStates(mut inBackendDAE: Arc<BackendDAE::BackendDAE>) -> Result<(Option<(Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>, Arc<AvlTreePathFunction::Tree>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32), Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32))> {
    let mut outJacobian: Option<(Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>;
    ...
    let mut outSparseColoring: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>;
    ...
    let mut knvarlst: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut states: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut inputvars: Arc<metamodelica::List<BackendDAE::Var>>;
    ...
    backendDAE2 = BackendDAEUtil::transformBackendDAE(backendDAE2, Some((openmodelica_backend_types::BackendDAE::IndexReduction::NO_INDEX_REDUCTION, openmodelica_backend_types::BackendDAE::EquationConstraints::EXACT)), None, None)?;
    ...
    paramvars = List::select(knvarlst, (std::sync::Arc::new(fnptr!(BackendVariable::isParam, BackendDAE::Var)) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Var) -> Result<bool> + 'static>))?;
    if Flags::isSet(Flags::JAC_DUMP2.clone())? {
    ...
    (outJacobian, outFunctionTree, outSparsePattern, outSparseColoring, outNonlinearPattern) = generateGenericJacobian(backendDAE2, states.clone(), BackendVariable::listVar1(states.clone())?, BackendVariable::listVar1(inputvars)?, BackendVariable::listVar1(paramvars)?, BackendVariable::listVar1(states)?, varlst, (literal!("A")).clone(), false, false)?;
...
// =============================================================================
pub(crate) fn generateSymbolicSensitivities(mut inBackendDAE: Arc<BackendDAE::BackendDAE>) -> Result<Arc<BackendDAE::BackendDAE>> {
    let mut outBackendDAE: Arc<BackendDAE::BackendDAE>;
    let mut eqs: Arc<metamodelica::List<Arc<BackendDAE::EqSystem>>>;
    let mut shared: Arc<BackendDAE::Shared>;
    let mut symJacS: Option<(Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>;
    let mut sparsePattern: (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32);
    let mut sparseColoring: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>;
    let mut nonlinearPattern: (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32);
    let mut funcs: Arc<AvlTreePathFunction::Tree>;
...
fn createSymbolicJacobianforParameters(mut inBackendDAE: Arc<BackendDAE::BackendDAE>) -> Result<(Option<(Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>, Arc<AvlTreePathFunction::Tree>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32), Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32))> {
    ...
    (outJacobian, outFunctionTree, outSparsePattern, outSparseColoring, outNonlinearPattern) = generateGenericJacobian(backendDAE2, paramvars, BackendVariable::listVar1(states.clone())?, BackendVariable::listVar1(inputvars)?, BackendVariable::listVar1(states.clone())?, BackendVariable::listVar1(states)?, varlst, (literal!("S")).clone(), false, false)?;
    Ok((outJacobian, outFunctionTree, outSparsePattern, outSparseColoring, outNonlinearPattern))
}
...
//
// =============================================================================
pub(crate) fn generateSymbolicLinearizationPast(mut inBackendDAE: Arc<BackendDAE::BackendDAE>) -> Arc<BackendDAE::BackendDAE> {
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                _ => {
                    let mut eqs: Arc<metamodelica::List<Arc<BackendDAE::EqSystem>>>;
                    let mut shared: Arc<BackendDAE::Shared>;
                    let mut linearModelMatrices: Arc<metamodelica::List<(Option<(Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32), Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32))>>;
                    let mut funcs: Arc<AvlTreePathFunction::Tree>;
                    let mut functionTree: Arc<AvlTreePathFunction::Tree>;
...
// =============================================================================
pub(crate) fn inputDerivativesUsed(mut inDAE: Arc<BackendDAE::BackendDAE>) -> Result<Arc<BackendDAE::BackendDAE>> {
    let mut outDAE: Arc<BackendDAE::BackendDAE>;
    (outDAE, _) = BackendDAEUtil::mapEqSystemAndFold(inDAE, (std::sync::Arc::new(inputDerivativesUsedWork) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::EqSystem>, Arc<BackendDAE::Shared>, bool) -> Result<(Arc<BackendDAE::EqSystem>, Arc<BackendDAE::Shared>, bool)> + 'static>), false)?;
    Ok(outDAE)
}
...
fn traverserinputDerivativesUsed(mut inExp: Arc<DAE::Exp>, mut itpl: (BackendDAE::Variables, Arc<metamodelica::List<Arc<DAE::Exp>>>)) -> Result<(Arc<DAE::Exp>, (BackendDAE::Variables, Arc<metamodelica::List<Arc<DAE::Exp>>>))> {
    ...
    let mut tpl: (BackendDAE::Variables, Arc<metamodelica::List<Arc<DAE::Exp>>>);
    (e, tpl) = Expression::traverseExpTopDown(inExp, (std::sync::Arc::new(fnptr!(traverserExpinputDerivativesUsed, Arc<DAE::Exp>, (BackendDAE::Variables, Arc<metamodelica::List<Arc<DAE::Exp>>>))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (BackendDAE::Variables, Arc<metamodelica::List<Arc<DAE::Exp>>>)) -> Result<(Arc<DAE::Exp>, bool, (BackendDAE::Variables, Arc<metamodelica::List<Arc<DAE::Exp>>>))> + 'static>), itpl)?;
    Ok((e, tpl))
}

fn traverserExpinputDerivativesUsed(mut inExp: Arc<DAE::Exp>, mut tpl: (BackendDAE::Variables, Arc<metamodelica::List<Arc<DAE::Exp>>>)) -> (Arc<DAE::Exp>, bool, (BackendDAE::Variables, Arc<metamodelica::List<Arc<DAE::Exp>>>)) {
...
fn jacobianIsConstant(mut jac: Arc<metamodelica::List<(i32, i32, Arc<BackendDAE::Equation>)>>) -> Result<bool> {
    ...
    eqs = List::map(jac, std::sync::Arc::new(fnptr!(Util::tuple33, _)))?;
    isConst = !(List::any(eqs, (std::sync::Arc::new(fnptr!(variableResidual, Arc<BackendDAE::Equation>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::Equation>) -> Result<bool> + 'static>))?);
    Ok(isConst)
}
...
fn replaceStrongComponent(mut systIn: Arc<BackendDAE::EqSystem>, mut idx: i32, mut compsNew: Arc<metamodelica::List<Arc<BackendDAE::StrongComponent>>>, mut compsAdd: Arc<metamodelica::List<Arc<BackendDAE::StrongComponent>>>) -> Result<Arc<BackendDAE::EqSystem>> {
    ...
    if !(compsAdd.clone().is_empty()) {
        ...
        List::map2_0(compsAdd.clone(), (std::sync::Arc::new(fnptr!(updateAssignment, Arc<BackendDAE::StrongComponent>, metamodelica::Array<i32>, metamodelica::Array<i32>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::StrongComponent>, metamodelica::Array<i32>, metamodelica::Array<i32>) -> Result<()> + 'static>), ass1.clone(), ass2.clone())?;
    }
    List::map2_0(compsNew.clone(), (std::sync::Arc::new(fnptr!(updateAssignment, Arc<BackendDAE::StrongComponent>, metamodelica::Array<i32>, metamodelica::Array<i32>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::StrongComponent>, metamodelica::Array<i32>, metamodelica::Array<i32>) -> Result<()> + 'static>), ass1.clone(), ass2.clone())?;
    comps = List::replaceAtWithList(compsNew, idx - 1, comps)?;
    assign_field!(systOut.matching = Arc::new(BackendDAE::Matching::MATCHING { ass1: ass1.clone(), ass2: ass2.clone(), comps: listAppend(comps, compsAdd) }));
    systOut = BackendDAEUtil::setEqSystMatrices(systOut, None, None, None)?;
    Ok(systOut)
}

fn updateAssignment(mut comp: Arc<BackendDAE::StrongComponent>, mut ass1: metamodelica::Array<i32>, mut ass2: metamodelica::Array<i32>) -> () {
    let () = 'mc: {
        let __mc_input = comp;
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
fn solveConstJacLinearSystem(mut syst: Arc<BackendDAE::EqSystem>, mut ishared: Arc<BackendDAE::Shared>, mut eqn_lst: Arc<metamodelica::List<Arc<BackendDAE::Equation>>>, mut eqn_indxs: Arc<metamodelica::List<i32>>, mut var_lst: Arc<metamodelica::List<BackendDAE::Var>>, mut var_indxs: Arc<metamodelica::List<i32>>, mut jac: Arc<metamodelica::List<(i32, i32, Arc<BackendDAE::Equation>)>>, mut sysIdxIn: i32, mut compIdxIn: i32) -> Result<(Arc<metamodelica::List<Arc<BackendDAE::Equation>>>, Arc<metamodelica::List<Arc<BackendDAE::Equation>>>, Arc<metamodelica::List<BackendDAE::Var>>, metamodelica::Array<i32>, i32)> {
    let mut sysEqsOut: Arc<metamodelica::List<Arc<BackendDAE::Equation>>>;
    let mut bEqsOut: Arc<metamodelica::List<Arc<BackendDAE::Equation>>>;
    let mut bVarsOut: Arc<metamodelica::List<BackendDAE::Var>>;
    ...
    let mut sources: Arc<metamodelica::List<Arc<DAE::ElementSource>>>;
    let mut matching: Arc<BackendDAE::Matching>;
    let mut funcs: Arc<AvlTreePathFunction::Tree>;
    ...
    let (__pa0, __pa1, __pa2, __pa3, __pa4) = ::match_deref::match_deref! { match &(syst) {
    ...
    let __pa5 = ::match_deref::match_deref! { match &(ishared) {
    ...
    beqs = beqs.reverse();
    A = evaluateConstantJacobianArray((var_lst.clone().len() as i32), jac)?;
...
fn createSysEquations(mut A: metamodelica::Array<metamodelica::Array<metamodelica::Real>>, mut b: metamodelica::Array<metamodelica::Real>, mut n: i32, mut order: metamodelica::Array<i32>, mut xVars: Arc<metamodelica::List<BackendDAE::Var>>, mut bVars: Arc<metamodelica::List<BackendDAE::Var>>) -> Result<Arc<metamodelica::List<Arc<BackendDAE::Equation>>>> {
    ...
    let mut coeffExps: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    let mut xExps: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    let mut bExps: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    let mut xProds: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    let mut bProds: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    let mut coeffs: Arc<metamodelica::List<metamodelica::Real>>;
    let mut eq: Arc<BackendDAE::Equation>;
    xExps = List::map(xVars, (std::sync::Arc::new(BackendVariable::varExp2) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Var) -> Result<Arc<DAE::Exp>> + 'static>))?;
    bExps = List::map(bVars, (std::sync::Arc::new(BackendVariable::varExp2) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Var) -> Result<Arc<DAE::Exp>> + 'static>))?;
    for mut i in 1..=n {
        row = metamodelica::arrayGet(order.clone(), i)?;
        coeffs = Arc::new(({let __elt = A.borrow()[(row-1) as usize].clone(); __elt}).borrow().iter().cloned().collect::<metamodelica::List<_>>());
...
pub(crate) fn makeBinaryExp(mut inLhs: Arc<DAE::Exp>, mut inRhs: Arc<DAE::Exp>, mut inOp: DAE::Operator) -> Arc<DAE::Exp> {
    let mut outExp: Arc<DAE::Exp>;
...
fn createBVecVars(mut sysIdx: i32, mut compIdx: i32, mut size: i32, mut typ: Arc<DAE::Type>, mut bExps: Arc<metamodelica::List<Arc<DAE::Exp>>>) -> Result<(Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<BackendDAE::Equation>>>)> {
    ...
    let mut var: BackendDAE::Var;
    let mut beq: Arc<BackendDAE::Equation>;
    for mut i in 1..=size {
        ident = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("$sys")); __mm_s.push_str(&*intString(sysIdx)); __mm_s.push_str(&*literal!("_")); __mm_s.push_str(&*intString(compIdx)); __mm_s.push_str(&*literal!("_b")); __mm_s.push_str(&*intString(i)); ArcStr::from(__mm_s) }).clone();
        ...
        beq = Arc::new(BackendDAE::Equation::EQUATION { exp: (bExps.clone()).get(i)?, scalar: Expression::crefExp(cref.clone())?, source: DAE::emptyElementSource().clone(), attr: BackendDAE::EQ_ATTR_DEFAULT_DYNAMIC.clone() });
        eqLst = metamodelica::cons(beq.clone(), eqLst.clone());
    }
    Ok((varLst, eqLst))
}

fn gauss(mut A: metamodelica::Array<metamodelica::Array<metamodelica::Real>>, mut b: metamodelica::Array<metamodelica::Real>, mut indxIn: i32, mut n: i32, mut rangeIn: Arc<metamodelica::List<i32>>, mut permutation: metamodelica::Array<i32>) -> () {
    ...
    let () = 'mc: {
        ...
        if let Ok((__v, __wb0, __wb1, __wb2, __wb3, __wb4, __wb5, __wb6)) = (|| -> Result<_> {
            ...
            let true = (intLe(indxIn, n)) else { bail!("pattern mismatch") };
            (pivotIdx, pivot) = getPivotElement(A.clone(), rangeIn.clone(), indxIn, n)?;
            metamodelica::arrayUpdate(permutation.clone(), indxIn, pivotIdx)?;
            (range, _) = List::deleteMemberOnTrue(pivotIdx, rangeIn.clone(), (std::sync::Arc::new(fnptr!(intEq, i32, i32)) as std::sync::Arc<dyn ::std::ops::Fn(i32, i32) -> Result<bool> + 'static>))?;
            for mut ic in indxIn..=n {
                entry = metamodelica::arrayGet(({let __elt = A.borrow()[(pivotIdx-1) as usize].clone(); __elt}), ic)?;
                entry = realDiv(entry, pivot);
                metamodelica::arrayUpdate(({let __elt = A.borrow()[(pivotIdx-1) as usize].clone(); __elt}), ic, entry)?;
            }
            for mut ic in 1..=n {
                pos = (pivotIdx - 1) * n + ic;
            ...
            for mut ir in &*range.clone() {
                let mut ir = ir.clone();
                first = metamodelica::arrayGet(({let __elt = A.borrow()[(ir-1) as usize].clone(); __elt}), indxIn)?;
                for mut ic in indxIn..=n {
                    pos = (ir - 1) * n + ic;
                    entry = metamodelica::arrayGet(({let __elt = A.borrow()[(ir-1) as usize].clone(); __elt}), ic)?;
                    pivot = metamodelica::arrayGet(({let __elt = A.borrow()[(pivotIdx-1) as usize].clone(); __elt}), ic)?;
                    entry = (entry) - ((first) * (pivot));
                    metamodelica::arrayUpdate(({let __elt = A.borrow()[(ir-1) as usize].clone(); __elt}), ic, entry)?;
                    b_entry = metamodelica::arrayGet(b.clone(), pos)?;
            ...
            gauss(A.clone(), b.clone(), indxIn + 1, n, range.clone(), permutation.clone());
...
fn getPivotElement(mut A: metamodelica::Array<metamodelica::Array<metamodelica::Real>>, mut rangeIn: Arc<metamodelica::List<i32>>, mut startIdx: i32, mut n: i32) -> Result<(i32, metamodelica::Real)> {
    ...
    for mut i in &*rangeIn {
        let mut i = i.clone();
        entry = metamodelica::arrayGet(({let __elt = A.borrow()[(i-1) as usize].clone(); __elt}), startIdx)?;
...
fn rListStr(mut l: Arc<metamodelica::List<metamodelica::Real>>) -> Result<ArcStr> {
    let mut s: ArcStr;
    s = stringDelimitList(List::map(l, (std::sync::Arc::new(fnptr!(realString, metamodelica::Real)) as std::sync::Arc<dyn ::std::ops::Fn(metamodelica::Real) -> Result<ArcStr> + 'static>))?, (literal!(" , ")).clone());
    Ok(s)
}
...
fn constantLinearSystem1(mut isyst: Arc<BackendDAE::EqSystem>, mut ishared: Arc<BackendDAE::Shared>, mut inComps: Arc<metamodelica::List<Arc<BackendDAE::StrongComponent>>>, mut inRunMatching: bool, mut sysIdxIn: i32, mut compIdxIn: i32) -> (Arc<BackendDAE::EqSystem>, Arc<BackendDAE::Shared>, bool, i32) {
    let mut osyst: Arc<BackendDAE::EqSystem>;
    let mut oshared: Arc<BackendDAE::Shared>;
    let mut runMatching: bool = false;
    let mut sysIdxOut: i32;
    (osyst, oshared, runMatching, sysIdxOut) = (::match_deref::match_deref! { match &(inComps) {
        Deref @ metamodelica::List::Nil => {
            (isyst, ishared, inRunMatching, sysIdxIn)
        },
        Deref @ metamodelica::List::Cons { head: comp, tail: comps } => {
            ...
            let mut sysIdx: i32;
            let mut compIdx: i32;
            (syst, shared, b, sysIdx, compIdx) = constantLinearSystemWork(isyst, ishared, comp.clone(), sysIdxIn, compIdxIn);
            (syst, shared, runMatching, sysIdx) = constantLinearSystem1(syst, shared, comps.clone(), b || inRunMatching, sysIdx, compIdx);
            (syst, shared, runMatching, sysIdx)
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn constantLinearSystemWork(mut isyst: Arc<BackendDAE::EqSystem>, mut ishared: Arc<BackendDAE::Shared>, mut comp: Arc<BackendDAE::StrongComponent>, mut sysIdxIn: i32, mut compIdxIn: i32) -> (Arc<BackendDAE::EqSystem>, Arc<BackendDAE::Shared>, bool, i32, i32) {
    let mut osyst: Arc<BackendDAE::EqSystem>;
    let mut oshared: Arc<BackendDAE::Shared>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (syst @ Deref @ BackendDAE::EqSystem { orderedVars: vars, orderedEqs: eqns, .. }, shared, Deref @ BackendDAE::StrongComponent::EQUATIONSYSTEM { eqns: eindex, vars: vindx, jac: Deref @ BackendDAE::Jacobian::FULL_JACOBIAN { jacobian: Some(jac) }, jacType: BackendDAE::JacobianType::JAC_LINEAR { .. }, .. }) => {
                    ...
                    let mut sysIdx: i32;
                    let mut order: metamodelica::Array<i32>;
                    let mut bVarIdcs: Arc<metamodelica::List<i32>>;
                    let mut bEqIdcs: Arc<metamodelica::List<i32>>;
                    let mut bVars: Arc<metamodelica::List<BackendDAE::Var>>;
                    let mut bEqs: Arc<metamodelica::List<Arc<BackendDAE::Equation>>>;
                    let mut sysEqs: Arc<metamodelica::List<Arc<BackendDAE::Equation>>>;
                    let mut bComps: Arc<metamodelica::List<Arc<BackendDAE::StrongComponent>>>;
                    let mut sysComps: Arc<metamodelica::List<Arc<BackendDAE::StrongComponent>>>;
                    let mut syst = (*syst).clone();
                    let mut eqns = (*eqns).clone();
                    ...
                    eqn_lst = BackendEquation::getList(eindex.clone(), eqns.clone())?;
                    var_lst = List::map1r(vindx.clone(), (std::sync::Arc::new(BackendVariable::getVarAt) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Variables, i32) -> Result<BackendDAE::Var> + 'static>), vars.clone())?;
                    (sysEqs, bEqs, bVars, order, sysIdx) = solveConstJacLinearSystem(syst.clone(), shared.clone(), eqn_lst.clone(), eindex.clone(), var_lst.clone().reverse(), vindx.clone(), jac.clone(), sysIdxIn, compIdxIn)?;
                    ...
                    Ok((syst.clone(), ishared.clone(), false, sysIdx.clone(), compIdxIn + (sysComps.clone().len() as i32)))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
...
fn solveLinearSystem(mut inSyst: Arc<BackendDAE::EqSystem>, mut ishared: Arc<BackendDAE::Shared>, mut eqn_lst: Arc<metamodelica::List<Arc<BackendDAE::Equation>>>, mut eqn_indxs: Arc<metamodelica::List<i32>>, mut var_lst: Arc<metamodelica::List<BackendDAE::Var>>, mut var_indxs: Arc<metamodelica::List<i32>>, mut jac: Arc<metamodelica::List<(i32, i32, Arc<BackendDAE::Equation>)>>) -> Result<(Arc<BackendDAE::EqSystem>, Arc<BackendDAE::Shared>)> {
    ...
    (osyst, oshared) = (::match_deref::match_deref! { match &((inSyst, ishared.clone())) {
        (syst @ Deref @ BackendDAE::EqSystem { .. }, Deref @ BackendDAE::Shared { functionTree: funcs, .. }) => {
            ...
            let mut beqs: Arc<metamodelica::List<Arc<DAE::Exp>>>;
            let mut sources: Arc<metamodelica::List<Arc<DAE::ElementSource>>>;
            let mut rhsVals: Arc<metamodelica::List<metamodelica::Real>>;
            let mut solvedVals: Arc<metamodelica::List<metamodelica::Real>>;
            let mut jacVals: Arc<metamodelica::List<Arc<metamodelica::List<metamodelica::Real>>>>;
            let mut linInfo: i32;
            ...
            beqs = beqs.reverse();
            rhsVals = ValuesUtil::valueReals(List::map(beqs, (std::sync::Arc::new(Ceval::cevalSimple) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<Arc<Values::Value>> + 'static>))?);
            jacVals = evaluateConstantJacobian((var_lst.clone().len() as i32), jac)?;
            ...
            sources = List::map1(sources, (std::sync::Arc::new(ElementSource::addSymbolicTransformation) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::ElementSource>, Arc<DAE::SymbolicOperation>) -> Result<Arc<DAE::ElementSource>> + 'static>), Arc::new(DAE::SymbolicOperation::LINEAR_SOLVED { vars: names, jac: jacVals, rhs: rhsVals, result: solvedVals.clone() }))?;
            ...
            assign_field!(
                syst.orderedVars = v,
                syst.orderedEqs = List::fold(eqn_indxs, (std::sync::Arc::new(BackendEquation::delete) as std::sync::Arc<dyn ::std::ops::Fn(i32, Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>) -> Result<Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>> + 'static>), eqns)?
            );
...
fn changeConstantLinearSystemVars(mut inVarLst: Arc<metamodelica::List<BackendDAE::Var>>, mut inSolvedVals: Arc<metamodelica::List<metamodelica::Real>>, mut inSources: Arc<metamodelica::List<Arc<DAE::ElementSource>>>, mut var_indxs: Arc<metamodelica::List<i32>>, mut inVars: BackendDAE::Variables, mut ieqns: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, mut ishared: Arc<BackendDAE::Shared>) -> Result<(BackendDAE::Variables, Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, Arc<BackendDAE::Shared>)> {
    '__tco: loop {
        ...
        (Deref @ metamodelica::List::Cons { head: BackendDAE::Var { varName: cref, varKind: BackendDAE::VarKind::STATE { .. }, varType: tp, .. }, tail: varlst }, Deref @ metamodelica::List::Cons { head: r, tail: rlst }, Deref @ metamodelica::List::Cons { head: _, tail: slst }, Deref @ metamodelica::List::Cons { head: _, tail: vindxs }, vars, eqns) => {
            ...
            eqns = BackendEquation::add(Arc::new(BackendDAE::Equation::EQUATION { exp: e, scalar: Arc::new(DAE::Exp::RCONST { real: r.clone() }), source: DAE::emptyElementSource().clone(), attr: BackendDAE::EQ_ATTR_DEFAULT_UNKNOWN.clone() }), eqns.clone())?;
        ...
        (Deref @ metamodelica::List::Cons { head: v, tail: varlst }, Deref @ metamodelica::List::Cons { head: r, tail: rlst }, Deref @ metamodelica::List::Cons { head: _, tail: slst }, Deref @ metamodelica::List::Cons { head: indx, tail: vindxs }, vars, eqns) => {
            ...
            v1 = BackendVariable::setVarStartValue(v1, Arc::new(DAE::Exp::RCONST { real: r.clone() }))?;
...
pub(crate) fn evaluateConstantJacobian(mut size: i32, mut jac: Arc<metamodelica::List<(i32, i32, Arc<BackendDAE::Equation>)>>) -> Result<Arc<metamodelica::List<Arc<metamodelica::List<metamodelica::Real>>>>> {
    let mut vals: Arc<metamodelica::List<Arc<metamodelica::List<metamodelica::Real>>>>;
    let mut valarr: metamodelica::Array<metamodelica::Array<metamodelica::Real>>;
    let mut tmp2: Arc<metamodelica::List<metamodelica::Array<metamodelica::Real>>>;
    valarr = evaluateConstantJacobianArray(size, jac)?;
    tmp2 = Arc::new(valarr.clone().borrow().iter().cloned().collect::<metamodelica::List<_>>());
    vals = List::map(tmp2, Arc::new(fnptr!(arrayList, metamodelica::Array<metamodelica::Real>)))?;
    Ok(vals)
}
...
fn evaluateConstantJacobianArray(mut size: i32, mut jac: Arc<metamodelica::List<(i32, i32, Arc<BackendDAE::Equation>)>>) -> Result<metamodelica::Array<metamodelica::Array<metamodelica::Real>>> {
    ...
    tmp2 = List::map(List::fill(tmp.clone(), size), Arc::new(fnptr!(arrayCopy, metamodelica::Array<metamodelica::Real>)))?;
    valarr = metamodelica::arrayFromVec(tmp2.into_iter().cloned().collect());
    List::map1_0(jac, (std::sync::Arc::new(evaluateConstantJacobian2) as std::sync::Arc<dyn ::std::ops::Fn((i32, i32, Arc<BackendDAE::Equation>), metamodelica::Array<metamodelica::Array<metamodelica::Real>>) -> Result<()> + 'static>), valarr.clone())?;
    Ok(valarr)
}
...
fn checkLinearSystem(mut info: i32, mut vars: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, mut jac: Arc<metamodelica::List<Arc<metamodelica::List<metamodelica::Real>>>>, mut rhs: Arc<metamodelica::List<metamodelica::Real>>, mut eqnlst: Arc<metamodelica::List<Arc<BackendDAE::Equation>>>) -> Result<()> {
    let () = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ...
            let true = (info > 0) else { bail!("pattern mismatch") };
            varname = (ComponentReferenceBasics::printComponentRefStr((vars.clone()).get(info)?)?).clone();
            infoStr = (intString(info)).clone();
...
pub(crate) fn generateSparsePattern(mut inBackendDAE: Arc<BackendDAE::BackendDAE>, mut inIndependentVars: Arc<metamodelica::List<BackendDAE::Var>>, mut inDependentVars: Arc<metamodelica::List<BackendDAE::Var>>, mut nonlinearPattern: bool) -> Result<((Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32), Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>)> {
    ...
    let mut outColoredCols: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>;
    let debug: bool = false;
    let mut patternName: ArcStr = if (nonlinearPattern) {literal!("Nonlinear")} else {literal!("Sparsity")};
    (outSparsePattern, outColoredCols) = 'mc: {
        let __mc_input = (inBackendDAE.clone(), inIndependentVars, inDependentVars);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ BackendDAE::BackendDAE { eqs: Deref @ metamodelica::List::Cons { head: syst @ Deref @ BackendDAE::EqSystem { matching: bdaeMatching @ Deref @ BackendDAE::Matching::MATCHING { comps, ass1, .. }, .. }, tail: Deref @ metamodelica::List::Nil }, .. }, independentVars, dependentVars) => {
                    ...
                    let mut nodesEqnsIndex: Arc<metamodelica::List<i32>>;
                    let mut sparsepattern: Arc<metamodelica::List<Arc<metamodelica::List<i32>>>>;
                    let mut sparsepatternT: Arc<metamodelica::List<Arc<metamodelica::List<i32>>>>;
                    let mut jacDiffVars: Arc<metamodelica::List<BackendDAE::Var>>;
                    ...
                    let mut depCompRefsLst: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
                    let mut inDepCompRefsLst: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
                    let mut depCompRefs: metamodelica::Array<Arc<DAE::ComponentRef>>;
                    let mut inDepCompRefs: metamodelica::Array<Arc<DAE::ComponentRef>>;
                    let mut eqnSparse: metamodelica::Array<Arc<metamodelica::List<i32>>>;
                    let mut varSparse: metamodelica::Array<Arc<metamodelica::List<i32>>>;
                    let mut sparseArray: metamodelica::Array<Arc<metamodelica::List<i32>>>;
                    let mut sparseArrayT: metamodelica::Array<Arc<metamodelica::List<i32>>>;
                    let mut mark: metamodelica::Array<i32>;
                    let mut usedvar: metamodelica::Array<i32>;
                    let mut coloring: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>;
                    let mut translated: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>;
                    let mut sparsetuple: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>;
                    let mut sparsetupleT: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>;
        ...
        __acc.reverse()
                    ...
                    if nonlinearPattern || Flags::isSet(Flags::DISABLE_COLORING.clone())? {
                        coloring = ({
        let mut __acc: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>> = metamodelica::nil();
...
fn getSparsePattern(mut inComponents: Arc<metamodelica::List<Arc<BackendDAE::StrongComponent>>>, mut ineqnSparse: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut invarSparse: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut inMark: metamodelica::Array<i32>, mut inUsed: metamodelica::Array<i32>, mut inmarkValue: i32, mut inMatrix: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut inMatrixT: metamodelica::Array<Arc<metamodelica::List<i32>>>) -> Result<metamodelica::Array<Arc<metamodelica::List<i32>>>> {
    '__tco: loop {
        ...
        (Deref @ metamodelica::List::Cons { head: Deref @ BackendDAE::StrongComponent::SINGLEEQUATION { eqn, var }, tail: rest }, result) => {
            let mut inputVars: Arc<metamodelica::List<i32>>;
            let mut result = (*result).clone();
            inputVars = metamodelica::arrayGet(inMatrix.clone(), eqn.clone())?;
            inputVars = List::removeOnTrue(var.clone(), (std::sync::Arc::new(fnptr!(intEq, i32, i32)) as std::sync::Arc<dyn ::std::ops::Fn(i32, i32) -> Result<bool> + 'static>), inputVars)?;
        ...
        __acc.reverse()
            ...
            getSparsePattern2(inputVars, solvedVars.clone(), list![eqn.clone()], ineqnSparse.clone(), invarSparse.clone(), inMark.clone(), inUsed.clone(), inmarkValue)?;
            { (inComponents, ineqnSparse, invarSparse, inMark, inUsed, inmarkValue, inMatrix, inMatrixT) = (rest.clone(), result.clone(), invarSparse.clone(), inMark.clone(), inUsed.clone(), inmarkValue + 1, inMatrix.clone(), inMatrixT.clone()); continue '__tco; }
        },
        ...
        (Deref @ metamodelica::List::Cons { head: Deref @ BackendDAE::StrongComponent::EQUATIONSYSTEM { eqns, vars: solvedVars, .. }, tail: rest }, result) => {
            let mut inputVars: Arc<metamodelica::List<i32>>;
            let mut inputVarsLst: Arc<metamodelica::List<Arc<metamodelica::List<i32>>>>;
            let mut result = (*result).clone();
        ...
        __acc.reverse()
            ...
            getSparsePattern2(inputVars, solvedVars.clone(), eqns.clone(), ineqnSparse.clone(), invarSparse.clone(), inMark.clone(), inUsed.clone(), inmarkValue)?;
            { (inComponents, ineqnSparse, invarSparse, inMark, inUsed, inmarkValue, inMatrix, inMatrixT) = (rest.clone(), result.clone(), invarSparse.clone(), inMark.clone(), inUsed.clone(), inmarkValue + 1, inMatrix.clone(), inMatrixT.clone()); continue '__tco; }
        },
        (Deref @ metamodelica::List::Cons { head: Deref @ BackendDAE::StrongComponent::TORNSYSTEM { strictTearingSet: BackendDAE::TearingSet { residualequations: eqns, tearingvars: vars, innerEquations, .. }, .. }, tail: rest }, result) => {
            ...
            let mut inputVars: Arc<metamodelica::List<i32>>;
            let mut inputVarsLst: Arc<metamodelica::List<Arc<metamodelica::List<i32>>>>;
            let mut solvedVars: Arc<metamodelica::List<i32>>;
            let mut result = (*result).clone();
        ...
        __acc.reverse()
            ...
            getSparsePattern2(inputVars, solvedVars.clone(), eqns1, ineqnSparse.clone(), invarSparse.clone(), inMark.clone(), inUsed.clone(), inmarkValue)?;
            { (inComponents, ineqnSparse, invarSparse, inMark, inUsed, inmarkValue, inMatrix, inMatrixT) = (rest.clone(), result.clone(), invarSparse.clone(), inMark.clone(), inUsed.clone(), inmarkValue + 1, inMatrix.clone(), inMatrixT.clone()); continue '__tco; }
        },
        _ => {
...
fn getSparsePattern2(mut inInputVars: Arc<metamodelica::List<i32>>, mut inSolvedVars: Arc<metamodelica::List<i32>>, mut inEqns: Arc<metamodelica::List<i32>>, mut ineqnSparse: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut invarSparse: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut inMark: metamodelica::Array<i32>, mut inUsed: metamodelica::Array<i32>, mut inmarkValue: i32) -> Result<()> {
    let mut localList: Arc<metamodelica::List<i32>>;
    localList = getSparsePatternHelp(inInputVars, invarSparse.clone(), inMark.clone(), inUsed.clone(), inmarkValue)?;
    List::map2_0(inSolvedVars, (std::sync::Arc::new(Array::updateIndexFirst) as std::sync::Arc<dyn ::std::ops::Fn(i32, _, _) -> Result<()> + 'static>), localList.clone(), invarSparse.clone())?;
    List::map2_0(inEqns, (std::sync::Arc::new(Array::updateIndexFirst) as std::sync::Arc<dyn ::std::ops::Fn(i32, _, _) -> Result<()> + 'static>), localList, ineqnSparse.clone())?;
    Ok(())
}
...
fn getSparsePatternHelp(mut inInputVars: Arc<metamodelica::List<i32>>, mut invarSparse: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut inMark: metamodelica::Array<i32>, mut inUsed: metamodelica::Array<i32>, mut inmarkValue: i32) -> Result<Arc<metamodelica::List<i32>>> {
    ...
    let mut varSparse: Arc<metamodelica::List<i32>>;
...
pub(crate) fn transposeSparsePattern(mut inSparsePattern: Arc<metamodelica::List<Arc<metamodelica::List<i32>>>>, mut inAccumList: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut inValue: i32) -> Result<metamodelica::Array<Arc<metamodelica::List<i32>>>> {
    ...
    for mut oneList in &*inSparsePattern {
        ...
        for mut oneElem in &*oneList.clone() {
            ...
            metamodelica::Dangerous::arrayUpdateNoBoundsChecking(outSparsePattern.clone(), oneElem.clone(), metamodelica::cons(value, tmplist.clone()));
        }
        value = value + 1;
    }
    Ok(outSparsePattern)
...
pub(crate) fn transposeSparsePatternTuple(mut inSparsePattern: Arc<metamodelica::List<(i32, Arc<metamodelica::List<i32>>)>>, mut inAccumList: metamodelica::Array<(i32, Arc<metamodelica::List<i32>>)>) -> Result<metamodelica::Array<(i32, Arc<metamodelica::List<i32>>)>> {
    let mut outSparsePattern: metamodelica::Array<(i32, Arc<metamodelica::List<i32>>)> = inAccumList.clone();
    let mut value: i32;
    let mut tmplist: Arc<metamodelica::List<i32>>;
    let mut oneList: Arc<metamodelica::List<i32>>;
    let mut tmpTuple: (i32, Arc<metamodelica::List<i32>>);
    let mut i: i32 = 0;
    for mut oneListTuple in &*inSparsePattern.clone() {
        ...
        for mut oneElem in &*oneList.clone() {
            ...
            tmplist = metamodelica::cons(value, tmplist.clone());
    ...
    for mut i in 1..=(inSparsePattern.len() as i32) {
        ...
        tmpTuple = (value, tmplist.clone());
        metamodelica::Dangerous::arrayUpdateNoBoundsChecking(outSparsePattern.clone(), i, tmpTuple.clone());
    }
    Ok(outSparsePattern)
}
...
pub fn createFMIModelDerivatives(mut inBackendDAE: Arc<BackendDAE::BackendDAE>) -> Result<(Arc<metamodelica::List<(Option<(Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32), Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32))>>, Arc<AvlTreePathFunction::Tree>)> {
    ...
    let mut eqSyst: Arc<BackendDAE::EqSystem>;
    let mut outJacobian: Option<(Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>;
    let mut varlst: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut knvarlst: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut states: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut inputvars: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut outputvars: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut paramvars: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut indepVars: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut depVars: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut v: BackendDAE::Variables;
...
pub(crate) fn createFMIModelDerivativesForInitialization(mut initDAE: Arc<BackendDAE::BackendDAE>, mut simDAE: Arc<BackendDAE::BackendDAE>, mut depVars: Arc<metamodelica::List<BackendDAE::Var>>, mut indepVars: Arc<metamodelica::List<BackendDAE::Var>>, mut orderedVars: BackendDAE::Variables, mut sparsePattern_: (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32), mut sparseColoring_: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>) -> Result<Arc<metamodelica::List<(Option<(Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32), Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32))>>> {
    ...
    let mut rhsCr: Arc<DAE::ComponentRef>;
    let mut crefsVarsToRemove: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
    let mut protectedCrefs: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
...
fn createLinearModelMatrices(mut inBackendDAE: Arc<BackendDAE::BackendDAE>, mut useOptimica: bool) -> Result<(Arc<metamodelica::List<(Option<(Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32), Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32))>>, Arc<AvlTreePathFunction::Tree>)> {
    let mut outJacobianMatrices: Arc<metamodelica::List<(Option<(Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32), Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32))>>;
    let mut outFunctionTree: Arc<AvlTreePathFunction::Tree>;
    (outJacobianMatrices, outFunctionTree) = (::match_deref::match_deref! { match &((inBackendDAE, useOptimica)) {
        (backendDAE, false) => {
            let mut backendDAE2: Arc<BackendDAE::BackendDAE>;
            let mut varlst: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut knvarlst: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut states: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut inputvars: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut inputvars2: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut outputvars: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut paramvars: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut v: BackendDAE::Variables;
            let mut globalKnownVars: BackendDAE::Variables;
            ...
            let mut linearModelMatrices: Arc<metamodelica::List<(Option<(Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32), Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32))>>;
            let mut linearModelMatrix: Option<(Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>;
            let mut sparsePattern: (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32);
            let mut sparseColoring: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>;
            let mut nonlinearPattern: (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32);
            let mut funcs: Arc<AvlTreePathFunction::Tree>;
            ...
            inputvars2 = List::select(knvarlst, (std::sync::Arc::new(fnptr!(BackendVariable::isVarOnTopLevelAndInput, BackendDAE::Var)) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Var) -> Result<bool> + 'static>))?;
            ...
            functionTree = AvlTreePathFunction::join(functionTree, funcs, (std::sync::Arc::new(fnptr!(AvlTreePathFunction::addConflictDefault, _, _, _)) as std::sync::Arc<dyn ::std::ops::Fn(_, _, _) -> Result<_> + 'static>))?;
            backendDAE2 = BackendDAEUtil::setFunctionTree(backendDAE2, functionTree.clone())?;
            linearModelMatrices = metamodelica::cons((linearModelMatrix, sparsePattern, sparseColoring, nonlinearPattern), linearModelMatrices);
            if Flags::isSet(Flags::JAC_DUMP2.clone())? {
            ...
            (linearModelMatrix, funcs, sparsePattern, sparseColoring, nonlinearPattern) = generateGenericJacobian(backendDAE2.clone(), states, statesarr.clone(), inputvarsarr.clone(), paramvarsarr.clone(), outputvarsarr.clone(), varlst.clone(), (literal!("C")).clone(), false, false)?;
            functionTree = AvlTreePathFunction::join(functionTree, funcs, (std::sync::Arc::new(fnptr!(AvlTreePathFunction::addConflictDefault, _, _, _)) as std::sync::Arc<dyn ::std::ops::Fn(_, _, _) -> Result<_> + 'static>))?;
            backendDAE2 = BackendDAEUtil::setFunctionTree(backendDAE2, functionTree.clone())?;
            linearModelMatrices = metamodelica::cons((linearModelMatrix, sparsePattern, sparseColoring, nonlinearPattern), linearModelMatrices);
            if Flags::isSet(Flags::JAC_DUMP2.clone())? {
            ...
            (linearModelMatrix, funcs, sparsePattern, sparseColoring, nonlinearPattern) = generateGenericJacobian(backendDAE2, inputvars2, statesarr, inputvarsarr, paramvarsarr, outputvarsarr, varlst, (literal!("D")).clone(), false, false)?;
            functionTree = AvlTreePathFunction::join(functionTree, funcs, (std::sync::Arc::new(fnptr!(AvlTreePathFunction::addConflictDefault, _, _, _)) as std::sync::Arc<dyn ::std::ops::Fn(_, _, _) -> Result<_> + 'static>))?;
            linearModelMatrices = metamodelica::cons((linearModelMatrix, sparsePattern, sparseColoring, nonlinearPattern), linearModelMatrices);
            if Flags::isSet(Flags::JAC_DUMP2.clone())? {
        ...
        (backendDAE, true) => {
            ...
            let mut outputvars: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut paramvars: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut states_inputs: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut conVarsList: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut fconVarsList: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut object: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut v: BackendDAE::Variables;
            let mut globalKnownVars: BackendDAE::Variables;
            let mut statesarr: BackendDAE::Variables;
            let mut inputvarsarr: BackendDAE::Variables;
            ...
            inputvars = List::select(knvarlst.clone(), (std::sync::Arc::new(fnptr!(BackendVariable::isInput, BackendDAE::Var)) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Var) -> Result<bool> + 'static>))?;
            paramvars = List::select(knvarlst.clone(), (std::sync::Arc::new(fnptr!(BackendVariable::isParam, BackendDAE::Var)) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Var) -> Result<bool> + 'static>))?;
            inputvars2 = List::select(knvarlst, (std::sync::Arc::new(fnptr!(BackendVariable::isVarOnTopLevelAndInputNoDerInput, BackendDAE::Var)) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Var) -> Result<bool> + 'static>))?;
            outputvars = List::select(varlst.clone(), (std::sync::Arc::new(fnptr!(BackendVariable::isVarOnTopLevelAndOutput, BackendDAE::Var)) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Var) -> Result<bool> + 'static>))?;
            ...
            (linearModelMatrix, functionTree, sparsePattern, sparseColoring, nonlinearPattern) = generateGenericJacobian(backendDAE2.clone(), states, statesarr.clone(), inputvarsarr.clone(), paramvarsarr.clone(), statesarr.clone(), varlst.clone(), (literal!("A")).clone(), false, false)?;
            backendDAE2 = BackendDAEUtil::setFunctionTree(backendDAE2, functionTree.clone())?;
            linearModelMatrices = list![(linearModelMatrix, sparsePattern, sparseColoring, nonlinearPattern)];
            if Flags::isSet(Flags::JAC_DUMP2.clone())? {
                metamodelica::print(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("analytical Jacobians -> generated system for matrix A time: ")); __mm_s.push_str(&*realString(clock())); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
            ...
            object = DynamicOptimization::checkObjectIsSet(outputvarsarr, (arcstr::literal!(BackendDAE::optimizationMayerTermName)).clone());
            optimizer_vars = BackendVariable::addVars(object, optimizer_vars)?;
            (linearModelMatrix, funcs, sparsePattern, sparseColoring, nonlinearPattern) = generateGenericJacobian(backendDAE2.clone(), states_inputs.clone(), statesarr.clone(), inputvarsarr.clone(), paramvarsarr.clone(), optimizer_vars, varlst.clone(), (literal!("C")).clone(), false, false)?;
            functionTree = AvlTreePathFunction::join(functionTree, funcs, (std::sync::Arc::new(fnptr!(AvlTreePathFunction::addConflictDefault, _, _, _)) as std::sync::Arc<dyn ::std::ops::Fn(_, _, _) -> Result<_> + 'static>))?;
            backendDAE2 = BackendDAEUtil::setFunctionTree(backendDAE2, functionTree.clone())?;
            linearModelMatrices = metamodelica::cons((linearModelMatrix, sparsePattern, sparseColoring, nonlinearPattern), linearModelMatrices);
            if Flags::isSet(Flags::JAC_DUMP2.clone())? {
            ...
            (linearModelMatrix, funcs, sparsePattern, sparseColoring, nonlinearPattern) = generateGenericJacobian(backendDAE2, states_inputs, statesarr, inputvarsarr, paramvarsarr, optimizer_vars, varlst, (literal!("D")).clone(), false, false)?;
            functionTree = AvlTreePathFunction::join(functionTree, funcs, (std::sync::Arc::new(fnptr!(AvlTreePathFunction::addConflictDefault, _, _, _)) as std::sync::Arc<dyn ::std::ops::Fn(_, _, _) -> Result<_> + 'static>))?;
            linearModelMatrices = metamodelica::cons((linearModelMatrix, sparsePattern, sparseColoring, nonlinearPattern), linearModelMatrices);
            if Flags::isSet(Flags::JAC_DUMP2.clone())? {
                metamodelica::print(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("analytical Jacobians -> generated system for matrix D time: ")); __mm_s.push_str(&*realString(clock())); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
...
fn generateGenericJacobian(mut inBackendDAE: Arc<BackendDAE::BackendDAE>, mut inDiffVars: Arc<metamodelica::List<BackendDAE::Var>>, mut inStateVars: BackendDAE::Variables, mut inInputVars: BackendDAE::Variables, mut inParameterVars: BackendDAE::Variables, mut inDifferentiatedVars: BackendDAE::Variables, mut inVars: Arc<metamodelica::List<BackendDAE::Var>>, mut inName: ArcStr, mut onlySparsePattern: bool, mut daeMode: bool) -> Result<(Option<(Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>, Arc<AvlTreePathFunction::Tree>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32), Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32))> {
    ...
    let mut jacDAE: Arc<BackendDAE::BackendDAE>;
    let mut jacDiffedVars: Arc<metamodelica::List<BackendDAE::Var>>;
    match '__try0: {
        outFunctionTree = shared.functionTree.clone();
        if !(onlySparsePattern) {
            (symbolicJacobian, outFunctionTree) = unwrap_break_err!(createJacobian(inBackendDAE.clone(), inDiffVars.clone(), inStateVars.clone(), inInputVars.clone(), inParameterVars.clone(), inDifferentiatedVars.clone(), inVars.clone(), (inName.clone()).clone(), daeMode), '__try0);
...
fn createJacobian(mut inBackendDAE: Arc<BackendDAE::BackendDAE>, mut inDiffVars: Arc<metamodelica::List<BackendDAE::Var>>, mut inStateVars: BackendDAE::Variables, mut inInputVars: BackendDAE::Variables, mut inParameterVars: BackendDAE::Variables, mut inDifferentiatedVars: BackendDAE::Variables, mut inVars: Arc<metamodelica::List<BackendDAE::Var>>, mut inName: ArcStr, mut daeMode: bool) -> Result<((Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), Arc<AvlTreePathFunction::Tree>)> {
    ...
    (outJacobian, outFunctionTree) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            let _ = __mc_input.clone() else { bail!("nomatch") };
            let mut backendDAE: Arc<BackendDAE::BackendDAE>;
            let mut reducedDAE: Arc<BackendDAE::BackendDAE>;
            let mut comref_vars: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
            let mut comref_differentiatedVars: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
            let mut dependencies: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
            let mut diffedVars: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut seedlst: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut indepVars: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut funcs: Arc<AvlTreePathFunction::Tree>;
            ...
            let (__pa0, __pa1) = ::match_deref::match_deref! { match &(generateSymbolicJacobian(reducedDAE.clone(), indepVars.clone(), inDifferentiatedVars.clone(), BackendVariable::listVar1(seedlst.clone())?, inStateVars.clone(), inInputVars.clone(), inParameterVars.clone(), (inName.clone()).clone(), daeMode)?) {
...
fn generateSymbolicJacobian(mut inBackendDAE: Arc<BackendDAE::BackendDAE>, mut inVars: Arc<metamodelica::List<BackendDAE::Var>>, mut inDiffedVars: BackendDAE::Variables, mut inSeedVars: BackendDAE::Variables, mut inStateVars: BackendDAE::Variables, mut inInputVars: BackendDAE::Variables, mut inParamVars: BackendDAE::Variables, mut inMatrixName: ArcStr, mut daeMode: bool) -> Result<(Arc<BackendDAE::BackendDAE>, Arc<AvlTreePathFunction::Tree>)> {
    let mut outJacobian: Arc<BackendDAE::BackendDAE>;
    let mut outFunctions: Arc<AvlTreePathFunction::Tree>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ BackendDAE::BackendDAE { shared: Deref @ BackendDAE::Shared { cache, graph, info: ei, functionTree: functions, .. }, .. }, Deref @ metamodelica::List::Nil, _, _) => {
                    let mut jacobian: Arc<BackendDAE::BackendDAE>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ BackendDAE::BackendDAE { eqs: Deref @ metamodelica::List::Cons { head: Deref @ BackendDAE::EqSystem { orderedVars, orderedEqs, matching: Deref @ BackendDAE::Matching::MATCHING { ass2, .. }, .. }, tail: Deref @ metamodelica::List::Nil }, shared: Deref @ BackendDAE::Shared { globalKnownVars, cache, graph, functionTree: functions, info: ei, .. } }, diffVars, diffedVars, matrixName) => {
                    let mut comref_diffvars: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
                    ...
                    let mut derivedVariables: Arc<metamodelica::List<BackendDAE::Var>>;
                    let mut eqns: Arc<metamodelica::List<Arc<BackendDAE::Equation>>>;
                    let mut derivedEquations: Arc<metamodelica::List<Arc<BackendDAE::Equation>>>;
                    let mut shared: Arc<BackendDAE::Shared>;
                    ...
                    (derivedEquations, functions) = deriveAll(eqns.clone(), Arc::new(ass2.clone().borrow().iter().cloned().collect::<metamodelica::List<_>>()), x.clone(), diffData.clone(), functions.clone(), daeMode)?;
                    if Flags::isSet(Flags::JAC_DUMP2.clone())? {
...
fn createAllDiffedVarsWork(mut inVars: Arc<metamodelica::List<BackendDAE::Var>>, mut inCref: Arc<DAE::ComponentRef>, mut inAllVars: BackendDAE::Variables, mut inIndex: i32, mut inMatrixName: ArcStr, mut iVars: Arc<metamodelica::List<BackendDAE::Var>>) -> Result<Arc<metamodelica::List<BackendDAE::Var>>> {
    '__tco: loop {
        ...
        (Deref @ metamodelica::List::Cons { head: v @ BackendDAE::Var { varName: currVar, varKind: BackendDAE::VarKind::STATE { .. }, .. }, tail: restVar }, cref, index) => {
            ...
            { (inVars, inCref, inAllVars, inIndex, inMatrixName, iVars) = (restVar.clone(), cref.clone(), inAllVars, index.clone(), (inMatrixName).clone(), metamodelica::cons(r1, iVars)); continue '__tco; }
        },
...
fn deriveAll(mut inEquations: Arc<metamodelica::List<Arc<BackendDAE::Equation>>>, mut ass2: Arc<metamodelica::List<i32>>, mut inDiffCref: Arc<DAE::ComponentRef>, mut inDiffData: BackendDAE::DifferentiateInputData, mut inFunctions: Arc<AvlTreePathFunction::Tree>, mut daeMode: bool) -> Result<(Arc<metamodelica::List<Arc<BackendDAE::Equation>>>, Arc<AvlTreePathFunction::Tree>)> {
    ...
    let mut currDerivedEquation: Arc<BackendDAE::Equation>;
    let mut tmpEquations: Arc<metamodelica::List<Arc<BackendDAE::Equation>>>;
    match '__try0: {
        ...
        for mut currEquation in &*inEquations.clone() {
            ...
            (currDerivedEquation, outFunctions) = unwrap_break_err!(Differentiate::differentiateEquation(currEquation.clone(), inDiffCref.clone(), inDiffData.clone(), BackendDAE::DifferentiationType::GENERIC_GRADIENT { daeMode: daeMode }, outFunctions.clone()), '__try0);
...
pub(crate) fn updateJacobianDependencies(mut jacobian: Arc<BackendDAE::Jacobian>) -> Result<Arc<BackendDAE::Jacobian>> {
    ...
    jacobian = (::match_deref::match_deref! { match &(jacobian.clone()) {
        jac @ Deref @ BackendDAE::Jacobian::GENERIC_JACOBIAN { .. } => {
            ...
            let mut shared: Arc<BackendDAE::Shared>;
            let mut name: ArcStr;
            let mut diffVars: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut diffedVars: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut allDiffedVars: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut dependencies: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
            ...
            assign_variant_field!(jac => BackendDAE::Jacobian::GENERIC_JACOBIAN; jacobian = Some((Arc::new(BackendDAE::BackendDAE { eqs: list![syst], shared: shared }), name, diffVars, diffedVars, allDiffedVars, dependencies)));
            jac.clone()
        },
        _ => {
            jacobian
...
pub(crate) fn calcJacobianDependencies(mut jacobian: (Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)) -> Result<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>> {
    let mut dependencies: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
    let mut systems: Arc<metamodelica::List<Arc<BackendDAE::EqSystem>>>;
    let mut shared: Arc<BackendDAE::Shared>;
    let mut syst: Arc<BackendDAE::EqSystem>;
    let (__pa0, __pa1) = ::match_deref::match_deref! { match &(jacobian) {
        (Deref @ BackendDAE::BackendDAE { eqs: __pa0, shared: __pa1 }, _, _, _, _, _) => (__pa0.clone(), __pa1.clone()),
        _ => bail!("pattern mismatch"),
    } };
    systems = __pa0.clone();
    shared = __pa1.clone();
    syst = listHead(systems)?;
...
pub(crate) fn getJacobianDependencies(mut jacobian: Arc<BackendDAE::Jacobian>) -> Result<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>> {
...
fn calculateEqSystemJacobians(mut inSyst: Arc<BackendDAE::EqSystem>, mut inShared: Arc<BackendDAE::Shared>) -> Result<(Arc<BackendDAE::EqSystem>, Arc<BackendDAE::Shared>)> {
    ...
    let mut outShared: Arc<BackendDAE::Shared>;
    (outSyst, outShared) = (::match_deref::match_deref! { match &((inSyst, inShared)) {
...
pub(crate) fn prepareTornStrongComponentData(mut inVars: BackendDAE::Variables, mut inEqns: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, mut inIterationvarsInts: Arc<metamodelica::List<i32>>, mut inResidualequations: Arc<metamodelica::List<i32>>, mut innerEquations: Arc<metamodelica::List<BackendDAE::InnerEquation>>, mut funcTree: Arc<AvlTreePathFunction::Tree>, mut name: ArcStr) -> Result<(BackendDAE::Variables, BackendDAE::Variables, BackendDAE::Variables, Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>)> {
    ...
    let mut iterationvars: Arc<metamodelica::List<BackendDAE::Var>>;
...
fn calculateTearingSetJacobian(mut inVars: BackendDAE::Variables, mut inEqns: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, mut inTearingSet: BackendDAE::TearingSet, mut inShared: Arc<BackendDAE::Shared>, mut isLinear: bool) -> Result<(Arc<BackendDAE::Jacobian>, Arc<BackendDAE::Shared>)> {
    ...
    match '__try0: {
        ...
        if !(isLinear || unwrap_break_err!(checkForSymbolicJacobian(unwrap_break_err!(BackendEquation::equationList(resEqns.clone()), '__try0), unwrap_break_err!(BackendEquation::equationList(oEqns.clone()), '__try0), (name.clone()).clone()), '__try0)) {
            onlySparsePattern = true;
        }
        (outJacobian, outShared) = unwrap_break_err!(getSymbolicJacobian(diffVars.clone(), resEqns.clone(), resVars.clone(), oEqns.clone(), oVars.clone(), inShared.clone(), inVars.clone(), (name.clone()).clone(), onlySparsePattern), '__try0);
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ BackendDAE::StrongComponent::TORNSYSTEM { strictTearingSet: strictTearingset, casualTearingSet: optCasualTearingSet, linear, mixedSystem } => {
                    let mut shared: Arc<BackendDAE::Shared>;
                    let mut jacobian: Arc<BackendDAE::Jacobian>;
                    let mut jacobianCausal: Arc<BackendDAE::Jacobian>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ BackendDAE::StrongComponent::EQUATIONSYSTEM { eqns: residualequations, vars: iterationvarsInts, mixedSystem, .. } => {
                    ...
                    (jacobian, shared) = getSymbolicJacobian(diffVars.clone(), eqns.clone(), resVars.clone(), oeqns.clone(), ovars.clone(), inShared.clone(), inVars.clone(), (name.clone()).clone(), onlySparsePattern)?;
...
fn checkNonLinDependecies(mut inComp: Arc<BackendDAE::StrongComponent>, mut inEqns: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>) -> Result<()> {
    ...
    if Flags::isSet(Flags::INITIALIZATION.clone())? {
        ...
        (::match_deref::match_deref! { match &(inComp) {
        ...
        _ => {
            ...
            printNonLinIterVarsAndEqs(jac.clone(), eqnIndices.clone(), inEqns)?;
            ()
        },
        ...
        (existNonLin, name) = (::match_deref::match_deref! { match &(inComp) {
        ...
        if existNonLin {
            ...
            Error::addMessage(Error::INITIALIZATION_ITERATION_VARIABLES.clone(), list![(name).clone(), (msg).clone()])?;
        }
    }
    Ok(())
}
...
pub(crate) fn getNonLinearVariables(mut jacobian: Arc<BackendDAE::Jacobian>) -> Result<Arc<metamodelica::List<BackendDAE::Var>>> {
    let mut nonLin: Arc<metamodelica::List<BackendDAE::Var>> = metamodelica::nil();
...
fn traverserhasEqnNonDiffParts(mut inExp: Arc<DAE::Exp>, mut inTpl: (Arc<metamodelica::List<Arc<DAE::Exp>>>, bool, bool)) -> Result<(Arc<DAE::Exp>, bool, (Arc<metamodelica::List<Arc<DAE::Exp>>>, bool, bool))> {
    ...
    let (__pa0, (__pa1, __pa2, _)) = Expression::traverseExpTopDown(inExp, (std::sync::Arc::new(hasEqnNonDiffParts) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (Arc<metamodelica::List<Arc<DAE::Exp>>>, bool, bool)) -> Result<(Arc<DAE::Exp>, bool, (Arc<metamodelica::List<Arc<DAE::Exp>>>, bool, bool))> + 'static>), inTpl)?;
    outExp = __pa0.clone();
    expList = __pa1.clone();
    cont = __pa2.clone();
    if Flags::isSet(Flags::DUMP_EXCLUDED_EXP.clone())? && !(cont) {
        ...
        metamodelica::print(stringDelimitList(List::map(expList, (std::sync::Arc::new(ExpressionBasics::printExpStr) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<ArcStr> + 'static>))?, (literal!("\n")).clone()));
        metamodelica::print((literal!("\n\n")).clone());
    }
...
pub fn getSymbolicJacobian(mut inDiffVars: BackendDAE::Variables, mut inResEquations: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, mut inResVars: BackendDAE::Variables, mut inotherEquations: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, mut inotherVars: BackendDAE::Variables, mut inShared: Arc<BackendDAE::Shared>, mut inAllVars: BackendDAE::Variables, mut inName: ArcStr, mut inOnlySparsePattern: bool) -> Result<(Arc<BackendDAE::Jacobian>, Arc<BackendDAE::Shared>)> {
    ...
    let mut cache: FCore::Cache;
    let mut graph: FCore::Graph;
    let mut knvarLst1: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut knvarLst2: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut independentVarsLst: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut dependentVarsLst: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut otherVarsLst: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut independentComRefs: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
    let mut otherVarsLstComRefs: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
    let mut symJacBDAE: Option<(Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>;
    match '__try0: {
...
pub(crate) fn hasGenericSymbolicJacobian(mut inJacobian: Arc<BackendDAE::Jacobian>) -> bool {
    let mut out: bool;
...
fn calculateEqSystemStateSetsJacobians(mut inSyst: Arc<BackendDAE::EqSystem>, mut inShared: Arc<BackendDAE::Shared>) -> Result<(Arc<BackendDAE::EqSystem>, Arc<BackendDAE::Shared>)> {
    ...
    (outSyst, outShared) = (::match_deref::match_deref! { match &((inSyst, inShared)) {
        (syst @ Deref @ BackendDAE::EqSystem { orderedVars: vars, orderedEqs: eqns, stateSets, .. }, shared) => {
            ...
            (stateSets, shared) = calculateStateSetsJacobian(stateSets.clone(), vars.clone(), eqns.clone(), comps, shared.clone())?;
...
fn calculateStateSetsJacobian(mut inStateSets: Arc<metamodelica::List<BackendDAE::StateSet>>, mut inVars: BackendDAE::Variables, mut inEqns: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, mut inComps: Arc<metamodelica::List<Arc<BackendDAE::StrongComponent>>>, mut inShared: Arc<BackendDAE::Shared>) -> Result<(Arc<metamodelica::List<BackendDAE::StateSet>>, Arc<BackendDAE::Shared>)> {
    let mut outStateSets: Arc<metamodelica::List<BackendDAE::StateSet>>;
    let mut outShared: Arc<BackendDAE::Shared> = inShared.clone();
    outStateSets = ({
...
fn calculateStateSetJacobian(mut inStateSet: BackendDAE::StateSet, mut inVars: BackendDAE::Variables, mut inEqns: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, mut inComps: Arc<metamodelica::List<Arc<BackendDAE::StrongComponent>>>, mut inShared: Arc<BackendDAE::Shared>) -> Result<(BackendDAE::StateSet, Arc<BackendDAE::Shared>)> {
    ...
    let mut outShared: Arc<BackendDAE::Shared>;
    (outStateSet, outShared) = (match inStateSet {
        BackendDAE::StateSet { index: mut index, rang: mut rang, state: mut state, crA: mut crA, varA: mut varA, statescandidates: mut statescandidates, ovars: mut ovars, eqns: mut eqns, oeqns: mut oeqns, crJ: mut crJ, varJ: mut varJ, .. } => {
            ...
            let mut compvars: Arc<metamodelica::List<BackendDAE::Var>>;
            let mut diffVars: BackendDAE::Variables;
            let mut allvars: BackendDAE::Variables;
            let mut oVars: BackendDAE::Variables;
            let mut resVars: BackendDAE::Variables;
            let mut compeqns: Arc<metamodelica::List<Arc<BackendDAE::Equation>>>;
            let mut ceqns: Arc<metamodelica::List<Arc<BackendDAE::Equation>>>;
            ...
            compeqns = List::select(compeqns, (std::sync::Arc::new(fnptr!(removeStateSetEqn, Arc<BackendDAE::Equation>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::Equation>) -> Result<bool> + 'static>))?;
            hs = List::fold(crstates, (std::sync::Arc::new(BaseHashSet::add) as std::sync::Arc<dyn ::std::ops::Fn(_, _) -> Result<_> + 'static>), HashSet::emptyHashSet())?;
            compvars = List::select1(compvars, (std::sync::Arc::new(removeStateSetStates) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Var, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<Arc<DAE::ComponentRef>>>), i32, i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>))) -> Result<bool> + 'static>), hs)?;
            (ceqns, oeqns) = IndexReduction::splitEqnsinConstraintAndOther(compvars.clone(), compeqns, inShared.clone())?;
            compvars = List::map(compvars, (std::sync::Arc::new(fnptr!(BackendVariable::transformXToXd, BackendDAE::Var)) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Var) -> Result<BackendDAE::Var> + 'static>))?;
            ...
            statevars = List::map(statevars, (std::sync::Arc::new(fnptr!(BackendVariable::transformXToXd, BackendDAE::Var)) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Var) -> Result<BackendDAE::Var> + 'static>))?;
            ...
            (jacobian, shared) = getSymbolicJacobian(diffVars, cEqns, resVars, oEqns, oVars, inShared, allvars, (name).clone(), false)?;
            (BackendDAE::StateSet { index: index.clone(), rang: rang.clone(), state: state.clone(), crA: crA.clone(), varA: varA.clone(), statescandidates: statescandidates.clone(), ovars: ovars.clone(), eqns: eqns.clone(), oeqns: oeqns.clone(), crJ: crJ.clone(), varJ: varJ.clone(), jacobian: jacobian }, shared)
        },
    });
...
fn foundMarked(mut ilst: Arc<metamodelica::List<i32>>, mut marked: metamodelica::Array<bool>) -> bool {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: i, tail: rest } => {
            ...
            if (!(b)) {{ (ilst, marked) = (rest.clone(), marked.clone()); continue '__tco; }} else {return b}
        },
        _ => unreachable!("tail-call lowered match: no arm matched"),
    } }
    }
...
fn createResidualSetEquations(mut iEqs: Arc<metamodelica::List<Arc<BackendDAE::Equation>>>, mut crJ: Arc<DAE::ComponentRef>, mut index: i32, mut applySubs: bool) -> Result<Arc<metamodelica::List<Arc<BackendDAE::Equation>>>> {
    ...
    oEqs = ({
        ...
        Deref @ BackendDAE::Equation::EQUATION { exp: e1, scalar: e2, source, attr: eqAttr } => {
            ...
            let mut expJ: Arc<DAE::Exp>;
            let mut eqn: Arc<BackendDAE::Equation>;
            crj = if (applySubs) {ComponentReference::subscriptCrefWithInt(crJ.clone(), idx)?} else {crJ.clone()};
        ...
        Deref @ BackendDAE::Equation::RESIDUAL_EQUATION { exp: e1, source, attr: eqAttr } => {
            let mut expJ: Arc<DAE::Exp>;
            let mut eqn: Arc<BackendDAE::Equation>;
            expJ = Expression::crefExp(ComponentReference::subscriptCrefWithInt(crJ.clone(), idx)?)?;
...
pub fn calculateJacobian(mut inVariables: BackendDAE::Variables, mut inEquationArray: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, mut inAdjacencyMatrix: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut differentiateIfExp: bool, mut iShared: Arc<BackendDAE::Shared>) -> (Option<Arc<metamodelica::List<(i32, i32, Arc<BackendDAE::Equation>)>>>, Arc<BackendDAE::Shared>) {
    let mut outTplIntegerIntegerEquationLstOption: Option<Arc<metamodelica::List<(i32, i32, Arc<BackendDAE::Equation>)>>>;
    let mut oShared: Arc<BackendDAE::Shared>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (vars, eqns, m) => {
                    ...
                    let mut shared: Arc<BackendDAE::Shared>;
                    (jac, shared) = calculateJacobianRows(eqns.clone(), vars.clone(), m.clone(), 1, 1, differentiateIfExp, iShared.clone(), (std::sync::Arc::new(BackendDAEUtil::varsInEqn) as std::sync::Arc<dyn ::std::ops::Fn(metamodelica::Array<Arc<metamodelica::List<i32>>>, i32) -> Result<Arc<metamodelica::List<i32>>> + 'static>))?;
...
fn calculateJacobianRows<Type_a: Clone + 'static + metamodelica::gc::MMTrace>(mut inEquationArray: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, mut vars: BackendDAE::Variables, mut m: Type_a, mut eqn_indx: i32, mut scalar_eqn_indx: i32, mut differentiateIfExp: bool, mut iShared: Arc<BackendDAE::Shared>, mut varsInEqn: Arc<dyn ::std::ops::Fn(Type_a, i32) -> Result<Arc<metamodelica::List<i32>>> + 'static>) -> Result<(Arc<metamodelica::List<(i32, i32, Arc<BackendDAE::Equation>)>>, Arc<BackendDAE::Shared>)> {
    ...
    for mut k in 1..=n {
        if ExpandableArray::occupied(k, inEquationArray.clone()) {
            ...
            (outLst, size, oShared) = calculateJacobianRow(eqn.clone(), vars.clone(), m.clone(), i, j, differentiateIfExp, oShared.clone(), varsInEqn.clone(), outLst.clone())?;
            i = i + 1;
            j = j + size;
        }
    }
...
fn calculateJacobianRow<Type_a: Clone + 'static + metamodelica::gc::MMTrace>(mut inEquation: Arc<BackendDAE::Equation>, mut vars: BackendDAE::Variables, mut m: Type_a, mut eqn_indx: i32, mut scalar_eqn_indx: i32, mut differentiateIfExp: bool, mut iShared: Arc<BackendDAE::Shared>, mut fvarsInEqn: Arc<dyn ::std::ops::Fn(Type_a, i32) -> Result<Arc<metamodelica::List<i32>>> + 'static>, mut iAcc: Arc<metamodelica::List<(i32, i32, Arc<BackendDAE::Equation>)>>) -> Result<(Arc<metamodelica::List<(i32, i32, Arc<BackendDAE::Equation>)>>, i32, Arc<BackendDAE::Shared>)> {
    ...
    (outLst, size, oShared) = (::match_deref::match_deref! { match &(inEquation.clone()) {
        Deref @ BackendDAE::Equation::EQUATION { exp: e1, scalar: e2, source, .. } => {
            let mut var_indxs: Arc<metamodelica::List<i32>>;
            let mut var_indxs_1: Arc<metamodelica::List<i32>>;
            let mut eqns: Arc<metamodelica::List<(i32, i32, Arc<BackendDAE::Equation>)>>;
            let mut shared: Arc<BackendDAE::Shared>;
            var_indxs = fvarsInEqn(m, eqn_indx)?;
            var_indxs_1 = List::sort(var_indxs, (std::sync::Arc::new(fnptr!(intGt, i32, i32)) as std::sync::Arc<dyn ::std::ops::Fn(i32, i32) -> Result<bool> + 'static>))?;
            var_indxs_1 = List::sortedUnique(var_indxs_1, (std::sync::Arc::new(fnptr!(intEq, i32, i32)) as std::sync::Arc<dyn ::std::ops::Fn(i32, i32) -> Result<bool> + 'static>))?;
            (eqns, shared) = calculateJacobianRow2(Expression::expSub(e1.clone(), e2.clone())?, vars, scalar_eqn_indx, var_indxs_1, differentiateIfExp, iShared, source.clone(), iAcc)?;
            (eqns, 1, shared)
        },
        Deref @ BackendDAE::Equation::RESIDUAL_EQUATION { exp: e, source, .. } => {
            ...
            var_indxs_1 = List::sort(var_indxs, (std::sync::Arc::new(fnptr!(intGt, i32, i32)) as std::sync::Arc<dyn ::std::ops::Fn(i32, i32) -> Result<bool> + 'static>))?;
            var_indxs_1 = List::sortedUnique(var_indxs_1, (std::sync::Arc::new(fnptr!(intEq, i32, i32)) as std::sync::Arc<dyn ::std::ops::Fn(i32, i32) -> Result<bool> + 'static>))?;
            (eqns, shared) = calculateJacobianRow2(e.clone(), vars, scalar_eqn_indx, var_indxs_1, differentiateIfExp, iShared, source.clone(), iAcc)?;
            (eqns, 1, shared)
        },
        ...
        Deref @ BackendDAE::Equation::ARRAY_EQUATION { dimSize: ds, left: e1, right: e2, source, .. } => {
            ...
            expl = List::map1r(subslst, (std::sync::Arc::new(Expression::applyExpSubscripts) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Subscript>>>) -> Result<Arc<DAE::Exp>> + 'static>), e.clone())?;
            var_indxs = fvarsInEqn(m, eqn_indx)?;
            var_indxs_1 = List::sort(var_indxs, (std::sync::Arc::new(fnptr!(intGt, i32, i32)) as std::sync::Arc<dyn ::std::ops::Fn(i32, i32) -> Result<bool> + 'static>))?;
            var_indxs_1 = List::sortedUnique(var_indxs_1, (std::sync::Arc::new(fnptr!(intEq, i32, i32)) as std::sync::Arc<dyn ::std::ops::Fn(i32, i32) -> Result<bool> + 'static>))?;
            (eqns, shared) = calculateJacobianRowLst(expl, vars, scalar_eqn_indx, var_indxs_1, differentiateIfExp, iShared, source.clone(), iAcc)?;
        ...
        _ => {
            ...
            r#str = (BackendDump::dumpEqnsStr(list![inEquation])?).clone();
            Debug::traceln(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("- BackendDAE.calculateJacobianRow failed on ")); __mm_s.push_str(&*r#str); ArcStr::from(__mm_s) }).clone())?;
            bail!("fail")
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn calculateJacobianRowLst(mut inExps: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut vars: BackendDAE::Variables, mut eqn_indx: i32, mut inIntegerLst: Arc<metamodelica::List<i32>>, mut differentiateIfExp: bool, mut iShared: Arc<BackendDAE::Shared>, mut source: Arc<DAE::ElementSource>, mut iAcc: Arc<metamodelica::List<(i32, i32, Arc<BackendDAE::Equation>)>>) -> Result<(Arc<metamodelica::List<(i32, i32, Arc<BackendDAE::Equation>)>>, Arc<BackendDAE::Shared>)> {
    ...
    for mut e in &*inExps {
        let mut e = e.clone();
        (outLst, oShared) = calculateJacobianRow2(e.clone(), vars.clone(), eqn_indx_arr, inIntegerLst.clone(), differentiateIfExp, oShared.clone(), source.clone(), outLst.clone())?;
...
fn calculateJacobianRow2(mut inExp: Arc<DAE::Exp>, mut vars: BackendDAE::Variables, mut eqn_indx: i32, mut inIntegerLst: Arc<metamodelica::List<i32>>, mut differentiateIfExp: bool, mut iShared: Arc<BackendDAE::Shared>, mut source: Arc<DAE::ElementSource>, mut iAcc: Arc<metamodelica::List<(i32, i32, Arc<BackendDAE::Equation>)>>) -> Result<(Arc<metamodelica::List<(i32, i32, Arc<BackendDAE::Equation>)>>, Arc<BackendDAE::Shared>)> {
    ...
    match '__try0: {
        for mut vindx in &*inIntegerLst.clone() {
            ...
            v = unwrap_break_err!(BackendVariable::getVarAt(vars.clone(), vindx), '__try0);
            ...
            if !(unwrap_break_err!(Expression::isZero(e_1.clone()), '__try0)) {
                outLst = metamodelica::cons((eqn_indx, vindx, Arc::new(BackendDAE::Equation::RESIDUAL_EQUATION { exp: e_1.clone(), source: source.clone(), attr: BackendDAE::EQ_ATTR_DEFAULT_UNKNOWN.clone() })), outLst.clone());
            }
        }
        Ok::<(), anyhow::Error>(())
    } {
...
fn addBackendDAESharedJacobian(mut inSymJac: Option<(Arc<BackendDAE::BackendDAE>, ArcStr, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>, mut inSparsePattern: (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32), mut inSparseColoring: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>, mut inNonlinearPattern: (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32), mut inShared: Arc<BackendDAE::Shared>) -> Result<Arc<BackendDAE::Shared>> {
    ...
    symjacs = list![(inSymJac, inSparsePattern, inSparseColoring, inNonlinearPattern), (None, (metamodelica::nil(), metamodelica::nil(), (metamodelica::nil(), metamodelica::nil()), -1), metamodelica::nil(), (metamodelica::nil(), metamodelica::nil(), (metamodelica::nil(), metamodelica::nil()), -1)), (None, (metamodelica::nil(), metamodelica::nil(), (metamodelica::nil(), metamodelica::nil()), -1), metamodelica::nil(), (metamodelica::nil(), metamodelica::nil(), (metamodelica::nil(), metamodelica::nil()), -1)), (None, (metamodelica::nil(), metamodelica::nil(), (metamodelica::nil(), metamodelica::nil()), -1), metamodelica::nil(), (metamodelica::nil(), metamodelica::nil(), (metamodelica::nil(), metamodelica::nil()), -1))];
...
fn addBackendDAESharedJacobianSparsePattern(mut inSparsePattern: (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32), mut inSparseColoring: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::ComponentRef>>>>>, mut inIndex: i32, mut inShared: Arc<BackendDAE::Shared>) -> Result<Arc<BackendDAE::Shared>> {
    ...
    symjacs = __pa0.clone();
    (symJac, _, _, _) = (symjacs.clone()).get(inIndex)?;
    symjacs = List::set(symjacs, inIndex, (symJac, inSparsePattern, inSparseColoring, nonlinearPattern))?;
    outShared = BackendDAEUtil::setSharedSymJacs(inShared, symjacs)?;
    Ok(outShared)
}

pub(crate) fn analyzeJacobian(mut vars: BackendDAE::Variables, mut eqns: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, mut inTplIntegerIntegerEquationLstOption: Option<Arc<metamodelica::List<(i32, i32, Arc<BackendDAE::Equation>)>>>) -> (BackendDAE::JacobianType, bool) {
...
fn jacobianNonlinearExp(mut vars: BackendDAE::Variables, mut inExp: Arc<DAE::Exp>) -> Result<bool> {
    ...
    let (_, (_, __pa0)) = Expression::traverseExpTopDown(inExp, (std::sync::Arc::new(fnptr!(traverserjacobianNonlinearExp, Arc<DAE::Exp>, (BackendDAE::Variables, bool))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (BackendDAE::Variables, bool)) -> Result<(Arc<DAE::Exp>, bool, (BackendDAE::Variables, bool))> + 'static>), (vars, false))?;
    outBoolean = __pa0.clone();
    Ok(outBoolean)
}

fn traverserjacobianNonlinearExp(mut inExp: Arc<DAE::Exp>, mut tpl: (BackendDAE::Variables, bool)) -> (Arc<DAE::Exp>, bool, (BackendDAE::Variables, bool)) {
    let mut outExp: Arc<DAE::Exp>;
    let mut cont: bool;
    let mut outTpl: (BackendDAE::Variables, bool);
    (outExp, cont, outTpl) = 'mc: {
        let __mc_input = (inExp, tpl.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
pub(crate) fn isJacobianGeneric(mut inJac: Arc<BackendDAE::Jacobian>) -> bool {
    let mut result: bool;
    result = (::match_deref::match_deref! { match &(inJac) {
...
fn varsNotInRelations(mut exp: Arc<DAE::Exp>, mut tpl: (BackendDAE::Variables, bool)) -> Result<(Arc<DAE::Exp>, bool, (BackendDAE::Variables, bool))> {
    ...
    (exp, cont, tpl) = (::match_deref::match_deref! { match &((exp.clone(), tpl.clone())) {
        (Deref @ DAE::Exp::IFEXP { expCond: cond, expThen: t, expElse: f }, (vars, b)) => {
            ...
            let mut f = (*f).clone();
            let mut b = (*b).clone();
            let (_, (_, __pa0)) = Expression::traverseExpTopDown(cond.clone(), (std::sync::Arc::new(fnptr!(BackendDAEUtil::getEqnsysRhsExp2, Arc<DAE::Exp>, (BackendDAE::Variables, bool))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (BackendDAE::Variables, bool)) -> Result<(Arc<DAE::Exp>, bool, (BackendDAE::Variables, bool))> + 'static>), (vars.clone(), b.clone()))?;
            b = __pa0.clone();
        ...
        (Deref @ DAE::Exp::CALL { expLst, .. }, _) => {
            (_, tpl) = Expression::traverseExpListTopDown(expLst.clone(), (std::sync::Arc::new(fnptr!(BackendDAEUtil::getEqnsysRhsExp2, Arc<DAE::Exp>, (BackendDAE::Variables, bool))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (BackendDAE::Variables, bool)) -> Result<(Arc<DAE::Exp>, bool, (BackendDAE::Variables, bool))> + 'static>), tpl)?;
            (exp, false, tpl.clone())
        },
        (Deref @ DAE::Exp::LBINARY { .. }, _) => {
            (_, tpl) = Expression::traverseExpTopDown(exp.clone(), (std::sync::Arc::new(fnptr!(BackendDAEUtil::getEqnsysRhsExp2, Arc<DAE::Exp>, (BackendDAE::Variables, bool))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (BackendDAE::Variables, bool)) -> Result<(Arc<DAE::Exp>, bool, (BackendDAE::Variables, bool))> + 'static>), tpl)?;
            (exp, false, tpl.clone())
        },
        (Deref @ DAE::Exp::LUNARY { .. }, __esc_tpl) => {
            tpl = (*__esc_tpl).clone();
            (_, tpl) = Expression::traverseExpTopDown(exp.clone(), (std::sync::Arc::new(fnptr!(BackendDAEUtil::getEqnsysRhsExp2, Arc<DAE::Exp>, (BackendDAE::Variables, bool))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (BackendDAE::Variables, bool)) -> Result<(Arc<DAE::Exp>, bool, (BackendDAE::Variables, bool))> + 'static>), tpl.clone())?;
            (exp, false, tpl.clone())
        },
        ...
        __acc.reverse()
            ...
            let (_, ref __pa1 @ (_, ref __pa0)) = Expression::traverseExpTopDown(e1.clone(), (std::sync::Arc::new(varsNotInRelations) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (BackendDAE::Variables, bool)) -> Result<(Arc<DAE::Exp>, bool, (BackendDAE::Variables, bool))> + 'static>), tpl)?;
            b = __pa0.clone();
            tpl = __pa1.clone();
            if b {
                (_, tpl) = Expression::traverseExpListTopDown(expLst.clone(), (std::sync::Arc::new(fnptr!(BackendDAEUtil::getEqnsysRhsExp2, Arc<DAE::Exp>, (BackendDAE::Variables, bool))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (BackendDAE::Variables, bool)) -> Result<(Arc<DAE::Exp>, bool, (BackendDAE::Variables, bool))> + 'static>), tpl)?;
...
fn rhsConstant(mut vars: BackendDAE::Variables, mut eqns: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>) -> Result<bool> {
    ...
    if BackendEquation::equationArraySize(eqns.clone())? == 0 {
        ...
        (_, outBoolean, _) = BackendEquation::traverseEquationArray_WithStop(eqns, (std::sync::Arc::new(fnptr!(rhsConstant2, Arc<BackendDAE::Equation>, (BackendDAE::Variables, bool, BackendVarTransform::VariableReplacements))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::Equation>, (BackendDAE::Variables, bool, BackendVarTransform::VariableReplacements)) -> Result<(Arc<BackendDAE::Equation>, bool, (BackendDAE::Variables, bool, BackendVarTransform::VariableReplacements))> + 'static>), (vars, true, repl))?;
    }
    Ok(outBoolean)
}

fn rhsConstant2(mut inEq: Arc<BackendDAE::Equation>, mut inTpl: (BackendDAE::Variables, bool, BackendVarTransform::VariableReplacements)) -> (Arc<BackendDAE::Equation>, bool, (BackendDAE::Variables, bool, BackendVarTransform::VariableReplacements)) {
...
fn getJacobianResiduals(mut jacDAE: Arc<BackendDAE::BackendDAE>) -> Result<Arc<metamodelica::List<BackendDAE::Var>>> {
    let mut diffedRes: Arc<metamodelica::List<BackendDAE::Var>>;
    let mut syst: Arc<BackendDAE::EqSystem>;
...
pub(crate) fn getFixedStatesForSelfdependentSets(mut stateSet: BackendDAE::StateSet, mut unfixedStates: Arc<metamodelica::List<BackendDAE::Var>>, mut toFix: i32) -> Result<Arc<metamodelica::List<BackendDAE::Var>>> {
    let mut statesToFix: Arc<metamodelica::List<BackendDAE::Var>>;
...
fn getNonlinearStateCount(mut state: BackendDAE::Var, mut diffVars: Arc<metamodelica::List<BackendDAE::Var>>, mut dae: Arc<BackendDAE::BackendDAE>, mut matrixName: ArcStr) -> Result<(i32, BackendDAE::Var)> {
    let mut outTpl: (i32, BackendDAE::Var);
    outTpl = ({
        let mut nonlinearCount: i32 = 0;
        (::match_deref::match_deref! { match &(dae) {
...
fn getNonlinearStateCount0(mut inEq: Arc<BackendDAE::Equation>, mut inTpl: (BackendDAE::Var, Arc<metamodelica::List<BackendDAE::Var>>, i32, ArcStr)) -> Result<(Arc<BackendDAE::Equation>, (BackendDAE::Var, Arc<metamodelica::List<BackendDAE::Var>>, i32, ArcStr))> {
    let mut outEq: Arc<BackendDAE::Equation>;
    let mut outTpl: (BackendDAE::Var, Arc<metamodelica::List<BackendDAE::Var>>, i32, ArcStr);
    outEq = inEq.clone();
    outTpl = (::match_deref::match_deref! { match &(inEq) {
        Deref @ BackendDAE::Equation::EQUATION { scalar: exp, .. } => {
            ...
            let mut matrixName: ArcStr;
            let mut seedVar: Arc<DAE::ComponentRef>;
...
fn fixedVarsFromNonlinearCount(mut tplLst: Arc<metamodelica::List<(i32, BackendDAE::Var)>>, mut toFix: i32) -> Result<Arc<metamodelica::List<BackendDAE::Var>>> {
    ...
    let mut sortedTplLst: Arc<metamodelica::List<(i32, BackendDAE::Var)>>;
    let mut strippedTplLst: Arc<metamodelica::List<(i32, BackendDAE::Var)>>;
...
fn stripPartialDerNonlinearPattern(mut pat: (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32)) -> (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, (Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), i32) {
    ...
    let mut pat_cref: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>;
    let mut pat_crefT: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>;
    let mut v1: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
    let mut v2: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
    let mut index: i32;
    ...
    (cref, strip) = (::match_deref::match_deref! { match &(cref.clone()) {
        ...
        Deref @ DAE::ComponentRef::CREF_QUAL { .. } => {
            ...
            if strip {
                ...
                cr = Arc::new(DAE::ComponentRef::CREF_QUAL { ident: (var_field!((*cref).ident, DAE::ComponentRef::CREF_QUAL).clone()).clone(), identType: var_field!((*cref).identType, DAE::ComponentRef::CREF_QUAL).clone(), subscriptLst: var_field!((*cref).subscriptLst, DAE::ComponentRef::CREF_QUAL).clone(), componentRef: cr });
            }
            (cr, false)
        },
        _ => {
...
#[derive(Clone, Debug, Eq, metamodelica::MetaCmp, metamodelica::ReferenceEq)]
    ...
    pub(crate) fn toString(mut linJac: Arc<LinearJacobian>, mut heading: ArcStr) -> Result<ArcStr> {
        let mut r#str: ArcStr;
        r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("######################################################\n")); __mm_s.push_str(&*literal!(" LinearJacobian sparsity pattern: ")); __mm_s.push_str(&*heading); __mm_s.push_str(&*literal!("\n")); __mm_s.push_str(&*literal!("######################################################\n")); __mm_s.push_str(&*literal!("(scal_idx|arr_idx|changed) [var_index, value] || RHS_EXPRESSION\n")); ArcStr::from(__mm_s) }).clone();
    ...
    fn rowToString(mut row: Arc<UnorderedMap::UnorderedMap<i32, metamodelica::Real>>, mut rhs: Arc<DAE::Exp>, mut indices: (i32, i32), mut changed: bool) -> Result<ArcStr> {
        ...
        r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("(")); __mm_s.push_str(&*intString(i_arr)); __mm_s.push_str(&*literal!("|")); __mm_s.push_str(&*intString(i_scal)); __mm_s.push_str(&*literal!("|")); __mm_s.push_str(&*boolString(changed)); __mm_s.push_str(&*literal!("):    ")); ArcStr::from(__mm_s) }).clone();
        if row_lst.clone().is_empty() {
            r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*r#str); __mm_s.push_str(&*literal!("EMPTY ROW     ")); ArcStr::from(__mm_s) }).clone();
        } else {
            for mut element in &*row_lst {
                let mut element = element.clone();
                (index, value) = element.clone();
                r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*r#str.clone()); __mm_s.push_str(&*literal!("[")); __mm_s.push_str(&*intString(index)); __mm_s.push_str(&*literal!("|")); __mm_s.push_str(&*realString(value)); __mm_s.push_str(&*literal!("] ")); ArcStr::from(__mm_s) }).clone();
            }
        }
        r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*r#str); __mm_s.push_str(&*literal!("    || RHS: ")); __mm_s.push_str(&*ExpressionBasics::printExpStr((ExpressionSimplify::simplify(rhs)?).0)?); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone();
        Ok(r#str)
    }

    pub(crate) fn generate(mut loopEqs: Arc<metamodelica::List<(Arc<BackendDAE::Equation>, (i32, i32))>>, mut loopVars: Arc<metamodelica::List<(BackendDAE::Var, i32)>>, mut ass1: metamodelica::Array<i32>) -> Result<Arc<LinearJacobian>> {
        ...
        let mut row: Arc<UnorderedMap::UnorderedMap<i32, metamodelica::Real>>;
        ...
        for mut loopEq in &*loopEqs.clone() {
            ...
            if '__try0: {
                ...
                (res, _) = BackendVarTransform::replaceExp(res.clone(), varRep.clone(), None);
    ...
    pub(crate) fn emptyOrSingle(mut linJac: Arc<LinearJacobian>) -> bool {
    ...
    pub(crate) fn solve(mut linJac: Arc<LinearJacobian>) -> Arc<LinearJacobian> {
        ...
        for mut i in 1..=metamodelica::arrayLength(linJac.rows.clone()) {
            if '__try0: {
                ...
                for mut j in i.clone() + 1..=metamodelica::arrayLength(linJac.rows.clone()) {
                    row_value = unwrap_break_err!(UnorderedMap::getOrDefault(col_index, ({let __elt = linJac.rows.borrow()[(j.clone()-1) as usize].clone(); __elt}), metamodelica::OrderedFloat(0.0_f64)), '__try0);
    ...
    pub(crate) fn solveRow(mut pivot_row: Arc<UnorderedMap::UnorderedMap<i32, metamodelica::Real>>, mut row: Arc<UnorderedMap::UnorderedMap<i32, metamodelica::Real>>, mut piv_value: metamodelica::Real, mut row_value: metamodelica::Real) -> Result<()> {
        let mut idx: i32 = 0;
        ...
        for mut idx in &*UnorderedMap::keyList(pivot_row.clone()) {
            ...
            let () = (match (UnorderedMap::get(idx, row.clone())?, UnorderedMap::get(idx, pivot_row.clone())?) {
    ...
    pub(crate) fn updatePivotRow(mut pivot_row: Arc<UnorderedMap::UnorderedMap<i32, metamodelica::Real>>, mut piv_value: metamodelica::Real) -> Result<()> {
        let mut value: metamodelica::Real;
    ...
    pub(crate) fn resolveASSC(mut linJac: Arc<LinearJacobian>, mut ass1: metamodelica::Array<i32>, mut ass2: metamodelica::Array<i32>, mut syst: Arc<BackendDAE::EqSystem>, mut init: bool) -> Result<(metamodelica::Array<i32>, metamodelica::Array<i32>, Arc<BackendDAE::EqSystem>)> {
        ...
        for mut r in 1..=metamodelica::arrayLength(linJac.rows.clone()) {
            if ({let __elt = linJac.eq_marks.borrow()[(r.clone()-1) as usize].clone(); __elt}) && (UnorderedMap::isEmpty(({let __elt = linJac.rows.borrow()[(r.clone()-1) as usize].clone(); __elt})) || fullASSC) {
                (i_arr, i_scal) = ({let __elt = linJac.ind.borrow()[(r.clone()-1) as usize].clone(); __elt});
                {
                    let __cell0 = -1;
                    let __idx0 = ({let __elt = ass1.borrow()[(i_scal-1) as usize].clone(); __elt});
                ...
                if Flags::isSet(Flags::DUMP_ASSC.clone())? || Flags::isSet(Flags::BLT_DUMP.clone())? && UnorderedMap::isEmpty(({let __elt = linJac.rows.borrow()[(r.clone()-1) as usize].clone(); __elt})) {
                    metamodelica::print(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("[ASSC] The equation: ")); __mm_s.push_str(&*BackendDump::equationString(BackendEquation::get(syst.orderedEqs.clone(), i_arr)?)?); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
                ...
                assign_field!(syst.orderedEqs = BackendEquation::setAtIndex(syst.orderedEqs.clone(), i_arr, newEqn.clone())?);
                updateList_arr = metamodelica::cons(i_arr, updateList_arr.clone());
            }
        }
        ...
        if !(updateList_arr.is_empty()) && !(Flags::isSet(Flags::DUMP_ASSC.clone())?) && Flags::isSet(Flags::BLT_DUMP.clone())? {
    ...
    pub(crate) fn anyChanges(mut linJac: Arc<LinearJacobian>) -> bool {
