...
fn makeDebugResult(mut inFlag: Flags::DebugFlag, mut res: ArcStr) -> Result<ArcStr> {
    ...
    res_1 = ('mc: {
        let __mc_input = (inFlag.clone(), res.clone());
        if let Ok(__v) = (|| -> Result<_> {
            let (Flags::DebugFlag { name: mut flagstr, .. }, _) = __mc_input.clone() else { bail!("nomatch") };
...
fn handleCommand2(mut inStatements: Option<GlobalScript::Statements>, mut inProgram: Option<Absyn::Program>, mut inCommand: ArcStr) -> Result<ArcStr> {
    ...
    outResult = ('mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ...
            prog2 = InteractiveUtil::updateProgram(prog2.clone(), ast.clone(), false)?;
...
fn makeClassDefResult(mut p: Absyn::Program) -> Result<ArcStr> {
    ...
    res = ((match p.clone() {
        ...
        __acc.reverse()
            ...
            names = List::map1(names.clone(), Arc::new(AbsynUtil::joinPaths), scope.clone());
...
fn isEmptyOrFirstIsModelicaFile(mut libs: Arc<metamodelica::List<ArcStr>>) -> Result<()> {
    let _ = (::match_deref::match_deref! { match &(libs.clone()) {
            ...
            ::match_deref::match_deref! { match &__mc_input {
                libs => {
                    ...
                    if Flags::isSet(Flags::DUMP_JL.clone())? {
                        ...
                        println!("{}", ({ let mut __mm_s = String::new(); __mm_s.push_str(&*Tpl::tplString(Arc::new(AbsynJLDumpTpl::dump), SymbolTable::getAbsyn())?); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
                    }
                    if Flags::isSet(Flags::DUMP_GRAPHVIZ.clone())? {
                    ...
                    (_, _, _, _, _) = CevalScriptBackend::translateModel(FCore::emptyCache(), FGraph::empty(), cname.clone(), (fileNamePrefix.clone()).clone(), runBackend.clone(), runSilent.clone(), None)?;
                    showErrors((Print::getErrorString()?).clone(), (ErrorExt::printMessagesStr(false)).clone());
                    Ok(())
                }
                _ => bail!("nomatch"),
...
fn readSettingsFile(mut filePath: ArcStr) -> Result<()> {
    ...
    if System::regularFileExists((filePath.clone()).clone()) {
        ...
        (_, _) = handleCommand((command.clone()).clone())?;
    }
    Ok(())
}

pub fn setWindowsPaths(mut inOMHome: ArcStr) -> Result<()> {
    let _ = (match inOMHome.clone() {
        mut omHome => {
            ...
            omdevPath = (Util::makeValueOrDefault(Arc::new(System::readEnv), (literal!("OMDEV")).clone(), (literal!("")).clone())).clone();
    ...
    if '__try0: {
        match '__try1: {
            ...
            Err(_) => {
                ...
                if '__try2: {
                    let _ = unwrap_break_err!(FlagsUtil::new(args.clone()), '__try2);
                    Ok::<(), anyhow::Error>(())
