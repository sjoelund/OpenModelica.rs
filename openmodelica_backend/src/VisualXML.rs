...
pub(crate) fn visualizationInfoXML(mut daeIn: Arc<BackendDAE::BackendDAE>, mut fileName: ArcStr, mut program: Absyn::Program) -> Result<Arc<BackendDAE::BackendDAE>> {
    ...
    eqs = List::map(eqs0, (std::sync::Arc::new(BackendDAEUtil::copyEqSystem) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::EqSystem>) -> Result<Arc<BackendDAE::EqSystem>> + 'static>))?;
    eqs = List::map(eqs, (std::sync::Arc::new(fnptr!(setBindingForProtectedVars, Arc<BackendDAE::EqSystem>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::EqSystem>) -> Result<Arc<BackendDAE::EqSystem>> + 'static>))?;
    ...
    (globalKnownVarLst, allVisuals) = List::fold(globalKnownVarLst, (std::sync::Arc::new(fnptr!(isVisualizationVarFold, BackendDAE::Var, (Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, ArcStr)>>))) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Var, (Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, ArcStr)>>)) -> Result<(Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, ArcStr)>>)> + 'static>), (metamodelica::nil(), metamodelica::nil()))?;
    (allVarLst, allVisuals) = List::fold(allVarLst, (std::sync::Arc::new(fnptr!(isVisualizationVarFold, BackendDAE::Var, (Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, ArcStr)>>))) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Var, (Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, ArcStr)>>)) -> Result<(Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, ArcStr)>>)> + 'static>), (metamodelica::nil(), allVisuals))?;
    (aliasVarLst, allVisuals) = List::fold(aliasVarLst, (std::sync::Arc::new(fnptr!(isVisualizationVarFold, BackendDAE::Var, (Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, ArcStr)>>))) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Var, (Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, ArcStr)>>)) -> Result<(Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, ArcStr)>>)> + 'static>), (metamodelica::nil(), allVisuals))?;
    allVarLst = listAppend(globalKnownVarLst, listAppend(allVarLst, aliasVarLst));
    (visuals, _, _) = List::mapFold2(allVisuals, (std::sync::Arc::new(fillVisualizationObjects) as std::sync::Arc<dyn ::std::ops::Fn((Arc<DAE::ComponentRef>, ArcStr), Arc<metamodelica::List<BackendDAE::Var>>, Absyn::Program) -> Result<(Visualization, Arc<metamodelica::List<BackendDAE::Var>>, Absyn::Program)> + 'static>), allVarLst, program.clone())?;
    visuals = List::map2(visuals, (std::sync::Arc::new(fnptr!(replaceVisualBinding, Visualization, BackendDAE::Variables, Absyn::Program)) as std::sync::Arc<dyn ::std::ops::Fn(Visualization, BackendDAE::Variables, Absyn::Program) -> Result<Visualization> + 'static>), globalKnownVars.clone(), program)?;
    dumpVis(metamodelica::arrayFromVec(visuals.into_iter().cloned().collect()), ({ let mut __mm_s = String::new(); __mm_s.push_str(&*fileName); __mm_s.push_str(&*literal!("_visual.xml")); ArcStr::from(__mm_s) }).clone())?;
    (globalKnownVars, _) = BackendVariable::traverseBackendDAEVarsWithUpdate(globalKnownVars, (std::sync::Arc::new(setVisVarsPublic) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Var, ArcStr) -> Result<(BackendDAE::Var, ArcStr)> + 'static>), (literal!("")).clone())?;
    (aliasVars, _) = BackendVariable::traverseBackendDAEVarsWithUpdate(aliasVars, (std::sync::Arc::new(setVisVarsPublic) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Var, ArcStr) -> Result<(BackendDAE::Var, ArcStr)> + 'static>), (literal!("")).clone())?;
    daeOut = Arc::new(BackendDAE::BackendDAE { eqs: eqs, shared: shared });
    Ok(daeOut)
}
...
fn makeVarPublicHideResultFalse(mut inVar: BackendDAE::Var) -> Result<BackendDAE::Var> {
    ...
    outVar = BackendVariable::setHideResult(outVar, Some(Arc::new(DAE::Exp::BCONST { bool: false })));
    Ok(outVar)
}
...
fn setBindingForProtectedVars1(mut varIn: BackendDAE::Var, mut tplIn: (i32, metamodelica::Array<i32>, Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>)) -> Result<(BackendDAE::Var, (i32, metamodelica::Array<i32>, Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>))> {
    ...
    (varOut, tplOut) = 'mc: {
        let __mc_input = (varIn.clone(), tplIn);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
fn makeCrefQualFromString(mut s: ArcStr) -> Result<Arc<DAE::ComponentRef>> {
    ...
    sLst = Util::stringSplitAtChar((s).clone(), (literal!(".")).clone())?;
    crefs = List::map2(sLst, (std::sync::Arc::new(fnptr!(ComponentReferenceBasics::makeCrefIdent, ArcStr, Arc<DAE::Type>, Arc<metamodelica::List<Arc<DAE::Subscript>>>)) as std::sync::Arc<dyn ::std::ops::Fn(ArcStr, Arc<DAE::Type>, Arc<metamodelica::List<Arc<DAE::Subscript>>>) -> Result<Arc<DAE::ComponentRef>> + 'static>), DAE::T_REAL_DEFAULT().clone(), metamodelica::nil())?;
    ...
    cref = __pa0.clone();
    crefs = __pa1.clone();
    crefOut = List::foldr(crefs, (std::sync::Arc::new(ComponentReference::joinCrefs) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<Arc<DAE::ComponentRef>> + 'static>), cref)?;
    Ok(crefOut)
}
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (BackendDAE::Var { varName: cref, .. }, (vars, vis @ Visualization::SHAPE { ident, .. })) => {
                    ...
                    cref1 = __pa0.clone();
                    filled_vis = fillShapeObject(cref1.clone(), varIn.clone(), storeProtectedCrefs, program.clone(), vis.clone());
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (BackendDAE::Var { varName: cref, .. }, (vars, vis @ Visualization::VECTOR { ident, .. })) => {
                    ...
                    cref1 = __pa0.clone();
                    filled_vis = fillVectorObject(cref1.clone(), varIn.clone(), storeProtectedCrefs, program.clone(), vis.clone());
                    Ok((vars.clone(), filled_vis.clone()))
                }
                _ => bail!("nomatch"),
            }}
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (BackendDAE::Var { varName: cref, .. }, (vars, vis @ Visualization::SURFACE { ident, .. })) => {
                    ...
                    cref1 = __pa0.clone();
                    filled_vis = fillSurfaceObject(cref1.clone(), varIn.clone(), storeProtectedCrefs, program.clone(), vis.clone());
                    Ok((vars.clone(), filled_vis.clone()))
                }
                _ => bail!("nomatch"),
            }}
...
fn getFullCADFilePath(mut sIn: ArcStr, mut program: Absyn::Program) -> Result<ArcStr> {
    ...
    if (chars.clone().len() as i32) > 11 && stringEqual(stringDelimitList(List::firstN(chars, 11)?, (literal!("")).clone()), (literal!("modelica://")).clone()) {
        sOut = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("file://")); __mm_s.push_str(&*ProgramUtil::getFullPathFromUri(program, (sIn).clone(), true)?); ArcStr::from(__mm_s) }).clone();
    }
    Ok(sOut)
}
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::ComponentRef::CREF_QUAL { ident: Deref @ "R", componentRef: Deref @ DAE::ComponentRef::CREF_IDENT { ident: Deref @ "T", subscriptLst: Deref @ metamodelica::List::Cons { head: Deref @ DAE::Subscript::INDEX { exp: Deref @ DAE::Exp::ICONST { integer: pos } }, tail: Deref @ metamodelica::List::Cons { head: Deref @ DAE::Subscript::INDEX { exp: Deref @ DAE::Exp::ICONST { integer: pos1 } }, tail: Deref @ metamodelica::List::Nil } }, .. }, .. }, Visualization::SHAPE { .. }) => {
                    ...
                    let mut T0: Arc<metamodelica::List<Arc<DAE::Exp>>>;
                    exp = getVariableBinding(var.clone(), storeProtectedCrefs)?;
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::ComponentRef::CREF_IDENT { ident: Deref @ "length", .. }, Visualization::SHAPE { .. }) => {
                    let mut vis: Visualization = vis.clone();
                    let __owned_variant_length_0 = getVariableBinding(var.clone(), storeProtectedCrefs)?;
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::ComponentRef::CREF_IDENT { ident: Deref @ "width", .. }, Visualization::SHAPE { .. }) => {
                    let mut vis: Visualization = vis.clone();
                    let __owned_variant_width_0 = getVariableBinding(var.clone(), storeProtectedCrefs)?;
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::ComponentRef::CREF_IDENT { ident: Deref @ "height", .. }, Visualization::SHAPE { .. }) => {
                    let mut vis: Visualization = vis.clone();
                    let __owned_variant_height_0 = getVariableBinding(var.clone(), storeProtectedCrefs)?;
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::ComponentRef::CREF_IDENT { ident: Deref @ "extra", .. }, Visualization::SHAPE { .. }) => {
                    let mut vis: Visualization = vis.clone();
                    let __owned_variant_extra_0 = getVariableBinding(var.clone(), storeProtectedCrefs)?;
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::ComponentRef::CREF_IDENT { ident: Deref @ "specularCoefficient", .. }, Visualization::SHAPE { .. }) => {
                    let mut vis: Visualization = vis.clone();
                    let __owned_variant_specularCoeff_0 = getVariableBinding(var.clone(), storeProtectedCrefs)?;
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::ComponentRef::CREF_IDENT { ident: Deref @ "quantity", .. }, Visualization::VECTOR { .. }) => {
                    let mut vis: Visualization = vis.clone();
                    let __owned_variant_quantity_0 = getVariableBinding(var.clone(), storeProtectedCrefs)?;
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::ComponentRef::CREF_IDENT { ident: Deref @ "headAtOrigin", .. }, Visualization::VECTOR { .. }) => {
                    let mut vis: Visualization = vis.clone();
                    let __owned_variant_headAtOrigin_0 = getVariableBinding(var.clone(), storeProtectedCrefs)?;
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::ComponentRef::CREF_IDENT { ident: Deref @ "twoHeadedArrow", .. }, Visualization::VECTOR { .. }) => {
                    let mut vis: Visualization = vis.clone();
                    let __owned_variant_twoHeadedArrow_0 = getVariableBinding(var.clone(), storeProtectedCrefs)?;
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::ComponentRef::CREF_IDENT { ident: Deref @ "nu", .. }, Visualization::SURFACE { .. }) => {
                    let mut vis: Visualization = vis.clone();
                    let __owned_variant_nu_0 = getVariableBinding(var.clone(), storeProtectedCrefs)?;
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::ComponentRef::CREF_IDENT { ident: Deref @ "nv", .. }, Visualization::SURFACE { .. }) => {
                    let mut vis: Visualization = vis.clone();
                    let __owned_variant_nv_0 = getVariableBinding(var.clone(), storeProtectedCrefs)?;
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::ComponentRef::CREF_IDENT { ident: Deref @ "wireframe", .. }, Visualization::SURFACE { .. }) => {
                    let mut vis: Visualization = vis.clone();
                    let __owned_variant_wireframe_0 = getVariableBinding(var.clone(), storeProtectedCrefs)?;
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::ComponentRef::CREF_IDENT { ident: Deref @ "multiColored", .. }, Visualization::SURFACE { .. }) => {
                    let mut vis: Visualization = vis.clone();
                    let __owned_variant_multiColored_0 = getVariableBinding(var.clone(), storeProtectedCrefs)?;
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::ComponentRef::CREF_IDENT { ident: Deref @ "transparency", .. }, Visualization::SURFACE { .. }) => {
                    let mut vis: Visualization = vis.clone();
                    let __owned_variant_transparency_0 = getVariableBinding(var.clone(), storeProtectedCrefs)?;
...
fn getVariableBinding(mut var: BackendDAE::Var, mut storeProtectedCrefs: bool) -> Result<Arc<DAE::Exp>> {
    ...
    if isSome(binding.clone()) {
        let __pa1 = ::match_deref::match_deref! { match &(binding) {
            Some(__pa1) => __pa1.clone(),
            _ => bail!("pattern mismatch"),
        } };
        exp = __pa1.clone();
        if !(Expression::isConstValue(exp.clone())?) && storeProtectedCrefs {
...
fn dumpVis(mut visIn: metamodelica::Array<Visualization>, mut iFileName: ArcStr) -> Result<()> {
    ...
    Tpl::tplNoret2((std::sync::Arc::new(VisualXMLTpl::dumpVisXML) as std::sync::Arc<dyn ::std::ops::Fn(Tpl::Text, metamodelica::Array<Visualization>, ArcStr) -> Result<Tpl::Text> + 'static>), visIn.clone(), (iFileName).clone())?;
    Ok(())
}
