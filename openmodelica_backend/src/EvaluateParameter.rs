...
pub(crate) fn evaluateParameters(mut DAE: Arc<BackendDAE::BackendDAE>) -> Result<Arc<BackendDAE::BackendDAE>> {
    ...
    if !(Flags::isSet(Flags::EVAL_PARAM.clone())?) {
        ...
        let (__pa0, __pa6, __pa1, __pa2, __pa3, __pa4, __pa5) = ::match_deref::match_deref! { match &(DAE) {
            Deref @ BackendDAE::BackendDAE { eqs: __pa0, shared: __pa6 @ Deref @ BackendDAE::Shared { globalKnownVars: __pa1, aliasVars: __pa2, initialEqs: __pa3, cache: __pa4, graph: __pa5, .. } } => (__pa0.clone(), __pa6.clone(), __pa1.clone(), __pa2.clone(), __pa3.clone(), __pa4.clone(), __pa5.clone()),
        ...
        if Flags::isSet(Flags::EVAL_PARAM_DUMP.clone())? {
            ...
            metamodelica::print(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("Number of parameters: ")); __mm_s.push_str(&*intString(size)); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
            metamodelica::print(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("Number of parameters selected for evaluation: ")); __mm_s.push_str(&*intString(nselect)); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
        ...
        (globalKnownVars, cache, repl, oRepl, mark) = evaluateSelectedParameters(selectedParameters, globalKnownVars, m.clone(), initialEqs.clone(), cache, graph.clone(), markarr.clone(), isInitial, repl, oRepl, 1)?;
        if Flags::isSet(Flags::EVAL_PARAM_DUMP.clone())? {
            ...
            metamodelica::print(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("\nmark: ")); __mm_s.push_str(&*intString(mark)); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
        ...
        (globalKnownVars, repl, oRepl, cache, mark) = traverseParameterSorted(comps, globalKnownVars, m.clone(), initialEqs.clone(), cache, graph.clone(), mark, markarr.clone(), repl, oRepl, isInitial)?;
        if Flags::isSet(Flags::EVAL_PARAM_DUMP.clone())? {
        ...
        let (__pa8, (__pa9, __pa10, __pa11, __pa12, __pa13, __pa14, __pa15, _, __pa16, __pa17)) = List::mapFold(systs, (std::sync::Arc::new(replaceEvaluatedParametersSystem) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::EqSystem>, (BackendDAE::Variables, metamodelica::Array<Arc<metamodelica::List<i32>>>, Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, FCore::Cache, FCore::Graph, i32, metamodelica::Array<i32>, bool, BackendVarTransform::VariableReplacements, BackendVarTransform::VariableReplacements)) -> Result<(Arc<BackendDAE::EqSystem>, (BackendDAE::Variables, metamodelica::Array<Arc<metamodelica::List<i32>>>, Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, FCore::Cache, FCore::Graph, i32, metamodelica::Array<i32>, bool, BackendVarTransform::VariableReplacements, BackendVarTransform::VariableReplacements))> + 'static>), (globalKnownVars, m.clone(), initialEqs, cache, graph, mark, markarr.clone(), isInitial, repl, oRepl))?;
        systs = __pa8.clone();
        ...
        DAE = Arc::new(BackendDAE::BackendDAE { eqs: systs, shared: shared });
        if Flags::getConfigBool(Flags::REPLACE_EVALUATED_PARAMS.clone())? {
    ...
    (outVar, outTpl) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (v @ BackendDAE::Var { varKind: BackendDAE::VarKind::PARAM { .. }, bindExp: Some(e), .. }, (globalKnownVars, index, selectParameter, selectedParameters, m, mt, ht, isInitial)) => {
                    ...
                    let mut m = (*m).clone();
                    let mut mt = (*mt).clone();
                    let (_, (_, __pa0, _)) = Expression::traverseExpTopDown(e.clone(), (std::sync::Arc::new(fnptr!(BackendDAEUtil::traversingadjacencyRowExpFinder, Arc<DAE::Exp>, (BackendDAE::Variables, Arc<AvlSetInt::Tree>, bool))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (BackendDAE::Variables, Arc<AvlSetInt::Tree>, bool)) -> Result<(Arc<DAE::Exp>, bool, (BackendDAE::Variables, Arc<AvlSetInt::Tree>, bool))> + 'static>), (globalKnownVars.clone(), crate::AvlSetInt::Tree::interned_EMPTY(), isInitial.clone()))?;
                    tree = __pa0.clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (v @ BackendDAE::Var { varKind: BackendDAE::VarKind::PARAM { .. }, values: attr, .. }, (globalKnownVars, index, selectParameter, selectedParameters, m, mt, ht, isInitial)) => {
...
fn evaluateSelectedParameters(mut iSelected: Arc<metamodelica::List<i32>>, mut globalKnownVars: BackendDAE::Variables, mut m: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut inIEqns: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, mut cache: FCore::Cache, mut graph: FCore::Graph, mut markarr: metamodelica::Array<i32>, mut isInitial: bool, mut repl: BackendVarTransform::VariableReplacements, mut replEvaluate: BackendVarTransform::VariableReplacements, mut mark: i32) -> Result<(BackendDAE::Variables, FCore::Cache, BackendVarTransform::VariableReplacements, BackendVarTransform::VariableReplacements, i32)> {
    ...
    for mut i in &*iSelected {
        let mut i = i.clone();
        (globalKnownVars, cache, repl, replEvaluate, mark) = evaluateSelectedParameters0(i.clone(), globalKnownVars.clone(), m.clone(), inIEqns.clone(), cache.clone(), graph.clone(), markarr.clone(), isInitial, repl.clone(), replEvaluate.clone(), mark)?;
    }
...
fn evaluateSelectedParameters0(mut i: i32, mut globalKnownVars: BackendDAE::Variables, mut m: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut inIEqns: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, mut cache: FCore::Cache, mut graph: FCore::Graph, mut markarr: metamodelica::Array<i32>, mut isInitial: bool, mut repl: BackendVarTransform::VariableReplacements, mut replEvaluate: BackendVarTransform::VariableReplacements, mut mark: i32) -> Result<(BackendDAE::Variables, FCore::Cache, BackendVarTransform::VariableReplacements, BackendVarTransform::VariableReplacements, i32)> {
    ...
    match '__try0: {
        let false = (intGt(({let __elt = markarr.borrow()[(i-1) as usize].clone(); __elt}), 0)) else { break '__try0 Err::<_, _>(anyhow::anyhow!("pattern mismatch")) };
        unwrap_break_err!(metamodelica::arrayUpdate(markarr.clone(), i, mark), '__try0);
        (globalKnownVars, cache, mark, repl, replEvaluate) = evaluateSelectedParameters1(({let __elt = m.borrow()[(i-1) as usize].clone(); __elt}), globalKnownVars.clone(), m.clone(), inIEqns.clone(), cache.clone(), graph.clone(), mark, markarr.clone(), isInitial, repl.clone(), replEvaluate.clone());
        v = unwrap_break_err!(BackendVariable::getVarAt(globalKnownVars.clone(), i), '__try0);
        (v, globalKnownVars, cache, mark, repl) = unwrap_break_err!(evaluateFixedAttribute(v.clone(), true, globalKnownVars.clone(), m.clone(), inIEqns.clone(), cache.clone(), graph.clone(), mark, markarr.clone(), isInitial, repl.clone()), '__try0);
        ...
        Err(_) => {
            v = BackendVariable::getVarAt(globalKnownVars.clone(), i)?;
            (globalKnownVars, repl, replEvaluate, cache) = evaluateSelectedParameter(v.clone(), i, globalKnownVars.clone(), inIEqns.clone(), repl.clone(), replEvaluate.clone(), cache.clone(), graph.clone())?;
        }
    }
    Ok((globalKnownVars, cache, repl, replEvaluate, mark))
}

fn evaluateSelectedParameters1(mut iUsed: Arc<metamodelica::List<i32>>, mut globalKnownVars: BackendDAE::Variables, mut m: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut inIEqns: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, mut cache: FCore::Cache, mut graph: FCore::Graph, mut mark: i32, mut markarr: metamodelica::Array<i32>, mut isInitial: bool, mut repl: BackendVarTransform::VariableReplacements, mut replEvaluate: BackendVarTransform::VariableReplacements) -> (BackendDAE::Variables, FCore::Cache, i32, BackendVarTransform::VariableReplacements, BackendVarTransform::VariableReplacements) {
    let mut globalKnownVars: BackendDAE::Variables = globalKnownVars;
    let mut cache: FCore::Cache = cache;
    let mut mark: i32 = mark;
    let mut repl: BackendVarTransform::VariableReplacements = repl;
        ...
        if let Ok((__v, __wb0, __wb1, __wb2, __wb3, __wb4)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: i, tail: rest } => {
                    ...
                    (globalKnownVars, cache, mark, repl, replEvaluate) = evaluateSelectedParameters1(({let __elt = m.borrow()[(i.clone()-1) as usize].clone(); __elt}), globalKnownVars.clone(), m.clone(), inIEqns.clone(), cache.clone(), graph.clone(), mark, markarr.clone(), isInitial, repl.clone(), replEvaluate.clone());
                    v = BackendVariable::getVarAt(globalKnownVars.clone(), i.clone())?;
                    (v, globalKnownVars, cache, mark, repl) = evaluateFixedAttribute(v.clone(), true, globalKnownVars.clone(), m.clone(), inIEqns.clone(), cache.clone(), graph.clone(), mark, markarr.clone(), isInitial, repl.clone())?;
                    ...
                    (globalKnownVars, cache, mark, repl, replEvaluate) = evaluateSelectedParameters1(rest.clone(), globalKnownVars.clone(), m.clone(), inIEqns.clone(), cache.clone(), graph.clone(), mark, markarr.clone(), isInitial, repl.clone(), replEvaluate.clone());
...
fn evaluateSelectedParameter(mut var: BackendDAE::Var, mut index: i32, mut globalKnownVars: BackendDAE::Variables, mut inIEqns: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, mut repl: BackendVarTransform::VariableReplacements, mut replEvaluate: BackendVarTransform::VariableReplacements, mut cache: FCore::Cache, mut graph: FCore::Graph) -> Result<(BackendDAE::Variables, BackendVarTransform::VariableReplacements, BackendVarTransform::VariableReplacements, FCore::Cache)> {
    ...
    let () = 'mc: {
        ...
        if let Ok((__v, __wb0, __wb1)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                BackendDAE::Var { varName: cr, varKind: BackendDAE::VarKind::CONST { .. }, bindExp: Some(e), .. } => {
                    let mut repl: BackendVarTransform::VariableReplacements = repl.clone();
                    let mut replEvaluate: BackendVarTransform::VariableReplacements = replEvaluate.clone();
        ...
        if let Ok((__v, __wb0, __wb1, __wb2)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                BackendDAE::Var { varName: cr, varKind: BackendDAE::VarKind::PARAM { .. }, bindExp: Some(e), .. } => {
                    let mut v: BackendDAE::Var;
                    let mut globalKnownVars: BackendDAE::Variables = globalKnownVars.clone();
                    let mut repl: BackendVarTransform::VariableReplacements = repl.clone();
                    let mut replEvaluate: BackendVarTransform::VariableReplacements = replEvaluate.clone();
        ...
        if let Ok((__v, __wb0, __wb1, __wb2, __wb3)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                BackendDAE::Var { varName: cr, varKind: BackendDAE::VarKind::PARAM { .. }, values: attr, .. } => {
                    let mut v: BackendDAE::Var;
                    let mut e: Arc<DAE::Exp>;
                    let mut e1: Arc<DAE::Exp>;
                    let mut value: Arc<Values::Value>;
...
fn evaluateFixedAttribute(mut var: BackendDAE::Var, mut addVar: bool, mut globalKnownVars: BackendDAE::Variables, mut m: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut inIEqns: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, mut cache: FCore::Cache, mut graph: FCore::Graph, mut mark: i32, mut markarr: metamodelica::Array<i32>, mut isInitial: bool, mut repl: BackendVarTransform::VariableReplacements) -> Result<(BackendDAE::Var, BackendDAE::Variables, FCore::Cache, i32, BackendVarTransform::VariableReplacements)> {
    ...
    (var, globalKnownVars, cache, mark, repl) = (::match_deref::match_deref! { match &(var.clone()) {
        ...
        BackendDAE::Var { varName: cr, values: attr @ Some(Deref @ DAE::VariableAttributes::VAR_ATTR_REAL { fixed: Some(e), .. }), source, .. } => {
            (var, globalKnownVars, cache, mark, repl) = evaluateFixedAttribute1(cr.clone(), e.clone(), attr.clone(), source.clone(), var, addVar, globalKnownVars, m.clone(), inIEqns, cache, graph, mark, markarr.clone(), isInitial, repl)?;
            (var, globalKnownVars, cache, mark, repl)
        },
        BackendDAE::Var { varName: cr, values: attr @ Some(Deref @ DAE::VariableAttributes::VAR_ATTR_INT { fixed: Some(e), .. }), source, .. } => {
            (var, globalKnownVars, cache, mark, repl) = evaluateFixedAttribute1(cr.clone(), e.clone(), attr.clone(), source.clone(), var, addVar, globalKnownVars, m.clone(), inIEqns, cache, graph, mark, markarr.clone(), isInitial, repl)?;
            (var, globalKnownVars, cache, mark, repl)
        },
        BackendDAE::Var { varName: cr, values: attr @ Some(Deref @ DAE::VariableAttributes::VAR_ATTR_BOOL { fixed: Some(e), .. }), source, .. } => {
            (var, globalKnownVars, cache, mark, repl) = evaluateFixedAttribute1(cr.clone(), e.clone(), attr.clone(), source.clone(), var, addVar, globalKnownVars, m.clone(), inIEqns, cache, graph, mark, markarr.clone(), isInitial, repl)?;
            (var, globalKnownVars, cache, mark, repl)
        },
        BackendDAE::Var { varName: cr, values: attr @ Some(Deref @ DAE::VariableAttributes::VAR_ATTR_ENUMERATION { fixed: Some(e), .. }), source, .. } => {
            (var, globalKnownVars, cache, mark, repl) = evaluateFixedAttribute1(cr.clone(), e.clone(), attr.clone(), source.clone(), var, addVar, globalKnownVars, m.clone(), inIEqns, cache, graph, mark, markarr.clone(), isInitial, repl)?;
            (var, globalKnownVars, cache, mark, repl)
        },
        _ => {
...
fn evaluateFixedAttribute1(mut cr: Arc<DAE::ComponentRef>, mut e: Arc<DAE::Exp>, mut attr: Option<Arc<DAE::VariableAttributes>>, mut source: Arc<DAE::ElementSource>, mut var: BackendDAE::Var, mut addVar: bool, mut globalKnownVars: BackendDAE::Variables, mut m: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut inIEqns: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, mut cache: FCore::Cache, mut graph: FCore::Graph, mut mark: i32, mut markarr: metamodelica::Array<i32>, mut isInitial: bool, mut repl: BackendVarTransform::VariableReplacements) -> Result<(BackendDAE::Var, BackendDAE::Variables, FCore::Cache, i32, BackendVarTransform::VariableReplacements)> {
    ...
    (e1, _) = BackendVarTransform::replaceExp(e.clone(), repl.clone(), None);
    let (_, (_, __pa0, _)) = Expression::traverseExpTopDown(e1.clone(), (std::sync::Arc::new(fnptr!(BackendDAEUtil::traversingadjacencyRowExpFinder, Arc<DAE::Exp>, (BackendDAE::Variables, Arc<AvlSetInt::Tree>, bool))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (BackendDAE::Variables, Arc<AvlSetInt::Tree>, bool)) -> Result<(Arc<DAE::Exp>, bool, (BackendDAE::Variables, Arc<AvlSetInt::Tree>, bool))> + 'static>), (globalKnownVars.clone(), crate::AvlSetInt::Tree::interned_EMPTY(), isInitial))?;
    ilst = __pa0.clone();
    (globalKnownVars, cache, mark, repl, _) = evaluateSelectedParameters1(AvlSetInt::listKeys(ilst, metamodelica::nil()), globalKnownVars, m.clone(), inIEqns, cache, graph, mark, markarr.clone(), isInitial, repl, BackendVarTransform::emptyReplacements());
    (e1, _) = BackendVarTransform::replaceExp(e1, repl.clone(), None);
    ...
    globalKnownVars = if (addVar) {BackendVariable::addVar(var.clone(), globalKnownVars)?} else {globalKnownVars};
    Ok((var, globalKnownVars, cache, mark, repl))
}
...
fn evaluateFixedAttributeReportWarning(mut b: bool, mut cr: Arc<DAE::ComponentRef>, mut e: Arc<DAE::Exp>, mut e1: Arc<DAE::Exp>, mut source: Arc<DAE::ElementSource>, mut globalKnownVars: BackendDAE::Variables) -> Result<Arc<DAE::Exp>> {
    ...
    if b {
        ...
        Error::addSourceMessage(Error::COMPILER_WARNING.clone(), list![(msg).clone()], info)?;
    }
    Ok(outExp)
}
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::CREF { componentRef: cr, .. }, (vars, b, hs)) => {
                    ...
                    let (__pa0, (_, __pa1, __pa2)) = Expression::traverseExpBottomUp(e.clone(), (std::sync::Arc::new(fnptr!(replaceCrefWithBindStartExp, Arc<DAE::Exp>, (BackendDAE::Variables, bool, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<Arc<DAE::ComponentRef>>>), i32, i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>))))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (BackendDAE::Variables, bool, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<Arc<DAE::ComponentRef>>>), i32, i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>)))) -> Result<(Arc<DAE::Exp>, (BackendDAE::Variables, bool, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<Arc<DAE::ComponentRef>>>), i32, i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>))))> + 'static>), (vars.clone(), b.clone(), hs.clone()))?;
                    e = __pa0.clone();
                    b = __pa1.clone();
                    hs = __pa2.clone();
...
fn traverseParameterSorted(mut inComps: Arc<metamodelica::List<Arc<metamodelica::List<i32>>>>, mut inGlobalKnownVars: BackendDAE::Variables, mut m: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut inIEqns: Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, mut iCache: FCore::Cache, mut graph: FCore::Graph, mut iMark: i32, mut markarr: metamodelica::Array<i32>, mut repl: BackendVarTransform::VariableReplacements, mut replEvaluate: BackendVarTransform::VariableReplacements, mut isInitial: bool) -> Result<(BackendDAE::Variables, BackendVarTransform::VariableReplacements, BackendVarTransform::VariableReplacements, FCore::Cache, i32)> {
    ...
    for mut ilst in &*inComps {
        ...
        for mut i in &*ilst.clone() {
            ...
            (v, oKnVars, oCache, oMark, oRepl) = evaluateFixedAttribute(v.clone(), true, oKnVars.clone(), m.clone(), inIEqns.clone(), oCache.clone(), graph.clone(), oMark, markarr.clone(), isInitial, oRepl.clone())?;
...
fn evaluateParameterBindings(mut var: BackendDAE::Var, mut index: i32, mut globalKnownVars: BackendDAE::Variables, mut cache: FCore::Cache, mut graph: FCore::Graph, mut repl: BackendVarTransform::VariableReplacements, mut replEvaluate: BackendVarTransform::VariableReplacements) -> (BackendDAE::Variables, BackendVarTransform::VariableReplacements, BackendVarTransform::VariableReplacements) {
    ...
    let () = 'mc: {
        ...
        if let Ok((__v, __wb0, __wb1, __wb2)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                v @ BackendDAE::Var { varName: cr, varKind: BackendDAE::VarKind::PARAM { .. }, bindExp: Some(e), hideResult: hideResultOpt, .. } => {
        ...
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
                    ...
                    let (__pa0, (__pa1, _)) = BackendDAEUtil::traverseBackendDAEVarAttr(v.values.clone(), (std::sync::Arc::new(fnptr!(traverseExpVisitorWrapper, Arc<DAE::Exp>, (BackendVarTransform::VariableReplacements, bool))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (BackendVarTransform::VariableReplacements, bool)) -> Result<(Arc<DAE::Exp>, (BackendVarTransform::VariableReplacements, bool))> + 'static>), (replEvaluate.clone(), false))?;
                    attr = __pa0.clone();
        ...
        if let Ok((__v, __wb0, __wb1, __wb2)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                v @ BackendDAE::Var { varName: cr, varKind: BackendDAE::VarKind::PARAM { .. }, values: attr, hideResult: hideResultOpt, .. } => {
                    let mut e: Arc<DAE::Exp>;
                    let mut exps: Arc<metamodelica::List<Arc<DAE::Exp>>> = metamodelica::nil();
                    let mut value: Arc<Values::Value> = Arc::new(Values::Value::META_FAIL);
                    let mut hideResultExp: Arc<DAE::Exp> = Arc::new(<DAE::Exp as ::std::default::Default>::default());
        ...
        if let Ok((__v, __wb0, __wb1)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                v @ BackendDAE::Var { bindExp: Some(e), hideResult: hideResultOpt, .. } => {
                    let mut exps: Arc<metamodelica::List<Arc<DAE::Exp>>> = metamodelica::nil();
                    let mut attr: Option<Arc<DAE::VariableAttributes>>;
                    let mut value: Arc<Values::Value> = Arc::new(Values::Value::META_FAIL);
                    let mut hideResultExp: Arc<DAE::Exp> = Arc::new(<DAE::Exp as ::std::default::Default>::default());
        ...
        if let Ok((__v, __wb0, __wb1)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                BackendDAE::Var { values: attr, hideResult: hideResultOpt, .. } => {
                    let mut v: BackendDAE::Var;
                    let mut hideResultExp: Arc<DAE::Exp> = Arc::new(<DAE::Exp as ::std::default::Default>::default());
                    let mut b: bool = false;
                    let mut attr = (*attr).clone();
...
fn traverseExpVisitorWrapper(mut inExp: Arc<DAE::Exp>, mut inTpl: (BackendVarTransform::VariableReplacements, bool)) -> (Arc<DAE::Exp>, (BackendVarTransform::VariableReplacements, bool)) {
    let mut outExp: Arc<DAE::Exp>;
...
fn replaceEvaluatedParametersSystem(mut isyst: Arc<BackendDAE::EqSystem>, mut inTypeA: (BackendDAE::Variables, metamodelica::Array<Arc<metamodelica::List<i32>>>, Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, FCore::Cache, FCore::Graph, i32, metamodelica::Array<i32>, bool, BackendVarTransform::VariableReplacements, BackendVarTransform::VariableReplacements)) -> Result<(Arc<BackendDAE::EqSystem>, (BackendDAE::Variables, metamodelica::Array<Arc<metamodelica::List<i32>>>, Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, FCore::Cache, FCore::Graph, i32, metamodelica::Array<i32>, bool, BackendVarTransform::VariableReplacements, BackendVarTransform::VariableReplacements))> {
    ...
    vars = __pa0.clone();
    (vars, outTypeA) = BackendVariable::traverseBackendDAEVarsWithUpdate(vars, (std::sync::Arc::new(replaceEvaluatedParameterTraverser) as std::sync::Arc<dyn ::std::ops::Fn(BackendDAE::Var, (BackendDAE::Variables, metamodelica::Array<Arc<metamodelica::List<i32>>>, Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, FCore::Cache, FCore::Graph, i32, metamodelica::Array<i32>, bool, BackendVarTransform::VariableReplacements, BackendVarTransform::VariableReplacements)) -> Result<(BackendDAE::Var, (BackendDAE::Variables, metamodelica::Array<Arc<metamodelica::List<i32>>>, Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, FCore::Cache, FCore::Graph, i32, metamodelica::Array<i32>, bool, BackendVarTransform::VariableReplacements, BackendVarTransform::VariableReplacements))> + 'static>), inTypeA)?;
...
fn replaceEvaluatedParameterTraverser(mut inVar: BackendDAE::Var, mut inTpl: (BackendDAE::Variables, metamodelica::Array<Arc<metamodelica::List<i32>>>, Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, FCore::Cache, FCore::Graph, i32, metamodelica::Array<i32>, bool, BackendVarTransform::VariableReplacements, BackendVarTransform::VariableReplacements)) -> Result<(BackendDAE::Var, (BackendDAE::Variables, metamodelica::Array<Arc<metamodelica::List<i32>>>, Arc<ExpandableArray::ExpandableArray<Arc<BackendDAE::Equation>>>, FCore::Cache, FCore::Graph, i32, metamodelica::Array<i32>, bool, BackendVarTransform::VariableReplacements, BackendVarTransform::VariableReplacements))> {
    ...
    (outVar, outTpl) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (v @ BackendDAE::Var { bindExp: Some(e), values: attr, .. }, (globalKnownVars, m, ieqns, cache, graph, mark, markarr, isInitial, repl, replEvaluate)) => {
                    let mut e1: Arc<DAE::Exp>;
                    let mut b: bool;
                    let mut v = (*v).clone();
                    let mut attr = (*attr).clone();
...
fn replaceEvaluatedParametersEqns(mut inDAE: Arc<BackendDAE::BackendDAE>, mut inRepl: BackendVarTransform::VariableReplacements) -> Result<Arc<BackendDAE::BackendDAE>> {
    ...
    (lsteqns, b) = BackendVarTransform::replaceEquations(lsteqns, inRepl.clone(), None)?;
    if b {
    ...
    systs = List::map1(systs, (std::sync::Arc::new(replaceEvaluatedParametersSystemEqns) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::EqSystem>, BackendVarTransform::VariableReplacements) -> Result<Arc<BackendDAE::EqSystem>> + 'static>), inRepl)?;
