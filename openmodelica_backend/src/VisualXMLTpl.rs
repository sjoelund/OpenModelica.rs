...
pub(crate) fn dumpVisXML1(mut txt: Tpl::Text, mut a_visArr: metamodelica::Array<VisualXML::Visualization>) -> Result<Tpl::Text> {
    ...
    l_visDump = lm_9(l_visDump, ret_1)?;
    l_visDump = Tpl::popIter(l_visDump)?;
    out_txt = Tpl::writeTok(txt, Arc::new(Tpl::StringToken::ST_STRING_LIST { strList: list![(literal!("<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>\n")).clone(), (literal!("<visualization>\n")).clone()], lastHasNewLine: true }))?;
    out_txt = Tpl::writeText(out_txt, l_visDump)?;
    out_txt = Tpl::softNewLine(out_txt)?;
    out_txt = Tpl::writeTok(out_txt, Arc::new(Tpl::StringToken::ST_STRING { value: (literal!("</visualization>")).clone() }))?;
    Ok(out_txt)
}
...
pub(crate) fn dumpVisualization(mut in_txt: Tpl::Text, mut in_a_vis: VisualXML::Visualization) -> Result<Tpl::Text> {
    let mut out_txt: Tpl::Text;
    out_txt = (::match_deref::match_deref! { match &((in_txt, in_a_vis)) {
        (txt, VisualXML::Visualization::SHAPE { shapeType: Deref @ DAE::Exp::SCONST { string: i_svalue }, T: i_T, r: i_r, r_shape: i_r__shape, lengthDir: i_lengthDir, widthDir: i_widthDir, color: i_color, ident: i_ident, length: i_length, width: i_width, height: i_height, extra: i_extra, specularCoeff: i_specularCoeff }) => {
            ...
            l_TDump = Tpl::pushIter(Tpl::emptyTxt.clone(), Arc::new(Tpl::IterOptions { startIndex0: 0, empty: None, separator: Some(openmodelica_tpl::Tpl::StringToken::interned_ST_NEW_LINE()), alignNum: 0, alignOfset: 0, alignSeparator: openmodelica_tpl::Tpl::StringToken::interned_ST_NEW_LINE(), wrapWidth: 0, wrapSeparator: openmodelica_tpl::Tpl::StringToken::interned_ST_NEW_LINE() }))?;
            l_TDump = lm_11(l_TDump, ret_1)?;
            l_TDump = Tpl::popIter(l_TDump)?;
            ret_3 = Arc::new(i_r.clone().borrow().iter().cloned().collect::<metamodelica::List<_>>());
            l_rDump = dumpVecExp(Tpl::emptyTxt.clone(), ret_3)?;
            ret_5 = Arc::new(i_r__shape.clone().borrow().iter().cloned().collect::<metamodelica::List<_>>());
            l_r__shapeDump = dumpVecExp(Tpl::emptyTxt.clone(), ret_5)?;
            ret_7 = Arc::new(i_lengthDir.clone().borrow().iter().cloned().collect::<metamodelica::List<_>>());
            l_lDirDump = dumpVecExp(Tpl::emptyTxt.clone(), ret_7)?;
            ret_9 = Arc::new(i_widthDir.clone().borrow().iter().cloned().collect::<metamodelica::List<_>>());
            l_wDirDump = dumpVecExp(Tpl::emptyTxt.clone(), ret_9)?;
            ret_11 = Arc::new(i_color.clone().borrow().iter().cloned().collect::<metamodelica::List<_>>());
            l_colorDump = dumpVecExp(Tpl::emptyTxt.clone(), ret_11)?;
            ...
            txt = Tpl::pushBlock(txt.clone(), Arc::new(Tpl::BlockType::BT_INDENT { width: 4 }))?;
            txt = Tpl::writeText(txt.clone(), l_colorDump)?;
            txt = Tpl::softNewLine(txt.clone())?;
            txt = Tpl::popBlock(txt.clone())?;
        ...
        (txt, VisualXML::Visualization::VECTOR { T: i_T, r: i_r, coordinates: i_coordinates, color: i_color, ident: i_ident, specularCoeff: i_specularCoeff, quantity: i_quantity, headAtOrigin: i_headAtOrigin, twoHeadedArrow: i_twoHeadedArrow }) => {
            ...
            txt = Tpl::writeStr(txt.clone(), (ret_18).clone())?;
            txt = Tpl::writeTok(txt.clone(), Arc::new(Tpl::StringToken::ST_STRING_LIST { strList: list![(literal!("</ident>\n")).clone(), (literal!("<T>\n")).clone()], lastHasNewLine: true }))?;
            txt = Tpl::pushBlock(txt.clone(), Arc::new(Tpl::BlockType::BT_INDENT { width: 4 }))?;
            txt = Tpl::writeText(txt.clone(), l_TDump)?;
            txt = Tpl::softNewLine(txt.clone())?;
            ...
            txt = Tpl::pushBlock(txt.clone(), Arc::new(Tpl::BlockType::BT_INDENT { width: 4 }))?;
            txt = Tpl::writeText(txt.clone(), l_coordDump)?;
            txt = Tpl::softNewLine(txt.clone())?;
            txt = Tpl::popBlock(txt.clone())?;
        ...
        (txt, VisualXML::Visualization::SURFACE { T: i_T, r_0: i_r__0, color: i_color, ident: i_ident, nu: i_nu, nv: i_nv, wireframe: i_wireframe, multiColored: i_multiColored, specularCoeff: i_specularCoeff, transparency: i_transparency }) => {
            ...
            txt = Tpl::writeStr(txt.clone(), (ret_23).clone())?;
            txt = Tpl::writeTok(txt.clone(), Arc::new(Tpl::StringToken::ST_STRING_LIST { strList: list![(literal!("</ident>\n")).clone(), (literal!("<T>\n")).clone()], lastHasNewLine: true }))?;
            txt = Tpl::pushBlock(txt.clone(), Arc::new(Tpl::BlockType::BT_INDENT { width: 4 }))?;
            txt = Tpl::writeText(txt.clone(), l_TDump)?;
            txt = Tpl::softNewLine(txt.clone())?;
            ...
            txt = Tpl::writeTok(txt.clone(), Arc::new(Tpl::StringToken::ST_STRING_LIST { strList: list![(literal!("</T>\n")).clone(), (literal!("<r>\n")).clone()], lastHasNewLine: true }))?;
            txt = Tpl::pushBlock(txt.clone(), Arc::new(Tpl::BlockType::BT_INDENT { width: 4 }))?;
            txt = Tpl::writeText(txt.clone(), l_r__0Dump)?;
            txt = Tpl::softNewLine(txt.clone())?;
            txt = Tpl::popBlock(txt.clone())?;
...
pub(crate) fn dumpVecExp(mut txt: Tpl::Text, mut a_vector: Arc<metamodelica::List<Arc<DAE::Exp>>>) -> Result<Tpl::Text> {
    ...
    l_vecDump = Tpl::pushIter(Tpl::emptyTxt.clone(), Arc::new(Tpl::IterOptions { startIndex0: 0, empty: None, separator: Some(openmodelica_tpl::Tpl::StringToken::interned_ST_NEW_LINE()), alignNum: 0, alignOfset: 0, alignSeparator: openmodelica_tpl::Tpl::StringToken::interned_ST_NEW_LINE(), wrapWidth: 0, wrapSeparator: openmodelica_tpl::Tpl::StringToken::interned_ST_NEW_LINE() }))?;
    l_vecDump = lm_15(l_vecDump, a_vector)?;
...
pub(crate) fn dumpExp(mut in_txt: Tpl::Text, mut in_a_expIn: Arc<DAE::Exp>) -> Result<Tpl::Text> {
    let mut out_txt: Tpl::Text;
    out_txt = (::match_deref::match_deref! { match &((in_txt, in_a_expIn)) {
        ...
        (txt, Deref @ DAE::Exp::CALL { expLst: i_expLst, path: i_path, .. }) => {
            ...
            let mut txt = (*txt).clone();
            l_elist = Tpl::pushIter(Tpl::emptyTxt.clone(), Arc::new(Tpl::IterOptions { startIndex0: 0, empty: None, separator: Some(openmodelica_tpl::Tpl::StringToken::interned_ST_NEW_LINE()), alignNum: 0, alignOfset: 0, alignSeparator: openmodelica_tpl::Tpl::StringToken::interned_ST_NEW_LINE(), wrapWidth: 0, wrapSeparator: openmodelica_tpl::Tpl::StringToken::interned_ST_NEW_LINE() }))?;
            l_elist = lm_17(l_elist, i_expLst.clone())?;
            ...
            txt = Tpl::writeText(txt.clone(), l_elist)?;
