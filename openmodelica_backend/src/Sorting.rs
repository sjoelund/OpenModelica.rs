...
pub fn Tarjan(mut m: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut ass1: metamodelica::Array<i32>, mut N: i32) -> Result<Arc<metamodelica::List<Arc<metamodelica::List<i32>>>>> {
    ...
    for mut var in 1..=metamodelica::arrayLength(ass1.clone()) {
        ...
        if eqn > 0 && ({let __elt = number.borrow()[(eqn-1) as usize].clone(); __elt}) == -1 {
            (stack, index, outComponents) = StrongConnect(m.clone(), ass1.clone(), eqn, stack.clone(), index, number.clone(), lowlink.clone(), onStack.clone(), outComponents.clone())?;
        }
    }
...
fn StrongConnect(mut m: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut ass1: metamodelica::Array<i32>, mut eqn: i32, mut stack: Arc<metamodelica::List<i32>>, mut index: i32, mut number: metamodelica::Array<i32>, mut lowlink: metamodelica::Array<i32>, mut onStack: metamodelica::Array<bool>, mut inComponents: Arc<metamodelica::List<Arc<metamodelica::List<i32>>>>) -> Result<(Arc<metamodelica::List<i32>>, i32, Arc<metamodelica::List<Arc<metamodelica::List<i32>>>>)> {
    ...
    let mut eqn2: i32 = 0;
    metamodelica::arrayUpdate(number.clone(), eqn, outIndex)?;
    metamodelica::arrayUpdate(lowlink.clone(), eqn, outIndex)?;
    metamodelica::arrayUpdate(onStack.clone(), eqn, true)?;
    outIndex = outIndex + 1;
    outStack = metamodelica::cons(eqn, outStack);
    for mut eqn2 in &*Matching::incomingEquations(eqn, m.clone(), ass1.clone()) {
        let mut eqn2 = eqn2.clone();
        if ({let __elt = number.borrow()[(eqn2-1) as usize].clone(); __elt}) == -1 {
            (outStack, outIndex, outComponents) = StrongConnect(m.clone(), ass1.clone(), eqn2, outStack.clone(), outIndex, number.clone(), lowlink.clone(), onStack.clone(), outComponents.clone())?;
    ...
    if ({let __elt = lowlink.borrow()[(eqn-1) as usize].clone(); __elt}) == ({let __elt = number.borrow()[(eqn-1) as usize].clone(); __elt}) {
        let (__pa0, __pa1) = ::match_deref::match_deref! { match &(outStack) {
...
pub fn TarjanTransposed(mut mT: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut ass2: metamodelica::Array<i32>) -> Result<Arc<metamodelica::List<Arc<metamodelica::List<i32>>>>> {
    ...
    for mut eqn in 1..=N {
        if ({let __elt = number.borrow()[(eqn.clone()-1) as usize].clone(); __elt}) == -1 && ({let __elt = ass2.borrow()[(eqn.clone()-1) as usize].clone(); __elt}) > 0 {
            (stack, index, outComponents) = StrongConnectTransposed(mT.clone(), ass2.clone(), eqn.clone(), stack.clone(), index, number.clone(), lowlink.clone(), onStack.clone(), outComponents.clone())?;
        }
    }
    Ok(outComponents)
}
...
fn StrongConnectTransposed(mut mT: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut ass2: metamodelica::Array<i32>, mut eqn: i32, mut stack: Arc<metamodelica::List<i32>>, mut index: i32, mut number: metamodelica::Array<i32>, mut lowlink: metamodelica::Array<i32>, mut onStack: metamodelica::Array<bool>, mut inComponents: Arc<metamodelica::List<Arc<metamodelica::List<i32>>>>) -> Result<(Arc<metamodelica::List<i32>>, i32, Arc<metamodelica::List<Arc<metamodelica::List<i32>>>>)> {
    ...
    if var > 0 {
        let __range0 = &*({let __elt = mT.borrow()[(var-1) as usize].clone(); __elt});
        for mut eqn2 in __range0 {
            let mut eqn2 = eqn2.clone();
            if eqn2 > 0 && eqn2 != eqn {
                if ({let __elt = number.borrow()[(eqn2-1) as usize].clone(); __elt}) == -1 {
                    (outStack, outIndex, outComponents) = StrongConnectTransposed(mT.clone(), ass2.clone(), eqn2, outStack.clone(), outIndex, number.clone(), lowlink.clone(), onStack.clone(), outComponents.clone())?;
                    metamodelica::arrayUpdate(lowlink.clone(), eqn, intMin(({let __elt = lowlink.borrow()[(eqn-1) as usize].clone(); __elt}), ({let __elt = lowlink.borrow()[(eqn2-1) as usize].clone(); __elt})))?;
                } else if ({let __elt = onStack.borrow()[(eqn2-1) as usize].clone(); __elt}) {
                    metamodelica::arrayUpdate(lowlink.clone(), eqn, intMin(({let __elt = lowlink.borrow()[(eqn-1) as usize].clone(); __elt}), ({let __elt = number.borrow()[(eqn2-1) as usize].clone(); __elt})))?;
                }
    ...
    if ({let __elt = lowlink.borrow()[(eqn-1) as usize].clone(); __elt}) == ({let __elt = number.borrow()[(eqn-1) as usize].clone(); __elt}) {
        let (__pa1, __pa2) = ::match_deref::match_deref! { match &(outStack) {
