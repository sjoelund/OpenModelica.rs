...
pub type Value = Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<BackendDAE::Equation>)>>;

pub type HashTableCrefFunctionsType = (FuncHashCref, FuncCrefEqual, FuncCrefStr, FuncExpStr);

pub type HashTable = (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<BackendDAE::Equation>)>>)>>), i32, (FuncHashCref, FuncCrefEqual, FuncCrefStr, FuncExpStr));
...
pub fn emptyHashTableSized(mut size: i32) -> HashTable {
    let mut hashTable: HashTable;
    hashTable = BaseHashTable::emptyHashTableWork(size.clone(), (ComponentReference::hashComponentRef, ComponentReferenceBasics::crefEqual, ComponentReferenceBasics::printComponentRefStr, fnptr!(printTupleComponentRefEqListStr, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<BackendDAE::Equation>)>>)));
    hashTable
}

pub fn printTupleComponentRefEqListStr(mut cr_eq_lst: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<BackendDAE::Equation>)>>) -> ArcStr {
    let mut res: ArcStr = arcstr::literal!("");
    res = stringDelimitList(List::map(cr_eq_lst.clone(), Arc::new(printTupleComponentRefEqStr)), (literal!(",")).clone());
    res
}
