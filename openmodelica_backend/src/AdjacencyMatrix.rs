...
pub fn traverseAdjacencyMatrix<T: Clone + 'static + metamodelica::gc::MMTrace>(mut inM: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut func: Arc<dyn ::std::ops::Fn(Arc<metamodelica::List<i32>>, i32, T) -> Result<(Arc<metamodelica::List<i32>>, T)> + 'static>, mut inTypeA: T) -> Result<(metamodelica::Array<Arc<metamodelica::List<i32>>>, T)> {
...
fn traverseAdjacencyMatrix2<T: Clone + 'static + metamodelica::gc::MMTrace>(mut inM: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut func: Arc<dyn ::std::ops::Fn(Arc<metamodelica::List<i32>>, i32, T) -> Result<(Arc<metamodelica::List<i32>>, T)> + 'static>, mut pos: i32, mut len: i32, mut stop: bool, mut inTypeA: T) -> Result<(metamodelica::Array<Arc<metamodelica::List<i32>>>, T)> {
    ...
    '__tco: loop {
        match stop.clone() {
        true => {
            return Ok((inM.clone(), inTypeA.clone()))
        },
        false => {
            let mut m1: metamodelica::Array<Arc<metamodelica::List<i32>>> = Default::default();
            let mut m2: metamodelica::Array<Arc<metamodelica::List<i32>>> = Default::default();
            let mut extArg: T;
            let mut extArg1: T;
            let mut extArg2: T;
            ...
            (m1, extArg1) = traverseAdjacencyMatrixList(eqns1.clone(), inM.clone(), func.clone(), metamodelica::arrayLength(inM.clone()), pos.clone(), extArg.clone())?;
            { (inM, func, pos, len, stop, inTypeA) = (m1.clone(), func.clone(), pos.clone() + 1, len.clone(), intGt(pos.clone() + 1, len.clone()), extArg1.clone()); continue '__tco; }
        },
    }
    }
}
...
pub fn getOtherEqSysAdjacencyMatrix(mut m: metamodelica::Array<Arc<metamodelica::List<i32>>>, mut size: i32, mut index: i32, mut skip: metamodelica::Array<i32>, mut rowskip: metamodelica::Array<i32>, mut mnew: metamodelica::Array<Arc<metamodelica::List<i32>>>) -> Result<metamodelica::Array<Arc<metamodelica::List<i32>>>> {
    '__tco: loop {
        ...
        for mut r in (({let __elt = m.borrow()[(index.clone()-1) as usize].clone(); __elt})).into_iter().cloned() {
