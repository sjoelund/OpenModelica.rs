...
// collect for-loops
//--------------------------------
pub(crate) fn collectForLoops(mut varsIn: Arc<metamodelica::List<BackendDAE::Var>>, mut eqsIn: Arc<metamodelica::List<Arc<BackendDAE::Equation>>>) -> Result<(Arc<metamodelica::List<BackendDAE::Var>>, Arc<metamodelica::List<Arc<BackendDAE::Equation>>>)> {
    ...
    (forEqs, mixEqs, nonArrEqs) = List::fold1(eqsIn, (std::sync::Arc::new(dispatchLoopEquations) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::Equation>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, (Arc<metamodelica::List<Arc<BackendDAE::Equation>>>, Arc<metamodelica::List<Arc<BackendDAE::Equation>>>, Arc<metamodelica::List<Arc<BackendDAE::Equation>>>)) -> Result<(Arc<metamodelica::List<Arc<BackendDAE::Equation>>>, Arc<metamodelica::List<Arc<BackendDAE::Equation>>>, Arc<metamodelica::List<Arc<BackendDAE::Equation>>>)> + 'static>), List::map(arrayCrefs, std::sync::Arc::new(fnptr!(Util::tuple31, _)))?, (metamodelica::nil(), metamodelica::nil(), metamodelica::nil()))?;
    forEqs = buildBackendDAEForEquations(forEqs, metamodelica::nil());
    mixEqs = List::fold(mixEqs, (std::sync::Arc::new(fnptr!(buildAccumExpInEquations, Arc<BackendDAE::Equation>, Arc<metamodelica::List<Arc<BackendDAE::Equation>>>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::Equation>, Arc<metamodelica::List<Arc<BackendDAE::Equation>>>) -> Result<Arc<metamodelica::List<Arc<BackendDAE::Equation>>>> + 'static>), metamodelica::nil())?.reverse();
...
fn buildAccumExpInEquations1(mut termIn: Arc<DAE::Exp>, mut minmaxTermsIn: Arc<metamodelica::List<(Arc<DAE::Exp>, i32, i32)>>) -> Arc<metamodelica::List<(Arc<DAE::Exp>, i32, i32)>> {
    ...
    match '__try0: {
        ...
        if intEq(pos, -1) {
            ...
            (term, min, max) = unwrap_break_err!((minmaxTermsIn.clone()).get(pos), '__try0);
...
pub(crate) fn replaceSubscriptInCrefExp(mut expIn: Arc<DAE::Exp>, mut subsIn: Arc<metamodelica::List<Arc<DAE::Subscript>>>) -> (Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Subscript>>>) {
    let mut expOut: Arc<DAE::Exp>;
    let mut subsOut: Arc<metamodelica::List<Arc<DAE::Subscript>>>;
    (expOut, subsOut) = (::match_deref::match_deref! { match &(expIn.clone()) {
        Deref @ DAE::Exp::CREF { componentRef: cref, ty } => {
            let mut cref = (*cref).clone();
            cref = replaceFirstSubsInCref(cref.clone(), subsIn.clone());
            (Arc::new(DAE::Exp::CREF { componentRef: cref.clone(), ty: ty.clone() }), subsIn)
        },
        _ => {
            (expIn, subsIn)
        },
...
pub(crate) fn equationEqualNoCrefSubs(mut e1: Arc<BackendDAE::Equation>, mut e2: Arc<BackendDAE::Equation>) -> bool {
...
pub(crate) fn expEqualNoCrefSubs(mut inExp1: Arc<DAE::Exp>, mut inExp2: Arc<DAE::Exp>) -> Result<bool> {
...
fn expEqualNoCrefSubsList(mut inExpl1: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut inExpl2: Arc<metamodelica::List<Arc<DAE::Exp>>>) -> Result<bool> {
    ...
    if (inExpl1.clone().len() as i32) != (inExpl2.len() as i32) {
        outEqual = false;
        return Ok(outEqual.clone());
    }
...
fn addToArrayCrefLst(mut tplLstIn: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, mut varIn: BackendDAE::Var, mut tplRef: (Arc<DAE::ComponentRef>, i32, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>), mut tplLstFoldIn: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, mut varLstIn: Arc<metamodelica::List<BackendDAE::Var>>) -> Result<(Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>)>>, Arc<metamodelica::List<BackendDAE::Var>>)> {
    ...
    (tplLstFoldOut, varLstOut) = 'mc: {
        let __mc_input = (tplLstIn, tplRef.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
fn crefPartlyEqualToCrefs(mut cref0: Arc<DAE::ComponentRef>, mut crefLst: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>) -> Result<bool> {
    let mut b: bool;
    b = List::exist1(crefLst, (std::sync::Arc::new(fnptr!(crefPartlyEqual, Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>), cref0)?;
    Ok(b)
}

fn crefPartlyEqual(mut cref0: Arc<DAE::ComponentRef>, mut cref1: Arc<DAE::ComponentRef>) -> bool {
    let mut partlyEq: bool;
    partlyEq = (::match_deref::match_deref! { match &((cref0.clone(), cref1.clone())) {
        (Deref @ DAE::ComponentRef::CREF_IDENT { .. }, Deref @ DAE::ComponentRef::CREF_IDENT { .. }) => {
            var_field!((*cref0).ident, DAE::ComponentRef::CREF_IDENT).clone() == var_field!((*cref1).ident, DAE::ComponentRef::CREF_IDENT).clone()
        },
        ...
        (Deref @ DAE::ComponentRef::CREF_IDENT { .. }, Deref @ DAE::ComponentRef::CREF_QUAL { .. }) => {
            var_field!((*cref0).ident, DAE::ComponentRef::CREF_IDENT).clone() == var_field!((*cref1).ident, DAE::ComponentRef::CREF_QUAL).clone()
        },
        _ => {
            false
        },
...
pub(crate) fn reduceLoopExpressions(mut expIn: Arc<DAE::Exp>, mut maxSub: i32) -> (Arc<DAE::Exp>, bool) {
    let mut expOut: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Exp::CREF { componentRef: cref, .. } => {
                    let mut b: bool;
                    b = intLe(getIndexSubScript(listHead(ComponentReferenceBasics::crefSubs(cref.clone())?)?)?, maxSub);
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Exp::BINARY { exp1, operator: op, exp2 } => {
                    ...
                    let mut exp1 = (*exp1).clone();
                    let mut exp2 = (*exp2).clone();
                    (exp1, b1) = reduceLoopExpressions(exp1.clone(), maxSub);
                    (exp2, b2) = reduceLoopExpressions(exp2.clone(), maxSub);
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Exp::UNARY { exp, .. } => {
                    ...
                    (exp, b) = reduceLoopExpressions(exp.clone(), maxSub);
...
pub(crate) fn insertSUMexp(mut expIn: Arc<DAE::Exp>, mut tplIn: (Arc<DAE::ComponentRef>, Arc<DAE::Exp>)) -> (Arc<DAE::Exp>, (Arc<DAE::ComponentRef>, Arc<DAE::Exp>)) {
    let mut expOut: Arc<DAE::Exp>;
...
pub(crate) fn replaceFirstSubsInCref(mut crefIn: Arc<DAE::ComponentRef>, mut subs: Arc<metamodelica::List<Arc<DAE::Subscript>>>) -> Arc<DAE::ComponentRef> {
    ...
    crefOut = (::match_deref::match_deref! { match &(crefIn.clone()) {
        Deref @ DAE::ComponentRef::CREF_QUAL { ident, identType, subscriptLst, componentRef: cref } => {
            ...
            cref = replaceFirstSubsInCref(cref.clone(), subs);
            Arc::new(DAE::ComponentRef::CREF_QUAL { ident: (ident.clone()).clone(), identType: identType.clone(), subscriptLst: subscriptLst.clone(), componentRef: cref.clone() })
        },
        Deref @ DAE::ComponentRef::CREF_IDENT { ident, identType, subscriptLst } => {
            ...
            Arc::new(DAE::ComponentRef::CREF_IDENT { ident: (ident.clone()).clone(), identType: identType.clone(), subscriptLst: subscriptLst.clone() })
        },
        _ => {
            crefIn
        },
