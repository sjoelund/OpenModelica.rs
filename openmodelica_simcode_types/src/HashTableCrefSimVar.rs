...
use openmodelica_util::Error::addInternalError;
...
pub type HashTable = (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, SimCodeVar::SimVar)>>), i32, (FuncHashCref, FuncCrefEqual, FuncCrefStr, FuncExpStr));
...
pub fn addSimVarToHashTable(mut simvarIn: SimCodeVar::SimVar, mut inHT: HashTable) -> Result<HashTable> {
    ...
    outHT = 'mc: {
        let __mc_input = simvarIn;
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                sv @ SimCodeVar::SimVar { name: cr, arrayCref: None, .. } => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                sv @ SimCodeVar::SimVar { name: cr, arrayCref: Some(acr), .. } => {
                    let mut outHT: (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, SimCodeVar::SimVar)>>), i32, (FuncHashCref, FuncCrefEqual, FuncCrefStr, FuncExpStr));
