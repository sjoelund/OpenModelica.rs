...
pub(crate) fn keyStr(mut inKey: Key) -> Result<ArcStr> {
    ...
    outString = (ComponentReferenceBasics::printComponentRefStr(inKey)?).clone();
    Ok(outString)
}

pub(crate) fn valueStr(mut inValue: Value) -> ArcStr {
    let mut outString: ArcStr;
    outString = ArcStr::from(::std::format!("{}", inValue));
    outString
}
    ...
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        ...
        Deref @ Tree::NODE { key, .. } => {
            ...
            if key_comp == -1 {
                ...
                if !(((var_field!((*tree).value, Tree::NODE).clone()) == (value))) {
        ...
        Deref @ Tree::LEAF { .. } => {
            ...
            if key_comp == -1 {
                outTree = Arc::new(Tree::NODE { key: var_field!((*tree).key, Tree::LEAF).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: Arc::new(Tree::LEAF { key: inKey, value: inValue }), right: crate::AvlTreeCRToInt::Tree::interned_EMPTY() });
            } else if key_comp == 1 {
                outTree = Arc::new(Tree::NODE { key: var_field!((*tree).key, Tree::LEAF).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: crate::AvlTreeCRToInt::Tree::interned_EMPTY(), right: Arc::new(Tree::LEAF { key: inKey, value: inValue }) });
            } else {
                value = conflictFunc(inValue, var_field!((*tree).value, Tree::LEAF).clone(), var_field!((*tree).key, Tree::LEAF).clone())?;
                if !(((var_field!((*tree).value, Tree::LEAF).clone()) == (value))) {
...
pub(crate) fn addList(mut tree: Arc<Tree>, mut inValues: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>, mut conflictFunc: Arc<dyn ::std::ops::Fn(i32, i32, Arc<DAE::ComponentRef>) -> Result<i32> + 'static>) -> Result<Arc<Tree>> {
    ...
    for mut t in &*inValues {
        let mut t = t.clone();
        (key, value) = t.clone();
        tree = add(tree.clone(), key.clone(), value, conflictFunc.clone())?;
    }
    Ok(tree)
}

pub(crate) fn addUpdate(mut tree: Arc<Tree>, mut key: Key, mut r#fn: Arc<dyn ::std::ops::Fn(Option<i32>) -> Result<i32> + 'static>) -> Result<Arc<Tree>> {
    ...
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        ...
        Deref @ Tree::LEAF { .. } => {
            ...
            if key_comp == -1 {
                new_tree = Arc::new(Tree::NODE { key: var_field!((*tree).key, Tree::LEAF).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: Arc::new(Tree::LEAF { key: key, value: r#fn(None)? }), right: crate::AvlTreeCRToInt::Tree::interned_EMPTY() });
            } else if key_comp == 1 {
                new_tree = Arc::new(Tree::NODE { key: var_field!((*tree).key, Tree::LEAF).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: crate::AvlTreeCRToInt::Tree::interned_EMPTY(), right: Arc::new(Tree::LEAF { key: key, value: r#fn(None)? }) });
            } else {
...
pub(crate) fn fold<FT: Clone + 'static + metamodelica::gc::MMTrace>(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, i32, FT) -> Result<FT> + 'static>, mut inStartValue: FT) -> Result<FT> {
...
pub(crate) fn foldCond<FT: Clone + 'static + metamodelica::gc::MMTrace>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, i32, FT) -> Result<(FT, bool)> + 'static>, mut value: FT) -> Result<FT> {
...
pub(crate) fn fold_2<FT1: Clone + 'static + metamodelica::gc::MMTrace, FT2: Clone + 'static + metamodelica::gc::MMTrace>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, i32, FT1, FT2) -> Result<(FT1, FT2)> + 'static>, mut foldArg1: FT1, mut foldArg2: FT2) -> Result<(FT1, FT2)> {
...
pub(crate) fn forEach(mut tree: Arc<Tree>, mut func: Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, i32) -> Result<()> + 'static>) -> Result<()> {
...
pub(crate) fn fromList(mut inValues: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>, mut conflictFunc: Arc<dyn ::std::ops::Fn(i32, i32, Arc<DAE::ComponentRef>) -> Result<i32> + 'static>) -> Result<Arc<Tree>> {
    let mut tree: Arc<Tree> = crate::AvlTreeCRToInt::Tree::interned_EMPTY();
    let mut key: Key;
    let mut value: Value;
    for mut t in &*inValues {
        let mut t = t.clone();
...
pub(crate) fn getOpt(mut tree: Arc<Tree>, mut key: Key) -> Result<Option<i32>> {
    '__tco: loop {
        let mut k: Key;
        k = (::match_deref::match_deref! { match &(tree.clone()) {
        Deref @ Tree::NODE { .. } => var_field!((*tree).key, Tree::NODE).clone(),
...
pub(crate) fn join(mut tree: Arc<Tree>, mut treeToJoin: Arc<Tree>, mut conflictFunc: Arc<dyn ::std::ops::Fn(i32, i32, Arc<DAE::ComponentRef>) -> Result<i32> + 'static>) -> Result<Arc<Tree>> {
    let mut tree: Arc<Tree> = tree;
...
pub(crate) fn listKeys(mut tree: Arc<Tree>, mut lst: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>) -> Arc<metamodelica::List<Arc<DAE::ComponentRef>>> {
...
pub(crate) fn listKeysReverse(mut inTree: Arc<Tree>, mut lst: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>) -> Arc<metamodelica::List<Arc<DAE::ComponentRef>>> {
    let mut lst: Arc<metamodelica::List<Arc<DAE::ComponentRef>>> = lst;
...
pub(crate) fn listValues(mut tree: Arc<Tree>, mut lst: Arc<metamodelica::List<i32>>) -> Arc<metamodelica::List<i32>> {
...
pub(crate) fn map(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, i32) -> Result<i32> + 'static>) -> Result<Arc<Tree>> {
    ...
    outTree = (::match_deref::match_deref! { match &(outTree.clone()) {
        Deref @ Tree::NODE { key, value, .. } => {
            ...
            if !(referenceEq(&*(new_left.clone()),&*(var_field!((*outTree).left, Tree::NODE).clone()))) || !(((value.clone()) == (new_value))) || !(referenceEq(&*(new_right.clone()),&*(var_field!((*outTree).right, Tree::NODE).clone()))) {
...
pub(crate) fn mapFold<FT: Clone + 'static + metamodelica::gc::MMTrace>(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, i32, FT) -> Result<(i32, FT)> + 'static>, mut inStartValue: FT) -> Result<(Arc<Tree>, FT)> {
...
pub(crate) fn new() -> Arc<Tree> {
    let mut outTree: Arc<Tree> = crate::AvlTreeCRToInt::Tree::interned_EMPTY();
    outTree
}
...
fn rotateLeft(mut inNode: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    outNode = (::match_deref::match_deref! { match &(outNode.clone()) {
        ...
        Deref @ Tree::NODE { right: child @ Deref @ Tree::LEAF { .. }, .. } => {
            let mut node: Arc<Tree>;
            node = setTreeLeftRight(outNode.clone(), var_field!((*outNode).left, Tree::NODE).clone(), crate::AvlTreeCRToInt::Tree::interned_EMPTY())?;
            setTreeLeftRight(child.clone(), node, crate::AvlTreeCRToInt::Tree::interned_EMPTY())?
        },
        _ => {
            inNode
        },
...
fn rotateRight(mut inNode: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    outNode = (::match_deref::match_deref! { match &(outNode.clone()) {
        ...
        Deref @ Tree::NODE { left: child @ Deref @ Tree::LEAF { .. }, .. } => {
            let mut node: Arc<Tree>;
            node = setTreeLeftRight(outNode.clone(), crate::AvlTreeCRToInt::Tree::interned_EMPTY(), var_field!((*outNode).right, Tree::NODE).clone())?;
            setTreeLeftRight(child.clone(), crate::AvlTreeCRToInt::Tree::interned_EMPTY(), node)?
        },
        _ => {
            inNode
        },
...
pub(crate) fn toList(mut inTree: Arc<Tree>, mut lst: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>) -> Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>> {
...
pub(crate) fn update(mut tree: Arc<Tree>, mut key: Key, mut value: Value) -> Result<Arc<Tree>> {
    let mut outTree: Arc<Tree> = add(tree.clone(), key.clone(), value, (std::sync::Arc::new(fnptr!(addConflictReplace, i32, i32, Arc<DAE::ComponentRef>)) as std::sync::Arc<dyn ::std::ops::Fn(i32, i32, Arc<DAE::ComponentRef>) -> Result<i32> + 'static>))?;
    Ok(outTree)
}
