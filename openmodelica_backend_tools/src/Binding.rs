...
pub fn inferBindings(mut model_path: Path, mut env: Absyn::Program) -> Result<Absyn::Program> {
    ...
    client_list = buildInstList(model_def.clone(), env.clone(), crate::Binding::Client_e::interned_NO_PRED(), ms, metamodelica::nil(), metamodelica::nil())?;
    out_vmodel = inferBindingClientList(client_list, model_def, env.clone())?;
    ...
    out_model_def = ProgramUtil::updateProgram(Absyn::Program { classes: list![out_vmodel], within_: ProgramUtil::buildWithin(model_path)? }, env, false)?;
    Ok(out_model_def)
}
...
pub fn generateVerificationScenarios(mut package_path: Path, mut in_env: Absyn::Program) -> Result<Absyn::Program> {
    ...
    out_env = ProgramUtil::updateProgram(Absyn::Program { classes: list![out_vmodel], within_: ProgramUtil::buildWithin(package_path)? }, in_env, false)?;
    Ok(out_env)
}
...
pub(crate) fn updatePackage(mut in_class: Arc<Absyn::Class>, mut ag_elems: Arc<metamodelica::List<Arc<Absyn::ElementItem>>>) -> Result<Arc<Absyn::Class>> {
    ...
    out_class = (::match_deref::match_deref! { match &(in_class) {
        __esc_out_class @ Deref @ Absyn::Class { body: Deref @ Absyn::ClassDef::PARTS { typeVars, classAttrs, classParts: _, ann, comment }, .. } => {
            ...
            assign_field!(out_class.body = Arc::new(Absyn::ClassDef::PARTS { typeVars: typeVars.clone(), classAttrs: classAttrs.clone(), classParts: list![Arc::new(Absyn::ClassPart::PUBLIC { contents: ag_elems })], ann: ann.clone(), comment: comment.clone() }));
            out_class.clone()
        },
...
fn getAllElementsOfType(mut element_defs: Arc<metamodelica::List<Arc<SCode::Element>>>, mut typeName: Ident, mut pathInProg: ArcStr, mut elements_in: Arc<metamodelica::List<(Arc<SCode::Element>, ArcStr)>>) -> Arc<metamodelica::List<(Arc<SCode::Element>, ArcStr)>> {
    '__tco: loop {
        ::match_deref::match_deref! { match &(element_defs) {
        Deref @ metamodelica::List::Nil => {
            return elements_in
        ...
        Deref @ metamodelica::List::Cons { head: el, tail: rest } => {
            let mut m: Arc<metamodelica::List<(Arc<SCode::Element>, ArcStr)>>;
            m = listAppend(getAllElementsOfType2(el.clone(), (typeName.clone()).clone(), (pathInProg.clone()).clone()), elements_in);
            { (element_defs, typeName, pathInProg, elements_in) = (rest.clone(), (typeName).clone(), (pathInProg).clone(), m); continue '__tco; }
        },
...
fn getAllElementsOfType2(mut el: Arc<SCode::Element>, mut typeName: Ident, mut pathInProg: ArcStr) -> Arc<metamodelica::List<(Arc<SCode::Element>, ArcStr)>> {
...
fn isOfType(mut elems: Arc<metamodelica::List<Arc<SCode::Element>>>, mut typeName: ArcStr) -> bool {
    let mut result: bool;
    result = 'mc: {
...
fn inferBindingClientList(mut client_list: Arc<metamodelica::List<Arc<Client_e>>>, mut vmodel: Arc<Absyn::Class>, mut env: Absyn::Program) -> Result<Arc<Absyn::Class>> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: ce, tail: rest } => {
            ...
            upd_vmodel = inferBindingClient(ce.clone(), vmodel, env.clone())?;
            { (client_list, vmodel, env) = (rest.clone(), upd_vmodel, env); continue '__tco; }
        },
        _ => return Err(anyhow::anyhow!("match: no arm matched")),
    } }
    }
    ...
    out_vmodel = (::match_deref::match_deref! { match &(client_e) {
        Deref @ Client_e::CLIENT_E { components: _, typeSpec, rootType, def: _, instance: iname, predecessors: _, mediator: Deref @ metamodelica::List::Cons { head: Mediator { mType: _, template, clients: _, providers, preferred: Deref @ metamodelica::List::Nil }, tail: _ } } => {
            ...
            if template.clone() == literal!("") {
                out_class = updateClass(vmodel, typeSpec.clone(), rootType.clone(), out_es, iname.clone(), env, false, metamodelica::nil(), (literal!("")).clone())?;
            } else {
                ...
                exp = __pa0.clone();
                new_exp = parseAggregator(exp, Arc::new(Absyn::FunctionArgs::FUNCTIONARGS { args: list![Arc::new(Absyn::Exp::LIST { exps: toExpList(out_es, metamodelica::nil()) })], argNames: metamodelica::nil() }));
                out_class = updateClass(vmodel, typeSpec.clone(), rootType.clone(), list![(new_exp, literal!(""))], iname.clone(), env, false, metamodelica::nil(), (literal!("")).clone())?;
            }
            out_class
        },
        Deref @ Client_e::CLIENT_E { components: _, typeSpec, rootType, def: _, instance: iname, predecessors: _, mediator: Deref @ metamodelica::List::Cons { head: Mediator { mType: _, template: _, clients: _, providers, preferred }, tail: _ } } => {
            ...
            let mut out_class: Arc<Absyn::Class>;
            out_es = getProviders(providers.clone(), vmodel.clone(), env.clone(), metamodelica::nil())?;
            out_class = updateClass(vmodel, typeSpec.clone(), rootType.clone(), out_es, iname.clone(), env, true, preferred.clone(), (literal!("")).clone())?;
            out_class
        },
...
pub(crate) fn toExpList(mut e_list: Arc<metamodelica::List<(Arc<Absyn::Exp>, ArcStr)>>, mut in_es: Arc<metamodelica::List<Arc<Absyn::Exp>>>) -> Arc<metamodelica::List<Arc<Absyn::Exp>>> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: (exp, _), tail: rest } => {
            { (e_list, in_es) = (rest.clone(), metamodelica::cons(exp.clone(), in_es)); continue '__tco; }
        },
        _ => unreachable!("tail-call lowered match: no arm matched"),
    } }
    }
...
pub(crate) fn updateClass(mut in_class: Arc<Absyn::Class>, mut typeSpec: TypeSpec, mut rootType: TypeSpec, mut exp: Arc<metamodelica::List<(Arc<Absyn::Exp>, ArcStr)>>, mut instance_name: Arc<metamodelica::List<Arc<metamodelica::List<ArcStr>>>>, mut defs: Absyn::Program, mut hasPreferred: bool, mut preferred: Arc<metamodelica::List<Preferred>>, mut path: ArcStr) -> Result<Arc<Absyn::Class>> {
    let mut out_class: Arc<Absyn::Class>;
    let __pa0 = ::match_deref::match_deref! { match &(in_class) {
        __pa0 @ Deref @ Absyn::Class { .. } => __pa0.clone(),
        _ => bail!("pattern mismatch"),
    } };
    out_class = __pa0.clone();
    assign_field!(out_class.body = parseClassDef(out_class.body.clone(), defs, typeSpec, rootType, exp, instance_name, hasPreferred, preferred, ({ let mut __mm_s = String::new(); __mm_s.push_str(&*path); __mm_s.push_str(&*out_class.name.clone()); __mm_s.push_str(&*literal!(".")); ArcStr::from(__mm_s) }).clone())?);
    Ok(out_class)
}
...
fn parseClassDef(mut in_def: Arc<Absyn::ClassDef>, mut defs: Absyn::Program, mut typeSpec: TypeSpec, mut rootType: TypeSpec, mut exp: Arc<metamodelica::List<(Arc<Absyn::Exp>, ArcStr)>>, mut instance_name: Arc<metamodelica::List<Arc<metamodelica::List<ArcStr>>>>, mut hasPreferred: bool, mut preferred: Arc<metamodelica::List<Preferred>>, mut path: ArcStr) -> Result<Arc<Absyn::ClassDef>> {
    ...
    out_def = (::match_deref::match_deref! { match &(in_def) {
        Deref @ Absyn::ClassDef::PARTS { typeVars, classAttrs, classParts, ann, comment } => {
            ...
            nclsp = parseClassParts(classParts.clone(), defs, typeSpec, rootType, exp, instance_name, hasPreferred, preferred, (path).clone())?;
            Arc::new(Absyn::ClassDef::PARTS { typeVars: typeVars.clone(), classAttrs: classAttrs.clone(), classParts: nclsp, ann: ann.clone(), comment: comment.clone() })
        },
        _ => bail!("match: no arm matched"),
    } });
    Ok(out_def)
...
fn parseClassParts(mut classes: Arc<metamodelica::List<Arc<Absyn::ClassPart>>>, mut defs: Absyn::Program, mut typeSpec: TypeSpec, mut rootType: TypeSpec, mut exp: Arc<metamodelica::List<(Arc<Absyn::Exp>, ArcStr)>>, mut instance_name: Arc<metamodelica::List<Arc<metamodelica::List<ArcStr>>>>, mut hasPreferred: bool, mut preferred: Arc<metamodelica::List<Preferred>>, mut path: ArcStr) -> Result<Arc<metamodelica::List<Arc<Absyn::ClassPart>>>> {
    ...
    out_classes = (::match_deref::match_deref! { match &(classes) {
        ...
        Deref @ metamodelica::List::Cons { head: cls, tail: r_classes } => {
            ...
            n_cls = parseClassPart(cls.clone(), defs.clone(), typeSpec.clone(), rootType.clone(), exp.clone(), instance_name.clone(), hasPreferred, preferred.clone(), (path.clone()).clone())?;
            nr_classes = parseClassParts(r_classes.clone(), defs, typeSpec, rootType, exp, instance_name, hasPreferred, preferred, (path).clone())?;
...
fn parseClassPart(mut in_def: Arc<Absyn::ClassPart>, mut defs: Absyn::Program, mut typeSpec: TypeSpec, mut rootType: TypeSpec, mut exp: Arc<metamodelica::List<(Arc<Absyn::Exp>, ArcStr)>>, mut instance_name: Arc<metamodelica::List<Arc<metamodelica::List<ArcStr>>>>, mut hasPreferred: bool, mut preferred: Arc<metamodelica::List<Preferred>>, mut path: ArcStr) -> Result<Arc<Absyn::ClassPart>> {
    ...
    out_def = (::match_deref::match_deref! { match &(in_def.clone()) {
        Deref @ Absyn::ClassPart::PUBLIC { contents: elems } => {
            ...
            elems1 = parseElems(elems.clone(), defs, typeSpec, rootType, exp, instance_name, hasPreferred, preferred, (path).clone());
            Arc::new(Absyn::ClassPart::PUBLIC { contents: elems1 })
        },
        _ => {
            in_def
        },
...
fn parseElems(mut in_elems: Arc<metamodelica::List<Arc<Absyn::ElementItem>>>, mut defs: Absyn::Program, mut typeSpec: TypeSpec, mut rootType: TypeSpec, mut exp2: Arc<metamodelica::List<(Arc<Absyn::Exp>, ArcStr)>>, mut instance_name: Arc<metamodelica::List<Arc<metamodelica::List<ArcStr>>>>, mut hasPreferred: bool, mut preferred: Arc<metamodelica::List<Preferred>>, mut pathInClass: ArcStr) -> Arc<metamodelica::List<Arc<Absyn::ElementItem>>> {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementItem::ELEMENTITEM { element: Deref @ Absyn::Element::ELEMENT { finalPrefix, redeclareKeywords, innerOuter, specification: Deref @ Absyn::ElementSpec::COMPONENTS { attributes, typeSpec: tSpec, components }, info, constrainClass } }, tail: rest } => {
                    let mut e_list: Arc<metamodelica::List<Arc<Absyn::ElementItem>>>;
                    ...
                    Ok(listAppend(e_list.clone(), parseElems(rest.clone(), defs.clone(), typeSpec.clone(), rootType.clone(), exp2.clone(), instance_name.clone(), hasPreferred, preferred.clone(), (pathInClass.clone()).clone())))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: e_item, tail: rest } => {
                    Ok(metamodelica::cons(e_item.clone(), parseElems(rest.clone(), defs.clone(), typeSpec.clone(), rootType.clone(), exp2.clone(), instance_name.clone(), hasPreferred, preferred.clone(), (pathInClass.clone()).clone())))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
...
fn applyModifierPreferred(mut comps: Arc<metamodelica::List<Arc<Absyn::ComponentItem>>>, mut exp: Arc<Absyn::Exp>, mut typeSp: ArcStr, mut instance_name: Arc<metamodelica::List<Arc<metamodelica::List<ArcStr>>>>, mut ename: ArcStr) -> Arc<metamodelica::List<Arc<Absyn::ComponentItem>>> {
...
fn applyModifier(mut comps: Arc<metamodelica::List<Arc<Absyn::ComponentItem>>>, mut exp: Arc<Absyn::Exp>, mut instance_name: Arc<metamodelica::List<Arc<metamodelica::List<ArcStr>>>>, mut counter: i32, mut newName: bool) -> Arc<metamodelica::List<Arc<Absyn::ComponentItem>>> {
    ...
    out_comps = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ComponentItem { component: Absyn::Component { name, arrayDim, modification: _ }, condition, comment }, tail: _ } => {
                    ...
                    new_name = (if (newName) {{ let mut __mm_s = String::new(); __mm_s.push_str(&*name.clone()); __mm_s.push_str(&*literal!("_autogen_bind_")); __mm_s.push_str(&*intString(counter)); ArcStr::from(__mm_s) }} else {name.clone()}).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: _, tail: rest } => {
                    Ok(applyModifier(rest.clone(), exp.clone(), instance_name.clone(), counter, newName))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
...
fn buildAllComponentModifiers(mut name_list: Arc<metamodelica::List<Arc<metamodelica::List<ArcStr>>>>) -> Arc<metamodelica::List<ArcStr>> {
    ...
    out_names = (::match_deref::match_deref! { match &(name_list) {
        ...
        Deref @ metamodelica::List::Cons { head: l, tail: rest } => {
            let mut tmp_names: Arc<metamodelica::List<ArcStr>>;
            tmp_names = buildAllComponentModifiers(rest.clone());
            buildAllComponentModifiers2(l.clone(), tmp_names)
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    out_names
...
fn buildAllComponentModifiers2(mut name_list: Arc<metamodelica::List<ArcStr>>, mut name_list2: Arc<metamodelica::List<ArcStr>>) -> Arc<metamodelica::List<ArcStr>> {
    ...
    out_names = (::match_deref::match_deref! { match &(name_list) {
        ...
        Deref @ metamodelica::List::Cons { head: s, tail: rest } => {
            ...
            listAppend(buildAllComponentModifiers3((s.clone()).clone(), name_list2), tmp_names)
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    out_names
...
fn buildAllComponentModifiers3(mut prefix: ArcStr, mut name_list2: Arc<metamodelica::List<ArcStr>>) -> Arc<metamodelica::List<ArcStr>> {
    ...
    out_names = (::match_deref::match_deref! { match &(name_list2.clone()) {
        ...
        Deref @ metamodelica::List::Cons { head: s, tail: rest } => {
            metamodelica::cons(({ let mut __mm_s = String::new(); __mm_s.push_str(&*s.clone()); __mm_s.push_str(&*literal!(".")); __mm_s.push_str(&*prefix); ArcStr::from(__mm_s) }).clone(), buildAllComponentModifiers2(rest.clone(), name_list2))
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    out_names
...
fn parseAggregator(mut in_eq: Arc<Absyn::Exp>, mut fargs: Arc<Absyn::FunctionArgs>) -> Arc<Absyn::Exp> {
    ...
    out_eq = (::match_deref::match_deref! { match &(in_eq.clone()) {
        Deref @ Absyn::Exp::BINARY { exp1, op, exp2 } => {
            ...
            nexp2 = parseAggregator(exp2.clone(), fargs);
            Arc::new(Absyn::Exp::BINARY { exp1: nexp1, op: op.clone(), exp2: nexp2 })
        },
        Deref @ Absyn::Exp::LBINARY { exp1, op, exp2 } => {
            ...
            nexp1 = parseAggregator(exp1.clone(), fargs.clone());
            nexp2 = parseAggregator(exp2.clone(), fargs);
            Arc::new(Absyn::Exp::LBINARY { exp1: nexp1, op: op.clone(), exp2: nexp2 })
        },
        Deref @ Absyn::Exp::RELATION { exp1, op, exp2 } => {
            ...
            nexp1 = parseAggregator(exp1.clone(), fargs.clone());
            nexp2 = parseAggregator(exp2.clone(), fargs);
            Arc::new(Absyn::Exp::RELATION { exp1: nexp1, op: op.clone(), exp2: nexp2 })
        },
        Deref @ Absyn::Exp::UNARY { op, exp: exp2 } => {
            let mut nexp1: Arc<Absyn::Exp>;
            nexp1 = parseAggregator(exp2.clone(), fargs);
            Arc::new(Absyn::Exp::UNARY { op: op.clone(), exp: nexp1 })
        },
        Deref @ Absyn::Exp::LUNARY { op, exp: exp2 } => {
            let mut nexp1: Arc<Absyn::Exp>;
            nexp1 = parseAggregator(exp2.clone(), fargs);
            Arc::new(Absyn::Exp::LUNARY { op: op.clone(), exp: nexp1 })
        },
        Deref @ Absyn::Exp::IFEXP { ifExp: ife, trueBranch: exp1, elseBranch: exp2, elseIfBranch: elif } => {
            ...
            nexp1 = parseAggregator(exp1.clone(), fargs.clone());
            nexp2 = parseAggregator(exp2.clone(), fargs);
            Arc::new(Absyn::Exp::IFEXP { ifExp: nife, trueBranch: nexp1, elseBranch: nexp2, elseIfBranch: elif.clone() })
        },
        Deref @ Absyn::Exp::CALL { function_: crf, functionArgs: _, .. } => {
            Arc::new(Absyn::Exp::CALL { function_: crf.clone(), functionArgs: fargs, typeVars: var_field!((*in_eq).typeVars, Absyn::Exp::CALL).clone() })
        },
        _ => {
            in_eq
        },
...
pub(crate) fn getProviders(mut providers: Arc<metamodelica::List<Provider>>, mut vmodel: Arc<Absyn::Class>, mut env: Absyn::Program, mut in_es: Arc<metamodelica::List<(Arc<Absyn::Exp>, ArcStr)>>) -> Result<Arc<metamodelica::List<(Arc<Absyn::Exp>, ArcStr)>>> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: Provider { modelID: className, component: _, template }, tail: rest } => {
            let mut comps: Arc<metamodelica::List<(Arc<metamodelica::List<Arc<Absyn::ComponentItem>>>, ArcStr)>>;
            let mut exps: Arc<metamodelica::List<(Arc<Absyn::Exp>, ArcStr)>>;
            let mut new_es: Arc<metamodelica::List<(Arc<Absyn::Exp>, ArcStr)>>;
            let mut mlist: Arc<metamodelica::List<Arc<Absyn::ElementItem>>>;
            let mut exp: Arc<Absyn::Exp>;
            mlist = AbsynUtil::getElementItemsInClass(vmodel.clone())?;
            comps = getAllProviderInstances((className.clone()).clone(), (template.clone()).clone(), mlist, env.clone(), metamodelica::nil(), (literal!("")).clone());
            let __pa0 = ::match_deref::match_deref! { match &(Parser::parsestringexp((template.clone()).clone(), (literal!("<interactive>")).clone())?) {
                GlobalScript::Statements { interactiveStmtLst: Deref @ metamodelica::List::Cons { head: GlobalScript::Statement::IEXP { exp: __pa0, info: _ }, tail: Deref @ metamodelica::List::Nil }, semicolon: _ } => __pa0.clone(),
                _ => bail!("pattern mismatch"),
            } };
            ...
            { (providers, vmodel, env, in_es) = (rest.clone(), vmodel, env, new_es); continue '__tco; }
        },
        _ => return Err(anyhow::anyhow!("match: no arm matched")),
    } }
    }
...
fn applyTemplate(mut exp: Arc<Absyn::Exp>, mut comps: Arc<metamodelica::List<(Arc<metamodelica::List<Arc<Absyn::ComponentItem>>>, ArcStr)>>, mut in_es: Arc<metamodelica::List<(Arc<Absyn::Exp>, ArcStr)>>) -> Arc<metamodelica::List<(Arc<Absyn::Exp>, ArcStr)>> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: (clist, pathInClass), tail: rest } => {
            ...
            { (exp, comps, in_es) = (exp, rest.clone(), new_es); continue '__tco; }
        },
        Deref @ metamodelica::List::Cons { head: _, tail: rest } => {
...
fn applyTemplate2(mut exp: Arc<Absyn::Exp>, mut comps: Arc<metamodelica::List<Arc<Absyn::ComponentItem>>>, mut in_es: Arc<metamodelica::List<(Arc<Absyn::Exp>, ArcStr)>>, mut pathInClass: ArcStr) -> Arc<metamodelica::List<(Arc<Absyn::Exp>, ArcStr)>> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ComponentItem { component: Absyn::Component { name, arrayDim: _, modification: _ }, condition: _, comment: _ }, tail: rest } => {
            ...
            { (exp, comps, in_es, pathInClass) = (exp.clone(), rest.clone(), metamodelica::cons((parseExpression(exp, (newName.clone()).clone()), newName), in_es), (pathInClass).clone()); continue '__tco; }
        },
        Deref @ metamodelica::List::Cons { head: _, tail: rest } => {
...
fn parseExpression(mut in_eq: Arc<Absyn::Exp>, mut fargs: ArcStr) -> Arc<Absyn::Exp> {
    ...
    out_eq = (::match_deref::match_deref! { match &(in_eq.clone()) {
        Deref @ Absyn::Exp::BINARY { exp1, op, exp2 } => {
            ...
            nexp2 = parseExpression(exp2.clone(), (fargs).clone());
            Arc::new(Absyn::Exp::BINARY { exp1: nexp1, op: op.clone(), exp2: nexp2 })
        },
        Deref @ Absyn::Exp::LBINARY { exp1, op, exp2 } => {
            let mut nexp1: Arc<Absyn::Exp>;
        ...
        Deref @ Absyn::Exp::UNARY { op, exp: exp2 } => {
            let mut nexp1: Arc<Absyn::Exp>;
            nexp1 = parseExpression(exp2.clone(), (fargs).clone());
            Arc::new(Absyn::Exp::UNARY { op: op.clone(), exp: nexp1 })
        },
        Deref @ Absyn::Exp::LUNARY { op, exp: exp2 } => {
            let mut nexp1: Arc<Absyn::Exp>;
        ...
        Deref @ Absyn::Exp::CREF { componentRef: crf } => {
            let mut new_crf: Arc<Absyn::ComponentRef>;
            new_crf = updateCRF(crf.clone(), (fargs).clone());
            Arc::new(Absyn::Exp::CREF { componentRef: new_crf })
        },
        _ => {
            in_eq
        },
...
fn updateCRF(mut componentRef: Arc<Absyn::ComponentRef>, mut name: ArcStr) -> Arc<Absyn::ComponentRef> {
    '__tco: loop {
        ...
        Deref @ Absyn::ComponentRef::CREF_QUAL { name: id, subscripts, componentRef: cRef } => {
            ...
            return Arc::new(Absyn::ComponentRef::CREF_QUAL { name: (id.clone()).clone(), subscripts: subscripts.clone(), componentRef: new_cRef })
        },
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementItem::ELEMENTITEM { element: Deref @ Absyn::Element::ELEMENT { finalPrefix: _, redeclareKeywords: _, innerOuter: _, specification: Deref @ Absyn::ElementSpec::COMPONENTS { attributes: _, typeSpec, components }, info: _, constrainClass: _ } }, tail: rest } => {
                    let mut re_items: Arc<metamodelica::List<Arc<Absyn::ElementItem>>>;
                    let mut cnew: Arc<metamodelica::List<(Arc<metamodelica::List<Arc<Absyn::ComponentItem>>>, ArcStr)>>;
                    let mut cnew2: Arc<metamodelica::List<(Arc<metamodelica::List<Arc<Absyn::ComponentItem>>>, ArcStr)>>;
                    let mut path: Path;
...
fn parseComponents(mut className: ArcStr, mut template: ArcStr, mut e_items: Arc<metamodelica::List<Arc<Absyn::ElementItem>>>, mut env: Absyn::Program, mut components: Arc<metamodelica::List<Arc<Absyn::ComponentItem>>>, mut in_components: Arc<metamodelica::List<(Arc<metamodelica::List<Arc<Absyn::ComponentItem>>>, ArcStr)>>, mut pathInClass: ArcStr) -> Arc<metamodelica::List<(Arc<metamodelica::List<Arc<Absyn::ComponentItem>>>, ArcStr)>> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ComponentItem { component: Absyn::Component { name, arrayDim: _, modification: _ }, condition: _, comment: _ }, tail: rest } => {
            ...
            let mut newName: ArcStr;
            newName = (if (pathInClass.clone() == literal!("")) {name.clone()} else {{ let mut __mm_s = String::new(); __mm_s.push_str(&*pathInClass.clone()); __mm_s.push_str(&*literal!(".")); __mm_s.push_str(&*name.clone()); ArcStr::from(__mm_s) }}).clone();
            tmp = getAllProviderInstances((className.clone()).clone(), (template.clone()).clone(), e_items.clone(), env.clone(), in_components, (newName).clone());
            { (className, template, e_items, env, components, in_components, pathInClass) = ((className).clone(), (template).clone(), e_items, env, rest.clone(), tmp, (pathInClass).clone()); continue '__tco; }
        },
        Deref @ metamodelica::List::Cons { head: _, tail: rest } => {
...
fn buildInstList(mut clazz: Arc<Absyn::Class>, mut env: Absyn::Program, mut predecessors: Arc<Client_e>, mut mediators: Arc<metamodelica::List<Mediator>>, mut client_list_in: Arc<metamodelica::List<Arc<Client_e>>>, mut instance_list: Arc<metamodelica::List<Arc<metamodelica::List<ArcStr>>>>) -> Result<Arc<metamodelica::List<Arc<Client_e>>>> {
    ...
    client_list = parseElementInstList(e_items, env, crate::Binding::Client_e::interned_NO_PRED(), mediators, client_list_in, instance_list);
    Ok(client_list)
}
...
fn buildInstList2(mut clazz: Arc<Absyn::Class>, mut env: Absyn::Program, mut predecessors: Arc<Client_e>, mut mediators: Arc<metamodelica::List<Mediator>>, mut client_list_in: Arc<metamodelica::List<Arc<Client_e>>>, mut instance_list: Arc<metamodelica::List<Arc<metamodelica::List<ArcStr>>>>, mut rootType: TypeSpec) -> Result<Arc<metamodelica::List<Arc<Client_e>>>> {
    ...
    let mut e_items: Arc<metamodelica::List<Arc<Absyn::ElementItem>>>;
    e_items = AbsynUtil::getElementItemsInClass(clazz)?;
    client_list = parseElementInstList2(e_items, env, crate::Binding::Client_e::interned_NO_PRED(), mediators, client_list_in, instance_list, rootType);
    Ok(client_list)
}
...
fn isAlreadyInList(mut ts: Arc<Absyn::TypeSpec>, mut predecessors: Arc<metamodelica::List<Arc<Client_e>>>) -> Result<bool> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: Deref @ Client_e::CLIENT_E { components: _, typeSpec: _, rootType: ots, def: _, instance: _, predecessors: _, mediator: _ }, tail: rest } => {
            if (AbsynUtil::typeSpecEqual(ts.clone(), ots.clone())?) {return Ok(true)} else {{ (ts, predecessors) = (ts, rest.clone()); continue '__tco; }}
        },
        _ => return Err(anyhow::anyhow!("match: no arm matched")),
    } }
    }
...
fn parseElementInstList(mut e_items: Arc<metamodelica::List<Arc<Absyn::ElementItem>>>, mut env: Absyn::Program, mut predecessors: Arc<Client_e>, mut mediators: Arc<metamodelica::List<Mediator>>, mut in_client_list: Arc<metamodelica::List<Arc<Client_e>>>, mut instance_list: Arc<metamodelica::List<Arc<metamodelica::List<ArcStr>>>>) -> Arc<metamodelica::List<Arc<Client_e>>> {
    let mut client_list: Arc<metamodelica::List<Arc<Client_e>>>;
    client_list = 'mc: {
        let __mc_input = e_items;
        if let Ok(__v) = (|| -> Result<_> {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementItem::ELEMENTITEM { element: Deref @ Absyn::Element::ELEMENT { finalPrefix: _, redeclareKeywords: _, innerOuter: _, specification: Deref @ Absyn::ElementSpec::COMPONENTS { attributes: _, typeSpec, components }, info: _, constrainClass: _ } }, tail: rest } => {
                    ...
                    let mut new_predecessors: Arc<Client_e>;
                    let mut def: Arc<Absyn::Class>;
                    let mut l1: Arc<metamodelica::List<Arc<Client_e>>>;
                    let mut l2: Arc<metamodelica::List<Arc<Client_e>>>;
                    let mut isCl: bool;
...
fn parseElementInstList2(mut e_items: Arc<metamodelica::List<Arc<Absyn::ElementItem>>>, mut env: Absyn::Program, mut predecessors: Arc<Client_e>, mut mediators: Arc<metamodelica::List<Mediator>>, mut in_client_list: Arc<metamodelica::List<Arc<Client_e>>>, mut instance_list: Arc<metamodelica::List<Arc<metamodelica::List<ArcStr>>>>, mut rootType: TypeSpec) -> Arc<metamodelica::List<Arc<Client_e>>> {
    let mut client_list: Arc<metamodelica::List<Arc<Client_e>>>;
    client_list = 'mc: {
        let __mc_input = e_items;
        if let Ok(__v) = (|| -> Result<_> {
...
fn isClientInMediator(mut ci_name: ArcStr, mut clients: Arc<metamodelica::List<Client>>) -> (bool, ArcStr) {
    let mut isClient: bool;
    let mut iname: ArcStr;
    (isClient, iname) = 'mc: {
...
fn getMediatorDefsElements(mut mediator_defs: Arc<metamodelica::List<Arc<SCode::Element>>>, mut mediators_in: Arc<metamodelica::List<Mediator>>) -> Arc<metamodelica::List<Mediator>> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: el, tail: rest } => {
            let mut m: Arc<metamodelica::List<Mediator>>;
            m = listAppend(getMediatorDefsElement(el.clone()), mediators_in);
            { (mediator_defs, mediators_in) = (rest.clone(), m); continue '__tco; }
        },
        _ => unreachable!("tail-call lowered match: no arm matched"),
    } }
    }
...
fn getMediatorDefsElement(mut el: Arc<SCode::Element>) -> Arc<metamodelica::List<Mediator>> {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ SCode::Element::CLASS { name: _, prefixes: _, encapsulatedPrefix: _, partialPrefix: _, restriction: SCode::Restriction::R_RECORD { isOperator: _ }, classDef: Deref @ SCode::ClassDef::PARTS { elementLst: elist, normalEquationLst: _, initialEquationLst: _, normalAlgorithmLst: _, initialAlgorithmLst: _, constraintLst: _, clsattrs: _, externalDecl: _ }, cmt: _, info: _ } => {
                    ...
                    let mut cMod: Arc<metamodelica::List<Arc<Absyn::Exp>>>;
                    let mut pMod: Arc<metamodelica::List<Arc<Absyn::Exp>>>;
                    let mut prMod: Arc<metamodelica::List<Arc<Absyn::Exp>>>;
                    let mut template: ArcStr;
                    let mut mType: ArcStr;
                    let mut str1: ArcStr;
                    let mut str2: ArcStr;
                    let mut cls: Arc<metamodelica::List<Client>>;
                    let mut prvs: Arc<metamodelica::List<Provider>>;
                    let mut pref: Arc<metamodelica::List<Preferred>>;
...
fn getPreferredList(mut e: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut val: Arc<metamodelica::List<Preferred>>) -> Result<Arc<metamodelica::List<Preferred>>> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: Deref @ Absyn::Exp::CALL { function_: _, functionArgs: Deref @ Absyn::FunctionArgs::FUNCTIONARGS { args: _, argNames }, .. }, tail: rest } => {
            ...
            { (e, val) = (rest.clone(), metamodelica::cons(Preferred { clientInstancePath: (clientInstancePath).clone(), providerInstancePath: (providerInstancePath).clone() }, val)); continue '__tco; }
        },
        _ => return Err(anyhow::anyhow!("match: no arm matched")),
    } }
    }
...
fn getClientList(mut e: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut val: Arc<metamodelica::List<Client>>) -> Result<Arc<metamodelica::List<Client>>> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: Deref @ Absyn::Exp::CALL { function_: _, functionArgs: Deref @ Absyn::FunctionArgs::FUNCTIONARGS { args: _, argNames }, .. }, tail: rest } => {
            ...
            { (e, val) = (rest.clone(), metamodelica::cons(Client { modelID: (className).clone(), component: (instance).clone(), template: (template).clone(), isMandatory: isMandatory }, val)); continue '__tco; }
        },
        _ => return Err(anyhow::anyhow!("match: no arm matched")),
    } }
    }
...
fn getProviderList(mut e: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut val: Arc<metamodelica::List<Provider>>) -> Result<Arc<metamodelica::List<Provider>>> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: Deref @ Absyn::Exp::CALL { function_: _, functionArgs: Deref @ Absyn::FunctionArgs::FUNCTIONARGS { args: _, argNames }, .. }, tail: rest } => {
            ...
            { (e, val) = (rest.clone(), metamodelica::cons(Provider { modelID: (className).clone(), component: (instance).clone(), template: (providerTemplate).clone() }, val)); continue '__tco; }
        },
        _ => return Err(anyhow::anyhow!("match: no arm matched")),
    } }
    }
...
fn getArg(mut argNames: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut name: ArcStr) -> ArcStr {
    let mut val: ArcStr;
    val = ('mc: {
...
fn extendsType(mut elems: Arc<metamodelica::List<Arc<SCode::Element>>>, mut typeName: ArcStr) -> (bool, Option<Arc<SCode::Mod>>) {
    let mut result: bool;
...
fn getValue(mut r#mod: Arc<SCode::Mod>, mut name: Ident, mut retype: ArcStr) -> Result<Arc<Absyn::Exp>> {
    ...
    val = (::match_deref::match_deref! { match &(r#mod) {
        Deref @ SCode::Mod::MOD { finalPrefix: _, eachPrefix: _, subModLst: smod, binding: _, comment: _, .. } => {
            getValueR(smod.clone(), (name).clone(), (retype).clone())?
        },
        _ => bail!("match: no arm matched"),
    } });
    Ok(val)
