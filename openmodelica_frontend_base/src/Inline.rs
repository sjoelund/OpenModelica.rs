...
pub fn inlineStartAttribute(mut inVariableAttributesOption: Option<Arc<DAE::VariableAttributes>>, mut isource: Arc<DAE::ElementSource>, mut fns: Functiontuple) -> (Option<Arc<DAE::VariableAttributes>>, Arc<DAE::ElementSource>, bool) {
    let mut outVariableAttributesOption: Option<Arc<DAE::VariableAttributes>>;
...
pub fn inlineCallsInFunctions(mut inElementList: Arc<metamodelica::List<DAE::Function>>, mut inFunctions: Functiontuple) -> Arc<metamodelica::List<DAE::Function>> {
    let mut outElementList: Arc<metamodelica::List<DAE::Function>>;
...
fn inlineDAEElementsLst(mut inElementList: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Element>>>>>, mut inFunctions: Functiontuple, mut iAcc: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Element>>>>>, mut iInlined: bool) -> (Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Element>>>>>, bool) {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: elem, tail: rest } => {
            let mut acc: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Element>>>>>;
            let mut inlined: bool;
            let mut elem = (*elem).clone();
            (elem, inlined) = inlineDAEElements(elem.clone(), inFunctions.clone(), metamodelica::nil(), false);
            { (inElementList, inFunctions, iAcc, iInlined) = (rest.clone(), inFunctions, metamodelica::cons(elem.clone(), iAcc), inlined || iInlined); continue '__tco; }
        },
...
fn inlineDAEElements(mut inElementList: Arc<metamodelica::List<Arc<DAE::Element>>>, mut inFunctions: Functiontuple, mut iAcc: Arc<metamodelica::List<Arc<DAE::Element>>>, mut iInlined: bool) -> (Arc<metamodelica::List<Arc<DAE::Element>>>, bool) {
    '__tco: loop {
        ::match_deref::match_deref! { match &(inElementList) {
        Deref @ metamodelica::List::Nil => {
            return (iAcc.reverse(), iInlined)
        ...
        Deref @ metamodelica::List::Cons { head: elem, tail: rest } => {
            let mut acc: Arc<metamodelica::List<Arc<DAE::Element>>>;
            let mut inlined: bool;
            let mut elem = (*elem).clone();
...
fn inlineDAEElement(mut inElement: Arc<DAE::Element>, mut inFunctions: Functiontuple) -> (Arc<DAE::Element>, bool) {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Element::WHEN_EQUATION { condition: exp, equations: elist, elsewhen_: Some(el), source }, fns) => {
                    let mut elist_1: Arc<metamodelica::List<Arc<DAE::Element>>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Element::IF_EQUATION { condition1: explst, equations2: dlist, equations3: elist, source }, fns) => {
                    let mut elist_1: Arc<metamodelica::List<Arc<DAE::Element>>>;
                    let mut dlist_1: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Element>>>>>;
                    let mut explst_1: Arc<metamodelica::List<Arc<DAE::Exp>>>;
                    let mut b1: bool;
                    let mut b2: bool;
                    let mut b3: bool;
                    let mut source = (*source).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Element::ALGORITHM { algorithm_: alg, source }, fns) => {
                    let mut alg_1: Arc<DAE::Algorithm>;
...
pub(crate) fn inlineAlgorithm(mut inAlgorithm: Arc<DAE::Algorithm>, mut inElementList: Functiontuple) -> Result<(Arc<DAE::Algorithm>, bool)> {
    ...
    (outAlgorithm, inlined) = (::match_deref::match_deref! { match &((inAlgorithm, inElementList)) {
        (Deref @ DAE::Algorithm { statementLst: stmts }, fns) => {
            ...
            (Arc::new(DAE::Algorithm { statementLst: stmts_1 }), inlined)
        },
        _ => {
...
pub fn inlineStatements(mut inStatements: Arc<metamodelica::List<Arc<DAE::Statement>>>, mut inElementList: Functiontuple, mut iAcc: Arc<metamodelica::List<Arc<DAE::Statement>>>, mut iInlined: bool) -> (Arc<metamodelica::List<Arc<DAE::Statement>>>, bool) {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: stmt, tail: rest } => {
            let mut acc: Arc<metamodelica::List<Arc<DAE::Statement>>>;
            let mut inlined: bool;
            let mut stmt = (*stmt).clone();
            (stmt, inlined) = inlineStatement(stmt.clone(), inElementList.clone());
            { (inStatements, inElementList, iAcc, iInlined) = (rest.clone(), inElementList, metamodelica::cons(stmt.clone(), iAcc), inlined || iInlined); continue '__tco; }
        },
        _ => unreachable!("tail-call lowered match: no arm matched"),
    } }
    }
...
fn inlineStatement(mut inStatement: Arc<DAE::Statement>, mut inElementList: Functiontuple) -> (Arc<DAE::Statement>, bool) {
...
fn inlineElse(mut inElse: Arc<DAE::Else>, mut inElementList: Functiontuple, mut inSource: Arc<DAE::ElementSource>) -> (Arc<DAE::Else>, Arc<DAE::ElementSource>, bool) {
...
pub fn inlineExpOpt(mut inExpOption: Option<Arc<DAE::Exp>>, mut inElementList: Functiontuple, mut inSource: Arc<DAE::ElementSource>) -> (Option<Arc<DAE::Exp>>, Arc<DAE::ElementSource>, bool) {
    let mut outExpOption: Option<Arc<DAE::Exp>>;
    let mut outSource: Arc<DAE::ElementSource>;
    let mut inlined: bool;
...
pub fn inlineExp(mut inExp: Arc<DAE::Exp>, mut inElementList: Functiontuple, mut inSource: Arc<DAE::ElementSource>) -> (Arc<DAE::Exp>, Arc<DAE::ElementSource>, bool, Arc<metamodelica::List<Arc<DAE::Statement>>>) {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (e, fns, source) => {
                    ...
                    let mut source = (*source).clone();
                    (e_1, assrtLst) = Expression::traverseExpBottomUp(e.clone(), (std::sync::Arc::new({ let __pe_b2 = fns.clone(); move |__pe_a0, __pe_a1| Ok(inlineCall(__pe_a0, __pe_a1, __pe_b2.clone())) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Statement>>>) -> Result<(Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Statement>>>)> + 'static>), metamodelica::nil())?;
...
pub fn forceInlineExp(mut inExp: Arc<DAE::Exp>, mut inElementList: Functiontuple, mut inSource: Arc<DAE::ElementSource>, mut cevalConst: Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, Arc<AvlTreePathFunction::Tree>) -> Result<Arc<DAE::Exp>> + 'static>) -> Result<(Arc<DAE::Exp>, Arc<DAE::ElementSource>, bool)> {
    ...
    (outExp, outSource, inlineperformed) = (::match_deref::match_deref! { match &((inExp.clone(), inElementList, inSource.clone())) {
        ...
        (e, fns, source) => {
            ...
            let mut b: bool;
            let mut source = (*source).clone();
            (e_1, _) = Expression::traverseExpBottomUp(e.clone(), (std::sync::Arc::new({ let __pe_b2 = fns.clone(); let __pe_b3 = openmodelica_ast_collections::AvlSetPath::Tree::interned_EMPTY(); move |__pe_a0, __pe_a1| Ok(forceInlineCall(__pe_a0, __pe_a1, __pe_b2.clone(), __pe_b3.clone())) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Statement>>>) -> Result<(Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Statement>>>)> + 'static>), metamodelica::nil())?;
            b = !(referenceEq(&*(e.clone()),&*(e_1.clone())));
            if b {
                source = ElementSource::addSymbolicTransformation(source.clone(), Arc::new(DAE::SymbolicOperation::OP_INLINE { before: Arc::new(DAE::EquationExp::PARTIAL_EQUATION { exp: e.clone() }), after: Arc::new(DAE::EquationExp::PARTIAL_EQUATION { exp: e_1.clone() }) }))?;
                let (__pa0, __pa1) = ::match_deref::match_deref! { match &(ExpressionSimplify::simplifyAddSymbolicOperation(Arc::new(DAE::EquationExp::PARTIAL_EQUATION { exp: e_1 }), source.clone())?) {
                    (Deref @ DAE::EquationExp::PARTIAL_EQUATION { exp: __pa0 }, __pa1) => (__pa0.clone(), __pa1.clone()),
                    _ => bail!("pattern mismatch"),
                } };
                e_1 = __pa0.clone();
...
pub fn inlineExps(mut inExps: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut inElementList: Functiontuple, mut inSource: Arc<DAE::ElementSource>) -> (Arc<metamodelica::List<Arc<DAE::Exp>>>, Arc<DAE::ElementSource>, bool) {
...
fn inlineExpsWork(mut inExps: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut fns: Functiontuple, mut inSource: Arc<DAE::ElementSource>, mut iAcc: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut iInlined: bool) -> (Arc<metamodelica::List<Arc<DAE::Exp>>>, Arc<DAE::ElementSource>, bool) {
    '__tco: loop {
    ...
    bEquiv = (::match_deref::match_deref! { match &(inExp2.clone()) {
        _ => {
            ...
            if Config::acceptMetaModelicaGrammar()? {
                ...
                (ty2, _) = Types::traverseType(ty2, -1, (std::sync::Arc::new(fnptr!(Types::makeExpDimensionsUnknown, Arc<DAE::Type>, i32)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Type>, i32) -> Result<(Arc<DAE::Type>, i32)> + 'static>))?;
...
pub fn inlineCall(mut exp: Arc<DAE::Exp>, mut assrtLst: Arc<metamodelica::List<Arc<DAE::Statement>>>, mut fns: Functiontuple) -> (Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Statement>>>) {
    ...
    (exp, assrtLst) = 'mc: {
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                e1 @ Deref @ DAE::Exp::CALL { path: p, expLst: args, attr: Deref @ DAE::CallAttributes { ty, inlineType, .. } } => {
                    ...
                    let mut lst_cr: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
                    let mut newExp: Arc<DAE::Exp>;
                    ...
                    let mut assrtStmts: Arc<metamodelica::List<Arc<DAE::Statement>>>;
        ...
        __acc.reverse()
                            ...
                            let __pa1 = ::match_deref::match_deref! { match &(Expression::traverseExpBottomUp(newExp.clone(), (std::sync::Arc::new(fnptr!(replaceArgs, Arc<DAE::Exp>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>)), bool))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>)), bool)) -> Result<(Arc<DAE::Exp>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>)), bool))> + 'static>), (argmap.clone(), checkcr.clone(), true))?) {
        ...
        __acc.reverse()
                            ...
                            generateEvents = hasGenerateEventsAnnotation(comment.clone())?;
                            newExp = if (!(generateEvents.clone())) {Expression::addNoEventToRelationsAndConds(newExp.clone())?} else {newExp.clone()};
                            let __pa2 = ::match_deref::match_deref! { match &(Expression::traverseExpBottomUp(newExp.clone(), (std::sync::Arc::new(fnptr!(replaceArgs, Arc<DAE::Exp>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>)), bool))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>)), bool)) -> Result<(Arc<DAE::Exp>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>)), bool))> + 'static>), (argmap.clone(), checkcr.clone(), true))?) {
                                        (__pa2, (_, _, true)) => __pa2.clone(),
                                        _ => bail!("pattern mismatch"),
                            } };
                            newExp = __pa2.clone();
                            assrt = inlineAssert(assrt.clone(), fns.clone(), argmap.clone(), checkcr.clone())?;
                            (newExp1, assrtLst) = Expression::traverseExpBottomUp(newExp.clone(), (std::sync::Arc::new({ let __pe_b2 = fns.clone(); move |__pe_a0, __pe_a1| Ok(inlineCall(__pe_a0, __pe_a1, __pe_b2.clone())) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Statement>>>) -> Result<(Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Statement>>>)> + 'static>), metamodelica::cons(assrt.clone(), assrtLst.clone()))?;
                        }
                    }
...
fn inlineAssert(mut assrtIn: Arc<DAE::Statement>, mut fns: Functiontuple, mut argmap: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, mut checkcr: (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>))) -> Result<Arc<DAE::Statement>> {
    ...
    level = __pa2.clone();
    source = __pa3.clone();
    let __pa4 = ::match_deref::match_deref! { match &(Expression::traverseExpBottomUp(cond, (std::sync::Arc::new(fnptr!(replaceArgs, Arc<DAE::Exp>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>)), bool))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>)), bool)) -> Result<(Arc<DAE::Exp>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>)), bool))> + 'static>), (argmap.clone(), checkcr.clone(), true))?) {
        (__pa4, (_, _, true)) => __pa4.clone(),
        _ => bail!("pattern mismatch"),
    } };
    cond = __pa4.clone();
    let __pa5 = ::match_deref::match_deref! { match &(Expression::traverseExpBottomUp(msg, (std::sync::Arc::new(fnptr!(replaceArgs, Arc<DAE::Exp>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>)), bool))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>)), bool)) -> Result<(Arc<DAE::Exp>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>)), bool))> + 'static>), (argmap, checkcr, true))?) {
        (__pa5, (_, _, true)) => __pa5.clone(),
        _ => bail!("pattern mismatch"),
    } };
    msg = __pa5.clone();
    assrtOut = Arc::new(DAE::Statement::STMT_ASSERT { cond: cond, msg: msg, level: level, source: source });
    Ok(assrtOut)
}
...
fn dumpArgmap(mut inTpl: (Arc<DAE::ComponentRef>, Arc<DAE::Exp>)) -> Result<()> {
    ...
    (cr, exp) = inTpl;
    metamodelica::print(({ let mut __mm_s = String::new(); __mm_s.push_str(&*ComponentReferenceBasics::printComponentRefStr(cr)?); __mm_s.push_str(&*literal!(" -> ")); __mm_s.push_str(&*ExpressionBasics::printExpStr(exp)?); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
    Ok(())
}

pub fn forceInlineCall(mut exp: Arc<DAE::Exp>, mut assrtLst: Arc<metamodelica::List<Arc<DAE::Statement>>>, mut fns: Functiontuple, mut visitedPaths: Arc<AvlSetPath::Tree>) -> (Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Statement>>>) {
    let mut exp: Arc<DAE::Exp> = exp;
    let mut assrtLst: Arc<metamodelica::List<Arc<DAE::Statement>>> = assrtLst;
    (exp, assrtLst) = 'mc: {
        let __mc_input = exp.clone();
        ...
        __acc.reverse()
                    ...
                    newExp = __pa0.clone();
                    (newExp1, assrtLst) = Expression::traverseExpBottomUp(newExp.clone(), (std::sync::Arc::new({ let __pe_b2 = fns.clone(); let __pe_b3 = AvlSetPath::add(visitedPaths.clone(), p.clone())?; move |__pe_a0, __pe_a1| Ok(forceInlineCall(__pe_a0, __pe_a1, __pe_b2.clone(), __pe_b3.clone())) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Statement>>>) -> Result<(Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Statement>>>)> + 'static>), assrtLst.clone())?;
                    Ok(((newExp1.clone(), assrtLst.clone()), assrtLst.clone()))
                }
                _ => bail!("nomatch"),
            }}
...
fn mergeFunctionBody(mut iStmts: Arc<metamodelica::List<Arc<DAE::Statement>>>, mut iRepl: VarTransform::VariableReplacements, mut assertStmtsIn: Arc<metamodelica::List<Arc<DAE::Statement>>>) -> Result<(VarTransform::VariableReplacements, Arc<metamodelica::List<Arc<DAE::Statement>>>)> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: Deref @ DAE::Statement::STMT_ASSIGN { exp1: Deref @ DAE::Exp::CREF { componentRef: cr, .. }, exp, .. }, tail: stmts } => {
            let mut repl: VarTransform::VariableReplacements;
            let mut assertStmts: Arc<metamodelica::List<Arc<DAE::Statement>>>;
            let mut exp = (*exp).clone();
            ...
            { (iStmts, iRepl, assertStmtsIn) = (stmts.clone(), repl, assertStmtsIn); continue '__tco; }
        },
        Deref @ metamodelica::List::Cons { head: Deref @ DAE::Statement::STMT_ASSIGN_ARR { lhs: Deref @ DAE::Exp::CREF { componentRef: cr, .. }, exp, .. }, tail: stmts } => {
            let mut repl: VarTransform::VariableReplacements;
            let mut assertStmts: Arc<metamodelica::List<Arc<DAE::Statement>>>;
            let mut exp = (*exp).clone();
            (exp, _) = VarTransform::replaceExp(exp.clone(), iRepl.clone(), None)?;
        ...
        Deref @ metamodelica::List::Cons { head: Deref @ DAE::Statement::STMT_TUPLE_ASSIGN { expExpLst: explst, exp, .. }, tail: stmts } => {
            let mut repl: VarTransform::VariableReplacements;
            let mut assertStmts: Arc<metamodelica::List<Arc<DAE::Statement>>>;
            let mut exp = (*exp).clone();
            (exp, _) = VarTransform::replaceExp(exp.clone(), iRepl.clone(), None)?;
            repl = addTplAssignToRepl(explst.clone(), 1, exp.clone(), iRepl)?;
            { (iStmts, iRepl, assertStmtsIn) = (stmts.clone(), repl, assertStmtsIn); continue '__tco; }
        },
        Deref @ metamodelica::List::Cons { head: Deref @ DAE::Statement::STMT_ASSERT { cond: exp, msg: exp1, level: exp2, source }, tail: stmts } => {
            let mut repl: VarTransform::VariableReplacements;
            ...
            { (iStmts, iRepl, assertStmtsIn) = (stmts.clone(), iRepl, metamodelica::cons(stmt, assertStmtsIn)); continue '__tco; }
        },
        Deref @ metamodelica::List::Cons { head: Deref @ DAE::Statement::STMT_IF { exp, statementLst: Deref @ metamodelica::List::Cons { head: Deref @ DAE::Statement::STMT_ASSIGN { exp1: Deref @ DAE::Exp::CREF { componentRef: cr1, .. }, exp: exp1, .. }, tail: Deref @ metamodelica::List::Nil }, else_: Deref @ DAE::Else::ELSE { statementLst: Deref @ metamodelica::List::Cons { head: Deref @ DAE::Statement::STMT_ASSIGN { exp1: Deref @ DAE::Exp::CREF { componentRef: cr2, .. }, exp: exp2, .. }, tail: Deref @ metamodelica::List::Nil } }, .. }, tail: stmts } if (ComponentReferenceBasics::crefEqual(cr1.clone(), cr2.clone())?) => {
            let mut repl: VarTransform::VariableReplacements;
            let mut assertStmts: Arc<metamodelica::List<Arc<DAE::Statement>>>;
            let mut exp = (*exp).clone();
            let mut exp1 = (*exp1).clone();
            ...
            (exp1, _) = VarTransform::replaceExp(exp1.clone(), iRepl.clone(), None)?;
            (exp2, _) = VarTransform::replaceExp(exp2.clone(), iRepl.clone(), None)?;
            repl = VarTransform::addReplacementNoTransitive(iRepl, cr1.clone(), Arc::new(DAE::Exp::IFEXP { expCond: exp.clone(), expThen: exp1.clone(), expElse: exp2.clone() }))?;
            { (iStmts, iRepl, assertStmtsIn) = (stmts.clone(), repl, assertStmtsIn); continue '__tco; }
        },
        Deref @ metamodelica::List::Cons { head: Deref @ DAE::Statement::STMT_IF { exp, statementLst: Deref @ metamodelica::List::Cons { head: Deref @ DAE::Statement::STMT_ASSIGN_ARR { lhs: Deref @ DAE::Exp::CREF { componentRef: cr1, .. }, exp: exp1, .. }, tail: Deref @ metamodelica::List::Nil }, else_: Deref @ DAE::Else::ELSE { statementLst: Deref @ metamodelica::List::Cons { head: Deref @ DAE::Statement::STMT_ASSIGN_ARR { lhs: Deref @ DAE::Exp::CREF { componentRef: cr2, .. }, exp: exp2, .. }, tail: Deref @ metamodelica::List::Nil } }, .. }, tail: stmts } if (ComponentReferenceBasics::crefEqual(cr1.clone(), cr2.clone())?) => {
            let mut repl: VarTransform::VariableReplacements;
            let mut assertStmts: Arc<metamodelica::List<Arc<DAE::Statement>>>;
            let mut exp = (*exp).clone();
            let mut exp1 = (*exp1).clone();
...
fn addTplAssignToRepl(mut explst: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut indx: i32, mut iExp: Arc<DAE::Exp>, mut iRepl: VarTransform::VariableReplacements) -> Result<VarTransform::VariableReplacements> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: Deref @ DAE::Exp::CREF { componentRef: cr, ty: tp }, tail: rest } => {
            let mut repl: VarTransform::VariableReplacements;
            let mut exp: Arc<DAE::Exp>;
            exp = Arc::new(DAE::Exp::TSUB { exp: iExp.clone(), ix: indx, ty: tp.clone() });
            repl = VarTransform::addReplacementNoTransitive(iRepl, cr.clone(), exp)?;
            { (explst, indx, iExp, iRepl) = (rest.clone(), indx + 1, iExp, repl); continue '__tco; }
        },
        _ => return Err(anyhow::anyhow!("match: no arm matched")),
    } }
    }
...
fn getFunctionInputsOutputBody(mut r#fn: Arc<metamodelica::List<Arc<DAE::Element>>>, mut iRepl: VarTransform::VariableReplacements) -> Result<(Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::ComponentRef>>>, Arc<metamodelica::List<Arc<DAE::Statement>>>, VarTransform::VariableReplacements)> {
    ...
    for mut elt in &*r#fn {
        ...
        Deref @ DAE::Element::VAR { componentRef: __esc_cr, direction: DAE::VarDirection::INPUT { .. }, .. } => {
            cr = (*__esc_cr).clone();
            oInputs = metamodelica::cons(cr.clone(), oInputs.clone());
            ()
        },
        Deref @ DAE::Element::VAR { componentRef: __esc_cr, direction: DAE::VarDirection::OUTPUT { .. }, binding: __esc_binding, .. } => {
            cr = (*__esc_cr).clone();
            ...
            oOutputs = metamodelica::cons(cr.clone(), oOutputs.clone());
            ()
        },
...
fn makeComplexBinding(mut binding: Option<Arc<DAE::Exp>>, mut ty: Arc<DAE::Type>) -> Option<Arc<DAE::Exp>> {
    ...
    binding = (::match_deref::match_deref! { match &((binding.clone(), ty.clone())) {
        ...
        Deref @ DAE::Var { binding: Deref @ DAE::Binding::EQBOUND { exp: __esc_exp, .. }, .. } => {
            ...
            strl = metamodelica::cons((var.name.clone()).clone(), strl.clone());
            ()
        },
        _ => {
        ...
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
            ...
            Some(Arc::new(DAE::Exp::RECORD { path: ClassInfUtil::getStateName(var_field!((*ty).complexClassType, DAE::Type::T_COMPLEX).clone()), exps: expl, comp: strl, ty: ty }))
        },
        _ => {
            binding
        },
...
fn addReplacement(mut iCr: Arc<DAE::ComponentRef>, mut iExp: Arc<DAE::Exp>, mut iRepl: VarTransform::VariableReplacements) -> Result<VarTransform::VariableReplacements> {
    ...
    oRepl = (::match_deref::match_deref! { match &(iCr.clone()) {
        Deref @ DAE::ComponentRef::CREF_IDENT { .. } => VarTransform::addReplacement(iRepl, iCr, iExp)?,
        _ => bail!("fail"),
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    Ok(oRepl)
...
pub fn extendCrefRecords(mut inArgmap: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, mut inCheckCr: (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>))) -> Result<(Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>)))> {
    let mut outArgmap: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>;
    ...
    (outArgmap, outCheckCr) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: (c, Deref @ DAE::Exp::CAST { exp: e, ty: Deref @ DAE::Type::T_COMPLEX { .. } }), tail: res }, ht) => {
                    ...
                    let mut new1: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: (c, e @ Deref @ DAE::Exp::CREF { componentRef: cref, ty: Deref @ DAE::Type::T_COMPLEX { varLst, .. } }), tail: res }, ht) => {
                    let mut ht1: (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (HashTableCG::FuncHashCref, HashTableCG::FuncCrefEqual, HashTableCG::FuncCrefStr, HashTableCG::FuncExpStr));
                    ...
                    let mut res1: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>;
                    let mut res2: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>;
                    let mut new: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>;
                    let mut new1: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: (c, e @ Deref @ DAE::Exp::CALL { expLst: expl, attr: Deref @ DAE::CallAttributes { ty: Deref @ DAE::Type::T_COMPLEX { complexClassType: ClassInf::State::RECORD { path: rpath }, varLst, .. }, .. }, .. }), tail: res }, ht) => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: (c, e @ Deref @ DAE::Exp::RECORD { exps: expl, ty: Deref @ DAE::Type::T_COMPLEX { varLst, .. }, .. }), tail: res }, ht) => {
                    let mut ht1: (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (HashTableCG::FuncHashCref, HashTableCG::FuncCrefEqual, HashTableCG::FuncCrefStr, HashTableCG::FuncExpStr));
                    let mut ht2: (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (HashTableCG::FuncHashCref, HashTableCG::FuncCrefEqual, HashTableCG::FuncCrefStr, HashTableCG::FuncExpStr));
                    let mut res1: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>;
                    let mut res2: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: (c, e), tail: res }, ht) => {
                    ...
                    let mut varLst: Arc<metamodelica::List<Arc<DAE::Var>>>;
                    let mut crlst: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
                    let mut creftpllst: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Var { name, ty: tp, .. } => {
                    ...
                    let mut e1: Arc<DAE::ComponentRef>;
                    let mut exp: Arc<DAE::Exp>;
...
fn extendCrefRecords2(mut ev: Arc<DAE::Var>, mut c: Arc<DAE::ComponentRef>) -> Result<Arc<DAE::ComponentRef>> {
    let mut outArg: Arc<DAE::ComponentRef>;
    outArg = 'mc: {
        let __mc_input = ev;
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
pub fn getFunctionBody(mut p: Arc<Absyn::Path>, mut fns: Functiontuple) -> Result<(Arc<metamodelica::List<Arc<DAE::Element>>>, Option<Arc<SCode::Comment>>)> {
    let mut outfn: Arc<metamodelica::List<Arc<DAE::Element>>>;
    let mut oComment: Option<Arc<SCode::Comment>>;
    (outfn, oComment) = 'mc: {
        let __mc_input = fns;
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Some(ftree), _) => {
                    ...
                    let (__pa0, __pa1) = ::match_deref::match_deref! { match &(AvlTreePathFunction::get(ftree.clone(), p.clone())?) {
                        Some(DAE::Function::FUNCTION { functions: Deref @ metamodelica::List::Cons { head: DAE::FunctionDefinition::FUNCTION_DEF { body: __pa0 }, tail: _ }, comment: __pa1, .. }) => (__pa0.clone(), __pa1.clone()),
                        _ => bail!("pattern mismatch"),
                    } };
                    body = __pa0.clone();
                    comment = __pa1.clone();
...
fn getRhsExp(mut inElementList: Arc<metamodelica::List<Arc<DAE::Element>>>) -> Result<Arc<DAE::Exp>> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: _, tail: cdr } => {
            ...
            { inElementList = cdr.clone(); continue '__tco; }
        },
        _ => return Err(anyhow::anyhow!("match: no arm matched")),
    } }
    }
...
pub fn replaceArgs(mut inExp: Arc<DAE::Exp>, mut inTuple: (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>)), bool)) -> (Arc<DAE::Exp>, (Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>)), bool)) {
    ...
    (outExp, outTuple) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::CREF { componentRef: cref, .. }, (argmap, _, true)) => {
                    let mut firstCref: Arc<DAE::ComponentRef>;
                    let mut e: Arc<DAE::Exp>;
...
fn boxIfUnboxedFunRef(mut iexp: Arc<DAE::Exp>, mut ty: Arc<DAE::Type>) -> Arc<DAE::Exp> {
    let mut outExp: Arc<DAE::Exp>;
    outExp = (::match_deref::match_deref! { match &((iexp.clone(), ty)) {
...
fn getExpFromArgMap(mut inArgMap: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<DAE::Exp>)>>, mut inComponentRef: Arc<DAE::ComponentRef>) -> Result<Arc<DAE::Exp>> {
    let mut outExp: Arc<DAE::Exp>;
    let mut arg: (Arc<DAE::ComponentRef>, Arc<DAE::Exp>) = (Arc::new(DAE::ComponentRef::WILD), Arc::new(<DAE::Exp as ::std::default::Default>::default()));
    let mut subs: Arc<metamodelica::List<Arc<DAE::Subscript>>>;
    let mut key: Arc<DAE::ComponentRef>;
    let mut cref: Arc<DAE::ComponentRef>;
    let mut exp: Arc<DAE::Exp>;
    ...
    if Flags::isSet(Flags::FAILTRACE.clone())? {
        Debug::traceln(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("Inline.getExpFromArgMap failed with empty argmap and cref: ")); __mm_s.push_str(&*ComponentReferenceBasics::printComponentRefStr(inComponentRef)?); ArcStr::from(__mm_s) }).clone())?;
    }
    bail!("fail");
    Ok(outExp)
}
...
fn getInputCrefs(mut inElement: Arc<DAE::Element>) -> Arc<DAE::ComponentRef> {
    ...
    outComponentRef = (::match_deref::match_deref! { match &(inElement) {
        Deref @ DAE::Element::VAR { componentRef: cref, direction: DAE::VarDirection::INPUT { .. }, .. } => {
            cref.clone()
        },
        _ => {
...
fn removeWilds(mut inComponentRef: Arc<DAE::ComponentRef>) -> bool {
    ...
    outBoolean = (::match_deref::match_deref! { match &(inComponentRef) {
...
pub(crate) fn simplifyAndInlineEquationExp(mut inExp: Arc<DAE::EquationExp>, mut fns: Functiontuple, mut inSource: Arc<DAE::ElementSource>) -> Result<(Arc<DAE::EquationExp>, Arc<DAE::ElementSource>)> {
    ...
    (exp, source) = inlineEquationExp(exp, (std::sync::Arc::new({ let __pe_b2 = fns; move |__pe_a0, __pe_a1| Ok(inlineCall(__pe_a0, __pe_a1, __pe_b2.clone())) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Statement>>>) -> Result<(Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Statement>>>)> + 'static>), source)?;
    Ok((exp, source))
}
...
pub fn simplifyAndForceInlineEquationExp(mut inExp: Arc<DAE::EquationExp>, mut fns: Functiontuple, mut inSource: Arc<DAE::ElementSource>) -> Result<(Arc<DAE::EquationExp>, Arc<DAE::ElementSource>)> {
    ...
    let mut source: Arc<DAE::ElementSource>;
    (exp, source) = ExpressionSimplify::simplifyAddSymbolicOperation(inExp, inSource)?;
    (exp, source) = inlineEquationExp(exp, (std::sync::Arc::new({ let __pe_b2 = fns; let __pe_b3 = openmodelica_ast_collections::AvlSetPath::Tree::interned_EMPTY(); move |__pe_a0, __pe_a1| Ok(forceInlineCall(__pe_a0, __pe_a1, __pe_b2.clone(), __pe_b3.clone())) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Statement>>>) -> Result<(Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Statement>>>)> + 'static>), source)?;
    Ok((exp, source))
}
...
pub(crate) fn inlineEquationExp(mut inExp: Arc<DAE::EquationExp>, mut r#fn: Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Statement>>>) -> Result<(Arc<DAE::Exp>, Arc<metamodelica::List<Arc<DAE::Statement>>>)> + 'static>, mut inSource: Arc<DAE::ElementSource>) -> Result<(Arc<DAE::EquationExp>, Arc<DAE::ElementSource>)> {
    ...
    (outExp, source) = (::match_deref::match_deref! { match &(inExp.clone()) {
        Deref @ DAE::EquationExp::PARTIAL_EQUATION { exp: e } => {
            ...
            eq2 = Arc::new(DAE::EquationExp::PARTIAL_EQUATION { exp: e_1 });
            source = ElementSource::condAddSymbolicTransformation(changed, inSource, Arc::new(DAE::SymbolicOperation::OP_INLINE { before: inExp, after: eq2.clone() }))?;
        ...
        Deref @ DAE::EquationExp::RESIDUAL_EXP { exp: e } => {
            ...
            (e_1, _) = Expression::traverseExpBottomUp(e.clone(), r#fn.clone(), metamodelica::nil())?;
            changed = !(referenceEq(&*(e.clone()),&*(e_1.clone())));
            eq2 = Arc::new(DAE::EquationExp::RESIDUAL_EXP { exp: e_1 });
            source = ElementSource::condAddSymbolicTransformation(changed, inSource, Arc::new(DAE::SymbolicOperation::OP_INLINE { before: inExp, after: eq2.clone() }))?;
            (eq2, source) = ExpressionSimplify::condSimplifyAddSymbolicOperation(changed, eq2, source)?;
            (eq2, source)
        },
        Deref @ DAE::EquationExp::EQUALITY_EXPS { lhs: e1, rhs: e2 } => {
            ...
            eq2 = Arc::new(DAE::EquationExp::EQUALITY_EXPS { lhs: e1_1, rhs: e2_1 });
            source = ElementSource::condAddSymbolicTransformation(changed, inSource, Arc::new(DAE::SymbolicOperation::OP_INLINE { before: inExp, after: eq2.clone() }))?;
            (eq2, source) = ExpressionSimplify::condSimplifyAddSymbolicOperation(changed, eq2, source)?;
            (eq2, source)
        },
...
fn getInlineHashTableVarTransform() -> Result<((metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>)), VarTransform::VariableReplacements)> {
    ...
    let mut opt: Option<((metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>)>>), i32, (HashTableCG::FuncHashCref, HashTableCG::FuncCrefEqual, HashTableCG::FuncCrefStr, HashTableCG::FuncExpStr)), VarTransform::VariableReplacements)>;
    ...
    (ht, repl) = (match opt {
        Some((mut __esc_ht, ref __esc_repl @ VarTransform::VariableReplacements { hashTable: ref __esc_regRepl, invHashTable: ref __esc_invRepl })) => {
            ht = __esc_ht.clone();
            repl = __esc_repl.clone();
            regRepl = __esc_regRepl.clone();
            invRepl = __esc_invRepl.clone();
