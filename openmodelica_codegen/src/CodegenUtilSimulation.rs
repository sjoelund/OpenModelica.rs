...
fn fun_50(mut in_txt: Tpl::Text, mut in_mArg: ArcStr, mut in_a_eqs: Arc<metamodelica::List<Arc<SimCode::SimEqSystem>>>) -> Result<Tpl::Text> {
    let mut out_txt: Tpl::Text;
    out_txt = (::match_deref::match_deref! { match &((in_txt, in_mArg, in_a_eqs)) {
...
pub fn dumpEqs(mut txt: Tpl::Text, mut a_eqs: Arc<metamodelica::List<Arc<SimCode::SimEqSystem>>>) -> Result<Tpl::Text> {
    ...
    out_txt = fun_50(txt, (ret_0).clone(), a_eqs)?;
    Ok(out_txt)
}
...
fn lm_68(mut in_txt: Tpl::Text, mut in_items: Arc<metamodelica::List<Arc<SimCode::OMSIFunction>>>, mut in_a_varsBuffer: Tpl::Text, mut in_a_columnBuffer: Tpl::Text) -> Result<(Tpl::Text, Tpl::Text, Tpl::Text)> {
    '__tco: loop {
        ::match_deref::match_deref! { match &((in_txt, in_items, in_a_varsBuffer, in_a_columnBuffer)) {
        (txt, Deref @ metamodelica::List::Nil, a_varsBuffer, a_columnBuffer) => {
            return Ok((txt.clone(), a_varsBuffer.clone(), a_columnBuffer.clone()))
        },
        (txt, Deref @ metamodelica::List::Cons { head: i_column, tail: rest }, a_varsBuffer, a_columnBuffer) => {
            ...
            { (in_txt, in_items, in_a_varsBuffer, in_a_columnBuffer) = (txt.clone(), rest.clone(), a_varsBuffer.clone(), a_columnBuffer.clone()); continue '__tco; }
        },
        _ => return Err(anyhow::anyhow!("match: no arm matched")),
    } }
    }
...
fn fun_69(mut in_txt: Tpl::Text, mut in_a_derivativeMatrix: Option<Arc<SimCode::DerivativeMatrix>>, mut in_a_varsBuffer: Tpl::Text, mut in_a_columnBuffer: Tpl::Text) -> Result<(Tpl::Text, Tpl::Text, Tpl::Text)> {
    ...
    (out_txt, out_a_varsBuffer, out_a_columnBuffer) = (::match_deref::match_deref! { match &((in_txt, in_a_derivativeMatrix, in_a_varsBuffer, in_a_columnBuffer)) {
...
pub(crate) fn dumpAlgSystemOps(mut txt: Tpl::Text, mut a_derivativeMatrix: Option<Arc<SimCode::DerivativeMatrix>>) -> Result<Tpl::Text> {
    ...
    (out_txt, l_varsBuffer, l_columnBuffer) = fun_69(txt, a_derivativeMatrix, l_varsBuffer, l_columnBuffer)?;
    Ok(out_txt)
}
...
fn lm_72(mut in_txt: Tpl::Text, mut in_items: Arc<metamodelica::List<Arc<SimCode::SimEqSystem>>>, mut in_a_columnBuffer: Tpl::Text) -> Result<(Tpl::Text, Tpl::Text)> {
    '__tco: loop {
        ::match_deref::match_deref! { match &((in_txt, in_items, in_a_columnBuffer)) {
        (txt, Deref @ metamodelica::List::Nil, a_columnBuffer) => {
            return Ok((txt.clone(), a_columnBuffer.clone()))
        },
        (txt, Deref @ metamodelica::List::Cons { head: Deref @ SimCode::SimEqSystem::SES_SIMPLE_ASSIGN { cref: i_equation_cref, exp: i_equation_exp, .. }, tail: rest }, a_columnBuffer) => {
            let mut txt_0: Tpl::Text;
            let mut txt = (*txt).clone();
            let mut a_columnBuffer = (*a_columnBuffer).clone();
...
pub(crate) fn dumpAlgSystemColumn(mut in_txt: Tpl::Text, mut in_a_column: Arc<SimCode::OMSIFunction>, mut in_a_columnBuffer: Tpl::Text, mut in_a_varsBuffer: Tpl::Text) -> Result<(Tpl::Text, Tpl::Text, Tpl::Text)> {
    ...
    let mut out_a_columnBuffer: Tpl::Text;
    let mut out_a_varsBuffer: Tpl::Text;
    (out_txt, out_a_columnBuffer, out_a_varsBuffer) = (::match_deref::match_deref! { match &((in_txt, in_a_column, in_a_columnBuffer, in_a_varsBuffer)) {
...
pub(crate) fn dumpWhenOps(mut in_txt: Tpl::Text, mut in_a_whenOps: Arc<metamodelica::List<BackendDAE::WhenOperator>>) -> Result<Tpl::Text> {
    let mut out_txt: Tpl::Text;
    out_txt = (::match_deref::match_deref! { match &((in_txt, in_a_whenOps)) {
        (txt, Deref @ metamodelica::List::Nil) => {
            txt.clone()
        },
...
pub fn dumpEqsAlternativeTearing(mut txt: Tpl::Text, mut a_eqs: Arc<metamodelica::List<Arc<SimCode::SimEqSystem>>>) -> Result<Tpl::Text> {
    ...
    let mut ret_0: ArcStr;
    ret_0 = (Flags::getConfigString(Flags::OBFUSCATE.clone())?).clone();
    out_txt = fun_75(txt, (ret_0).clone(), a_eqs)?;
    Ok(out_txt)
}
