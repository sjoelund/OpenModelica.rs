...
#[test]
fn test_pop_front_empty_fails() -> Result<()> {
    let de = DoubleEnded::empty(0i32)?;
...
#[test]
fn test_to_list_and_clear_empty() -> Result<()> {
    let de = DoubleEnded::empty(0i32)?;
...
#[test]
fn test_push_list_back_empty_de() -> Result<()> {
    let de = DoubleEnded::empty(0i32)?;
    let lst = list![1i32, 2];
    DoubleEnded::push_list_back(de.clone(), Arc::clone(&lst))?;
    assert_eq!(DoubleEnded::length(de)?, 2);
...
#[test]
fn test_push_list_back_empty_list() -> Result<()> {
    let de = DoubleEnded::new(1)?;
    let lst: Arc<List<i32>> = nil();
