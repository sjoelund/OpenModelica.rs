...
fn printRow(mut es_1: Arc<metamodelica::List<Arc<DAE::Exp>>>) -> Result<()> {
    printList(es_1.clone(), Arc::new(printExp), (literal!(",")).clone())?;
    Ok(())
}
...
pub fn printSubscriptLstStr(mut inSubscriptLst: Arc<metamodelica::List<Arc<DAE::Subscript>>>) -> ArcStr {
    let mut outString: ArcStr = arcstr::literal!("");
    outString = stringDelimitList(List::map(inSubscriptLst.clone(), Arc::new(ExpressionBasics::printSubscriptStr)), (literal!(" , ")).clone());
    outString
}

pub fn printExpListStr(mut expl: Arc<metamodelica::List<Arc<DAE::Exp>>>) -> ArcStr {
    let mut res: ArcStr = arcstr::literal!("");
    res = stringDelimitList(List::map(expl.clone(), Arc::new(ExpressionBasics::printExpStr)), (literal!(", ")).clone());
    res
}

// stefan
pub fn printExpListStrNoSpace(mut expl: Arc<metamodelica::List<Arc<DAE::Exp>>>) -> ArcStr {
    let mut res: ArcStr = arcstr::literal!("");
    res = stringAppendList(List::map(expl.clone(), Arc::new(ExpressionBasics::printExpStr)));
    res
}
...
pub fn printCrefsFromExpStr(mut e: Arc<DAE::Exp>) -> Result<ArcStr> {
    let mut s: ArcStr = arcstr::literal!("");
    s = (Tpl::tplString2(Arc::new(ExpressionDumpTpl::dumpExpCrefs), e.clone(), (literal!("")).clone())?).clone();
    Ok(s)
}
...
pub fn printExp2Str<Type_a: Clone + 'static>(mut inExp: Arc<DAE::Exp>, mut stringDelimiter: ArcStr, mut opcreffunc: Option<(Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Type_a) -> Result<ArcStr> + 'static>, Type_a)>, mut opcallfunc: Option<Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, ArcStr, Option<(Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Type_a) -> Result<ArcStr> + 'static>, Type_a)>) -> Result<ArcStr> + 'static>>) -> Result<ArcStr> {
    pub type printComponentRefStrFunc<Type_a: Clone> = fn(Arc<DAE::ComponentRef>, Type_a) -> Result<ArcStr>;

    pub type printCallFunc<Type_a: Clone> = fn(Arc<DAE::Exp>, ArcStr, Option<(Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Type_a) -> Result<ArcStr> + 'static>, Type_a)>) -> Result<ArcStr>;

    let mut outString: ArcStr = arcstr::literal!("");
    outString = ('mc: {
        let __mc_input = (inExp.clone(), stringDelimiter.clone(), opcreffunc.clone(), opcallfunc.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::EMPTY { tyStr, name, scope, .. }, _, _, _) => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::ICONST { integer: i }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::RCONST { real: r }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::SCONST { string: s }, _, _, _) => {
                    let mut s = (*s).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::BCONST { bool: b }, _, _, _) => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::CREF { componentRef: c, .. }, _, Some((pcreffunc, creffuncparam)), _) => {
                    let mut s: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::CREF { componentRef: c, .. }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::ENUM_LITERAL { name: lit, .. }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (e @ Deref @ DAE::Exp::BINARY { exp1: e1, operator: op, exp2: e2 }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (e @ Deref @ DAE::Exp::UNARY { operator: op, exp: e1 }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (e @ Deref @ DAE::Exp::LBINARY { exp1: e1, operator: op, exp2: e2 }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
                    let mut sym: ArcStr = arcstr::literal!("");
                    let mut s1: ArcStr = arcstr::literal!("");
                    let mut s2: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (e @ Deref @ DAE::Exp::LUNARY { operator: op, exp: e1 }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
                    let mut s_1: ArcStr = arcstr::literal!("");
                    let mut s_2: ArcStr = arcstr::literal!("");
                    let mut sym: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (e @ Deref @ DAE::Exp::RELATION { exp2: e2, operator: op, exp1: e1, .. }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
                    let mut sym: ArcStr = arcstr::literal!("");
                    let mut s1: ArcStr = arcstr::literal!("");
                    let mut s2: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (e @ Deref @ DAE::Exp::IFEXP { expCond: cond, expThen: tb, expElse: fb }, _, _, _) => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (e @ Deref @ DAE::Exp::CALL { .. }, _, _, Some(pcallfunc)) => {
                    let mut s_2: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::CALL { expLst: args, path: fcn, .. }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
                    let mut fs: ArcStr = arcstr::literal!("");
                    ...
                    argstr = stringDelimitList(List::map3(args.clone(), Arc::new(printExp2Str), (stringDelimiter.clone()).clone(), opcreffunc.clone(), opcallfunc.clone()), (literal!(",")).clone());
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::PARTEVALFUNCTION { expList: args, path: fcn, .. }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
                    let mut fs: ArcStr = arcstr::literal!("");
                    let mut argstr: ArcStr = arcstr::literal!("");
                    fs = (AbsynUtil::pathString(AbsynUtil::makeNotFullyQualified(fcn.clone()), (literal!(".")).clone(), true, false)?).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::ARRAY { array: es, .. }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
                    s = stringDelimitList(List::map3(es.clone(), Arc::new(printExp2Str), (stringDelimiter.clone()).clone(), opcreffunc.clone(), opcallfunc.clone()), (literal!(",")).clone());
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::TUPLE { PR: es }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
                    s = stringDelimitList(List::map3(es.clone(), Arc::new(printExp2Str), (stringDelimiter.clone()).clone(), opcreffunc.clone(), opcallfunc.clone()), (literal!(",")).clone());
                    s = stringAppendList(list![(literal!("(")).clone(), (s.clone()).clone(), (literal!(")")).clone()]);
                    Ok(s.clone())
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::MATRIX { matrix: lstes, .. }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
                    s = stringDelimitList(List::map1(lstes.clone(), Arc::new(fnptr!(printRowStr, Arc<metamodelica::List<Arc<DAE::Exp>>>, ArcStr)), (stringDelimiter.clone()).clone()), (literal!("},{")).clone());
                    s = stringAppendList(list![(literal!("{{")).clone(), (s.clone()).clone(), (literal!("}}")).clone()]);
                    Ok(s.clone())
                }
                _ => bail!("nomatch"),
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (e @ Deref @ DAE::Exp::RANGE { ty: _, start, step: None, stop }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
                    let mut s1: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (e @ Deref @ DAE::Exp::RANGE { ty: _, start, step: Some(step), stop }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
                    let mut s1: ArcStr = arcstr::literal!("");
                    let mut s2: ArcStr = arcstr::literal!("");
                    let mut s3: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::CAST { exp: e, ty: tp }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
                    let mut res: ArcStr = arcstr::literal!("");
                    let mut r#str: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (e @ Deref @ DAE::Exp::ASUB { sub: subs, exp: e1 }, _, _, _) => {
                    let mut s1: ArcStr = arcstr::literal!("");
        ...
        __acc.reverse()
                    ...
                    s4 = stringDelimitList(List::map3(aexpl.clone(), Arc::new(printExp2Str), (stringDelimiter.clone()).clone(), opcreffunc.clone(), opcallfunc.clone()), (literal!(",")).clone());
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::SIZE { sz: Some(dim), exp: cr }, _, _, _) => {
                    let mut r#str: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::SIZE { sz: None, exp: cr }, _, _, _) => {
                    let mut r#str: ArcStr = arcstr::literal!("");
                    let mut crstr: ArcStr = arcstr::literal!("");
                    crstr = (printExp2Str(cr.clone(), (stringDelimiter.clone()).clone(), opcreffunc.clone(), opcallfunc.clone())?).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::REDUCTION { iterators: riters, expr: exp, reductionInfo: Deref @ DAE::ReductionInfo { path: fcn, .. } }, _, _, _) => {
                    let mut fs: ArcStr = arcstr::literal!("");
                    let mut r#str: ArcStr = arcstr::literal!("");
                    ...
                    iterstr = stringDelimitList(List::map(riters.clone(), Arc::new(reductionIteratorStr)), (literal!(",")).clone());
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::META_TUPLE { listExp: es }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::LIST { valList: es }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
                    s = stringDelimitList(List::map3(es.clone(), Arc::new(printExp2Str), (stringDelimiter.clone()).clone(), opcreffunc.clone(), opcallfunc.clone()), (literal!(",")).clone());
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::CONS { cdr: e2, car: e1 }, _, _, _) => {
                    let mut s_2: ArcStr = arcstr::literal!("");
                    let mut s1: ArcStr = arcstr::literal!("");
                    let mut s2: ArcStr = arcstr::literal!("");
                    s1 = (printExp2Str(e1.clone(), (stringDelimiter.clone()).clone(), opcreffunc.clone(), opcallfunc.clone())?).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::META_OPTION { exp: None }, _, _, _) => {
                    Ok(literal!("NONE()"))
                }
                _ => bail!("nomatch"),
            }}
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::META_OPTION { exp: Some(e1) }, _, _, _) => {
                    let mut s_1: ArcStr = arcstr::literal!("");
                    let mut s1: ArcStr = arcstr::literal!("");
                    s1 = (printExp2Str(e1.clone(), (stringDelimiter.clone()).clone(), opcreffunc.clone(), opcallfunc.clone())?).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::BOX { exp: e1 }, _, _, _) => {
                    let mut s_1: ArcStr = arcstr::literal!("");
                    let mut s1: ArcStr = arcstr::literal!("");
                    s1 = (printExp2Str(e1.clone(), (stringDelimiter.clone()).clone(), opcreffunc.clone(), opcallfunc.clone())?).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::UNBOX { exp: e1, ty: _ }, _, _, _) => {
                    let mut s_1: ArcStr = arcstr::literal!("");
                    let mut s1: ArcStr = arcstr::literal!("");
                    s1 = (printExp2Str(e1.clone(), (stringDelimiter.clone()).clone(), opcreffunc.clone(), opcallfunc.clone())?).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::METARECORDCALL { args, path: fcn, .. }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
                    let mut fs: ArcStr = arcstr::literal!("");
                    let mut argstr: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::MATCHEXPRESSION { cases, inputs: es, matchType: matchTy, .. }, _, _, _) => {
                    let mut s: ArcStr = arcstr::literal!("");
                    let mut s1: ArcStr = arcstr::literal!("");
                    let mut s2: ArcStr = arcstr::literal!("");
                    let mut s3: ArcStr = arcstr::literal!("");
                    ...
                    s3 = stringAppendList(List::map(cases.clone(), Arc::new(printCase2Str)));
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::SHARED_LITERAL { exp: e, .. }, _, _, _) => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::PATTERN { pattern: pat }, _, _, _) => {
...
fn reductionIteratorStr(mut riter: Arc<DAE::ReductionIterator>) -> Result<ArcStr> {
    ...
    r#str = ((::match_deref::match_deref! { match &(riter.clone()) {
        ...
        Deref @ DAE::ReductionIterator { guardExp: Some(gexp), exp, id, .. } => {
...
fn printCase2Str(mut matchCase: Arc<DAE::MatchCase>) -> Result<ArcStr> {
    ...
    r#str = (::match_deref::match_deref! { match &(matchCase.clone()) {
        ...
        Deref @ DAE::MatchCase { result: Some(result), body, patterns, .. } => {
            ...
            patternsStr = (Patternm::patternStr(Arc::new(DAE::Pattern::PAT_META_TUPLE { patterns: patterns.clone() }))?).clone();
            resultStr = (ExpressionBasics::printExpStr(result.clone())?).clone();
            bodyStr = stringAppendList(List::map1(body.clone(), Arc::new(DAEDump::ppStmtStr), 8));
...
pub fn printRowStr(mut es_1: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut stringDelimiter: ArcStr) -> ArcStr {
    let mut s: ArcStr = arcstr::literal!("");
    s = stringDelimitList(List::map3(es_1.clone(), Arc::new(printExp2Str), (stringDelimiter.clone()).clone(), None, None), (literal!(",")).clone());
    s
}
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Exp::CALL { expLst: args, path: fcn, .. } => {
                    ...
                    fs = (AbsynUtil::pathString(fcn.clone(), (literal!(".")).clone(), true, false)?).clone();
                    argnodes = List::map(args.clone(), Arc::new(dumpExpGraphviz));
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Exp::ARRAY { array: es, .. } => {
                    ...
                    nodes = List::map(es.clone(), Arc::new(dumpExpGraphviz));
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Exp::MATRIX { matrix: lstes, .. } => {
                    let mut s: ArcStr = arcstr::literal!("");
                    s = stringDelimitList(List::map1(lstes.clone(), Arc::new(fnptr!(printRowStr, Arc<metamodelica::List<Arc<DAE::Exp>>>, ArcStr)), (literal!("\"")).clone()), (literal!("},{")).clone());
                    s = stringAppendList(list![(literal!("{{")).clone(), (s.clone()).clone(), (literal!("}}")).clone()]);
...
pub fn dumpExpStr(mut inExp: Arc<DAE::Exp>, mut inInteger: i32) -> Result<ArcStr> {
    ...
    outString = ('mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::ENUM_LITERAL { index: i, name: fcn }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
                    let mut s: ArcStr = arcstr::literal!("");
                    let mut istr: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::CREF { ty, componentRef: c }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
                    let mut s: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (exp @ Deref @ DAE::Exp::BINARY { exp2: e2, operator: op, exp1: e1 }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
                    let mut sym: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::UNARY { exp: e, operator: op }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
                    let mut sym: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::LBINARY { exp2: e2, operator: op, exp1: e1 }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
                    let mut sym: ArcStr = arcstr::literal!("");
                    let mut lt: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::LUNARY { exp: e, operator: op }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
                    let mut sym: ArcStr = arcstr::literal!("");
                    let mut ct: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::RELATION { exp2: e2, operator: op, exp1: e1, .. }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
                    let mut sym: ArcStr = arcstr::literal!("");
                    let mut lt: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::IFEXP { expElse: f, expThen: t, expCond: cond }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
                    let mut ct: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::CALL { expLst: args, path: fcn, .. }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
                    let mut fs: ArcStr = arcstr::literal!("");
                    ...
                    fs = (AbsynUtil::pathString(fcn.clone(), (literal!(".")).clone(), true, false)?).clone();
                    new_level1 = level.clone() + 1;
                    argnodes = List::map1(args.clone(), Arc::new(dumpExpStr), new_level1.clone());
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::PARTEVALFUNCTION { expList: args, path: fcn, .. }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
                    let mut fs: ArcStr = arcstr::literal!("");
                    let mut argnodes_1: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::ARRAY { ty: tp, scalar: b, array: es }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
                    let mut s: ArcStr = arcstr::literal!("");
                    ...
                    gen_str = (genStringNTime((literal!("   |")).clone(), level.clone())?).clone();
                    new_level1 = level.clone() + 1;
                    nodes = List::map1(es.clone(), Arc::new(dumpExpStr), new_level1.clone());
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::RANGE { stop, step: None, start, .. }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::RANGE { stop, step: Some(step), start, .. }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
                    let mut t1: ArcStr = arcstr::literal!("");
                    let mut t2: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::ASUB { sub: Deref @ metamodelica::List::Cons { head: Deref @ DAE::Subscript::INDEX { exp: Deref @ DAE::Exp::ICONST { integer: i } }, tail: Deref @ metamodelica::List::Nil }, exp: e }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
                    let mut s: ArcStr = arcstr::literal!("");
                    let mut ct: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::SIZE { sz: Some(dim), exp: cr }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::SIZE { sz: None, exp: cr }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
                    let mut crt: ArcStr = arcstr::literal!("");
                    let mut new_level1: i32 = 0;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::REDUCTION { iterators: Deref @ metamodelica::List::Cons { head: Deref @ DAE::ReductionIterator { exp: iterexp, .. }, tail: Deref @ metamodelica::List::Nil }, expr: exp, reductionInfo: Deref @ DAE::ReductionInfo { .. } }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::RECORD { exps: args, path: fcn, .. }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
                    let mut fs: ArcStr = arcstr::literal!("");
                    let mut argnodes_1: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::RSUB { ty: tp, fieldName: fs, ix: i, exp: e }, level) => {
                    let mut gen_str: ArcStr = arcstr::literal!("");
                    let mut res_str: ArcStr = arcstr::literal!("");
                    let mut s: ArcStr = arcstr::literal!("");
                    let mut ct: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Exp::ARRAY { ty: _, scalar: _, array: expl } => {
                    let mut s1: ArcStr = arcstr::literal!("");
                    s1 = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("{")); __mm_s.push_str(&*stringAppendList(List::map(expl.clone(), Arc::new(debugPrintComponentRefExp)))); __mm_s.push_str(&*literal!("}")); ArcStr::from(__mm_s) }).clone();
                    Ok(s1.clone())
                }
                _ => bail!("nomatch"),
            }}
...
pub fn printSubscript(mut inSubscript: Arc<DAE::Subscript>) -> Result<()> {
    let _ = (::match_deref::match_deref! { match &(inSubscript.clone()) {
        Deref @ DAE::Subscript::WHOLEDIM => {
...
pub fn printExp(mut e: Arc<DAE::Exp>) -> Result<()> {
    Tpl::tplPrint2(Arc::new(ExpressionDumpTpl::dumpExp), e.clone(), (literal!("\"")).clone())?;
    Ok(())
}
...
pub fn clockKindString(mut inClockKind: Arc<DAE::ClockKind>) -> Result<ArcStr> {
    ...
    outString = ((::match_deref::match_deref! { match &(inClockKind.clone()) {
        ...
        Deref @ DAE::ClockKind::EVENT_CLOCK { startInterval, condition } => {
