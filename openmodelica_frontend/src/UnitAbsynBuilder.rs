...
pub(crate) fn registerUnitWeights(mut cache: FCore::Cache, mut env: FCore::Graph, mut dae: DAE::DAElist) -> Result<()> {
    ...
    let () = (match dae {
        ...
        DAE::DAElist { elementLst: ref elts } => {
            ...
            du = List::unionList(List::map1(paths, (std::sync::Arc::new(fnptr!(retrieveUnitsFromEnv, Arc<Absyn::Path>, (FCore::Cache, FCore::Graph))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<Absyn::Path>, (FCore::Cache, FCore::Graph)) -> Result<Arc<metamodelica::List<Arc<SCode::Element>>>> + 'static>), (cache, env))?)?;
...
fn registerUnitWeightDefineunits(mut du: Arc<metamodelica::List<Arc<SCode::Element>>>) -> () {
    let () = (::match_deref::match_deref! { match &(du.clone()) {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cl @ Deref @ Absyn::Class { .. }, pa, i) => {
                    let mut defunits: Arc<metamodelica::List<Arc<Absyn::Element>>>;
...
pub(crate) fn add(mut unit: UnitAbsyn::Unit, mut ist: UnitAbsyn::Store) -> Result<(UnitAbsyn::Store, i32)> {
    ...
    (outSt, index) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ...
            vector = metamodelica::arrayUpdate(vector.clone(), newIndx.clone(), Some(unit.clone()))?;
...
pub(crate) fn updateInstStore(mut store: UnitAbsyn::InstStore, mut st: UnitAbsyn::Store) -> Result<UnitAbsyn::InstStore> {
    let mut outStore: UnitAbsyn::InstStore;
    outStore = (match store {
        UnitAbsyn::InstStore::INSTSTORE { store: _, ht: mut ht, checkResult: mut res } => {
            UnitAbsyn::InstStore::INSTSTORE { store: st, ht: ht.clone(), checkResult: res.clone() }
        },
...
pub(crate) fn update(mut unit: UnitAbsyn::Unit, mut index: i32, mut st: UnitAbsyn::Store) -> Result<UnitAbsyn::Store> {
    ...
    outSt = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ...
            metamodelica::print((intString(index)).clone());
...
pub(crate) fn find(mut index: i32, mut st: UnitAbsyn::Store) -> Result<UnitAbsyn::Unit> {
    ...
    unit = 'mc: {
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ...
            let __pa0 = ::match_deref::match_deref! { match &(({let __elt = vector.borrow()[(index-1) as usize].clone(); __elt})) {
...
pub(crate) fn instGetStore(mut store: UnitAbsyn::InstStore) -> Result<UnitAbsyn::Store> {
...
fn emptyInstStore2(mut wantInstStore: bool) -> UnitAbsyn::InstStore {
    ...
    st = (match wantInstStore {
        true => {
            ...
            UnitAbsyn::InstStore::INSTSTORE { store: s, ht: ht, checkResult: None }
        },
        _ => {
...
pub(crate) fn printTerms(mut terms: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>) -> Result<()> {
    metamodelica::print((printTermsStr(terms)?).clone());
    Ok(())
}

pub(crate) fn printTermsStr(mut terms: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>) -> Result<ArcStr> {
    let mut r#str: ArcStr;
    r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("{")); __mm_s.push_str(&*stringDelimitList(List::map(terms, (std::sync::Arc::new(printTermStr) as std::sync::Arc<dyn ::std::ops::Fn(Arc<UnitAbsyn::UnitTerm>) -> Result<ArcStr> + 'static>))?, (literal!(",")).clone())); __mm_s.push_str(&*literal!("}")); ArcStr::from(__mm_s) }).clone();
    Ok(r#str)
}

pub(crate) fn printTermStr(mut term: Arc<UnitAbsyn::UnitTerm>) -> Result<ArcStr> {
    let mut r#str: ArcStr;
    r#str = ((::match_deref::match_deref! { match &(term) {
...
pub(crate) fn printStore(mut st: UnitAbsyn::Store) -> Result<()> {
    let () = (match st {
        UnitAbsyn::Store { storeVector: mut vector, numElts: _ } => {
            let mut lst: Arc<metamodelica::List<Option<UnitAbsyn::Unit>>>;
...
fn printStore2(mut lst: Arc<metamodelica::List<Option<UnitAbsyn::Unit>>>, mut indx: i32) -> Result<()> {
    let () = (::match_deref::match_deref! { match &(lst) {
        ...
        Deref @ metamodelica::List::Cons { head: Some(unit), tail: rest } => {
            metamodelica::print((intString(indx)).clone());
            metamodelica::print((literal!("->")).clone());
            printUnit(unit.clone())?;
            metamodelica::print((literal!("\n")).clone());
            printStore2(rest.clone(), indx + 1)?;
            ()
        },
...
fn printBaseUnitsStr(mut lst: Arc<metamodelica::List<MMath::Rational>>) -> ArcStr {
    ...
    r#str = ((::match_deref::match_deref! { match &(lst.clone()) {
...
pub(crate) fn splitRationals(mut inRationals: Arc<metamodelica::List<MMath::Rational>>) -> Result<(Arc<metamodelica::List<i32>>, Arc<metamodelica::List<i32>>)> {
...
pub(crate) fn joinRationals(mut inums: Arc<metamodelica::List<i32>>, mut idenoms: Arc<metamodelica::List<i32>>) -> Result<Arc<metamodelica::List<MMath::Rational>>> {
...
pub(crate) fn joinTypeParams(mut inums: Arc<metamodelica::List<i32>>, mut idenoms: Arc<metamodelica::List<i32>>, mut itpstrs: Arc<metamodelica::List<ArcStr>>, mut funcInstIdOpt: Option<i32>) -> Result<Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>> {
    ...
    typeParams = (::match_deref::match_deref! { match &((inums, idenoms, itpstrs)) {
        ...
        (Deref @ metamodelica::List::Cons { head: i1, tail: nums }, Deref @ metamodelica::List::Cons { head: i2, tail: denoms }, Deref @ metamodelica::List::Cons { head: tpParam, tail: tpstrs }) => {
            ...
            s = (Util::applyOptionOrDefault(funcInstIdOpt, (std::sync::Arc::new(fnptr!(intString, i32)) as std::sync::Arc<dyn ::std::ops::Fn(i32) -> Result<ArcStr> + 'static>), (literal!("")).clone())?).clone();
            tpParam = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*tpParam.clone()); __mm_s.push_str(&*s); ArcStr::from(__mm_s) }).clone();
            metamodelica::cons((MMath::Rational { nom: i1.clone(), denom: i2.clone() }, UnitAbsyn::TypeParameter { name: (tpParam.clone()).clone(), indx: 0 }), typeParams)
        },
        _ => bail!("match: no arm matched"),
    } });
    Ok(typeParams)
...
pub(crate) fn splitTypeParams(mut iTypeParams: Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>) -> Result<(Arc<metamodelica::List<i32>>, Arc<metamodelica::List<i32>>, Arc<metamodelica::List<ArcStr>>)> {
    let mut nums: Arc<metamodelica::List<i32>> = metamodelica::nil();
    let mut denoms: Arc<metamodelica::List<i32>> = metamodelica::nil();
    ...
    (nums, denoms, tpstrs) = (::match_deref::match_deref! { match &(iTypeParams) {
        ...
        Deref @ metamodelica::List::Cons { head: (MMath::Rational { nom: i1, denom: i2 }, UnitAbsyn::TypeParameter { name: tpParam, indx: _ }), tail: typeParams } => {
            ...
            (metamodelica::cons(i1.clone(), nums), metamodelica::cons(i2.clone(), denoms), metamodelica::cons((tpParam.clone()).clone(), tpstrs))
        },
        _ => bail!("match: no arm matched"),
    } });
...
pub(crate) fn instBuildUnitTerms(mut env: FCore::Graph, mut dae: DAE::DAElist, mut compDae: DAE::DAElist, mut store: UnitAbsyn::InstStore) -> Result<(UnitAbsyn::InstStore, Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>)> {
    let mut outStore: UnitAbsyn::InstStore;
    let mut terms: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>> = metamodelica::nil();
    (outStore, terms) = 'mc: {
        let __mc_input = store;
        if let Ok(__v) = (|| -> Result<_> {
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ...
            let mut terms2: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>;
            ...
            Ok(((UnitAbsyn::InstStore::INSTSTORE { store: st.clone(), ht: ht.clone(), checkResult: res.clone() }, terms.clone()), terms.clone()))
...
pub(crate) fn buildUnitTerms(mut env: FCore::Graph, mut dae: DAE::DAElist) -> Result<(Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>, UnitAbsyn::Store, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, i32)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(i32) -> Result<ArcStr> + 'static>)))> {
    let mut terms: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>;
    let mut store: UnitAbsyn::Store;
    let mut ht: (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, i32)>>), i32, (HashTable::FuncHashCref, HashTable::FuncCrefEqual, HashTable::FuncCrefStr, HashTable::FuncExpStr));
    (store, ht) = buildStores(dae.clone())?;
    (terms, store) = buildTerms(env, dae, ht.clone(), store)?;
    store = createTypeParameterLocations(store)?;
    Ok((terms, store, ht))
}

pub(crate) fn instAddStore(mut istore: UnitAbsyn::InstStore, mut itp: Arc<DAE::Type>, mut cr: Arc<DAE::ComponentRef>) -> UnitAbsyn::InstStore {
...
pub(crate) fn storeSize(mut store: UnitAbsyn::Store) -> Result<i32> {
    let mut size: i32 = 0;
    size = (match store {
...
fn createTypeParameterLocations2(mut istore: UnitAbsyn::Store, mut iht: (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, i32)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(i32) -> Result<ArcStr> + 'static>)), mut i: i32, mut inextElt: i32) -> Result<(UnitAbsyn::Store, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, i32)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(i32) -> Result<ArcStr> + 'static>)), i32)> {
    ...
    (outStore, outHt, outNextElt) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ...
            let true = (i > numElts.clone()) else { bail!("pattern mismatch") };
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ...
            let mut store: UnitAbsyn::Store;
            let __pa0 = ::match_deref::match_deref! { match &(({let __elt = vect.borrow()[(i-1) as usize].clone(); __elt})) {
                Some(__pa0) => __pa0.clone(),
                _ => bail!("pattern mismatch"),
            } };
            unit = __pa0.clone();
            ...
            (store, ht, nextElt) = createTypeParameterLocations2(UnitAbsyn::Store { storeVector: vect.clone(), numElts: numElts.clone() }, ht.clone(), i + 1, nextElt.clone())?;
            Ok((store.clone(), ht.clone(), nextElt.clone()))
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            let (UnitAbsyn::Store { storeVector: mut vect, numElts: mut numElts }, mut ht, mut nextElt) = __mc_input.clone() else { bail!("nomatch") };
...
fn createTypeParameterLocations3(mut unit: UnitAbsyn::Unit, mut iht: (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, i32)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(i32) -> Result<ArcStr> + 'static>)), mut inextElt: i32) -> Result<(UnitAbsyn::Unit, (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, i32)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(i32) -> Result<ArcStr> + 'static>)), i32)> {
    ...
    let mut outHt: (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, i32)>>), i32, (HashTable::FuncHashCref, HashTable::FuncCrefEqual, HashTable::FuncCrefStr, HashTable::FuncExpStr));
    let mut outNextElt: i32;
    (outUnit, outHt, outNextElt) = (::match_deref::match_deref! { match &((unit, iht, inextElt)) {
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (DAE::DAElist { elementLst: Deref @ metamodelica::List::Cons { head: Deref @ DAE::Element::EQUATION { exp: e1, scalar: e2, source: _ }, tail: elts } }, store) => {
                    ...
                    let mut terms1: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>;
                    let mut terms2: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>;
                    ...
                    Ok(((metamodelica::cons(Arc::new(UnitAbsyn::UnitTerm::EQN { ut1: ut1.clone(), ut2: ut2.clone(), origExp: Arc::new(DAE::Exp::BINARY { exp1: e1.clone(), operator: DAE::Operator::SUB { ty: DAE::T_REAL_DEFAULT().clone() }, exp2: e2.clone() }) }), terms.clone()), store.clone()), terms.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { terms = __wb0; break 'mc __v; }
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (DAE::DAElist { elementLst: Deref @ metamodelica::List::Cons { head: Deref @ DAE::Element::EQUEQUATION { cr1, cr2, source: _ }, tail: elts } }, store) => {
                    ...
                    (terms, store) = buildTerms(env.clone(), DAE::DAElist { elementLst: elts.clone() }, ht.clone(), store.clone())?;
                    terms = listAppend(terms1.clone(), listAppend(terms2.clone(), terms.clone()));
                    Ok(((metamodelica::cons(Arc::new(UnitAbsyn::UnitTerm::EQN { ut1: ut1.clone(), ut2: ut2.clone(), origExp: Arc::new(DAE::Exp::BINARY { exp1: crefExp1.clone(), operator: DAE::Operator::SUB { ty: DAE::T_REAL_DEFAULT().clone() }, exp2: crefExp2.clone() }) }), terms.clone()), store.clone()), terms.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { terms = __wb0; break 'mc __v; }
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (DAE::DAElist { elementLst: Deref @ metamodelica::List::Cons { head: Deref @ DAE::Element::VAR { componentRef: cr1 @ Deref @ DAE::ComponentRef::CREF_IDENT { ident: _, identType: _, subscriptLst: _ }, binding: Some(e1), .. }, tail: elts } }, store) => {
                    ...
                    (terms, store) = buildTerms(env.clone(), DAE::DAElist { elementLst: elts.clone() }, ht.clone(), store.clone())?;
                    terms = listAppend(terms1.clone(), listAppend(terms2.clone(), terms.clone()));
                    Ok(((metamodelica::cons(Arc::new(UnitAbsyn::UnitTerm::EQN { ut1: ut1.clone(), ut2: ut2.clone(), origExp: Arc::new(DAE::Exp::BINARY { exp1: crefExp1.clone(), operator: DAE::Operator::SUB { ty: DAE::T_REAL_DEFAULT().clone() }, exp2: e1.clone() }) }), terms.clone()), store.clone()), terms.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { terms = __wb0; break 'mc __v; }
...
fn buildTermExp(mut env: FCore::Graph, mut exp: Arc<DAE::Exp>, mut idivOrMul: bool, mut iht: (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, i32)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(i32) -> Result<ArcStr> + 'static>)), mut istore: UnitAbsyn::Store) -> Result<(Arc<UnitAbsyn::UnitTerm>, Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>, UnitAbsyn::Store)> {
    ...
    let mut extraTerms: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>;
    let mut outStore: UnitAbsyn::Store;
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (e @ Deref @ DAE::Exp::ARRAY { ty: _, scalar: _, array: expl }, _, ht, store) => {
                    let mut terms: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>;
                    let mut uts: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>;
                    let mut store = (*store).clone();
                    let mut ut: Arc<UnitAbsyn::UnitTerm> = ut.clone();
...
fn buildTermCall(mut env: FCore::Graph, mut path: Arc<Absyn::Path>, mut funcCallExp: Arc<DAE::Exp>, mut expl: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut divOrMul: bool, mut ht: (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, i32)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(i32) -> Result<ArcStr> + 'static>)), mut istore: UnitAbsyn::Store) -> Result<(Arc<UnitAbsyn::UnitTerm>, Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>, UnitAbsyn::Store)> {
    ...
    (ut, extraTerms, outStore) = (match istore {
        mut store => {
            let mut formalParamIndxs: Arc<metamodelica::List<i32>>;
            let mut funcInstId: i32;
            let mut actTermLst: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>;
            let mut terms: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>;
            let mut extraTerms2: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>;
            let mut functp: Arc<DAE::Type>;
            (_, functp, _) = Lookup::lookupType(FCore::noCache(), env.clone(), path, None)?;
            funcInstId = tick();
            (store, formalParamIndxs) = buildFuncTypeStores(functp.clone(), funcInstId, store.clone())?;
            (actTermLst, extraTerms, store) = buildTermExpList(env, expl, ht, store.clone())?;
            terms = buildFormal2ActualParamTerms(formalParamIndxs, actTermLst)?;
            let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(buildResultTerms(functp, funcInstId, funcCallExp, store.clone())?) {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Type::T_FUNCTION { funcArg: _, funcResultType: functp, functionAttributes: _, path: _ }, store) => {
                    ...
                    unit = str2unit((unitStr.clone()).clone(), Some(funcInstId))?;
        ...
        if let Ok((__v, __wb0, __wb1)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Type::T_FUNCTION { funcResultType: Deref @ DAE::Type::T_TUPLE { types: typeLst, .. }, .. }, store) => {
                    ...
                    let mut terms: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>> = terms.clone();
                    (terms, extraTerms, store) = buildTupleResultTerms(typeLst.clone(), funcInstId, funcCallExp.clone(), store.clone())?;
                    Ok(((terms.clone(), extraTerms.clone(), store.clone()), extraTerms.clone(), terms.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { extraTerms = __wb0; terms = __wb1; break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                _ => {
...
fn buildTupleResultTerms(mut ifunctps: Arc<metamodelica::List<Arc<DAE::Type>>>, mut funcInstId: i32, mut funcCallExp: Arc<DAE::Exp>, mut istore: UnitAbsyn::Store) -> Result<(Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>, Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>, UnitAbsyn::Store)> {
    ...
    let mut extraTerms: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>> = metamodelica::nil();
    let mut outStore: UnitAbsyn::Store;
    (terms, extraTerms, outStore) = (::match_deref::match_deref! { match &((ifunctps, istore)) {
        ...
        (Deref @ metamodelica::List::Cons { head: tp, tail: functps }, store) => {
            let mut terms1: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>;
            let mut terms2: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>;
            let mut extraTerms1: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>;
            let mut extraTerms2: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>;
            let mut store = (*store).clone();
            (terms1, extraTerms1, store) = buildResultTerms(tp.clone(), funcInstId, funcCallExp.clone(), store.clone())?;
            (terms2, extraTerms2, store) = buildTupleResultTerms(functps.clone(), funcInstId, funcCallExp, store.clone())?;
            terms = listAppend(terms1, terms2);
            extraTerms = listAppend(extraTerms1, extraTerms2);
            (terms, extraTerms, store.clone())
        },
        _ => bail!("match: no arm matched"),
    } });
    Ok((terms, extraTerms, outStore))
        ...
        if let Ok((__v, __wb0, __wb1)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: e, tail: expl }, store) => {
                    let mut eterms1: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>;
                    let mut eterms2: Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>;
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Type::T_FUNCTION { funcArg: args, .. }, store) => {
                    ...
                    (store, indxs) = buildFuncTypeStores2(args.clone(), funcInstId, store.clone())?;
                    Ok(((store.clone(), indxs.clone()), indxs.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { indxs = __wb0; break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (tp, _) => {
...
fn buildFuncTypeStores2(mut ifargs: Arc<metamodelica::List<Arc<DAE::FuncArg>>>, mut funcInstId: i32, mut istore: UnitAbsyn::Store) -> Result<(UnitAbsyn::Store, Arc<metamodelica::List<i32>>)> {
    let mut outStore: UnitAbsyn::Store;
    let mut indxs: Arc<metamodelica::List<i32>> = metamodelica::nil();
    (outStore, indxs) = (::match_deref::match_deref! { match &((ifargs, istore)) {
        (Deref @ metamodelica::List::Nil, store) => {
        ...
        (Deref @ metamodelica::List::Cons { head: Deref @ DAE::FuncArg { ty: tp, .. }, tail: fargs }, store) => {
            ...
            unit = str2unit((unitStr.clone()).clone(), Some(funcInstId))?;
            unit = if (0 == stringCompare((unitStr).clone(), (literal!("")).clone())) {crate::UnitAbsyn::Unit::UNSPECIFIED} else {unit};
            (store, indx) = add(unit, store.clone())?;
            (store, indxs) = buildFuncTypeStores2(fargs.clone(), funcInstId, store.clone())?;
            (store.clone(), metamodelica::cons(indx, indxs))
        },
        _ => bail!("match: no arm matched"),
    } });
    Ok((outStore, indxs))
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: loc1, tail: formalParamIndxs }, Deref @ metamodelica::List::Cons { head: ut, tail: actualParamIndxs }) => {
                    ...
                    Ok((metamodelica::cons(Arc::new(UnitAbsyn::UnitTerm::EQN { ut1: Arc::new(UnitAbsyn::UnitTerm::LOC { loc: loc1.clone(), origExp: e.clone() }), ut2: ut.clone(), origExp: e.clone() }), terms.clone()), terms.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { terms = __wb0; break 'mc __v; }
...
fn origExpInTerm(mut ut: Arc<UnitAbsyn::UnitTerm>) -> Result<Arc<DAE::Exp>> {
    ...
    origExp = (::match_deref::match_deref! { match &(ut) {
        Deref @ UnitAbsyn::UnitTerm::ADD { ut1: _, ut2: _, origExp: e } => {
            e.clone()
        },
        Deref @ UnitAbsyn::UnitTerm::SUB { ut1: _, ut2: _, origExp: e } => {
...
fn buildTermOp(mut ut1: Arc<UnitAbsyn::UnitTerm>, mut ut2: Arc<UnitAbsyn::UnitTerm>, mut op: DAE::Operator, mut origExp: Arc<DAE::Exp>) -> Result<Arc<UnitAbsyn::UnitTerm>> {
    let mut ut: Arc<UnitAbsyn::UnitTerm>;
    ut = (match op {
        DAE::Operator::ADD { .. } => Arc::new(UnitAbsyn::UnitTerm::ADD { ut1: ut1, ut2: ut2, origExp: origExp }),
        DAE::Operator::SUB { .. } => Arc::new(UnitAbsyn::UnitTerm::SUB { ut1: ut1, ut2: ut2, origExp: origExp }),
        DAE::Operator::MUL { .. } => Arc::new(UnitAbsyn::UnitTerm::MUL { ut1: ut1, ut2: ut2, origExp: origExp }),
        DAE::Operator::DIV { .. } => Arc::new(UnitAbsyn::UnitTerm::DIV { ut1: ut1, ut2: ut2, origExp: origExp }),
        _ => bail!("match: no arm matched"),
    });
    Ok(ut)
}
...
pub(crate) fn unitMultiply(mut u1: UnitAbsyn::Unit, mut u2: UnitAbsyn::Unit) -> Result<UnitAbsyn::Unit> {
    ...
    u = (match (u1, u2) {
        (UnitAbsyn::Unit::SPECIFIED { specified: UnitAbsyn::SpecUnit { typeParameters: ref tparams1, units: ref units1 } }, UnitAbsyn::Unit::SPECIFIED { specified: UnitAbsyn::SpecUnit { typeParameters: ref tparams2, units: ref units2 } }) => {
            let mut tparams: Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>;
            let mut units: Arc<metamodelica::List<MMath::Rational>>;
            ...
            UnitAbsyn::Unit::SPECIFIED { specified: UnitAbsyn::SpecUnit { typeParameters: tparams, units: units } }
        },
        _ => bail!("match: no arm matched"),
    });
    Ok(u)
...
pub(crate) fn unit2str(mut unit: UnitAbsyn::Unit) -> Result<ArcStr> {
    ...
    res = ((match unit {
        UnitAbsyn::Unit::SPECIFIED { specified: UnitAbsyn::SpecUnit { typeParameters: ref typeParams, units: mut units } } => {
            let mut nums: Arc<metamodelica::List<i32>>;
            let mut denoms: Arc<metamodelica::List<i32>>;
            let mut tpnoms: Arc<metamodelica::List<i32>>;
            let mut tpdenoms: Arc<metamodelica::List<i32>>;
            let mut tpstrs: Arc<metamodelica::List<ArcStr>>;
            (nums, denoms) = splitRationals(units.clone())?;
            (tpnoms, tpdenoms, tpstrs) = splitTypeParams(typeParams.clone())?;
            res = (UnitParserExt::unit2str(nums, denoms, tpnoms, tpdenoms, tpstrs, metamodelica::OrderedFloat(1.0_f64), metamodelica::OrderedFloat(0.0_f64))).clone();
            res
        },
        UnitAbsyn::Unit::UNSPECIFIED { .. } => {
            literal!("unspecified")
...
pub fn str2unit(mut res: ArcStr, mut funcInstIdOpt: Option<i32>) -> Result<UnitAbsyn::Unit> {
    let mut unit: UnitAbsyn::Unit;
    (unit, _, _) = str2unitWithScaleFactor((res).clone(), funcInstIdOpt)?;
    Ok(unit)
}
...
pub fn str2unitWithScaleFactor(mut res: ArcStr, mut funcInstIdOpt: Option<i32>) -> Result<(UnitAbsyn::Unit, metamodelica::Real, metamodelica::Real)> {
    ...
    let mut tpdenoms: Arc<metamodelica::List<i32>>;
    let mut tpstrs: Arc<metamodelica::List<ArcStr>>;
    let mut typeParams: Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>;
    let mut units: Arc<metamodelica::List<MMath::Rational>>;
    (nums, denoms, tpnoms, tpdenoms, tpstrs, scaleFactor, offset) = UnitParserExt::str2unit((res).clone())?;
    units = joinRationals(nums, denoms)?;
    typeParams = joinTypeParams(tpnoms, tpdenoms, tpstrs, funcInstIdOpt)?;
    unit = UnitAbsyn::Unit::SPECIFIED { specified: UnitAbsyn::SpecUnit { typeParameters: typeParams, units: units } };
...
pub fn getDerivedUnits(mut baseUnit: UnitAbsyn::Unit, mut baseUnitStr: ArcStr) -> Result<Arc<metamodelica::List<ArcStr>>> {
    let mut derivedUnits: Arc<metamodelica::List<ArcStr>>;
    let mut unitSymbols: Arc<metamodelica::List<ArcStr>>;
    unitSymbols = UnitParserExt::allUnitSymbols();
    derivedUnits = getDerivedUnitsHelper(baseUnit, (baseUnitStr).clone(), unitSymbols)?;
    Ok(derivedUnits)
}
...
*/
pub(crate) fn buildTest1() -> Result<(Arc<metamodelica::List<Arc<UnitAbsyn::UnitTerm>>>, UnitAbsyn::Store)> {
