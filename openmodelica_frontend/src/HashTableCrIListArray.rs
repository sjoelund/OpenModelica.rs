...
pub type Value = (Arc<metamodelica::List<i32>>, metamodelica::Array<i32>);

pub type HashTableCrefFunctionsType = (FuncHashCref, FuncCrefEqual, FuncCrefStr, FuncExpStr);

pub type HashTable = (metamodelica::Array<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<DAE::ComponentRef>, (Arc<metamodelica::List<i32>>, metamodelica::Array<i32>))>>), i32, (FuncHashCref, FuncCrefEqual, FuncCrefStr, FuncExpStr));
...
pub fn emptyHashTableSized(mut size: i32) -> HashTable {
    let mut hashTable: HashTable;
    hashTable = BaseHashTable::emptyHashTableWork(size.clone(), (ComponentReference::hashComponentRef, ComponentReferenceBasics::crefEqual, ComponentReferenceBasics::printComponentRefStr, fnptr!(printIntListArrayStr, (Arc<metamodelica::List<i32>>, metamodelica::Array<i32>))));
    hashTable
}
...
pub fn printIntListArrayStr(mut iValue: (Arc<metamodelica::List<i32>>, metamodelica::Array<i32>)) -> ArcStr {
    ...
    res = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("[")); __mm_s.push_str(&*stringDelimitList(List::map(iList.clone(), Arc::new(fnptr!(intString, i32))), (literal!(",")).clone())); __mm_s.push_str(&*literal!("] {")); __mm_s.push_str(&*stringDelimitList(List::mapArray(iArray.clone(), Arc::new(fnptr!(intString, i32))), (literal!(",")).clone())); __mm_s.push_str(&*literal!("}")); ArcStr::from(__mm_s) }).clone();
    res
}
