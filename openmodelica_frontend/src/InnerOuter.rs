...
fn changeInnerOuterInOuterConnect2(mut inOC: DAE::Connect::OuterConnect) -> DAE::Connect::OuterConnect {
    ...
    outOC = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ...
            let mut ncr1: Arc<DAE::ComponentRef>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            let DAE::Connect::OuterConnect { scope: mut scope, cr1: mut cr1, io1: mut io1, f1: mut f1, cr2: mut cr2, io2: mut io2, f2: mut f2, source: mut source } = __mc_input.clone() else { bail!("nomatch") };
            let mut ncr2: Arc<DAE::ComponentRef>;
...
fn removeInnerPrefixFromCref(mut inPrefix: DAE::Prefix, mut inCref: Arc<DAE::ComponentRef>) -> Arc<DAE::ComponentRef> {
    ...
    outCref = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            let _ = __mc_input.clone() else { bail!("nomatch") };
            let mut crefPrefix: Arc<DAE::ComponentRef>;
            let mut crOuter: Arc<DAE::ComponentRef>;
...
fn retrieveOuterConnections2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: InstHierarchy, mut inPrefix: DAE::Prefix, mut inOuterConnects: Arc<metamodelica::List<DAE::Connect::OuterConnect>>, mut inSets: DAE::Connect::Sets, mut inTopCall: bool, mut inCGraph: ConnectionGraph::ConnectionGraph) -> Result<(Arc<metamodelica::List<DAE::Connect::OuterConnect>>, DAE::Connect::Sets, Arc<metamodelica::List<DAE::Connect::OuterConnect>>, ConnectionGraph::ConnectionGraph)> {
    let mut outOuterConnects: Arc<metamodelica::List<DAE::Connect::OuterConnect>>;
    let mut outSets: DAE::Connect::Sets;
    let mut outInnerOuterConnects: Arc<metamodelica::List<DAE::Connect::OuterConnect>>;
    let mut outCGraph: ConnectionGraph::ConnectionGraph;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: DAE::Connect::OuterConnect { scope, cr1, io1, f1, cr2, io2, f2, source: source @ Deref @ DAE::ElementSource { info, .. } }, tail: rest_oc }, sets, _, graph) => {
                    let mut ioc: Arc<metamodelica::List<DAE::Connect::OuterConnect>>;
                    let mut inner1: bool;
                    let mut outer1: bool;
                    let mut added: bool;
...
fn addOuterConnectIfEmpty(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: InstHierarchy, mut pre: DAE::Prefix, mut inSets: DAE::Connect::Sets, mut added: bool, mut cr1: Arc<DAE::ComponentRef>, mut iio1: Absyn::InnerOuter, mut f1: DAE::Connect::Face, mut cr2: Arc<DAE::ComponentRef>, mut iio2: Absyn::InnerOuter, mut f2: DAE::Connect::Face, mut info: SourceInfo, mut inCGraph: ConnectionGraph::ConnectionGraph) -> Result<(DAE::Connect::Sets, ConnectionGraph::ConnectionGraph)> {
    ...
    (outSets, outCGraph) = (::match_deref::match_deref! { match &((inCache, inEnv, inIH, inSets.clone(), added, iio1, iio2, inCGraph.clone())) {
        ...
        (cache, env, ih, DAE::Connect::Sets { sets, setCount: sc, connections: cl, outerConnects: oc }, false, io1, io2, graph) => {
            ...
            let (__pa7, __pa8, __pa9, __pa10, __pa11, __pa12, __pa13) = ::match_deref::match_deref! { match &(InstSection::connectComponents(cache.clone(), env.clone(), ih.clone(), DAE::Connect::Sets { sets: sets.clone(), setCount: sc.clone(), connections: cl.clone(), outerConnects: metamodelica::nil() }, pre, cr1, f1, t1, vt1, cr2, f2, t2, vt2, ct, io1.clone(), io2.clone(), graph.clone(), info)?) {
...
pub(crate) fn switchInnerToOuterInGraph(mut inEnv: FCore::Graph, mut inCr: Arc<DAE::ComponentRef>) -> Result<FCore::Graph> {
    ...
    outEnv = (::match_deref::match_deref! { match &((inEnv.clone(), inCr)) {
        ...
        (_, cr) => {
            ...
            n = switchInnerToOuterInNode(n, cr.clone())?;
...
pub(crate) fn updateInstHierarchy(mut inIH: InstHierarchy, mut inPrefix: DAE::Prefix, mut inInnerOuter: Absyn::InnerOuter, mut inInstInner: InstInner) -> Result<InstHierarchy> {
    '__tco: loop {
        ...
        (Deref @ metamodelica::List::Cons { head: TopInstance { path: pathOpt, ht, outerPrefixes, sm }, tail: restIH }, InstInner { name, .. }) => {
            ...
            (_, cref) = PrefixUtil::prefixCref(FCore::emptyCache(), FGraph::empty(), emptyInstHierarchy().clone(), inPrefix, cref_)?;
            ht = add((cref, inInstInner), ht.clone())?;
...
pub(crate) fn updateSMHierarchy(mut smState: Arc<DAE::ComponentRef>, mut inIH: InstHierarchy) -> Result<InstHierarchy> {
    ...
    outIH = (::match_deref::match_deref! { match &((smState.clone(), inIH)) {
        (_, Deref @ metamodelica::List::Nil) => {
            ...
            tih = TopInstance { path: None, ht: ht.clone(), outerPrefixes: emptyOuterPrefixes().clone(), sm: sm2 };
            ih = list![tih];
            ih
        },
...
pub(crate) fn prefixOuterCrefWithTheInnerPrefix(mut inIH: InstHierarchy, mut inOuterComponentRef: Arc<DAE::ComponentRef>, mut inPrefix: DAE::Prefix) -> Result<Arc<DAE::ComponentRef>> {
    ...
    outInnerComponentRef = (::match_deref::match_deref! { match &(inIH) {
        ...
        Deref @ metamodelica::List::Cons { head: TopInstance { path: _, ht: _, outerPrefixes: outerPrefixes @ Deref @ metamodelica::List::Cons { head: _, tail: _ }, sm: _ }, tail: Deref @ metamodelica::List::Nil } => {
            let mut outerCrefPrefix: Arc<DAE::ComponentRef>;
...
fn changeOuterReferenceToInnerReference(mut inFullCref: Arc<DAE::ComponentRef>, mut inOuterCrefPrefix: Arc<DAE::ComponentRef>, mut inInnerCrefPrefix: Arc<DAE::ComponentRef>) -> Result<Arc<DAE::ComponentRef>> {
    ...
    outInnerCref = (::match_deref::match_deref! { match &((inFullCref, inOuterCrefPrefix, inInnerCrefPrefix)) {
        (ifull, ocp, icp) => {
            ...
            let mut eocp: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
            let mut eicp: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
            let mut epre: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
            let mut erest: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
            let mut esuffix: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
            eifull = ComponentReference::explode(ifull.clone())?;
            eicp = ComponentReference::explode(icp.clone())?;
            (eocp, esuffix) = List::split(eifull, ComponentReference::identifierCount(ocp.clone()))?;
            (epre, erest) = List::splitEqualPrefix(eocp, eicp.clone(), (std::sync::Arc::new(ComponentReferenceBasics::crefFirstIdentEqual) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>), metamodelica::nil())?;
            (_, eicp) = List::splitEqualPrefix(eicp, epre.clone(), (std::sync::Arc::new(ComponentReferenceBasics::crefFirstIdentEqual) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>), metamodelica::nil())?;
            (erest, _) = List::splitEqualPrefix(erest.reverse(), eicp.reverse(), (std::sync::Arc::new(ComponentReferenceBasics::crefFirstIdentEqual) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<DAE::ComponentRef>) -> Result<bool> + 'static>), metamodelica::nil())?;
...
pub(crate) fn getExistingInnerDeclarations(mut inIH: InstHierarchy, mut inEnv: FCore::Graph) -> Result<ArcStr> {
    ...
    innerDeclarations = ((::match_deref::match_deref! { match &(inIH) {
        ...
        Deref @ metamodelica::List::Cons { head: TopInstance { path: _, ht, outerPrefixes: _, sm: _ }, tail: _ } => {
            ...
            r#str = stringDelimitList(List::map(inners, (std::sync::Arc::new(printInnerDefStr) as std::sync::Arc<dyn ::std::ops::Fn(InstInner) -> Result<ArcStr> + 'static>))?, (literal!("\n    ")).clone());
            r#str
        },
        _ => bail!("match: no arm matched"),
    } })).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (v @ (key, _), InstHierarchyHashTable { hashTable: hashvec, valueArr: varr, bucketSize: bsize, numberOfEntries: _ }) => {
                    ...
                    let mut indexes: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, i32)>>;
...
fn get1(mut key: Key, mut hashTable: InstHierarchyHashTable) -> Result<(Value, i32)> {
    ...
    (value, indx) = (match hashTable {
        InstHierarchyHashTable { hashTable: mut hashvec, valueArr: mut varr, bucketSize: mut bsize, numberOfEntries: _ } => {
            ...
            indx = get2(key.clone(), indexes)?;
            (k, v) = valueArrayNth(varr.clone(), indx)?;
            let true = (keyEqual(k, key)?) else { bail!("pattern mismatch") };
            (v, indx)
        },
    });
    Ok((value, indx))
...
fn valueArrayList(mut valueArray: ValueArray) -> Result<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, InstInner)>>> {
    ...
    tplLst = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ...
            let mut lastpos: i32;
            let mut lst: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, InstInner)>>;
            lastpos = n.clone() - 1;
...
fn valueArrayList2(mut inVarOptionArray1: metamodelica::Array<Option<(Arc<DAE::ComponentRef>, InstInner)>>, mut inInteger2: i32, mut inInteger3: i32) -> Result<Arc<metamodelica::List<(Arc<DAE::ComponentRef>, InstInner)>>> {
    ...
    outVarLst = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ...
            let mut v: (Arc<DAE::ComponentRef>, InstInner);
            let mut pos_1: i32;
            let mut res: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, InstInner)>>;
            pos_1 = pos.clone() + 1;
            let __pa0 = ::match_deref::match_deref! { match &(({let __elt = arr.borrow()[(pos.clone() + 1-1) as usize].clone(); __elt})) {
                Some(__pa0) => __pa0.clone(),
                _ => bail!("pattern mismatch"),
