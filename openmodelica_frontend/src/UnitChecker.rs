        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Some(_), tail: lst }, st2) => {
                    ...
                    (comp1, _) = completeCheck(lst.clone(), indx + 1, st3.clone())?;
...
fn unify(mut insu1: UnitAbsyn::SpecUnit, mut insu2: UnitAbsyn::SpecUnit, mut st: UnitAbsyn::Store) -> Result<(UnitAbsyn::UnitCheckResult, UnitAbsyn::Store)> {
    ...
    let mut st2: UnitAbsyn::Store;
    let (UnitAbsyn::SPECIFIED { specified: __pa0 }, __pa1) = (normalizeOnUnit(UnitAbsyn::Unit::SPECIFIED { specified: insu1 }, st)?) else { bail!("pattern mismatch") };
    su1 = __pa0.clone();
    st1 = __pa1.clone();
    let (UnitAbsyn::SPECIFIED { specified: __pa2 }, __pa3) = (normalizeOnUnit(UnitAbsyn::Unit::SPECIFIED { specified: insu2 }, st1)?) else { bail!("pattern mismatch") };
    su2 = __pa2.clone();
    st2 = __pa3.clone();
...
pub(crate) fn mulSpecUnit(mut u1: UnitAbsyn::SpecUnit, mut u2: UnitAbsyn::SpecUnit) -> Result<UnitAbsyn::SpecUnit> {
    ...
    u = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ...
            let mut tparams3: Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>;
            let mut tparams4: Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: expo1, tail: rest1 }, Deref @ metamodelica::List::Cons { head: expo2, tail: rest2 }) => {
                    ...
                    let mut rest3: Arc<metamodelica::List<MMath::Rational>>;
...
pub(crate) fn divSpecUnit(mut u1: UnitAbsyn::SpecUnit, mut u2: UnitAbsyn::SpecUnit) -> Result<UnitAbsyn::SpecUnit> {
    ...
    u = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ...
            let mut tparams3: Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>;
            let mut tparams4: Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>;
            let mut tparams5: Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>;
            let mut units: Arc<metamodelica::List<MMath::Rational>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: (expo1, param), tail: rest1 }, expo2) => {
                    let mut expo3: MMath::Rational;
                    let mut rest2: Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: expo1, tail: rest1 }, expo2) => {
                    let mut expo3: MMath::Rational;
                    let mut rest2: Arc<metamodelica::List<MMath::Rational>>;
                    expo3 = MMath::multRational(expo1.clone(), expo2.clone())?;
...
fn negParamList(mut ine: Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>, mut ac: Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>) -> Result<Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>> {
    let mut oute: Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>;
    oute = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: (MMath::Rational { nom: i1, denom: i2 }, UnitAbsyn::TypeParameter { name, indx }), tail: rest }, ac2) => {
                    ...
                    let mut pres: Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>;
...
pub(crate) fn normalize(mut loc: i32, mut st: UnitAbsyn::Store) -> Result<(UnitAbsyn::Unit, UnitAbsyn::Store)> {
    ...
    let mut u2: UnitAbsyn::Unit;
    let mut st2: UnitAbsyn::Store;
    u1 = UnitAbsynBuilder::find(loc, st.clone())?;
...
pub(crate) fn normalizeOnUnit(mut u: UnitAbsyn::Unit, mut st: UnitAbsyn::Store) -> Result<(UnitAbsyn::Unit, UnitAbsyn::Store)> {
    ...
    (unit, outSt) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ...
            let mut params2: Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>;
            let mut params3: Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>;
            let mut unitvec2: Arc<metamodelica::List<MMath::Rational>>;
            let mut st2: UnitAbsyn::Store;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: (expo1, UnitAbsyn::TypeParameter { name, indx: loc1 }), tail: rest1 } => {
                    let mut rest2: Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>;
                    let mut rest3: Arc<metamodelica::List<(MMath::Rational, UnitAbsyn::TypeParameter)>>;
                    let mut expo2: MMath::Rational;
                    let mut expo3: MMath::Rational;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: (expo, UnitAbsyn::TypeParameter { name: _, indx: loc2 }), tail: rest } => {
                    let true = (intEq(loc2.clone(), loc)) else { bail!("pattern mismatch") };
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: param, tail: rest } => {
                    ...
                    let mut found2: bool;
                    (found2, expo, rest2) = getParam(rest.clone(), loc);
...
pub(crate) fn printSpecUnit(mut text: ArcStr, mut su: UnitAbsyn::SpecUnit) -> Result<()> {
    let () = (match (text, su.clone()) {
        (mut r#str, UnitAbsyn::SpecUnit { typeParameters: ref params, units: _ }) => {
            ...
            metamodelica::print((UnitAbsynBuilder::unit2str(UnitAbsyn::Unit::SPECIFIED { specified: su })?).clone());
            metamodelica::print((literal!("\" {")).clone());
