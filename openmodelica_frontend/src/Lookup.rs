...
pub fn lookupType(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPath: Arc<Absyn::Path>, mut msg: Option<SourceInfo>) -> Result<(FCore::Cache, Arc<DAE::Type>, FCore::Graph)> {
    ...
    (cache, t, env) = (::match_deref::match_deref! { match &(inPath.clone()) {
        Deref @ Absyn::Path::IDENT { .. } => {
            (cache, t, env) = lookupTypeIdent(inCache, inEnv, (var_field!((*inPath).name, Absyn::Path::IDENT).clone()).clone(), msg)?;
            (cache, t, env)
        },
        _ => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env_1, c @ Deref @ SCode::Element::CLASS { name: id, encapsulatedPrefix: encflag, restriction: r @ SCode::Restriction::R_ENUMERATION { .. }, .. }) => {
                    ...
                    let mut types: Arc<metamodelica::List<Arc<DAE::Var>>>;
                    let mut names: Arc<metamodelica::List<ArcStr>>;
                    let mut ci_state: ClassInf::State;
                    let mut r#mod: Arc<DAE::Mod>;
                    let mut cache = (*cache).clone();
                    env_2 = FGraph::openScope(env_1.clone(), encflag.clone(), (id.clone()).clone(), Some(openmodelica_frontend_dump::FCore::ScopeType::CLASS_SCOPE))?;
...
pub fn lookupMetarecordsRecursive(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inUniontypePaths: Arc<metamodelica::List<Arc<Absyn::Path>>>) -> Result<(FCore::Cache, Arc<metamodelica::List<Arc<DAE::Type>>>)> {
    let mut outCache: FCore::Cache;
    let mut outMetarecordTypes: Arc<metamodelica::List<Arc<DAE::Type>>>;
    (outCache, _, outMetarecordTypes) = lookupMetarecordsRecursive2(inCache, inEnv, inUniontypePaths, HashTableStringToPath::emptyHashTable(), metamodelica::nil())?;
...
fn lookupMetarecordsRecursive3(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut path: Arc<Absyn::Path>, mut r#str: ArcStr, mut inHt: (metamodelica::Array<Arc<metamodelica::List<(ArcStr, i32)>>>, (i32, i32, metamodelica::Array<Option<(ArcStr, Arc<Absyn::Path>)>>), i32, (Arc<dyn ::std::ops::Fn(ArcStr) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(ArcStr, ArcStr) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(ArcStr) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<Absyn::Path>) -> Result<ArcStr> + 'static>)), mut inAcc: Arc<metamodelica::List<Arc<DAE::Type>>>) -> Result<(FCore::Cache, (metamodelica::Array<Arc<metamodelica::List<(ArcStr, i32)>>>, (i32, i32, metamodelica::Array<Option<(ArcStr, Arc<Absyn::Path>)>>), i32, (Arc<dyn ::std::ops::Fn(ArcStr) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(ArcStr, ArcStr) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(ArcStr) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(Arc<Absyn::Path>) -> Result<ArcStr> + 'static>)), Arc<metamodelica::List<Arc<DAE::Type>>>)> {
    ...
    let mut outMetarecordTypes: Arc<metamodelica::List<Arc<DAE::Type>>>;
    (outCache, outHt, outMetarecordTypes) = (::match_deref::match_deref! { match &((inCache, inEnv, inHt, inAcc)) {
        ...
        (cache, env, ht, acc) => {
            ...
            let mut uniontypeTypes: Arc<metamodelica::List<Arc<DAE::Type>>>;
            let mut ty: Arc<DAE::Type>;
            let mut cache = (*cache).clone();
            let mut ht = (*ht).clone();
            let mut acc = (*acc).clone();
            ht = BaseHashTable::add((r#str.clone(), path.clone()), ht.clone())?;
            (cache, ty, _) = lookupType(cache.clone(), env.clone(), path, Some(Absyn::dummyInfo.clone()))?;
            acc = metamodelica::cons(ty.clone(), acc.clone());
            uniontypeTypes = Types::getAllInnerTypesOfType(ty, (std::sync::Arc::new(fnptr!(Types::uniontypeFilter, Arc<DAE::Type>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Type>) -> Result<bool> + 'static>))?;
            uniontypePaths = List::flatten(List::map(uniontypeTypes, (std::sync::Arc::new(Types::getUniontypePaths) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Type>) -> Result<Arc<metamodelica::List<Arc<Absyn::Path>>>> + 'static>))?)?;
            (cache, ht, acc) = lookupMetarecordsRecursive2(cache.clone(), env.clone(), uniontypePaths, ht.clone(), acc.clone())?;
            (cache.clone(), ht.clone(), acc.clone())
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
pub fn lookupClass(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPath: Arc<Absyn::Path>, mut inInfo: Option<SourceInfo>) -> Result<(FCore::Cache, Arc<SCode::Element>, FCore::Graph)> {
    ...
    (outCache, outClass, outEnv) = 'mc: {
        ...
        })() { outCache = __wb0; outClass = __wb1; outEnv = __wb2; break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
pub fn lookupClassIdent(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut ident: ArcStr, mut inInfo: Option<SourceInfo>) -> Result<(FCore::Cache, Arc<SCode::Element>, FCore::Graph)> {
    ...
    let mut outEnv: FCore::Graph;
    (outCache, outClass, outEnv, _) = lookupClassInEnv(inCache, inEnv, (ident).clone(), metamodelica::nil(), Mutable::create(false), inInfo)?;
    Ok((outCache, outClass, outEnv))
}
...
fn lookupClass1(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPath: Arc<Absyn::Path>, mut inPrevFrames: Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>, mut inState: Mutable::Mutable<bool>, mut inInfo: Option<SourceInfo>) -> Result<(FCore::Cache, Arc<SCode::Element>, FCore::Graph, Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>)> {
    ...
    let mut outClass: Arc<SCode::Element>;
    let mut outEnv: FCore::Graph;
    let mut outPrevFrames: Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>;
    ...
    if let Ok((__pa0, __pa1, __pa2, __pa3)) = lookupClass2(inCache.clone(), inEnv.clone(), inPath.clone(), inPrevFrames.clone(), inState.clone(), inInfo.clone()) {
        ...
        if isSome(inInfo.clone()) && errors == Error::getNumErrorMessages() {
...
fn lookupClass2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPath: Arc<Absyn::Path>, mut inPrevFrames: Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>, mut inState: Mutable::Mutable<bool>, mut inInfo: Option<SourceInfo>) -> Result<(FCore::Cache, Arc<SCode::Element>, FCore::Graph, Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>)> {
    '__tco: loop {
        ...
        (cache, env, Deref @ Absyn::Path::FULLYQUALIFIED { path }, Deref @ metamodelica::List::Nil) => {
            ...
            let mut c: Arc<SCode::Element>;
            let mut env_1: FCore::Graph;
            let mut prevFrames: Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>;
            let mut cache = (*cache).clone();
        ...
        (cache, env, Deref @ Absyn::Path::QUALIFIED { name: pack, path }, prevFrames) => {
            let mut c: Arc<SCode::Element>;
            let mut env_2: FCore::Graph;
            let mut optFrame: Option<metamodelica::Array<FCore::Node>>;
            let mut cache = (*cache).clone();
            let mut prevFrames = (*prevFrames).clone();
            (optFrame, prevFrames) = lookupPrevFrames((pack.clone()).clone(), prevFrames.clone());
            return Ok(lookupClassQualified(cache.clone(), env.clone(), (pack.clone()).clone(), path.clone(), optFrame, prevFrames.clone(), inState, inInfo)?)
        },
        (cache, env, Deref @ Absyn::Path::IDENT { name: id }, prevFrames) => {
            ...
            let mut cache = (*cache).clone();
            let mut prevFrames = (*prevFrames).clone();
            return Ok(lookupClassInEnv(cache.clone(), env.clone(), (id.clone()).clone(), prevFrames.clone(), inState, inInfo)?)
        },
...
fn lookupClassQualified(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut id: ArcStr, mut path: Arc<Absyn::Path>, mut inOptFrame: Option<metamodelica::Array<FCore::Node>>, mut inPrevFrames: Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>, mut inState: Mutable::Mutable<bool>, mut inInfo: Option<SourceInfo>) -> Result<(FCore::Cache, Arc<SCode::Element>, FCore::Graph, Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>)> {
    ...
    let mut outEnv: FCore::Graph;
    let mut outPrevFrames: Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>;
    (outCache, outClass, outEnv, outPrevFrames) = (::match_deref::match_deref! { match &((inCache, inEnv, inOptFrame, inPrevFrames)) {
        (cache, env, Some(frame), prevFrames) => {
            ...
            (cache, c, env, prevFrames) = lookupClass2(cache.clone(), env.clone(), path, prevFrames.clone(), inState, inInfo)?;
            (cache.clone(), c, env.clone(), prevFrames.clone())
        },
        (cache, env, None, _) => {
            let mut c: Arc<SCode::Element>;
            let mut prevFrames: Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>;
            ...
            (optFrame, prevFrames) = lookupPrevFrames((id).clone(), prevFrames);
            (cache, c, env, prevFrames) = lookupClassQualified2(cache.clone(), env.clone(), path, c, optFrame, prevFrames, inState, inInfo)?;
            (cache.clone(), c, env.clone(), prevFrames.clone())
        },
...
fn lookupClassQualified2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut path: Arc<Absyn::Path>, mut inC: Arc<SCode::Element>, mut optFrame: Option<metamodelica::Array<FCore::Node>>, mut inPrevFrames: Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>, mut inState: Mutable::Mutable<bool>, mut inInfo: Option<SourceInfo>) -> Result<(FCore::Cache, Arc<SCode::Element>, FCore::Graph, Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>)> {
    ...
    (outCache, outClass, outEnv, outPrevFrames) = 'mc: {
        let __mc_input = (inCache, inEnv.clone(), inC.clone(), optFrame, inPrevFrames);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
fn checkPartialScope(mut inEnv: FCore::Graph, mut inParentEnv: FCore::Graph, mut inCache: FCore::Cache, mut inInfo: Option<SourceInfo>) -> Result<()> {
    ...
    if isSome(inInfo.clone()) && FGraph::isPartialScope(inEnv.clone()) && Config::languageStandardAtLeast(Config::LanguageStandard::_3_2.clone())? {
        let FCore::N { data: FCore::CL { e: __pa0, pre: __pa1, .. }, .. } = (FNode::fromRef(FGraph::lastScopeRef(inEnv.clone())?)?) else { bail!("pattern mismatch") };
        el = __pa0.clone();
        pre = __pa1.clone();
        name = (SCodeUtil::elementName(el.clone())?).clone();
        if FGraph::graphPrefixOf(inParentEnv, inEnv.clone())? && !(PrefixUtil::isNoPrefix(pre.clone())) {
            ...
            cc_str = (getConstrainingClass(el, (FGraph::stripLastScopeRef(inEnv)?).0, inCache)?).clone();
            Error::addMultiSourceMessage(Error::USE_OF_PARTIAL_CLASS.clone(), list![(pre_str).clone(), (name).clone(), (cc_str).clone()], list![cls_info, pre_info])?;
            bail!("fail");
        } else {
            let __pa2 = ::match_deref::match_deref! { match &(inInfo) {
                Some(__pa2) => __pa2.clone(),
                _ => bail!("pattern mismatch"),
            } };
            info = __pa2.clone();
            if !(Config::getGraphicsExpMode()?) {
                Error::addSourceMessage(Error::LOOKUP_IN_PARTIAL_CLASS.clone(), list![(name).clone()], info)?;
            }
        }
    }
    Ok(())
...
fn lookupPrevFrames(mut id: ArcStr, mut inPrevFrames: Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>) -> (Option<metamodelica::Array<FCore::Node>>, Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>) {
    let mut outFrame: Option<metamodelica::Array<FCore::Node>>;
    let mut outPrevFrames: Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>;
...
fn lookupQualifiedImportedVarInFrame(mut inImports: Arc<metamodelica::List<Absyn::Import>>, mut ident: ArcStr) -> Result<Arc<DAE::ComponentRef>> {
    ...
    outCref = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Absyn::Import::NAMED_IMPORT { name: id, path }, tail: _ } => {
                    let true = (id.clone() == ident.clone()) else { bail!("pattern mismatch") };
                    Ok(ComponentReference::pathToCref(path.clone())?)
                }
                _ => bail!("nomatch"),
        ...
        if let Ok((__v, __wb0, __wb1)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, Deref @ metamodelica::List::Cons { head: Absyn::Import::UNQUAL_IMPORT { path }, tail: rest }, env, ident) => {
                    ...
                    Ok(((cache.clone(), classEnv.clone(), attr.clone(), ty.clone(), bind.clone(), cnstForRange.clone(), unique.clone(), splicedExpData.clone(), componentEnv.clone(), name.clone()), name.clone(), splicedExpData.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { name = __wb0; splicedExpData = __wb1; break 'mc __v; }
        if let Ok((__v, __wb0, __wb1)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, Deref @ metamodelica::List::Cons { head: _, tail: rest }, env, ident) => {
                    let mut unique: bool;
...
fn lookupQualifiedImportedClassInFrame(mut inCache: FCore::Cache, mut inImport: Arc<metamodelica::List<Absyn::Import>>, mut inEnv: FCore::Graph, mut inIdent: ArcStr, mut inState: Mutable::Mutable<bool>, mut inInfo: Option<SourceInfo>) -> Result<(FCore::Cache, Arc<SCode::Element>, FCore::Graph, Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>)> {
    ...
    (outCache, outClass, outEnv, outPrevFrames) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, Deref @ metamodelica::List::Cons { head: Absyn::Import::NAMED_IMPORT { name: id, path }, tail: _ }, env, ident) => {
                    let mut r: metamodelica::Array<FCore::Node>;
                    let mut c: Arc<SCode::Element>;
                    let mut env_1: FCore::Graph;
                    let mut prevFrames: Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>;
...
pub(crate) fn lookupConnectorVar(mut env: FCore::Graph, mut cr: Arc<DAE::ComponentRef>, mut firstId: bool) -> Result<(Arc<DAE::Attributes>, Arc<DAE::Type>, FCore::Status, bool)> {
    ...
    (attr, ty, status) = (::match_deref::match_deref! { match &(cr.clone()) {
        Deref @ DAE::ComponentRef::CREF_IDENT { .. } => {
            let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(lookupConnectorVar2(env, (var_field!((*cr).ident, DAE::ComponentRef::CREF_IDENT).clone()).clone())?) {
                (Deref @ DAE::Var { attributes: __pa0, ty: __pa1, .. }, __pa2, _) => (__pa0.clone(), __pa1.clone(), __pa2.clone()),
                _ => bail!("pattern mismatch"),
            } };
            attr = __pa0.clone();
            ty = __pa1.clone();
            status = __pa2.clone();
            ty = checkSubscripts(ty, var_field!((*cr).subscriptLst, DAE::ComponentRef::CREF_IDENT).clone())?;
            (attr, ty, status)
        },
        Deref @ DAE::ComponentRef::CREF_QUAL { .. } => {
            let (__pa0, __pa1, __pa2, __pa3) = ::match_deref::match_deref! { match &(lookupConnectorVar2(env, (var_field!((*cr).ident, DAE::ComponentRef::CREF_QUAL).clone()).clone())?) {
                (Deref @ DAE::Var { attributes: __pa0, ty: __pa1, .. }, __pa2, __pa3) => (__pa0.clone(), __pa1.clone(), __pa2.clone(), __pa3.clone()),
                _ => bail!("pattern mismatch"),
            } };
            parent_attr = __pa0.clone();
            ty = __pa1.clone();
            status = __pa2.clone();
            ...
            if FCore::isDeletedComp(status.clone()) {
                ...
                attr = DAEUtil::setAttrVariability(attr.clone(), SCodeUtil::variabilityOr(DAEUtil::getAttrVariability(attr), DAEUtil::getAttrVariability(parent_attr.clone())));
                if firstId {
...
fn lookupConnectorVar2(mut env: FCore::Graph, mut name: ArcStr) -> Result<(Arc<DAE::Var>, FCore::Status, FCore::Graph)> {
    ...
    let mut scope: Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>;
    let mut ht: Arc<FCore::RefTree::Tree>;
...
fn checkPackageVariableConstant(mut parentEnv: FCore::Graph, mut classEnv: FCore::Graph, mut componentEnv: FCore::Graph, mut attr: Arc<DAE::Attributes>, mut tp: Arc<DAE::Type>, mut cref: Arc<DAE::ComponentRef>) -> Result<()> {
    let () = (::match_deref::match_deref! { match &(attr) {
        ...
        _ => {
            ...
            Debug::traceln(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("- Lookup.checkPackageVariableConstant failed: ")); __mm_s.push_str(&*s1); __mm_s.push_str(&*literal!(" in ")); __mm_s.push_str(&*s2); ArcStr::from(__mm_s) }).clone())?;
            bail!("fail")
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn frameIsImplAddedScope(mut f: FCore::Node) -> bool {
    let mut b: bool;
    b = (match f {
...
pub(crate) fn lookupVarInPackages(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inComponentRef: Arc<DAE::ComponentRef>, mut inPrevFrames: Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>, mut inState: Mutable::Mutable<bool>) -> Result<(FCore::Cache, FCore::Graph, Arc<DAE::Attributes>, Arc<DAE::Type>, Arc<DAE::Binding>, Option<DAE::Const>, InstTypes::SplicedExpData, FCore::Graph, ArcStr)> {
    ...
    (outCache, outClassEnv, outAttributes, outType, outBinding, constOfForIteratorRange, splicedExpData, outComponentEnv, name) = 'mc: {
        ...
        if let Ok((__v, __wb0, __wb1)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ DAE::ComponentRef::CREF_QUAL { ident: id, subscriptLst: Deref @ metamodelica::List::Nil, componentRef: cref, .. }, prevFrames) => {
                    ...
                    let mut of: Option<metamodelica::Array<FCore::Node>>;
                    let mut cnstForRange: Option<DAE::Const>;
        ...
        if let Ok((__v, __wb0, __wb1)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, prevFrames) => {
                    ...
                    let mut cnstForRange: Option<DAE::Const>;
                    let mut unique: bool;
                    let mut qimports: Arc<metamodelica::List<Absyn::Import>>;
                    let mut uqimports: Arc<metamodelica::List<Absyn::Import>>;
                    let mut cache = (*cache).clone();
                    let mut env = (*env).clone();
                    let mut prevFrames = (*prevFrames).clone();
                    let mut name: ArcStr = name.clone();
...
pub(crate) fn lookupVarLocal(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inComponentRef: Arc<DAE::ComponentRef>) -> Result<(FCore::Cache, Arc<DAE::Attributes>, Arc<DAE::Type>, Arc<DAE::Binding>, Option<DAE::Const>, InstTypes::SplicedExpData, FCore::Graph, FCore::Graph, ArcStr)> {
    ...
    let mut outComponentEnv: FCore::Graph;
    let mut name: ArcStr;
    (outCache, outAttributes, outType, outBinding, constOfForIteratorRange, splicedExpData, outClassEnv, outComponentEnv, name) = lookupVarInternal(inCache, inEnv, inComponentRef, openmodelica_frontend_inst::InstTypes::SearchStrategy::SEARCH_LOCAL_ONLY)?;
    Ok((outCache, outAttributes, outType, outBinding, constOfForIteratorRange, splicedExpData, outClassEnv, outComponentEnv, name))
}
...
pub(crate) fn lookupIdentLocal(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIdent: ArcStr) -> Result<(FCore::Cache, Arc<DAE::Var>, Arc<SCode::Element>, Arc<DAE::Mod>, FCore::Status, FCore::Graph)> {
    ...
    (outCache, outVar, outElement, outMod, instStatus, outComponentEnv) = 'mc: {
        let __mc_input = (inCache, inEnv.clone(), inIdent);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ Absyn::Path::IDENT { name: r#str }, info) => {
                    let mut res: Arc<metamodelica::List<Arc<DAE::Type>>>;
                    let mut httypes: Arc<FCore::RefTree::Tree>;
                    let mut ht: Arc<FCore::RefTree::Tree>;
                    let mut cache = (*cache).clone();
                    let mut env = (*env).clone();
...
fn lookupFunctionsInEnv2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPath: Arc<Absyn::Path>, mut followedQual: bool, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<metamodelica::List<Arc<DAE::Type>>>)> {
    ...
    let mut outTypesTypeLst: Arc<metamodelica::List<Arc<DAE::Type>>>;
    (outCache, outTypesTypeLst) = 'mc: {
        let __mc_input = (inCache, inEnv.clone(), inPath, followedQual);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, FCore::Graph::G { scope: Deref @ metamodelica::List::Cons { head: r, tail: _ }, .. }, Deref @ Absyn::Path::QUALIFIED { name: pack, path }, _) => {
                    let mut res: Arc<metamodelica::List<Arc<DAE::Type>>>;
                    let mut env_1: FCore::Graph;
                    let mut env2: FCore::Graph;
                    let mut r#str: ArcStr;
...
fn createGenericBuiltinFunctions(mut inEnv: FCore::Graph, mut inString: ArcStr) -> Result<Arc<metamodelica::List<Arc<DAE::Type>>>> {
    let mut outTypesTypeLst: Arc<metamodelica::List<Arc<DAE::Type>>>;
    outTypesTypeLst = (::match_deref::match_deref! { match &(inString) {
...
fn getHtTypes(mut inParentRef: metamodelica::Array<FCore::Node>) -> Arc<FCore::RefTree::Tree> {
...
fn lookupTypeInFrame(mut inCache: FCore::Cache, mut inBinTree1: Arc<FCore::RefTree::Tree>, mut inBinTree2: Arc<FCore::RefTree::Tree>, mut inEnv3: FCore::Graph, mut inIdent4: ArcStr) -> Result<(FCore::Cache, Arc<DAE::Type>, FCore::Graph)> {
    ...
    (outCache, outType, outEnv) = (::match_deref::match_deref! { match &((inCache, inBinTree2, inEnv3, inIdent4)) {
        (cache, httypes, env, id) => {
            let mut t: Arc<DAE::Type>;
            let mut item: FCore::Node;
            let mut cache = (*cache).clone();
            let mut env = (*env).clone();
            item = FNode::fromRef(FCore::RefTree::get(httypes.clone(), (id.clone()).clone())?)?;
            (cache, t, env) = lookupTypeInFrame2(cache.clone(), item, env.clone(), (id.clone()).clone())?;
...
fn lookupTypeInFrame2(mut inCache: FCore::Cache, mut item: FCore::Node, mut inEnv3: FCore::Graph, mut inIdent4: ArcStr) -> Result<(FCore::Cache, Arc<DAE::Type>, FCore::Graph)> {
    ...
    let mut outType: Arc<DAE::Type>;
    let mut outEnv: FCore::Graph;
    (outCache, outType, outEnv) = (::match_deref::match_deref! { match &((inCache, item, inEnv3, inIdent4)) {
        ...
        (_, FCore::Node { data: FCore::Data::CO { e: comp, .. }, .. }, _, id) => {
            ...
            Error::addSourceMessage(Error::LOOKUP_TYPE_FOUND_COMP.clone(), list![(id.clone()).clone()], info)?;
            bail!("fail")
        },
        ...
        (cache, FCore::Node { data: FCore::Data::CL { e: cdef @ Deref @ SCode::Element::CLASS { restriction: SCode::Restriction::R_FUNCTION { functionRestriction: _ }, .. }, .. }, .. }, env, id) => {
            ...
            let mut cache = (*cache).clone();
            cenv = env.clone();
            (cache, env_1, _) = InstFunction::implicitFunctionInstantiation(cache.clone(), cenv, InnerOuter::emptyInstHierarchy().clone(), openmodelica_frontend_types::DAE::Mod::interned_NOMOD(), openmodelica_frontend_types::DAE::Prefix::NOPRE, cdef.clone(), metamodelica::nil())?;
            (cache, ty, env_3) = lookupTypeInEnv(cache.clone(), env_1, (id.clone()).clone())?;
            (cache.clone(), ty, env_3)
        },
        _ => bail!("match: no arm matched"),
    } });
...
fn lookupFunctionsInFrame(mut inCache: FCore::Cache, mut inClasses: Arc<FCore::RefTree::Tree>, mut inFuncTypes: Arc<FCore::RefTree::Tree>, mut inEnv: FCore::Graph, mut inFuncName: ArcStr, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<metamodelica::List<Arc<DAE::Type>>>)> {
    let mut outCache: FCore::Cache;
    let mut outFuncTypes: Arc<metamodelica::List<Arc<DAE::Type>>>;
    let mut r: metamodelica::Array<FCore::Node>;
    let mut data: FCore::Data;
    let mut ty: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
    match '__try0: {
...
fn buildRecordType(mut cache: FCore::Cache, mut env: FCore::Graph, mut icdef: Arc<SCode::Element>) -> Result<(FCore::Cache, FCore::Graph, Arc<DAE::Type>)> {
    ...
    (outCache, outEnv, _) = InstFunction::implicitFunctionTypeInstantiation(outCache, env, InnerOuter::emptyInstHierarchy().clone(), cdef)?;
    (outCache, ftype, _) = lookupTypeInEnv(outCache, outEnv.clone(), (name).clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, cl @ Deref @ SCode::Element::CLASS { name: id, info, .. }) => {
                    let mut funcelts: Arc<metamodelica::List<Arc<SCode::Element>>>;
...
fn buildRecordConstructorClass2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut cl: Arc<SCode::Element>, mut mods: Arc<DAE::Mod>) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<Arc<SCode::Element>>>, Arc<metamodelica::List<Arc<SCode::Element>>>)> {
    ...
    (outCache, outEnv, funcelts, elts) = 'mc: {
        let __mc_input = (inCache, inEnv, cl.clone());
        if let Ok((__v, __wb0, __wb1)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ SCode::Element::CLASS { name, info, .. }) => {
                    ...
                    let mut eltsMods: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>;
        ...
        })() { elts = __wb0; funcelts = __wb1; break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                _ => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ metamodelica::List::Cons { head: (Deref @ SCode::Element::COMPONENT { name: id, prefixes: Deref @ SCode::Prefixes { visibility: _, redeclarePrefix: redecl, finalPrefix: f @ SCode::Final::FINAL { .. }, innerOuter: io, replaceablePrefix: repl }, attributes: SCode::Attributes { arrayDims: d, connectorType: ct, parallelism: prl, variability: var, direction: _, isField: isf }, typeSpec: tp, modifications: r#mod, comment, condition: cond, info }, cmod), tail: rest }) => {
                    ...
                    let mut compMod: Arc<DAE::Mod>;
                    let mut fullMod: Arc<DAE::Mod>;
                    let mut selectedMod: Arc<DAE::Mod>;
                    let mut cache = (*cache).clone();
                    let mut env = (*env).clone();
...
fn lookupClassInFrame(mut inCache: FCore::Cache, mut inFrame: FCore::Node, mut inEnv: FCore::Graph, mut inIdent: ArcStr, mut inPrevFrames: Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>, mut inState: Mutable::Mutable<bool>, mut inInfo: Option<SourceInfo>) -> Result<(FCore::Cache, Arc<SCode::Element>, FCore::Graph, Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>)> {
    ...
    let mut outPrevFrames: Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>;
    (outCache, outClass, outEnv, outPrevFrames) = 'mc: {
        let __mc_input = (inCache, inFrame.clone(), inEnv, inIdent, inPrevFrames);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
fn reportSeveralNamesError(mut unique: bool, mut name: ArcStr) -> Result<()> {
    let () = (match unique {
        true => (),
        false => {
            Error::addMessage(Error::IMPORT_SEVERAL_NAMES.clone(), list![(name).clone()])?;
            ()
        },
    });
    Ok(())
...
fn lookupVar2(mut inBinTree: Arc<FCore::RefTree::Tree>, mut inIdent: ArcStr, mut inGraph: FCore::Graph) -> Result<(Arc<DAE::Var>, Arc<SCode::Element>, Arc<DAE::Mod>, FCore::Status, FCore::Graph)> {
    ...
    let mut outEnv: FCore::Graph;
    let mut r: metamodelica::Array<FCore::Node>;
    let mut s: Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>;
    let mut n: FCore::Node;
    let mut name: ArcStr;
    r = FCore::RefTree::get(inBinTree, (inIdent.clone()).clone())?;
    ...
    if !(FNode::isComponent(n.clone())) && Flags::isSet(Flags::LOOKUP.clone())? {
        ...
        name = __pa0.clone();
        name = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*inIdent); __mm_s.push_str(&*literal!(" = ")); __mm_s.push_str(&*FGraph::printGraphPathStr(inGraph.clone())); __mm_s.push_str(&*literal!(".")); __mm_s.push_str(&*name); ArcStr::from(__mm_s) }).clone();
        Debug::traceln(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("- Lookup.lookupVar2 failed because we found a class instead of a variable: ")); __mm_s.push_str(&*name); ArcStr::from(__mm_s) }).clone())?;
        bail!("fail");
    }
    let FCore::N { data: FCore::CO { e: __pa2, r#mod: __pa3, kind: _, status: __pa4 }, .. } = (n) else { bail!("pattern mismatch") };
...
fn checkSubscripts(mut inType: Arc<DAE::Type>, mut inExpSubscriptLst: Arc<metamodelica::List<Arc<DAE::Subscript>>>) -> Result<Arc<DAE::Type>> {
    ...
    outType = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: dim, tail: Deref @ metamodelica::List::Nil }, ty: t }, Deref @ metamodelica::List::Cons { head: Deref @ DAE::Subscript::WHOLEDIM { .. }, tail: ys }) => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: _, tail: Deref @ metamodelica::List::Nil }, ty: t }, Deref @ metamodelica::List::Cons { head: Deref @ DAE::Subscript::SLICE { exp: e @ Deref @ DAE::Exp::RANGE { .. } }, tail: ys }) => {
                    let mut t_1: Arc<DAE::Type>;
                    let mut dim_int: i32;
                    t_1 = checkSubscripts(t.clone(), ys.clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: dim, tail: Deref @ metamodelica::List::Nil }, ty: t }, Deref @ metamodelica::List::Cons { head: Deref @ DAE::Subscript::SLICE { exp: Deref @ DAE::Exp::ARRAY { array: se, .. } }, tail: ys }) => {
                    let mut t_1: Arc<DAE::Type>;
                    let mut dim_int: i32;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: _, tail: Deref @ metamodelica::List::Nil }, ty: t }, Deref @ metamodelica::List::Cons { head: Deref @ DAE::Subscript::SLICE { exp: e }, tail: ys }) => {
                    let mut t_1: Arc<DAE::Type>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: dim, tail: Deref @ metamodelica::List::Nil }, ty: t }, Deref @ metamodelica::List::Cons { head: Deref @ DAE::Subscript::INDEX { exp: Deref @ DAE::Exp::ICONST { integer: ind } }, tail: ys }) => {
                    let mut t_1: Arc<DAE::Type>;
                    let mut sz: i32;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: dim, tail: Deref @ metamodelica::List::Nil }, ty: t }, Deref @ metamodelica::List::Cons { head: Deref @ DAE::Subscript::INDEX { .. }, tail: ys }) => {
                    let mut t_1: Arc<DAE::Type>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: Deref @ DAE::Dimension::DIM_UNKNOWN { .. }, tail: Deref @ metamodelica::List::Nil }, ty: t }, Deref @ metamodelica::List::Cons { head: Deref @ DAE::Subscript::INDEX { .. }, tail: ys }) => {
                    let mut t_1: Arc<DAE::Type>;
                    t_1 = checkSubscripts(t.clone(), ys.clone())?;
                    Ok(t_1.clone())
                }
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: Deref @ DAE::Dimension::DIM_EXP { .. }, tail: Deref @ metamodelica::List::Nil }, ty: t }, Deref @ metamodelica::List::Cons { head: Deref @ DAE::Subscript::INDEX { .. }, tail: ys }) => {
                    let mut t_1: Arc<DAE::Type>;
                    t_1 = checkSubscripts(t.clone(), ys.clone())?;
                    Ok(t_1.clone())
                }
...
fn lookupVarF(mut inCache: FCore::Cache, mut inBinTree: Arc<FCore::RefTree::Tree>, mut inComponentRef: Arc<DAE::ComponentRef>, mut inEnv: FCore::Graph) -> Result<(FCore::Cache, Arc<DAE::Attributes>, Arc<DAE::Type>, Arc<DAE::Binding>, Option<DAE::Const>, InstTypes::SplicedExpData, FCore::Graph, ArcStr)> {
    ...
    (outCache, outAttributes, outType, outBinding, constOfForIteratorRange, splicedExpData, outComponentEnv, name) = (::match_deref::match_deref! { match &((inCache.clone(), inBinTree.clone(), inComponentRef.clone())) {
        (_, _, Deref @ DAE::ComponentRef::CREF_IDENT { ident: id, subscriptLst: ss, .. }) => {
            (outCache, outAttributes, outType, outBinding, constOfForIteratorRange, splicedExpData, outComponentEnv, name) = lookupVarFIdent(inCache, inBinTree, (id.clone()).clone(), ss.clone(), inEnv)?;
            (outCache, outAttributes, outType, outBinding, constOfForIteratorRange, splicedExpData, outComponentEnv, name)
        },
        (cache, ht, Deref @ DAE::ComponentRef::CREF_QUAL { ident: id, subscriptLst: ss, componentRef: ids, .. }) => {
            ...
            let mut parentBinding: Arc<DAE::Binding>;
            let mut componentEnv: FCore::Graph;
            ...
            let mut cache = (*cache).clone();
            let mut ss = (*ss).clone();
            let (__pa0, __pa1, __pa2, __pa3) = ::match_deref::match_deref! { match &(lookupVar2(ht.clone(), (id.clone()).clone(), inEnv)?) {
        ...
        _ => {
            let (__pa0, __pa1, __pa2, __pa3, __pa4, __pa5, __pa6, __pa7, __pa8, __pa9, __pa10, __pa11, __pa12, __pa13) = ::match_deref::match_deref! { match &(lookupVar(cache.clone(), componentEnv, ids.clone())?) {
        ...
        Deref @ metamodelica::List::Cons { head: __esc_tCref, tail: _ } => {
            ...
            xCref = ComponentReferenceBasics::makeCrefQual((id.clone()).clone(), ty2_2, ss.clone(), tCref.clone());
        ...
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
            ...
            binding = lookupBinding(inComponentRef, tyParent.clone(), ty.clone(), parentBinding, binding);
            splicedExpData = InstTypes::SplicedExpData { splicedExp: oSplicedExp, identType: idTp };
            (Arc::new(DAE::Attributes { connectorType: ct, parallelism: prl, variability: vt, direction: di, innerOuter: io, visibility: vis }), ty, binding, cnstForRange, componentEnv, name)
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
            (cache.clone(), attr, ty, binding, cnstForRange, splicedExpData, componentEnv, name)
        },
        _ => bail!("match: no arm matched"),
    } });
...
fn lookupVarFIdent(mut cache: FCore::Cache, mut ht: Arc<FCore::RefTree::Tree>, mut ident: ArcStr, mut ss: Arc<metamodelica::List<Arc<DAE::Subscript>>>, mut inEnv: FCore::Graph) -> Result<(FCore::Cache, Arc<DAE::Attributes>, Arc<DAE::Type>, Arc<DAE::Binding>, Option<DAE::Const>, InstTypes::SplicedExpData, FCore::Graph, ArcStr)> {
    ...
    let (__pa0, __pa1, __pa2, __pa3, __pa4, __pa5) = ::match_deref::match_deref! { match &(lookupVar2(ht, (ident.clone()).clone(), inEnv)?) {
    ...
    ss_1 = addArrayDimensions(tty.clone(), ss);
    splicedExpData = InstTypes::SplicedExpData { splicedExp: Some(Expression::makeCrefExp(ComponentReferenceBasics::makeCrefIdent((ident).clone(), tty.clone(), ss_1), tty)?), identType: ty };
...
fn lookupVarFMetaModelica(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut cr: Arc<DAE::ComponentRef>, mut inType: Arc<DAE::Type>) -> Result<(FCore::Cache, Arc<DAE::Attributes>, Arc<DAE::Type>, Arc<DAE::Binding>, Option<DAE::Const>, ArcStr)> {
    ...
    (attr, ty, binding, cnstForRange, name) = (::match_deref::match_deref! { match &(cr.clone()) {
        Deref @ DAE::ComponentRef::CREF_IDENT { .. } => {
            ...
            let (__pa0, __pa1, __pa2, __pa3, __pa4) = ::match_deref::match_deref! { match &((fields.clone()).get(Types::findVarIndex((var_field!((*cr).ident, DAE::ComponentRef::CREF_IDENT).clone()).clone(), fields)? + 1)?) {
        ...
        Deref @ DAE::ComponentRef::CREF_QUAL { .. } => {
            let mut fields: Arc<metamodelica::List<Arc<DAE::Var>>>;
            fields = Types::getMetaRecordFields(inType)?;
            let (__pa0, __pa1, __pa2, __pa3, __pa4) = ::match_deref::match_deref! { match &((fields.clone()).get(Types::findVarIndex((var_field!((*cr).ident, DAE::ComponentRef::CREF_QUAL).clone()).clone(), fields)? + 1)?) {
                Deref @ DAE::Var { name: __pa0, attributes: __pa1, ty: __pa2, binding: __pa3, bind_from_outside: _, constOfForIteratorRange: __pa4 } => (__pa0.clone(), __pa1.clone(), __pa2.clone(), __pa3.clone(), __pa4.clone()),
                _ => bail!("pattern mismatch"),
            } };
            name = __pa0.clone();
...
fn lookupBinding(mut inCref: Arc<DAE::ComponentRef>, mut inParentType: Arc<DAE::Type>, mut inChildType: Arc<DAE::Type>, mut inParentBinding: Arc<DAE::Binding>, mut inChildBinding: Arc<DAE::Binding>) -> Arc<DAE::Binding> {
    let mut outBinding: Arc<DAE::Binding>;
    outBinding = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::ComponentRef::CREF_QUAL { ident: _, identType: _, subscriptLst: ss, componentRef: Deref @ DAE::ComponentRef::CREF_IDENT { ident: cId, identType: _, subscriptLst: Deref @ metamodelica::List::Nil } }, Deref @ DAE::Binding::EQBOUND { exp: e, evaluatedExp: _, constant_: c, source: s }) => {
                    let mut tyElement: Arc<DAE::Type>;
                    let mut b: Arc<DAE::Binding>;
...
fn elabComponentRecursive(mut oCref: Option<Arc<DAE::Exp>>) -> Arc<metamodelica::List<Arc<DAE::ComponentRef>>> {
    let mut lref: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
...
fn addArrayDimensions(mut tySub: Arc<DAE::Type>, mut ss: Arc<metamodelica::List<Arc<DAE::Subscript>>>) -> Arc<metamodelica::List<Arc<DAE::Subscript>>> {
    let mut outType: Arc<metamodelica::List<Arc<DAE::Subscript>>>;
    outType = 'mc: {
...
fn makeDimensionSubscript(mut inDim: Arc<DAE::Dimension>) -> Result<Arc<DAE::Subscript>> {
    ...
    outSub = (::match_deref::match_deref! { match &(inDim.clone()) {
        ...
        Deref @ DAE::Dimension::DIM_BOOLEAN { .. } => {
            ...
            Arc::new(DAE::Subscript::SLICE { exp: Arc::new(DAE::Exp::ARRAY { ty: DAE::T_BOOL_DEFAULT().clone(), scalar: true, array: expl }) })
        },
        Deref @ DAE::Dimension::DIM_ENUM { enumTypeName: enum_name, literals: l, .. } => {
            let mut expl: Arc<metamodelica::List<Arc<DAE::Exp>>>;
            expl = makeEnumLiteralIndices(enum_name.clone(), l.clone(), 1)?;
            Arc::new(DAE::Subscript::SLICE { exp: Arc::new(DAE::Exp::ARRAY { ty: Arc::new(DAE::Type::T_ENUMERATION { index: None, path: enum_name.clone(), names: l.clone(), literalVarLst: metamodelica::nil(), attributeLst: metamodelica::nil() }), scalar: true, array: expl }) })
        },
        _ => bail!("match: no arm matched"),
    } });
    Ok(outSub)
...
fn makeEnumLiteralIndices(mut enumTypeName: Arc<Absyn::Path>, mut enumLiterals: Arc<metamodelica::List<ArcStr>>, mut enumIndex: i32) -> Result<Arc<metamodelica::List<Arc<DAE::Exp>>>> {
    ...
    enumIndices = (::match_deref::match_deref! { match &(enumLiterals) {
        ...
        Deref @ metamodelica::List::Cons { head: l, tail: ls } => {
            ...
            expl = makeEnumLiteralIndices(enumTypeName, ls.clone(), enumIndex + 1)?;
...
fn sliceDimensionType(mut inTypeD: Arc<DAE::Type>, mut inTypeL: Arc<DAE::Type>) -> Result<Arc<DAE::Type>> {
    ...
    outType = (::match_deref::match_deref! { match &((inTypeD, inTypeL)) {
        (t, tOrg) => {
            ...
            dim2 = List::map(dimensions, (std::sync::Arc::new(fnptr!(Expression::intDimension, i32)) as std::sync::Arc<dyn ::std::ops::Fn(i32) -> Result<Arc<DAE::Dimension>> + 'static>))?;
            dim2 = dim2.reverse();
            t = List::foldr(dim2, (std::sync::Arc::new(fnptr!(Types::liftArray, Arc<DAE::Type>, Arc<DAE::Dimension>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Type>, Arc<DAE::Dimension>) -> Result<Arc<DAE::Type>> + 'static>), tOrg.clone())?;
            t.clone()
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
pub(crate) fn buildMetaRecordType(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut cdef: Arc<SCode::Element>) -> Result<(FCore::Cache, FCore::Graph, Arc<DAE::Type>)> {
    ...
    let mut cache: FCore::Cache;
    let mut typeVarsType: Arc<metamodelica::List<Arc<DAE::Type>>>;
    ...
    utPath = __pa1.clone();
    index = __pa2.clone();
    singleton = __pa3.clone();
    typeVars = __pa4.clone();
    els = __pa5.clone();
    env = FGraph::openScope(inEnv, openmodelica_frontend_types::SCode::Encapsulated::NOT_ENCAPSULATED, (id.clone()).clone(), Some(openmodelica_frontend_dump::FCore::ScopeType::CLASS_SCOPE))?;
    (cache, utPath) = Inst::makeFullyQualified(inCache, env.clone(), utPath)?;
    path = AbsynUtil::joinPaths(utPath.clone(), Arc::new(Absyn::Path::IDENT { name: (id).clone() }))?;
    (outCache, outEnv, _, _, _, _, _, varlst, _, _) = Inst::instElementList(cache, env, InnerOuter::emptyInstHierarchy().clone(), UnitAbsyn::noStore().clone(), openmodelica_frontend_types::DAE::Mod::interned_NOMOD(), openmodelica_frontend_types::DAE::Prefix::NOPRE, ClassInf::State::META_RECORD { path: Arc::new(Absyn::Path::IDENT { name: (literal!("")).clone() }) }, List::map1(els, std::sync::Arc::new(fnptr!(Util::makeTuple, _, _)), openmodelica_frontend_types::DAE::Mod::interned_NOMOD())?, metamodelica::nil(), false, openmodelica_frontend_inst::InstTypes::CallingScope::INNER_CALL, ConnectionGraph::EMPTY().clone(), Connect::emptySet().clone(), true)?;
    ...
    ftype = Arc::new(DAE::Type::T_METARECORD { path: path, utPath: utPath, typeVars: typeVarsType, index: index, fields: varlst, knownSingleton: singleton });
    Ok((outCache, outEnv, ftype))
}
