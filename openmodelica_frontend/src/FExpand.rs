...
pub(crate) fn all(mut inGraph: Graph) -> Result<Graph> {
    ...
    outGraph = (match inGraph {
        mut g => {
            ...
            metamodelica::print(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("Extends:        ")); __mm_s.push_str(&*realString(listHead(lst.clone())?)); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
            System::startTimer();
            g = FResolve::derived(FGraph::top(g.clone())?, g.clone())?;
            System::stopTimer();
            lst = List::consr(lst, System::getTimerIntervalTime());
            metamodelica::print(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("Derived:        ")); __mm_s.push_str(&*realString(listHead(lst.clone())?)); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
            System::startTimer();
            g = FResolve::cc(FGraph::top(g.clone())?, g.clone())?;
            System::stopTimer();
            lst = List::consr(lst, System::getTimerIntervalTime());
            metamodelica::print(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("ConstrainedBy:  ")); __mm_s.push_str(&*realString(listHead(lst.clone())?)); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
            System::startTimer();
            g = FResolve::clsext(FGraph::top(g.clone())?, g.clone())?;
            System::stopTimer();
            lst = List::consr(lst, System::getTimerIntervalTime());
            metamodelica::print(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("ClassExtends:   ")); __mm_s.push_str(&*realString(listHead(lst.clone())?)); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
            System::startTimer();
            g = FResolve::ty(FGraph::top(g.clone())?, g.clone())?;
            System::stopTimer();
            lst = List::consr(lst, System::getTimerIntervalTime());
            metamodelica::print(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("ComponentTypes: ")); __mm_s.push_str(&*realString(listHead(lst.clone())?)); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
            System::startTimer();
            g = FResolve::cr(FGraph::top(g.clone())?, g.clone())?;
            System::stopTimer();
            lst = List::consr(lst, System::getTimerIntervalTime());
            metamodelica::print(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("Comp Refs:      ")); __mm_s.push_str(&*realString(listHead(lst.clone())?)); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
            System::startTimer();
            g = FResolve::r#mod(FGraph::top(g.clone())?, g.clone())?;
            System::stopTimer();
            lst = List::consr(lst, System::getTimerIntervalTime());
            metamodelica::print(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("Modifiers:      ")); __mm_s.push_str(&*realString(listHead(lst.clone())?)); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
            metamodelica::print(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("FExpand.all:    ")); __mm_s.push_str(&*realString(List::fold(lst, (std::sync::Arc::new(fnptr!(realAdd, metamodelica::Real, metamodelica::Real)) as std::sync::Arc<dyn ::std::ops::Fn(metamodelica::Real, metamodelica::Real) -> Result<metamodelica::Real> + 'static>), metamodelica::OrderedFloat(0.0_f64))?)); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
            g.clone()
        },
    });
    Ok(outGraph)
