...
pub static BUILTIN_TIME: std::sync::LazyLock<Option<(Arc<DAE::Exp>, DAE::Properties, Arc<DAE::Attributes>)>> = std::sync::LazyLock::new(|| { Some((Arc::new(DAE::Exp::CREF { componentRef: Arc::new(DAE::ComponentRef::CREF_IDENT { ident: (literal!("time")).clone(), identType: DAE::T_REAL_DEFAULT.clone(), subscriptLst: metamodelica::nil() }), ty: DAE::T_REAL_DEFAULT.clone() }), DAE::Properties::PROP { type_: DAE::T_REAL_DEFAULT.clone(), constFlag: openmodelica_frontend_types::DAE::Const::C_VAR }, DAE::dummyAttrInput.clone())) });
...
pub fn elabExp(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    e = if (BackendInterface::noRewriteRulesFrontEnd()) {inExp.clone()} else {BackendInterface::rewriteFrontEnd(inExp.clone())};
    ...
    match '__try0: {
        ...
        Deref @ Absyn::Exp::CREF { .. } => elabExp_Cref,
        Deref @ Absyn::Exp::BINARY { .. } => elabExp_Binary,
        Deref @ Absyn::Exp::UNARY { .. } => elabExp_Unary,
        Deref @ Absyn::Exp::LBINARY { .. } => elabExp_Binary,
        Deref @ Absyn::Exp::LUNARY { .. } => elabExp_LUnary,
        Deref @ Absyn::Exp::RELATION { .. } => elabExp_Binary,
        Deref @ Absyn::Exp::IFEXP { .. } => elabExp_If,
        Deref @ Absyn::Exp::CALL { .. } => elabExp_Call,
        Deref @ Absyn::Exp::PARTEVALFUNCTION { .. } => elabExp_PartEvalFunction,
        Deref @ Absyn::Exp::TUPLE { .. } => elabExp_Tuple,
        Deref @ Absyn::Exp::RANGE { .. } => elabExp_Range,
        Deref @ Absyn::Exp::ARRAY { .. } => elabExp_Array,
        Deref @ Absyn::Exp::MATRIX { .. } => elabExp_Matrix,
        Deref @ Absyn::Exp::CODE { .. } => elabExp_Code,
        Deref @ Absyn::Exp::CONS { .. } => elabExp_Cons,
        Deref @ Absyn::Exp::LIST { .. } => elabExp_List,
        Deref @ Absyn::Exp::MATCHEXP { .. } => Patternm::elabMatchExpression,
        Deref @ Absyn::Exp::DOT { .. } => elabExp_Dot,
        Deref @ Absyn::Exp::EXPRESSIONCOMMENT { .. } => elabExp_Comment,
        _ => elabExp_BuiltinType,
...
pub type PartialElabExpFunc = fn(FCore::Cache, FCore::Graph, Arc<Absyn::Exp>, bool, bool, DAE::Prefix, SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)>;
...
fn elabExp_Dot(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outExp, outProperties) = (::match_deref::match_deref! { match &(inExp.clone()) {
        ...
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
            ...
            let _ = (::match_deref::match_deref! { match &(ty.clone()) {
...
fn elabExp_PartEvalFunction(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    if pos_args.clone().is_empty() && named_args.clone().is_empty() {
        ...
        c = List::fold(consts.clone(), Arc::new(fnptr!(Types::constAnd, DAE::Const, DAE::Const)), openmodelica_frontend_types::DAE::Const::C_CONST);
...
fn elabExp_Matrix(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut dess2: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Exp>>>>> = metamodelica::nil();
    let mut props: Arc<metamodelica::List<Arc<metamodelica::List<DAE::Properties>>>> = metamodelica::nil();
    let mut tps: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Type>>>>> = metamodelica::nil();
    ...
    if have_real.clone() {
        (dess, props) = List::threadMapList_2(dess.clone(), props.clone(), Arc::new(elabExp_Matrix_realCast))?;
    }
...
fn elabExp_List(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    if es.clone().is_empty() {
        ...
        c = List::fold(consts.clone(), Arc::new(fnptr!(Types::constAnd, DAE::Const, DAE::Const)), openmodelica_frontend_types::DAE::Const::C_CONST);
        ty = Types::boxIfUnboxedType(List::reduce(types.clone(), Arc::new(Types::superType))?)?;
...
fn elabExpInExpression2(mut inExp: Arc<DAE::Exp>, mut inProperties: DAE::Properties) -> (Arc<DAE::Exp>, DAE::Properties) {
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    (outExp, outProperties) = (::match_deref::match_deref! { match &((inExp.clone(), inProperties.clone())) {
        (_, DAE::Properties::PROP_TUPLE { tupleConst: Deref @ DAE::TupleConst::TUPLE_CONST { tupleConstLst: Deref @ metamodelica::List::Cons { head: Deref @ DAE::TupleConst::SINGLE_CONST { r#const: c }, tail: _ } }, type_: Deref @ DAE::Type::T_TUPLE { types: Deref @ metamodelica::List::Cons { head: ty, tail: _ }, .. } }) => {
...
fn checkAssignmentToInput2(mut inExp: Arc<Absyn::Exp>, mut inAttributes: Arc<DAE::Attributes>, mut inInfo: SourceInfo) -> Result<()> {
    let _ = (::match_deref::match_deref! { match &((inExp.clone(), inAttributes.clone(), inInfo.clone())) {
        (Deref @ Absyn::Exp::CREF { componentRef: cr }, Deref @ DAE::Attributes { direction: Absyn::Direction::INPUT, .. }, _) => {
...
pub fn checkAssignmentToInputs(mut inExpCrefs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inAttributes: Arc<metamodelica::List<Arc<DAE::Attributes>>>, mut inEnv: FCore::Graph, mut inInfo: SourceInfo) -> Result<()> {
    if FGraph::inFunctionScope(inEnv.clone())? {
        List::threadMap1_0(inExpCrefs.clone(), inAttributes.clone(), Arc::new(checkAssignmentToInput2), inInfo.clone())?;
    }
    Ok(())
}
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Exp::ARRAY { array: expl, ty: a @ Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: _, tail: Deref @ metamodelica::List::Cons { head: _, tail: Deref @ metamodelica::List::Nil } }, .. }, .. } => {
                    ...
                    let mut d1: i32 = 0;
                    mexpl = List::map(expl.clone(), Arc::new(Expression::arrayContent));
...
pub fn deduceIterationRange(mut inIterator: ArcStr, mut inCrefs: Arc<metamodelica::List<(Arc<Absyn::ComponentRef>, i32)>>, mut inEnv: FCore::Graph, mut inCache: FCore::Cache, mut inInfo: SourceInfo) -> Result<(Arc<DAE::Exp>, DAE::Properties, FCore::Cache)> {
    ...
    for mut cr in &*inCrefs.clone() {
        ...
        if '__try0: {
            (outCache, _, ty, _, _, _, _, _, _) = unwrap_break_err!(Lookup::lookupVar(outCache.clone(), inEnv.clone(), cref.clone()), '__try0);
            Ok::<(), anyhow::Error>(())
        }.is_err() {
    ...
    let (__pa1, __pa2) = ::match_deref::match_deref! { match &(ranges.clone()) {
        Deref @ metamodelica::List::Cons { head: __pa1, tail: __pa2 } => (__pa1.clone(), __pa2.clone()),
        _ => bail!("pattern mismatch"),
    } };
    outRange = __pa1.clone();
    ranges = __pa2.clone();
    idx = 2;
...
fn reductionType(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inFn: Arc<Absyn::Path>, mut inExp: Arc<DAE::Exp>, mut inType: Arc<DAE::Type>, mut unboxedType: Arc<DAE::Type>, mut dims: Arc<metamodelica::List<Arc<DAE::Dimension>>>, mut hasGuardExp: bool, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, Arc<DAE::Type>, Arc<DAE::Type>, Option<Arc<Values::Value>>, Arc<Absyn::Path>)> {
    ...
    (outExp, outType, resultType, defaultValue, outPath) = (::match_deref::match_deref! { match &((r#fn.clone(), unboxedType.clone())) {
        (Deref @ Absyn::Path::IDENT { name: Deref @ "array" }, _) => {
            let mut ty: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
            ty = List::foldr(dims.clone(), Arc::new(fnptr!(Types::liftArray, Arc<DAE::Type>, Arc<DAE::Dimension>)), inType.clone());
        ...
        (Deref @ Absyn::Path::IDENT { name: Deref @ "list" }, _) => {
            ...
            ty = List::foldr(dims.clone(), Arc::new(fnptr!(Types::liftList, Arc<DAE::Type>, Arc<DAE::Dimension>)), ty.clone());
...
fn checkReductionType1(mut inEnv: FCore::Graph, mut inPath: Arc<Absyn::Path>, mut fnTypes: Arc<metamodelica::List<Arc<DAE::Type>>>, mut info: SourceInfo) -> Result<(Arc<DAE::Type>, Arc<DAE::Type>, Arc<DAE::Type>, Option<Arc<Values::Value>>, Arc<Absyn::Path>)> {
    ...
    (typeA, typeB, resType, startValue, outPath) = (::match_deref::match_deref! { match &(fnTypes.clone()) {
        ...
        _ => {
            let mut str1: ArcStr = arcstr::literal!("");
            str1 = stringDelimitList(List::map(fnTypes.clone(), Arc::new(TypesDump::unparseType)), (literal!(",")).clone());
...
pub fn elabGraphicsExp(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, _, Deref @ Absyn::Exp::REAL { value: s }, _, _, _) => {
                    let mut r: metamodelica::Real = metamodelica::OrderedFloat(0.0_f64);
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ Absyn::Exp::CREF { componentRef: cr }, r#impl, pre, _) => {
                    let mut dexp: Arc<DAE::Exp>;
                    let mut prop: DAE::Properties;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, exp @ Deref @ Absyn::Exp::BINARY { exp2: e2, op, exp1: e1 }, r#impl, pre, _) => {
                    let mut dexp: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, e @ Deref @ Absyn::Exp::UNARY { op: Absyn::Operator::UPLUS, .. }, r#impl, pre, _) => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, exp @ Deref @ Absyn::Exp::UNARY { exp: e, op }, r#impl, pre, _) => {
                    let mut dexp: Arc<DAE::Exp>;
                    let mut e_1: Arc<DAE::Exp>;
                    let mut prop: DAE::Properties;
                    let mut prop1: DAE::Properties;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, exp @ Deref @ Absyn::Exp::LBINARY { exp2: e2, op, exp1: e1 }, r#impl, pre, _) => {
                    let mut dexp: Arc<DAE::Exp>;
                    let mut e1_1: Arc<DAE::Exp>;
                    let mut e2_1: Arc<DAE::Exp>;
                    let mut prop: DAE::Properties;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, exp @ Deref @ Absyn::Exp::LUNARY { exp: e, op }, r#impl, pre, _) => {
                    let mut dexp: Arc<DAE::Exp>;
                    let mut e_1: Arc<DAE::Exp>;
                    let mut prop: DAE::Properties;
                    let mut prop1: DAE::Properties;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, exp @ Deref @ Absyn::Exp::RELATION { exp2: e2, op, exp1: e1 }, r#impl, pre, _) => {
                    let mut dexp: Arc<DAE::Exp>;
                    let mut e1_1: Arc<DAE::Exp>;
                    let mut e2_1: Arc<DAE::Exp>;
                    let mut prop: DAE::Properties;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, e @ Deref @ Absyn::Exp::IFEXP { .. }, r#impl, pre, _) => {
                    let mut e1_1: Arc<DAE::Exp>;
                    let mut e2_1: Arc<DAE::Exp>;
                    let mut e_1: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ Absyn::Exp::TUPLE { expressions: es @ Deref @ metamodelica::List::Cons { head: _, tail: _ } }, r#impl, pre, _) => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ Absyn::Exp::RANGE { stop, step: None, start }, r#impl, pre, _) => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ Absyn::Exp::RANGE { stop, step: Some(step), start }, r#impl, pre, _) => {
                    let mut start_1: Arc<DAE::Exp>;
                    let mut stop_1: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ Absyn::Exp::ARRAY { arrayExp: es }, r#impl, pre, _) => {
                    let mut l: i32 = 0;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ Absyn::Exp::MATRIX { matrix: ess }, r#impl, pre, _) => {
                    let mut nmax: i32 = 0;
                    let mut dim1: Arc<DAE::Dimension> = Arc::new(DAE::Dimension::DIM_BOOLEAN);
                    let mut dim2: Arc<DAE::Dimension> = Arc::new(DAE::Dimension::DIM_BOOLEAN);
                    ...
                    tps_1 = List::mapList(tps.clone(), Arc::new(Types::getPropType));
                    ...
                    dim1 = __pa4.clone();
                    dim2 = __pa5.clone();
                    let _ = elabMatrixToMatrixExp(mexp.clone())?;
    ...
    (outStart, outStep, outStop, outRangeType) = (::match_deref::match_deref! { match &((inStartType.clone(), inStepType.clone(), inStopType.clone())) {
        ...
        (Deref @ DAE::Type::T_ENUMERATION { names: ns, .. }, None, Deref @ DAE::Type::T_ENUMERATION { names: ne, .. }) => {
            ...
            if List::isEqual(ns.clone(), ne.clone(), true) {
                ...
                let _ = TypesDump::unparseTypeNoAttr(inStopType.clone())?;
...
fn elabArrayHasMixedIntReals(mut inTypes: Arc<metamodelica::List<Arc<DAE::Type>>>) -> Result<(Arc<DAE::Type>, bool)> {
    ...
    let mut has_int: bool = false;
    let mut has_real: bool = false;
    let mut ty: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
...
fn elabArray2(mut inExpl: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut inTypes: Arc<metamodelica::List<Arc<DAE::Type>>>, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(Arc<metamodelica::List<Arc<DAE::Exp>>>, Arc<DAE::Type>)> {
    ...
    for mut exp2 in &*rest_expl.clone() {
        ...
        if !(Types::equivtypes(outType.clone(), ty2.clone())?) {
            match '__try6: {
                (exp2, outType) = unwrap_break_err!(Types::matchType(exp2.clone(), outType.clone(), ty2.clone(), false), '__try6);
                Ok::<_, anyhow::Error>((exp2.clone(), outType.clone()))
            } {
                ...
                Err(_) => {
                    ...
                    expl_str = (List::toString(inExpl.clone(), Arc::new(ExpressionBasics::printExpStr), (literal!("")).clone(), (literal!("[")).clone(), (literal!(",")).clone(), (literal!("]")).clone(), true, 0)?).clone();
...
fn elabMatrixCatOne(mut inExpl: Arc<metamodelica::List<Arc<DAE::Exp>>>) -> Result<Arc<DAE::Exp>> {
    ...
    let mut ty: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
    match '__try0: {
        outExp = unwrap_break_err!(List::reduce(inExpl.clone(), Arc::new(elabMatrixCatOne2)), '__try0);
        Ok::<_, anyhow::Error>((outExp.clone(),))
    } {
        Ok((__try0_o0,)) => {
            outExp = __try0_o0;
...
fn elabMatrixSemi(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inMatrix: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Exp>>>>>, mut inProperties: Arc<metamodelica::List<Arc<metamodelica::List<DAE::Properties>>>>, mut inImpl: bool, mut inHaveReal: bool, mut inDims: i32, mut inDoVectorization: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties, Arc<DAE::Dimension>, Arc<DAE::Dimension>)> {
    ...
    while !(rest_expl.clone().is_empty()) {
        ...
        if !(Expression::dimensionsEqual(dim2.clone(), outDim2.clone())?) {
            ...
            el_str = (List::toString(expl.clone(), Arc::new(ExpressionBasics::printExpStr), (literal!("")).clone(), (literal!("{")).clone(), (literal!(", ")).clone(), (literal!("}")).clone(), true, 0)?).clone();
        ...
        match '__try8: {
            outProperties = unwrap_break_err!(Types::matchWithPromote(outProperties.clone(), prop.clone(), inHaveReal.clone()), '__try8);
            Ok::<_, anyhow::Error>((outProperties.clone(),))
        } {
            Ok((__try8_o0,)) => {
                outProperties = __try8_o0;
            }
            Err(_) => {
...
fn verifyBuiltInHandlerType(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExpl: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inImplicit: bool, mut inTypeChecker: Arc<dyn ::std::ops::Fn(Arc<DAE::Type>) -> Result<bool> + 'static>, mut inFnName: ArcStr, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    pub type extraFunc = fn(Arc<DAE::Type>) -> Result<bool>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: _, tail: _ } => {
                    ...
                    let mut prop: DAE::Properties;
                    let mut ty: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
                    dim_expl = List::map(inDimensions.clone(), Arc::new(Expression::dimensionSizeExp));
...
fn elabBuiltinNDims(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inAbsynExpLst: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArg: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = 'mc: {
        let __mc_input = (inCache.clone(), inEnv.clone(), inAbsynExpLst.clone(), inNamedArg.clone(), inBoolean.clone(), inPrefix.clone(), info.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ metamodelica::List::Cons { head: arraycr, tail: Deref @ metamodelica::List::Nil }, _, r#impl, pre, _) => {
                    let mut exp: Arc<DAE::Exp>;
                    let mut arrtp: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
                    let mut nd: i32 = 0;
                    let mut cache = (*cache).clone();
...
fn elabBuiltinFill(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inAbsynExpLst: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArg: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ metamodelica::List::Cons { head: s, tail: dims }, _, r#impl, pre, _) => {
                    ...
                    (dims_1, _) = Types::matchTypes(dims_1.clone(), List::map(dimprops.clone(), Arc::new(Types::getPropType)), DAE::T_INTEGER_DEFAULT.clone(), false)?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, dims, _, r#impl, pre, _) => {
                    ...
                    expstrs = List::map(dims.clone(), Arc::new(Dump::printExpStr));
...
pub fn elabBuiltinFill2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<DAE::Exp>, mut inType: Arc<DAE::Type>, mut inValuesValueLst: Arc<metamodelica::List<Arc<Values::Value>>>, mut constVar: DAE::Const, mut inPrefix: DAE::Prefix, mut inDims: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = 'mc: {
        let __mc_input = (inCache.clone(), inEnv.clone(), inExp.clone(), inType.clone(), inValuesValueLst.clone(), constVar.clone(), inPrefix.clone(), inDims.clone(), inInfo.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, _, s, sty, Deref @ metamodelica::List::Cons { head: Deref @ Values::Value::INTEGER { integer: v }, tail: Deref @ metamodelica::List::Nil }, c1, _, _, _) => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, s, sty, Deref @ metamodelica::List::Cons { head: Deref @ Values::Value::INTEGER { integer: v }, tail: rest }, c1, pre, _, _) => {
                    let mut arraylist: Arc<metamodelica::List<Arc<DAE::Exp>>> = metamodelica::nil();
                    let mut at: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
                    let mut exp: Arc<DAE::Exp>;
                    let mut ty: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
...
fn elabBuiltinSymmetric(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inAbsynExpLst: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArg: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache.clone(), inEnv.clone(), inAbsynExpLst.clone(), inNamedArg.clone(), inBoolean.clone(), inPrefix.clone(), info.clone())) {
        (cache, env, Deref @ metamodelica::List::Cons { head: matexp, tail: Deref @ metamodelica::List::Nil }, _, r#impl, pre, _) => {
...
fn elabBuiltinPreMatrix(mut inExp: Arc<DAE::Exp>, mut inType: Arc<DAE::Type>) -> Result<Arc<DAE::Exp>> {
    ...
    outExp = (::match_deref::match_deref! { match &(inExp.clone()) {
        Deref @ DAE::Exp::CALL { expLst: Deref @ metamodelica::List::Cons { head: exp @ Deref @ DAE::Exp::MATRIX { .. }, tail: Deref @ metamodelica::List::Nil }, .. } => {
            ...
            todo!("unhandled field-assign shape: exp.matrix");
            exp.clone()
        },
        _ => {
            inExp.clone()
...
fn sameDimensions(mut inProps: Arc<metamodelica::List<DAE::Properties>>) -> Result<bool> {
    ...
    types = List::map(inProps.clone(), Arc::new(Types::getPropType));
    dims = List::map(types.clone(), Arc::new(fnptr!(TypesDump::getDimensions, Arc<DAE::Type>)));
...
fn elabBuiltinClock(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = 'mc: {
        let __mc_input = (inCache.clone(), inEnv.clone(), args.clone(), nargs.clone(), inBoolean.clone(), inPrefix.clone(), info.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, _, Deref @ metamodelica::List::Nil, Deref @ metamodelica::List::Nil, _, _, _) => {
                    let mut call: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ metamodelica::List::Cons { head: aintervalCounter, tail: Deref @ metamodelica::List::Nil }, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
                    let mut call: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ metamodelica::List::Cons { head: aintervalCounter, tail: Deref @ metamodelica::List::Cons { head: aresolution, tail: Deref @ metamodelica::List::Nil } }, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
                    let mut call: Arc<DAE::Exp>;
                    let mut intervalCounter: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ metamodelica::List::Cons { head: ainterval, tail: Deref @ metamodelica::List::Nil }, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
                    let mut call: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ metamodelica::List::Cons { head: acondition, tail: Deref @ metamodelica::List::Nil }, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
                    let mut call: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ metamodelica::List::Cons { head: acondition, tail: Deref @ metamodelica::List::Cons { head: astartInterval, tail: Deref @ metamodelica::List::Nil } }, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
                    let mut call: Arc<DAE::Exp>;
                    let mut condition: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ metamodelica::List::Cons { head: ac, tail: Deref @ metamodelica::List::Cons { head: asolverMethod, tail: Deref @ metamodelica::List::Nil } }, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
                    let mut call: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ metamodelica::List::Cons { head: ac, tail: Deref @ metamodelica::List::Nil }, Deref @ metamodelica::List::Cons { head: Deref @ Absyn::NamedArg { argValue: asolverMethod, argName: Deref @ "solverMethod" }, tail: Deref @ metamodelica::List::Nil }, r#impl, pre, _) => {
                    let mut call: Arc<DAE::Exp>;
                    let mut c: Arc<DAE::Exp>;
                    let mut solverMethod: Arc<DAE::Exp>;
                    let mut ty1: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
...
fn elabBuiltinHold(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache.clone(), inEnv.clone(), args.clone(), nargs.clone(), inBoolean.clone(), inPrefix.clone(), info.clone())) {
        (cache, env, Deref @ metamodelica::List::Cons { head: au, tail: Deref @ metamodelica::List::Nil }, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
            let mut call: Arc<DAE::Exp>;
            let mut ty1: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
            let mut ty: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
            let mut prop1: DAE::Properties;
            ...
            let (__pa0, Some((__pa1, __pa2))) = (elabCallArgs3(cache.clone(), env.clone(), list![ty.clone()], Arc::new(Absyn::Path::IDENT { name: (literal!("hold")).clone() }), args.clone(), nargs.clone(), metamodelica::nil(), r#impl.clone(), pre.clone(), info.clone())?) else { bail!("pattern mismatch") };
            cache = __pa0.clone();
            call = __pa1.clone();
            prop = __pa2.clone();
...
fn elabBuiltinSample(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ metamodelica::List::Cons { head: astart, tail: Deref @ metamodelica::List::Cons { head: ainterval, tail: Deref @ metamodelica::List::Nil } }, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
                    let mut call: Arc<DAE::Exp>;
                    ...
                    let (__pa0, Some((__pa1, __pa2))) = (elabCallArgs3(cache.clone(), env.clone(), list![ty.clone()], Arc::new(Absyn::Path::IDENT { name: (literal!("sample")).clone() }), args.clone(), nargs.clone(), metamodelica::nil(), r#impl.clone(), pre.clone(), info.clone())?) else { bail!("pattern mismatch") };
                    cache = __pa0.clone();
                    call = __pa1.clone();
                    prop = __pa2.clone();
                    Ok((cache.clone(), call.clone(), prop.clone()))
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ metamodelica::List::Cons { head: au, tail: Deref @ metamodelica::List::Cons { head: ac, tail: Deref @ metamodelica::List::Nil } }, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
                    let mut call: Arc<DAE::Exp>;
                    let mut c: Arc<DAE::Exp>;
                    let mut ty1: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
                    let mut ty2: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
...
fn elabBuiltinShiftSample(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache.clone(), inEnv.clone(), args.clone(), nargs.clone(), inBoolean.clone(), inPrefix.clone(), info.clone())) {
        (cache, env, Deref @ metamodelica::List::Cons { head: au, tail: Deref @ metamodelica::List::Cons { head: ashiftCounter, tail: Deref @ metamodelica::List::Nil } }, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
            let mut call: Arc<DAE::Exp>;
            ...
            let (__pa0, Some((__pa1, __pa2))) = (elabCallArgs3(cache.clone(), env.clone(), list![ty.clone()], Arc::new(Absyn::Path::IDENT { name: (literal!("shiftSample")).clone() }), list![au.clone(), ashiftCounter.clone(), aresolution.clone()], nargs.clone(), metamodelica::nil(), r#impl.clone(), pre.clone(), info.clone())?) else { bail!("pattern mismatch") };
            cache = __pa0.clone();
            call = __pa1.clone();
            prop = __pa2.clone();
            (cache.clone(), call.clone(), prop.clone())
        },
        (cache, env, Deref @ metamodelica::List::Cons { head: au, tail: Deref @ metamodelica::List::Cons { head: ashiftCounter, tail: Deref @ metamodelica::List::Cons { head: aresolution, tail: Deref @ metamodelica::List::Nil } } }, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
            let mut call: Arc<DAE::Exp>;
            let mut shiftCounter: Arc<DAE::Exp>;
            let mut resolution: Arc<DAE::Exp>;
            let mut ty1: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
...
fn elabBuiltinBackSample(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache.clone(), inEnv.clone(), args.clone(), nargs.clone(), inBoolean.clone(), inPrefix.clone(), info.clone())) {
        (cache, env, Deref @ metamodelica::List::Cons { head: au, tail: Deref @ metamodelica::List::Cons { head: abackCounter, tail: Deref @ metamodelica::List::Nil } }, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
            let mut call: Arc<DAE::Exp>;
            ...
            let (__pa0, Some((__pa1, __pa2))) = (elabCallArgs3(cache.clone(), env.clone(), list![ty.clone()], Arc::new(Absyn::Path::IDENT { name: (literal!("backSample")).clone() }), list![au.clone(), abackCounter.clone(), aresolution.clone()], nargs.clone(), metamodelica::nil(), r#impl.clone(), pre.clone(), info.clone())?) else { bail!("pattern mismatch") };
            cache = __pa0.clone();
            call = __pa1.clone();
            prop = __pa2.clone();
            (cache.clone(), call.clone(), prop.clone())
        },
        (cache, env, Deref @ metamodelica::List::Cons { head: au, tail: Deref @ metamodelica::List::Cons { head: abackCounter, tail: Deref @ metamodelica::List::Cons { head: aresolution, tail: Deref @ metamodelica::List::Nil } } }, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
            let mut call: Arc<DAE::Exp>;
            let mut backCounter: Arc<DAE::Exp>;
            let mut resolution: Arc<DAE::Exp>;
            let mut ty1: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
...
fn elabBuiltinNoClock(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache.clone(), inEnv.clone(), args.clone(), nargs.clone(), inBoolean.clone(), inPrefix.clone(), info.clone())) {
        (cache, env, Deref @ metamodelica::List::Cons { head: au, tail: Deref @ metamodelica::List::Nil }, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
            ...
            let (__pa0, Some((__pa1, __pa2))) = (elabCallArgs3(cache.clone(), env.clone(), list![ty.clone()], Arc::new(Absyn::Path::IDENT { name: (literal!("noClock")).clone() }), args.clone(), nargs.clone(), metamodelica::nil(), r#impl.clone(), pre.clone(), info.clone())?) else { bail!("pattern mismatch") };
            cache = __pa0.clone();
            call = __pa1.clone();
            prop = __pa2.clone();
            (cache.clone(), call.clone(), prop.clone())
...
fn elabBuiltinFirstTick(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache.clone(), inEnv.clone(), args.clone(), nargs.clone(), inBoolean.clone(), inPrefix.clone(), info.clone())) {
        (cache, env, Deref @ metamodelica::List::Nil, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
            let mut call: Arc<DAE::Exp>;
            let mut ty: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
            let mut prop: DAE::Properties;
            let mut cache = (*cache).clone();
            ...
            let (__pa0, Some((__pa1, __pa2))) = (elabCallArgs3(cache.clone(), env.clone(), list![ty.clone()], Arc::new(Absyn::Path::IDENT { name: (literal!("firstTick")).clone() }), args.clone(), nargs.clone(), metamodelica::nil(), r#impl.clone(), pre.clone(), info.clone())?) else { bail!("pattern mismatch") };
            cache = __pa0.clone();
            call = __pa1.clone();
            prop = __pa2.clone();
            (cache.clone(), call.clone(), prop.clone())
...
fn elabBuiltinInterval(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache.clone(), inEnv.clone(), args.clone(), nargs.clone(), inBoolean.clone(), inPrefix.clone(), info.clone())) {
        (cache, env, Deref @ metamodelica::List::Nil, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
            ...
            let (__pa0, Some((__pa1, __pa2))) = (elabCallArgs3(cache.clone(), env.clone(), list![ty.clone()], Arc::new(Absyn::Path::IDENT { name: (literal!("interval")).clone() }), args.clone(), nargs.clone(), metamodelica::nil(), r#impl.clone(), pre.clone(), info.clone())?) else { bail!("pattern mismatch") };
            cache = __pa0.clone();
            call = __pa1.clone();
            prop = __pa2.clone();
            (cache.clone(), call.clone(), prop.clone())
...
fn elabBuiltinTransition(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache.clone(), inEnv.clone(), args.clone(), nargs.clone(), inBoolean.clone(), inPrefix.clone(), info.clone())) {
        (cache, env, _, _, r#impl, pre, _) => {
            ...
            let mut cache = (*cache).clone();
            slist = List::map(nargs.clone(), Arc::new(Dump::printNamedArgStr));
            s1 = (Dump::printExpLstStr(args.clone())).clone();
            s2 = stringDelimitList(cons(s1.clone(), slist.clone()), (literal!(", ")).clone());
            ...
            let (__pa0, Some((__pa1, __pa2))) = (elabCallArgs3(cache.clone(), env.clone(), list![ty.clone()], Arc::new(Absyn::Path::IDENT { name: (literal!("transition")).clone() }), args.clone(), nargs.clone(), metamodelica::nil(), r#impl.clone(), pre.clone(), info.clone())?) else { bail!("pattern mismatch") };
            cache = __pa0.clone();
            call = __pa1.clone();
            prop = __pa2.clone();
            (cache.clone(), call.clone(), prop.clone())
...
fn elabBuiltinTransition2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo, mut argName: ArcStr, mut n: i32, mut strMsg0: ArcStr, mut strPre: ArcStr) -> Result<Arc<DAE::Type>> {
    ...
    b1 = List::isMemberOnTrue((argName.clone()).clone(), nargs.clone(), Arc::new(fnptr!(elabBuiltinTransition3, ArcStr, Arc<Absyn::NamedArg>)));
...
fn elabBuiltinTransition5(mut argName: ArcStr, mut getAsNamedArg: bool, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>) -> Result<Arc<Absyn::Exp>> {
    ...
    argValue = (::match_deref::match_deref! { match &((argName.clone(), getAsNamedArg.clone())) {
        (Deref @ "from", true) => {
            ...
            namedArg = List::getMemberOnTrue((literal!("from")).clone(), nargs.clone(), Arc::new(fnptr!(elabBuiltinTransition3, ArcStr, Arc<Absyn::NamedArg>)))?;
        ...
        (Deref @ "to", true) => {
            let mut namedArg: Arc<Absyn::NamedArg>;
            namedArg = List::getMemberOnTrue((literal!("to")).clone(), nargs.clone(), Arc::new(fnptr!(elabBuiltinTransition3, ArcStr, Arc<Absyn::NamedArg>)))?;
            elabBuiltinTransition4(namedArg.clone())?
        },
        (Deref @ "to", false) => {
            (args.clone()).get(2)?
...
fn elabBuiltinInitialState(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache.clone(), inEnv.clone(), args.clone(), nargs.clone(), inBoolean.clone(), inPrefix.clone(), info.clone())) {
        (cache, env, Deref @ metamodelica::List::Cons { head: astate, tail: Deref @ metamodelica::List::Nil }, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
            let mut call: Arc<DAE::Exp>;
            let mut ty1: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
            let mut ty: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
            let mut prop1: DAE::Properties;
            ...
            let (__pa0, Some((__pa1, __pa2))) = (elabCallArgs3(cache.clone(), env.clone(), list![ty.clone()], Arc::new(Absyn::Path::IDENT { name: (literal!("initialState")).clone() }), args.clone(), nargs.clone(), metamodelica::nil(), r#impl.clone(), pre.clone(), info.clone())?) else { bail!("pattern mismatch") };
            cache = __pa0.clone();
            call = __pa1.clone();
            prop = __pa2.clone();
            (cache.clone(), call.clone(), prop.clone())
...
fn elabBuiltinActiveState(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache.clone(), inEnv.clone(), args.clone(), nargs.clone(), inBoolean.clone(), inPrefix.clone(), info.clone())) {
        (cache, env, Deref @ metamodelica::List::Cons { head: astate, tail: Deref @ metamodelica::List::Nil }, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
            ...
            let (__pa0, Some((__pa1, __pa2))) = (elabCallArgs3(cache.clone(), env.clone(), list![ty.clone()], Arc::new(Absyn::Path::IDENT { name: (literal!("activeState")).clone() }), args.clone(), nargs.clone(), metamodelica::nil(), r#impl.clone(), pre.clone(), info.clone())?) else { bail!("pattern mismatch") };
            cache = __pa0.clone();
            call = __pa1.clone();
            prop = __pa2.clone();
            (cache.clone(), call.clone(), prop.clone())
...
fn elabBuiltinTicksInState(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache.clone(), inEnv.clone(), args.clone(), nargs.clone(), inBoolean.clone(), inPrefix.clone(), info.clone())) {
        (cache, env, Deref @ metamodelica::List::Nil, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
            ...
            let (__pa0, Some((__pa1, __pa2))) = (elabCallArgs3(cache.clone(), env.clone(), list![ty.clone()], Arc::new(Absyn::Path::IDENT { name: (literal!("ticksInState")).clone() }), args.clone(), nargs.clone(), metamodelica::nil(), r#impl.clone(), pre.clone(), info.clone())?) else { bail!("pattern mismatch") };
            cache = __pa0.clone();
            call = __pa1.clone();
            prop = __pa2.clone();
            (cache.clone(), call.clone(), prop.clone())
...
fn elabBuiltinTimeInState(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache.clone(), inEnv.clone(), args.clone(), nargs.clone(), inBoolean.clone(), inPrefix.clone(), info.clone())) {
        (cache, env, Deref @ metamodelica::List::Nil, Deref @ metamodelica::List::Nil, r#impl, pre, _) => {
            ...
            let (__pa0, Some((__pa1, __pa2))) = (elabCallArgs3(cache.clone(), env.clone(), list![ty.clone()], Arc::new(Absyn::Path::IDENT { name: (literal!("timeInState")).clone() }), args.clone(), nargs.clone(), metamodelica::nil(), r#impl.clone(), pre.clone(), info.clone())?) else { bail!("pattern mismatch") };
            cache = __pa0.clone();
            call = __pa1.clone();
            prop = __pa2.clone();
            (cache.clone(), call.clone(), prop.clone())
...
fn elabBuiltinBoolean(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = verifyBuiltInHandlerType(inCache.clone(), inEnv.clone(), inPosArgs.clone(), inImplicit.clone(), Arc::new(Types::isIntegerOrRealOrBooleanOrSubTypeOfEither), (literal!("boolean")).clone(), inPrefix.clone(), inInfo.clone())?;
    Ok((outCache, outExp, outProperties))
}
...
fn elabBuiltinIntegerEnum(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArg: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = verifyBuiltInHandlerType(inCache.clone(), inEnv.clone(), inPosArgs.clone(), inImplicit.clone(), Arc::new(fnptr!(Types::isEnumeration, Arc<DAE::Type>)), (literal!("Integer")).clone(), inPrefix.clone(), inInfo.clone())?;
    Ok((outCache, outExp, outProperties))
}
...
fn elabBuiltinCat(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    ty = __pa4.clone();
    tys = __pa5.clone();
    result_ty = List::fold1(tys.clone(), Arc::new(Types::arraySuperType), inInfo.clone(), ty.clone());
    if '__try6: {
        (arr_expl, arr_tys) = unwrap_break_err!(Types::matchTypes(arr_expl.clone(), arr_tys.clone(), result_ty.clone(), false), '__try6);
        Ok::<(), anyhow::Error>(())
    }.is_err() {
...
fn elabBuiltinUniqueRootIndices(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inAbsynExpLst: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArg: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache.clone(), inEnv.clone(), inAbsynExpLst.clone(), inNamedArg.clone(), inBoolean.clone(), inPrefix.clone(), info.clone())) {
        (cache, env, Deref @ metamodelica::List::Cons { head: aexp1, tail: Deref @ metamodelica::List::Cons { head: aexp2, tail: Deref @ metamodelica::List::Nil } }, Deref @ metamodelica::List::Nil, _, pre, _) => {
            let mut exp1: Arc<DAE::Exp>;
            let mut exp2: Arc<DAE::Exp>;
        ...
        (cache, env, Deref @ metamodelica::List::Cons { head: aexp1, tail: Deref @ metamodelica::List::Cons { head: aexp2, tail: Deref @ metamodelica::List::Cons { head: _, tail: Deref @ metamodelica::List::Nil } } }, Deref @ metamodelica::List::Nil, _, pre, _) => {
            let mut exp1: Arc<DAE::Exp>;
            let mut exp2: Arc<DAE::Exp>;
            let mut exp3: Arc<DAE::Exp>;
            let mut lst: Arc<metamodelica::List<Arc<DAE::Exp>>> = metamodelica::nil();
        ...
        (cache, env, Deref @ metamodelica::List::Cons { head: aexp1, tail: Deref @ metamodelica::List::Cons { head: aexp2, tail: Deref @ metamodelica::List::Nil } }, Deref @ metamodelica::List::Cons { head: Deref @ Absyn::NamedArg { argName: Deref @ "message", argValue: _ }, tail: Deref @ metamodelica::List::Nil }, _, pre, _) => {
            let mut exp1: Arc<DAE::Exp>;
            let mut exp2: Arc<DAE::Exp>;
            let mut exp3: Arc<DAE::Exp>;
            let mut lst: Arc<metamodelica::List<Arc<DAE::Exp>>> = metamodelica::nil();
...
pub static STRING_ARG_MINLENGTH: std::sync::LazyLock<Slot> = std::sync::LazyLock::new(|| { Slot { defaultArg: Arc::new(DAE::FuncArg { name: (literal!("minimumLength")).clone(), ty: DAE::T_INTEGER_DEFAULT.clone(), r#const: openmodelica_frontend_types::DAE::Const::C_VAR, par: openmodelica_frontend_types::DAE::VarParallelism::NON_PARALLEL, defaultBinding: None }), slotFilled: false, arg: Some(Arc::new(DAE::Exp::ICONST { integer: 0 })), dims: metamodelica::nil(), idx: 2, evalStatus: SLOT_NOT_EVALUATED.clone() } });

pub static STRING_ARG_LEFTJUSTIFIED: std::sync::LazyLock<Slot> = std::sync::LazyLock::new(|| { Slot { defaultArg: Arc::new(DAE::FuncArg { name: (literal!("leftJustified")).clone(), ty: DAE::T_BOOL_DEFAULT.clone(), r#const: openmodelica_frontend_types::DAE::Const::C_VAR, par: openmodelica_frontend_types::DAE::VarParallelism::NON_PARALLEL, defaultBinding: None }), slotFilled: false, arg: Some(Arc::new(DAE::Exp::BCONST { bool: true })), dims: metamodelica::nil(), idx: 3, evalStatus: SLOT_NOT_EVALUATED.clone() } });

pub static STRING_ARG_SIGNIFICANT_DIGITS: std::sync::LazyLock<Slot> = std::sync::LazyLock::new(|| { Slot { defaultArg: Arc::new(DAE::FuncArg { name: (literal!("significantDigits")).clone(), ty: DAE::T_INTEGER_DEFAULT.clone(), r#const: openmodelica_frontend_types::DAE::Const::C_VAR, par: openmodelica_frontend_types::DAE::VarParallelism::NON_PARALLEL, defaultBinding: None }), slotFilled: false, arg: Some(Arc::new(DAE::Exp::ICONST { integer: 6 })), dims: metamodelica::nil(), idx: 4, evalStatus: SLOT_NOT_EVALUATED.clone() } });
...
fn elabBuiltinMatrix2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inArg: Arc<DAE::Exp>, mut inProperties: DAE::Properties, mut inType: Arc<DAE::Type>, mut inInfo: SourceInfo) -> Result<(Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outExp, outProperties) = (::match_deref::match_deref! { match &(inArg.clone()) {
        ...
        Deref @ DAE::Exp::ARRAY { array: expl, scalar, ty: Deref @ DAE::Type::T_ARRAY { ty: ety, dims: Deref @ metamodelica::List::Cons { head: dim1, tail: Deref @ metamodelica::List::Cons { head: dim2, tail: _ } } } } => {
            ...
            let mut props: DAE::Properties;
            let mut expl = (*expl).clone();
            expl = List::map1(expl.clone(), Arc::new(elabBuiltinMatrix3), inInfo.clone());
...
pub fn elabBuiltinHandler(mut inIdent: ArcStr) -> Result<fn(FCore::Cache, FCore::Graph, Arc<metamodelica::List<Arc<Absyn::Exp>>>, Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, bool, DAE::Prefix, SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)>> {
    pub type HandlerFunc = fn(FCore::Cache, FCore::Graph, Arc<metamodelica::List<Arc<Absyn::Exp>>>, Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, bool, DAE::Prefix, SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)>;
    ...
    outHandler = (::match_deref::match_deref! { match &(inIdent.clone()) {
        Deref @ "smooth" => elabBuiltinSmooth,
        Deref @ "size" => elabBuiltinSize,
        Deref @ "ndims" => elabBuiltinNDims,
        Deref @ "zeros" => elabBuiltinZeros,
        Deref @ "ones" => elabBuiltinOnes,
        Deref @ "fill" => elabBuiltinFill,
        Deref @ "max" => elabBuiltinMax,
        Deref @ "min" => elabBuiltinMin,
        Deref @ "transpose" => elabBuiltinTranspose,
        Deref @ "symmetric" => elabBuiltinSymmetric,
        Deref @ "array" => elabBuiltinArray,
        Deref @ "sum" => elabBuiltinSum,
        Deref @ "product" => elabBuiltinProduct,
        Deref @ "pre" => elabBuiltinPre,
        Deref @ "firstTick" => elabBuiltinFirstTick,
        Deref @ "interval" => elabBuiltinInterval,
        Deref @ "boolean" => elabBuiltinBoolean,
        Deref @ "noEvent" => elabBuiltinNoevent,
        Deref @ "edge" => elabBuiltinEdge,
        Deref @ "der" => elabBuiltinDer,
        Deref @ "change" => elabBuiltinChange,
        Deref @ "cat" => elabBuiltinCat,
        Deref @ "identity" => elabBuiltinIdentity,
        Deref @ "vector" => elabBuiltinVector,
        Deref @ "matrix" => elabBuiltinMatrix,
        Deref @ "scalar" => elabBuiltinScalar,
        Deref @ "String" => elabBuiltinString,
        Deref @ "rooted" => elabBuiltinRooted,
        Deref @ "Integer" => elabBuiltinIntegerEnum,
        Deref @ "EnumToInteger" => elabBuiltinIntegerEnum,
        Deref @ "inStream" => elabBuiltinInStream,
        Deref @ "actualStream" => elabBuiltinActualStream,
        Deref @ "getInstanceName" => elabBuiltinGetInstanceName,
        Deref @ "classDirectory" => elabBuiltinClassDirectory,
        Deref @ "sample" => elabBuiltinSample,
        Deref @ "cardinality" => elabBuiltinCardinality,
        Deref @ "homotopy" => elabBuiltinHomotopy,
        Deref @ "DynamicSelect" => elabBuiltinDynamicSelect,
        Deref @ "Clock" => {
...
fn elabCall(mut cache: FCore::Cache, mut env: FCore::Graph, mut r#fn: Arc<Absyn::ComponentRef>, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut typeVars: Arc<metamodelica::List<Arc<Absyn::Path>>>, mut r#impl: bool, mut pre: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    if hasBuiltInHandler(r#fn.clone())? {
        if '__try0: {
            ...
            s1 = stringDelimitList(List::map(args.clone(), Arc::new(Dump::printExpStr)), (literal!(", ")).clone());
            s2 = stringDelimitList(List::map(nargs.clone(), Arc::new(Dump::printNamedArgStr)), (literal!(", ")).clone());
    ...
    (cache, e, prop) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ...
            argstrs = List::map(args.clone(), Arc::new(Dump::printExpStr));
...
pub fn getOptionalNamedArg(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inImplicit: bool, mut inArgName: ArcStr, mut inType: Arc<DAE::Type>, mut inArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inDefaultExp: Arc<DAE::Exp>, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>)> {
    ...
    let mut exp_str: ArcStr = arcstr::literal!("");
    let mut ty_str: ArcStr = arcstr::literal!("");
    let mut ety_str: ArcStr = arcstr::literal!("");
    let mut ty: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
    let mut e: Arc<Absyn::Exp> = Arc::new(Absyn::Exp::BREAK);
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (newf, oldf) => {
                    ...
                    let Some(__pa0) = (System::getFileModificationTime((newf.clone()).clone())) else { bail!("pattern mismatch") };
                    nfmt = __pa0.clone();
...
fn elabCallArgsEvaluateArrayLength(mut inCache: FCore::Cache, mut env: FCore::Graph, mut inProperties: DAE::Properties, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> (FCore::Cache, DAE::Properties) {
    ...
    match '__try0: {
        ...
        let (__pa1, (__pa2, _)) = unwrap_break_err!(Types::traverseType(ty.clone(), (inCache.clone(), env.clone()), Arc::new(elabCallArgsEvaluateArrayLength2)), '__try0);
        ty = __pa1.clone();
        outCache = __pa2.clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Type::T_ARRAY { .. }, tpl) => {
                    ...
                    (dims, tpl) = List::mapFold(var_field!((*oty).dims, DAE::Type::T_ARRAY).clone(), Arc::new(elabCallArgsEvaluateArrayLength3), tpl.clone());
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, r#fn, args, nargs, r#impl, pre) => {
                    ...
                    names = __pa5.clone();
                    fargs = List::map(names.clone(), Arc::new(fnptr!(createDummyFarg, ArcStr)));
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, r#fn, args, nargs, r#impl, pre) => {
                    ...
                    r#const = List::fold(constlist.clone(), Arc::new(fnptr!(Types::constAnd, DAE::Const, DAE::Const)), openmodelica_frontend_types::DAE::Const::C_CONST);
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, r#fn, _, _, _, pre) => {
                    ...
                    cache = __pa0.clone();
                    typelist = __pa1.clone();
                    t_lst = List::map(typelist.clone(), Arc::new(TypesDump::unparseType));
                    fn_str = (AbsynUtil::pathString(r#fn.clone(), (literal!(".")).clone(), true, false)?).clone();
                    pre_str = (PrefixUtil::printPrefixStr3(pre.clone())?).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, r#fn, _, _, _, _) => {
                    ...
                    if '__try0: {
                        (_, _, _) = unwrap_break_err!(Lookup::lookupType(cache.clone(), env.clone(), r#fn.clone(), None), '__try0);
                        Ok::<(), anyhow::Error>(())
                    }.is_ok() { bail!("failure(): body succeeded") }
...
fn isValidWRTParallelScope_dispatch(mut inFn: Arc<Absyn::Path>, mut isBuiltin: bool, mut inFuncParallelism: DAE::FunctionParallelism, mut inScope: Arc<metamodelica::List<metamodelica::Array<FCore::Node>>>, mut inInfo: SourceInfo) -> Result<bool> {
    let mut isValid: bool = false;
    isValid = 'mc: {
        let __mc_input = (inFn.clone(), isBuiltin.clone(), inFuncParallelism.clone(), inScope.clone(), inInfo.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, true, DAE::FunctionParallelism::FP_NON_PARALLEL, _, _) => {
                    Ok(true)
                }
                _ => bail!("nomatch"),
            }}
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, _, Deref @ metamodelica::List::Cons { head: r#ref, tail: restScope }, _) => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, DAE::FunctionParallelism::FP_NON_PARALLEL, Deref @ metamodelica::List::Cons { head: r#ref, tail: _ }, _) => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, DAE::FunctionParallelism::FP_PARALLEL_FUNCTION, Deref @ metamodelica::List::Cons { head: r#ref, tail: _ }, _) => {
                    let mut scopeName: ArcStr = arcstr::literal!("");
                    let false = (FNode::isRefTop(r#ref.clone())?) else { bail!("pattern mismatch") };
                    scopeName = (FNode::refName(r#ref.clone())?).clone();
                    let true = (FGraph::checkScopeType(list![r#ref.clone()], Some(crate::FCore::ScopeType::PARALLEL_SCOPE))?) else { bail!("pattern mismatch") };
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, DAE::FunctionParallelism::FP_KERNEL_FUNCTION, Deref @ metamodelica::List::Cons { head: r#ref, tail: _ }, _) => {
                    let mut scopeName: ArcStr = arcstr::literal!("");
                    let mut errorString: ArcStr = arcstr::literal!("");
                    let false = (FNode::isRefTop(r#ref.clone())?) else { bail!("pattern mismatch") };
                    scopeName = (FNode::refName(r#ref.clone())?).clone();
...
fn elabCallArgsMetarecord(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inType: Arc<DAE::Type>, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut stopElab: Mutable::Mutable<bool>, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Option<(Arc<DAE::Exp>, DAE::Properties)>)> {
    let mut outCache: FCore::Cache = FCore::Cache::NO_CACHE;
    let mut expProps: Option<(Arc<DAE::Exp>, DAE::Properties)> = None;
    (outCache, expProps) = { let ty = inType.clone(); 'mc: {
        let __mc_input = ty;
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Type::T_METARECORD { path: fq_path, .. } => {
                    let mut r#str: ArcStr = arcstr::literal!("");
                    let mut fn_str: ArcStr = arcstr::literal!("");
                    let __pa0 = ::match_deref::match_deref! { match &(List::find(var_field!((*inType).fields, DAE::Type::T_METARECORD).clone(), Arc::new(fnptr!(Types::varHasMetaRecordType, Arc<DAE::Var>)))?) {
        ...
        __acc.reverse()
                    ...
                    r#const = List::fold(const_lst.clone(), Arc::new(fnptr!(Types::constAnd, DAE::Const, DAE::Const)), openmodelica_frontend_types::DAE::Const::C_CONST);
                    ty_const = elabConsts(inType.clone(), r#const.clone())?;
                    let true = (List::fold(slots.clone(), Arc::new(slotAnd), true)) else { bail!("pattern mismatch") };
...
impl Default for ForceFunctionInst {
    fn default() -> Self { Self::FORCE_FUNCTION_INST }
}
...
fn addDefaultArgs(mut inSlots: Arc<metamodelica::List<Slot>>, mut inInfo: SourceInfo) -> (Arc<metamodelica::List<Arc<DAE::Exp>>>, Arc<metamodelica::List<Slot>>) {
    ...
    (outArgs, outSlots) = List::map2_2(inSlots.clone(), Arc::new(fillDefaultSlot), metamodelica::arrayFromVec(inSlots.clone().into_iter().cloned().collect()), inInfo.clone());
    (outArgs, outSlots)
}
...
fn evaluateSlotExp(mut inExp: Arc<DAE::Exp>, mut inSlotArray: metamodelica::Array<Slot>, mut inInfo: SourceInfo) -> Result<Arc<DAE::Exp>> {
    let mut outExp: Arc<DAE::Exp>;
    (outExp, _) = Expression::traverseExpBottomUp(inExp.clone(), Arc::new(evaluateSlotExp_traverser), (inSlotArray.clone(), inInfo.clone()))?;
    Ok(outExp)
}
...
fn lookupSlotInArray(mut inSlotName: ArcStr, mut inSlots: metamodelica::Array<Slot>) -> Option<Slot> {
    ...
    let mut slot: Slot;
    match '__try0: {
        (slot, _) = unwrap_break_err!(Array::getMemberOnTrue((inSlotName.clone()).clone(), inSlots.clone(), Arc::new(isSlotNamed)), '__try0);
...
fn elabTypes(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut typeVars: Arc<metamodelica::List<Arc<Absyn::Path>>>, mut inTypes: Arc<metamodelica::List<Arc<DAE::Type>>>, mut inOnlyOneFunction: bool, mut inCheckTypes: bool, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<metamodelica::List<Arc<DAE::Exp>>>, Arc<metamodelica::List<DAE::Const>>, Arc<DAE::Type>, Arc<DAE::Type>, Arc<metamodelica::List<Arc<DAE::Dimension>>>, Arc<metamodelica::List<Slot>>)> {
    ...
    let mut funcarg: Arc<DAE::FuncArg>;
    ...
    while !(success.clone()) {
        ...
        DAE::FunctionBuiltin::FUNCTION_BUILTIN { unboxArgs: true, .. } => (List::map(outArgs.clone(), Arc::new(fnptr!(Expression::unboxExp, Arc<DAE::Exp>))), {
...
fn applyArgTypesToFuncType(mut inSlots: Arc<metamodelica::List<Slot>>, mut inParameters: Arc<metamodelica::List<Arc<DAE::FuncArg>>>, mut inResultType: Arc<DAE::Type>, mut inEnv: FCore::Graph, mut checkTypes: bool, mut inInfo: SourceInfo) -> Result<(Arc<metamodelica::List<Arc<DAE::FuncArg>>>, Arc<DAE::Type>)> {
    ...
    dims = listAppend(List::mapFlat(tys.clone(), Arc::new(fnptr!(TypesDump::getDimensions, Arc<DAE::Type>))), dims.clone());
...
fn getAllOutputDimensions(mut inOutputType: Arc<DAE::Type>) -> Arc<metamodelica::List<Arc<DAE::Dimension>>> {
    ...
    outDimensions = (::match_deref::match_deref! { match &(inOutputType.clone()) {
        Deref @ DAE::Type::T_TUPLE { types: tys, .. } => {
            List::mapFlat(tys.clone(), Arc::new(fnptr!(TypesDump::getDimensions, Arc<DAE::Type>)))
        },
        _ => {
...
fn extractNamesFromDims(mut inDimensions: Arc<metamodelica::List<Arc<DAE::Dimension>>>, mut inAccumNames: Arc<metamodelica::List<ArcStr>>) -> Result<Arc<metamodelica::List<ArcStr>>> {
    ...
    outNames = (::match_deref::match_deref! { match &(inDimensions.clone()) {
        Deref @ metamodelica::List::Cons { head: Deref @ DAE::Dimension::DIM_EXP { exp }, tail: rest_dims } => {
            ...
            names = List::fold(crefs.clone(), Arc::new(fnptr!(extractNamesFromDims2, Arc<DAE::ComponentRef>, Arc<metamodelica::List<ArcStr>>)), inAccumNames.clone());
...
fn extractNamesFromDims2(mut inCref: Arc<DAE::ComponentRef>, mut inAccumNames: Arc<metamodelica::List<ArcStr>>) -> Arc<metamodelica::List<ArcStr>> {
    let mut outNames: Arc<metamodelica::List<ArcStr>> = metamodelica::nil();
    outNames = (::match_deref::match_deref! { match &((inCref.clone(), inAccumNames.clone())) {
        (Deref @ DAE::ComponentRef::CREF_IDENT { ident: name, .. }, _) => {
            outNames = if (List::isMemberOnTrue((name.clone()).clone(), inAccumNames.clone(), Arc::new(fnptr!(stringEq, ArcStr, ArcStr)))) {inAccumNames.clone()} else {cons(name.clone(), inAccumNames.clone())};
            outNames.clone()
        },
        _ => {
            inAccumNames.clone()
...
fn isSlotUsed(mut inSlot: Slot, mut inUsedNames: Arc<metamodelica::List<ArcStr>>) -> Result<bool> {
    ...
    outIsUsed = List::isMemberOnTrue((slot_name.clone()).clone(), inUsedNames.clone(), Arc::new(fnptr!(stringEq, ArcStr, ArcStr)));
    Ok(outIsUsed)
}
...
fn evaluateFuncParamDimAndMatchTypes(mut inSlot: Slot, mut inParam: Arc<DAE::FuncArg>, mut inEnv: FCore::Graph, mut inCache: FCore::Cache, mut inInfo: SourceInfo) -> Result<Arc<DAE::FuncArg>> {
    let mut outParam: Arc<DAE::FuncArg>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                ty @ Deref @ DAE::Type::T_TUPLE { .. } => {
                    let mut ty = (*ty).clone();
                    let __owned_variant_types_0 = List::map2(var_field!((**ty).types, DAE::Type::T_TUPLE).clone(), Arc::new(evaluateFuncArgTypeDims), inEnv.clone(), inCache.clone());
...
fn sameSlotsVectorizable(mut inSlots: Arc<metamodelica::List<Slot>>, mut inDims: Arc<metamodelica::List<Arc<DAE::Dimension>>>, mut name: ArcStr, mut exp: Arc<DAE::Exp>, mut info: SourceInfo) -> Result<()> {
    let _ = (::match_deref::match_deref! { match &(inSlots.clone()) {
...
fn makeEmptySlots(mut inArgs: Arc<metamodelica::List<Arc<DAE::FuncArg>>>) -> Arc<metamodelica::List<Slot>> {
    let mut outSlots: Arc<metamodelica::List<Slot>> = metamodelica::nil();
    (outSlots, _) = List::mapFold(inArgs.clone(), Arc::new(fnptr!(makeEmptySlot, Arc<DAE::FuncArg>, i32)), 1);
    outSlots
}
...
fn slotListArgs(mut inSlots: Arc<metamodelica::List<Slot>>) -> Arc<metamodelica::List<Arc<DAE::Exp>>> {
    let mut outArgs: Arc<metamodelica::List<Arc<DAE::Exp>>> = metamodelica::nil();
    outArgs = List::filterMap(inSlots.clone(), Arc::new(slotArg));
    outArgs
}

fn slotArg(mut inSlot: Slot) -> Result<Arc<DAE::Exp>> {
    let mut outArg: Arc<DAE::Exp>;
    let Slot { arg: Some(__pa0), .. } = (inSlot.clone()) else { bail!("pattern mismatch") };
    outArg = __pa0.clone();
    Ok(outArg)
}
...
fn fillGraphicsDefaultSlots(mut inCache: FCore::Cache, mut inSlots: Arc<metamodelica::List<Slot>>, mut inClass: Arc<SCode::Element>, mut inEnv: FCore::Graph, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<metamodelica::List<Slot>>, Arc<metamodelica::List<DAE::Const>>, Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>)> {
    ...
    let mut exp: Arc<DAE::Exp>;
    let mut id: ArcStr = arcstr::literal!("");
    let mut defarg: Arc<DAE::FuncArg>;
    let mut ty: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
    let mut c: DAE::Const = DAE::Const::C_CONST;
    for mut slot in &*inSlots.clone() {
        let mut slot = slot.clone();
...
fn printSlotsStr(mut inSlots: Arc<metamodelica::List<Slot>>) -> Result<ArcStr> {
    ...
    outString = ((::match_deref::match_deref! { match &(inSlots.clone()) {
        Deref @ metamodelica::List::Cons { head: Slot { dims: ds, arg: exp, slotFilled: filled, defaultArg: farg, .. }, tail: xs } => {
            ...
            r#str = (Util::applyOptionOrDefault(exp.clone(), Arc::new(ExpressionBasics::printExpStr), (literal!("")).clone())).clone();
            str_lst = List::map(ds.clone(), Arc::new(ExpressionBasics::dimensionString));
...
fn elabPositionalInputArg(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut farg: Arc<DAE::FuncArg>, mut position: i32, mut inSlotLst: Arc<metamodelica::List<Slot>>, mut onlyOneFunction: bool, mut checkTypes: bool, mut r#impl: bool, mut inPolymorphicBindings: Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>, mut inPrefix: DAE::Prefix, mut info: SourceInfo, mut path: Arc<Absyn::Path>, mut isGraphicsExp: bool) -> Result<(FCore::Cache, Arc<metamodelica::List<Slot>>, DAE::Const, Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>)> {
    ...
    (outCache, outSlotLst, outConst, outPolymorphicBindings) = 'mc: {
        let __mc_input = (inCache.clone(), inEnv.clone(), inExp.clone(), farg.clone(), position.clone(), inSlotLst.clone(), onlyOneFunction.clone(), checkTypes.clone(), r#impl.clone(), inPolymorphicBindings.clone(), inPrefix.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, e, Deref @ DAE::FuncArg { par: pr, ty: vt @ Deref @ DAE::Type::T_CODE { ty: ct }, name: id, .. }, _, slots, _, true, _, polymorphicBindings, pre) => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, e, Deref @ DAE::FuncArg { par: pr, ty: vt, name: id, .. }, _, slots, _, true, _, polymorphicBindings, pre) => {
                    let mut slots_1: Arc<metamodelica::List<Slot>> = metamodelica::nil();
                    let mut e_1: Arc<DAE::Exp>;
                    ...
                    (vt, _) = Types::traverseType(vt.clone(), -1, Arc::new(fnptr!(Types::makeExpDimensionsUnknown, Arc<DAE::Type>, i32)))?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, e, Deref @ DAE::FuncArg { par: pr, name: id, .. }, _, slots, _, false, _, polymorphicBindings, pre) => {
                    let mut slots_1: Arc<metamodelica::List<Slot>> = metamodelica::nil();
                    let mut e_1: Arc<DAE::Exp>;
                    let mut t: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
                    let mut c1: DAE::Const = DAE::Const::C_CONST;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, e, Deref @ DAE::FuncArg { ty: vt, name: id, .. }, _, _, true, true, _, _, pre) => {
                    let mut e_1: Arc<DAE::Exp>;
                    let mut prop: DAE::Properties;
                    let mut s1: ArcStr = arcstr::literal!("");
                    let mut s2: ArcStr = arcstr::literal!("");
...
fn elabNamedInputArg(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inNamedArg: Arc<Absyn::NamedArg>, mut inTypesFuncArgLst: Arc<metamodelica::List<Arc<DAE::FuncArg>>>, mut inSlotLst: Arc<metamodelica::List<Slot>>, mut onlyOneFunction: bool, mut checkTypes: bool, mut r#impl: bool, mut inPolymorphicBindings: Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>, mut inPrefix: DAE::Prefix, mut info: SourceInfo, mut path: Arc<Absyn::Path>, mut numErrors: i32, mut isGraphicsExp: bool) -> Result<(FCore::Cache, Arc<metamodelica::List<Slot>>, DAE::Const, Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>)> {
    ...
    (outCache, outSlotLst, outTypesConstLst, outPolymorphicBindings) = 'mc: {
        let __mc_input = (inCache.clone(), inEnv.clone(), inNamedArg.clone(), inTypesFuncArgLst.clone(), inSlotLst.clone(), onlyOneFunction.clone(), checkTypes.clone(), r#impl.clone(), inPolymorphicBindings.clone(), inPrefix.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ Absyn::NamedArg { argValue: e, argName: id }, farg, slots, _, true, _, polymorphicBindings, pre) => {
                    let mut e_1: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ Absyn::NamedArg { argValue: e, argName: id }, farg, slots, _, false, _, polymorphicBindings, pre) => {
                    let mut e_1: Arc<DAE::Exp>;
                    let mut vt: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
                    let mut c1: DAE::Const = DAE::Const::C_CONST;
                    let mut pr: DAE::VarParallelism = DAE::VarParallelism::NON_PARALLEL;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, _, Deref @ Absyn::NamedArg { argName: id, .. }, farg, slots, true, _, _, polymorphicBindings, _) => {
                    let mut s1: ArcStr = arcstr::literal!("");
                    if '__try0: {
                        let _ = unwrap_break_err!(findNamedArgType((id.clone()).clone(), farg.clone()), '__try0);
                        Ok::<(), anyhow::Error>(())
                    }.is_ok() { bail!("failure(): body succeeded") }
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ Absyn::NamedArg { argValue: e, argName: id }, farg, _, true, true, _, _, pre) => {
                    let mut e_1: Arc<DAE::Exp>;
                    let mut vt: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
                    let mut prop: DAE::Properties;
                    let mut s1: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, c, r#impl, pre) => {
                    ...
                    let mut scope: ArcStr = arcstr::literal!("");
                    if '__try0: {
                        (_, _, _, _) = unwrap_break_err!(elabCrefSubs(cache.clone(), env.clone(), env.clone(), c.clone(), pre.clone(), openmodelica_frontend_types::DAE::Prefix::NOPRE, r#impl.clone(), false, info.clone()), '__try0);
                        Ok::<(), anyhow::Error>(())
                    }.is_ok() { bail!("failure(): body succeeded") }
...
fn lookupFunctionsInEnvNoError(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPath: Arc<Absyn::Path>, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<metamodelica::List<Arc<DAE::Type>>>)> {
    ...
    (outCache, outTypesTypeLst) = 'mc: {
        let __mc_input = (inCache.clone(), inEnv.clone(), inPath.clone(), inInfo.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, _, _) => {
                    let mut outCache: FCore::Cache = outCache.clone();
...
fn evaluateEmptyVariable(mut hasZeroSizeDim: bool, mut inExp: Arc<DAE::Exp>, mut ty: Arc<DAE::Type>, mut c: DAE::Const) -> Result<(Arc<DAE::Exp>, DAE::Const)> {
    ...
    (oexp, oc) = 'mc: {
        let __mc_input = (hasZeroSizeDim.clone(), inExp.clone(), ty.clone(), c.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (true, Deref @ DAE::Exp::ASUB { sub: ss, .. }, _, _) => {
                    let mut sc: bool = false;
                    let mut a: bool = false;
                    let mut et: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
                    let mut expl: Arc<metamodelica::List<Arc<DAE::Exp>>> = metamodelica::nil();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (true, Deref @ DAE::Exp::CREF { componentRef: cr, .. }, _, _) => {
                    let mut sc: bool = false;
                    let mut a: bool = false;
                    let mut et: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
                    let mut exp: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (true, Deref @ DAE::Exp::CREF { componentRef: cr, .. }, _, _) => {
                    ...
                    ss = __pa0.clone();
                    exp = Arc::new(DAE::Exp::ARRAY { ty: et.clone(), scalar: sc.clone(), array: metamodelica::nil() });
                    exp = Expression::makeASUB(exp.clone(), List::map(ss.clone(), Arc::new(Expression::getSubscriptExp)))?;
                    Ok((exp.clone(), c.clone()))
                }
                _ => bail!("nomatch"),
            }}
...
fn fillSubscripts(mut inExpSubscriptLst: Arc<metamodelica::List<Arc<DAE::Subscript>>>, mut inType: Arc<DAE::Type>) -> Result<Arc<metamodelica::List<Arc<DAE::Subscript>>>> {
    ...
    outExpSubscriptLst = 'mc: {
        let __mc_input = (inExpSubscriptLst.clone(), inType.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, Deref @ DAE::Type::T_ARRAY { .. }) => {
                    let mut subs: Arc<metamodelica::List<Arc<DAE::Subscript>>> = metamodelica::nil();
...
pub fn crefVectorize(mut performVectorization: bool, mut inExp: Arc<DAE::Exp>, mut inType: Arc<DAE::Type>, mut splicedExp: Option<Arc<DAE::Exp>>, mut crefIdType: Arc<DAE::Type>) -> Result<Arc<DAE::Exp>> {
    let mut outExp: Arc<DAE::Exp>;
    outExp = 'mc: {
        let __mc_input = (performVectorization.clone(), inExp.clone(), inType.clone(), splicedExp.clone(), crefIdType.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (false, e, _, _, _) => {
                    Ok(e.clone())
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, e, Deref @ DAE::Type::T_SUBTYPE_BASIC { complexType: t, .. }, _, _) => {
                    let mut e = (*e).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, Deref @ DAE::Type::T_ARRAY { ty: Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: d2, tail: Deref @ metamodelica::List::Nil }, .. }, dims: Deref @ metamodelica::List::Cons { head: d1, tail: Deref @ metamodelica::List::Nil } }, Some(Deref @ DAE::Exp::CREF { componentRef: cr, .. }), _) => {
                    let mut b1: bool = false;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, Deref @ DAE::Type::T_ARRAY { ty: t, dims: Deref @ metamodelica::List::Cons { head: d1, tail: Deref @ metamodelica::List::Nil } }, Some(Deref @ DAE::Exp::CREF { componentRef: cr, .. }), _) => {
                    let mut e: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, Deref @ DAE::Exp::CREF { ty: exptp, componentRef: cr }, Deref @ DAE::Type::T_ARRAY { ty: t @ Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: d2, tail: Deref @ metamodelica::List::Nil }, .. }, dims: Deref @ metamodelica::List::Cons { head: d1, tail: Deref @ metamodelica::List::Nil } }, _, _) => {
                    let mut b1: bool = false;
                    let mut b2: bool = false;
                    let mut e: Arc<DAE::Exp>;
                    let mut ds: i32 = 0;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, Deref @ DAE::Exp::CREF { ty: exptp, componentRef: cr }, Deref @ DAE::Type::T_ARRAY { ty: t, dims: Deref @ metamodelica::List::Cons { head: d1, tail: Deref @ metamodelica::List::Nil } }, _, _) => {
                    let mut e: Arc<DAE::Exp>;
                    let mut ds: i32 = 0;
                    let false = (Types::isArray(t.clone())) else { bail!("pattern mismatch") };
                    ds = Expression::dimensionSize(d1.clone())?;
...
fn mergeQualWithRest(mut qual: Arc<DAE::Exp>, mut rest: Arc<DAE::Exp>, mut inType: Arc<DAE::Type>) -> Result<Arc<DAE::Exp>> {
    ...
    outExp = (::match_deref::match_deref! { match &((qual.clone(), rest.clone(), inType.clone())) {
        ...
        (Deref @ DAE::Exp::ARRAY { ty: _, scalar: _, array: expl1 }, exp2, ety) => {
            ...
            expl1 = List::map2(expl1.clone(), Arc::new(mergeQualWithRest), exp2.clone(), ety.clone());
...
fn mergeQualWithRest2(mut rest: Arc<DAE::Exp>, mut qual: Arc<DAE::Exp>) -> Result<Arc<DAE::Exp>> {
    ...
    outExp = (::match_deref::match_deref! { match &((rest.clone(), qual.clone())) {
        ...
        (exp1 @ Deref @ DAE::Exp::ARRAY { ty: ety, scalar: _, array: expl1 }, exp2 @ Deref @ DAE::Exp::CREF { componentRef: Deref @ DAE::ComponentRef::CREF_IDENT { ident: _, identType: _, subscriptLst: _ }, ty: _ }) => {
            ...
            let mut expl1 = (*expl1).clone();
            expl1 = List::map1(expl1.clone(), Arc::new(mergeQualWithRest2), exp2.clone());
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ DAE::Subscript::SLICE { exp: Deref @ DAE::Exp::ARRAY { ty: _, scalar: _, array: expl1 @ Deref @ metamodelica::List::Cons { head: Deref @ DAE::Exp::ICONST { integer: 0 }, tail: Deref @ metamodelica::List::Nil } } }, tail: subs1 }, id, ety) => {
                    ...
                    let mut exp3: Arc<DAE::Exp>;
                    exp2 = flattenSubscript2(subs1.clone(), (id.clone()).clone(), ety.clone())?;
                    expl2 = List::map3(expl1.clone(), Arc::new(applySubscript), exp2.clone(), (id.clone()).clone(), ety.clone());
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Exp::ARRAY { array: expl1 @ Deref @ metamodelica::List::Cons { head: Deref @ DAE::Exp::ARRAY { .. }, tail: expl3 }, scalar: sc, ty: ty1 } => {
                    ...
                    expl3 = List::map(expl1.clone(), Arc::new(removeDoubleEmptyArrays));
...
fn applySubscript2(mut inSub: Arc<DAE::Exp>, mut inSubs: Arc<DAE::Exp>, mut inType: Arc<DAE::Type>) -> Result<Arc<DAE::Exp>> {
    ...
    outExp = (::match_deref::match_deref! { match &((inSub.clone(), inSubs.clone(), inType.clone())) {
        ...
        (exp1, Deref @ DAE::Exp::ARRAY { ty: _, scalar: _, array: expl1 }, ety) => {
            ...
            let mut expl1 = (*expl1).clone();
            let mut ety = (*ety).clone();
            expl1 = List::map2(expl1.clone(), Arc::new(applySubscript3), exp1.clone(), ety.clone());
            exp2 = Arc::new(DAE::Exp::ARRAY { ty: DAE::T_INTEGER_DEFAULT.clone(), scalar: false, array: expl1.clone() });
            (iLst, scalar) = extractDimensionOfChild(exp2.clone())?;
            ety = Expression::arrayEltType(ety.clone());
            exp2 = Arc::new(DAE::Exp::ARRAY { ty: Arc::new(DAE::Type::T_ARRAY { ty: ety.clone(), dims: iLst.clone() }), scalar: scalar.clone(), array: expl1.clone() });
...
fn createCrefArray(mut inComponentRef1: Arc<DAE::ComponentRef>, mut inInteger2: i32, mut inInteger3: i32, mut inType4: Arc<DAE::Type>, mut inType5: Arc<DAE::Type>, mut crefIdType: Arc<DAE::Type>) -> Result<Arc<DAE::Exp>> {
    let mut outExp: Arc<DAE::Exp>;
    outExp = 'mc: {
        let __mc_input = (inComponentRef1.clone(), inInteger2.clone(), inInteger3.clone(), inType4.clone(), inType5.clone(), crefIdType.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, indx, ds, et, _, _) => {
...
fn createCrefArray2d(mut inCref: Arc<DAE::ComponentRef>, mut inIndex: i32, mut inDim1: i32, mut inDim2: i32, mut inType5: Arc<DAE::Type>, mut inType6: Arc<DAE::Type>, mut crefIdType: Arc<DAE::Type>) -> Result<Arc<DAE::Exp>> {
    let mut outExp: Arc<DAE::Exp>;
    outExp = 'mc: {
        let __mc_input = (inCref.clone(), inIndex.clone(), inDim1.clone(), inDim2.clone(), inType5.clone(), inType6.clone(), crefIdType.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, indx, ds, _, et, _, _) => {
                    if !((indx.clone() > ds.clone())) { bail!("guard") }
...
fn elabCrefSubs(mut inCache: FCore::Cache, mut inCrefEnv: FCore::Graph, mut inSubsEnv: FCore::Graph, mut inComponentRef: Arc<Absyn::ComponentRef>, mut inTopPrefix: DAE::Prefix, mut inCrefPrefix: DAE::Prefix, mut inBoolean: bool, mut inHasZeroSizeDim: bool, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::ComponentRef>, DAE::Const, bool)> {
    ...
    (outCache, outComponentRef, outConst, outHasZeroSizeDim) = 'mc: {
        let __mc_input = (inCache.clone(), inCrefEnv.clone(), inSubsEnv.clone(), inComponentRef.clone(), inTopPrefix.clone(), inCrefPrefix.clone(), inBoolean.clone(), inHasZeroSizeDim.clone(), info.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, crefEnv, crefSubs, Deref @ Absyn::ComponentRef::CREF_IDENT { subscripts: ss, name: id }, topPrefix, crefPrefix, r#impl, hasZeroSizeDim, _) => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, crefEnv, crefSubs, Deref @ Absyn::ComponentRef::CREF_QUAL { componentRef: restCref, subscripts: Deref @ metamodelica::List::Nil, name: id }, topPrefix, crefPrefix, r#impl, hasZeroSizeDim, _) => {
                    let mut t: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
                    let mut sl: Arc<metamodelica::List<Arc<DAE::Dimension>>> = metamodelica::nil();
                    let mut r#const: DAE::Const = DAE::Const::C_CONST;
                    let mut cr: Arc<DAE::ComponentRef> = Arc::new(DAE::ComponentRef::WILD);
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, crefEnv, crefSubs, Deref @ Absyn::ComponentRef::CREF_QUAL { componentRef: restCref, subscripts: ss @ Deref @ metamodelica::List::Cons { head: _, tail: _ }, name: id }, topPrefix, crefPrefix, r#impl, hasZeroSizeDim, _) => {
                    let mut t: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
                    let mut sl: Arc<metamodelica::List<Arc<DAE::Dimension>>> = metamodelica::nil();
                    let mut r#const: DAE::Const = DAE::Const::C_CONST;
                    let mut const1: DAE::Const = DAE::Const::C_CONST;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, crefEnv, crefSubs, Deref @ Absyn::ComponentRef::CREF_FULLYQUALIFIED { componentRef: absynCr }, topPrefix, crefPrefix, r#impl, hasZeroSizeDim, _) => {
                    let mut const1: DAE::Const = DAE::Const::C_CONST;
                    let mut cr: Arc<DAE::ComponentRef> = Arc::new(DAE::ComponentRef::WILD);
                    let mut cache = (*cache).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, crefEnv, _, absynCref, topPrefix, crefPrefix, _, _, _) => {
                    let true = (Flags::isSet(Flags::FAILTRACE.clone())?) else { bail!("pattern mismatch") };
...
pub fn elabSubscripts(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inAbsynSubscriptLst: Arc<metamodelica::List<Arc<Absyn::Subscript>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<metamodelica::List<Arc<DAE::Subscript>>>, DAE::Const)> {
    ...
    let mut outExpSubscriptLst: Arc<metamodelica::List<Arc<DAE::Subscript>>> = metamodelica::nil();
    let mut outConst: DAE::Const = DAE::Const::C_CONST;
    (outCache, outExpSubscriptLst, outConst) = (::match_deref::match_deref! { match &((inCache.clone(), inEnv.clone(), inAbsynSubscriptLst.clone(), inBoolean.clone(), inPrefix.clone(), info.clone())) {
        (cache, _, Deref @ metamodelica::List::Nil, _, _, _) => {
            (cache.clone(), metamodelica::nil(), openmodelica_frontend_types::DAE::Const::C_CONST)
        },
        (cache, env, Deref @ metamodelica::List::Cons { head: sub, tail: subs }, r#impl, pre, _) => {
...
fn makeIfExp(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inCondition: Arc<DAE::Exp>, mut inCondProp: DAE::Properties, mut inTrueBranch: Arc<DAE::Exp>, mut inTrueProp: DAE::Properties, mut inFalseBranch: Arc<DAE::Exp>, mut inFalseProp: DAE::Properties, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut true_ty2: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
    let mut false_ty2: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
    let mut exp_ty: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ Absyn::Exp::ARRAY { arrayExp: es }, DAE::CodeType::C_VARIABLENAMES) => {
                    ...
                    let mut et: Arc<DAE::Type> = Arc::new(DAE::Type::T_NORETCALL);
                    let mut i: i32 = 0;
                    es_1 = List::map4(es.clone(), Arc::new(elabCodeExp), cache.clone(), env.clone(), openmodelica_frontend_types::DAE::CodeType::C_VARIABLENAME, info.clone());
...
pub fn elabCodeExp_dispatch(mut exp: Arc<Absyn::Exp>, mut cache: FCore::Cache, mut env: FCore::Graph, mut ct: DAE::CodeType, mut info: SourceInfo) -> Result<Arc<DAE::Exp>> {
    ...
    outExp = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
                    ...
                    let mut dexp: Arc<DAE::Exp>;
                    if '__try0: {
                        (_, _, _) = unwrap_break_err!(Lookup::lookupClassIdent(cache.clone(), env.clone(), (id.clone()).clone(), None), '__try0);
                        Ok::<(), anyhow::Error>(())
                    }.is_ok() { bail!("failure(): body succeeded") }
                    (_, dexp, prop) = elabExpInExpression(cache.clone(), env.clone(), exp.clone(), false, false, openmodelica_frontend_types::DAE::Prefix::NOPRE, info.clone())?;
                    Ok(())
...
fn elabArrayDim(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inCref: Arc<Absyn::ComponentRef>, mut inDimension: Arc<Absyn::Subscript>, mut inImpl: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Dimension>)> {
    ...
    (outCache, outDimension) = 'mc: {
        let __mc_input = (inCache.clone(), inEnv.clone(), inCref.clone(), inDimension.clone(), inImpl.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, _, Deref @ Absyn::Subscript::NOSUB, _) => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, _, Deref @ Absyn::Subscript::SUBSCRIPT { subscript: Deref @ Absyn::Exp::CALL { functionArgs: Deref @ Absyn::FunctionArgs::FUNCTIONARGS { args: Deref @ metamodelica::List::Cons { head: cr_exp @ Deref @ Absyn::Exp::CREF { componentRef: cr }, tail: Deref @ metamodelica::List::Cons { head: size_arg, tail: Deref @ metamodelica::List::Nil } }, .. }, function_: Deref @ Absyn::ComponentRef::CREF_IDENT { name: Deref @ "size", .. }, .. } }, _) => {
                    let mut dim: Arc<DAE::Dimension> = Arc::new(DAE::Dimension::DIM_BOOLEAN);
                    let mut cache: FCore::Cache = FCore::Cache::NO_CACHE;
                    let mut e: Arc<DAE::Exp>;
                    let mut dim_exp: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, _, Deref @ Absyn::Subscript::SUBSCRIPT { subscript: Deref @ Absyn::Exp::CREF { componentRef: Deref @ Absyn::ComponentRef::CREF_IDENT { name: Deref @ "Boolean", .. } } }, _) => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, _, _, Deref @ Absyn::Subscript::SUBSCRIPT { subscript: Deref @ Absyn::Exp::CREF { componentRef: cr } }, _) => {
                    let mut dim: Arc<DAE::Dimension> = Arc::new(DAE::Dimension::DIM_BOOLEAN);
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, _, Deref @ Absyn::Subscript::SUBSCRIPT { subscript: Deref @ Absyn::Exp::EXPRESSIONCOMMENT { exp: sub, .. } }, _) => {
                    let mut dim: Arc<DAE::Dimension> = Arc::new(DAE::Dimension::DIM_BOOLEAN);
                    let mut cache: FCore::Cache = FCore::Cache::NO_CACHE;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, _, Deref @ Absyn::Subscript::SUBSCRIPT { subscript: sub }, _) => {
                    let mut dim: Arc<DAE::Dimension> = Arc::new(DAE::Dimension::DIM_BOOLEAN);
                    let mut cache: FCore::Cache = FCore::Cache::NO_CACHE;
                    let mut e: Arc<DAE::Exp>;
                    let mut prop: DAE::Properties;
...
fn elabArrayDim2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inCref: Arc<Absyn::ComponentRef>, mut inExp: Arc<DAE::Exp>, mut inProperties: DAE::Properties, mut inImpl: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Option<Arc<DAE::Dimension>>)> {
    ...
    (outCache, outDimension) = 'mc: {
        let __mc_input = (inCache.clone(), inEnv.clone(), inCref.clone(), inExp.clone(), inProperties.clone(), inImpl.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, _, _, DAE::Properties::PROP { type_: Deref @ DAE::Type::T_INTEGER { .. }, constFlag: cnst }, _) => {
                    let mut cache: FCore::Cache = FCore::Cache::NO_CACHE;
                    let mut i: i32 = 0;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, _, _, DAE::Properties::PROP { type_: Deref @ DAE::Type::T_INTEGER { .. }, constFlag: DAE::Const::C_PARAM }, _) => {
                    let false = (Config::splitArrays()?) else { bail!("pattern mismatch") };
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, _, _, DAE::Properties::PROP { type_: Deref @ DAE::Type::T_INTEGER { .. }, constFlag: DAE::Const::C_VAR }, false) => {
                    let mut e_str: ArcStr = arcstr::literal!("");
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, _, _, DAE::Properties::PROP { type_: Deref @ DAE::Type::T_INTEGER { .. }, constFlag: _ }, true) => {
                    let mut cache: FCore::Cache = FCore::Cache::NO_CACHE;
                    let mut e: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, _, _, DAE::Properties::PROP { type_: ty, constFlag: _ }, _) => {
                    let mut e_str: ArcStr = arcstr::literal!("");
                    let mut t_str: ArcStr = arcstr::literal!("");
                    e_str = (ExpressionBasics::printExpStr(inExp.clone())?).clone();
...
fn consStrippedCref(mut e: Arc<Absyn::Exp>, mut es: Arc<metamodelica::List<Arc<Absyn::Exp>>>) -> Result<Arc<metamodelica::List<Arc<Absyn::Exp>>>> {
    ...
    oes = (::match_deref::match_deref! { match &((e.clone(), es.clone())) {
...
fn replaceEndTraverser(mut inExp: Arc<Absyn::Exp>, mut inTuple: (Arc<Absyn::ComponentRef>, i32)) -> Result<Arc<Absyn::Exp>> {
    ...
    outExp = (::match_deref::match_deref! { match &(inExp.clone()) {
        ...
        _ => {
            AbsynUtil::traverseExpShallow(inExp.clone(), inTuple.clone(), Arc::new(replaceEndTraverser))?
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    Ok(outExp)
