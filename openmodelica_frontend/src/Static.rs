...
pub(crate) fn elabExpList(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExpl: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo, mut inLastType: Arc<DAE::Type>) -> Result<(FCore::Cache, Arc<metamodelica::List<Arc<DAE::Exp>>>, Arc<metamodelica::List<DAE::Properties>>)> {
    ...
    for mut e in &*inExpl {
        ...
        match '__try0: {
            ...
            path2 = __pa3.clone();
            names = __pa4.clone();
            path = unwrap_break_err!(AbsynUtil::crefToPath(cr.clone()), '__try0);
            let (__pa5, __pa6) = ::match_deref::match_deref! { match &(unwrap_break_err!(AbsynUtil::splitQualAndIdentPath(path.clone()), '__try0)) {
                (__pa5, Deref @ Absyn::Path::IDENT { name: __pa6 }) => (__pa5.clone(), __pa6.clone()),
                _ => break '__try0 Err::<_, _>(anyhow::anyhow!("pattern mismatch")),
            } };
            path1 = __pa5.clone();
            name = __pa6.clone();
            let true = (AbsynUtil::pathEqual(path1.clone(), path2.clone())) else { break '__try0 Err::<_, _>(anyhow::anyhow!("pattern mismatch")) };
            idx = unwrap_break_err!(List::position((name.clone()).clone(), names.clone()), '__try0);
            exp = Arc::new(DAE::Exp::ENUM_LITERAL { name: path.clone(), index: idx });
            prop = DAE::Properties::PROP { type_: last_ty.clone(), constFlag: openmodelica_frontend_types::DAE::Const::C_CONST };
            Ok::<_, anyhow::Error>((exp.clone(), prop.clone()))
        } {
            ...
            Err(_) => {
                (outCache, exp, prop) = elabExpInExpression(outCache.clone(), inEnv.clone(), e.clone(), inImplicit, inDoVect, inPrefix.clone(), inInfo.clone())?;
                last_ty = Types::getPropType(prop.clone())?;
            }
        }
        outExpl = metamodelica::cons(exp.clone(), outExpl.clone());
        outProperties = metamodelica::cons(prop.clone(), outProperties.clone());
    }
...
fn elabExpList_enum(mut inExp: Arc<Absyn::Exp>, mut inLastType: Arc<DAE::Type>) -> i32 {
...
pub(crate) fn elabExpListList(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExpl: Arc<metamodelica::List<Arc<metamodelica::List<Arc<Absyn::Exp>>>>>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo, mut inLastType: Arc<DAE::Type>) -> Result<(FCore::Cache, Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Exp>>>>>, Arc<metamodelica::List<Arc<metamodelica::List<DAE::Properties>>>>)> {
    ...
    for mut lst in &*inExpl {
        ...
        (outCache, expl, props) = elabExpList(outCache.clone(), inEnv.clone(), lst.clone(), inImplicit, inDoVect, inPrefix.clone(), inInfo.clone(), last_ty.clone())?;
        outExpl = metamodelica::cons(expl.clone(), outExpl.clone());
        outProperties = metamodelica::cons(props.clone(), outProperties.clone());
...
fn elabExpOptAndMatchType(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Option<Arc<Absyn::Exp>>, mut inDefaultType: Arc<DAE::Type>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Option<Arc<DAE::Exp>>, DAE::Properties)> {
    ...
    let mut prop: DAE::Properties;
    outProperties = DAE::Properties::PROP { type_: inDefaultType, constFlag: openmodelica_frontend_types::DAE::Const::C_CONST };
...
pub fn elabExp(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    e = if (BackendInterface::noRewriteRulesFrontEnd()?) {inExp} else {(BackendInterface::rewriteFrontEnd(inExp)?).0};
    ...
    match '__try0: {
        ...
        Err(__try0_err) => {
            let true = (num_errmsgs == Error::getNumErrorMessages()) else { bail!("pattern mismatch") };
...
fn elabExp_Cref(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    if !(Flags::getConfigBool(Flags::CEVAL_EQUATION.clone())?) {
        ...
        ty = __pa4.clone();
        c = __pa5.clone();
        outProperties = if (Types::isParameter(c)) {DAE::Properties::PROP { type_: ty, constFlag: openmodelica_frontend_types::DAE::Const::C_VAR }} else {outProperties};
    }
    Ok((outCache, outExp, outProperties))
}
...
fn elabExp_Binary(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let () = (::match_deref::match_deref! { match &(inExp.clone()) {
        Deref @ Absyn::Exp::BINARY { exp1: __esc_e1, op: __esc_op, exp2: __esc_e2 } => {
            e1 = (*__esc_e1).clone();
            op = (*__esc_op).clone();
            e2 = (*__esc_e2).clone();
            ()
        },
        Deref @ Absyn::Exp::LBINARY { exp1: __esc_e1, op: __esc_op, exp2: __esc_e2 } => {
            e1 = (*__esc_e1).clone();
            op = (*__esc_op).clone();
            e2 = (*__esc_e2).clone();
            ()
        },
        Deref @ Absyn::Exp::RELATION { exp1: __esc_e1, op: __esc_op, exp2: __esc_e2 } => {
            e1 = (*__esc_e1).clone();
            op = (*__esc_op).clone();
            e2 = (*__esc_e2).clone();
            ()
    ...
    (outCache, exp1, prop1) = elabExpInExpression(inCache, inEnv.clone(), e1.clone(), inImplicit, inDoVect, inPrefix.clone(), inInfo.clone())?;
    (outCache, exp2, prop2) = elabExpInExpression(outCache, inEnv.clone(), e2.clone(), inImplicit, inDoVect, inPrefix.clone(), inInfo.clone())?;
...
fn elabExp_LUnary(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    op = __pa0.clone();
    e = __pa1.clone();
    (outCache, outExp, outProperties) = elabExpInExpression(outCache, inEnv.clone(), e.clone(), inImplicit, inDoVect, inPrefix.clone(), inInfo.clone())?;
    (outCache, outExp, outProperties) = OperatorOverloading::unary(outCache, inEnv, op, outProperties, outExp, inExp, e, inImplicit, inPrefix, inInfo)?;
    Ok((outCache, outExp, outProperties))
}
...
fn elabExp_If(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut cond_e: Arc<Absyn::Exp>;
    let mut true_e: Arc<Absyn::Exp>;
    let mut false_e: Arc<Absyn::Exp>;
    ...
    true_e = __pa1.clone();
    false_e = __pa2.clone();
    (cache, cond_exp, cond_prop) = elabExpInExpression(inCache, inEnv.clone(), cond_e, inImplicit, inDoVect, inPrefix.clone(), inInfo.clone())?;
    (outCache, outExp, outProperties) = 'mc: {
        ...
        if let Ok((__v, __wb0, __wb1, __wb2, __wb3, __wb4, __wb5, __wb6)) = (|| -> Result<_> {
            ...
            (outCache, true_exp, true_prop) = elabExpInExpression(cache.clone(), inEnv.clone(), true_e.clone(), inImplicit, inDoVect, inPrefix.clone(), inInfo.clone())?;
            (outCache, false_exp, false_prop) = elabExpInExpression(outCache.clone(), inEnv.clone(), false_e.clone(), inImplicit, inDoVect, inPrefix.clone(), inInfo.clone())?;
            (outCache, outExp, outProperties) = makeIfExp(outCache.clone(), inEnv.clone(), cond_exp.clone(), cond_prop.clone(), true_exp.clone(), true_prop.clone(), false_exp.clone(), false_prop.clone(), inImplicit, inPrefix.clone(), inInfo.clone())?;
        ...
        if let Ok((__v, __wb0, __wb1, __wb2, __wb3)) = (|| -> Result<_> {
            ...
            b = __pa1.clone();
            (outCache, outExp, outProperties) = elabExpInExpression(outCache.clone(), inEnv.clone(), if (b) {true_e.clone()} else {false_e.clone()}, inImplicit, inDoVect, inPrefix.clone(), inInfo.clone())?;
...
fn elabExp_Call(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp> = Arc::new(<DAE::Exp as ::std::default::Default>::default());
    let mut outProperties: DAE::Properties = <DAE::Properties as ::std::default::Default>::default();
    let mut func_name: Arc<Absyn::ComponentRef>;
    let mut args: Arc<Absyn::FunctionArgs>;
    let mut type_vars: Arc<metamodelica::List<Arc<Absyn::Path>>>;
    ...
    func_name = __pa0.clone();
    args = __pa1.clone();
    type_vars = __pa2.clone();
    let () = (::match_deref::match_deref! { match &(args.clone()) {
        Deref @ Absyn::FunctionArgs::FUNCTIONARGS { .. } => {
            (outCache, outExp, outProperties) = elabCall(inCache, inEnv, func_name, var_field!((*args).args, Absyn::FunctionArgs::FUNCTIONARGS).clone(), var_field!((*args).argNames, Absyn::FunctionArgs::FUNCTIONARGS).clone(), type_vars, inImplicit, inPrefix, inInfo)?;
            (outExp, _) = ExpressionSimplify::simplify1(outExp)?;
            ()
        },
        Deref @ Absyn::FunctionArgs::FOR_ITER_FARG { .. } => {
            (outCache, outExp, outProperties) = elabCallReduction(inCache, inEnv, func_name, var_field!((*args).exp, Absyn::FunctionArgs::FOR_ITER_FARG).clone(), var_field!((*args).iterType, Absyn::FunctionArgs::FOR_ITER_FARG).clone(), var_field!((*args).iterators, Absyn::FunctionArgs::FOR_ITER_FARG).clone(), inImplicit, inDoVect, inPrefix, inInfo)?;
            ()
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn elabExp_Dot(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outExp, outProperties) = (::match_deref::match_deref! { match &(inExp.clone()) {
        ...
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
            ...
            (outCache, outExp, outProperties) = elabExp(inCache, inEnv, var_field!((*inExp).exp, Absyn::Exp::DOT).clone(), inImplicit, inDoVect, inPrefix, inInfo.clone())?;
        ...
        Deref @ DAE::Type::T_TUPLE { names: Some(names), .. } => {
            ...
            if !(listMember((s.clone()).clone(), names.clone())) {
                Error::addSourceMessage(Error::COMPILER_ERROR.clone(), list![({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("Dot operator could not find ")); __mm_s.push_str(&*s.clone()); __mm_s.push_str(&*literal!(" in ")); __mm_s.push_str(&*TypesDump::unparseType(ty.clone())?); ArcStr::from(__mm_s) }).clone()], inInfo)?;
                bail!("fail");
            }
            i = List::position((s).clone(), names.clone())?;
            outExp = Arc::new(DAE::Exp::TSUB { exp: outExp, ix: i, ty: (var_field!((*ty).types, DAE::Type::T_TUPLE).clone()).get(i)? });
            outProperties = DAE::Properties::PROP { type_: (var_field!((*ty).types, DAE::Type::T_TUPLE).clone()).get(i)?, constFlag: Types::propAllConst(outProperties)? };
            ()
        },
        _ => {
...
fn elabExp_PartEvalFunction(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut pos_args: Arc<metamodelica::List<Arc<Absyn::Exp>>>;
    let mut named_args: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
    let mut path: Arc<Absyn::Path>;
    let mut ty: Arc<DAE::Type>;
    let mut tty: Arc<DAE::Type>;
    let mut tty2: Arc<DAE::Type>;
    let mut args: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    let mut consts: Arc<metamodelica::List<DAE::Const>>;
    let mut slots: Arc<metamodelica::List<Slot>>;
    let mut c: DAE::Const;
    let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(inExp) {
    ...
    if pos_args.clone().is_empty() && named_args.clone().is_empty() {
        (outCache, outExp, outProperties) = elabExpInExpression(inCache, inEnv, Arc::new(Absyn::Exp::CREF { componentRef: cref }), inImplicit, inDoVect, inPrefix, inInfo)?;
    } else {
        ...
        (outCache, args, consts, _, tty, _, slots) = elabTypes(outCache, inEnv.clone(), pos_args, named_args, metamodelica::nil(), list![tty], true, true, inImplicit, inPrefix, inInfo)?;
        if !(Types::isFunctionPointer(tty.clone())) {
            (outCache, path) = Inst::makeFullyQualified(outCache, inEnv.clone(), path)?;
            let (__pa7, Util::SUCCESS { .. }) = (instantiateDaeFunction(outCache, inEnv, path.clone(), false, None, true)) else { bail!("pattern mismatch") };
        ...
        c = List::fold(consts, (std::sync::Arc::new(fnptr!(Types::constAnd, DAE::Const, DAE::Const)) as std::sync::Arc<dyn ::std::ops::Fn(DAE::Const, DAE::Const) -> Result<DAE::Const> + 'static>), openmodelica_frontend_types::DAE::Const::C_CONST)?;
        outExp = Arc::new(DAE::Exp::PARTEVALFUNCTION { path: path, expList: args, ty: ty, origType: tty });
        outProperties = DAE::Properties::PROP { type_: tty2, constFlag: c };
    }
    Ok((outCache, outExp, outProperties))
}
...
fn elabExp_Tuple(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = elabExp_Tuple_LHS_RHS(inCache, inEnv, inExp, inImplicit, inDoVect, inPrefix, inInfo, false)?;
    Ok((outCache, outExp, outProperties))
}
...
fn elabExp_Tuple_LHS_RHS(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo, mut isLhs: bool) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    let mut el: Arc<metamodelica::List<Arc<Absyn::Exp>>>;
    let mut expl: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    let mut props: Arc<metamodelica::List<DAE::Properties>>;
    let mut types: Arc<metamodelica::List<Arc<DAE::Type>>>;
    let mut consts: Arc<metamodelica::List<Arc<DAE::TupleConst>>>;
    let __pa0 = ::match_deref::match_deref! { match &(inExp) {
    ...
    if (el.clone().len() as i32) == 1 {
        (outCache, outExp, outProperties) = elabExp(outCache, inEnv, (el).get(1)?, inImplicit, inDoVect, inPrefix, inInfo)?;
...
pub(crate) fn elabExpLHS(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &(inExp.clone()) {
        Deref @ Absyn::Exp::TUPLE { .. } => {
            (outCache, outExp, outProperties) = elabExp_Tuple_LHS_RHS(inCache, inEnv, inExp, inImplicit, inDoVect, inPrefix, inInfo, true)?;
...
fn elabExp_Range(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut step: Arc<Absyn::Exp>;
    let mut stop: Arc<Absyn::Exp>;
    let mut ostep: Option<Arc<Absyn::Exp>>;
    ...
    let mut start_ty: Arc<DAE::Type>;
    let mut step_ty: Arc<DAE::Type>;
    let mut stop_ty: Arc<DAE::Type>;
        ...
        if let Ok((__v, __wb0, __wb1, __wb2, __wb3, __wb4, __wb5, __wb6)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ Absyn::Exp::ARRAY { arrayExp: es } => {
                    ...
                    (outCache, expl, props) = elabExpList(inCache.clone(), inEnv.clone(), es.clone(), inImplicit, inDoVect, inPrefix.clone(), inInfo.clone(), DAE::T_UNKNOWN_DEFAULT().clone())?;
        ...
        if let Ok((__v, __wb0, __wb1, __wb2)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ Absyn::Exp::ARRAY { arrayExp: es } => {
                    ...
                    let mut outExp: Arc<DAE::Exp> = outExp.clone();
                    let mut outProperties: DAE::Properties = outProperties.clone();
                    (outCache, outExp, outProperties) = elabExpInExpression(inCache.clone(), inEnv.clone(), Arc::new(Absyn::Exp::LIST { exps: es.clone() }), inImplicit, inDoVect, inPrefix.clone(), inInfo.clone())?;
...
fn elabExp_Matrix(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    let mut ess: Arc<metamodelica::List<Arc<metamodelica::List<Arc<Absyn::Exp>>>>>;
    let mut dess: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Exp>>>>>;
    let mut props: Arc<metamodelica::List<Arc<metamodelica::List<DAE::Properties>>>>;
    ...
    ess = __pa0.clone();
    (outCache, dess, props) = elabExpListList(inCache, inEnv.clone(), ess, inImplicit, inDoVect, inPrefix.clone(), inInfo.clone(), DAE::T_UNKNOWN_DEFAULT().clone())?;
    tys = ({
    ...
    if have_real {
        (dess, props) = List::threadMapList_2(dess, props, (std::sync::Arc::new(elabExp_Matrix_realCast) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, DAE::Properties) -> Result<(Arc<DAE::Exp>, DAE::Properties)> + 'static>))?;
    }
    ...
    ty = Arc::new(DAE::Type::T_ARRAY { ty: ty, dims: list![dim2] });
    ty = Arc::new(DAE::Type::T_ARRAY { ty: ty, dims: list![dim1] });
    outProperties = DAE::Properties::PROP { type_: ty, constFlag: c };
    Ok((outCache, outExp, outProperties))
}
...
fn elabExp_Matrix_realCast(mut inExp: Arc<DAE::Exp>, mut inProperties: DAE::Properties) -> Result<(Arc<DAE::Exp>, DAE::Properties)> {
    ...
    if Types::isInteger(ty.clone()) {
        ...
        (outExp, _) = ExpressionSimplify::simplify1(Arc::new(DAE::Exp::CAST { ty: ty, exp: inExp }))?;
    } else {
        outExp = inExp;
...
fn elabExp_Code(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    outExp = Arc::new(DAE::Exp::CODE { code: cn, ty: ty2 });
    outProperties = DAE::Properties::PROP { type_: ty, constFlag: openmodelica_frontend_types::DAE::Const::C_CONST };
    Ok((outCache, outExp, outProperties))
}
...
fn elabExp_Cons(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    e1 = __pa2.clone();
    e2 = __pa3.clone();
    (outCache, exp1, prop1) = elabExpInExpression(outCache, inEnv.clone(), e1, inImplicit, inDoVect, inPrefix.clone(), inInfo.clone())?;
...
fn elabExp_List(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    let mut es: Arc<metamodelica::List<Arc<Absyn::Exp>>>;
    let mut expl: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    let mut props: Arc<metamodelica::List<DAE::Properties>>;
    let mut types: Arc<metamodelica::List<Arc<DAE::Type>>>;
    let mut consts: Arc<metamodelica::List<DAE::Const>>;
    ...
    if es.clone().is_empty() {
        ...
        (outCache, expl, props) = elabExpList(inCache, inEnv, es, inImplicit, inDoVect, inPrefix, inInfo, DAE::T_UNKNOWN_DEFAULT().clone())?;
        types = ({
        let mut __acc: Arc<metamodelica::List<Arc<DAE::Type>>> = metamodelica::nil();
        ...
        (expl, _) = Types::matchTypes(expl, types, ty.clone(), true)?;
        outExp = Arc::new(DAE::Exp::LIST { valList: expl });
        outProperties = DAE::Properties::PROP { type_: Arc::new(DAE::Type::T_METALIST { ty: ty }), constFlag: c };
    }
    Ok((outCache, outExp, outProperties))
}
...
fn elabExpInExpression2(mut inExp: Arc<DAE::Exp>, mut inProperties: DAE::Properties) -> (Arc<DAE::Exp>, DAE::Properties) {
    ...
    (outExp, outProperties) = (::match_deref::match_deref! { match &(inProperties.clone()) {
        DAE::Properties::PROP_TUPLE { type_: Deref @ DAE::Type::T_TUPLE { types: Deref @ metamodelica::List::Cons { head: ty, tail: _ }, .. }, tupleConst: Deref @ DAE::TupleConst::TUPLE_CONST { tupleConstLst: Deref @ metamodelica::List::Cons { head: Deref @ DAE::TupleConst::SINGLE_CONST { r#const: c }, tail: _ } } } => {
            (Arc::new(DAE::Exp::TSUB { exp: inExp, ix: 1, ty: ty.clone() }), DAE::Properties::PROP { type_: ty.clone(), constFlag: c.clone() })
        },
        _ => {
            (inExp, inProperties)
        },
...
fn checkAssignmentToInput2(mut inExp: Arc<Absyn::Exp>, mut inAttributes: Arc<DAE::Attributes>, mut inInfo: SourceInfo) -> Result<()> {
    let () = (::match_deref::match_deref! { match &((inExp, inAttributes)) {
        (Deref @ Absyn::Exp::CREF { componentRef: cr }, Deref @ DAE::Attributes { direction: Absyn::Direction::INPUT { .. }, .. }) => {
            ...
            Error::addSourceMessage(Error::ASSIGN_READONLY_ERROR.clone(), list![(literal!("input")).clone(), (cr_str).clone()], inInfo)?;
            bail!("fail")
        },
        _ => {
            ()
...
pub(crate) fn checkAssignmentToInputs(mut inExpCrefs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inAttributes: Arc<metamodelica::List<Arc<DAE::Attributes>>>, mut inEnv: FCore::Graph, mut inInfo: SourceInfo) -> Result<()> {
    if FGraph::inFunctionScope(inEnv) {
        List::threadMap1_0(inExpCrefs, inAttributes, (std::sync::Arc::new(checkAssignmentToInput2) as std::sync::Arc<dyn ::std::ops::Fn(Arc<Absyn::Exp>, Arc<DAE::Attributes>, SourceInfo) -> Result<()> + 'static>), inInfo)?;
    }
    Ok(())
}
...
pub(crate) fn elabExpCrefNoEvalList(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExpl: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<metamodelica::List<Arc<DAE::Exp>>>, Arc<metamodelica::List<DAE::Properties>>, Arc<metamodelica::List<Arc<DAE::Attributes>>>)> {
    ...
    for mut e in &*inExpl {
        ...
        if '__try0: {
            ...
            let true = (num_err == Error::getNumErrorMessages()) else { bail!("pattern mismatch") };
            Error::addSourceMessage(Error::GENERIC_ELAB_EXPRESSION.clone(), list![(Dump::printExpStr(e.clone())?).clone()], inInfo.clone())?;
        }
    }
    if !(Flags::getConfigBool(Flags::CEVAL_EQUATION.clone())?) {
...
pub(crate) fn elabListExp(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExpList: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inProp: DAE::Properties, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = 'mc: {
        ...
        if let Ok((__v, __wb0, __wb1)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                _ => {
                    ...
                    c = __pa0.clone();
                    (outCache, expl, props) = elabExpList(inCache.clone(), inEnv.clone(), inExpList.clone(), inImplicit, inDoVect, inPrefix.clone(), inInfo.clone(), DAE::T_UNKNOWN_DEFAULT().clone())?;
                    types = ({
        let mut __acc: Arc<metamodelica::List<Arc<DAE::Type>>> = metamodelica::nil();
        for mut p in (props.clone()).into_iter().cloned() {
                    let __x = Types::getPropType(p.clone())?;
        ...
        __acc.reverse()
                    ...
                    Ok(((outCache.clone(), Arc::new(DAE::Exp::LIST { valList: expl.clone() }), outProperties.clone()), outCache.clone(), outProperties.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { outCache = __wb0; outProperties = __wb1; break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                _ => {
...
pub fn fromEquationsToAlgAssignments(mut cp: Arc<Absyn::ClassPart>) -> Result<Arc<metamodelica::List<Arc<Absyn::AlgorithmItem>>>> {
    let mut algsOut: Arc<metamodelica::List<Arc<Absyn::AlgorithmItem>>>;
...
fn fromEquationsToAlgAssignmentsWork(mut eqsIn: Arc<metamodelica::List<Arc<Absyn::EquationItem>>>) -> Result<Arc<metamodelica::List<Arc<Absyn::AlgorithmItem>>>> {
    ...
    for mut ei in &*eqsIn {
        ...
        Deref @ Absyn::EquationItem::EQUATIONITEM { equation_: eq, comment, info } => {
...
fn fromEquationBranchesToAlgBranches(mut eqsIn: Arc<metamodelica::List<(Arc<Absyn::Exp>, Arc<metamodelica::List<Arc<Absyn::EquationItem>>>)>>) -> Result<Arc<metamodelica::List<(Arc<Absyn::Exp>, Arc<metamodelica::List<Arc<Absyn::AlgorithmItem>>>)>>> {
    ...
    for mut branch in &*eqsIn {
        ...
        algsOut = metamodelica::cons((e.clone(), algs.clone()), algsOut.clone());
    }
    algsOut = algsOut.reverse();
    Ok(algsOut)
}
...
fn fromEquationToAlgAssignment(mut eq: Arc<Absyn::Equation>, mut comment: Option<Arc<Absyn::Comment>>, mut info: SourceInfo) -> Result<Arc<metamodelica::List<Arc<Absyn::AlgorithmItem>>>> {
    let mut algStatement: Arc<metamodelica::List<Arc<Absyn::AlgorithmItem>>>;
    algStatement = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ Absyn::Equation::EQ_IF { ifExp: e, equationTrueItems: eqTrueItems, elseIfBranches: eqBranches, equationElseItems: eqElseItems } => {
                    let mut res: Arc<Absyn::AlgorithmItem>;
                    let mut algTrueItems: Arc<metamodelica::List<Arc<Absyn::AlgorithmItem>>>;
                    let mut algElseItems: Arc<metamodelica::List<Arc<Absyn::AlgorithmItem>>>;
                    let mut algBranches: Arc<metamodelica::List<(Arc<Absyn::Exp>, Arc<metamodelica::List<Arc<Absyn::AlgorithmItem>>>)>>;
...
fn elabMatrixToMatrixExp(mut inExp: Arc<DAE::Exp>) -> Arc<DAE::Exp> {
    let mut outExp: Arc<DAE::Exp>;
    outExp = 'mc: {
        let __mc_input = inExp.clone();
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Exp::ARRAY { ty: a @ Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: _, tail: Deref @ metamodelica::List::Cons { head: _, tail: Deref @ metamodelica::List::Nil } }, .. }, array: expl, .. } => {
...
fn matrixConstrMaxDim(mut inTypes: Arc<metamodelica::List<Arc<DAE::Type>>>) -> i32 {
    let mut outMaxDim: i32 = 2;
    for mut ty in &*inTypes {
        let mut ty = ty.clone();
        outMaxDim = std::cmp::max(Types::numberOfDimensions(ty.clone()), outMaxDim);
    }
    outMaxDim
}
...
fn elabCallReduction(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inReductionFn: Arc<Absyn::ComponentRef>, mut inReductionExp: Arc<Absyn::Exp>, mut inIterType: Absyn::ReductionIterType, mut inIterators: Arc<metamodelica::List<Arc<Absyn::ForIterator>>>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut reduction_iters: Arc<metamodelica::List<Arc<DAE::ReductionIterator>>>;
    ...
    let mut exp: Arc<DAE::Exp>;
    let mut afold_exp: Option<Arc<Absyn::Exp>>;
    let mut fold_exp: Option<Arc<DAE::Exp>>;
    let mut exp_ty: Arc<DAE::Type>;
    let mut res_ty: Arc<DAE::Type>;
    let mut r#fn: Arc<Absyn::Path>;
    let mut v: Option<Arc<Values::Value>>;
...
fn elabCallReductionIterators(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIterators: Arc<metamodelica::List<Arc<Absyn::ForIterator>>>, mut inReductionExp: Arc<Absyn::Exp>, mut inImpl: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<Arc<DAE::ReductionIterator>>>, Arc<metamodelica::List<Arc<DAE::Dimension>>>, DAE::Const, bool)> {
    ...
    let mut aiter_exp: Arc<Absyn::Exp>;
    let mut oaguard_exp: Option<Arc<Absyn::Exp>>;
    let mut oaiter_exp: Option<Arc<Absyn::Exp>>;
    let mut iter_exp: Arc<DAE::Exp>;
    let mut guard_exp: Option<Arc<DAE::Exp>>;
    let mut full_iter_ty: Arc<DAE::Type>;
    let mut iter_ty: Arc<DAE::Type>;
    let mut iter_const: DAE::Const;
    ...
    for mut iter in &*inIterators {
        ...
        if isSome(oaiter_exp.clone()) {
            ...
            let (__pa4, __pa5, __pa6, __pa7) = ::match_deref::match_deref! { match &(elabExpInExpression(outCache.clone(), inEnv.clone(), aiter_exp.clone(), inImpl, inDoVect, inPrefix.clone(), inInfo.clone())?) {
...
pub(crate) fn deduceIterationRange(mut inIterator: ArcStr, mut inCrefs: Arc<metamodelica::List<(Arc<Absyn::ComponentRef>, i32)>>, mut inEnv: FCore::Graph, mut inCache: FCore::Cache, mut inInfo: SourceInfo) -> Result<(Arc<DAE::Exp>, DAE::Properties, FCore::Cache)> {
    ...
    if inCrefs.clone().is_empty() {
        Error::addSourceMessageAndFail(Error::IMPLICIT_ITERATOR_NOT_FOUND_IN_LOOP_BODY.clone(), list![(inIterator).clone()], inInfo.clone())?;
    ...
    for mut cr in &*inCrefs.clone() {
        ...
        if idx <= (dims.clone().len() as i32) {
            dim = (dims.clone()).get(idx)?;
            (range, outProperties) = deduceReductionIterationRange2(dim.clone(), cref.clone(), ty.clone(), idx)?;
        } else {
    ...
    for mut r in &*ranges {
        ...
        if !(ExpressionBasics::expEqual(r.clone(), outRange.clone())?) {
            ...
            (acref, i2) = (inCrefs.clone()).get(idx)?;
            cr_str2 = (Dump::printComponentRefStr(acref.clone())?).clone();
            Error::addSourceMessageAndFail(Error::INCOMPATIBLE_IMPLICIT_RANGES.clone(), list![(intString(i2)).clone(), (cr_str2.clone()).clone(), (intString(i1)).clone(), (cr_str1.clone()).clone()], inInfo.clone())?;
            unreachable!("Error.addSourceMessageAndFail always fails — caller-side flow-analysis hint");
        }
        idx = idx + 1;
    }
...
fn iteratorIndexedCrefsEqual(mut inCref1: (Arc<Absyn::ComponentRef>, i32), mut inCref2: (Arc<Absyn::ComponentRef>, i32)) -> Result<bool> {
...
fn deduceReductionIterationRange_traverser(mut inExp: Arc<Absyn::Exp>, mut inCrefs: Arc<metamodelica::List<(Arc<Absyn::ComponentRef>, i32)>>, mut inIterator: ArcStr) -> (Arc<Absyn::Exp>, Arc<metamodelica::List<(Arc<Absyn::ComponentRef>, i32)>>) {
    ...
    outCrefs = (::match_deref::match_deref! { match &(inExp) {
        Deref @ Absyn::Exp::CREF { componentRef: cref } => {
            getIteratorIndexedCrefs(cref.clone(), (inIterator).clone(), inCrefs)
        },
        _ => {
            inCrefs
        },
    ...
    outCrefs = (::match_deref::match_deref! { match &(inCref) {
        ...
        Deref @ Absyn::Subscript::SUBSCRIPT { subscript: Deref @ Absyn::Exp::CREF { componentRef: Deref @ Absyn::ComponentRef::CREF_IDENT { name: __esc_name, subscripts: Deref @ metamodelica::List::Nil } } } => {
            ...
            if name.clone() == inIterator.clone() {
                outCrefs = metamodelica::cons((Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (id.clone()).clone(), subscripts: metamodelica::nil() }), idx), outCrefs.clone());
            }
            ()
        },
        _ => (),
        ...
        Deref @ Absyn::ComponentRef::CREF_QUAL { name: id, subscripts: subs, componentRef: cref } => {
            ...
            for mut cr in &*crefs {
                let mut cr = cr.clone();
                (cref, idx) = cr.clone();
                outCrefs = metamodelica::cons((Arc::new(Absyn::ComponentRef::CREF_QUAL { name: (id.clone()).clone(), subscripts: subs.clone(), componentRef: cref.clone() }), idx), outCrefs.clone());
            }
            getIteratorIndexedCrefs(Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (id.clone()).clone(), subscripts: subs.clone() }), (inIterator).clone(), outCrefs)
        },
        Deref @ Absyn::ComponentRef::CREF_FULLYQUALIFIED { componentRef: cref } => {
            let mut idx: i32;
            let mut cref = (*cref).clone();
            crefs = getIteratorIndexedCrefs(cref.clone(), (inIterator).clone(), metamodelica::nil());
            for mut cr in &*crefs {
                let mut cr = cr.clone();
                (cref, idx) = cr.clone();
                outCrefs = metamodelica::cons((Arc::new(Absyn::ComponentRef::CREF_FULLYQUALIFIED { componentRef: cref.clone() }), idx), outCrefs.clone());
            }
            outCrefs
        },
        _ => {
...
fn deduceReductionIterationRange2(mut inDimension: Arc<DAE::Dimension>, mut inCref: Arc<DAE::ComponentRef>, mut inType: Arc<DAE::Type>, mut inIndex: i32) -> Result<(Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut enum_start: Arc<Absyn::Path> = Arc::new(<Absyn::Path as ::std::default::Default>::default());
    let mut enum_end: Arc<Absyn::Path> = Arc::new(<Absyn::Path as ::std::default::Default>::default());
    let mut enum_lits: Arc<metamodelica::List<ArcStr>> = metamodelica::nil();
    let mut sz: i32 = 0;
    let mut size_exp: Arc<DAE::Exp> = Arc::new(<DAE::Exp as ::std::default::Default>::default());
    outRange = (::match_deref::match_deref! { match &(inDimension.clone()) {
        Deref @ DAE::Dimension::DIM_BOOLEAN { .. } => {
            range_ty = Arc::new(DAE::Type::T_ARRAY { ty: DAE::T_BOOL_DEFAULT().clone(), dims: list![inDimension] });
        ...
        Deref @ DAE::Dimension::DIM_ENUM { enumTypeName: __esc_enum_path, literals: __esc_enum_lits, .. } => {
            ...
            range_ty = Arc::new(DAE::Type::T_ARRAY { ty: range_ty, dims: list![inDimension] });
            range_const = openmodelica_frontend_types::DAE::Const::C_CONST;
            Arc::new(DAE::Exp::RANGE { ty: range_ty.clone(), start: Arc::new(DAE::Exp::ENUM_LITERAL { name: enum_start, index: 1 }), step: None, stop: Arc::new(DAE::Exp::ENUM_LITERAL { name: enum_end, index: (enum_lits.clone().len() as i32) }) })
        },
        Deref @ DAE::Dimension::DIM_INTEGER { integer: __esc_sz } => {
            sz = (*__esc_sz).clone();
            range_ty = Arc::new(DAE::Type::T_ARRAY { ty: DAE::T_INTEGER_DEFAULT().clone(), dims: list![inDimension] });
            range_const = openmodelica_frontend_types::DAE::Const::C_CONST;
        ...
        _ => {
            size_exp = Arc::new(DAE::Exp::SIZE { exp: Arc::new(DAE::Exp::CREF { componentRef: inCref, ty: inType }), sz: Some(Arc::new(DAE::Exp::ICONST { integer: inIndex })) });
            range_ty = Arc::new(DAE::Type::T_ARRAY { ty: DAE::T_INTEGER_DEFAULT().clone(), dims: list![inDimension] });
            range_const = openmodelica_frontend_types::DAE::Const::C_PARAM;
            Arc::new(DAE::Exp::RANGE { ty: range_ty.clone(), start: Arc::new(DAE::Exp::ICONST { integer: 1 }), step: None, stop: size_exp })
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    outProperties = DAE::Properties::PROP { type_: range_ty, constFlag: range_const };
...
fn makeReductionFoldExp(mut inEnv: FCore::Graph, mut path: Arc<Absyn::Path>, mut expty: Arc<DAE::Type>, mut resultTy: Arc<DAE::Type>, mut foldId: ArcStr, mut resultId: ArcStr) -> Result<(FCore::Graph, Option<Arc<Absyn::Exp>>)> {
    ...
    let mut afoldExp: Option<Arc<Absyn::Exp>>;
    (outEnv, afoldExp) = (::match_deref::match_deref! { match &(AbsynUtil::makeNotFullyQualified(path.clone())) {
        ...
        Deref @ Absyn::Path::IDENT { name: Deref @ "sum" } => {
            ...
            let mut cr2: Arc<Absyn::ComponentRef>;
            let mut env: FCore::Graph;
            env = FGraph::addForIterator(inEnv, (foldId.clone()).clone(), expty.clone(), openmodelica_frontend_types::DAE::Binding::interned_UNBOUND(), openmodelica_frontend_types::SCode::Variability::VAR, Some(openmodelica_frontend_types::DAE::Const::C_VAR))?;
            env = FGraph::addForIterator(env, (resultId.clone()).clone(), expty, openmodelica_frontend_types::DAE::Binding::interned_UNBOUND(), openmodelica_frontend_types::SCode::Variability::VAR, Some(openmodelica_frontend_types::DAE::Const::C_VAR))?;
            cr1 = Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (foldId).clone(), subscripts: metamodelica::nil() });
            cr2 = Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (resultId).clone(), subscripts: metamodelica::nil() });
            exp = Arc::new(Absyn::Exp::BINARY { exp1: Arc::new(Absyn::Exp::CREF { componentRef: cr2 }), op: openmodelica_ast::Absyn::Operator::ADD, exp2: Arc::new(Absyn::Exp::CREF { componentRef: cr1 }) });
            (env, Some(exp))
        },
        Deref @ Absyn::Path::IDENT { name: Deref @ "product" } => {
            ...
            cr1 = Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (foldId).clone(), subscripts: metamodelica::nil() });
            cr2 = Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (resultId).clone(), subscripts: metamodelica::nil() });
            exp = Arc::new(Absyn::Exp::BINARY { exp1: Arc::new(Absyn::Exp::CREF { componentRef: cr2 }), op: openmodelica_ast::Absyn::Operator::MUL, exp2: Arc::new(Absyn::Exp::CREF { componentRef: cr1 }) });
            (env, Some(exp))
        },
        _ => {
            let mut exp: Arc<Absyn::Exp>;
            ...
            env = FGraph::addForIterator(inEnv, (foldId.clone()).clone(), expty, openmodelica_frontend_types::DAE::Binding::interned_UNBOUND(), openmodelica_frontend_types::SCode::Variability::VAR, Some(openmodelica_frontend_types::DAE::Const::C_VAR))?;
            env = FGraph::addForIterator(env, (resultId.clone()).clone(), resultTy, openmodelica_frontend_types::DAE::Binding::interned_UNBOUND(), openmodelica_frontend_types::SCode::Variability::VAR, Some(openmodelica_frontend_types::DAE::Const::C_VAR))?;
            cr1 = Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (foldId).clone(), subscripts: metamodelica::nil() });
            cr2 = Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (resultId).clone(), subscripts: metamodelica::nil() });
            exp = Arc::new(Absyn::Exp::CALL { function_: cr, functionArgs: Arc::new(Absyn::FunctionArgs::FUNCTIONARGS { args: list![Arc::new(Absyn::Exp::CREF { componentRef: cr1 }), Arc::new(Absyn::Exp::CREF { componentRef: cr2 })], argNames: metamodelica::nil() }), typeVars: metamodelica::nil() });
            (env, Some(exp))
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn reductionType(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inFn: Arc<Absyn::Path>, mut inExp: Arc<DAE::Exp>, mut inType: Arc<DAE::Type>, mut unboxedType: Arc<DAE::Type>, mut dims: Arc<metamodelica::List<Arc<DAE::Dimension>>>, mut hasGuardExp: bool, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, Arc<DAE::Type>, Arc<DAE::Type>, Option<Arc<Values::Value>>, Arc<Absyn::Path>)> {
    ...
    let mut resultType: Arc<DAE::Type>;
    let mut defaultValue: Option<Arc<Values::Value>>;
    ...
    (outExp, outType, resultType, defaultValue, outPath) = (::match_deref::match_deref! { match &((r#fn.clone(), unboxedType.clone())) {
        (Deref @ Absyn::Path::IDENT { name: Deref @ "array" }, _) => {
            let mut ty: Arc<DAE::Type>;
            ty = List::foldr(dims, (std::sync::Arc::new(fnptr!(Types::liftArray, Arc<DAE::Type>, Arc<DAE::Dimension>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Type>, Arc<DAE::Dimension>) -> Result<Arc<DAE::Type>> + 'static>), inType)?;
            (inExp, ty.clone(), ty, Some(Arc::new(Values::Value::ARRAY { valueLst: metamodelica::nil(), dimLst: list![0] })), r#fn)
        },
        (Deref @ Absyn::Path::IDENT { name: Deref @ "$array" }, _) => {
            let mut ty: Arc<DAE::Type>;
            ty = List::foldr(dims, (std::sync::Arc::new(fnptr!(Types::liftArray, Arc<DAE::Type>, Arc<DAE::Dimension>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Type>, Arc<DAE::Dimension>) -> Result<Arc<DAE::Type>> + 'static>), inType)?;
        ...
        (Deref @ Absyn::Path::IDENT { name: Deref @ "list" }, _) => {
            ...
            ty = List::foldr(dims, (std::sync::Arc::new(fnptr!(Types::liftList, Arc<DAE::Type>, Arc<DAE::Dimension>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Type>, Arc<DAE::Dimension>) -> Result<Arc<DAE::Type>> + 'static>), ty)?;
            (exp, ty.clone(), ty, Some(Arc::new(Values::Value::LIST { valueLst: metamodelica::nil() })), r#fn)
        },
        ...
        (Deref @ Absyn::Path::IDENT { name: Deref @ "product" }, Deref @ DAE::Type::T_STRING { .. }) => {
            Error::addSourceMessage(Error::INTERNAL_ERROR.clone(), list![(literal!("product reduction not defined for String")).clone()], info)?;
            bail!("fail")
        },
        ...
        _ => {
            let mut fnTypes: Arc<metamodelica::List<Arc<DAE::Type>>>;
            let mut ty: Arc<DAE::Type>;
            let mut ty2: Arc<DAE::Type>;
            let mut typeA: Arc<DAE::Type>;
            let mut typeB: Arc<DAE::Type>;
            let mut resType: Arc<DAE::Type>;
            let mut path: Arc<Absyn::Path>;
            let mut exp: Arc<DAE::Exp>;
            let mut bindings: Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>;
            let mut defaultBinding: Option<Arc<Values::Value>>;
            (outCache, fnTypes) = Lookup::lookupFunctionsInEnv(inCache, inEnv.clone(), inFn.clone(), info.clone());
            (typeA, typeB, resType, defaultBinding, path) = checkReductionType1(inEnv.clone(), inFn, fnTypes, info.clone())?;
            ty2 = if (isSome(defaultBinding.clone())) {typeB.clone()} else {inType.clone()};
            (exp, typeA, bindings) = Types::matchTypePolymorphicWithError(inExp, inType.clone(), typeA, Some(path.clone()), metamodelica::nil(), info.clone())?;
            (_, typeB, bindings) = Types::matchTypePolymorphicWithError(Arc::new(DAE::Exp::CREF { componentRef: Arc::new(DAE::ComponentRef::CREF_IDENT { ident: (literal!("$result")).clone(), identType: DAE::T_ANYTYPE_DEFAULT().clone(), subscriptLst: metamodelica::nil() }), ty: DAE::T_ANYTYPE_DEFAULT().clone() }), ty2, typeB, Some(path.clone()), bindings, info.clone())?;
            bindings = Types::solvePolymorphicBindings(bindings, info.clone(), path.clone())?;
            typeA = Types::fixPolymorphicRestype(typeA, bindings.clone(), info.clone())?;
            typeB = Types::fixPolymorphicRestype(typeB, bindings.clone(), info.clone())?;
            resType = Types::fixPolymorphicRestype(resType, bindings, info.clone())?;
            (exp, ty) = checkReductionType2(exp, inType, typeA.clone(), typeB.clone(), resType.clone(), Types::equivtypes(typeA, typeB.clone()) || isSome(defaultBinding.clone()), Types::equivtypes(typeB.clone(), resType), info.clone())?;
            let (__pa0, Util::SUCCESS { .. }) = (instantiateDaeFunction(outCache, inEnv, path.clone(), false, None, true)) else { bail!("pattern mismatch") };
            outCache = __pa0.clone();
            Error::assertionOrAddSourceMessage(Config::acceptMetaModelicaGrammar()? || Flags::isSet(Flags::EXPERIMENTAL_REDUCTIONS.clone())?, Error::COMPILER_NOTIFICATION.clone(), list![(literal!("Custom reduction functions are an OpenModelica extension to the Modelica Specification. Do not use them if you need your model to compile using other tools or if you are concerned about using experimental features. Use -d=experimentalReductions to disable this message.")).clone()], info)?;
...
fn checkReductionType1(mut inEnv: FCore::Graph, mut inPath: Arc<Absyn::Path>, mut fnTypes: Arc<metamodelica::List<Arc<DAE::Type>>>, mut info: SourceInfo) -> Result<(Arc<DAE::Type>, Arc<DAE::Type>, Arc<DAE::Type>, Option<Arc<Values::Value>>, Arc<Absyn::Path>)> {
    ...
    let mut startValue: Option<Arc<Values::Value>>;
    let mut outPath: Arc<Absyn::Path>;
    (typeA, typeB, resType, startValue, outPath) = (::match_deref::match_deref! { match &(fnTypes.clone()) {
        Deref @ metamodelica::List::Nil => {
            let mut str1: ArcStr;
            let mut str2: ArcStr;
            str1 = (AbsynUtil::pathString(inPath, (literal!(".")).clone(), true, false)?).clone();
            str2 = (FGraph::printGraphPathStr(inEnv)).clone();
            Error::addSourceMessage(Error::LOOKUP_FUNCTION_ERROR.clone(), list![(str1).clone(), (str2).clone()], info)?;
            bail!("fail")
        },
        Deref @ metamodelica::List::Cons { head: Deref @ DAE::Type::T_FUNCTION { funcArg: Deref @ metamodelica::List::Cons { head: Deref @ DAE::FuncArg { ty: __esc_typeA, r#const: DAE::Const::C_VAR { .. }, .. }, tail: Deref @ metamodelica::List::Cons { head: Deref @ DAE::FuncArg { ty: __esc_typeB, r#const: DAE::Const::C_VAR { .. }, defaultBinding: Some(e), .. }, tail: Deref @ metamodelica::List::Nil } }, funcResultType: __esc_resType, path, .. }, tail: Deref @ metamodelica::List::Nil } => {
        ...
        Deref @ metamodelica::List::Cons { head: Deref @ DAE::Type::T_FUNCTION { funcArg: Deref @ metamodelica::List::Cons { head: Deref @ DAE::FuncArg { ty: __esc_typeA, r#const: DAE::Const::C_VAR { .. }, .. }, tail: Deref @ metamodelica::List::Cons { head: Deref @ DAE::FuncArg { ty: __esc_typeB, r#const: DAE::Const::C_VAR { .. }, defaultBinding: None, .. }, tail: Deref @ metamodelica::List::Nil } }, funcResultType: __esc_resType, path, .. }, tail: Deref @ metamodelica::List::Nil } => {
            typeA = (*__esc_typeA).clone();
            typeB = (*__esc_typeB).clone();
            resType = (*__esc_resType).clone();
        ...
        _ => {
            let mut str1: ArcStr;
            str1 = stringDelimitList(List::map(fnTypes, (std::sync::Arc::new(TypesDump::unparseType) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Type>) -> Result<ArcStr> + 'static>))?, (literal!(",")).clone());
            Error::addSourceMessage(Error::UNSUPPORTED_REDUCTION_TYPE.clone(), list![(str1).clone()], info)?;
            bail!("fail")
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn checkReductionType2(mut inExp: Arc<DAE::Exp>, mut expType: Arc<DAE::Type>, mut typeA: Arc<DAE::Type>, mut typeB: Arc<DAE::Type>, mut typeC: Arc<DAE::Type>, mut equivAB: bool, mut equivBC: bool, mut info: SourceInfo) -> Result<(Arc<DAE::Exp>, Arc<DAE::Type>)> {
    ...
    (outExp, outTy) = (match (equivAB, equivBC) {
        ...
        (_, false) => {
            let mut str1: ArcStr;
            let mut str2: ArcStr;
            str1 = (TypesDump::unparseType(typeB)?).clone();
            str2 = (TypesDump::unparseType(typeC)?).clone();
            Error::addSourceMessage(Error::REDUCTION_TYPE_ERROR.clone(), list![(literal!("second argument")).clone(), (literal!("result-type")).clone(), (literal!("identical")).clone(), (str1).clone(), (str2).clone()], info)?;
            bail!("fail")
        },
        (false, true) => {
            let mut str1: ArcStr;
            let mut str2: ArcStr;
            str1 = (TypesDump::unparseType(typeA)?).clone();
            str2 = (TypesDump::unparseType(typeB)?).clone();
            Error::addSourceMessage(Error::REDUCTION_TYPE_ERROR.clone(), list![(literal!("first")).clone(), (literal!("second arguments")).clone(), (literal!("identical")).clone(), (str1).clone(), (str2).clone()], info)?;
            bail!("fail")
        },
        (true, true) => {
            let mut str1: ArcStr;
            let mut str2: ArcStr;
            str1 = (TypesDump::unparseType(expType)?).clone();
            str2 = (TypesDump::unparseType(typeA)?).clone();
            Error::addSourceMessage(Error::REDUCTION_TYPE_ERROR.clone(), list![(literal!("reduction expression")).clone(), (literal!("first argument")).clone(), (literal!("compatible")).clone(), (str1).clone(), (str2).clone()], info)?;
            bail!("fail")
        },
        _ => bail!("match: no arm matched"),
    });
...
fn constructArrayType(mut arrayType: Arc<DAE::Type>, mut expType: Arc<DAE::Type>) -> Result<Arc<DAE::Type>> {
    ...
    resType = (::match_deref::match_deref! { match &(arrayType) {
        ...
        Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: dim, tail: Deref @ metamodelica::List::Nil }, ty } => {
            let mut ty = (*ty).clone();
            ty = constructArrayType(ty.clone(), expType)?;
...
fn elabCodeType(mut inCode: Arc<Absyn::CodeNode>) -> Result<Arc<DAE::Type>> {
    let mut outType: Arc<DAE::Type>;
    outType = (::match_deref::match_deref! { match &(inCode) {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ Absyn::Exp::RANGE { start, step: None, stop }, r#impl, pre) => {
                    ...
                    let mut t: Arc<DAE::Type>;
                    let mut start_t: Arc<DAE::Type>;
                    let mut stop_t: Arc<DAE::Type>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ Absyn::Exp::MATRIX { matrix: ess }, r#impl, pre) => {
                    ...
                    let mut t_2: Arc<DAE::Type>;
                    let mut c: DAE::Const;
                    let mut tps_2: Arc<metamodelica::List<Arc<DAE::Type>>>;
                    let mut tps: Arc<metamodelica::List<Arc<metamodelica::List<DAE::Properties>>>>;
                    let mut tps_1: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Type>>>>>;
                    let mut dess: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Exp>>>>>;
                    let mut cache = (*cache).clone();
...
fn deoverloadRange(mut inStartExp: Arc<DAE::Exp>, mut inStartType: Arc<DAE::Type>, mut inStepExp: Option<Arc<DAE::Exp>>, mut inStepType: Option<Arc<DAE::Type>>, mut inStopExp: Arc<DAE::Exp>, mut inStopType: Arc<DAE::Type>, mut inInfo: SourceInfo) -> Result<(Arc<DAE::Exp>, Option<Arc<DAE::Exp>>, Arc<DAE::Exp>, Arc<DAE::Type>)> {
    let mut outStart: Arc<DAE::Exp> = Arc::new(<DAE::Exp as ::std::default::Default>::default());
    let mut outStep: Option<Arc<DAE::Exp>>;
    let mut outStop: Arc<DAE::Exp> = Arc::new(<DAE::Exp as ::std::default::Default>::default());
    let mut outRangeType: Arc<DAE::Type>;
...
fn elabRangeType(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inStart: Arc<DAE::Exp>, mut inStep: Option<Arc<DAE::Exp>>, mut inStop: Arc<DAE::Exp>, mut inType: Arc<DAE::Type>, mut inExpType: Arc<DAE::Type>, mut co: DAE::Const, mut inImpl: bool) -> (FCore::Cache, Arc<DAE::Type>) {
    let mut outCache: FCore::Cache;
    let mut outType: Arc<DAE::Type>;
    (outCache, outType) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, DAE::Const::C_VAR { .. }) => {
                    Ok((inCache.clone(), Arc::new(DAE::Type::T_ARRAY { ty: inType.clone(), dims: list![openmodelica_frontend_types::DAE::Dimension::interned_DIM_UNKNOWN()] })))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (None, _) => {
                    let mut start_val: Arc<Values::Value>;
                    let mut stop_val: Arc<Values::Value>;
                    let mut dim: i32;
                    let mut cache: FCore::Cache;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Some(step_exp), _) => {
                    let mut start_val: Arc<Values::Value>;
                    let mut step_val: Arc<Values::Value>;
                    let mut stop_val: Arc<Values::Value>;
                    let mut dim: i32;
                    let mut cache: FCore::Cache;
                    (cache, start_val) = Ceval::ceval(inCache.clone(), inEnv.clone(), inStart.clone(), inImpl, Absyn::Msg::MSG { info: Absyn::dummyInfo.clone() }, 0)?;
...
fn elabRangeSize(mut inStartValue: Arc<Values::Value>, mut inStepValue: Option<Arc<Values::Value>>, mut inStopValue: Arc<Values::Value>) -> Result<i32> {
    let mut outSize: i32;
    outSize = 'mc: {
        let __mc_input = (inStartValue.clone(), inStepValue, inStopValue.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, None, _) => {
...
fn elabTuple(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExpl: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo, mut isLhs: bool) -> Result<(FCore::Cache, Arc<metamodelica::List<Arc<DAE::Exp>>>, Arc<metamodelica::List<DAE::Properties>>)> {
    ...
    let mut exp: Arc<DAE::Exp>;
    let mut prop: DAE::Properties;
    if if (!(isLhs)) {!(Config::acceptMetaModelicaGrammar()?)} else {false} {
    ...
    for mut e in &*inExpl {
        let mut e = e.clone();
        (outCache, exp, prop) = elabExp(outCache.clone(), inEnv.clone(), e.clone(), inImplicit, inDoVect, inPrefix.clone(), inInfo.clone())?;
...
fn stripExtraArgsFromType2(mut inSlots: Arc<metamodelica::List<Slot>>, mut inType: Arc<metamodelica::List<Arc<DAE::FuncArg>>>, mut inAccumType: Arc<metamodelica::List<Arc<DAE::FuncArg>>>) -> Result<Arc<metamodelica::List<Arc<DAE::FuncArg>>>> {
    '__tco: loop {
        ...
        (Deref @ metamodelica::List::Cons { head: Slot { slotFilled: true, .. }, tail: slotsRest }, Deref @ metamodelica::List::Cons { head: _, tail: rest }) => {
            { (inSlots, inType, inAccumType) = (slotsRest.clone(), rest.clone(), inAccumType); continue '__tco; }
        },
        (Deref @ metamodelica::List::Cons { head: Slot { slotFilled: false, .. }, tail: slotsRest }, Deref @ metamodelica::List::Cons { head: arg, tail: rest }) => {
            { (inSlots, inType, inAccumType) = (slotsRest.clone(), rest.clone(), metamodelica::cons(arg.clone(), inAccumType)); continue '__tco; }
        },
...
fn elabArray(mut inExpl: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut inProps: Arc<metamodelica::List<DAE::Properties>>, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(Arc<metamodelica::List<Arc<DAE::Exp>>>, DAE::Properties)> {
    ...
    for mut p in &*inProps {
        let mut p = p.clone();
        let DAE::PROP { type_: __pa0, constFlag: __pa1 } = (p.clone()) else { bail!("pattern mismatch") };
        ty = __pa0.clone();
        c2 = __pa1.clone();
...
fn elabArrayHasMixedIntReals(mut inTypes: Arc<metamodelica::List<Arc<DAE::Type>>>) -> Result<(Arc<DAE::Type>, bool)> {
    ...
    let mut ty: Arc<DAE::Type>;
    let mut rest_tys: Arc<metamodelica::List<Arc<DAE::Type>>>;
...
fn elabArray2(mut inExpl: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut inTypes: Arc<metamodelica::List<Arc<DAE::Type>>>, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(Arc<metamodelica::List<Arc<DAE::Exp>>>, Arc<DAE::Type>)> {
    ...
    let mut rest_tys: Arc<metamodelica::List<Arc<DAE::Type>>>;
    let mut exp1: Arc<DAE::Exp>;
    let mut rest_expl: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    ...
    for mut exp2 in &*rest_expl {
        ...
        outExpl = metamodelica::cons(exp2.clone(), outExpl.clone());
    }
    outExpl = outExpl.reverse();
    Ok((outExpl, outType))
}
...
fn elabGraphicsArray(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExpl: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<metamodelica::List<Arc<DAE::Exp>>>, DAE::Properties)> {
    ...
    for mut e in &*inExpl {
        let mut e = e.clone();
        let (__pa0, __pa1, __pa2, __pa3) = ::match_deref::match_deref! { match &(elabGraphicsExp(outCache.clone(), inEnv.clone(), e.clone(), inImplicit, inPrefix.clone(), inInfo.clone())?) {
            (__pa0, __pa1, DAE::Properties::PROP { type_: __pa2, constFlag: __pa3 }) => (__pa0.clone(), __pa1.clone(), __pa2.clone(), __pa3.clone()),
            _ => bail!("pattern mismatch"),
        } };
        outCache = __pa0.clone();
...
fn elabMatrixComma(mut inExpl: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut inProps: Arc<metamodelica::List<DAE::Properties>>, mut inHaveReal: bool, mut inDims: i32, mut inInfo: SourceInfo) -> Result<(Arc<DAE::Exp>, DAE::Properties, Arc<DAE::Dimension>, Arc<DAE::Dimension>)> {
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    let mut outDim1: Arc<DAE::Dimension>;
    let mut outDim2: Arc<DAE::Dimension>;
    let mut exp: Arc<DAE::Exp>;
    let mut rest_expl: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    let mut accum_expl: Arc<metamodelica::List<Arc<DAE::Exp>>> = metamodelica::nil();
    let mut prop: DAE::Properties;
    let mut rest_props: Arc<metamodelica::List<DAE::Properties>>;
    let mut ty: Arc<DAE::Type>;
    let mut sty: Arc<DAE::Type>;
    let mut dim1: Arc<DAE::Dimension>;
    let mut dim2: Arc<DAE::Dimension>;
    match '__try0: {
        ...
        while !(rest_expl.clone().is_empty()) {
            ...
            outProperties = unwrap_break_err!(Types::matchWithPromote(prop.clone(), outProperties.clone(), inHaveReal), '__try0);
        }
...
fn elabMatrixCatTwo(mut inExpl: Arc<metamodelica::List<Arc<DAE::Exp>>>) -> Result<Arc<DAE::Exp>> {
    ...
    match '__try0: {
        ...
        Err(_) => {
            ...
            outExp = Expression::makePureBuiltinCall((literal!("cat")).clone(), metamodelica::cons(Arc::new(DAE::Exp::ICONST { integer: 2 }), inExpl.clone()), ty.clone());
        }
    }
    Ok(outExp)
}
...
fn elabMatrixCatTwo2(mut inExp1: Arc<DAE::Exp>, mut inExp2: Arc<DAE::Exp>) -> Result<Arc<DAE::Exp>> {
    ...
    ty = Expression::liftArrayLeft(ty, Arc::new(DAE::Dimension::DIM_INTEGER { integer: (expl1.clone().len() as i32) }));
    outExp = Arc::new(DAE::Exp::ARRAY { ty: ty, scalar: sc, array: expl1 });
    Ok(outExp)
}
...
fn elabMatrixCatTwo3(mut inExp1: Arc<DAE::Exp>, mut inExp2: Arc<DAE::Exp>) -> Result<Arc<DAE::Exp>> {
    ...
    outExp = Arc::new(DAE::Exp::ARRAY { ty: ty1, scalar: sc, array: expl2 });
    Ok(outExp)
}
...
fn elabMatrixCatOne(mut inExpl: Arc<metamodelica::List<Arc<DAE::Exp>>>) -> Result<Arc<DAE::Exp>> {
    ...
    match '__try0: {
        ...
        Err(_) => {
            ty = Expression::r#typeof(listHead(inExpl.clone())?)?;
            outExp = Expression::makePureBuiltinCall((literal!("cat")).clone(), metamodelica::cons(Arc::new(DAE::Exp::ICONST { integer: 1 }), inExpl.clone()), ty.clone());
        }
    }
    Ok(outExp)
}
...
fn elabMatrixCatOne2(mut inArray1: Arc<DAE::Exp>, mut inArray2: Arc<DAE::Exp>) -> Result<Arc<DAE::Exp>> {
    ...
    let mut dim1: Arc<DAE::Dimension>;
    let mut dim2: Arc<DAE::Dimension>;
    let mut dim_rest: Arc<metamodelica::List<Arc<DAE::Dimension>>>;
    let mut expl: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    let mut expl1: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    let mut expl2: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    ...
    let (__pa6, __pa7) = ::match_deref::match_deref! { match &(inArray2) {
        Deref @ DAE::Exp::ARRAY { ty: Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: __pa6, tail: _ }, .. }, array: __pa7, .. } => (__pa6.clone(), __pa7.clone()),
        _ => bail!("pattern mismatch"),
    } };
    dim2 = __pa6.clone();
    expl2 = __pa7.clone();
    ...
    outExp = Arc::new(DAE::Exp::ARRAY { ty: Arc::new(DAE::Type::T_ARRAY { ty: ety, dims: metamodelica::cons(dim, dim_rest) }), scalar: at, array: expl });
    Ok(outExp)
}
...
fn elabMatrixSemi(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inMatrix: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Exp>>>>>, mut inProperties: Arc<metamodelica::List<Arc<metamodelica::List<DAE::Properties>>>>, mut inImpl: bool, mut inHaveReal: bool, mut inDims: i32, mut inDoVectorization: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties, Arc<DAE::Dimension>, Arc<DAE::Dimension>)> {
    ...
    let mut outDim2: Arc<DAE::Dimension>;
    let mut expl: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    let mut rest_expl: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Exp>>>>>;
    let mut props: Arc<metamodelica::List<DAE::Properties>>;
    let mut rest_props: Arc<metamodelica::List<Arc<metamodelica::List<DAE::Properties>>>>;
    let mut exp: Arc<DAE::Exp>;
    let mut prop: DAE::Properties;
    let mut dim1: Arc<DAE::Dimension>;
    let mut dim2: Arc<DAE::Dimension>;
    ...
    (outExp, outProperties, outDim1, outDim2) = elabMatrixComma(expl.clone(), props.clone(), inHaveReal, inDims, inInfo.clone())?;
    ...
    while !(rest_expl.clone().is_empty()) {
        ...
        (exp, prop, dim1, dim2) = elabMatrixComma(expl.clone(), props.clone(), inHaveReal, inDims, inInfo.clone())?;
        ...
        if let Ok(__iflet8) = Types::matchWithPromote(outProperties.clone(), prop.clone(), inHaveReal) {
...
fn verifyBuiltInHandlerType(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExpl: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inImplicit: bool, mut inTypeChecker: Arc<dyn ::std::ops::Fn(Arc<DAE::Type>) -> Result<bool> + 'static>, mut inFnName: ArcStr, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    e = __pa0.clone();
    (outCache, _, outProperties) = elabExpInExpression(inCache, inEnv.clone(), e.clone(), inImplicit, true, inPrefix.clone(), inInfo.clone())?;
    ty = Types::getPropType(outProperties)?;
    ty = Types::arrayElementType(ty);
    let true = (inTypeChecker(ty)?) else { bail!("pattern mismatch") };
    let (__pa2, __pa3, __pa4) = ::match_deref::match_deref! { match &(elabCallArgs(outCache, inEnv, Arc::new(Absyn::Path::FULLYQUALIFIED { path: Arc::new(Absyn::Path::IDENT { name: (inFnName).clone() }) }), list![e], metamodelica::nil(), metamodelica::nil(), inImplicit, inPrefix, inInfo)?) {
...
fn elabBuiltinCardinality(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut ty: Arc<DAE::Type>;
    let mut e: Arc<Absyn::Exp>;
    checkBuiltinCallArgs(inPosArgs.clone(), inNamedArgs, 1, (literal!("cardinality")).clone(), inInfo.clone())?;
    ...
    let DAE::PROP { type_: __pa2, .. } = (outProperties) else { bail!("pattern mismatch") };
    ty = __pa2.clone();
    ty = Types::liftArrayListDims(DAE::T_INTEGER_DEFAULT().clone(), TypesDump::getDimensions(ty));
    outExp = Expression::makePureBuiltinCall((literal!("cardinality")).clone(), list![outExp], ty.clone());
    outProperties = DAE::Properties::PROP { type_: ty, constFlag: openmodelica_frontend_types::DAE::Const::C_CONST };
    Ok((outCache, outExp, outProperties))
}
...
fn elabBuiltinSmooth(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut p: Arc<Absyn::Exp>;
    let mut expr: Arc<Absyn::Exp>;
    ...
    if (inPosArgs.clone().len() as i32) != 2 || !(inNamedArgs.clone().is_empty()) {
        ...
        printBuiltinFnArgError((literal!("smooth")).clone(), (msg_str).clone(), inPosArgs.clone(), inNamedArgs.clone(), inPrefix.clone(), inInfo.clone())?;
    }
    ...
    if !(Types::isReal(ty.clone()) || Types::isRecordWithOnlyReals(ty.clone())?) {
        ...
        printBuiltinFnArgError((literal!("smooth")).clone(), (msg_str).clone(), inPosArgs, inNamedArgs, inPrefix, inInfo)?;
    }
    ty = Types::simplifyType(ty)?;
    outExp = Expression::makePureBuiltinCall((literal!("smooth")).clone(), list![dp, dexpr], ty);
    Ok((outCache, outExp, outProperties))
}
...
fn printBuiltinFnArgError(mut inFnName: ArcStr, mut inMsg: ArcStr, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<()> {
    ...
    let mut pre_str: ArcStr;
    let mut msg_str: ArcStr;
    let mut pos_args: Arc<metamodelica::List<ArcStr>>;
    let mut named_args: Arc<metamodelica::List<ArcStr>>;
    pos_args = ({
    ...
    args_str = stringDelimitList(listAppend(pos_args, named_args), (literal!(", ")).clone());
    pre_str = (PrefixUtil::printPrefixStr3(inPrefix)?).clone();
    msg_str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*inFnName); __mm_s.push_str(&*literal!("(")); __mm_s.push_str(&*args_str); __mm_s.push_str(&*literal!(")")); __mm_s.push_str(&*inMsg); ArcStr::from(__mm_s) }).clone();
    Error::addSourceMessageAndFail(Error::WRONG_TYPE_OR_NO_OF_ARGS.clone(), list![(msg_str).clone(), (pre_str).clone()], inInfo)?;
    unreachable!("Error.addSourceMessageAndFail always fails — caller-side flow-analysis hint");
    Ok(())
}
...
fn elabBuiltinSize(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inAbsynExpLst: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArg: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache, inEnv, inAbsynExpLst, inBoolean, inPrefix)) {
        (cache, env, Deref @ metamodelica::List::Cons { head: arraycr, tail: Deref @ metamodelica::List::Cons { head: dim, tail: Deref @ metamodelica::List::Nil } }, r#impl, pre) => {
            ...
            dims = if ((dims1.clone().len() as i32) >= (dims2.clone().len() as i32)) {dims1} else {dims2};
            let (__pa0, __pa1) = ::match_deref::match_deref! { match &(elabBuiltinSizeIndex(arraycrefe, prop, ety, dimp, dims, env.clone(), info)) {
                (Some(__pa0), Some(__pa1)) => (__pa0.clone(), __pa1.clone()),
                _ => bail!("pattern mismatch"),
            } };
            exp = __pa0.clone();
            prop = __pa1.clone();
            (cache.clone(), exp, prop)
        },
        (cache, env, Deref @ metamodelica::List::Cons { head: arraycr, tail: Deref @ metamodelica::List::Nil }, r#impl, pre) => {
            let mut arraycrefe: Arc<DAE::Exp>;
            let mut exp: Arc<DAE::Exp>;
            let mut arrtp: Arc<DAE::Type>;
            let mut prop: DAE::Properties;
            let mut ety: Arc<DAE::Type>;
            let mut dims: Arc<metamodelica::List<Arc<DAE::Dimension>>>;
            let mut cache = (*cache).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: _, tail: _ } => {
                    let mut dim_expl: Arc<metamodelica::List<Arc<DAE::Exp>>>;
                    let mut dim_int: i32;
                    let mut exp: Arc<DAE::Exp>;
                    let mut prop: DAE::Properties;
                    let mut ty: Arc<DAE::Type>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: _, tail: _ } => {
                    ...
                    ty = Arc::new(DAE::Type::T_ARRAY { ty: DAE::T_INTEGER_DEFAULT().clone(), dims: list![openmodelica_frontend_types::DAE::Dimension::interned_DIM_UNKNOWN()] });
...
fn elabBuiltinSizeIndex(mut inArrayExp: Arc<DAE::Exp>, mut inArrayProp: DAE::Properties, mut inArrayType: Arc<DAE::Type>, mut inIndexExp: Arc<DAE::Exp>, mut inDimensions: Arc<metamodelica::List<Arc<DAE::Dimension>>>, mut inEnv: FCore::Graph, mut inInfo: SourceInfo) -> (Option<Arc<DAE::Exp>>, Option<DAE::Properties>) {
    let mut outSizeExp: Option<Arc<DAE::Exp>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ metamodelica::List::Cons { head: s, tail: dims }, r#impl, pre) => {
                    ...
                    let mut exp: Arc<DAE::Exp>;
                    let mut prop: DAE::Properties;
                    let mut dims_1: Arc<metamodelica::List<Arc<DAE::Exp>>>;
                    let mut dimprops: Arc<metamodelica::List<DAE::Properties>>;
                    let mut sty: Arc<DAE::Type>;
                    let mut dimvals: Arc<metamodelica::List<Arc<Values::Value>>>;
                    let mut c1: DAE::Const;
                    let mut cache = (*cache).clone();
                    ...
                    (cache, exp, prop) = ExpressionSimplify::elabBuiltinFill2(cache.clone(), s_1.clone(), sty.clone(), dimvals.clone(), c1.clone(), dims.clone(), info.clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ metamodelica::List::Cons { head: s, tail: dims }, r#impl, pre) => {
                    ...
                    exp = Expression::makePureBuiltinCall((literal!("fill")).clone(), metamodelica::cons(s_1.clone(), dims_1.clone()), exp_type.clone());
                    Ok((cache.clone(), exp.clone(), prop.clone()))
                }
                _ => bail!("nomatch"),
            }}
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, dims, r#impl, pre) => {
                    ...
                    let mut r#str: ArcStr;
                    let mut sp: ArcStr;
                    let mut expstrs: Arc<metamodelica::List<ArcStr>>;
                    let true = (Flags::isSet(Flags::FAILTRACE.clone())?) else { bail!("pattern mismatch") };
...
fn elabBuiltinSymmetric(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inAbsynExpLst: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArg: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache, inEnv, inAbsynExpLst, inBoolean, inPrefix)) {
        (cache, env, Deref @ metamodelica::List::Cons { head: matexp, tail: Deref @ metamodelica::List::Nil }, r#impl, pre) => {
            ...
            let mut d1: Arc<DAE::Dimension>;
            let mut d2: Arc<DAE::Dimension>;
            let mut eltp: Arc<DAE::Type>;
            let mut newtp: Arc<DAE::Type>;
            let mut prop: DAE::Properties;
            let mut c: DAE::Const;
            let mut exp_1: Arc<DAE::Exp>;
            let mut exp: Arc<DAE::Exp>;
            let mut cache = (*cache).clone();
            let (__pa0, __pa1, __pa2, __pa3, __pa4, __pa5) = ::match_deref::match_deref! { match &(elabExpInExpression(cache.clone(), env.clone(), matexp.clone(), r#impl.clone(), true, pre.clone(), info)?) {
                (__pa0, __pa1, DAE::Properties::PROP { type_: Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: __pa2, tail: Deref @ metamodelica::List::Nil }, ty: Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: __pa3, tail: Deref @ metamodelica::List::Nil }, ty: __pa4 } }, constFlag: __pa5 }) => (__pa0.clone(), __pa1.clone(), __pa2.clone(), __pa3.clone(), __pa4.clone(), __pa5.clone()),
                _ => bail!("pattern mismatch"),
            } };
            cache = __pa0.clone();
            exp_1 = __pa1.clone();
            ...
            eltp = __pa4.clone();
            c = __pa5.clone();
            newtp = Arc::new(DAE::Type::T_ARRAY { ty: Arc::new(DAE::Type::T_ARRAY { ty: eltp, dims: list![d1] }), dims: list![d2] });
            tp = Types::simplifyType(newtp.clone())?;
            exp = Expression::makePureBuiltinCall((literal!("symmetric")).clone(), list![exp_1], tp);
...
fn elabBuiltinClassDirectory(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inAbsynExpLst: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArg: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (match info.clone() {
        SourceInfo { fileName: mut fileName, .. } => {
            ...
            Error::addSourceMessage(Error::NON_STANDARD_OPERATOR_CLASS_DIRECTORY.clone(), metamodelica::nil(), info)?;
            (inCache, Arc::new(DAE::Exp::SCONST { string: (r#str).clone() }), DAE::Properties::PROP { type_: DAE::T_STRING_DEFAULT().clone(), constFlag: openmodelica_frontend_types::DAE::Const::C_CONST })
        },
        _ => bail!("match: no arm matched"),
    });
    Ok((outCache, outExp, outProperties))
...
fn elabBuiltinSourceInfo(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inAbsynExpLst: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArg: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (match info.clone() {
        SourceInfo { .. } => {
            ...
            outExp = Arc::new(DAE::Exp::METARECORDCALL { path: Arc::new(Absyn::Path::QUALIFIED { name: (literal!("SourceInfo")).clone(), path: Arc::new(Absyn::Path::IDENT { name: (literal!("SOURCEINFO")).clone() }) }), args: args, fieldNames: list![(literal!("fileName")).clone(), (literal!("isReadOnly")).clone(), (literal!("lineNumberStart")).clone(), (literal!("columnNumberStart")).clone(), (literal!("lineNumberEnd")).clone(), (literal!("columnNumberEnd")).clone(), (literal!("lastEditTime")).clone()], index: 0, typeVars: metamodelica::nil() });
            (inCache, outExp, DAE::Properties::PROP { type_: DAE::T_SOURCEINFO_DEFAULT().clone(), constFlag: openmodelica_frontend_types::DAE::Const::C_CONST })
        },
        _ => bail!("match: no arm matched"),
    });
    Ok((outCache, outExp, outProperties))
...
fn elabBuiltinSome(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut ty: Arc<DAE::Type>;
    let mut c: DAE::Const;
    if (inPosArgs.clone().len() as i32) != 1 || !(inNamedArgs.is_empty()) {
        Error::addSourceMessageAndFail(Error::WRONG_TYPE_OR_NO_OF_ARGS.clone(), list![(literal!("SOME")).clone(), (literal!("")).clone()], inInfo)?;
        unreachable!("Error.addSourceMessageAndFail always fails — caller-side flow-analysis hint");
    } else {
        ...
        outExp = Arc::new(DAE::Exp::META_OPTION { exp: Some(arg) });
        outProperties = DAE::Properties::PROP { type_: Arc::new(DAE::Type::T_METAOPTION { ty: ty }), constFlag: c };
    }
    Ok((outCache, outExp, outProperties))
}
...
fn elabBuiltinNone(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    if !(inPosArgs.is_empty()) || !(inNamedArgs.is_empty()) {
        Error::addSourceMessageAndFail(Error::WRONG_TYPE_OR_NO_OF_ARGS.clone(), list![(literal!("NONE")).clone(), (literal!("")).clone()], inInfo)?;
        unreachable!("Error.addSourceMessageAndFail always fails — caller-side flow-analysis hint");
    } else {
...
fn elabBuiltinHomotopy(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    if replaceWith.clone() == literal!("actual") || replaceWith.clone() == literal!("simplified") {
        ...
        e1 = __pa0.clone();
        e2 = __pa1.clone();
        e = if (replaceWith == literal!("actual")) {e1} else {e2};
        (outCache, outExp, outProperties) = elabExpInExpression(inCache, inEnv, e, inImplicit, true, inPrefix, inInfo)?;
    } else {
        (outCache, outExp, outProperties) = elabCallArgs(inCache, inEnv, Arc::new(Absyn::Path::IDENT { name: (literal!("homotopy")).clone() }), inPosArgs, inNamedArgs, metamodelica::nil(), inImplicit, inPrefix, inInfo)?;
    }
    Ok((outCache, outExp, outProperties))
}
...
fn getHomotopyArguments(mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>) -> Result<Arc<metamodelica::List<Arc<Absyn::Exp>>>> {
    let mut outPositionalArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>;
    outPositionalArgs = (::match_deref::match_deref! { match &((args.clone(), nargs.clone())) {
        ...
        _ => {
            Error::addCompilerError(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("+replaceHomotopy: homotopy called with wrong arguments: ")); __mm_s.push_str(&*Dump::printFunctionArgsStr(Arc::new(Absyn::FunctionArgs::FUNCTIONARGS { args: args, argNames: nargs }))?); ArcStr::from(__mm_s) }).clone())?;
            bail!("fail")
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn elabBuiltinDynamicSelect(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outProperties: DAE::Properties;
    let mut msg_str: ArcStr;
    let mut astatic: Arc<Absyn::Exp>;
    let mut adynamic: Arc<Absyn::Exp>;
    let mut dstatic: Arc<DAE::Exp>;
    let mut ddynamic: Arc<DAE::Exp>;
    let mut ty: Arc<DAE::Type>;
    if (inPosArgs.clone().len() as i32) != 2 || !(inNamedArgs.clone().is_empty()) {
        msg_str = (literal!(", expected DynamicSelect(staticExp, dynamicExp)")).clone();
        printBuiltinFnArgError((literal!("DynamicSelect")).clone(), (msg_str).clone(), inPosArgs.clone(), inNamedArgs, inPrefix.clone(), inInfo.clone())?;
    }
...
fn elabBuiltinTranspose(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArg: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImpl: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    d1 = __pa7.clone();
    d2 = __pa8.clone();
    ty = Arc::new(DAE::Type::T_ARRAY { ty: Arc::new(DAE::Type::T_ARRAY { ty: el_ty, dims: list![d2] }), dims: list![d1] });
    outProperties = DAE::Properties::PROP { type_: ty.clone(), constFlag: c };
    ty = Types::simplifyType(ty)?;
    outExp = Expression::makePureBuiltinCall((literal!("transpose")).clone(), list![exp], ty);
    Ok((outCache, outExp, outProperties))
}
...
fn elabBuiltinSum(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inAbsynExpLst: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArg: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache, inEnv, inAbsynExpLst, inBoolean, inPrefix)) {
        (cache, env, Deref @ metamodelica::List::Cons { head: arrexp, tail: Deref @ metamodelica::List::Nil }, r#impl, pre) => {
            ...
            b = b && Types::isSimpleType(tp.clone());
            estr = (Dump::printExpStr(arrexp.clone())?).clone();
            tstr = (TypesDump::unparseType(t)?).clone();
            Error::assertionOrAddSourceMessage(b, Error::SUM_EXPECTED_ARRAY.clone(), list![(estr).clone(), (tstr).clone()], info)?;
            exp_2 = Expression::makePureBuiltinCall((literal!("sum")).clone(), list![exp_1], etp);
            (cache.clone(), exp_2, DAE::Properties::PROP { type_: tp, constFlag: c })
        },
        _ => bail!("match: no arm matched"),
    } });
    Ok((outCache, outExp, outProperties))
            ...
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ metamodelica::List::Cons { head: arrexp, tail: Deref @ metamodelica::List::Nil }, r#impl, pre) => {
                    ...
                    let (__pa0, __pa1, __pa3, __pa2, __pa4) = ::match_deref::match_deref! { match &(elabExpInExpression(cache.clone(), env.clone(), arrexp.clone(), r#impl.clone(), true, pre.clone(), info.clone())?) {
                        (__pa0, __pa1, DAE::Properties::PROP { type_: __pa3 @ Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: _, tail: Deref @ metamodelica::List::Nil }, ty: __pa2 }, constFlag: __pa4 }) => (__pa0.clone(), __pa1.clone(), __pa3.clone(), __pa2.clone(), __pa4.clone()),
                        _ => bail!("pattern mismatch"),
                    } };
                    cache = __pa0.clone();
                    exp_1 = __pa1.clone();
...
fn elabBuiltinProduct2(mut inExp: Arc<DAE::Exp>) -> Arc<DAE::Exp> {
    let mut outExp: Arc<DAE::Exp>;
    outExp = 'mc: {
        let __mc_input = inExp.clone();
        if let Ok(__v) = (|| -> Result<_> {
...
fn elabBuiltinPre(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut exp_str: ArcStr;
    let mut pre_str: ArcStr;
    checkBuiltinCallArgs(inPosArgs.clone(), inNamedArgs, 1, (literal!("pre")).clone(), inInfo.clone())?;
    let (__pa0, __pa1, __pa2, __pa3) = ::match_deref::match_deref! { match &(elabExpInExpression(inCache, inEnv, listHead(inPosArgs)?, inImplicit, true, inPrefix.clone(), inInfo.clone())?) {
        (__pa0, __pa1, DAE::Properties::PROP { type_: __pa2, constFlag: __pa3 }) => (__pa0.clone(), __pa1.clone(), __pa2.clone(), __pa3.clone()),
        _ => bail!("pattern mismatch"),
    } };
    outCache = __pa0.clone();
    ...
    if Expression::isMatrix(exp.clone()) {
        ...
        outExp = Expression::makePureBuiltinCall((literal!("pre")).clone(), list![exp], Types::simplifyType(ty2.clone())?);
        ...
        outExp = Arc::new(DAE::Exp::ARRAY { ty: Types::simplifyType(ty.clone())?, scalar: sc, array: expl });
    } else {
        ty = Types::arrayElementType(ty);
        if Types::basicType(ty.clone()) {
            outExp = Expression::makePureBuiltinCall((literal!("pre")).clone(), list![exp], Types::simplifyType(ty.clone())?);
        } else {
            exp_str = (ExpressionBasics::printExpStr(exp)?).clone();
            pre_str = (PrefixUtil::printPrefixStr3(inPrefix)?).clone();
            Error::addSourceMessageAndFail(Error::OPERAND_BUILTIN_TYPE.clone(), list![(literal!("pre")).clone(), (pre_str).clone(), (exp_str).clone()], inInfo)?;
            unreachable!("Error.addSourceMessageAndFail always fails — caller-side flow-analysis hint");
        }
    }
    outProperties = DAE::Properties::PROP { type_: ty, constFlag: c };
...
fn elabBuiltinPre2(mut inExp: Arc<DAE::Exp>, mut inType: Arc<DAE::Type>) -> (Arc<metamodelica::List<Arc<DAE::Exp>>>, bool) {
    ...
    (outExp, outScalar) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Exp::CALL { expLst: Deref @ metamodelica::List::Cons { head: Deref @ DAE::Exp::ARRAY { scalar: sc, array: expl, .. }, tail: Deref @ metamodelica::List::Nil }, .. } => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Exp::CALL { expLst: Deref @ metamodelica::List::Cons { head: Deref @ DAE::Exp::MATRIX { ty, integer: i, matrix: mexpl }, tail: Deref @ metamodelica::List::Nil }, .. } => {
...
fn elabBuiltinInStream(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImpl: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    e = __pa0.clone();
    (outCache, exp, outProperties) = elabExpInExpression(inCache, inEnv.clone(), e.clone(), inImpl, true, inPrefix.clone(), inInfo.clone())?;
    ty = Types::getPropType(outProperties.clone())?;
    outExp = elabBuiltinStreamOperator(outCache.clone(), inEnv.clone(), (literal!("inStream")).clone(), exp, ty.clone(), inInfo.clone())?;
    if Types::dimensionsKnown(ty) {
        (outCache, outExp, outProperties) = elabCallArgs(outCache, inEnv, Arc::new(Absyn::Path::IDENT { name: (literal!("inStream")).clone() }), list![e], metamodelica::nil(), metamodelica::nil(), inImpl, inPrefix, inInfo)?;
    }
    Ok((outCache, outExp, outProperties))
}
...
fn elabBuiltinActualStream(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImpl: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, exp, outProperties) = elabExpInExpression(inCache, inEnv.clone(), e.clone(), inImpl, true, inPrefix.clone(), inInfo.clone())?;
    ty = Types::getPropType(outProperties.clone())?;
    outExp = elabBuiltinStreamOperator(outCache.clone(), inEnv.clone(), (literal!("actualStream")).clone(), exp, ty.clone(), inInfo.clone())?;
    if Types::dimensionsKnown(ty) {
        (outCache, outExp, outProperties) = elabCallArgs(outCache, inEnv, Arc::new(Absyn::Path::IDENT { name: (literal!("actualStream")).clone() }), list![e], metamodelica::nil(), metamodelica::nil(), inImpl, inPrefix, inInfo)?;
    }
    Ok((outCache, outExp, outProperties))
}
...
fn elabBuiltinStreamOperator(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inOperator: ArcStr, mut inExp: Arc<DAE::Exp>, mut inType: Arc<DAE::Type>, mut inInfo: SourceInfo) -> Result<Arc<DAE::Exp>> {
    ...
    outExp = (::match_deref::match_deref! { match &(inExp.clone()) {
        ...
        _ => {
            ...
            exp = __pa0.clone();
            validateBuiltinStreamOperator(inCache, inEnv, exp.clone(), inType.clone(), (inOperator.clone()).clone(), inInfo)?;
            et = Types::simplifyType(inType)?;
            exp = Expression::makePureBuiltinCall((inOperator).clone(), list![exp], et);
            exp
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn elabBuiltinArray(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut c: DAE::Const;
    let mut len: i32;
    (outCache, expl, props) = elabExpList(inCache, inEnv, inPosArgs, inImplicit, true, inPrefix.clone(), inInfo.clone(), DAE::T_UNKNOWN_DEFAULT().clone())?;
    let (__pa0, __pa1) = ::match_deref::match_deref! { match &(elabBuiltinArray2(expl.clone(), props, inPrefix, inInfo)?) {
    ...
    arr_ty = Arc::new(DAE::Type::T_ARRAY { ty: ty.clone(), dims: list![Arc::new(DAE::Dimension::DIM_INTEGER { integer: len })] });
    outProperties = DAE::Properties::PROP { type_: arr_ty.clone(), constFlag: c };
    arr_ty = Types::simplifyType(arr_ty)?;
    outExp = Arc::new(DAE::Exp::ARRAY { ty: arr_ty, scalar: Types::isArray(ty), array: expl });
    Ok((outCache, outExp, outProperties))
}
...
fn elabBuiltinArray2(mut inExpl: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut inProperties: Arc<metamodelica::List<DAE::Properties>>, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(Arc<metamodelica::List<Arc<DAE::Exp>>>, DAE::Properties)> {
    ...
    if !(sameDimensions(inProperties.clone())?) {
        pre_str = (PrefixUtil::printPrefixStr3(inPrefix)?).clone();
        Error::addSourceMessageAndFail(Error::DIFFERENT_DIM_SIZE_IN_ARGUMENTS.clone(), list![(literal!("array")).clone(), (pre_str).clone()], inInfo)?;
        unreachable!("Error.addSourceMessageAndFail always fails — caller-side flow-analysis hint");
    }
...
fn elabBuiltinZeros(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = elabBuiltinFill(inCache, inEnv, metamodelica::cons(Arc::new(Absyn::Exp::INTEGER { value: 0 }), inPosArgs), metamodelica::nil(), inImplicit, inPrefix, inInfo)?;
    Ok((outCache, outExp, outProperties))
}
...
fn sameDimensions(mut inProps: Arc<metamodelica::List<DAE::Properties>>) -> Result<bool> {
    let mut res: bool;
    let mut types: Arc<metamodelica::List<Arc<DAE::Type>>>;
    let mut dims: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>;
    types = List::map(inProps, (std::sync::Arc::new(Types::getPropType) as std::sync::Arc<dyn ::std::ops::Fn(DAE::Properties) -> Result<Arc<DAE::Type>> + 'static>))?;
    dims = List::map(types, (std::sync::Arc::new(fnptr!(TypesDump::getDimensions, Arc<DAE::Type>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Type>) -> Result<Arc<metamodelica::List<Arc<DAE::Dimension>>>> + 'static>))?;
...
fn sameDimensions3(mut inDims: Arc<metamodelica::List<Arc<DAE::Dimension>>>) -> Result<bool> {
    ...
    for mut dim2 in &*listRest(inDims)? {
...
fn elabBuiltinOnes(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArg: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = elabBuiltinFill(inCache, inEnv, metamodelica::cons(Arc::new(Absyn::Exp::INTEGER { value: 1 }), inPosArgs), metamodelica::nil(), inImplicit, inPrefix, inInfo)?;
    Ok((outCache, outExp, outProperties))
}
...
fn elabBuiltinMax(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inFnArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArg: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImpl: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = elabBuiltinMinMaxCommon(inCache, inEnv, (literal!("max")).clone(), inFnArgs, inImpl, inPrefix, info)?;
    Ok((outCache, outExp, outProperties))
}
...
fn elabBuiltinMin(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inFnArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArg: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImpl: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = elabBuiltinMinMaxCommon(inCache, inEnv, (literal!("min")).clone(), inFnArgs, inImpl, inPrefix, info)?;
    Ok((outCache, outExp, outProperties))
}
...
fn elabBuiltinMinMaxCommon(mut cache: FCore::Cache, mut env: FCore::Graph, mut inFnName: ArcStr, mut inFnArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut r#impl: bool, mut prefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outExp, outProperties) = (::match_deref::match_deref! { match &(inFnArgs) {
        Deref @ metamodelica::List::Cons { head: arrexp, tail: Deref @ metamodelica::List::Nil } => {
            ...
            call = Expression::makePureBuiltinCall((inFnName).clone(), list![arrexp_1], tp);
        ...
        Deref @ metamodelica::List::Cons { head: s1, tail: Deref @ metamodelica::List::Cons { head: s2, tail: Deref @ metamodelica::List::Nil } } => {
            ...
            let false = (Types::isString(tp.clone())) else { bail!("pattern mismatch") };
            call = Expression::makePureBuiltinCall((inFnName).clone(), list![s1_1, s2_1], tp);
...
fn elabBuiltinClock(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = ({
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, _, Deref @ metamodelica::List::Nil, Deref @ metamodelica::List::Nil, _, _) => {
                    let mut call: Arc<DAE::Exp>;
                    call = Arc::new(DAE::Exp::CLKCONST { clk: openmodelica_frontend_types::DAE::ClockKind::interned_INFERRED_CLOCK() });
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ metamodelica::List::Cons { head: ac, tail: Deref @ metamodelica::List::Nil }, Deref @ metamodelica::List::Cons { head: Deref @ Absyn::NamedArg { argName: Deref @ "solverMethod", argValue: asolverMethod }, tail: Deref @ metamodelica::List::Nil }, r#impl, pre) => {
                    let mut call: Arc<DAE::Exp>;
                    let mut c: Arc<DAE::Exp>;
                    let mut solverMethod: Arc<DAE::Exp>;
                    let mut ty1: Arc<DAE::Type>;
...
fn elabBuiltinHold(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache, inEnv, args.clone(), nargs.clone(), inBoolean, inPrefix)) {
        (cache, env, Deref @ metamodelica::List::Cons { head: au, tail: Deref @ metamodelica::List::Nil }, Deref @ metamodelica::List::Nil, r#impl, pre) => {
            ...
            ty = Arc::new(DAE::Type::T_FUNCTION { funcArg: list![Arc::new(DAE::FuncArg { name: (literal!("u")).clone(), ty: ty1.clone(), r#const: openmodelica_frontend_types::DAE::Const::C_VAR, par: openmodelica_frontend_types::DAE::VarParallelism::NON_PARALLEL, defaultBinding: None })], funcResultType: ty1, functionAttributes: DAE::FUNCTION_ATTRIBUTES_BUILTIN_IMPURE.clone(), path: Arc::new(Absyn::Path::IDENT { name: (literal!("hold")).clone() }) });
            let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(elabCallArgs3(cache.clone(), env.clone(), list![ty], Arc::new(Absyn::Path::IDENT { name: (literal!("hold")).clone() }), args, nargs, metamodelica::nil(), r#impl.clone(), pre.clone(), info)?) {
...
fn elabBuiltinSample(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = 'mc: {
        let __mc_input = (inCache, inEnv, args.clone(), nargs.clone(), inBoolean, inPrefix);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
fn elabBuiltinShiftSample(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache, inEnv, args, nargs.clone(), inBoolean, inPrefix)) {
        (cache, env, Deref @ metamodelica::List::Cons { head: au, tail: Deref @ metamodelica::List::Cons { head: ashiftCounter, tail: Deref @ metamodelica::List::Nil } }, Deref @ metamodelica::List::Nil, r#impl, pre) => {
            ...
            let mut prop2: DAE::Properties;
            let mut prop: DAE::Properties;
            let mut aresolution: Arc<Absyn::Exp>;
            let mut val: Arc<Values::Value>;
            let mut cache = (*cache).clone();
            ...
            (cache, val) = Ceval::ceval(cache.clone(), env.clone(), shiftCounter, false, Absyn::Msg::MSG { info: info.clone() }, 0)?;
            ...
            ashiftCounter = Arc::new(Absyn::Exp::INTEGER { value: ValuesUtil::valueInteger(val)? });
            aresolution = Arc::new(Absyn::Exp::INTEGER { value: 1 });
            ty1 = Types::arrayElementType(Types::getPropType(prop1)?);
            ty = Arc::new(DAE::Type::T_FUNCTION { funcArg: list![Arc::new(DAE::FuncArg { name: (literal!("u")).clone(), ty: ty1.clone(), r#const: openmodelica_frontend_types::DAE::Const::C_VAR, par: openmodelica_frontend_types::DAE::VarParallelism::NON_PARALLEL, defaultBinding: None }), Arc::new(DAE::FuncArg { name: (literal!("shiftCounter")).clone(), ty: DAE::T_INTEGER_DEFAULT().clone(), r#const: openmodelica_frontend_types::DAE::Const::C_PARAM, par: openmodelica_frontend_types::DAE::VarParallelism::NON_PARALLEL, defaultBinding: None }), Arc::new(DAE::FuncArg { name: (literal!("resolution")).clone(), ty: DAE::T_INTEGER_DEFAULT().clone(), r#const: openmodelica_frontend_types::DAE::Const::C_PARAM, par: openmodelica_frontend_types::DAE::VarParallelism::NON_PARALLEL, defaultBinding: None })], funcResultType: ty1, functionAttributes: DAE::FUNCTION_ATTRIBUTES_BUILTIN_IMPURE.clone(), path: Arc::new(Absyn::Path::IDENT { name: (literal!("shiftSample")).clone() }) });
            let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(elabCallArgs3(cache.clone(), env.clone(), list![ty], Arc::new(Absyn::Path::IDENT { name: (literal!("shiftSample")).clone() }), list![au.clone(), ashiftCounter.clone(), aresolution.clone()], nargs, metamodelica::nil(), r#impl.clone(), pre.clone(), info)?) {
                (__pa0, Some((__pa1, __pa2))) => (__pa0.clone(), __pa1.clone(), __pa2.clone()),
                _ => bail!("pattern mismatch"),
            } };
            cache = __pa0.clone();
        ...
        (cache, env, Deref @ metamodelica::List::Cons { head: au, tail: Deref @ metamodelica::List::Cons { head: ashiftCounter, tail: Deref @ metamodelica::List::Cons { head: aresolution, tail: Deref @ metamodelica::List::Nil } } }, Deref @ metamodelica::List::Nil, r#impl, pre) => {
            ...
            (cache, rval) = Ceval::ceval(cache.clone(), env.clone(), resolution, false, Absyn::Msg::MSG { info: info.clone() }, 0)?;
            ...
            aresolution = Arc::new(Absyn::Exp::INTEGER { value: ValuesUtil::valueInteger(rval)? });
            ty1 = Types::arrayElementType(Types::getPropType(prop1)?);
            ty = Arc::new(DAE::Type::T_FUNCTION { funcArg: list![Arc::new(DAE::FuncArg { name: (literal!("u")).clone(), ty: ty1.clone(), r#const: openmodelica_frontend_types::DAE::Const::C_VAR, par: openmodelica_frontend_types::DAE::VarParallelism::NON_PARALLEL, defaultBinding: None }), Arc::new(DAE::FuncArg { name: (literal!("shiftCounter")).clone(), ty: DAE::T_INTEGER_DEFAULT().clone(), r#const: openmodelica_frontend_types::DAE::Const::C_PARAM, par: openmodelica_frontend_types::DAE::VarParallelism::NON_PARALLEL, defaultBinding: None }), Arc::new(DAE::FuncArg { name: (literal!("resolution")).clone(), ty: DAE::T_INTEGER_DEFAULT().clone(), r#const: openmodelica_frontend_types::DAE::Const::C_PARAM, par: openmodelica_frontend_types::DAE::VarParallelism::NON_PARALLEL, defaultBinding: None })], funcResultType: ty1, functionAttributes: DAE::FUNCTION_ATTRIBUTES_BUILTIN_IMPURE.clone(), path: Arc::new(Absyn::Path::IDENT { name: (literal!("shiftSample")).clone() }) });
            let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(elabCallArgs3(cache.clone(), env.clone(), list![ty], Arc::new(Absyn::Path::IDENT { name: (literal!("shiftSample")).clone() }), list![au.clone(), ashiftCounter.clone(), aresolution.clone()], nargs, metamodelica::nil(), r#impl.clone(), pre.clone(), info)?) {
                (__pa0, Some((__pa1, __pa2))) => (__pa0.clone(), __pa1.clone(), __pa2.clone()),
...
fn elabBuiltinBackSample(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache, inEnv, args, nargs.clone(), inBoolean, inPrefix)) {
        (cache, env, Deref @ metamodelica::List::Cons { head: au, tail: Deref @ metamodelica::List::Cons { head: abackCounter, tail: Deref @ metamodelica::List::Nil } }, Deref @ metamodelica::List::Nil, r#impl, pre) => {
            ...
            (cache, val) = Ceval::ceval(cache.clone(), env.clone(), backCounter, false, Absyn::Msg::MSG { info: info.clone() }, 0)?;
            ...
            abackCounter = Arc::new(Absyn::Exp::INTEGER { value: ValuesUtil::valueInteger(val)? });
            aresolution = Arc::new(Absyn::Exp::INTEGER { value: 1 });
            ty1 = Types::arrayElementType(Types::getPropType(prop1)?);
            ty = Arc::new(DAE::Type::T_FUNCTION { funcArg: list![Arc::new(DAE::FuncArg { name: (literal!("u")).clone(), ty: ty1.clone(), r#const: openmodelica_frontend_types::DAE::Const::C_VAR, par: openmodelica_frontend_types::DAE::VarParallelism::NON_PARALLEL, defaultBinding: None }), Arc::new(DAE::FuncArg { name: (literal!("backCounter")).clone(), ty: DAE::T_INTEGER_DEFAULT().clone(), r#const: openmodelica_frontend_types::DAE::Const::C_PARAM, par: openmodelica_frontend_types::DAE::VarParallelism::NON_PARALLEL, defaultBinding: None }), Arc::new(DAE::FuncArg { name: (literal!("resolution")).clone(), ty: DAE::T_INTEGER_DEFAULT().clone(), r#const: openmodelica_frontend_types::DAE::Const::C_PARAM, par: openmodelica_frontend_types::DAE::VarParallelism::NON_PARALLEL, defaultBinding: None })], funcResultType: ty1, functionAttributes: DAE::FUNCTION_ATTRIBUTES_BUILTIN_IMPURE.clone(), path: Arc::new(Absyn::Path::IDENT { name: (literal!("backSample")).clone() }) });
            let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(elabCallArgs3(cache.clone(), env.clone(), list![ty], Arc::new(Absyn::Path::IDENT { name: (literal!("backSample")).clone() }), list![au.clone(), abackCounter.clone(), aresolution.clone()], nargs, metamodelica::nil(), r#impl.clone(), pre.clone(), info)?) {
                (__pa0, Some((__pa1, __pa2))) => (__pa0.clone(), __pa1.clone(), __pa2.clone()),
                _ => bail!("pattern mismatch"),
            } };
            cache = __pa0.clone();
...
fn elabBuiltinNoClock(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache, inEnv, args.clone(), nargs.clone(), inBoolean, inPrefix)) {
        (cache, env, Deref @ metamodelica::List::Cons { head: au, tail: Deref @ metamodelica::List::Nil }, Deref @ metamodelica::List::Nil, r#impl, pre) => {
            ...
            (cache, _, prop1) = elabExpInExpression(cache.clone(), env.clone(), au.clone(), r#impl.clone(), true, pre.clone(), info.clone())?;
            ty1 = Types::arrayElementType(Types::getPropType(prop1)?);
            ty = Arc::new(DAE::Type::T_FUNCTION { funcArg: list![Arc::new(DAE::FuncArg { name: (literal!("u")).clone(), ty: ty1.clone(), r#const: openmodelica_frontend_types::DAE::Const::C_VAR, par: openmodelica_frontend_types::DAE::VarParallelism::NON_PARALLEL, defaultBinding: None })], funcResultType: ty1, functionAttributes: DAE::FUNCTION_ATTRIBUTES_BUILTIN_IMPURE.clone(), path: Arc::new(Absyn::Path::IDENT { name: (literal!("noClock")).clone() }) });
            let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(elabCallArgs3(cache.clone(), env.clone(), list![ty], Arc::new(Absyn::Path::IDENT { name: (literal!("noClock")).clone() }), args, nargs, metamodelica::nil(), r#impl.clone(), pre.clone(), info)?) {
                (__pa0, Some((__pa1, __pa2))) => (__pa0.clone(), __pa1.clone(), __pa2.clone()),
                _ => bail!("pattern mismatch"),
            } };
            cache = __pa0.clone();
...
fn elabBuiltinFirstTick(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache, inEnv, args.clone(), nargs.clone(), inBoolean, inPrefix)) {
        (cache, env, Deref @ metamodelica::List::Nil, Deref @ metamodelica::List::Nil, r#impl, pre) => {
            ...
            let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(elabCallArgs3(cache.clone(), env.clone(), list![ty], Arc::new(Absyn::Path::IDENT { name: (literal!("firstTick")).clone() }), args, nargs, metamodelica::nil(), r#impl.clone(), pre.clone(), info)?) {
                (__pa0, Some((__pa1, __pa2))) => (__pa0.clone(), __pa1.clone(), __pa2.clone()),
                _ => bail!("pattern mismatch"),
            } };
            cache = __pa0.clone();
...
fn elabBuiltinInterval(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache, inEnv, args.clone(), nargs.clone(), inBoolean, inPrefix)) {
        (cache, env, Deref @ metamodelica::List::Nil, Deref @ metamodelica::List::Nil, r#impl, pre) => {
            ...
            let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(elabCallArgs3(cache.clone(), env.clone(), list![ty], Arc::new(Absyn::Path::IDENT { name: (literal!("interval")).clone() }), args, nargs, metamodelica::nil(), r#impl.clone(), pre.clone(), info)?) {
                (__pa0, Some((__pa1, __pa2))) => (__pa0.clone(), __pa1.clone(), __pa2.clone()),
                _ => bail!("pattern mismatch"),
            } };
            cache = __pa0.clone();
...
fn isBlockTypeWorkaround(mut ity: Arc<DAE::Type>) -> bool {
    '__tco: loop {
        ...
        Deref @ DAE::Type::T_SUBTYPE_BASIC { .. } => { ity = var_field!((*ity).complexType, DAE::Type::T_SUBTYPE_BASIC).clone(); continue '__tco; },
        Deref @ DAE::Type::T_COMPLEX { .. } => return true,
        _ => return false,
...
fn elabBuiltinTransition(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (match (inCache, inEnv, inBoolean, inPrefix) {
        (mut cache, mut env, mut r#impl, mut pre) => {
            ...
            let mut s1: ArcStr;
            let mut s2: ArcStr;
            let mut slist: Arc<metamodelica::List<ArcStr>>;
...
fn elabBuiltinTransition2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo, mut argName: ArcStr, mut n: i32, mut strMsg0: ArcStr, mut strPre: ArcStr) -> Result<Arc<DAE::Type>> {
    ...
    Error::assertionOrAddSourceMessage(!(b1 && n >= nPos), Error::WRONG_TYPE_OR_NO_OF_ARGS.clone(), list![(s1).clone(), (strPre.clone()).clone()], info.clone())?;
    ...
    Error::assertionOrAddSourceMessage(b1 || n >= nPos, Error::WRONG_TYPE_OR_NO_OF_ARGS.clone(), list![(s2).clone(), (strPre.clone()).clone()], info.clone())?;
    arg1 = elabBuiltinTransition5((argName).clone(), b1, args, nargs)?;
    (_, _, prop1) = elabExpInExpression(inCache, inEnv, arg1, inBoolean, true, inPrefix, info.clone())?;
    ty = Types::getPropType(prop1)?;
    strMsg1 = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*strMsg0); __mm_s.push_str(&*literal!(", ")); __mm_s.push_str(&*strPos); __mm_s.push_str(&*literal!("argument needs to be a block instance.")); ArcStr::from(__mm_s) }).clone();
    Error::assertionOrAddSourceMessage(isBlockTypeWorkaround(ty.clone()), Error::WRONG_TYPE_OR_NO_OF_ARGS.clone(), list![(strMsg1).clone(), (strPre).clone()], info)?;
    Ok(ty)
}
...
fn elabBuiltinTransition3(mut name: ArcStr, mut namedArg: Arc<Absyn::NamedArg>) -> bool {
    ...
    outIsEqual = (::match_deref::match_deref! { match &(namedArg.clone()) {
        Deref @ Absyn::NamedArg { .. } => {
            stringEq((name).clone(), (namedArg.argName.clone()).clone())
        },
        _ => {
            false
        },
...
fn elabBuiltinTransition4(mut inElement: Arc<Absyn::NamedArg>) -> Result<Arc<Absyn::Exp>> {
    let mut argValue: Arc<Absyn::Exp>;
...
fn elabBuiltinTransition5(mut argName: ArcStr, mut getAsNamedArg: bool, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>) -> Result<Arc<Absyn::Exp>> {
    let mut argValue: Arc<Absyn::Exp>;
    argValue = (::match_deref::match_deref! { match &((argName, getAsNamedArg)) {
        (Deref @ "from", true) => {
            let mut namedArg: Arc<Absyn::NamedArg>;
            namedArg = List::getMemberOnTrue((literal!("from")).clone(), nargs, (std::sync::Arc::new(fnptr!(elabBuiltinTransition3, ArcStr, Arc<Absyn::NamedArg>)) as std::sync::Arc<dyn ::std::ops::Fn(ArcStr, Arc<Absyn::NamedArg>) -> Result<bool> + 'static>))?;
        ...
        (Deref @ "to", true) => {
            let mut namedArg: Arc<Absyn::NamedArg>;
            namedArg = List::getMemberOnTrue((literal!("to")).clone(), nargs, (std::sync::Arc::new(fnptr!(elabBuiltinTransition3, ArcStr, Arc<Absyn::NamedArg>)) as std::sync::Arc<dyn ::std::ops::Fn(ArcStr, Arc<Absyn::NamedArg>) -> Result<bool> + 'static>))?;
            elabBuiltinTransition4(namedArg)?
        },
        (Deref @ "to", false) => {
            (args).get(2)?
...
fn elabBuiltinInitialState(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache, inEnv, args.clone(), nargs.clone(), inBoolean, inPrefix)) {
        (cache, env, Deref @ metamodelica::List::Cons { head: astate, tail: Deref @ metamodelica::List::Nil }, Deref @ metamodelica::List::Nil, r#impl, pre) => {
            ...
            strPre = (PrefixUtil::printPrefixStr3(pre.clone())?).clone();
            Error::assertionOrAddSourceMessage(isBlockTypeWorkaround(ty1.clone()), Error::WRONG_TYPE_OR_NO_OF_ARGS.clone(), list![(strMsg).clone(), (strPre).clone()], info.clone())?;
            ty = Arc::new(DAE::Type::T_FUNCTION { funcArg: list![Arc::new(DAE::FuncArg { name: (literal!("state")).clone(), ty: ty1, r#const: openmodelica_frontend_types::DAE::Const::C_VAR, par: openmodelica_frontend_types::DAE::VarParallelism::NON_PARALLEL, defaultBinding: None })], funcResultType: DAE::T_NORETCALL_DEFAULT().clone(), functionAttributes: DAE::FUNCTION_ATTRIBUTES_BUILTIN_IMPURE.clone(), path: Arc::new(Absyn::Path::IDENT { name: (literal!("initialState")).clone() }) });
            let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(elabCallArgs3(cache.clone(), env.clone(), list![ty], Arc::new(Absyn::Path::IDENT { name: (literal!("initialState")).clone() }), args, nargs, metamodelica::nil(), r#impl.clone(), pre.clone(), info)?) {
                (__pa0, Some((__pa1, __pa2))) => (__pa0.clone(), __pa1.clone(), __pa2.clone()),
                _ => bail!("pattern mismatch"),
            } };
            cache = __pa0.clone();
...
fn elabBuiltinActiveState(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache, inEnv, args.clone(), nargs.clone(), inBoolean, inPrefix)) {
        (cache, env, Deref @ metamodelica::List::Cons { head: astate, tail: Deref @ metamodelica::List::Nil }, Deref @ metamodelica::List::Nil, r#impl, pre) => {
            ...
            let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(elabCallArgs3(cache.clone(), env.clone(), list![ty], Arc::new(Absyn::Path::IDENT { name: (literal!("activeState")).clone() }), args, nargs, metamodelica::nil(), r#impl.clone(), pre.clone(), info)?) {
                (__pa0, Some((__pa1, __pa2))) => (__pa0.clone(), __pa1.clone(), __pa2.clone()),
                _ => bail!("pattern mismatch"),
            } };
            cache = __pa0.clone();
...
fn elabBuiltinTicksInState(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache, inEnv, args.clone(), nargs.clone(), inBoolean, inPrefix)) {
        (cache, env, Deref @ metamodelica::List::Nil, Deref @ metamodelica::List::Nil, r#impl, pre) => {
            ...
            let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(elabCallArgs3(cache.clone(), env.clone(), list![ty], Arc::new(Absyn::Path::IDENT { name: (literal!("ticksInState")).clone() }), args, nargs, metamodelica::nil(), r#impl.clone(), pre.clone(), info)?) {
                (__pa0, Some((__pa1, __pa2))) => (__pa0.clone(), __pa1.clone(), __pa2.clone()),
                _ => bail!("pattern mismatch"),
            } };
            cache = __pa0.clone();
...
fn elabBuiltinTimeInState(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache, inEnv, args.clone(), nargs.clone(), inBoolean, inPrefix)) {
        (cache, env, Deref @ metamodelica::List::Nil, Deref @ metamodelica::List::Nil, r#impl, pre) => {
            ...
            let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(elabCallArgs3(cache.clone(), env.clone(), list![ty], Arc::new(Absyn::Path::IDENT { name: (literal!("timeInState")).clone() }), args, nargs, metamodelica::nil(), r#impl.clone(), pre.clone(), info)?) {
                (__pa0, Some((__pa1, __pa2))) => (__pa0.clone(), __pa1.clone(), __pa2.clone()),
                _ => bail!("pattern mismatch"),
            } };
            cache = __pa0.clone();
...
fn elabBuiltinBoolean(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = verifyBuiltInHandlerType(inCache, inEnv, inPosArgs, inImplicit, (std::sync::Arc::new(fnptr!(Types::isIntegerOrRealOrBooleanOrSubTypeOfEither, Arc<DAE::Type>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Type>) -> Result<bool> + 'static>), (literal!("boolean")).clone(), inPrefix, inInfo)?;
    Ok((outCache, outExp, outProperties))
}
...
fn elabBuiltinIntegerEnum(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArg: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = verifyBuiltInHandlerType(inCache, inEnv, inPosArgs, inImplicit, (std::sync::Arc::new(fnptr!(Types::isEnumeration, Arc<DAE::Type>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Type>) -> Result<bool> + 'static>), (literal!("Integer")).clone(), inPrefix, inInfo)?;
    Ok((outCache, outExp, outProperties))
}
...
fn elabBuiltinNoevent(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outProperties: DAE::Properties;
    let mut e: Arc<Absyn::Exp>;
    checkBuiltinCallArgs(inPosArgs.clone(), inNamedArgs, 1, (literal!("noEvent")).clone(), inInfo.clone())?;
    e = listHead(inPosArgs)?;
    (outCache, outExp, outProperties) = elabExpInExpression(inCache, inEnv, e, inImplicit, true, inPrefix, inInfo)?;
    outExp = Expression::makePureBuiltinCall((literal!("noEvent")).clone(), list![outExp], DAE::T_BOOL_DEFAULT().clone());
    Ok((outCache, outExp, outProperties))
}
...
fn elabBuiltinEdge(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut c: DAE::Const;
    let mut msg: ArcStr;
    checkBuiltinCallArgs(inPosArgs.clone(), inNamedArgs, 1, (literal!("edge")).clone(), inInfo.clone())?;
    (outCache, outExp, outProperties) = elabExpInExpression(inCache, inEnv, listHead(inPosArgs)?, inImplicit, true, inPrefix, inInfo.clone())?;
    ...
    if !(Types::isScalarBoolean(ty)) {
        ...
        Error::addSourceMessageAndFail(Error::TYPE_ERROR.clone(), list![(msg).clone()], inInfo)?;
        unreachable!("Error.addSourceMessageAndFail always fails — caller-side flow-analysis hint");
    }
    if Types::isVar(c) {
        outExp = Expression::makePureBuiltinCall((literal!("edge")).clone(), list![outExp], DAE::T_BOOL_DEFAULT().clone());
    } else {
...
fn elabBuiltinDer(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    checkBuiltinCallArgs(inPosArgs.clone(), inNamedArgs, 1, (literal!("der")).clone(), inInfo.clone())?;
    (outCache, outExp, outProperties) = elabExpInExpression(inCache.clone(), inEnv.clone(), listHead(inPosArgs.clone())?, inImplicit, true, inPrefix.clone(), inInfo.clone())?;
    let DAE::PROP { type_: __pa0, constFlag: __pa1 } = (outProperties.clone()) else { bail!("pattern mismatch") };
    ty = __pa0.clone();
    c = __pa1.clone();
    if !(Types::isRealOrSubTypeReal(Types::arrayElementType(ty.clone()))) {
        ...
        Error::addSourceMessageAndFail(Error::DERIVATIVE_NON_REAL.clone(), list![(exp_str).clone(), (ty_str).clone()], inInfo.clone())?;
        unreachable!("Error.addSourceMessageAndFail always fails — caller-side flow-analysis hint");
    }
    if Types::isVar(c) {
        if Types::dimensionsKnown(ty.clone()) {
            (outCache, outExp, outProperties) = elabCallArgs(inCache, inEnv, Arc::new(Absyn::Path::IDENT { name: (literal!("der")).clone() }), inPosArgs, metamodelica::nil(), metamodelica::nil(), inImplicit, inPrefix, inInfo)?;
        } else {
            outExp = Expression::makePureBuiltinCall((literal!("der")).clone(), list![outExp], Types::simplifyType(ty)?);
        }
    } else {
...
fn elabBuiltinChange(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    checkBuiltinCallArgs(inPosArgs.clone(), inNamedArgs, 1, (literal!("change")).clone(), inInfo.clone())?;
    e = listHead(inPosArgs)?;
    if !(AbsynUtil::isCref(e.clone())) {
        pre_str = (PrefixUtil::printPrefixStr3(inPrefix.clone())?).clone();
        Error::addSourceMessageAndFail(Error::ARGUMENT_MUST_BE_VARIABLE.clone(), list![(literal!("First")).clone(), (literal!("change")).clone(), (pre_str).clone()], inInfo.clone())?;
        unreachable!("Error.addSourceMessageAndFail always fails — caller-side flow-analysis hint");
    }
    (outCache, outExp, outProperties) = elabExpInExpression(inCache, inEnv.clone(), e, inImplicit, true, inPrefix.clone(), inInfo.clone())?;
    let DAE::PROP { type_: __pa0, constFlag: __pa1 } = (outProperties.clone()) else { bail!("pattern mismatch") };
    ty = __pa0.clone();
    c = __pa1.clone();
    if Types::isSimpleType(ty.clone()) {
        if Types::isParameterOrConstant(c) {
            ...
            outExp = Expression::makePureBuiltinCall((literal!("change")).clone(), list![outExp], DAE::T_BOOL_DEFAULT().clone());
            ...
            let __pa2 = ::match_deref::match_deref! { match &(attr) {
            ...
            if var == openmodelica_frontend_types::SCode::Variability::DISCRETE {
                ...
                pre_str = (PrefixUtil::printPrefixStr3(inPrefix)?).clone();
                Error::addSourceMessageAndFail(Error::ARGUMENT_MUST_BE_DISCRETE_VAR.clone(), list![(literal!("First")).clone(), (literal!("change")).clone(), (pre_str).clone()], inInfo)?;
                unreachable!("Error.addSourceMessageAndFail always fails — caller-side flow-analysis hint");
            }
        }
    } else {
        pre_str = (PrefixUtil::printPrefixStr3(inPrefix)?).clone();
        Error::addSourceMessageAndFail(Error::TYPE_MUST_BE_SIMPLE.clone(), list![(literal!("operand to change")).clone(), (pre_str).clone()], inInfo)?;
        unreachable!("Error.addSourceMessageAndFail always fails — caller-side flow-analysis hint");
    }
    Ok((outCache, outExp, outProperties))
}
...
fn elabBuiltinCat(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut dim_ty: Arc<DAE::Type>;
    let mut ty: Arc<DAE::Type>;
    let mut result_ty: Arc<DAE::Type>;
    ...
    let mut exp_str: ArcStr;
    let mut dim_int: i32;
    let mut arr_expl: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    let mut arr_props: Arc<metamodelica::List<DAE::Properties>>;
    let mut arr_tys: Arc<metamodelica::List<Arc<DAE::Type>>>;
    let mut tys: Arc<metamodelica::List<Arc<DAE::Type>>>;
    let mut dims: Arc<metamodelica::List<Arc<DAE::Dimension>>>;
    let mut dim: Arc<DAE::Dimension>;
    if (inPosArgs.clone().len() as i32) < 2 || !(inNamedArgs.is_empty()) {
        Error::addSourceMessageAndFail(Error::WRONG_NO_OF_ARGS.clone(), list![(literal!("cat")).clone()], inInfo.clone())?;
        unreachable!("Error.addSourceMessageAndFail always fails — caller-side flow-analysis hint");
    }
    (outCache, dim_exp, dim_props) = elabExpInExpression(inCache.clone(), inEnv.clone(), listHead(inPosArgs.clone())?, inImplicit, true, inPrefix.clone(), inInfo.clone())?;
    let DAE::PROP { type_: __pa0, constFlag: __pa1 } = (dim_props) else { bail!("pattern mismatch") };
    dim_ty = __pa0.clone();
    dim_c = __pa1.clone();
    ...
    outCache = __pa2.clone();
    dim_int = __pa3.clone();
    (outCache, arr_expl, arr_props) = elabExpList(outCache, inEnv, listRest(inPosArgs.clone())?, inImplicit, true, inPrefix.clone(), inInfo.clone(), DAE::T_UNKNOWN_DEFAULT().clone())?;
    arr_tys = ({
        let mut __acc: Arc<metamodelica::List<Arc<DAE::Type>>> = metamodelica::nil();
    ...
    ty = __pa4.clone();
    tys = __pa5.clone();
    result_ty = List::fold1(tys, (std::sync::Arc::new(Types::arraySuperType) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Type>, SourceInfo, Arc<DAE::Type>) -> Result<Arc<DAE::Type>> + 'static>), inInfo.clone(), ty)?;
    ...
    outExp = Expression::makePureBuiltinCall((literal!("cat")).clone(), metamodelica::cons(dim_exp, arr_expl), ty);
    outProperties = DAE::Properties::PROP { type_: result_ty, constFlag: c };
    Ok((outCache, outExp, outProperties))
}
...
fn elabBuiltinIdentity(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut sz: i32;
    let mut dim_size: Arc<DAE::Dimension>;
    let mut dim_exp: Arc<DAE::Exp>;
    let mut check_model: bool;
    checkBuiltinCallArgs(inPosArgs.clone(), inNamedArgs, 1, (literal!("identity")).clone(), inInfo.clone())?;
    (outCache, dim_exp, outProperties) = elabExpInExpression(inCache, inEnv.clone(), listHead(inPosArgs)?, inImplicit, true, inPrefix.clone(), inInfo.clone())?;
    let DAE::PROP { type_: __pa0, constFlag: __pa1 } = (outProperties) else { bail!("pattern mismatch") };
    ty = __pa0.clone();
    c = __pa1.clone();
    ...
    if Types::isParameterOrConstant(c) {
        ...
        match '__try2: {
            ...
            outCache = __pa3.clone();
            sz = __pa4.clone();
            dim_size = Arc::new(DAE::Dimension::DIM_INTEGER { integer: sz });
            dim_exp = Arc::new(DAE::Exp::ICONST { integer: sz });
    ...
    outExp = Expression::makePureBuiltinCall((literal!("identity")).clone(), list![dim_exp], exp_ty);
    outProperties = DAE::Properties::PROP { type_: ty, constFlag: c };
    Ok((outCache, outExp, outProperties))
}
...
fn zeroSizeOverconstrainedOperator(mut inExp: Arc<DAE::Exp>, mut inFExp: Arc<DAE::Exp>, mut inInfo: SourceInfo) -> Result<()> {
    let () = (::match_deref::match_deref! { match &(inExp) {
        Deref @ DAE::Exp::ARRAY { array: Deref @ metamodelica::List::Nil, .. } => {
            let mut s: ArcStr;
            s = (ExpressionBasics::printExpStr(inFExp)?).clone();
            Error::addSourceMessage(Error::OVERCONSTRAINED_OPERATOR_SIZE_ZERO_RETURN_FALSE.clone(), list![(s).clone()], inInfo)?;
            ()
        },
        _ => {
            ()
...
fn elabBuiltinIsRoot(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outProperties: DAE::Properties;
    let mut exp: Arc<DAE::Exp>;
    checkBuiltinCallArgs(inPosArgs.clone(), inNamedArgs, 1, (literal!("Connections.isRoot")).clone(), inInfo.clone())?;
    (outCache, exp, _) = elabExpInExpression(inCache, inEnv, listHead(inPosArgs)?, false, false, inPrefix, inInfo.clone())?;
...
fn elabBuiltinRooted(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outProperties: DAE::Properties;
    let mut exp: Arc<DAE::Exp>;
    checkBuiltinCallArgs(inPosArgs.clone(), inNamedArgs, 1, (literal!("rooted")).clone(), inInfo.clone())?;
    (outCache, exp, _) = elabExpInExpression(inCache, inEnv, listHead(inPosArgs)?, false, false, inPrefix, inInfo.clone())?;
...
fn elabBuiltinUniqueRootIndices(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inAbsynExpLst: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArg: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outExp: Arc<DAE::Exp>;
    let mut outProperties: DAE::Properties;
    (outCache, outExp, outProperties) = (::match_deref::match_deref! { match &((inCache, inEnv, inAbsynExpLst, inNamedArg, inPrefix)) {
        (cache, env, Deref @ metamodelica::List::Cons { head: aexp1, tail: Deref @ metamodelica::List::Cons { head: aexp2, tail: Deref @ metamodelica::List::Nil } }, Deref @ metamodelica::List::Nil, pre) => {
            ...
            let mut lst: Arc<metamodelica::List<Arc<DAE::Exp>>>;
            let mut dim: i32;
            let mut ty: Arc<DAE::Type>;
            let mut cache = (*cache).clone();
            ...
            exp1 = __pa2.clone();
            dim = (lst.len() as i32);
            (cache, exp2, _) = elabExpInExpression(cache.clone(), env.clone(), aexp2.clone(), false, false, pre.clone(), info)?;
            exp3 = Arc::new(DAE::Exp::SCONST { string: (literal!("")).clone() });
            ty = Arc::new(DAE::Type::T_ARRAY { ty: DAE::T_INTEGER_DEFAULT().clone(), dims: list![Arc::new(DAE::Dimension::DIM_INTEGER { integer: dim })] });
            (cache.clone(), Arc::new(DAE::Exp::CALL { path: Arc::new(Absyn::Path::QUALIFIED { name: (literal!("Connections")).clone(), path: Arc::new(Absyn::Path::IDENT { name: (literal!("uniqueRootIndices")).clone() }) }), expLst: list![exp1, exp2, exp3], attr: Arc::new(DAE::CallAttributes { ty: ty.clone(), tuple_: false, builtin: true, isImpure: false, isFunctionPointerCall: false, inlineType: openmodelica_frontend_types::DAE::InlineType::NO_INLINE, tailCall: openmodelica_frontend_types::DAE::TailCall::NO_TAIL }) }), DAE::Properties::PROP { type_: ty, constFlag: openmodelica_frontend_types::DAE::Const::C_VAR })
        },
        (cache, env, Deref @ metamodelica::List::Cons { head: aexp1, tail: Deref @ metamodelica::List::Cons { head: aexp2, tail: Deref @ metamodelica::List::Cons { head: _, tail: Deref @ metamodelica::List::Nil } } }, Deref @ metamodelica::List::Nil, pre) => {
            ...
            (cache, exp3, _) = elabExpInExpression(cache.clone(), env.clone(), aexp2.clone(), false, false, pre.clone(), info)?;
            ty = Arc::new(DAE::Type::T_ARRAY { ty: DAE::T_INTEGER_DEFAULT().clone(), dims: list![Arc::new(DAE::Dimension::DIM_INTEGER { integer: dim })] });
            (cache.clone(), Arc::new(DAE::Exp::CALL { path: Arc::new(Absyn::Path::QUALIFIED { name: (literal!("Connections")).clone(), path: Arc::new(Absyn::Path::IDENT { name: (literal!("uniqueRootIndices")).clone() }) }), expLst: list![exp1, exp2, exp3], attr: Arc::new(DAE::CallAttributes { ty: ty.clone(), tuple_: false, builtin: true, isImpure: false, isFunctionPointerCall: false, inlineType: openmodelica_frontend_types::DAE::InlineType::NO_INLINE, tailCall: openmodelica_frontend_types::DAE::TailCall::NO_TAIL }) }), DAE::Properties::PROP { type_: ty, constFlag: openmodelica_frontend_types::DAE::Const::C_VAR })
        },
...
fn elabBuiltinScalar(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outProperties: DAE::Properties;
    let mut ty: Arc<DAE::Type>;
    let mut scalar_ty: Arc<DAE::Type>;
    let mut c: DAE::Const;
    let mut dims: Arc<metamodelica::List<Arc<DAE::Dimension>>>;
    let mut ty_str: ArcStr;
    checkBuiltinCallArgs(inPosArgs.clone(), inNamedArgs, 1, (literal!("scalar")).clone(), inInfo.clone())?;
    let (__pa0, __pa1, __pa2, __pa3) = ::match_deref::match_deref! { match &(elabExpInExpression(inCache, inEnv, listHead(inPosArgs)?, inImplicit, true, inPrefix, inInfo.clone())?) {
        (__pa0, __pa1, DAE::Properties::PROP { type_: __pa2, constFlag: __pa3 }) => (__pa0.clone(), __pa1.clone(), __pa2.clone(), __pa3.clone()),
        _ => bail!("pattern mismatch"),
    } };
    outCache = __pa0.clone();
    ...
    if !(dims.is_empty()) {
        outExp = Expression::makePureBuiltinCall((literal!("scalar")).clone(), list![outExp], scalar_ty.clone());
    }
    (outExp, _) = ExpressionSimplify::simplify1(outExp)?;
    outProperties = DAE::Properties::PROP { type_: scalar_ty, constFlag: c };
    Ok((outCache, outExp, outProperties))
}
...
fn elabBuiltinString(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    match '__try0: {
        ...
        (outCache, outExp, outProperties) = unwrap_break_err!(OperatorOverloading::string(inCache.clone(), inEnv.clone(), e.clone(), inImplicit, true, inPrefix.clone(), inInfo.clone()), '__try0);
        ...
        Err(_) => {
            ...
            match '__try5: {
                ...
                (outCache, args, _, consts, _) = unwrap_break_err!(elabInputArgs(outCache.clone(), inEnv.clone(), inPosArgs.clone(), inNamedArgs.clone(), slots.clone(), false, true, inImplicit, inPrefix.clone(), inInfo.clone(), DAE::T_UNKNOWN_DEFAULT().clone(), Arc::new(Absyn::Path::IDENT { name: (literal!("String")).clone() }), false), '__try5);
                ...
                Err(_) => {
                    ...
                    (outCache, args, _, consts, _) = elabInputArgs(outCache.clone(), inEnv.clone(), inPosArgs.clone(), inNamedArgs.clone(), slots.clone(), false, true, inImplicit, inPrefix.clone(), inInfo.clone(), DAE::T_UNKNOWN_DEFAULT().clone(), Arc::new(Absyn::Path::IDENT { name: (literal!("String")).clone() }), false)?;
                }
            }
...
fn elabBuiltinGetInstanceName(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    checkBuiltinCallArgs(inPosArgs, inNamedArgs, 0, (literal!("getInstanceName")).clone(), inInfo)?;
    let FCore::CACHE { modelName: __pa0, .. } = (inCache) else { bail!("pattern mismatch") };
    name = __pa0.clone();
    if PrefixUtil::isNoPrefix(inPrefix.clone()) {
        envName = FGraph::getGraphNameNoImplicitScopes(inEnv)?;
        r#str = (if (AbsynUtil::pathEqual(envName.clone(), name.clone())) {AbsynUtil::pathLastIdent(name)?} else {AbsynUtil::pathString(envName, (literal!(".")).clone(), true, false)?}).clone();
    } else {
        r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*AbsynUtil::pathLastIdent(name)?); __mm_s.push_str(&*literal!(".")); __mm_s.push_str(&*PrefixUtil::printPrefixStr(inPrefix)?); ArcStr::from(__mm_s) }).clone();
    }
    outExp = Arc::new(DAE::Exp::SCONST { string: (r#str).clone() });
...
fn elabBuiltinIsPresent(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    checkBuiltinCallArgs(inPosArgs.clone(), inNamedArgs, 1, (literal!("isPresent")).clone(), info.clone())?;
    if !(FGraph::inFunctionScope(inEnv.clone())) {
    ...
    outExp = (::match_deref::match_deref! { match &((inPosArgs).get(1)?) {
        Deref @ Absyn::Exp::CREF { componentRef: Deref @ Absyn::ComponentRef::CREF_IDENT { name: __esc_str, .. } } => {
            ...
            let (__pa0, __pa1) = ::match_deref::match_deref! { match &(Lookup::lookupIdentLocal(outCache, inEnv, (r#str.clone()).clone())?) {
        ...
        _ => (),
            ...
            Expression::makeImpureBuiltinCall((literal!("isPresent")).clone(), metamodelica::cons(Arc::new(DAE::Exp::CREF { componentRef: Arc::new(DAE::ComponentRef::CREF_IDENT { ident: (r#str.clone()).clone(), identType: DAE::T_BOOL_DEFAULT().clone(), subscriptLst: metamodelica::nil() }), ty: DAE::T_BOOL_DEFAULT().clone() }), metamodelica::nil()), DAE::T_BOOL_DEFAULT().clone())
        },
        __esc_exp => {
            exp = (*__esc_exp).clone();
            Error::addSourceMessage(Error::IS_PRESENT_INVALID_EXP.clone(), list![(Dump::printExpStr(exp.clone())?).clone()], info)?;
            bail!("fail")
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn elabBuiltinVector(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut c: DAE::Const;
    let mut expl: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    checkBuiltinCallArgs(inPosArgs.clone(), inNamedArgs, 1, (literal!("vector")).clone(), inInfo.clone())?;
    e = listHead(inPosArgs)?;
    ...
    outProperties = __pa4.clone();
    if Types::isSimpleType(ty.clone()) {
        arr_ty = Types::liftArray(ty, Arc::new(DAE::Dimension::DIM_INTEGER { integer: 1 }));
        exp_ty = Types::simplifyType(arr_ty.clone())?;
        outExp = Arc::new(DAE::Exp::ARRAY { ty: exp_ty, scalar: true, array: list![outExp] });
        outProperties = DAE::Properties::PROP { type_: arr_ty, constFlag: c };
    } else if Expression::isArray(outExp.clone()) || Expression::isMatrix(outExp.clone()) {
        if Types::numberOfDimensions(ty.clone()) != 1 {
            checkBuiltinVectorDims(e, inEnv, ty.clone(), inPrefix, inInfo)?;
            expl = Expression::getArrayOrMatrixContents(outExp)?;
            expl = flattenArray(expl);
            el_ty = Types::arrayElementType(ty);
            arr_ty = Types::liftArray(el_ty, Arc::new(DAE::Dimension::DIM_INTEGER { integer: (expl.clone().len() as i32) }));
            outExp = Arc::new(DAE::Exp::ARRAY { ty: Types::simplifyType(arr_ty.clone())?, scalar: false, array: expl });
            outProperties = DAE::Properties::PROP { type_: arr_ty, constFlag: c };
        }
    } else {
        ty = Types::liftArray(Types::arrayElementType(ty), openmodelica_frontend_types::DAE::Dimension::interned_DIM_UNKNOWN());
        exp_ty = Types::simplifyType(ty.clone())?;
        outExp = Expression::makePureBuiltinCall((literal!("vector")).clone(), list![outExp], exp_ty);
        outProperties = DAE::Properties::PROP { type_: ty, constFlag: c };
    }
    Ok((outCache, outExp, outProperties))
}
...
fn flattenArray(mut arr: Arc<metamodelica::List<Arc<DAE::Exp>>>) -> Arc<metamodelica::List<Arc<DAE::Exp>>> {
    let mut flattenedExpl: Arc<metamodelica::List<Arc<DAE::Exp>>>;
...
pub(crate) fn elabBuiltinMatrix(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImpl: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outProperties: DAE::Properties;
    let mut ty: Arc<DAE::Type>;
    checkBuiltinCallArgs(inPosArgs.clone(), inNamedArgs, 1, (literal!("matrix")).clone(), inInfo.clone())?;
    (outCache, outExp, outProperties) = elabExpInExpression(inCache.clone(), inEnv.clone(), listHead(inPosArgs)?, inImpl, true, inPrefix, inInfo.clone())?;
    ty = Types::getPropType(outProperties.clone())?;
...
fn elabBuiltinMatrix2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inArg: Arc<DAE::Exp>, mut inProperties: DAE::Properties, mut inType: Arc<DAE::Type>, mut inInfo: SourceInfo) -> Result<(Arc<DAE::Exp>, DAE::Properties)> {
    ...
    (outExp, outProperties) = (::match_deref::match_deref! { match &(inArg.clone()) {
        ...
        Deref @ DAE::Exp::ARRAY { ty: Deref @ DAE::Type::T_ARRAY { ty: ety, dims: Deref @ metamodelica::List::Cons { head: dim1, tail: Deref @ metamodelica::List::Cons { head: dim2, tail: _ } } }, scalar, array: expl } => {
            let mut ty: Arc<DAE::Type>;
            let mut props: DAE::Properties;
            let mut expl = (*expl).clone();
            expl = List::map1(expl.clone(), (std::sync::Arc::new(elabBuiltinMatrix3) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, SourceInfo) -> Result<Arc<DAE::Exp>> + 'static>), inInfo)?;
            ...
            (Arc::new(DAE::Exp::ARRAY { ty: Arc::new(DAE::Type::T_ARRAY { ty: ety.clone(), dims: list![dim1.clone(), dim2.clone()] }), scalar: scalar.clone(), array: expl.clone() }), props)
        },
        _ => bail!("match: no arm matched"),
    } });
    Ok((outExp, outProperties))
...
fn elabBuiltinMatrix3(mut inExp: Arc<DAE::Exp>, mut inInfo: SourceInfo) -> Result<Arc<DAE::Exp>> {
    ...
    outExp = (::match_deref::match_deref! { match &(inExp) {
        Deref @ DAE::Exp::ARRAY { ty: Deref @ DAE::Type::T_ARRAY { ty: ety, dims: Deref @ metamodelica::List::Cons { head: dim, tail: _ } }, scalar, array: expl } => {
            let mut expl = (*expl).clone();
            expl = ({
        let mut __acc: Arc<metamodelica::List<Arc<DAE::Exp>>> = metamodelica::nil();
        for mut e in (expl.clone()).into_iter().cloned() {
        ...
        Deref @ DAE::Exp::MATRIX { ty: Deref @ DAE::Type::T_ARRAY { ty: ety, dims: Deref @ metamodelica::List::Cons { head: dim, tail: dims } }, matrix: matrix_expl, .. } => {
...
fn arrayScalar(mut inExp: Arc<DAE::Exp>, mut inDim: i32, mut inOperator: ArcStr, mut inInfo: SourceInfo) -> Result<Arc<DAE::Exp>> {
    '__tco: loop {
        ...
        Deref @ DAE::Exp::ARRAY { array: Deref @ metamodelica::List::Cons { head: exp, tail: Deref @ metamodelica::List::Nil }, .. } => {
            { (inExp, inDim, inOperator, inInfo) = (exp.clone(), inDim + 1, (inOperator).clone(), inInfo); continue '__tco; }
        },
        Deref @ DAE::Exp::ARRAY { array: expl, .. } => {
            let mut dim_str: ArcStr;
            let mut size_str: ArcStr;
            dim_str = (intString(inDim)).clone();
            size_str = (intString((expl.clone().len() as i32))).clone();
            Error::addSourceMessage(Error::INVALID_ARRAY_DIM_IN_CONVERSION_OP.clone(), list![(dim_str).clone(), (inOperator).clone(), (literal!("1")).clone(), (size_str).clone()], inInfo)?;
            return Ok(bail!("fail"))
        },
        Deref @ DAE::Exp::MATRIX { ty, matrix: Deref @ metamodelica::List::Cons { head: expl, tail: Deref @ metamodelica::List::Nil }, .. } => {
            { (inExp, inDim, inOperator, inInfo) = (Arc::new(DAE::Exp::ARRAY { ty: ty.clone(), scalar: true, array: expl.clone() }), inDim + 1, (inOperator).clone(), inInfo); continue '__tco; }
        },
...
pub(crate) fn elabBuiltinHandler(mut inIdent: ArcStr) -> Result<Arc<dyn ::std::ops::Fn(FCore::Cache, FCore::Graph, Arc<metamodelica::List<Arc<Absyn::Exp>>>, Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, bool, DAE::Prefix, SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> + 'static>> {
    ...
    outHandler = (::match_deref::match_deref! { match &(inIdent) {
...
fn elabCallBuiltin(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inFnName: Arc<Absyn::ComponentRef>, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    '__tco: loop {
        ::match_deref::match_deref! { match &(inFnName.clone()) {
        Deref @ Absyn::ComponentRef::CREF_IDENT { subscripts: Deref @ metamodelica::List::Nil, .. } => {
            ...
            return Ok(handler(inCache, inEnv, inPosArgs, inNamedArgs, inImplicit, inPrefix, inInfo)?)
        },
        Deref @ Absyn::ComponentRef::CREF_QUAL { name: Deref @ "Connections", componentRef: Deref @ Absyn::ComponentRef::CREF_IDENT { name: Deref @ "isRoot", .. }, .. } => {
            return Ok(elabBuiltinIsRoot(inCache, inEnv, inPosArgs, inNamedArgs, inImplicit, inPrefix, inInfo)?)
        },
        Deref @ Absyn::ComponentRef::CREF_QUAL { name: Deref @ "Connections", componentRef: Deref @ Absyn::ComponentRef::CREF_IDENT { name: Deref @ "uniqueRootIndices", .. }, .. } => {
            ...
            return Ok(elabBuiltinUniqueRootIndices(inCache, inEnv, inPosArgs, inNamedArgs, inImplicit, inPrefix, inInfo)?)
        },
        Deref @ Absyn::ComponentRef::CREF_QUAL { name: Deref @ "Connections", componentRef: Deref @ Absyn::ComponentRef::CREF_IDENT { name: Deref @ "rooted", .. }, .. } => {
            return Ok(elabBuiltinRooted(inCache, inEnv, inPosArgs, inNamedArgs, inImplicit, inPrefix, inInfo)?)
        },
        Deref @ Absyn::ComponentRef::CREF_FULLYQUALIFIED { componentRef: cr } => {
            { (inCache, inEnv, inFnName, inPosArgs, inNamedArgs, inImplicit, inPrefix, inInfo) = (inCache, inEnv, cr.clone(), inPosArgs, inNamedArgs, inImplicit, inPrefix, inInfo); continue '__tco; }
        },
        _ => return Err(anyhow::anyhow!("match: no arm matched")),
    } }
    }
...
fn elabCall(mut cache: FCore::Cache, mut env: FCore::Graph, mut r#fn: Arc<Absyn::ComponentRef>, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut typeVars: Arc<metamodelica::List<Arc<Absyn::Path>>>, mut r#impl: bool, mut pre: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut handles: Arc<metamodelica::List<i32>>;
    let mut name: ArcStr;
    let mut s: ArcStr;
    let mut s1: ArcStr;
    let mut s2: ArcStr;
    ...
    let mut argstrs: Arc<metamodelica::List<ArcStr>>;
    if hasBuiltInHandler(r#fn.clone()) {
        match '__try0: {
            (cache, e, prop) = unwrap_break_err!(elabCallBuiltin(cache.clone(), env.clone(), r#fn.clone(), args.clone(), nargs.clone(), r#impl, pre.clone(), info.clone()), '__try0);
            return Ok((cache.clone(), e.clone(), prop.clone()));
            Ok::<(), anyhow::Error>(())
        } {
            Ok(()) => {}
            Err(__try0_err) => {
                let true = (numErrorMessages == Error::getNumErrorMessages()) else { bail!("pattern mismatch") };
    ...
    match '__try1: {
        ...
        fn_1 = unwrap_break_err!(AbsynUtil::crefToPath(r#fn.clone()), '__try1);
        (cache, e, prop) = unwrap_break_err!(elabCallArgs(cache.clone(), env.clone(), fn_1.clone(), args.clone(), nargs.clone(), typeVars.clone(), r#impl, pre.clone(), info.clone()), '__try1);
        ...
        Err(_) => {
            ...
            (cache, e, prop) = BackendCevalInterface::elabCallInteractive(cache.clone(), env.clone(), r#fn.clone(), args.clone(), nargs.clone(), r#impl, pre.clone(), info.clone())?;
        }
    }
    Ok((cache, e, prop))
}

fn hasBuiltInHandler(mut r#fn: Arc<Absyn::ComponentRef>) -> bool {
    let mut b: bool;
    b = 'mc: {
        let __mc_input = r#fn;
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ Absyn::ComponentRef::CREF_IDENT { name, subscripts: Deref @ metamodelica::List::Nil } => {
...
fn isValidDerVariableName(mut exp: Arc<Absyn::Exp>, mut nested: bool) -> bool {
    '__tco: loop {
        ...
        Deref @ Absyn::Exp::CALL { function_: Deref @ Absyn::ComponentRef::CREF_IDENT { name: Deref @ "der", .. }, functionArgs: Deref @ Absyn::FunctionArgs::FUNCTIONARGS { args: Deref @ metamodelica::List::Cons { head: __esc_arg, tail: Deref @ metamodelica::List::Nil }, argNames: Deref @ metamodelica::List::Nil }, .. } => {
            arg = (*__esc_arg).clone();
...
pub fn getOptionalNamedArgExpList(mut name: ArcStr, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>) -> Arc<metamodelica::List<Arc<DAE::Exp>>> {
    ...
    out = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Deref @ Absyn::NamedArg { argName, argValue: Deref @ Absyn::Exp::ARRAY { arrayExp: absynExpList } }, tail: _ } => {
...
fn absynExpListToDaeExpList(mut absynExpList: Arc<metamodelica::List<Arc<Absyn::Exp>>>) -> Result<Arc<metamodelica::List<Arc<DAE::Exp>>>> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: Deref @ Absyn::Exp::CREF { componentRef: absynCr }, tail: absynRest } => {
            let mut daeExpList: Arc<metamodelica::List<Arc<DAE::Exp>>>;
            let mut absynPath: Arc<Absyn::Path>;
            let mut daeCr: Arc<DAE::ComponentRef>;
        ...
        Deref @ metamodelica::List::Cons { head: _, tail: absynRest } => {
            { absynExpList = absynRest.clone(); continue '__tco; }
        },
        _ => return Err(anyhow::anyhow!("match: no arm matched")),
    } }
    }
    ...
    for mut arg in &*inArgs {
        ...
        if name.clone() == inArgName.clone() {
            if '__try1: {
                ...
                e = __pa2.clone();
                let (__pa3, __pa4, __pa5) = ::match_deref::match_deref! { match &(unwrap_break_err!(elabExpInExpression(inCache.clone(), inEnv.clone(), e.clone(), inImplicit, true, inPrefix.clone(), inInfo.clone()), '__try1)) {
...
pub fn elabUntypedCref(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inCref: Arc<Absyn::ComponentRef>, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::ComponentRef>)> {
    ...
    outCref = (::match_deref::match_deref! { match &(inCref.clone()) {
        Deref @ Absyn::ComponentRef::CREF_IDENT { .. } => {
            ...
            (outCache, subs, _) = elabSubscripts(inCache, inEnv, var_field!((*inCref).subscripts, Absyn::ComponentRef::CREF_IDENT).clone(), inImplicit, inPrefix, inInfo)?;
            ComponentReferenceBasics::makeCrefIdent((var_field!((*inCref).name, Absyn::ComponentRef::CREF_IDENT).clone()).clone(), DAE::T_UNKNOWN_DEFAULT().clone(), subs)
        },
        Deref @ Absyn::ComponentRef::CREF_QUAL { .. } => {
            let mut subs: Arc<metamodelica::List<Arc<DAE::Subscript>>>;
            let mut cr: Arc<DAE::ComponentRef>;
            (outCache, subs, _) = elabSubscripts(inCache, inEnv.clone(), var_field!((*inCref).subscripts, Absyn::ComponentRef::CREF_QUAL).clone(), inImplicit, inPrefix.clone(), inInfo.clone())?;
            (outCache, cr) = elabUntypedCref(outCache, inEnv, var_field!((*inCref).componentRef, Absyn::ComponentRef::CREF_QUAL).clone(), inImplicit, inPrefix, inInfo)?;
            ComponentReferenceBasics::makeCrefQual((var_field!((*inCref).name, Absyn::ComponentRef::CREF_QUAL).clone()).clone(), DAE::T_UNKNOWN_DEFAULT().clone(), subs, cr)
        },
        _ => bail!("match: no arm matched"),
    } });
    Ok((outCache, outCref))
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (newf, oldf) => {
                    ...
                    nfmt = __pa0.clone();
                    let true = (realGt(buildTime, nfmt.clone())) else { bail!("pattern mismatch") };
                    Ok(false)
                }
                _ => bail!("nomatch"),
            }}
...
fn createDummyFarg(mut name: ArcStr) -> Arc<DAE::FuncArg> {
    ...
    farg = Arc::new(DAE::FuncArg { name: (name).clone(), ty: DAE::T_UNKNOWN_DEFAULT().clone(), r#const: openmodelica_frontend_types::DAE::Const::C_VAR, par: openmodelica_frontend_types::DAE::VarParallelism::NON_PARALLEL, defaultBinding: None });
    farg
}
...
fn elabCallArgsEvaluateArrayLength(mut inCache: FCore::Cache, mut env: FCore::Graph, mut inProperties: DAE::Properties, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> (FCore::Cache, DAE::Properties) {
    ...
    let mut ty: Arc<DAE::Type>;
    match '__try0: {
        let true = (FGraph::checkScopeType(list![unwrap_break_err!(FGraph::lastScopeRef(env.clone()), '__try0)], Some(openmodelica_frontend_dump::FCore::ScopeType::CLASS_SCOPE))) else { break '__try0 Err::<_, _>(anyhow::anyhow!("pattern mismatch")) };
        ty = unwrap_break_err!(Types::getPropType(inProperties.clone()), '__try0);
        let (__pa1, (__pa2, _)) = unwrap_break_err!(Types::traverseType(ty.clone(), (inCache.clone(), env.clone()), (std::sync::Arc::new(fnptr!(elabCallArgsEvaluateArrayLength2, Arc<DAE::Type>, (FCore::Cache, FCore::Graph))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Type>, (FCore::Cache, FCore::Graph)) -> Result<(Arc<DAE::Type>, (FCore::Cache, FCore::Graph))> + 'static>)), '__try0);
        ty = __pa1.clone();
        outCache = __pa2.clone();
...
fn elabCallArgsEvaluateArrayLength2(mut ty: Arc<DAE::Type>, mut inTpl: (FCore::Cache, FCore::Graph)) -> (Arc<DAE::Type>, (FCore::Cache, FCore::Graph)) {
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Type::T_ARRAY { .. }, tpl) => {
                    ...
                    (dims, tpl) = List::mapFold(var_field!((*oty).dims, DAE::Type::T_ARRAY).clone(), (std::sync::Arc::new(fnptr!(elabCallArgsEvaluateArrayLength3, Arc<DAE::Dimension>, (FCore::Cache, FCore::Graph))) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Dimension>, (FCore::Cache, FCore::Graph)) -> Result<(Arc<DAE::Dimension>, (FCore::Cache, FCore::Graph))> + 'static>), tpl.clone())?;
...
fn elabCallArgsEvaluateArrayLength3(mut inDim: Arc<DAE::Dimension>, mut inTpl: (FCore::Cache, FCore::Graph)) -> (Arc<DAE::Dimension>, (FCore::Cache, FCore::Graph)) {
...
fn createInputVariableReplacements(mut inSlotLst: Arc<metamodelica::List<Slot>>, mut inVarsRepl: VarTransform::VariableReplacements) -> Result<VarTransform::VariableReplacements> {
    ...
    outVarsRepl = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Slot { defaultArg: Deref @ DAE::FuncArg { name: id, .. }, slotFilled: true, arg: Some(e), .. }, tail: rest } => {
...
fn elabCallArgs2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPath: Arc<Absyn::Path>, mut inAbsynExpLst: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inAbsynNamedArgLst: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut typeVars: Arc<metamodelica::List<Arc<Absyn::Path>>>, mut inBoolean: bool, mut stopElab: Mutable::Mutable<bool>, mut inPrefix: DAE::Prefix, mut info: SourceInfo, mut numErrors: i32) -> Result<(FCore::Cache, Option<(Arc<DAE::Exp>, DAE::Properties)>)> {
    ...
    (outCache, expProps) = 'mc: {
        let __mc_input = (inCache, inEnv, inPath, inAbsynExpLst, inAbsynNamedArgLst, inBoolean, inPrefix);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, r#fn, args, nargs, r#impl, pre) => {
                    ...
                    let mut slots: Arc<metamodelica::List<Slot>>;
                    let mut newslots: Arc<metamodelica::List<Slot>>;
                    let mut newslots2: Arc<metamodelica::List<Slot>>;
                    let mut args_2: Arc<metamodelica::List<Arc<DAE::Exp>>>;
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, r#fn, args, nargs, r#impl, pre) => {
                    ...
                    let mut newslots2: Arc<metamodelica::List<Slot>>;
                    let mut args_2: Arc<metamodelica::List<Arc<DAE::Exp>>>;
                    let mut constlist: Arc<metamodelica::List<DAE::Const>>;
                    let mut constInputArgs: Arc<metamodelica::List<DAE::Const>>;
                    let mut r#const: DAE::Const;
                    let mut tyconst: Arc<DAE::TupleConst>;
                    let mut prop: DAE::Properties;
                    let mut prop_1: DAE::Properties;
                    let mut path: Arc<Absyn::Path>;
                    let mut vect_dims: Arc<metamodelica::List<Arc<DAE::Dimension>>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, r#fn, args, nargs, r#impl, pre) => {
                    ...
                    Error::assertionOrAddSourceMessage(Error::getNumErrorMessages() != numErrors, Error::NO_MATCHING_FUNCTION_FOUND.clone(), list![(fn_str.clone()).clone(), (pre_str.clone()).clone(), (types_str.clone()).clone()], info.clone())?;
                    ErrorExt::delCheckpoint((literal!("elabCallArgs2FunctionLookup")).clone());
                    Ok((cache.clone(), None))
                }
                _ => bail!("nomatch"),
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, r#fn, _, _, _, pre) => {
                    let mut typelist: Arc<metamodelica::List<Arc<DAE::Type>>>;
                    let mut t_lst: Arc<metamodelica::List<ArcStr>>;
                    let mut fn_str: ArcStr;
                    let mut types_str: ArcStr;
                    let mut pre_str: ArcStr;
                    let mut cache = (*cache).clone();
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, r#fn, Deref @ metamodelica::List::Cons { head: Deref @ Absyn::Exp::CREF { componentRef: Deref @ Absyn::ComponentRef::CREF_IDENT { name, subscripts: _ } }, tail: Deref @ metamodelica::List::Nil }, _, r#impl, pre) => {
                    ...
                    let mut tp: Arc<DAE::Type>;
                    let mut cref: Arc<Absyn::ComponentRef>;
                    let mut daecref: Arc<DAE::ComponentRef>;
                    let mut cache = (*cache).clone();
                    let mut expProps: Option<(Arc<DAE::Exp>, DAE::Properties)> = expProps.clone();
...
pub(crate) fn elabCallArgs3(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut typelist: Arc<metamodelica::List<Arc<DAE::Type>>>, mut r#fn: Arc<Absyn::Path>, mut args: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut nargs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut typeVars: Arc<metamodelica::List<Arc<Absyn::Path>>>, mut r#impl: bool, mut pre: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Option<(Arc<DAE::Exp>, DAE::Properties)>)> {
    ...
    let mut vect_dims: Arc<metamodelica::List<Arc<DAE::Dimension>>>;
    let mut slots: Arc<metamodelica::List<Slot>>;
    let mut slots2: Arc<metamodelica::List<Slot>>;
    ...
    let (__pa0, __pa1, __pa2, __pa3, __pa8, __pa4, __pa5, __pa6, __pa7, __pa9, __pa10) = ::match_deref::match_deref! { match &(elabTypes(inCache, inEnv.clone(), args, nargs, typeVars, typelist, onlyOneFunction, true, r#impl, pre, info.clone())?) {
        (__pa0, __pa1, __pa2, __pa3, __pa8 @ Deref @ DAE::Type::T_FUNCTION { functionAttributes: DAE::FunctionAttributes { purity: __pa4, inline: __pa5, isFunctionPointer: __pa6, functionParallelism: __pa7, .. }, .. }, __pa9, __pa10) => (__pa0.clone(), __pa1.clone(), __pa2.clone(), __pa3.clone(), __pa8.clone(), __pa4.clone(), __pa5.clone(), __pa6.clone(), __pa7.clone(), __pa9.clone(), __pa10.clone()),
        _ => bail!("pattern mismatch"),
    } };
    cache = __pa0.clone();
    args_1 = __pa1.clone();
    ...
    restype = __pa3.clone();
    purity = __pa4.clone();
    inlineType = __pa5.clone();
    isFunctionPointer = __pa6.clone();
    funcParal = __pa7.clone();
    functype = __pa8.clone();
    vect_dims = __pa9.clone();
    slots = __pa10.clone();
    isImpure = purity == DAE::Purity::IMPURE.clone();
    (fn_1, functype) = deoverloadFuncname(r#fn.clone(), functype, inEnv.clone());
    ...
    let true = (isValidWRTParallelScope(r#fn.clone(), builtin, funcParal, inEnv.clone(), info.clone())?) else { bail!("pattern mismatch") };
    r#const = List::fold(constlist, (std::sync::Arc::new(fnptr!(Types::constAnd, DAE::Const, DAE::Const)) as std::sync::Arc<dyn ::std::ops::Fn(DAE::Const, DAE::Const) -> Result<DAE::Const> + 'static>), openmodelica_frontend_types::DAE::Const::C_CONST)?;
    r#const = if (Flags::isSet(Flags::RML.clone())? && !(builtin) || purity == DAE::Purity::OM_IMPURE.clone()) {openmodelica_frontend_types::DAE::Const::C_VAR} else {r#const};
    (cache, r#const) = determineConstSpecialFunc(cache, inEnv.clone(), r#const, fn_1.clone())?;
    ...
    callExp = Arc::new(DAE::Exp::CALL { path: fn_1.clone(), expLst: args_2, attr: Arc::new(DAE::CallAttributes { ty: tp, tuple_: tuple_, builtin: builtin, isImpure: isImpure || purity == DAE::Purity::OM_IMPURE.clone(), isFunctionPointerCall: isFunctionPointer, inlineType: inlineType, tailCall: openmodelica_frontend_types::DAE::TailCall::NO_TAIL }) });
    (call_exp, prop_1) = vectorizeCall(callExp, vect_dims, slots2, prop, info.clone())?;
    (cache, status) = instantiateDaeFunction(cache.clone(), inEnv.clone(), if (Lookup::isFunctionCallViaComponent(cache, inEnv.clone(), r#fn.clone())) {r#fn} else {fn_1}, builtin, None, true);
    cache = instantiateImplicitRecordConstructors(cache, inEnv.clone(), args_1);
    functionTree = FCore::getFunctionTree(cache.clone());
    (call_exp, _, didInline, _) = Inline::inlineExp(call_exp, (Some(functionTree), list![openmodelica_frontend_types::DAE::InlineType::BUILTIN_EARLY_INLINE, openmodelica_frontend_types::DAE::InlineType::EARLY_INLINE]), DAE::emptyElementSource().clone());
    (call_exp, _) = ExpressionSimplify::condsimplify(didInline, call_exp)?;
    didInline = didInline && !(Config::acceptMetaModelicaGrammar()?);
    prop_1 = if (didInline) {Types::setPropType(prop_1, restype)?} else {prop_1};
    if !(isImpure) {
        (cache, call_exp, prop_1) = Ceval::cevalIfConstant(cache, inEnv, call_exp, prop_1, r#impl, info)?;
    }
    expProps = if (Util::isSuccess(status)?) {Some((call_exp, prop_1))} else {None};
    outCache = cache;
    Ok((outCache, expProps))
}
...
fn elabCallArgsMetarecord(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inType: Arc<DAE::Type>, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut stopElab: Mutable::Mutable<bool>, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Option<(Arc<DAE::Exp>, DAE::Properties)>)> {
    ...
    (outCache, expProps) = { let mut ty = inType.clone(); 'mc: {
        ...
        if let Ok((__v, __wb0, __wb1)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Type::T_METARECORD { path: fq_path, .. } => {
                    ...
                    let mut fargs: Arc<metamodelica::List<Arc<DAE::FuncArg>>>;
                    let mut slots: Arc<metamodelica::List<Slot>>;
                    let mut const_lst: Arc<metamodelica::List<DAE::Const>>;
                    let mut r#const: DAE::Const;
        ...
        __acc.reverse()
                    ...
                    slots = makeEmptySlots(fargs.clone())?;
                    (outCache, _, slots, const_lst, bindings) = elabInputArgs(inCache.clone(), inEnv.clone(), inPosArgs.clone(), inNamedArgs.clone(), slots.clone(), true, true, inImplicit, inPrefix.clone(), inInfo.clone(), inType.clone(), var_field!((*inType).utPath, DAE::Type::T_METARECORD).clone(), false)?;
...
pub fn instantiateDaeFunction(mut inCache: FCore::Cache, mut env: FCore::Graph, mut name: Arc<Absyn::Path>, mut builtin: bool, mut clOpt: Option<Arc<SCode::Element>>, mut printErrorMsg: bool) -> (FCore::Cache, Util::Status) {
    let mut outCache: FCore::Cache;
    let mut status: Util::Status;
    (outCache, status) = instantiateDaeFunction2(inCache, env, name, builtin, clOpt, printErrorMsg, crate::Static::ForceFunctionInst::NORMAL_FUNCTION_INST);
    (outCache, status)
}
...
pub fn instantiateDaeFunctionForceInst(mut inCache: FCore::Cache, mut env: FCore::Graph, mut name: Arc<Absyn::Path>, mut builtin: bool, mut clOpt: Option<Arc<SCode::Element>>, mut printErrorMsg: bool) -> (FCore::Cache, Util::Status) {
    let mut outCache: FCore::Cache;
    let mut status: Util::Status;
    (outCache, status) = instantiateDaeFunction2(inCache, env, name, builtin, clOpt, printErrorMsg, crate::Static::ForceFunctionInst::FORCE_FUNCTION_INST);
    (outCache, status)
}

fn instantiateDaeFunction2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inName: Arc<Absyn::Path>, mut builtin: bool, mut clOpt: Option<Arc<SCode::Element>>, mut printErrorMsg: bool, mut forceFunctionInst: ForceFunctionInst) -> (FCore::Cache, Util::Status) {
    let mut outCache: FCore::Cache = FCore::Cache::NO_CACHE;
    let mut status: Util::Status;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, true, _) => {
                    ...
                    let true = (Error::getNumErrorMessages() == numError) else { bail!("pattern mismatch") };
...
fn lookupAndFullyQualify(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inFunctionName: Arc<Absyn::Path>) -> Result<(FCore::Cache, FCore::Graph, Arc<SCode::Element>, Arc<Absyn::Path>)> {
    ...
    if Lookup::isFunctionCallViaComponent(inCache.clone(), inEnv.clone(), inFunctionName.clone()) {
        (_, outClass, outEnv) = Lookup::lookupClass(inCache.clone(), inEnv, inFunctionName, None)?;
...
fn instantiateImplicitRecordConstructors(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut args: Arc<metamodelica::List<Arc<DAE::Exp>>>) -> FCore::Cache {
    let mut outCache: FCore::Cache;
    outCache = 'mc: {
        let __mc_input = args;
        if let Ok(__v) = (|| -> Result<_> {
...
fn addDefaultArgs(mut inSlots: Arc<metamodelica::List<Slot>>, mut inInfo: SourceInfo) -> Result<(Arc<metamodelica::List<Arc<DAE::Exp>>>, Arc<metamodelica::List<Slot>>)> {
    let mut outArgs: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    let mut outSlots: Arc<metamodelica::List<Slot>>;
    (outArgs, outSlots) = List::map2_2(inSlots.clone(), (std::sync::Arc::new(fillDefaultSlot) as std::sync::Arc<dyn ::std::ops::Fn(Slot, metamodelica::Array<Slot>, SourceInfo) -> Result<(Arc<DAE::Exp>, Slot)> + 'static>), metamodelica::arrayFromVec(inSlots.into_iter().cloned().collect()), inInfo)?;
    Ok((outArgs, outSlots))
}
...
fn fillDefaultSlot(mut inSlot: Slot, mut inSlotArray: metamodelica::Array<Slot>, mut inInfo: SourceInfo) -> Result<(Arc<DAE::Exp>, Slot)> {
    ...
    (outArg, outSlot) = (::match_deref::match_deref! { match &(inSlot.clone()) {
        ...
        Slot { slotFilled: false, defaultArg: Deref @ DAE::FuncArg { defaultBinding: Some(_), .. }, idx, .. } => {
            fillDefaultSlot2(({let __elt = inSlotArray.borrow()[(idx.clone()-1) as usize].clone(); __elt}), inSlotArray.clone(), inInfo)?
        },
        Slot { defaultArg: Deref @ DAE::FuncArg { name: id, .. }, .. } => {
            Error::addSourceMessage(Error::UNFILLED_SLOT.clone(), list![(id.clone()).clone()], inInfo)?;
            bail!("fail")
        },
        _ => bail!("match: no arm matched"),
    } });
...
fn fillDefaultSlot2(mut inSlot: Slot, mut inSlotArray: metamodelica::Array<Slot>, mut inInfo: SourceInfo) -> Result<(Arc<DAE::Exp>, Slot)> {
    ...
    (outArg, outSlot) = (::match_deref::match_deref! { match &(outSlot.clone()) {
        ...
        Slot { defaultArg: Deref @ DAE::FuncArg { name: id, .. }, evalStatus: 1, .. } => {
            Error::addSourceMessage(Error::CYCLIC_DEFAULT_VALUE.clone(), list![(id.clone()).clone()], inInfo)?;
            bail!("fail")
        },
...
fn evaluateSlotExp(mut inExp: Arc<DAE::Exp>, mut inSlotArray: metamodelica::Array<Slot>, mut inInfo: SourceInfo) -> Result<Arc<DAE::Exp>> {
    let mut outExp: Arc<DAE::Exp>;
    (outExp, _) = Expression::traverseExpBottomUp(inExp, (std::sync::Arc::new(evaluateSlotExp_traverser) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>, (metamodelica::Array<Slot>, SourceInfo)) -> Result<(Arc<DAE::Exp>, (metamodelica::Array<Slot>, SourceInfo))> + 'static>), (inSlotArray.clone(), inInfo))?;
    Ok(outExp)
}
...
fn isSlotNamed(mut inName: ArcStr, mut inSlot: Slot) -> Result<bool> {
    ...
    let __pa0 = ::match_deref::match_deref! { match &(inSlot) {
        Slot { defaultArg: Deref @ DAE::FuncArg { name: __pa0, .. }, .. } => __pa0.clone(),
        _ => bail!("pattern mismatch"),
    } };
    id = __pa0.clone();
    outIsNamed = stringEq((id).clone(), (inName).clone());
    Ok(outIsNamed)
}
...
fn getOptSlotDefaultExp(mut inSlot: Option<Slot>, mut inSlots: metamodelica::Array<Slot>, mut inInfo: SourceInfo, mut inOrigExp: Arc<DAE::Exp>) -> Result<Arc<DAE::Exp>> {
    ...
    outExp = (match inSlot {
        Some(mut slot) => {
            let mut exp: Arc<DAE::Exp>;
            (exp, _) = fillDefaultSlot(slot.clone(), inSlots.clone(), inInfo)?;
            exp
        },
        None => {
            inOrigExp
...
fn determineConstSpecialFunc(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inConst: DAE::Const, mut inFuncName: Arc<Absyn::Path>) -> Result<(FCore::Cache, DAE::Const)> {
    ...
    outConst = if (is_ext) {openmodelica_frontend_types::DAE::Const::C_VAR} else {inConst};
...
fn vectorizeCall(mut inExp: Arc<DAE::Exp>, mut inDims: Arc<metamodelica::List<Arc<DAE::Dimension>>>, mut inSlots: Arc<metamodelica::List<Slot>>, mut inProperties: DAE::Properties, mut info: SourceInfo) -> Result<(Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut outProperties: DAE::Properties;
    (outExp, outProperties) = 'mc: {
        let __mc_input = (inExp.clone(), inDims.clone(), inProperties);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (e, Deref @ metamodelica::List::Cons { head: Deref @ DAE::Dimension::DIM_UNKNOWN { .. }, tail: ad }, prop) => {
                    ...
                    Ok(vectorizeCall(e.clone(), metamodelica::cons(Arc::new(DAE::Dimension::DIM_INTEGER { integer: 1 }), ad.clone()), inSlots.clone(), prop.clone(), info.clone())?)
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::CALL { path: r#fn, expLst: es, attr }, Deref @ metamodelica::List::Cons { head: dim, tail: ad }, prop @ DAE::Properties::PROP { type_: tp, constFlag: c }) => {
                    ...
                    let mut riters: Arc<metamodelica::List<Arc<DAE::ReductionIterator>>>;
...
fn vectorizeCallUnknownDimension(mut inEs: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut inSlots: Arc<metamodelica::List<Slot>>, mut info: SourceInfo) -> Result<(Arc<metamodelica::List<Arc<DAE::Exp>>>, Arc<metamodelica::List<Arc<DAE::ReductionIterator>>>)> {
    ...
    for mut e in &*inEs {
        ...
        let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(rest_slots.clone()) {
            Deref @ metamodelica::List::Cons { head: Slot { dims: __pa0, defaultArg: Deref @ DAE::FuncArg { ty: __pa1, .. }, .. }, tail: __pa2 } => (__pa0.clone(), __pa1.clone(), __pa2.clone()),
            _ => bail!("pattern mismatch"),
        } };
        dims = __pa0.clone();
        ty = __pa1.clone();
        ...
        if dims.clone().is_empty() {
            oes = metamodelica::cons(e.clone(), oes.clone());
        } else {
            name = (Util::getTempVariableIndex()).clone();
            tp = Types::expTypetoTypesType(Expression::r#typeof(e.clone())?);
            ofound = metamodelica::cons(Arc::new(DAE::ReductionIterator { id: (name.clone()).clone(), exp: e.clone(), guardExp: None, ty: tp.clone() }), ofound.clone());
            oes = metamodelica::cons(Arc::new(DAE::Exp::CREF { componentRef: Arc::new(DAE::ComponentRef::CREF_IDENT { ident: (name.clone()).clone(), identType: ty.clone(), subscriptLst: metamodelica::nil() }), ty: ty.clone() }), oes.clone());
        }
    }
    if ofound.clone().is_empty() {
        Error::addSourceMessageAndFail(Error::INTERNAL_ERROR.clone(), list![(literal!("Static.vectorizeCallUnknownDimension could not find any slot to vectorize")).clone()], info)?;
        unreachable!("Error.addSourceMessageAndFail always fails — caller-side flow-analysis hint");
    }
    oes = oes.reverse();
...
fn vectorizeCallArray(mut inExp: Arc<DAE::Exp>, mut inDim: i32, mut inSlots: Arc<metamodelica::List<Slot>>) -> Result<Arc<DAE::Exp>> {
    ...
    ty = __pa0.clone();
    expl = __pa1.clone();
    expl = vectorizeCallArray2(expl, ty.clone(), inDim, inSlots)?;
    sc = Expression::typeBuiltin(ty.clone());
    ty = Expression::liftArrayRight(ty, Arc::new(DAE::Dimension::DIM_INTEGER { integer: inDim }));
    outExp = Arc::new(DAE::Exp::ARRAY { ty: ty, scalar: sc, array: expl });
    Ok(outExp)
}
...
fn vectorizeCallArray2(mut inExpl: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut inType: Arc<DAE::Type>, mut inDim: i32, mut inSlots: Arc<metamodelica::List<Slot>>) -> Result<Arc<metamodelica::List<Arc<DAE::Exp>>>> {
    ...
    outExpl = ({
        ...
        for mut e in (inExpl).into_iter().cloned() {
            let __x = (::match_deref::match_deref! { match &(e.clone()) {
        Deref @ DAE::Exp::CALL { .. } => vectorizeCallScalar(e.clone(), inType.clone(), inDim, inSlots.clone())?,
        Deref @ DAE::Exp::ARRAY { .. } => vectorizeCallArray(e.clone(), inDim, inSlots.clone())?,
        _ => bail!("match: no arm matched"),
    } });
            __acc = cons(__x, __acc);
        }
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Exp::CALL { .. } => {
                    ...
                    let mut e_type: Arc<DAE::Type>;
                    let mut arr_type: Arc<DAE::Type>;
                    expl = vectorizeCallScalar2(var_field!((*exp).path, DAE::Exp::CALL).clone(), var_field!((*exp).expLst, DAE::Exp::CALL).clone(), var_field!((*exp).attr, DAE::Exp::CALL).clone(), slots.clone(), dim)?;
                    e_type = Expression::unliftArray(ty.clone())?;
                    scalar = Expression::typeBuiltin(e_type.clone());
                    arr_type = Arc::new(DAE::Type::T_ARRAY { ty: e_type.clone(), dims: list![Arc::new(DAE::Dimension::DIM_INTEGER { integer: dim })] });
...
fn vectorizeCallScalar2(mut r#fn: Arc<Absyn::Path>, mut exps: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut attr: Arc<DAE::CallAttributes>, mut slots: Arc<metamodelica::List<Slot>>, mut dim: i32) -> Result<Arc<metamodelica::List<Arc<DAE::Exp>>>> {
    ...
    let mut callargs: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    for mut cur_dim in ({let __s=dim; let __e=1; (0i32..).map(move |__k| __s + __k * (-1)).take_while(move |&__v| __v >= __e)}) {
...
fn vectorizeCallScalar3(mut inExpl: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut inSlots: Arc<metamodelica::List<Slot>>, mut inIndex: i32) -> Result<Arc<metamodelica::List<Arc<DAE::Exp>>>> {
    ...
    for mut e in &*inExpl {
        ...
        if !(dims.clone().is_empty()) {
            e = Expression::makeASUB(e.clone(), list![Arc::new(DAE::Exp::ICONST { integer: inIndex })])?;
...
fn elabTypes(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut typeVars: Arc<metamodelica::List<Arc<Absyn::Path>>>, mut inTypes: Arc<metamodelica::List<Arc<DAE::Type>>>, mut inOnlyOneFunction: bool, mut inCheckTypes: bool, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<metamodelica::List<Arc<DAE::Exp>>>, Arc<metamodelica::List<DAE::Const>>, Arc<DAE::Type>, Arc<DAE::Type>, Arc<metamodelica::List<Arc<DAE::Dimension>>>, Arc<metamodelica::List<Slot>>)> {
    ...
    let mut params: Arc<metamodelica::List<Arc<DAE::FuncArg>>>;
    let mut res_ty: Arc<DAE::Type>;
    let mut func_ty: Arc<DAE::Type>;
    let mut func_attr: DAE::FunctionAttributes;
    let mut slots: Arc<metamodelica::List<Slot>>;
    let mut pb: Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>;
    let mut path: Arc<Absyn::Path>;
    ...
    if (rest_tys.clone().len() as i32) > 1 {
        ...
        Deref @ DAE::Type::T_FUNCTION { .. } => numArgs <= (var_field!((*ty).funcArg, DAE::Type::T_FUNCTION).clone().len() as i32) && numArgs >= ({
        let mut __acc: i32 = 0;
    ...
    while !(success) {
        ...
        if '__try6: {
            ...
            (outCache, outArgs, outSlots, outConsts, pb) = unwrap_break_err!(elabInputArgs(inCache.clone(), inEnv.clone(), inPosArgs.clone(), inNamedArgs.clone(), slots.clone(), inOnlyOneFunction, inCheckTypes, inImplicit, inPrefix.clone(), inInfo.clone(), func_ty.clone(), path.clone(), false), '__try6);
        ...
        DAE::FunctionBuiltin::FUNCTION_BUILTIN { unboxArgs: true, .. } => (unwrap_break_err!(List::map(outArgs.clone(), (std::sync::Arc::new(fnptr!(Expression::unboxExp, Arc<DAE::Exp>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<Arc<DAE::Exp>> + 'static>)), '__try6), ({
        ...
        _ => (outArgs.clone(), outSlots.clone(), params.clone(), res_ty.clone()),
            ...
            (params, res_ty) = unwrap_break_err!(applyArgTypesToFuncType(outSlots.clone(), params.clone(), res_ty.clone(), inEnv.clone(), inCheckTypes, inInfo.clone()), '__try6);
...
fn addPolymorphicTypeVars(mut cache: FCore::Cache, mut env: FCore::Graph, mut typeVars: Arc<metamodelica::List<Arc<Absyn::Path>>>, mut funcTy: Arc<DAE::Type>, mut pb: Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>, mut fnPath: Arc<Absyn::Path>, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>)> {
    ...
    let mut poly_types: Arc<metamodelica::List<ArcStr>>;
    let mut ty_name: ArcStr;
    if typeVars.clone().is_empty() {
        return Ok((cache.clone(), pb.clone()));
    }
    (cache, e, _) = Lookup::lookupClass(cache, env.clone(), fnPath.clone(), None)?;
    poly_types = ({
        let mut __acc: Arc<metamodelica::List<ArcStr>> = metamodelica::nil();
    ...
    if (typeVars.clone().len() as i32) > (poly_types.clone().len() as i32) {
        Error::addSourceMessage(Error::TOO_MANY_TYPE_VARS_IN_CALL.clone(), list![(AbsynUtil::pathString(fnPath, (literal!(".")).clone(), true, false)?).clone()], info.clone())?;
        bail!("fail");
    }
...
fn applyArgTypesToFuncType(mut inSlots: Arc<metamodelica::List<Slot>>, mut inParameters: Arc<metamodelica::List<Arc<DAE::FuncArg>>>, mut inResultType: Arc<DAE::Type>, mut inEnv: FCore::Graph, mut checkTypes: bool, mut inInfo: SourceInfo) -> Result<(Arc<metamodelica::List<Arc<DAE::FuncArg>>>, Arc<DAE::Type>)> {
    let mut outParameters: Arc<metamodelica::List<Arc<DAE::FuncArg>>>;
    let mut outResultType: Arc<DAE::Type>;
    let mut tys: Arc<metamodelica::List<Arc<DAE::Type>>>;
    let mut dims: Arc<metamodelica::List<Arc<DAE::Dimension>>>;
    let mut used_args: Arc<metamodelica::List<ArcStr>>;
    let mut used_slots: Arc<metamodelica::List<Slot>>;
    let mut cache: FCore::Cache;
    let mut env: FCore::Graph;
    let mut vars: Arc<metamodelica::List<Arc<DAE::Var>>>;
    let mut dummy_var: Arc<SCode::Element>;
    if !(checkTypes) || inParameters.clone().is_empty() {
    ...
    dims = listAppend(List::mapFlat(tys, (std::sync::Arc::new(fnptr!(TypesDump::getDimensions, Arc<DAE::Type>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Type>) -> Result<Arc<metamodelica::List<Arc<DAE::Dimension>>>> + 'static>))?, dims);
    ...
    env = FGraph::openScope(inEnv, openmodelica_frontend_types::SCode::Encapsulated::NOT_ENCAPSULATED, (arcstr::literal!(FCore::forScopeName)).clone(), None)?;
...
fn getAllOutputDimensions(mut inOutputType: Arc<DAE::Type>) -> Result<Arc<metamodelica::List<Arc<DAE::Dimension>>>> {
    ...
    outDimensions = (::match_deref::match_deref! { match &(inOutputType.clone()) {
        ...
        _ => {
            TypesDump::getDimensions(inOutputType)
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    Ok(outDimensions)
...
fn extractNamesFromDims(mut inDimensions: Arc<metamodelica::List<Arc<DAE::Dimension>>>, mut inAccumNames: Arc<metamodelica::List<ArcStr>>) -> Result<Arc<metamodelica::List<ArcStr>>> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: Deref @ DAE::Dimension::DIM_EXP { exp }, tail: rest_dims } => {
            ...
            names = List::fold(crefs, (std::sync::Arc::new(extractNamesFromDims2) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::ComponentRef>, Arc<metamodelica::List<ArcStr>>) -> Result<Arc<metamodelica::List<ArcStr>>> + 'static>), inAccumNames)?;
            { (inDimensions, inAccumNames) = (rest_dims.clone(), names); continue '__tco; }
        },
        Deref @ metamodelica::List::Cons { head: _, tail: rest_dims } => {
            { (inDimensions, inAccumNames) = (rest_dims.clone(), inAccumNames); continue '__tco; }
        },
        Deref @ metamodelica::List::Nil => {
...
fn extractNamesFromDims2(mut inCref: Arc<DAE::ComponentRef>, mut inAccumNames: Arc<metamodelica::List<ArcStr>>) -> Result<Arc<metamodelica::List<ArcStr>>> {
    let mut outNames: Arc<metamodelica::List<ArcStr>> = metamodelica::nil();
    outNames = (::match_deref::match_deref! { match &(inCref) {
        Deref @ DAE::ComponentRef::CREF_IDENT { ident: name, .. } => {
            outNames = if (List::isMemberOnTrue((name.clone()).clone(), inAccumNames.clone(), (std::sync::Arc::new(fnptr!(stringEq, ArcStr, ArcStr)) as std::sync::Arc<dyn ::std::ops::Fn(ArcStr, ArcStr) -> Result<bool> + 'static>))?) {inAccumNames} else {metamodelica::cons((name.clone()).clone(), inAccumNames)};
            outNames
        },
        _ => {
            inAccumNames
...
fn isSlotUsed(mut inSlot: Slot, mut inUsedNames: Arc<metamodelica::List<ArcStr>>) -> Result<bool> {
    ...
    outIsUsed = List::isMemberOnTrue((slot_name).clone(), inUsedNames, (std::sync::Arc::new(fnptr!(stringEq, ArcStr, ArcStr)) as std::sync::Arc<dyn ::std::ops::Fn(ArcStr, ArcStr) -> Result<bool> + 'static>))?;
    Ok(outIsUsed)
}
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Slot { defaultArg: Deref @ DAE::FuncArg { name, ty, .. }, .. } => {
                    Ok(Arc::new(DAE::Var { name: (name.clone()).clone(), attributes: DAE::dummyAttrParam().clone(), ty: ty.clone(), binding: openmodelica_frontend_types::DAE::Binding::interned_UNBOUND(), bind_from_outside: false, constOfForIteratorRange: None }))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
...
fn evaluateStructuralSlots2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inSlots: Arc<metamodelica::List<Slot>>, mut usedSlots: Arc<metamodelica::List<ArcStr>>, mut acc: Arc<metamodelica::List<Slot>>) -> (FCore::Cache, Arc<metamodelica::List<Slot>>) {
        ...
        if let Ok((__v, __wb0, __wb1)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: slot, tail: rest } => {
                    ...
                    (cache, slots) = evaluateStructuralSlots2(inCache.clone(), inEnv.clone(), rest.clone(), usedSlots.clone(), metamodelica::cons(slot.clone(), acc.clone()));
                    Ok(((cache.clone(), slots.clone()), cache.clone(), slots.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { cache = __wb0; slots = __wb1; break 'mc __v; }
        if let Ok((__v, __wb0, __wb1)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Slot { defaultArg: defaultArg @ Deref @ DAE::FuncArg { .. }, slotFilled: _, arg: Some(exp), dims, idx, evalStatus: ses }, tail: rest } => {
                    ...
                    (cache, slots) = evaluateStructuralSlots2(cache.clone(), inEnv.clone(), rest.clone(), usedSlots.clone(), metamodelica::cons(slot.clone(), acc.clone()));
                    Ok(((cache.clone(), slots.clone()), cache.clone(), slots.clone()))
                }
                _ => bail!("nomatch"),
            }}
...
fn evaluateStructuralSlots(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inSlots: Arc<metamodelica::List<Slot>>, mut funcType: Arc<DAE::Type>) -> Result<(FCore::Cache, Arc<metamodelica::List<Slot>>)> {
    let mut cache: FCore::Cache = FCore::Cache::NO_CACHE;
    let mut slots: Arc<metamodelica::List<Slot>> = metamodelica::nil();
    (cache, slots) = (::match_deref::match_deref! { match &(funcType) {
        Deref @ DAE::Type::T_FUNCTION { funcArg, funcResultType, .. } => {
            let mut tys: Arc<metamodelica::List<Arc<DAE::Type>>>;
            let mut dims: Arc<metamodelica::List<Arc<DAE::Dimension>>>;
            let mut used_args: Arc<metamodelica::List<ArcStr>>;
            tys = ({
...
fn makeDummyFuncEnv(mut inEnv: FCore::Graph, mut inVars: Arc<metamodelica::List<Arc<DAE::Var>>>, mut inDummyVar: Arc<SCode::Element>) -> Result<FCore::Graph> {
    ...
    for mut var in &*inVars {
        ...
        outEnv = FGraph::mkComponentNode(outEnv.clone(), var.clone(), dummy_var.clone(), openmodelica_frontend_types::DAE::Mod::interned_NOMOD(), openmodelica_frontend_dump::FCore::Status::VAR_TYPED, FGraph::empty())?;
    }
    Ok(outEnv)
}
...
fn evaluateFuncParamDimAndMatchTypes(mut inSlot: Slot, mut inParam: Arc<DAE::FuncArg>, mut inEnv: FCore::Graph, mut inCache: FCore::Cache, mut inInfo: SourceInfo) -> Result<Arc<DAE::FuncArg>> {
    ...
    outParam = (::match_deref::match_deref! { match &((inSlot, inParam.clone())) {
        ...
        (Slot { arg: Some(Deref @ DAE::Exp::ARRAY { ty: sty, .. }), dims: vdims, .. }, _) => {
            ...
            let true = (Expression::dimsEqual(dims1, dims2)?) else { bail!("pattern mismatch") };
        ...
        (Slot { arg: Some(Deref @ DAE::Exp::MATRIX { ty: sty, .. }), dims: vdims, .. }, _) => {
            ...
            dims2 = TypesDump::getDimensions(sty.clone());
            let true = (Expression::dimsEqual(vdims.clone(), dims2)?) else { bail!("pattern mismatch") };
            outParam = Types::setFuncArgType(inParam, pty)?;
            outParam
        },
        _ => {
...
fn evaluateFuncArgTypeDims(mut inType: Arc<DAE::Type>, mut inEnv: FCore::Graph, mut inCache: FCore::Cache) -> Arc<DAE::Type> {
    let mut outType: Arc<DAE::Type>;
    outType = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                ty @ Deref @ DAE::Type::T_TUPLE { .. } => {
                    let mut ty = (*ty).clone();
                    assign_variant_field!(ty => DAE::Type::T_TUPLE; types = List::map2(var_field!((*ty).types, DAE::Type::T_TUPLE).clone(), (std::sync::Arc::new(fnptr!(evaluateFuncArgTypeDims, Arc<DAE::Type>, FCore::Graph, FCore::Cache)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Type>, FCore::Graph, FCore::Cache) -> Result<Arc<DAE::Type>> + 'static>), inEnv.clone(), inCache.clone())?);
                    Ok(ty.clone())
                }
                _ => bail!("nomatch"),
            }}
...
fn slotsVectorizable(mut inSlots: Arc<metamodelica::List<Slot>>, mut info: SourceInfo) -> Result<Arc<metamodelica::List<Arc<DAE::Dimension>>>> {
    ...
    outDims = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Slot { defaultArg: Deref @ DAE::FuncArg { name, .. }, arg: Some(exp), dims: ad @ Deref @ metamodelica::List::Cons { head: _, tail: _ }, .. }, tail: rest } => {
...
fn sameSlotsVectorizable(mut inSlots: Arc<metamodelica::List<Slot>>, mut inDims: Arc<metamodelica::List<Arc<DAE::Dimension>>>, mut name: ArcStr, mut exp: Arc<DAE::Exp>, mut info: SourceInfo) -> Result<()> {
    let () = (::match_deref::match_deref! { match &(inSlots) {
        Deref @ metamodelica::List::Cons { head: Slot { defaultArg: Deref @ DAE::FuncArg { name: name2, .. }, arg: Some(exp2), dims: slot_ad @ Deref @ metamodelica::List::Cons { head: _, tail: _ }, .. }, tail: rest } => {
            sameArraydimLst(inDims.clone(), (name.clone()).clone(), exp.clone(), slot_ad.clone(), (name2.clone()).clone(), exp2.clone(), info.clone())?;
            sameSlotsVectorizable(rest.clone(), inDims, (name).clone(), exp, info)?;
            ()
        },
        Deref @ metamodelica::List::Cons { head: Slot { dims: Deref @ metamodelica::List::Nil, .. }, tail: rest } => {
            sameSlotsVectorizable(rest.clone(), inDims, (name).clone(), exp, info)?;
...
fn sameArraydimLst(mut inDims1: Arc<metamodelica::List<Arc<DAE::Dimension>>>, mut name1: ArcStr, mut exp1: Arc<DAE::Exp>, mut inDims2: Arc<metamodelica::List<Arc<DAE::Dimension>>>, mut name2: ArcStr, mut exp2: Arc<DAE::Exp>, mut info: SourceInfo) -> Result<()> {
    let () = 'mc: {
        let __mc_input = (inDims2.clone(), inDims2);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
fn getProperties(mut inType: Arc<DAE::Type>, mut inTupleConst: Arc<DAE::TupleConst>) -> Result<DAE::Properties> {
    ...
    outProperties = (::match_deref::match_deref! { match &((inType, inTupleConst)) {
        ...
        (ty, r#const) => {
            ...
            Debug::trace((tystr).clone())?;
            Debug::trace((literal!(", ")).clone())?;
            Debug::traceln((conststr).clone())?;
            bail!("fail")
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn elabConsts(mut inType: Arc<DAE::Type>, mut inConst: DAE::Const) -> Result<Arc<DAE::TupleConst>> {
    ...
    outTupleConst = (::match_deref::match_deref! { match &((inType, inConst)) {
        (Deref @ DAE::Type::T_TUPLE { types: tys, .. }, c) => {
            let mut consts: Arc<metamodelica::List<Arc<DAE::TupleConst>>>;
            consts = checkConsts(tys.clone(), c.clone())?;
            Arc::new(DAE::TupleConst::TUPLE_CONST { tupleConstLst: consts })
        },
        (ty, c) => {
            let mut consts: Arc<metamodelica::List<Arc<DAE::TupleConst>>>;
...
fn checkConsts(mut inTypes: Arc<metamodelica::List<Arc<DAE::Type>>>, mut inConst: DAE::Const) -> Result<Arc<metamodelica::List<Arc<DAE::TupleConst>>>> {
    let mut outTupleConsts: Arc<metamodelica::List<Arc<DAE::TupleConst>>>;
    outTupleConsts = ({
...
fn checkConst(mut inType: Arc<DAE::Type>, mut c: DAE::Const) -> Result<Arc<DAE::TupleConst>> {
    ...
    outTupleConst = (::match_deref::match_deref! { match &(inType) {
        ...
        _ => Arc::new(DAE::TupleConst::SINGLE_CONST { r#const: c }),
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    Ok(outTupleConst)
}
...
fn elabInputArgs(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inSlots: Arc<metamodelica::List<Slot>>, mut inOnlyOneFunction: bool, mut inCheckTypes: bool, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo, mut inFuncType: Arc<DAE::Type>, mut inPath: Arc<Absyn::Path>, mut isGraphicsExp: bool) -> Result<(FCore::Cache, Arc<metamodelica::List<Arc<DAE::Exp>>>, Arc<metamodelica::List<Slot>>, Arc<metamodelica::List<DAE::Const>>, Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>)> {
    ...
    let mut outConsts: Arc<metamodelica::List<DAE::Const>>;
    let mut outPolymorphicBindings: Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>> = metamodelica::nil();
    let mut fargs: Arc<metamodelica::List<Arc<DAE::FuncArg>>>;
    let mut consts1: Arc<metamodelica::List<DAE::Const>>;
    let mut consts2: Arc<metamodelica::List<DAE::Const>>;
    if inPosArgs.clone().is_empty() && inNamedArgs.clone().is_empty() {
        ...
        (outCache, outSlots, consts1, outPolymorphicBindings) = elabPositionalInputArgs(outCache, inEnv.clone(), inPosArgs, fargs.clone(), outSlots, inOnlyOneFunction, inCheckTypes, inImplicit, outPolymorphicBindings, inPrefix.clone(), inInfo.clone(), inPath.clone(), isGraphicsExp)?;
        (outCache, outSlots, consts2, outPolymorphicBindings) = elabNamedInputArgs(outCache, inEnv.clone(), inNamedArgs, fargs, outSlots, inOnlyOneFunction, inCheckTypes, inImplicit, outPolymorphicBindings, inPrefix, inInfo, inPath, isGraphicsExp)?;
...
fn makeEmptySlots(mut inArgs: Arc<metamodelica::List<Arc<DAE::FuncArg>>>) -> Result<Arc<metamodelica::List<Slot>>> {
    let mut outSlots: Arc<metamodelica::List<Slot>>;
    (outSlots, _) = List::mapFold(inArgs, (std::sync::Arc::new(fnptr!(makeEmptySlot, Arc<DAE::FuncArg>, i32)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::FuncArg>, i32) -> Result<(Slot, i32)> + 'static>), 1)?;
    Ok(outSlots)
}
...
fn makeEmptySlot(mut inArg: Arc<DAE::FuncArg>, mut inIndex: i32) -> (Slot, i32) {
    let mut outSlot: Slot;
    let mut outIndex: i32;
    outSlot = Slot { defaultArg: inArg, slotFilled: false, arg: None, dims: metamodelica::nil(), idx: inIndex, evalStatus: SLOT_NOT_EVALUATED.clone() };
    outIndex = inIndex + 1;
    (outSlot, outIndex)
}

fn funcArgsFromSlots(mut inSlots: Arc<metamodelica::List<Slot>>) -> Result<Arc<metamodelica::List<Arc<DAE::FuncArg>>>> {
    let mut outFuncArgs: Arc<metamodelica::List<Arc<DAE::FuncArg>>>;
    outFuncArgs = ({
        let mut __acc: Arc<metamodelica::List<Arc<DAE::FuncArg>>> = metamodelica::nil();
...
fn funcArgFromSlot(mut inSlot: Slot) -> Result<Arc<DAE::FuncArg>> {
    let mut outFuncArg: Arc<DAE::FuncArg>;
    let Slot { defaultArg: __pa0, .. } = (inSlot) else { bail!("pattern mismatch") };
...
fn complexTypeFromSlots(mut inSlots: Arc<metamodelica::List<Slot>>, mut complexClassType: ClassInf::State) -> Result<Arc<DAE::Type>> {
    ...
    for mut slot in &*inSlots {
        ...
        let (__pa0, __pa1) = ::match_deref::match_deref! { match &(slot.clone()) {
            Slot { defaultArg: Deref @ DAE::FuncArg { name: __pa0, ty: __pa1, .. }, .. } => (__pa0.clone(), __pa1.clone()),
            _ => bail!("pattern mismatch"),
        } };
        id = __pa0.clone();
        ty = __pa1.clone();
        vars = metamodelica::cons(Expression::makeVar((id.clone()).clone(), Types::simplifyType(ty.clone())?), vars.clone());
    }
    vars = vars.reverse();
    outType = Arc::new(DAE::Type::T_COMPLEX { complexClassType: complexClassType, varLst: vars, equalityConstraint: None, usedExternally: false });
    Ok(outType)
}

fn slotListArgs(mut inSlots: Arc<metamodelica::List<Slot>>) -> Arc<metamodelica::List<Arc<DAE::Exp>>> {
    let mut outArgs: Arc<metamodelica::List<Arc<DAE::Exp>>>;
    outArgs = List::filterMap(inSlots, (std::sync::Arc::new(slotArg) as std::sync::Arc<dyn ::std::ops::Fn(Slot) -> Result<Arc<DAE::Exp>> + 'static>));
    outArgs
}
...
fn fillGraphicsDefaultSlots(mut inCache: FCore::Cache, mut inSlots: Arc<metamodelica::List<Slot>>, mut inClass: Arc<SCode::Element>, mut inEnv: FCore::Graph, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<metamodelica::List<Slot>>, Arc<metamodelica::List<DAE::Const>>, Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>)> {
    ...
    for mut slot in &*inSlots {
        ...
        outSlots = metamodelica::cons(slot.clone(), outSlots.clone());
    }
...
fn printSlotsStr(mut inSlots: Arc<metamodelica::List<Slot>>) -> Result<ArcStr> {
    ...
    outString = ((::match_deref::match_deref! { match &(inSlots) {
        Deref @ metamodelica::List::Cons { head: Slot { defaultArg: farg, slotFilled: filled, arg: exp, dims: ds, .. }, tail: xs } => {
            ...
            let mut s2: ArcStr;
            let mut res: ArcStr;
            let mut str_lst: Arc<metamodelica::List<ArcStr>>;
            ...
            s = stringDelimitList(str_lst, (literal!(", ")).clone());
            s1 = stringAppendList(list![(literal!("SLOT(")).clone(), (farg_str).clone(), (literal!(", ")).clone(), (filledStr).clone(), (literal!(", ")).clone(), (r#str).clone(), (literal!(", [")).clone(), (s).clone(), (literal!("])\n")).clone()]);
...
fn isFreeParameterExp(mut inExp: Arc<DAE::Exp>, mut inCache: FCore::Cache, mut inEnv: FCore::Graph) -> Result<(bool, FCore::Cache)> {
    ...
    let mut outCache: FCore::Cache;
    outCache = inCache.clone();
    isFree = (::match_deref::match_deref! { match &(inExp) {
        ...
        Deref @ DAE::Exp::CREF { componentRef: cr, .. } => {
            ...
            (outCache, _, _, binding, _, _, _, _, _) = Lookup::lookupVar(inCache, inEnv, cr.clone())?;
            (::match_deref::match_deref! { match &(binding) {
        ...
        Deref @ DAE::Exp::BINARY { exp1, exp2, .. } => {
            let mut isFree2: bool;
            (isFree, outCache) = isFreeParameterExp(exp1.clone(), inCache, inEnv.clone())?;
            (isFree2, outCache) = isFreeParameterExp(exp2.clone(), outCache, inEnv)?;
            isFree && isFree2
        },
        Deref @ DAE::Exp::UNARY { exp: exp1, .. } => {
            (isFree, outCache) = isFreeParameterExp(exp1.clone(), inCache, inEnv)?;
            isFree
        },
        Deref @ DAE::Exp::LBINARY { exp1, exp2, .. } => {
            let mut isFree2: bool;
            (isFree, outCache) = isFreeParameterExp(exp1.clone(), inCache, inEnv.clone())?;
            (isFree2, outCache) = isFreeParameterExp(exp2.clone(), outCache, inEnv)?;
            isFree && isFree2
...
fn elabPositionalInputArg(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inExp: Arc<Absyn::Exp>, mut farg: Arc<DAE::FuncArg>, mut position: i32, mut inSlotLst: Arc<metamodelica::List<Slot>>, mut onlyOneFunction: bool, mut checkTypes: bool, mut r#impl: bool, mut inPolymorphicBindings: Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>, mut inPrefix: DAE::Prefix, mut info: SourceInfo, mut path: Arc<Absyn::Path>, mut isGraphicsExp: bool) -> Result<(FCore::Cache, Arc<metamodelica::List<Slot>>, DAE::Const, Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>)> {
    let mut outCache: FCore::Cache;
    let mut outSlotLst: Arc<metamodelica::List<Slot>>;
    let mut outConst: DAE::Const;
    let mut outPolymorphicBindings: Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>;
    let mut numErrors: i32 = Error::getNumErrorMessages();
    (outCache, outSlotLst, outConst, outPolymorphicBindings) = 'mc: {
        let __mc_input = (inCache, inEnv, inExp, farg, inSlotLst, onlyOneFunction, checkTypes, inPolymorphicBindings, inPrefix);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, e, Deref @ DAE::FuncArg { name: id, ty: vt @ Deref @ DAE::Type::T_CODE { ty: ct }, par: pr, .. }, slots, _, true, polymorphicBindings, pre) => {
                    let mut slots_1: Arc<metamodelica::List<Slot>>;
                    let mut e_1: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, e, Deref @ DAE::FuncArg { name: id, ty: vt, .. }, _, true, true, _, pre) => {
                    ...
                    let mut s5: ArcStr;
                    let mut cache = (*cache).clone();
                    let true = (Error::getNumErrorMessages() == numErrors) else { bail!("pattern mismatch") };
                    (cache, e_1, prop) = elabExpInExpression(cache.clone(), env.clone(), e.clone(), r#impl, true, pre.clone(), info.clone())?;
                    s1 = (intString(position)).clone();
...
fn elabNamedInputArgs(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inAbsynNamedArgLst: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inTypesFuncArgLst: Arc<metamodelica::List<Arc<DAE::FuncArg>>>, mut inSlotLst: Arc<metamodelica::List<Slot>>, mut onlyOneFunction: bool, mut checkTypes: bool, mut r#impl: bool, mut inPolymorphicBindings: Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>, mut inPrefix: DAE::Prefix, mut info: SourceInfo, mut path: Arc<Absyn::Path>, mut isGraphicsExp: bool) -> Result<(FCore::Cache, Arc<metamodelica::List<Slot>>, Arc<metamodelica::List<DAE::Const>>, Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>)> {
    let mut outCache: FCore::Cache;
    let mut outSlotLst: Arc<metamodelica::List<Slot>>;
    let mut outTypesConstLst: Arc<metamodelica::List<DAE::Const>>;
    let mut outPolymorphicBindings: Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>;
    (outCache, outSlotLst, outTypesConstLst, outPolymorphicBindings) = (::match_deref::match_deref! { match &((inCache, inEnv, inAbsynNamedArgLst, inTypesFuncArgLst, inSlotLst, inPolymorphicBindings.clone())) {
        (cache, _, Deref @ metamodelica::List::Nil, _, slots, _) => {
            (cache.clone(), slots.clone(), metamodelica::nil(), inPolymorphicBindings)
        },
        (cache, env, Deref @ metamodelica::List::Cons { head: na, tail: nas }, farg, slots, polymorphicBindings) => {
            let mut c1: DAE::Const;
            let mut clist: Arc<metamodelica::List<DAE::Const>>;
            let mut cache = (*cache).clone();
            let mut slots = (*slots).clone();
            let mut polymorphicBindings = (*polymorphicBindings).clone();
            (cache, slots, c1, polymorphicBindings) = elabNamedInputArg(cache.clone(), env.clone(), na.clone(), farg.clone(), slots.clone(), onlyOneFunction, checkTypes, r#impl, polymorphicBindings.clone(), inPrefix.clone(), info.clone(), path.clone(), Error::getNumErrorMessages(), isGraphicsExp)?;
            (cache, slots, clist, polymorphicBindings) = elabNamedInputArgs(cache.clone(), env.clone(), nas.clone(), farg.clone(), slots.clone(), onlyOneFunction, checkTypes, r#impl, polymorphicBindings.clone(), inPrefix, info, path, isGraphicsExp)?;
            (cache.clone(), slots.clone(), metamodelica::cons(c1, clist), polymorphicBindings.clone())
        },
        _ => bail!("match: no arm matched"),
    } });
...
fn elabNamedInputArg(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inNamedArg: Arc<Absyn::NamedArg>, mut inTypesFuncArgLst: Arc<metamodelica::List<Arc<DAE::FuncArg>>>, mut inSlotLst: Arc<metamodelica::List<Slot>>, mut onlyOneFunction: bool, mut checkTypes: bool, mut r#impl: bool, mut inPolymorphicBindings: Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>, mut inPrefix: DAE::Prefix, mut info: SourceInfo, mut path: Arc<Absyn::Path>, mut numErrors: i32, mut isGraphicsExp: bool) -> Result<(FCore::Cache, Arc<metamodelica::List<Slot>>, DAE::Const, Arc<metamodelica::List<(ArcStr, Arc<metamodelica::List<Arc<DAE::Type>>>)>>)> {
    ...
    (outCache, outSlotLst, outTypesConstLst, outPolymorphicBindings) = 'mc: {
        let __mc_input = (inCache, inEnv, inNamedArg, inTypesFuncArgLst, inSlotLst, onlyOneFunction, checkTypes, inPolymorphicBindings, inPrefix);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, _, Deref @ Absyn::NamedArg { argName: id, .. }, farg, slots, true, _, polymorphicBindings, _) => {
                    ...
                    let true = (isGraphicsExp) else { bail!("pattern mismatch") };
...
fn findNamedArg(mut inIdent: ArcStr, mut inArgs: Arc<metamodelica::List<Arc<DAE::FuncArg>>>) -> Result<Arc<DAE::FuncArg>> {
    ...
    let mut inIdent2: ArcStr = if (haveMM) {{ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("$in_")); __mm_s.push_str(&*inIdent.clone()); ArcStr::from(__mm_s) }} else {literal!("")};
    for mut arg in &*inArgs {
        let mut arg = arg.clone();
        let __pa0 = ::match_deref::match_deref! { match &(arg.clone()) {
        ...
        id = __pa0.clone();
        if id.clone() == inIdent.clone() || haveMM && id.clone() == inIdent2.clone() {
            outArg = arg.clone();
...
fn findNamedArgType(mut inIdent: ArcStr, mut inArgs: Arc<metamodelica::List<Arc<DAE::FuncArg>>>) -> Result<Arc<DAE::Type>> {
    let mut outType: Arc<DAE::Type>;
    let __pa0 = ::match_deref::match_deref! { match &(findNamedArg((inIdent).clone(), inArgs)?) {
        Deref @ DAE::FuncArg { ty: __pa0, .. } => __pa0.clone(),
        _ => bail!("pattern mismatch"),
    } };
    outType = __pa0.clone();
...
fn fillSlot(mut inFuncArg: Arc<DAE::FuncArg>, mut inExp: Arc<DAE::Exp>, mut inDims: Arc<metamodelica::List<Arc<DAE::Dimension>>>, mut inSlotLst: Arc<metamodelica::List<Slot>>, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo, mut r#fn: Arc<Absyn::Path>) -> Result<Arc<metamodelica::List<Slot>>> {
    ...
    let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(inFuncArg) {
        Deref @ DAE::FuncArg { name: __pa0, ty: __pa1, r#const: __pa2, .. } => (__pa0.clone(), __pa1.clone(), __pa2.clone()),
        _ => bail!("pattern mismatch"),
    } };
    fa1 = __pa0.clone();
    ty1 = __pa1.clone();
    ...
    while !(rest_slots.clone().is_empty()) {
        ...
        let __pa5 = ::match_deref::match_deref! { match &(slot.clone()) {
            Slot { defaultArg: Deref @ DAE::FuncArg { name: __pa5, .. }, .. } => __pa5.clone(),
            _ => bail!("pattern mismatch"),
        } };
        fa2 = __pa5.clone();
        if stringEq((fa1.clone()).clone(), (fa2.clone()).clone()) || stringEq(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("$in_")); __mm_s.push_str(&*fa1.clone()); ArcStr::from(__mm_s) }).clone(), (fa2.clone()).clone()) {
            let (__pa7, __pa8, __pa9, __pa10, __pa11, __pa12) = ::match_deref::match_deref! { match &(slot.clone()) {
                Slot { defaultArg: Deref @ DAE::FuncArg { r#const: __pa7, par: __pa8, defaultBinding: __pa9, .. }, slotFilled: __pa10, idx: __pa11, evalStatus: __pa12, .. } => (__pa7.clone(), __pa8.clone(), __pa9.clone(), __pa10.clone(), __pa11.clone(), __pa12.clone()),
                _ => bail!("pattern mismatch"),
            } };
            c2 = __pa7.clone();
            prl = __pa8.clone();
        ...
        outSlotLst = metamodelica::cons(slot.clone(), outSlotLst.clone());
    }
    Error::addSourceMessageAndFail(Error::NO_SUCH_PARAMETER.clone(), list![(literal!("")).clone(), (fa1).clone()], inInfo)?;
    unreachable!("Error.addSourceMessageAndFail always fails — caller-side flow-analysis hint");
    Ok(outSlotLst)
}
...
fn elabCref1(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inComponentRef: Arc<Absyn::ComponentRef>, mut inImplicit: bool, mut performVectorization: bool, mut inPrefix: DAE::Prefix, mut evalCref: bool, mut info: SourceInfo) -> Result<(FCore::Cache, Option<(Arc<DAE::Exp>, DAE::Properties, Arc<DAE::Attributes>)>)> {
    ...
    (outCache, res) = 'mc: {
        let __mc_input = (inCache.clone(), inEnv.clone(), inComponentRef.clone(), inImplicit, inPrefix);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ Absyn::ComponentRef::CREF_QUAL { .. }, r#impl, pre) => {
                    ...
                    let mut stripped_cref: Arc<Absyn::ComponentRef>;
                    let mut cache = (*cache).clone();
                    let mut res: Option<(Arc<DAE::Exp>, DAE::Properties, Arc<DAE::Attributes>)> = res.clone();
                    let true = (Config::acceptMetaModelicaGrammar()?) else { bail!("pattern mismatch") };
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ Absyn::ComponentRef::CREF_IDENT { name: id, subscripts: Deref @ metamodelica::List::Cons { head: Deref @ Absyn::Subscript::SUBSCRIPT { subscript: e }, tail: Deref @ metamodelica::List::Nil } }, r#impl, pre) => {
                    let mut cache = (*cache).clone();
                    let mut res: Option<(Arc<DAE::Exp>, DAE::Properties, Arc<DAE::Attributes>)> = res.clone();
                    let true = (Config::acceptMetaModelicaGrammar()?) else { bail!("pattern mismatch") };
                    (cache, res) = elabCrefArraySubscripts(Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (id.clone()).clone(), subscripts: metamodelica::nil() }), e.clone(), cache.clone(), env.clone(), pre.clone(), evalCref, r#impl.clone(), info.clone())?;
                    Ok(((cache.clone(), res.clone()), res.clone()))
                }
                _ => bail!("nomatch"),
            }}
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, c, r#impl, pre) => {
                    ...
                    (cache, exp, r#const, attr) = elabCref2(cache.clone(), env.clone(), c_1.clone(), attr.clone(), constSubs.clone(), forIteratorConstOpt.clone(), t.clone(), binding.clone(), performVectorization, splicedExpData.clone(), pre.clone(), evalCref, info.clone())?;
                    t = fixEnumerationType(t.clone());
                    (exp, r#const) = evaluateEmptyVariable(hasZeroSizeDim.clone() && evalCref, exp.clone(), t.clone(), r#const.clone());
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, c, _, _) => {
                    ...
                    let mut exp: Arc<DAE::Exp>;
                    let mut path: Arc<Absyn::Path>;
                    let mut enum_lit_strs: Arc<metamodelica::List<ArcStr>>;
...
fn elabCrefArraySubscripts(mut cref: Arc<Absyn::ComponentRef>, mut e: Arc<Absyn::Exp>, mut cache: FCore::Cache, mut env: FCore::Graph, mut pre: DAE::Prefix, mut evalCref: bool, mut r#impl: bool, mut info: SourceInfo) -> Result<(FCore::Cache, Option<(Arc<DAE::Exp>, DAE::Properties, Arc<DAE::Attributes>)>)> {
    ...
    if Types::isMetaBoxedType(sub_ty.clone()) {
        ...
        exp2 = Arc::new(DAE::Exp::UNBOX { exp: exp2, ty: sub_ty.clone() });
    }
    let true = (Types::isScalarInteger(sub_ty)) else { bail!("pattern mismatch") };
    r#const = Types::constAnd(const1, const2);
    exp = Expression::makeASUB(exp1, list![exp2])?;
    res = Some((exp, DAE::Properties::PROP { type_: t, constFlag: r#const }, attr));
    Ok((cache, res))
}
...
fn evaluateEmptyVariable(mut hasZeroSizeDim: bool, mut inExp: Arc<DAE::Exp>, mut ty: Arc<DAE::Type>, mut c: DAE::Const) -> (Arc<DAE::Exp>, DAE::Const) {
    ...
    (oexp, oc) = 'mc: {
        let __mc_input = (hasZeroSizeDim, inExp.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (true, Deref @ DAE::Exp::CREF { componentRef: cr, .. }) => {
                    ...
                    let mut a: bool;
                    let mut et: Arc<DAE::Type>;
                    let mut ss: Arc<metamodelica::List<Arc<DAE::Subscript>>>;
                    let mut exp: Arc<DAE::Exp>;
                    a = Types::isArray(ty.clone());
                    sc = boolNot(a.clone());
                    et = Types::simplifyType(ty.clone())?;
...
pub(crate) fn makeEnumerationArray(mut enumTypeName: Arc<Absyn::Path>, mut enumLiterals: Arc<metamodelica::List<ArcStr>>) -> Result<(Arc<DAE::Exp>, Arc<DAE::Type>)> {
    ...
    ety = Arc::new(DAE::Type::T_ARRAY { ty: Arc::new(DAE::Type::T_ENUMERATION { index: None, path: enumTypeName.clone(), names: enumLiterals.clone(), literalVarLst: metamodelica::nil(), attributeLst: metamodelica::nil() }), dims: list![Arc::new(DAE::Dimension::DIM_ENUM { enumTypeName: enumTypeName, literals: enumLiterals, size: sz })] });
    enumArray = Arc::new(DAE::Exp::ARRAY { ty: ety.clone(), scalar: true, array: enum_lit_expl });
    enumArrayType = ety;
...
fn fillCrefSubscripts(mut inComponentRef: Arc<DAE::ComponentRef>, mut inType: Arc<DAE::Type>) -> Result<Arc<DAE::ComponentRef>> {
    ...
    outComponentRef = (::match_deref::match_deref! { match &((inComponentRef, inType)) {
        ...
        (Deref @ DAE::ComponentRef::CREF_IDENT { ident: id, identType: ty2, subscriptLst: subs }, t) => {
            ...
            ComponentReferenceBasics::makeCrefIdent((id.clone()).clone(), ty2.clone(), subs_1)
        },
        (Deref @ DAE::ComponentRef::CREF_QUAL { ident: id, subscriptLst: subs, componentRef: cref, identType: ty2 }, t) => {
            ...
            ComponentReferenceBasics::makeCrefQual((id.clone()).clone(), ty2.clone(), subs.clone(), cref_1)
        },
        _ => bail!("match: no arm matched"),
    } });
    Ok(outComponentRef)
...
fn stripPrefixType(mut inType: Arc<DAE::Type>, mut inPrefixType: Arc<DAE::Type>) -> Arc<DAE::Type> {
    '__tco: loop {
        ::match_deref::match_deref! { match &((inType.clone(), inPrefixType)) {
        (Deref @ DAE::Type::T_ARRAY { ty: t, .. }, Deref @ DAE::Type::T_ARRAY { ty: pt, .. }) => {
            { (inType, inPrefixType) = (t.clone(), pt.clone()); continue '__tco; }
        },
        _ => {
            return inType
        },
...
fn fillSubscripts(mut inExpSubscriptLst: Arc<metamodelica::List<Arc<DAE::Subscript>>>, mut inType: Arc<DAE::Type>) -> Arc<metamodelica::List<Arc<DAE::Subscript>>> {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Type::T_ARRAY { .. } => {
                    let mut subs: Arc<metamodelica::List<Arc<DAE::Subscript>>>;
                    subs = List::fill(openmodelica_frontend_types::DAE::Subscript::interned_WHOLEDIM(), (TypesDump::getDimensions(inType.clone()).len() as i32));
...
fn elabCref2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inCref: Arc<DAE::ComponentRef>, mut inAttributes: Arc<DAE::Attributes>, mut constSubs: DAE::Const, mut inIteratorConst: Option<DAE::Const>, mut inType: Arc<DAE::Type>, mut inBinding: Arc<DAE::Binding>, mut inVectorize: bool, mut splicedExpData: InstTypes::SplicedExpData, mut inPrefix: DAE::Prefix, mut evalCref: bool, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Const, Arc<DAE::Attributes>)> {
    ...
    (outExp, outConst, outAttributes) = 'mc: {
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (SCode::Variability::CONST { .. }, _, _, InstTypes::SplicedExpData { .. }) => {
                    ...
                    let mut e: Arc<DAE::Exp>;
                    let mut v: Arc<Values::Value>;
                    let mut subsc: Arc<metamodelica::List<Arc<DAE::Subscript>>>;
                    let mut outCache: FCore::Cache = outCache.clone();
        ...
        __acc.reverse()
                    ...
                    Ok(((e.clone(), r#const.clone(), inAttributes.clone()), outCache.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { outCache = __wb0; break 'mc __v; }
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (SCode::Variability::CONST { .. }, _, Deref @ DAE::Binding::EQBOUND { constant_: DAE::Const::C_CONST { .. }, .. }, InstTypes::SplicedExpData { splicedExp: sexp, identType: idTy }) => {
                    ...
                    e = crefVectorize(inVectorize, e.clone(), inType.clone(), sexp.clone(), expIdTy.clone());
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (SCode::Variability::CONST { .. }, _, Deref @ DAE::Binding::EQBOUND { evaluatedExp: Some(v), constant_: DAE::Const::C_CONST { .. }, .. }, InstTypes::SplicedExpData { splicedExp: Some(Deref @ DAE::Exp::CREF { componentRef: cr, .. }), identType: _ }) => {
                    let mut subCr1: Arc<DAE::ComponentRef>;
                    let mut subCr2: Arc<DAE::ComponentRef>;
                    let mut e: Arc<DAE::Exp>;
...
pub fn crefVectorize(mut performVectorization: bool, mut inExp: Arc<DAE::Exp>, mut inType: Arc<DAE::Type>, mut splicedExp: Option<Arc<DAE::Exp>>, mut crefIdType: Arc<DAE::Type>) -> Arc<DAE::Exp> {
    let mut outExp: Arc<DAE::Exp>;
    outExp = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: d1, tail: Deref @ metamodelica::List::Nil }, ty: Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: d2, tail: Deref @ metamodelica::List::Nil }, .. } }, Some(Deref @ DAE::Exp::CREF { componentRef: cr, .. })) => {
                    let mut b1: bool;
                    let mut b2: bool;
                    let mut e: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _, Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: d1, tail: Deref @ metamodelica::List::Nil }, ty: t }, Some(Deref @ DAE::Exp::CREF { componentRef: cr, .. })) => {
                    let mut e: Arc<DAE::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, Deref @ DAE::Exp::CREF { componentRef: cr, ty: exptp }, Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: d1, tail: Deref @ metamodelica::List::Nil }, ty: t @ Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: d2, tail: Deref @ metamodelica::List::Nil }, .. } }, _) => {
                    let mut b1: bool;
                    let mut b2: bool;
                    let mut e: Arc<DAE::Exp>;
                    let mut ds: i32;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, Deref @ DAE::Exp::CREF { componentRef: cr, ty: exptp }, Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: d1, tail: Deref @ metamodelica::List::Nil }, ty: t }, _) => {
                    let mut e: Arc<DAE::Exp>;
                    let mut ds: i32;
                    let false = (Types::isArray(t.clone())) else { bail!("pattern mismatch") };
                    ds = Expression::dimensionSize(d1.clone())?;
...
fn extractDimensionOfChild(mut inExp: Arc<DAE::Exp>) -> Result<(Arc<metamodelica::List<Arc<DAE::Dimension>>>, bool)> {
    let mut outExp: Arc<metamodelica::List<Arc<DAE::Dimension>>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Exp::ARRAY { array: expl1 @ Deref @ metamodelica::List::Cons { head: exp2 @ Deref @ DAE::Exp::ARRAY { ty: _, scalar: _, array: _ }, tail: _ }, .. } => {
                    let mut tl: Arc<metamodelica::List<Arc<DAE::Dimension>>>;
                    let mut x: i32;
                    (tl, _) = extractDimensionOfChild(exp2.clone())?;
                    x = (expl1.clone().len() as i32);
                    Ok((metamodelica::cons(Arc::new(DAE::Dimension::DIM_INTEGER { integer: x.clone() }), tl.clone()), false))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
...
fn elabCrefSlice(mut inCref: Arc<DAE::ComponentRef>, mut inType: Arc<DAE::Type>) -> Result<Arc<DAE::Exp>> {
    ...
    outCref = (::match_deref::match_deref! { match &((inCref, inType)) {
        (Deref @ DAE::ComponentRef::CREF_IDENT { ident: id, subscriptLst: ssl, .. }, ety) => {
            let mut exp1: Arc<DAE::Exp>;
            exp1 = flattenSubscript(ssl.clone(), (id.clone()).clone(), ety.clone())?;
            exp1
        },
        (Deref @ DAE::ComponentRef::CREF_QUAL { ident: id, identType: prety, subscriptLst: ssl, componentRef: child }, ety) => {
            let mut exp1: Arc<DAE::Exp>;
            ...
            exp1 = mergeQualWithRest(exp1, childExp, ety.clone())?;
            exp1
        },
        _ => bail!("match: no arm matched"),
    } });
...
fn mergeQualWithRest(mut qual: Arc<DAE::Exp>, mut rest: Arc<DAE::Exp>, mut inType: Arc<DAE::Type>) -> Result<Arc<DAE::Exp>> {
    let mut outExp: Arc<DAE::Exp>;
    outExp = (::match_deref::match_deref! { match &((qual, rest, inType)) {
        ...
        (Deref @ DAE::Exp::ARRAY { ty: _, scalar: _, array: expl1 }, exp2, ety) => {
            let mut iLst: Arc<metamodelica::List<Arc<DAE::Dimension>>>;
            let mut scalar: bool;
            ...
            exp2 = Arc::new(DAE::Exp::ARRAY { ty: Arc::new(DAE::Type::T_ARRAY { ty: ety.clone(), dims: iLst }), scalar: scalar, array: expl1.clone() });
            exp2.clone()
        },
        _ => bail!("match: no arm matched"),
    } });
...
fn mergeQualWithRest2(mut rest: Arc<DAE::Exp>, mut qual: Arc<DAE::Exp>) -> Result<Arc<DAE::Exp>> {
    ...
    outExp = (::match_deref::match_deref! { match &((rest, qual)) {
        ...
        (exp1 @ Deref @ DAE::Exp::ARRAY { ty: ety, scalar: _, array: expl1 }, exp2 @ Deref @ DAE::Exp::CREF { componentRef: Deref @ DAE::ComponentRef::CREF_IDENT { ident: _, identType: _, subscriptLst: _ }, ty: _ }) => {
            ...
            Arc::new(DAE::Exp::ARRAY { ty: ety.clone(), scalar: scalar, array: expl1.clone() })
        },
        _ => bail!("match: no arm matched"),
    } });
    Ok(outExp)
...
fn flattenSubscript3(mut inSubscripts: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut inName: ArcStr, mut inType: Arc<DAE::Type>, mut inExp: Arc<DAE::Exp>) -> Result<Arc<DAE::Exp>> {
    ...
    outExp = Arc::new(DAE::Exp::ARRAY { ty: Arc::new(DAE::Type::T_ARRAY { ty: ty, dims: dims }), scalar: scalar, array: expl });
    Ok(outExp)
}

fn removeDoubleEmptyArrays(mut inArr: Arc<DAE::Exp>) -> Arc<DAE::Exp> {
    ...
    outArr = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ DAE::Exp::ARRAY { ty: ty1, scalar: sc, array: expl1 @ Deref @ metamodelica::List::Cons { head: Deref @ DAE::Exp::ARRAY { .. }, tail: expl3 } } => {
                    let mut exp1: Arc<DAE::Exp>;
                    let mut expl3 = (*expl3).clone();
                    expl3 = List::map(expl1.clone(), (std::sync::Arc::new(fnptr!(removeDoubleEmptyArrays, Arc<DAE::Exp>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Exp>) -> Result<Arc<DAE::Exp>> + 'static>))?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Exp::ICONST { integer: 0 }, Deref @ DAE::Exp::ARRAY { ty: Deref @ DAE::Type::T_ARRAY { dims: arrDim, .. }, scalar: _, array: _ }, _, ety) => {
                    let mut ety = (*ety).clone();
                    ety = Expression::arrayEltType(ety.clone());
                    Ok(Arc::new(DAE::Exp::ARRAY { ty: Arc::new(DAE::Type::T_ARRAY { ty: ety.clone(), dims: metamodelica::cons(Arc::new(DAE::Dimension::DIM_INTEGER { integer: 0 }), arrDim.clone()) }), scalar: true, array: metamodelica::nil() }))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
...
fn applySubscript2(mut inSub: Arc<DAE::Exp>, mut inSubs: Arc<DAE::Exp>, mut inType: Arc<DAE::Type>) -> Result<Arc<DAE::Exp>> {
    let mut outExp: Arc<DAE::Exp>;
    outExp = (::match_deref::match_deref! { match &((inSub, inSubs, inType)) {
        (exp1, Deref @ DAE::Exp::CREF { componentRef: Deref @ DAE::ComponentRef::CREF_IDENT { ident: id, identType: ty2, subscriptLst: subs }, ty: _ }, _) => {
            let mut exp2: Arc<DAE::Exp>;
            let mut crty: Arc<DAE::Type>;
            let mut cref_: Arc<DAE::ComponentRef>;
            crty = Expression::unliftArrayTypeWithSubs(metamodelica::cons(Arc::new(DAE::Subscript::INDEX { exp: exp1.clone() }), subs.clone()), ty2.clone())?;
            cref_ = ComponentReferenceBasics::makeCrefIdent((id.clone()).clone(), ty2.clone(), metamodelica::cons(Arc::new(DAE::Subscript::INDEX { exp: exp1.clone() }), subs.clone()));
...
fn applySubscript3(mut inSubs: Arc<DAE::Exp>, mut inSub: Arc<DAE::Exp>, mut inType: Arc<DAE::Type>) -> Result<Arc<DAE::Exp>> {
    let mut outExp: Arc<DAE::Exp>;
    outExp = (::match_deref::match_deref! { match &((inSubs, inSub, inType)) {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (callexp @ Deref @ DAE::Exp::CALL { path: r#fn, expLst: args, attr }, Deref @ metamodelica::List::Cons { head: e, tail: es }) => {
                    ...
                    Ok(metamodelica::cons(Arc::new(DAE::Exp::CALL { path: r#fn.clone(), expLst: metamodelica::cons(e.clone(), args.clone()), attr: attr.clone() }), es_1.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cr, indx, ds, et, t) => {
                    ...
                    Ok(Arc::new(DAE::Exp::ARRAY { ty: et.clone(), scalar: true, array: metamodelica::cons(e_1.clone(), expl.clone()) }))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cr, indx, ds, ds2, et, t) => {
                    ...
                    let mut indx_1: i32;
                    let mut elt_tp: Arc<DAE::Type>;
                    let mut ms: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Exp>>>>>;
                    let mut expl: Arc<metamodelica::List<Arc<DAE::Exp>>>;
                    indx_1 = indx.clone() + 1;
                    ...
                    expl = __pa1.clone();
                    Ok(Arc::new(DAE::Exp::MATRIX { ty: et.clone(), integer: ds.clone(), matrix: metamodelica::cons(expl.clone(), ms.clone()) }))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
...
pub(crate) fn absynCrefToComponentReference(mut inComponentRef: Arc<Absyn::ComponentRef>) -> Result<Arc<DAE::ComponentRef>> {
    '__tco: loop {
        ...
        Deref @ Absyn::ComponentRef::CREF_QUAL { name: i, subscripts: Deref @ metamodelica::List::Nil, componentRef: c } => {
            ...
            return Ok(ComponentReferenceBasics::makeCrefQual((i.clone()).clone(), DAE::T_UNKNOWN_DEFAULT().clone(), metamodelica::nil(), cref))
        },
...
fn elabCrefSubs(mut inCache: FCore::Cache, mut inCrefEnv: FCore::Graph, mut inSubsEnv: FCore::Graph, mut inComponentRef: Arc<Absyn::ComponentRef>, mut inTopPrefix: DAE::Prefix, mut inCrefPrefix: DAE::Prefix, mut inBoolean: bool, mut inHasZeroSizeDim: bool, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::ComponentRef>, DAE::Const, bool)> {
    ...
    (outCache, outComponentRef, outConst, outHasZeroSizeDim) = 'mc: {
        let __mc_input = (inCache, inCrefEnv, inSubsEnv, inComponentRef.clone(), inTopPrefix, inCrefPrefix, inBoolean, inHasZeroSizeDim);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, crefEnv, crefSubs, Deref @ Absyn::ComponentRef::CREF_IDENT { name: id, subscripts: ss }, topPrefix, crefPrefix, r#impl, hasZeroSizeDim) => {
                    let mut sl: Arc<metamodelica::List<Arc<DAE::Dimension>>>;
                    let mut r#const: DAE::Const;
                    let mut cr: Arc<DAE::ComponentRef>;
                    let mut id_ty: Arc<DAE::Type>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, crefEnv, crefSubs, Deref @ Absyn::ComponentRef::CREF_QUAL { name: id, subscripts: Deref @ metamodelica::List::Nil, componentRef: restCref }, topPrefix, crefPrefix, r#impl, hasZeroSizeDim) => {
                    let mut t: Arc<DAE::Type>;
                    let mut sl: Arc<metamodelica::List<Arc<DAE::Dimension>>>;
                    let mut r#const: DAE::Const;
                    let mut cr: Arc<DAE::ComponentRef>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, crefEnv, crefSubs, Deref @ Absyn::ComponentRef::CREF_QUAL { name: id, subscripts: ss @ Deref @ metamodelica::List::Cons { head: _, tail: _ }, componentRef: restCref }, topPrefix, crefPrefix, r#impl, hasZeroSizeDim) => {
                    let mut t: Arc<DAE::Type>;
                    let mut sl: Arc<metamodelica::List<Arc<DAE::Dimension>>>;
                    let mut r#const: DAE::Const;
                    let mut const1: DAE::Const;
...
pub fn elabSubscripts(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inAbsynSubscriptLst: Arc<metamodelica::List<Arc<Absyn::Subscript>>>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<metamodelica::List<Arc<DAE::Subscript>>>, DAE::Const)> {
    ...
    let mut outExpSubscriptLst: Arc<metamodelica::List<Arc<DAE::Subscript>>>;
    let mut outConst: DAE::Const;
    (outCache, outExpSubscriptLst, outConst) = (::match_deref::match_deref! { match &((inCache, inEnv, inAbsynSubscriptLst, inBoolean, inPrefix)) {
        ...
        (cache, env, Deref @ metamodelica::List::Cons { head: sub, tail: subs }, r#impl, pre) => {
            let mut sub_1: Arc<DAE::Subscript>;
            let mut const1: DAE::Const;
            let mut const2: DAE::Const;
            let mut r#const: DAE::Const;
            let mut subs_1: Arc<metamodelica::List<Arc<DAE::Subscript>>>;
            ...
            (cache, subs_1, const2) = elabSubscripts(cache.clone(), env.clone(), subs.clone(), r#impl.clone(), pre.clone(), info)?;
            r#const = Types::constAnd(const1, const2);
            (cache.clone(), metamodelica::cons(sub_1, subs_1), r#const)
        },
        _ => bail!("match: no arm matched"),
    } });
...
fn elabSubscriptsDims(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inSubscripts: Arc<metamodelica::List<Arc<Absyn::Subscript>>>, mut inDimensions: Arc<metamodelica::List<Arc<DAE::Dimension>>>, mut inImpl: bool, mut inPrefix: DAE::Prefix, mut inCref: Arc<Absyn::ComponentRef>, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<metamodelica::List<Arc<DAE::Subscript>>>, DAE::Const)> {
    ...
    let mut dim: Arc<DAE::Dimension>;
    let mut dsub: Arc<DAE::Subscript>;
    let mut r#const: DAE::Const;
    ...
    if nrsubs > 0 {
        ...
        while nrsubs < nrdims {
            outSubs = metamodelica::cons(openmodelica_frontend_types::DAE::Subscript::interned_WHOLEDIM(), outSubs.clone());
            nrsubs = nrsubs + 1;
        }
    }
...
fn elabSubscriptsDims2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inSubscript: Arc<DAE::Subscript>, mut inDimension: Arc<DAE::Dimension>, mut inConst: DAE::Const, mut inProperties: Option<DAE::Properties>, mut inImpl: bool, mut inCref: Arc<Absyn::ComponentRef>, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Subscript>)> {
    ...
    (outCache, outSubscript) = 'mc: {
        let __mc_input = (inDimension.clone(), inProperties);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, _) => {
...
fn elabSubscript(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inSubscript: Arc<Absyn::Subscript>, mut inBoolean: bool, mut inPrefix: DAE::Prefix, mut info: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Subscript>, DAE::Const, Option<DAE::Properties>)> {
    ...
    (outCache, outSubscript, outConst, outProperties) = 'mc: {
        let __mc_input = (inCache, inEnv.clone(), inSubscript.clone(), inBoolean, inPrefix);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, Deref @ Absyn::Subscript::SUBSCRIPT { subscript: sub }, r#impl, pre) => {
                    ...
                    let mut ty: Arc<DAE::Type>;
                    let mut r#const: DAE::Const;
                    let mut sub_2: Arc<DAE::Subscript>;
                    let mut prop: DAE::Properties;
                    let mut cache = (*cache).clone();
...
fn elabSubscriptType(mut inType: Arc<DAE::Type>, mut inAbsynExp: Arc<Absyn::Exp>, mut inDaeExp: Arc<DAE::Exp>, mut inInfo: SourceInfo) -> Result<Arc<DAE::Subscript>> {
    '__tco: loop {
        ...
        Deref @ DAE::Type::T_INTEGER { .. } => {
            return Ok(Arc::new(DAE::Subscript::INDEX { exp: inDaeExp }))
        },
        ...
        Deref @ DAE::Type::T_ARRAY { ty: Deref @ DAE::Type::T_INTEGER { .. }, .. } => {
            return Ok(Arc::new(DAE::Subscript::SLICE { exp: inDaeExp }))
        },
        ...
        Deref @ DAE::Type::T_METABOXED { .. } => {
            { (inType, inAbsynExp, inDaeExp, inInfo) = (var_field!((*inType).ty, DAE::Type::T_METABOXED).clone(), inAbsynExp, inDaeExp, inInfo); continue '__tco; }
        },
        _ => {
            ...
            Error::addSourceMessage(Error::WRONG_DIMENSION_TYPE.clone(), list![(e_str).clone(), (t_str).clone()], inInfo)?;
            return Ok(bail!("fail"))
        },
        _ => return Err(anyhow::anyhow!("match: no arm matched")),
    } }
...
fn subscriptCrefType(mut inExp: Arc<DAE::Exp>, mut inType: Arc<DAE::Type>) -> Arc<DAE::Type> {
    let mut outType: Arc<DAE::Type>;
    outType = 'mc: {
...
fn subscriptType(mut inType: Arc<DAE::Type>, mut inExpSubscriptLst: Arc<metamodelica::List<Arc<DAE::Subscript>>>) -> Result<Arc<DAE::Type>> {
    ...
    outType = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: Deref @ DAE::Dimension::DIM_INTEGER { .. }, tail: Deref @ metamodelica::List::Nil }, ty: t }, Deref @ metamodelica::List::Cons { head: Deref @ DAE::Subscript::INDEX { .. }, tail: subs }) => {
                    let mut t_1: Arc<DAE::Type>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: dim, tail: Deref @ metamodelica::List::Nil }, ty: t }, Deref @ metamodelica::List::Cons { head: Deref @ DAE::Subscript::SLICE { .. }, tail: subs }) => {
                    let mut t_1: Arc<DAE::Type>;
                    t_1 = subscriptType(t.clone(), subs.clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ DAE::Type::T_ARRAY { dims: Deref @ metamodelica::List::Cons { head: dim, tail: Deref @ metamodelica::List::Nil }, ty: t }, Deref @ metamodelica::List::Cons { head: Deref @ DAE::Subscript::WHOLEDIM { .. }, tail: subs }) => {
                    let mut t_1: Arc<DAE::Type>;
                    t_1 = subscriptType(t.clone(), subs.clone())?;
                    Ok(Arc::new(DAE::Type::T_ARRAY { ty: t_1.clone(), dims: list![dim.clone()] }))
                }
...
fn makeIfExp(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inCondition: Arc<DAE::Exp>, mut inCondProp: DAE::Properties, mut inTrueBranch: Arc<DAE::Exp>, mut inTrueProp: DAE::Properties, mut inFalseBranch: Arc<DAE::Exp>, mut inFalseProp: DAE::Properties, mut inImplicit: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Exp>, DAE::Properties)> {
    ...
    let mut cond: bool;
    let mut cond_ty: Arc<DAE::Type>;
    let mut true_ty: Arc<DAE::Type>;
    let mut false_ty: Arc<DAE::Type>;
    let mut exp_ty: Arc<DAE::Type>;
    ...
    let DAE::PROP { type_: __pa0, constFlag: __pa1 } = (inCondProp) else { bail!("pattern mismatch") };
    cond_ty = __pa0.clone();
    cond_c = __pa1.clone();
    (cond_exp, _, ty_match) = Types::matchTypeNoFail(inCondition.clone(), cond_ty.clone(), DAE::T_BOOL_DEFAULT().clone());
    if !(ty_match) {
        cond_str = (ExpressionBasics::printExpStr(inCondition)?).clone();
        cond_ty_str = (TypesDump::unparseTypeNoAttr(cond_ty)?).clone();
        Error::addSourceMessageAndFail(Error::IF_CONDITION_TYPE_ERROR.clone(), list![(cond_str).clone(), (cond_ty_str).clone()], inInfo.clone())?;
        unreachable!("Error.addSourceMessageAndFail always fails — caller-side flow-analysis hint");
    }
    let DAE::PROP { type_: __pa2, constFlag: __pa3 } = (inTrueProp.clone()) else { bail!("pattern mismatch") };
    true_ty = __pa2.clone();
    true_c = __pa3.clone();
    let DAE::PROP { type_: __pa4, constFlag: __pa5 } = (inFalseProp.clone()) else { bail!("pattern mismatch") };
    false_ty = __pa4.clone();
    false_c = __pa5.clone();
    ...
    if !(ty_match) && !(Config::getGraphicsExpMode()?) {
        e1_str = (ExpressionBasics::printExpStr(inTrueBranch)?).clone();
        e2_str = (ExpressionBasics::printExpStr(inFalseBranch)?).clone();
        ty1_str = (TypesDump::unparseTypeNoAttr(true_ty)?).clone();
        ty2_str = (TypesDump::unparseTypeNoAttr(false_ty)?).clone();
        pre_str = (PrefixUtil::printPrefixStr3(inPrefix)?).clone();
    ...
    outExp = Arc::new(DAE::Exp::IFEXP { expCond: cond_exp, expThen: true_exp, expElse: false_exp });
    outProperties = DAE::Properties::PROP { type_: exp_ty, constFlag: exp_c };
    Ok((outCache, outExp, outProperties))
}
...
fn canonCref2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inComponentRef: Arc<DAE::ComponentRef>, mut inPrefixCref: Arc<DAE::ComponentRef>, mut inBoolean: bool) -> Result<(FCore::Cache, Arc<DAE::ComponentRef>)> {
    let mut outCache: FCore::Cache;
    let mut outComponentRef: Arc<DAE::ComponentRef>;
    (outCache, outComponentRef) = (::match_deref::match_deref! { match &((inCache, inEnv, inComponentRef, inPrefixCref, inBoolean)) {
        (cache, env, Deref @ DAE::ComponentRef::CREF_IDENT { ident: n, identType: ty2, subscriptLst: ss }, prefixCr, r#impl) => {
            let mut ss_1: Arc<metamodelica::List<Arc<DAE::Subscript>>>;
            let mut cr: Arc<DAE::ComponentRef>;
            let mut sl: Arc<metamodelica::List<i32>>;
            let mut t: Arc<DAE::Type>;
            let mut cache = (*cache).clone();
            cr = ComponentReference::crefPrependIdent(prefixCr.clone(), (n.clone()).clone(), metamodelica::nil(), ty2.clone())?;
            (cache, _, t, _, _, _, _, _, _) = Lookup::lookupVar(cache.clone(), env.clone(), cr)?;
            sl = Types::getDimensionSizes(t)?;
            (cache, ss_1) = Ceval::cevalSubscripts(cache.clone(), env.clone(), ss.clone(), sl, r#impl.clone(), openmodelica_ast::Absyn::Msg::NO_MSG, 0)?;
            (cache.clone(), ComponentReferenceBasics::makeCrefIdent((n.clone()).clone(), ty2.clone(), ss_1))
        },
        _ => bail!("match: no arm matched"),
    } });
...
fn slotAnd(mut s: Slot, mut b: bool) -> Result<bool> {
    let mut res: bool;
    let Slot { slotFilled: __pa0, .. } = (s) else { bail!("pattern mismatch") };
    res = __pa0.clone();
    res = b && res;
    Ok(res)
}
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ Absyn::Exp::CREF { componentRef: cr } => {
                    ...
                    match '__try0: {
                        let true = (id.clone() == literal!("OpenModelica")) else { break '__try0 Err::<_, _>(anyhow::anyhow!("pattern mismatch")) };
                        ...
                        Ok::<_, anyhow::Error>((dexp.clone(), prop.clone()))
                    } {
                        Ok((__try0_o0, __try0_o1)) => {
                            dexp = __try0_o0;
                            prop = __try0_o1;
                        ...
                        Err(_) => {
                            if '__try1: {
                                        unwrap_break_err!(Lookup::lookupClassIdent(cache.clone(), env.clone(), (id.clone()).clone(), None), '__try1);
                                        Ok::<(), anyhow::Error>(())
                            }.is_ok() { bail!("failure(): body succeeded") }
                    ...
                    let __pa2 = ::match_deref::match_deref! { match &(Types::getPropType(prop.clone())?) {
...
pub(crate) fn elabArrayDims(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inComponentRef: Arc<Absyn::ComponentRef>, mut inDimensions: Arc<metamodelica::List<Arc<Absyn::Subscript>>>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<metamodelica::List<Arc<DAE::Dimension>>>)> {
    let mut outCache: FCore::Cache;
    let mut outDimensions: Arc<metamodelica::List<Arc<DAE::Dimension>>>;
    (outCache, outDimensions) = elabArrayDims2(inCache, inEnv, inComponentRef, inDimensions, inImplicit, inDoVect, inPrefix, inInfo, metamodelica::nil())?;
...
fn elabArrayDims2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inCref: Arc<Absyn::ComponentRef>, mut inDimensions: Arc<metamodelica::List<Arc<Absyn::Subscript>>>, mut inImplicit: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo, mut inElaboratedDims: Arc<metamodelica::List<Arc<DAE::Dimension>>>) -> Result<(FCore::Cache, Arc<metamodelica::List<Arc<DAE::Dimension>>>)> {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: dim, tail: rest_dims } => {
            let mut cache: FCore::Cache;
            let mut elab_dim: Arc<DAE::Dimension>;
            let mut elab_dims: Arc<metamodelica::List<Arc<DAE::Dimension>>>;
            (cache, elab_dim) = elabArrayDim(inCache, inEnv.clone(), inCref.clone(), dim.clone(), inImplicit, inDoVect, inPrefix.clone(), inInfo.clone())?;
            elab_dims = metamodelica::cons(elab_dim, inElaboratedDims);
            { (inCache, inEnv, inCref, inDimensions, inImplicit, inDoVect, inPrefix, inInfo, inElaboratedDims) = (cache, inEnv, inCref, rest_dims.clone(), inImplicit, inDoVect, inPrefix, inInfo, elab_dims); continue '__tco; }
        },
        _ => return Err(anyhow::anyhow!("match: no arm matched")),
    } }
    }
...
fn elabArrayDim(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inCref: Arc<Absyn::ComponentRef>, mut inDimension: Arc<Absyn::Subscript>, mut inImpl: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Dimension>)> {
    ...
    (outCache, outDimension) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, Deref @ Absyn::Subscript::SUBSCRIPT { subscript: Deref @ Absyn::Exp::CALL { function_: Deref @ Absyn::ComponentRef::CREF_IDENT { name: Deref @ "size", .. }, functionArgs: Deref @ Absyn::FunctionArgs::FUNCTIONARGS { args: Deref @ metamodelica::List::Cons { head: cr_exp @ Deref @ Absyn::Exp::CREF { componentRef: cr }, tail: Deref @ metamodelica::List::Cons { head: size_arg, tail: Deref @ metamodelica::List::Nil } }, .. }, .. } }) => {
                    let mut dim: Arc<DAE::Dimension>;
                    let mut cache: FCore::Cache;
                    let mut e: Arc<DAE::Exp>;
                    let mut dim_exp: Arc<DAE::Exp>;
        ...
        Deref @ DAE::Type::T_BOOL { .. } => openmodelica_frontend_types::DAE::Dimension::interned_DIM_BOOLEAN(),
        _ => bail!("match: no arm matched"),
    } });
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, Deref @ Absyn::Subscript::SUBSCRIPT { subscript: Deref @ Absyn::Exp::EXPRESSIONCOMMENT { exp: sub, .. } }) => {
                    let mut dim: Arc<DAE::Dimension>;
                    let mut cache: FCore::Cache;
                    (cache, dim) = elabArrayDim(inCache.clone(), inEnv.clone(), inCref.clone(), Arc::new(Absyn::Subscript::SUBSCRIPT { subscript: sub.clone() }), inImpl, inDoVect, inPrefix.clone(), inInfo.clone())?;
                    Ok((cache.clone(), dim.clone()))
                }
                _ => bail!("nomatch"),
            }}
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, Deref @ Absyn::Subscript::SUBSCRIPT { subscript: sub }) => {
                    ...
                    let (__pa0, __pa1) = ::match_deref::match_deref! { match &(elabArrayDim2(cache.clone(), inEnv.clone(), inCref.clone(), e.clone(), prop.clone(), inImpl, inDoVect, inPrefix.clone(), inInfo.clone())?) {
...
fn elabArrayDim2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inCref: Arc<Absyn::ComponentRef>, mut inExp: Arc<DAE::Exp>, mut inProperties: DAE::Properties, mut inImpl: bool, mut inDoVect: bool, mut inPrefix: DAE::Prefix, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Option<Arc<DAE::Dimension>>)> {
    let mut outCache: FCore::Cache;
    let mut outDimension: Option<Arc<DAE::Dimension>>;
    (outCache, outDimension) = 'mc: {
        let __mc_input = (inProperties.clone(), inImpl);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
fn consStrippedCref(mut e: Arc<Absyn::Exp>, mut es: Arc<metamodelica::List<Arc<Absyn::Exp>>>) -> Result<Arc<metamodelica::List<Arc<Absyn::Exp>>>> {
    let mut oes: Arc<metamodelica::List<Arc<Absyn::Exp>>>;
    oes = (::match_deref::match_deref! { match &(e) {
        Deref @ Absyn::Exp::CREF { componentRef: cr } => {
            let mut cr = (*cr).clone();
            cr = AbsynUtil::crefStripLastSubs(cr.clone())?;
            metamodelica::cons(Arc::new(Absyn::Exp::CREF { componentRef: cr.clone() }), es)
        },
        _ => {
            es
        },
...
fn replaceEnd(mut inCref: Arc<Absyn::ComponentRef>) -> Result<Arc<Absyn::ComponentRef>> {
    ...
    let mut cr_parts: Arc<metamodelica::List<Arc<Absyn::ComponentRef>>>;
    let mut cr: Arc<Absyn::ComponentRef> = Arc::new(Absyn::ComponentRef::ALLWILD);
    let mut cr_no_subs: Arc<Absyn::ComponentRef>;
    ...
    outCref = replaceEndInSubs(AbsynUtil::crefStripLastSubs(outCref.clone())?, AbsynUtil::crefLastSubs(outCref)?)?;
...
fn replaceEndInSubs(mut inCref: Arc<Absyn::ComponentRef>, mut inSubscripts: Arc<metamodelica::List<Arc<Absyn::Subscript>>>) -> Result<Arc<Absyn::ComponentRef>> {
    ...
    let mut new_sub: Arc<Absyn::Subscript>;
    let mut i: i32 = 1;
    ...
    for mut sub in &*inSubscripts {
        ...
        subs = metamodelica::cons(new_sub.clone(), subs.clone());
        i = i + 1;
    }
...
fn replaceEndInSub(mut inSubscript: Arc<Absyn::Subscript>, mut inDimIndex: i32, mut inCref: Arc<Absyn::ComponentRef>) -> Result<Arc<Absyn::Subscript>> {
    ...
    outSubscript = (::match_deref::match_deref! { match &(inSubscript.clone()) {
        Deref @ Absyn::Subscript::SUBSCRIPT { .. } => Arc::new(Absyn::Subscript::SUBSCRIPT { subscript: replaceEndTraverser(var_field!((*inSubscript).subscript, Absyn::Subscript::SUBSCRIPT).clone(), (inCref, inDimIndex))? }),
        _ => inSubscript,
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    Ok(outSubscript)
...
fn replaceEndTraverser(mut inExp: Arc<Absyn::Exp>, mut inTuple: (Arc<Absyn::ComponentRef>, i32)) -> Result<Arc<Absyn::Exp>> {
    ...
    outExp = (::match_deref::match_deref! { match &(inExp.clone()) {
        Deref @ Absyn::Exp::END { .. } => {
            ...
            let mut i: i32;
            (cr, i) = inTuple;
            Arc::new(Absyn::Exp::CALL { function_: Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (literal!("size")).clone(), subscripts: metamodelica::nil() }), functionArgs: Arc::new(Absyn::FunctionArgs::FUNCTIONARGS { args: list![Arc::new(Absyn::Exp::CREF { componentRef: cr }), Arc::new(Absyn::Exp::INTEGER { value: i })], argNames: metamodelica::nil() }), typeVars: metamodelica::nil() })
        },
        Deref @ Absyn::Exp::CREF { .. } => {
            Arc::new(Absyn::Exp::CREF { componentRef: replaceEnd(var_field!((*inExp).componentRef, Absyn::Exp::CREF).clone())? })
        },
        _ => {
            AbsynUtil::traverseExpShallow(inExp, inTuple, (std::sync::Arc::new(replaceEndTraverser) as std::sync::Arc<dyn ::std::ops::Fn(Arc<Absyn::Exp>, (Arc<Absyn::ComponentRef>, i32)) -> Result<Arc<Absyn::Exp>> + 'static>))?
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    Ok(outExp)
...
fn fixTupleMetaModelica(mut exps: Arc<metamodelica::List<Arc<DAE::Exp>>>, mut types: Arc<metamodelica::List<Arc<DAE::Type>>>, mut consts: Arc<metamodelica::List<Arc<DAE::TupleConst>>>) -> Result<(Arc<DAE::Exp>, DAE::Properties)> {
    ...
    if Config::acceptMetaModelicaGrammar()? {
        ...
        exp = Arc::new(DAE::Exp::META_TUPLE { listExp: exps2 });
        prop = DAE::Properties::PROP { type_: Arc::new(DAE::Type::T_METATUPLE { types: tys2 }), constFlag: c };
    } else {
        exp = Arc::new(DAE::Exp::TUPLE { PR: exps });
        prop = DAE::Properties::PROP_TUPLE { type_: Arc::new(DAE::Type::T_TUPLE { types: types, names: None }), tupleConst: Arc::new(DAE::TupleConst::TUPLE_CONST { tupleConstLst: consts }) };
    }
    Ok((exp, prop))
}
...
fn checkBuiltinCallArgs(mut inPosArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>, mut inNamedArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inExpectedArgs: i32, mut inFnName: ArcStr, mut inInfo: SourceInfo) -> Result<()> {
    if (inPosArgs.len() as i32) != inExpectedArgs || !(inNamedArgs.is_empty()) {
        Error::addSourceMessageAndFail(Error::WRONG_NO_OF_ARGS.clone(), list![(inFnName).clone()], inInfo)?;
        unreachable!("Error.addSourceMessageAndFail always fails — caller-side flow-analysis hint");
    }
    Ok(())
}
