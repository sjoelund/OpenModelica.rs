        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, ci_state, r#mod, pre, n, cl @ Deref @ SCode::Element::CLASS { name: typeName, .. }, attr, pf, dims, idxs, inst_dims, r#impl, comment, graph, csets) => {
                    ...
                    let true = (AbsynUtil::isOnlyInner(io)) else { bail!("pattern mismatch") };
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, ci_state, r#mod, pre, n, cl, attr, pf, dims, idxs, inst_dims, r#impl, comment, graph, csets) => {
                    ...
                    let mut graph = (*graph).clone();
                    let mut csets = (*csets).clone();
                    let true = (AbsynUtil::isOnlyOuter(io)) else { bail!("pattern mismatch") };
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, ci_state, r#mod, pre, n, cl, attr @ SCode::Attributes { direction: Absyn::Direction::OUTPUT { .. }, .. }, pf, dims, idxs, inst_dims, r#impl, comment, graph, csets) => {
                    ...
                    let (__pa0, __pa1, __pa2, __pa3, __pa4) = ::match_deref::match_deref! { match &(InnerOuter::lookupInnerVar(cache.clone(), env.clone(), ih.clone(), pre.clone(), (n.clone()).clone(), io)?) {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, _, r#mod, pre, n, _, _, _, _, _, _, _, _, graph, csets) => {
                    ...
                    let mut crefOuter: Arc<DAE::ComponentRef>;
                    let mut crefInner: Arc<DAE::ComponentRef>;
                    let mut outers: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
                    ...
                    let true = (AbsynUtil::isOnlyOuter(io)) else { bail!("pattern mismatch") };
                    let true = (Mod::modEqual(r#mod.clone(), openmodelica_frontend_types::DAE::Mod::interned_NOMOD())?) else { bail!("pattern mismatch") };
                    let (__pa0, __pa1, __pa2, __pa3, __pa4, __pa5, __pa12, __pa6, __pa7, __pa8, __pa9, __pa10, __pa11, __pa13) = ::match_deref::match_deref! { match &(InnerOuter::lookupInnerVar(cache.clone(), env.clone(), ih.clone(), pre.clone(), (n.clone()).clone(), io)?) {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, ci_state, r#mod, pre, n, cl, attr, pf, dims, idxs, inst_dims, r#impl, comment, graph, csets) => {
                    ...
                    let true = (AbsynUtil::isOnlyOuter(io)) else { bail!("pattern mismatch") };
                    let true = (Mod::modEqual(r#mod.clone(), openmodelica_frontend_types::DAE::Mod::interned_NOMOD())?) else { bail!("pattern mismatch") };
                    let __pa0 = ::match_deref::match_deref! { match &(InnerOuter::lookupInnerVar(cache.clone(), env.clone(), ih.clone(), pre.clone(), (n.clone()).clone(), io)?) {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, ci_state, r#mod, pre, n, cl, attr, pf, dims, idxs, inst_dims, r#impl, comment, graph, csets) => {
                    ...
                    let true = (Mod::modEqual(r#mod.clone(), openmodelica_frontend_types::DAE::Mod::interned_NOMOD())?) else { bail!("pattern mismatch") };
                    if '__try0: {
                        unwrap_break_err!(InnerOuter::lookupInnerVar(cache.clone(), env.clone(), ih.clone(), pre.clone(), (n.clone()).clone(), io), '__try0);
                        Ok::<(), anyhow::Error>(())
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, ci_state, r#mod, pre, n, cl @ Deref @ SCode::Element::CLASS { name: typeName, .. }, attr @ SCode::Attributes { direction: Absyn::Direction::OUTPUT { .. }, .. }, pf, dims, idxs, inst_dims, r#impl, comment, graph, csets) => {
                    ...
                    let mut store = (*store).clone();
                    let mut graph = (*graph).clone();
                    let true = (AbsynUtil::isInnerOuter(io)) else { bail!("pattern mismatch") };
                    topInstance = listHead(ih.clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, ci_state, r#mod, pre, n, cl, attr, pf, dims, idxs, inst_dims, r#impl, comment, graph, csets) => {
                    ...
                    let mut graph = (*graph).clone();
                    let mut csets = (*csets).clone();
                    let true = (AbsynUtil::isNotInnerOuter(io)) else { bail!("pattern mismatch") };
                    (cache, compenv, ih, store, dae, csets, ty, graph) = instVar_dispatch(cache.clone(), env.clone(), ih.clone(), store.clone(), ci_state.clone(), r#mod.clone(), pre.clone(), (n.clone()).clone(), cl.clone(), attr.clone(), pf.clone(), dims.clone(), idxs.clone(), inst_dims.clone(), r#impl.clone(), comment.clone(), info.clone(), graph.clone(), csets.clone())?;
                    Ok((cache.clone(), compenv.clone(), ih.clone(), store.clone(), dae.clone(), csets.clone(), ty.clone(), graph.clone()))
                }
                _ => bail!("nomatch"),
...
fn liftUserTypeMod(mut inMod: Arc<DAE::Mod>, mut inDims: Arc<metamodelica::List<Arc<DAE::Dimension>>>) -> Arc<DAE::Mod> {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, ci_state, r#mod @ Deref @ DAE::Mod::MOD { binding: None, .. }, pre, n, cl @ Deref @ SCode::Element::CLASS { restriction: SCode::Restriction::R_RECORD { isOperator: _ }, .. }, attr, pf, dims, _, inst_dims, r#impl, comment, info, graph, csets) => {
                    ...
                    let mut dae: DAE::DAElist;
                    let mut dae_var_attr: Option<Arc<DAE::VariableAttributes>>;
                    ...
                    let (__pa2, __pa3, __pa4) = ::match_deref::match_deref! { match &(pf.clone()) {
                        Deref @ SCode::Prefixes { visibility: __pa2, finalPrefix: __pa3, innerOuter: __pa4, .. } => (__pa2.clone(), __pa3.clone(), __pa4.clone()),
                        _ => bail!("pattern mismatch"),
                    } };
                    vis = __pa2.clone();
                    fin = __pa3.clone();
            ...
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, ci_state, r#mod, pre, n, cl @ Deref @ SCode::Element::CLASS { .. }, attr, pf, dims, _, inst_dims, r#impl, comment, info, graph, csets) => {
                    ...
                    let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(pf.clone()) {
                        Deref @ SCode::Prefixes { visibility: __pa0, finalPrefix: __pa1, innerOuter: __pa2, .. } => (__pa0.clone(), __pa1.clone(), __pa2.clone()),
                        _ => bail!("pattern mismatch"),
                    } };
                    vis = __pa0.clone();
                    fin = __pa1.clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, ci_state, r#mod @ Deref @ DAE::Mod::MOD { binding: Some(DAE::EqMod::TYPED { .. }), .. }, pre, n, cl, attr, pf, Deref @ metamodelica::List::Cons { head: dim @ Deref @ DAE::Dimension::DIM_UNKNOWN { .. }, tail: dims }, idxs, inst_dims, r#impl, comment, info, graph, csets) => {
                    ...
                    let mut dae: DAE::DAElist;
                    let mut dim2: Arc<DAE::Dimension>;
                    let mut dime2: Arc<DAE::Subscript>;
                    let mut cache = (*cache).clone();
                    let mut ih = (*ih).clone();
                    let mut store = (*store).clone();
                    let mut graph = (*graph).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, ci_state, r#mod, pre, n, cl, attr, pf, Deref @ metamodelica::List::Cons { head: dim, tail: dims }, idxs, inst_dims, r#impl, comment, info, graph, csets) => {
                    ...
                    let mut ty: Arc<DAE::Type>;
                    let mut dae: DAE::DAElist;
                    let mut dime: Arc<DAE::Subscript>;
                    let mut cache = (*cache).clone();
                    let mut ih = (*ih).clone();
                    let mut store = (*store).clone();
                    let mut graph = (*graph).clone();
...
fn checkArrayModBindingDimSize(mut inBinding: Option<DAE::EqMod>, mut inDimension: Arc<DAE::Dimension>, mut inPrefix: DAE::Prefix, mut inIdent: ArcStr, mut inInfo: SourceInfo) -> bool {
    ...
    outIsCorrect = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ...
            let mut ty_dims: Arc<metamodelica::List<Arc<DAE::Dimension>>>;
