...
fn instantiateClass_dispatch(mut inCache: FCore::Cache, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inProgram: Arc<metamodelica::List<Arc<SCode::Element>>>, mut inPath: Arc<Absyn::Path>, mut doSCodeDep: bool, mut relaxedFrontEnd: bool, mut clearCache: bool) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, DAE::DAElist)> {
    ...
    (outCache, outEnv, outIH, outDAElist) = (::match_deref::match_deref! { match &((inCache, inIH, inProgram, inPath.clone())) {
        (cache, ih, cdecls @ Deref @ metamodelica::List::Cons { head: _, tail: _ }, path @ Deref @ Absyn::Path::IDENT { .. }) => {
            ...
            env = FGraphBuildEnv::mkProgramGraph(cdecls.clone(), openmodelica_frontend_dump::FCore::Kind::USERDEFINED, env)?;
            ...
            (cache, env, ih, dae2) = instClassInProgram(cache.clone(), env, ih.clone(), cdecls.clone(), path.clone(), source, relaxedFrontEnd)?;
            if clearCache {
                InstHashTable::release()?;
            }
            (cache.clone(), env, ih.clone(), dae2)
        },
        (cache, ih, cdecls @ Deref @ metamodelica::List::Cons { head: _, tail: _ }, path @ Deref @ Absyn::Path::QUALIFIED { .. }) => {
            ...
            (cache, env) = Builtin::initialGraph(cache.clone())?;
            env = FGraphBuildEnv::mkProgramGraph(cdecls.clone(), openmodelica_frontend_dump::FCore::Kind::USERDEFINED, env)?;
            let (__pa0, __pa2, __pa1, __pa3) = ::match_deref::match_deref! { match &(Lookup::lookupClass(cache.clone(), env, path.clone(), Some(Absyn::dummyInfo.clone()))?) {
            ...
            cdef = __pa2.clone();
            env = __pa3.clone();
            checkInstanceRestriction(cdef.clone(), path.clone(), relaxedFrontEnd)?;
            if Flags::isSet(Flags::GC_PROF.clone())? {
                metamodelica::print(({ let mut __mm_s = String::new(); __mm_s.push_str(&*GCExt::profStatsStr(GCExt::getProfStats(), (literal!("GC stats after pre-frontend work (building graphs):")).clone(), (literal!("\n  ")).clone())?); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
            }
            ExecStat::execStat((literal!("FrontEnd - mkProgramGraph")).clone())?;
            (cache, env, ih, _, dae, _, _, _, _, _) = instClass(cache.clone(), env.clone(), ih.clone(), UnitAbsynBuilder::emptyInstStore(), openmodelica_frontend_types::DAE::Mod::interned_NOMOD(), makeTopComponentPrefix(env, (n).clone()), cdef.clone(), metamodelica::nil(), false, openmodelica_frontend_inst::InstTypes::CallingScope::TOP_CALL, ConnectionGraph::EMPTY().clone(), Connect::emptySet().clone())?;
            ...
            dae = DAE::DAElist { elementLst: list![Arc::new(DAE::Element::COMP { ident: (pathstr).clone(), dAElist: daeElts, source: source, comment: cmt })] };
            if clearCache {
                InstHashTable::release()?;
            }
            (cache.clone(), env, ih.clone(), dae)
        },
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, ih, cdecls @ Deref @ metamodelica::List::Cons { head: _, tail: _ }, path @ Deref @ Absyn::Path::QUALIFIED { .. }) => {
                    ...
                    (cache, env_2, ih, _, dae, _, _, _, _, _) = instClass(cache.clone(), env_2.clone(), ih.clone(), UnitAbsynBuilder::emptyInstStore(), openmodelica_frontend_types::DAE::Mod::interned_NOMOD(), makeTopComponentPrefix(env_2.clone(), (n.clone()).clone()), cdef.clone(), metamodelica::nil(), false, openmodelica_frontend_inst::InstTypes::CallingScope::TOP_CALL, ConnectionGraph::EMPTY().clone(), Connect::emptySet().clone())?;
                    pathstr = (AbsynUtil::pathString(path.clone(), (literal!(".")).clone(), true, false)?).clone();
                    source = ElementSource::addElementSourcePartOfOpt(DAE::emptyElementSource().clone(), FGraph::getScopePath(env.clone())?)?;
...
fn instClassInProgram(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inProgram: Arc<metamodelica::List<Arc<SCode::Element>>>, mut inPath: Arc<Absyn::Path>, mut inSource: Arc<DAE::ElementSource>, mut relaxedFrontEnd: bool) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, DAE::DAElist)> {
    ...
    (outCache, outEnv, outIH, outDae) = 'mc: {
        let __mc_input = (inProgram.clone(), inPath);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
pub fn instClass(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inStore: UnitAbsyn::InstStore, mut inMod: Arc<DAE::Mod>, mut inPrefix: DAE::Prefix, mut inClass: Arc<SCode::Element>, mut inInstDims: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>, mut inImplicit: bool, mut inCallingScope: InstTypes::CallingScope, mut inGraph: ConnectionGraph::ConnectionGraph, mut inSets: DAE::Connect::Sets) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, UnitAbsyn::InstStore, DAE::DAElist, DAE::Connect::Sets, Arc<DAE::Type>, ClassInf::State, Option<SCode::Attributes>, ConnectionGraph::ConnectionGraph)> {
    ...
    (cache, outEnv, outIH, outStore, outDae, outSets, outType, outState, optDerAttr, outGraph) = 'mc: {
        let __mc_input = (inCache.clone(), inEnv.clone(), inIH.clone(), inStore, inMod.clone(), inPrefix.clone(), inClass.clone(), inInstDims.clone(), inImplicit, inCallingScope.clone(), inGraph.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, _, _, store, _, _, Deref @ SCode::Element::CLASS { name: n, partialPrefix: SCode::Partial::PARTIAL { .. }, restriction: r, info, .. }, _, _, _, _) => {
                    ...
                    (cache, env, ih, store, dae, csets, ty, ci_state_1, oDA, graph) = instClass(inCache.clone(), inEnv.clone(), inIH.clone(), store.clone(), inMod.clone(), inPrefix.clone(), c.clone(), inInstDims.clone(), inImplicit, inCallingScope.clone(), inGraph.clone(), inSets.clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, r#mod, pre, c @ Deref @ SCode::Element::CLASS { name: n, encapsulatedPrefix: encflag, restriction: r, partialPrefix, info, .. }, inst_dims, r#impl, callscope, graph) => {
                    ...
                    let mut dae: DAE::DAElist;
                    let mut tys: Arc<metamodelica::List<Arc<DAE::Var>>>;
                    let mut bc_ty: Option<Arc<DAE::Type>>;
                    let mut fq_class: Arc<Absyn::Path>;
                    let mut ty: Arc<DAE::Type>;
                    let mut oDA: Option<SCode::Attributes>;
                    let mut equalityConstraint: Option<(Arc<Absyn::Path>, i32, DAE::InlineType)>;
                    let mut cache = (*cache).clone();
                    let mut ih = (*ih).clone();
                    let mut store = (*store).clone();
...
fn instClassBasictype(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inStore: UnitAbsyn::InstStore, mut inMod: Arc<DAE::Mod>, mut inPrefix: DAE::Prefix, mut inClass: Arc<SCode::Element>, mut inInstDims: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>, mut inImplicit: bool, mut inCallingScope: InstTypes::CallingScope, mut inSets: DAE::Connect::Sets) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, UnitAbsyn::InstStore, DAE::DAElist, DAE::Connect::Sets, Arc<DAE::Type>, Arc<metamodelica::List<Arc<DAE::Var>>>, ClassInf::State)> {
    ...
    let mut outSets: DAE::Connect::Sets;
    let mut outType: Arc<DAE::Type>;
    let mut outTypeVars: Arc<metamodelica::List<Arc<DAE::Var>>>;
    let mut outState: ClassInf::State;
    (outCache, outEnv, outIH, outStore, outDae, outSets, outType, outTypeVars, outState) = (::match_deref::match_deref! { match &((inCache, inEnv, inIH, inStore, inMod, inPrefix, inClass, inInstDims, inImplicit)) {
        (cache, env, ih, store, r#mod, pre, c @ Deref @ SCode::Element::CLASS { name: n, encapsulatedPrefix: encflag, restriction: r, .. }, inst_dims, r#impl) => {
            ...
            (cache, env_3, ih, store, dae1, csets, ci_state_1, tys, bc_ty, _, _, _) = instClassIn(cache.clone(), env_1, ih.clone(), store.clone(), r#mod.clone(), pre.clone(), ci_state, c_1, openmodelica_frontend_types::SCode::Visibility::PUBLIC, inst_dims.clone(), r#impl.clone(), openmodelica_frontend_inst::InstTypes::CallingScope::INNER_CALL, ConnectionGraph::EMPTY().clone(), inSets, None)?;
            ...
            dae = dae1_1;
            ty = InstUtil::mktypeWithArrays(fq_class, ci_state_1.clone(), tys.clone(), bc_ty, c.clone(), InstUtil::extractComment(dae.elementLst.clone())?)?;
            (cache.clone(), env_3, ih.clone(), store.clone(), dae, csets, ty, tys, ci_state_1)
        },
...
pub fn instClassIn(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inStore: UnitAbsyn::InstStore, mut inMod: Arc<DAE::Mod>, mut inPrefix: DAE::Prefix, mut inState: ClassInf::State, mut inClass: Arc<SCode::Element>, mut inVisibility: SCode::Visibility, mut inInstDims: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>, mut implicitInstantiation: bool, mut inCallingScope: InstTypes::CallingScope, mut inGraph: ConnectionGraph::ConnectionGraph, mut inSets: DAE::Connect::Sets, mut instSingleCref: Option<Arc<DAE::ComponentRef>>) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, UnitAbsyn::InstStore, DAE::DAElist, DAE::Connect::Sets, ClassInf::State, Arc<metamodelica::List<Arc<DAE::Var>>>, Option<Arc<DAE::Type>>, Option<SCode::Attributes>, Option<(Arc<Absyn::Path>, i32, DAE::InlineType)>, ConnectionGraph::ConnectionGraph)> {
    ...
    let mut optDerAttr: Option<SCode::Attributes>;
    let mut outEqualityConstraint: Option<(Arc<Absyn::Path>, i32, DAE::InlineType)>;
    let mut outGraph: ConnectionGraph::ConnectionGraph;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ SCode::Element::CLASS { prefixes: Deref @ SCode::Prefixes { innerOuter: io, .. }, .. } => {
                    let mut bc: Option<Arc<DAE::Type>>;
                    let mut env: FCore::Graph;
                    let mut ci_state: ClassInf::State;
                    let mut dae: DAE::DAElist;
                    let mut csets: DAE::Connect::Sets;
                    ...
                    (cache, env, ih, store, ci_state, graph, csets, dae, tys, bc, oDA, equalityConstraint) = instClassIn2(inCache.clone(), inEnv.clone(), inIH.clone(), inStore.clone(), inMod.clone(), inPrefix.clone(), inState.clone(), inClass.clone(), inVisibility.clone(), inInstDims.clone(), implicitInstantiation, inCallingScope.clone(), inGraph.clone(), inSets.clone(), instSingleCref.clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ SCode::Element::CLASS { name: n, prefixes: Deref @ SCode::Prefixes { innerOuter: io, .. }, .. } => {
                    ...
                    c = __pa0.clone();
                    (cache, env, ih, store, ci_state, graph, csets, dae, tys, bc, oDA, equalityConstraint) = instClassIn2(inCache.clone(), inEnv.clone(), inIH.clone(), inStore.clone(), inMod.clone(), inPrefix.clone(), inState.clone(), c.clone(), inVisibility.clone(), inInstDims.clone(), implicitInstantiation, inCallingScope.clone(), inGraph.clone(), inSets.clone(), instSingleCref.clone())?;
                    Ok((cache.clone(), env.clone(), ih.clone(), store.clone(), dae.clone(), csets.clone(), ci_state.clone(), tys.clone(), bc.clone(), oDA.clone(), equalityConstraint.clone(), graph.clone()))
                }
                _ => bail!("nomatch"),
            }}
...
pub(crate) fn instClassIn2(mut cache: FCore::Cache, mut env: FCore::Graph, mut ih: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut store: UnitAbsyn::InstStore, mut r#mod: Arc<DAE::Mod>, mut prefix: DAE::Prefix, mut state: ClassInf::State, mut cls: Arc<SCode::Element>, mut visibility: SCode::Visibility, mut instDims: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>, mut implicitInst: bool, mut callingScope: InstTypes::CallingScope, mut graph: ConnectionGraph::ConnectionGraph, mut sets: DAE::Connect::Sets, mut instSingleCref: Option<Arc<DAE::ComponentRef>>) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, UnitAbsyn::InstStore, ClassInf::State, ConnectionGraph::ConnectionGraph, DAE::Connect::Sets, DAE::DAElist, Arc<metamodelica::List<Arc<DAE::Var>>>, Option<Arc<DAE::Type>>, Option<SCode::Attributes>, Option<(Arc<Absyn::Path>, i32, DAE::InlineType)>)> {
    ...
    let mut ty: Option<Arc<DAE::Type>>;
    let mut optDerAttr: Option<SCode::Attributes>;
    let mut equalityConstraint: Option<(Arc<Absyn::Path>, i32, DAE::InlineType)>;
    ...
    let mut r#impl: bool;
    let mut scr: Option<Arc<DAE::ComponentRef>>;
    ...
    if SCodeUtil::isPackage(cls.clone()) && SCodeUtil::isPartial(cls.clone()) {
        (cache, env, ih, state, _) = partialInstClassIn(cache, env, ih, r#mod, prefix, state, cls, visibility, instDims, 0)?;
    ...
    if Flags::isSet(Flags::CACHE.clone())? {
        if '__try0: {
            ...
            if dims.clone() == instDims.clone() && r#impl == implicitInst && m.clone() == r#mod.clone() && csets.clone() == sets.clone() && st.clone() == state.clone() && e.clone() == cls.clone() && scr.clone() == instSingleCref.clone() && callingScopeCacheEq(cs.clone(), callingScope.clone()) {
    ...
    match '__try13: {
        inputs = (r#mod.clone(), prefix.clone(), sets.clone(), state.clone(), cls.clone(), instDims.clone(), implicitInst, instSingleCref.clone(), callingScope.clone());
        (cache, env, ih, store, dae, sets, state, vars, ty, optDerAttr, equalityConstraint, graph) = unwrap_break_err!(instClassIn_dispatch(cache.clone(), env.clone(), ih.clone(), store.clone(), r#mod.clone(), prefix.clone(), state.clone(), cls.clone(), visibility.clone(), instDims.clone(), implicitInst, callingScope.clone(), graph.clone(), sets.clone(), instSingleCref.clone()), '__try13);
        ...
        InstHashTable::addToInstCache(cache_path.clone(), Some(InstHashTable::CachedInstItem::FUNC_instClassIn { inputs: inputs.clone(), outputs: outputs.clone() }), None);
...
fn markDerivedRecordOutsideBindings(mut inType: Arc<DAE::Type>, mut inClass: Arc<SCode::Element>) -> Result<Arc<DAE::Type>> {
    ...
    outType = (::match_deref::match_deref! { match &(inType.clone()) {
        Deref @ DAE::Type::T_COMPLEX { .. } => {
            let mut tvars: Arc<metamodelica::List<Arc<DAE::Var>>>;
            ...
            tvars = tvars.reverse();
            Arc::new(DAE::Type::T_COMPLEX { complexClassType: var_field!((*inType).complexClassType, DAE::Type::T_COMPLEX).clone(), varLst: tvars, equalityConstraint: var_field!((*inType).equalityConstraint, DAE::Type::T_COMPLEX).clone(), usedExternally: var_field!((*inType).usedExternally, DAE::Type::T_COMPLEX).clone() })
        },
        _ => bail!("match: no arm matched"),
    } });
    Ok(outType)
...
fn varIsModifiedInDerivedMod(mut inName: ArcStr, mut inSubmod: Arc<SCode::SubMod>) -> Result<bool> {
    ...
    b = (::match_deref::match_deref! { match &(inSubmod.clone()) {
        ...
        Deref @ SCode::SubMod { .. } => stringEqual((inSubmod.ident.clone()).clone(), (inName).clone()),
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    Ok(b)
}
...
fn varIsModifiedInMod(mut inName: ArcStr, mut inSubmod: Arc<DAE::SubMod>) -> Result<bool> {
    let mut b: bool;
    b = (::match_deref::match_deref! { match &(inSubmod.clone()) {
        Deref @ DAE::SubMod { .. } => stringEqual((inSubmod.ident.clone()).clone(), (inName).clone()),
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    Ok(b)
}
...
pub(crate) fn instClassIn_dispatch(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inStore: UnitAbsyn::InstStore, mut inMod: Arc<DAE::Mod>, mut inPrefix: DAE::Prefix, mut inState: ClassInf::State, mut inClass: Arc<SCode::Element>, mut inVisibility: SCode::Visibility, mut inInstDims: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>, mut implicitInstantiation: bool, mut inCallingScope: InstTypes::CallingScope, mut inGraph: ConnectionGraph::ConnectionGraph, mut inSets: DAE::Connect::Sets, mut instSingleCref: Option<Arc<DAE::ComponentRef>>) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, UnitAbsyn::InstStore, DAE::DAElist, DAE::Connect::Sets, ClassInf::State, Arc<metamodelica::List<Arc<DAE::Var>>>, Option<Arc<DAE::Type>>, Option<SCode::Attributes>, Option<(Arc<Absyn::Path>, i32, DAE::InlineType)>, ConnectionGraph::ConnectionGraph)> {
    ...
    let mut outSets: DAE::Connect::Sets;
    let mut outState: ClassInf::State;
    let mut outTypesVarLst: Arc<metamodelica::List<Arc<DAE::Var>>>;
    let mut outTypesTypeOption: Option<Arc<DAE::Type>>;
    let mut optDerAttr: Option<SCode::Attributes>;
    let mut outEqualityConstraint: Option<(Arc<Absyn::Path>, i32, DAE::InlineType)>;
    let mut outGraph: ConnectionGraph::ConnectionGraph;
    (outCache, outEnv, outIH, outStore, outDae, outSets, outState, outTypesVarLst, outTypesTypeOption, optDerAttr, outEqualityConstraint, outGraph) = 'mc: {
        let __mc_input = (inCache, inEnv, inIH, inStore, inMod, inPrefix, inState, inClass, inVisibility, inInstDims, implicitInstantiation, inCallingScope, inGraph);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, mods, pre, ci_state, c @ Deref @ SCode::Element::CLASS { name: n, restriction: SCode::Restriction::R_ENUMERATION { .. }, classDef: Deref @ SCode::ClassDef::PARTS { elementLst: els, .. }, info, .. }, _, inst_dims, r#impl, callscope, graph) => {
                    ...
                    let mut env_2: FCore::Graph;
                    let mut env_3: FCore::Graph;
                    let mut comp: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>;
                    let mut names: Arc<metamodelica::List<ArcStr>>;
                    let mut eqConstraint: Option<(Arc<Absyn::Path>, i32, DAE::InlineType)>;
                    let mut ty: Arc<DAE::Type>;
                    let mut ty2: Arc<DAE::Type>;
                    let mut fq_class: Arc<Absyn::Path>;
                    let mut tys1: Arc<metamodelica::List<Arc<DAE::Var>>>;
                    let mut tys2: Arc<metamodelica::List<Arc<DAE::Var>>>;
                    let mut cache = (*cache).clone();
                    let mut ih = (*ih).clone();
                    let mut store = (*store).clone();
                    let mut graph = (*graph).clone();
...
fn getBasicTypeType(mut inName: ArcStr) -> Result<Arc<DAE::Type>> {
    let mut outType: Arc<DAE::Type>;
    outType = (::match_deref::match_deref! { match &(inName) {
...
fn getBasicTypeAttrTyper(mut inName: ArcStr) -> Result<Arc<dyn ::std::ops::Fn(ArcStr, Arc<DAE::Type>, SourceInfo) -> Result<Arc<DAE::Type>> + 'static>> {
    ...
    outTyper = (::match_deref::match_deref! { match &(inName) {
...
fn getRealAttributeType(mut inAttrName: ArcStr, mut inBaseType: Arc<DAE::Type>, mut inInfo: SourceInfo) -> Result<Arc<DAE::Type>> {
    ...
    outType = (::match_deref::match_deref! { match &(inAttrName.clone()) {
        ...
        _ => {
            Error::addSourceMessage(Error::MISSING_MODIFIED_ELEMENT.clone(), list![(inAttrName).clone(), (literal!("Real")).clone()], inInfo)?;
            bail!("fail")
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn getIntAttributeType(mut inAttrName: ArcStr, mut inBaseType: Arc<DAE::Type>, mut inInfo: SourceInfo) -> Result<Arc<DAE::Type>> {
    ...
    outType = (::match_deref::match_deref! { match &(inAttrName.clone()) {
        ...
        Deref @ "distribution" => InstBinding::distributionType().clone(),
        _ => {
            Error::addSourceMessage(Error::MISSING_MODIFIED_ELEMENT.clone(), list![(inAttrName).clone(), (literal!("Integer")).clone()], inInfo)?;
            bail!("fail")
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn getStringAttributeType(mut inAttrName: ArcStr, mut inBaseType: Arc<DAE::Type>, mut inInfo: SourceInfo) -> Result<Arc<DAE::Type>> {
    ...
    outType = (::match_deref::match_deref! { match &(inAttrName.clone()) {
        ...
        Deref @ "start" => inBaseType,
        _ => {
            Error::addSourceMessage(Error::MISSING_MODIFIED_ELEMENT.clone(), list![(inAttrName).clone(), (literal!("String")).clone()], inInfo)?;
            bail!("fail")
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn getBoolAttributeType(mut inAttrName: ArcStr, mut inBaseType: Arc<DAE::Type>, mut inInfo: SourceInfo) -> Result<Arc<DAE::Type>> {
    ...
    outType = (::match_deref::match_deref! { match &(inAttrName.clone()) {
        ...
        Deref @ "fixed" => DAE::T_BOOL_DEFAULT().clone(),
        _ => {
            Error::addSourceMessage(Error::MISSING_MODIFIED_ELEMENT.clone(), list![(inAttrName).clone(), (literal!("Boolean")).clone()], inInfo)?;
            bail!("fail")
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn getEnumAttributeType(mut inAttrName: ArcStr, mut inBaseType: Arc<DAE::Type>, mut inInfo: SourceInfo) -> Result<Arc<DAE::Type>> {
    ...
    outType = (::match_deref::match_deref! { match &(inAttrName.clone()) {
        ...
        Deref @ "fixed" => DAE::T_BOOL_DEFAULT().clone(),
        _ => {
            Error::addSourceMessage(Error::MISSING_MODIFIED_ELEMENT.clone(), list![(inAttrName).clone(), (literal!("enumeration(:)")).clone()], inInfo)?;
            bail!("fail")
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn instBasicTypeAttributes(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inMod: Arc<DAE::Mod>, mut inBaseType: Arc<DAE::Type>, mut inTypeFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<DAE::Type>, SourceInfo) -> Result<Arc<DAE::Type>> + 'static>, mut inPrefix: DAE::Prefix) -> Result<Arc<metamodelica::List<Arc<DAE::Var>>>> {
    let mut outVars: Arc<metamodelica::List<Arc<DAE::Var>>>;
    outVars = (::match_deref::match_deref! { match &(inMod) {
        Deref @ DAE::Mod::MOD { subModLst: submods, .. } => {
            List::map4(submods.clone(), (std::sync::Arc::new(instBasicTypeAttributes2) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::SubMod>, FCore::Cache, FCore::Graph, Arc<DAE::Type>, Arc<dyn ::std::ops::Fn(ArcStr, Arc<DAE::Type>, SourceInfo) -> Result<Arc<DAE::Type>> + 'static>) -> Result<Arc<DAE::Var>> + 'static>), inCache, inEnv, inBaseType, inTypeFunc.clone())?
        },
        Deref @ DAE::Mod::NOMOD { .. } => {
            metamodelica::nil()
        },
...
fn instBasicTypeAttributes2(mut inSubMod: Arc<DAE::SubMod>, mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inBaseType: Arc<DAE::Type>, mut inTypeFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<DAE::Type>, SourceInfo) -> Result<Arc<DAE::Type>> + 'static>) -> Result<Arc<DAE::Var>> {
    ...
    outVar = (::match_deref::match_deref! { match &(inSubMod) {
        Deref @ DAE::SubMod { ident: name, r#mod: Deref @ DAE::Mod::MOD { binding: Some(DAE::EqMod::TYPED { modifierAsExp: exp, modifierAsValue: val, properties: p, .. }), info, .. } } => {
            let mut ty: Arc<DAE::Type>;
            ty = getRealAttributeType((name.clone()).clone(), inBaseType, info.clone())?;
            instBuiltinAttribute(inCache, inEnv, (name.clone()).clone(), val.clone(), exp.clone(), ty, p.clone())?
        },
...
fn instBuiltinAttribute(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut id: ArcStr, mut optVal: Option<Arc<Values::Value>>, mut bind: Arc<DAE::Exp>, mut inExpectedTp: Arc<DAE::Type>, mut bindProp: DAE::Properties) -> Result<Arc<DAE::Var>> {
    ...
    var = 'mc: {
        let __mc_input = (inCache, inEnv, optVal, inExpectedTp, bindProp.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
fn arrayBasictypeBaseclass(mut inInstDims: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>, mut inType: Arc<DAE::Type>) -> Result<Option<Arc<DAE::Type>>> {
    let mut outOptType: Option<Arc<DAE::Type>>;
...
pub fn partialInstClassIn(mut cache: FCore::Cache, mut env: FCore::Graph, mut ih: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut r#mod: Arc<DAE::Mod>, mut prefix: DAE::Prefix, mut state: ClassInf::State, mut cls: Arc<SCode::Element>, mut visibility: SCode::Visibility, mut instDims: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>, mut numIter: i32) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, ClassInf::State, Arc<metamodelica::List<Arc<DAE::Var>>>)> {
    ...
    match '__try9: {
        ...
        (cache, env, ih, state, vars) = unwrap_break_err!(partialInstClassIn_dispatch(cache.clone(), env.clone(), ih.clone(), r#mod.clone(), prefix.clone(), state.clone(), cls.clone(), visibility.clone(), instDims.clone(), partial_inst, numIter + 1), '__try9);
        ...
        InstHashTable::addToInstCache(cache_path.clone(), None, Some(InstHashTable::CachedInstItem::FUNC_partialInstClassIn { inputs: inputs.clone(), outputs: outputs.clone() }));
        ...
        if let Ok((__v, __wb0, __wb1, __wb2, __wb3, __wb4)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ SCode::Element::CLASS { .. } => {
                    ...
                    (outCache, outEnv, outIH, outState, outVars) = partialInstClassdef(inCache.clone(), inEnv.clone(), inIH.clone(), inMod.clone(), inPrefix.clone(), inState.clone(), inClass.clone(), var_field!((*inClass).classDef, SCode::Element::CLASS).clone(), inVisibility.clone(), inInstDims.clone(), numIter)?;
...
pub(crate) fn instClassdef(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut store: UnitAbsyn::InstStore, mut inMod2: Arc<DAE::Mod>, mut inPrefix3: DAE::Prefix, mut inState5: ClassInf::State, mut className: ArcStr, mut inClassDef6: Arc<SCode::ClassDef>, mut inRestriction7: SCode::Restriction, mut inVisibility: SCode::Visibility, mut inPartialPrefix: SCode::Partial, mut inEncapsulatedPrefix: SCode::Encapsulated, mut inInstDims9: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>, mut inImplicit: bool, mut inCallingScope: InstTypes::CallingScope, mut inGraph: ConnectionGraph::ConnectionGraph, mut inSets: DAE::Connect::Sets, mut instSingleCref: Option<Arc<DAE::ComponentRef>>, mut comment: Arc<SCode::Comment>, mut info: SourceInfo) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, UnitAbsyn::InstStore, DAE::DAElist, DAE::Connect::Sets, ClassInf::State, Arc<metamodelica::List<Arc<DAE::Var>>>, Option<Arc<DAE::Type>>, Option<SCode::Attributes>, Option<(Arc<Absyn::Path>, i32, DAE::InlineType)>, ConnectionGraph::ConnectionGraph)> {
    ...
    let mut outEqualityConstraint: Option<(Arc<Absyn::Path>, i32, DAE::InlineType)>;
    let mut outGraph: ConnectionGraph::ConnectionGraph;
    (outCache, outEnv, outIH, outStore, outDae, outSets, outState, outTypesVarLst, outTypesTypeOption, optDerAttr, outEqualityConstraint, outGraph) = instClassdef2(inCache, inEnv, inIH, store, inMod2, inPrefix3, inState5, (className).clone(), inClassDef6, inRestriction7, inVisibility, inPartialPrefix, inEncapsulatedPrefix, inInstDims9, inImplicit, inCallingScope, inGraph, inSets, instSingleCref, comment, info, Mutable::create(false))?;
    Ok((outCache, outEnv, outIH, outStore, outDae, outSets, outState, outTypesVarLst, outTypesTypeOption, optDerAttr, outEqualityConstraint, outGraph))
}
...
fn instClassdefBasicType(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inStore: UnitAbsyn::InstStore, mut inMod2: Arc<DAE::Mod>, mut inPrefix3: DAE::Prefix, mut inState5: ClassInf::State, mut className: ArcStr, mut inClassDef6: Arc<SCode::ClassDef>, mut inRestriction7: SCode::Restriction, mut inVisibility: SCode::Visibility, mut inInstDims9: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>, mut inImplicit: bool, mut inGraph: ConnectionGraph::ConnectionGraph, mut inSets: DAE::Connect::Sets, mut instSingleCref: Option<Arc<DAE::ComponentRef>>, mut info: SourceInfo, mut stopInst: Mutable::Mutable<bool>) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, UnitAbsyn::InstStore, DAE::DAElist, DAE::Connect::Sets, ClassInf::State, Arc<metamodelica::List<Arc<DAE::Var>>>, Option<Arc<DAE::Type>>, Option<SCode::Attributes>, Option<(Arc<Absyn::Path>, i32, DAE::InlineType)>, ConnectionGraph::ConnectionGraph)> {
    ...
    let mut outGraph: ConnectionGraph::ConnectionGraph;
    (outCache, outEnv, outIH, outStore, outDae, outSets, outState, outTypesVarLst, outTypesTypeOption, optDerAttr, outEqualityConstraint, outGraph) = 'mc: {
        let __mc_input = (inCache, inEnv, inIH, inStore, inMod2, inPrefix3, inState5, inClassDef6, inInstDims9, inImplicit, inGraph);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, mods, pre, ci_state, Deref @ SCode::ClassDef::PARTS { elementLst: els, normalEquationLst: Deref @ metamodelica::List::Nil, initialEquationLst: Deref @ metamodelica::List::Nil, normalAlgorithmLst: Deref @ metamodelica::List::Nil, initialAlgorithmLst: Deref @ metamodelica::List::Nil, .. }, inst_dims, r#impl, graph) => {
                    ...
                    let mut cdefelts_1: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>;
                    let mut cdefelts_2: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>;
                    let mut csets: DAE::Connect::Sets;
                    let mut dae1: DAE::DAElist;
                    let mut dae2: DAE::DAElist;
                    let mut dae: DAE::DAElist;
...
fn instClassdef2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inStore: UnitAbsyn::InstStore, mut inMod2: Arc<DAE::Mod>, mut inPrefix3: DAE::Prefix, mut inState5: ClassInf::State, mut className: ArcStr, mut inClassDef6: Arc<SCode::ClassDef>, mut inRestriction7: SCode::Restriction, mut inVisibility: SCode::Visibility, mut inPartialPrefix: SCode::Partial, mut inEncapsulatedPrefix: SCode::Encapsulated, mut inInstDims9: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>, mut inImplicit: bool, mut inCallingScope: InstTypes::CallingScope, mut inGraph: ConnectionGraph::ConnectionGraph, mut inSets: DAE::Connect::Sets, mut instSingleCref: Option<Arc<DAE::ComponentRef>>, mut comment: Arc<SCode::Comment>, mut info: SourceInfo, mut stopInst: Mutable::Mutable<bool>) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, UnitAbsyn::InstStore, DAE::DAElist, DAE::Connect::Sets, ClassInf::State, Arc<metamodelica::List<Arc<DAE::Var>>>, Option<Arc<DAE::Type>>, Option<SCode::Attributes>, Option<(Arc<Absyn::Path>, i32, DAE::InlineType)>, ConnectionGraph::ConnectionGraph)> {
    ...
    (outCache, outEnv, outIH, outStore, outDae, outSets, outState, outTypesVarLst, oty, optDerAttr, outEqualityConstraint, outGraph) = 'mc: {
        let __mc_input = (inCache, inEnv.clone(), inIH, inStore, inMod2, inPrefix3, inState5, inClassDef6.clone(), inRestriction7, inVisibility, inPartialPrefix, inEncapsulatedPrefix, inInstDims9, inImplicit, inCallingScope.clone(), inGraph, inSets.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, mods, pre, ci_state, Deref @ SCode::ClassDef::PARTS { elementLst: els, normalEquationLst: Deref @ metamodelica::List::Nil, initialEquationLst: Deref @ metamodelica::List::Nil, normalAlgorithmLst: Deref @ metamodelica::List::Nil, initialAlgorithmLst: Deref @ metamodelica::List::Nil, .. }, re, vis, _, _, inst_dims, r#impl, _, graph, _) => {
                    let mut cdefelts: Arc<metamodelica::List<Arc<SCode::Element>>>;
                    let mut extendselts: Arc<metamodelica::List<Arc<SCode::Element>>>;
                    let mut extendsclasselts: Arc<metamodelica::List<Arc<SCode::Element>>>;
                    let mut compelts_2_elem: Arc<metamodelica::List<Arc<SCode::Element>>>;
                    let mut env1: FCore::Graph;
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, mods, pre, ci_state, Deref @ SCode::ClassDef::PARTS { elementLst: els, normalEquationLst: eqs, initialEquationLst: initeqs, normalAlgorithmLst: alg, initialAlgorithmLst: initalg, constraintLst: constrs, clsattrs, externalDecl: ed }, re, _, _, _, inst_dims, r#impl, callscope, graph, csets) => {
                    ...
                    let mut cdefelts_1: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>;
                    let mut extcomps: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>;
                    let mut compelts_1: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>;
                    let mut compelts_2: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>;
                    let mut comp_cond: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>;
                    ...
                    let mut vars: Arc<metamodelica::List<Arc<DAE::Var>>>;
                    let mut emods: Arc<DAE::Mod>;
                    let mut checkMods: Arc<DAE::Mod>;
                    let mut eqs2: Arc<metamodelica::List<Arc<SCode::Equation>>>;
                    let mut initeqs2: Arc<metamodelica::List<Arc<SCode::Equation>>>;
                    let mut eqs_1: Arc<metamodelica::List<Arc<SCode::Equation>>>;
                    let mut initeqs_1: Arc<metamodelica::List<Arc<SCode::Equation>>>;
                    let mut alg2: Arc<metamodelica::List<Arc<SCode::AlgorithmSection>>>;
                    let mut initalg2: Arc<metamodelica::List<Arc<SCode::AlgorithmSection>>>;
                    let mut alg_1: Arc<metamodelica::List<Arc<SCode::AlgorithmSection>>>;
                    let mut initalg_1: Arc<metamodelica::List<Arc<SCode::AlgorithmSection>>>;
                    ...
                    let mut smCompCrefs: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
                    let mut smInitialCrefs: Arc<metamodelica::List<Arc<DAE::ComponentRef>>>;
                    ...
                    let mut domainFieldsLst: Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<Absyn::ComponentRef>>>)>>;
                    let mut cache = (*cache).clone();
                    let mut ih = (*ih).clone();
                    let mut store = (*store).clone();
                    let mut mods = (*mods).clone();
        ...
        _ => if (SCodeUtil::restrictionEqual(r.clone(), re.clone())) {Mod::isInvariantMod(r#mod.clone())? && Mod::isInvariantDAEMod(mods.clone())?} else {false},
                        ...
                        (cache, mod_1) = Mod::elabMod(cache.clone(), parentEnv.clone(), ih.clone(), pre.clone(), r#mod.clone(), false, Mod::ModScope::DERIVED { path: cn.clone() }, info.clone())?;
                        mods_1 = Mod::merge(mods.clone(), mod_1.clone(), (className.clone()).clone(), true)?;
                        (cache, env, ih, store, dae, csets, ci_state, vars, bc, oDA, eqConstraint, graph) = instClassdef2(cache.clone(), env.clone(), ih.clone(), store.clone(), mods_1.clone(), pre.clone(), ci_state.clone(), (className.clone()).clone(), Arc::new(SCode::ClassDef::PARTS { elementLst: list![Arc::new(SCode::Element::EXTENDS { baseClassPath: cn.clone(), visibility: vis.clone(), modifications: openmodelica_frontend_types::SCode::Mod::interned_NOMOD(), ann: None, info: info.clone() })], normalEquationLst: metamodelica::nil(), initialEquationLst: metamodelica::nil(), normalAlgorithmLst: metamodelica::nil(), initialAlgorithmLst: metamodelica::nil(), constraintLst: metamodelica::nil(), clsattrs: metamodelica::nil(), externalDecl: None }), re.clone(), vis.clone(), partialPrefix.clone(), encapsulatedPrefix.clone(), inst_dims.clone(), r#impl.clone(), callscope.clone(), graph.clone(), inSets.clone(), instSingleCref.clone(), comment.clone(), info.clone(), stopInst.clone())?;
                        oDA = SCodeUtil::mergeAttributes(DA.clone(), oDA.clone())?;
                    }
...
fn joinExtEquations(mut inEq: Arc<metamodelica::List<Arc<SCode::Equation>>>, mut inExtEq: Arc<metamodelica::List<Arc<SCode::Equation>>>, mut inCallingScope: InstTypes::CallingScope) -> Arc<metamodelica::List<Arc<SCode::Equation>>> {
    let mut outEq: Arc<metamodelica::List<Arc<SCode::Equation>>>;
...
fn joinExtAlgorithms(mut inAlg: Arc<metamodelica::List<Arc<SCode::AlgorithmSection>>>, mut inExtAlg: Arc<metamodelica::List<Arc<SCode::AlgorithmSection>>>, mut inCallingScope: InstTypes::CallingScope) -> Arc<metamodelica::List<Arc<SCode::AlgorithmSection>>> {
    let mut outAlg: Arc<metamodelica::List<Arc<SCode::AlgorithmSection>>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, Deref @ metamodelica::List::Cons { head: Deref @ Absyn::TypeSpec::TPATH { path: cn, arrayDim: _ }, tail: restTypeSpecs }, pre, dims, r#impl, localAccTypes) => {
                    ...
                    (cache, cenv, ih, _, _, csets, ty, _, oDA, _) = instClass(cache.clone(), cenv.clone(), ih.clone(), UnitAbsyn::noStore().clone(), openmodelica_frontend_types::DAE::Mod::interned_NOMOD(), pre.clone(), c.clone(), dims.clone(), r#impl.clone(), openmodelica_frontend_inst::InstTypes::CallingScope::INNER_CALL, ConnectionGraph::EMPTY().clone(), inSets.clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, Deref @ metamodelica::List::Cons { head: tSpec @ Deref @ Absyn::TypeSpec::TCOMPLEX { path: p, typeSpecs: _, arrayDim: _ }, tail: restTypeSpecs }, pre, dims, r#impl, localAccTypes) => {
                    ...
                    c = Arc::new(SCode::Element::CLASS { name: (id.clone()).clone(), prefixes: SCode::defaultPrefixes.clone(), encapsulatedPrefix: openmodelica_frontend_types::SCode::Encapsulated::NOT_ENCAPSULATED, partialPrefix: openmodelica_frontend_types::SCode::Partial::NOT_PARTIAL, restriction: openmodelica_frontend_types::SCode::Restriction::R_TYPE, classDef: Arc::new(SCode::ClassDef::DERIVED { typeSpec: tSpec.clone(), modifications: openmodelica_frontend_types::SCode::Mod::interned_NOMOD(), attributes: SCode::Attributes { arrayDims: metamodelica::nil(), connectorType: openmodelica_frontend_types::SCode::ConnectorType::POTENTIAL, parallelism: openmodelica_frontend_types::SCode::Parallelism::NON_PARALLEL, variability: openmodelica_frontend_types::SCode::Variability::VAR, direction: openmodelica_ast::Absyn::Direction::BIDIR, isField: openmodelica_ast::Absyn::IsField::NONFIELD } }), cmt: SCode::noComment.clone(), info: Absyn::dummyInfo.clone() });
                    (cache, _, ih, _, _, csets, ty, _, oDA, _) = instClass(cache.clone(), env.clone(), ih.clone(), UnitAbsyn::noStore().clone(), openmodelica_frontend_types::DAE::Mod::interned_NOMOD(), pre.clone(), c.clone(), dims.clone(), r#impl.clone(), openmodelica_frontend_inst::InstTypes::CallingScope::INNER_CALL, ConnectionGraph::EMPTY().clone(), inSets.clone())?;
                    localAccTypes = metamodelica::cons(ty.clone(), localAccTypes.clone());
                    (cache, env, ih, localAccTypes, csets, _) = instClassDefHelper(cache.clone(), env.clone(), ih.clone(), restTypeSpecs.clone(), pre.clone(), dims.clone(), r#impl.clone(), localAccTypes.clone(), csets.clone(), inInfo.clone())?;
                    Ok((cache.clone(), env.clone(), ih.clone(), localAccTypes.clone(), csets.clone(), oDA.clone()))
                }
...
fn instBasictypeBaseclass(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inStore: UnitAbsyn::InstStore, mut inSCodeElementLst2: Arc<metamodelica::List<Arc<SCode::Element>>>, mut inSCodeElementLst3: Arc<metamodelica::List<Arc<SCode::Element>>>, mut inMod4: Arc<DAE::Mod>, mut inInstDims5: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>, mut className: ArcStr, mut info: SourceInfo, mut stopInst: Mutable::Mutable<bool>) -> Result<(FCore::Cache, Arc<metamodelica::List<InnerOuter::TopInstance>>, UnitAbsyn::InstStore, DAE::DAElist, Option<Arc<DAE::Type>>, Arc<metamodelica::List<Arc<DAE::Var>>>)> {
    ...
    (outCache, outIH, outStore, outDae, outTypesTypeOption, outTypeVars) = 'mc: {
        let __mc_input = (inCache, inEnv, inIH, inStore, inSCodeElementLst2.clone(), inSCodeElementLst3.clone(), inMod4, inInstDims5);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
fn rollbackCheck(mut p: Arc<Absyn::Path>) -> () {
    let () = 'mc: {
...
fn instBasictypeBaseclass2(mut inCache: FCore::Cache, mut inEnv1: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut store: UnitAbsyn::InstStore, mut inSCodeElementLst2: Arc<metamodelica::List<Arc<SCode::Element>>>, mut inSCodeElementLst3: Arc<metamodelica::List<Arc<SCode::Element>>>, mut inMod4: Arc<DAE::Mod>, mut inInstDims5: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>, mut className: ArcStr, mut inInfo: SourceInfo, mut stopInst: Mutable::Mutable<bool>) -> () {
    let () = 'mc: {
        let __mc_input = (inCache, inEnv1, inIH, inSCodeElementLst2, inSCodeElementLst3, inInstDims5);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
fn partialInstClassdef(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inMod: Arc<DAE::Mod>, mut inPrefix: DAE::Prefix, mut inState: ClassInf::State, mut inClass: Arc<SCode::Element>, mut inClassDef: Arc<SCode::ClassDef>, mut inVisibility: SCode::Visibility, mut inInstDims: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>, mut numIter: i32) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, ClassInf::State, Arc<metamodelica::List<Arc<DAE::Var>>>)> {
    ...
    (outCache, outEnv, outIH, outState, outVars) = ({
        ...
        Deref @ SCode::ClassDef::PARTS { .. } => {
            ...
            let mut class_ext_els: Arc<metamodelica::List<Arc<SCode::Element>>>;
            let mut extends_els: Arc<metamodelica::List<Arc<SCode::Element>>>;
            let mut emods: Arc<DAE::Mod>;
            let mut r#mod: Arc<DAE::Mod>;
            let mut ext_comps: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>;
            let mut const_els: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>;
            partial_prefix = SCodeUtil::getClassPartialPrefix(inClass.clone())?;
            partial_prefix = InstUtil::isPartial(partial_prefix, inMod.clone());
            class_name = (SCodeUtil::elementName(inClass)?).clone();
            ...
            extends_els = SCodeInstUtil::addRedeclareAsElementsToExtends(extends_els, List::select(var_field!((*inClassDef).elementLst, SCode::ClassDef::PARTS).clone(), (std::sync::Arc::new(fnptr!(SCodeUtil::isRedeclareElement, Arc<SCode::Element>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<SCode::Element>) -> Result<bool> + 'static>))?)?;
            (outCache, outEnv, outIH) = InstUtil::addClassdefsToEnv(inCache, inEnv.clone(), inIH, inPrefix.clone(), cdef_els, true, Some(inMod.clone()), FGraph::isEmptyScope(inEnv))?;
            (outCache, outEnv, outIH, emods, ext_comps, _, _, _, _, _) = InstExtends::instExtendsAndClassExtendsList(outCache, outEnv, outIH, inMod.clone(), inPrefix.clone(), extends_els, class_ext_els, var_field!((*inClassDef).elementLst, SCode::ClassDef::PARTS).clone(), inState.clone(), (class_name.clone()).clone(), true, true)?;
            ...
            r#mod = Mod::merge(inMod, emods, (class_name).clone(), true)?;
            ...
            (outCache, outEnv, outIH) = InstUtil::addComponentsToEnv(outCache, outEnv, outIH, r#mod.clone(), inPrefix.clone(), inState, const_els.clone(), false)?;
            (outCache, outEnv, outIH, _, _, _, outState, outVars, _, _) = instElementList(outCache, outEnv, outIH, UnitAbsyn::noStore().clone(), r#mod, inPrefix, outState, const_els, inInstDims, true, openmodelica_frontend_inst::InstTypes::CallingScope::INNER_CALL, ConnectionGraph::EMPTY().clone(), Connect::emptySet().clone(), false)?;
            (outCache, outEnv, outIH, outState, outVars)
        },
        Deref @ SCode::ClassDef::DERIVED { typeSpec: Deref @ Absyn::TypeSpec::TPATH { path: class_path, arrayDim: class_dims }, modifications: class_mod, .. } => {
            ...
            is_basic_type = InstUtil::checkDerivedRestriction(parent_re, der_re.clone(), (class_name.clone()).clone())?;
            ...
            r#mod = Mod::merge(inMod, r#mod, (class_name.clone()).clone(), true)?;
            if has_dims && !(is_basic_type) {
                cls = SCodeUtil::setClassName((class_name.clone()).clone(), cls)?;
                eq = Mod::modEquation(r#mod.clone())?;
                (outCache, dims) = InstUtil::elabArraydimOpt(outCache, parent_env, Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (literal!("")).clone(), subscripts: metamodelica::nil() }), class_path.clone(), class_dims.clone(), eq, false, true, inPrefix.clone(), info.clone(), inInstDims.clone())?;
                inst_dims = List::appendLastList(inInstDims.clone(), dims)?;
            } else {
                inst_dims = inInstDims.clone();
            }
            if is_basic_type || has_dims {
                scope_ty = if (is_basic_type) {FGraph::restrictionToScopeType(der_re.clone())} else {FGraph::classInfToScopeType(inState)};
                cenv = FGraph::openScope(cenv, enc, (class_name).clone(), scope_ty)?;
                outState = ClassInfUtil::start(der_re, FGraph::getGraphName(cenv.clone())?)?;
                (outCache, outEnv, outIH, outState, outVars) = partialInstClassIn(outCache, cenv, inIH, r#mod, inPrefix, outState, cls.clone(), inVisibility, inst_dims, numIter)?;
            } else {
                cdef = Arc::new(SCode::ClassDef::PARTS { elementLst: list![Arc::new(SCode::Element::EXTENDS { baseClassPath: class_path.clone(), visibility: inVisibility.clone(), modifications: openmodelica_frontend_types::SCode::Mod::interned_NOMOD(), ann: None, info: info })], normalEquationLst: metamodelica::nil(), initialEquationLst: metamodelica::nil(), normalAlgorithmLst: metamodelica::nil(), initialAlgorithmLst: metamodelica::nil(), constraintLst: metamodelica::nil(), clsattrs: metamodelica::nil(), externalDecl: None });
                (outCache, outEnv, outIH, outState, outVars) = partialInstClassdef(outCache, inEnv.clone(), inIH, r#mod, inPrefix, inState, inClass, cdef, inVisibility, inInstDims, numIter)?;
            }
...
pub(crate) fn instElementList(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inStore: UnitAbsyn::InstStore, mut inMod: Arc<DAE::Mod>, mut inPrefix: DAE::Prefix, mut inState: ClassInf::State, mut inElements: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>, mut inInstDims: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>, mut inImplInst: bool, mut inCallingScope: InstTypes::CallingScope, mut inGraph: ConnectionGraph::ConnectionGraph, mut inSets: DAE::Connect::Sets, mut inStopOnError: bool) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, UnitAbsyn::InstStore, DAE::DAElist, DAE::Connect::Sets, ClassInf::State, Arc<metamodelica::List<Arc<DAE::Var>>>, ConnectionGraph::ConnectionGraph, Arc<metamodelica::List<(Arc<DAE::ComponentRef>, Arc<metamodelica::List<Arc<Absyn::ComponentRef>>>)>>)> {
    ...
    let mut el: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>;
    let mut cache: FCore::Cache;
    let mut vars: Arc<metamodelica::List<Arc<DAE::Var>>>;
    let mut dae: Arc<metamodelica::List<Arc<DAE::Element>>>;
    ...
    let mut fieldDomOpt: Option<(Arc<Absyn::ComponentRef>, Arc<DAE::ComponentRef>)>;
    let mut element_order: Arc<metamodelica::List<i32>>;
    ...
    if !(ClassInfUtil::isFunction(inState)) {
        ...
        for mut idx in &*element_order {
            ...
            (cache, outEnv, outIH, outStore, dae, outSets, outState, vars, outGraph, fieldDomOpt) = instElement2(cache.clone(), outEnv.clone(), outIH.clone(), outStore.clone(), inMod.clone(), inPrefix.clone(), outState.clone(), ({let __elt = el_arr.borrow()[(idx.clone()-1) as usize].clone(); __elt}), inInstDims.clone(), inImplInst, inCallingScope.clone(), outGraph.clone(), outSets.clone(), inStopOnError)?;
            metamodelica::arrayUpdate(var_arr.clone(), length - idx.clone() + 1, vars.clone())?;
            metamodelica::arrayUpdate(dae_arr.clone(), length - idx.clone() + 1, dae.clone())?;
            if intEq(Flags::getConfigEnum(Flags::GRAMMAR.clone())?, Flags::PDEMODELICA.clone()) {
        ...
        for mut e in &*el {
            let mut e = e.clone();
            (cache, outEnv, outIH, outStore, dae, outSets, outState, vars, outGraph, fieldDomOpt) = instElement2(cache.clone(), outEnv.clone(), outIH.clone(), outStore.clone(), inMod.clone(), inPrefix.clone(), outState.clone(), e.clone(), inInstDims.clone(), inImplInst, inCallingScope.clone(), outGraph.clone(), outSets.clone(), inStopOnError)?;
        ...
        outDae = DAE::DAElist { elementLst: List::flattenReverse(dael)? };
    }
...
fn getSortedElementOrdering(mut inElements: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>, mut inSortedElements: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>) -> Result<Arc<metamodelica::List<i32>>> {
    ...
    let mut sorted_el: Arc<metamodelica::List<Arc<SCode::Element>>>;
    let mut i: i32 = 1;
    for mut e in &*inElements {
        let mut e = e.clone();
        index_map = metamodelica::cons((Util::tuple21(e.clone()), i), index_map.clone());
        i = i + 1;
    }
    ...
    for mut e in &*sorted_el {
        ...
        i = __pa1.clone();
        outIndices = metamodelica::cons(i, outIndices.clone());
    }
...
pub(crate) fn instElement2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inStore: UnitAbsyn::InstStore, mut inMod: Arc<DAE::Mod>, mut inPrefix: DAE::Prefix, mut inState: ClassInf::State, mut inElement: (Arc<SCode::Element>, Arc<DAE::Mod>), mut inInstDims: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>, mut inImplicit: bool, mut inCallingScope: InstTypes::CallingScope, mut inGraph: ConnectionGraph::ConnectionGraph, mut inSets: DAE::Connect::Sets, mut inStopOnError: bool) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, UnitAbsyn::InstStore, Arc<metamodelica::List<Arc<DAE::Element>>>, DAE::Connect::Sets, ClassInf::State, Arc<metamodelica::List<Arc<DAE::Var>>>, ConnectionGraph::ConnectionGraph, Option<(Arc<Absyn::ComponentRef>, Arc<DAE::ComponentRef>)>)> {
    ...
    (is_deleted, outEnv, outCache) = isDeletedComponent(inElement.clone(), inPrefix.clone(), inStopOnError, inEnv.clone(), inCache.clone())?;
    if is_deleted {
    ...
    match '__try0: {
        ...
        outIH = __pa3.clone();
        elt = __pa4.clone();
        let (__pa6, __pa7, __pa8, __pa9, __pa10, __pa11, __pa12, __pa13, __pa14, __pa15) = ::match_deref::match_deref! { match &(unwrap_break_err!(instElement(outCache.clone(), outEnv.clone(), outIH.clone(), outStore.clone(), inMod.clone(), inPrefix.clone(), outState.clone(), elt.clone(), inInstDims.clone(), inImplicit, inCallingScope.clone(), outGraph.clone(), inSets.clone()), '__try0)) {
...
fn isDeletedComponent(mut element: (Arc<SCode::Element>, Arc<DAE::Mod>), mut prefix: DAE::Prefix, mut stopOnError: bool, mut env: FCore::Graph, mut cache: FCore::Cache) -> Result<(bool, FCore::Graph, FCore::Cache)> {
    ...
    if InstUtil::componentHasCondition(element.clone()) {
        ...
        isDeleted = !(cond_val);
        if isDeleted == true {
            var = Arc::new(DAE::Var { name: (el_name).clone(), attributes: DAE::dummyAttrVar().clone(), ty: DAE::T_UNKNOWN_DEFAULT().clone(), binding: openmodelica_frontend_types::DAE::Binding::interned_UNBOUND(), bind_from_outside: false, constOfForIteratorRange: None });
            env = FGraph::updateComp(env, var, openmodelica_frontend_dump::FCore::Status::VAR_DELETED, FGraph::emptyGraph().clone());
        }
    } else {
        isDeleted = false;
    }
...
pub(crate) fn instElement(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inUnitStore: UnitAbsyn::InstStore, mut inMod: Arc<DAE::Mod>, mut inPrefix: DAE::Prefix, mut inState: ClassInf::State, mut inElement: (Arc<SCode::Element>, Arc<DAE::Mod>), mut inInstDims: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>, mut inImplicit: bool, mut inCallingScope: InstTypes::CallingScope, mut inGraph: ConnectionGraph::ConnectionGraph, mut inSets: DAE::Connect::Sets) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, UnitAbsyn::InstStore, DAE::DAElist, DAE::Connect::Sets, ClassInf::State, Arc<metamodelica::List<Arc<DAE::Var>>>, ConnectionGraph::ConnectionGraph, Option<(Arc<Absyn::ComponentRef>, Arc<DAE::ComponentRef>)>)> {
    ...
    (outCache, outEnv, outIH, outUnitStore, outDae, outSets, outState, outVars, outGraph) = 'mc: {
        let __mc_input = (inCache.clone(), inEnv.clone(), inIH.clone(), inUnitStore.clone(), inMod, inPrefix.clone(), inState.clone(), inElement, inInstDims, inImplicit, inGraph.clone(), inSets.clone());
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, mods, pre, ci_state, (el @ Deref @ SCode::Element::COMPONENT { name, typeSpec: Deref @ Absyn::TypeSpec::TPATH { .. }, .. }, cmod), inst_dims, r#impl, graph, csets) => {
                    let mut own_cref: Arc<Absyn::ComponentRef>;
                    ...
                    let mut dae: DAE::DAElist;
                    let mut r#mod: Arc<DAE::Mod>;
                    let mut class_mod: Arc<DAE::Mod>;
                    let mut mm: Arc<DAE::Mod>;
                    let mut mod_1: Arc<DAE::Mod>;
                    let mut var_class_mod: Arc<DAE::Mod>;
                    let mut m_1: Arc<DAE::Mod>;
                    let mut cls_mod: Arc<DAE::Mod>;
                    let mut ty: Arc<DAE::Type>;
                    ...
                    let mut crefs3: Arc<metamodelica::List<Arc<Absyn::ComponentRef>>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, mods, pre, ci_state, (comp @ Deref @ SCode::Element::COMPONENT { name, prefixes: prefixes @ Deref @ SCode::Prefixes { finalPrefix: final_prefix, innerOuter: io, .. }, attributes: attr @ SCode::Attributes { arrayDims: ad, connectorType: ct, .. }, typeSpec: ts @ Deref @ Absyn::TypeSpec::TCOMPLEX { path: type_name, .. }, modifications: m, comment, condition: cond, info }, cmod), inst_dims, r#impl, graph, csets) => {
                    ...
                    cls = Arc::new(SCode::Element::CLASS { name: (id.clone()).clone(), prefixes: SCode::defaultPrefixes.clone(), encapsulatedPrefix: openmodelica_frontend_types::SCode::Encapsulated::NOT_ENCAPSULATED, partialPrefix: openmodelica_frontend_types::SCode::Partial::NOT_PARTIAL, restriction: openmodelica_frontend_types::SCode::Restriction::R_TYPE, classDef: Arc::new(SCode::ClassDef::DERIVED { typeSpec: ts.clone(), modifications: openmodelica_frontend_types::SCode::Mod::interned_NOMOD(), attributes: SCode::Attributes { arrayDims: ad.clone(), connectorType: ct.clone(), parallelism: openmodelica_frontend_types::SCode::Parallelism::NON_PARALLEL, variability: openmodelica_frontend_types::SCode::Variability::VAR, direction: openmodelica_ast::Absyn::Direction::BIDIR, isField: openmodelica_ast::Absyn::IsField::NONFIELD } }), cmt: SCode::noComment.clone(), info: info.clone() });
...
fn removePrefixFromBinding(mut inBind: Arc<DAE::Binding>, mut inPrefix: DAE::Prefix) -> Result<Arc<DAE::Binding>> {
    let mut outBind: Arc<DAE::Binding>;
...
fn updateCompeltsMods(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inPrefix: DAE::Prefix, mut inComponents: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>, mut inState: ClassInf::State, mut inImplicit: bool) -> (FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>) {
    ...
    (outCache, outEnv, outIH, outComponents) = 'mc: {
        ...
        if let Ok((__v, __wb0, __wb1, __wb2, __wb3)) = (|| -> Result<_> {
            ...
            (outCache, outEnv, outIH, outComponents) = updateCompeltsMods_dispatch(inCache.clone(), inEnv.clone(), inIH.clone(), inPrefix.clone(), inComponents.clone(), inState.clone(), inImplicit)?;
...
fn updateCompeltsMods_dispatch(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inPrefix: DAE::Prefix, mut inComponents: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>, mut inState: ClassInf::State, mut inImplicit: bool) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>)> {
    ...
    let mut outEnv: FCore::Graph;
    let mut outIH: Arc<metamodelica::List<InnerOuter::TopInstance>>;
    let mut outComponents: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>;
    (outCache, outEnv, outIH, outComponents) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, pre, Deref @ metamodelica::List::Cons { head: elMod @ (_, Deref @ DAE::Mod::NOMOD { .. }), tail: xs }, ci_state, r#impl) => {
                    let mut res: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>;
                    let mut cache = (*cache).clone();
                    let mut env = (*env).clone();
                    let mut ih = (*ih).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, pre, Deref @ metamodelica::List::Cons { head: (comp, cmod @ Deref @ DAE::Mod::REDECL { element: redComp, .. }), tail: xs }, ci_state, r#impl) => {
                    let mut env2: FCore::Graph;
                    let mut env3: FCore::Graph;
                    let mut umod: Arc<SCode::Mod>;
                    let mut crefs: Arc<metamodelica::List<Arc<Absyn::ComponentRef>>>;
                    let mut crefs_1: Arc<metamodelica::List<Arc<Absyn::ComponentRef>>>;
                    let mut cref: Arc<Absyn::ComponentRef>;
                    let mut cmod_1: Arc<DAE::Mod>;
                    let mut cmod2: Arc<DAE::Mod>;
                    let mut ltmod: Arc<metamodelica::List<Arc<DAE::Mod>>>;
                    let mut res: Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>;
                    let mut name: ArcStr;
                    let mut info: SourceInfo;
                    ...
                    cmod2 = List::fold2r(metamodelica::cons(cmod.clone(), ltmod.clone()), (std::sync::Arc::new(Mod::merge) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Mod>, Arc<DAE::Mod>, ArcStr, bool) -> Result<Arc<DAE::Mod>> + 'static>), (name.clone()).clone(), true, openmodelica_frontend_types::DAE::Mod::interned_NOMOD())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, pre, Deref @ metamodelica::List::Cons { head: (comp, cmod @ Deref @ DAE::Mod::MOD { .. }), tail: xs }, ci_state, r#impl) => {
                    ...
                    cref = Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (name.clone()).clone(), subscripts: metamodelica::nil() });
                    ltmod = List::map1(crefs.clone(), (std::sync::Arc::new(InstUtil::getModsForDep) as std::sync::Arc<dyn ::std::ops::Fn(Arc<Absyn::ComponentRef>, Arc<metamodelica::List<(Arc<SCode::Element>, Arc<DAE::Mod>)>>) -> Result<Arc<DAE::Mod>> + 'static>), xs.clone())?;
                    cmod2 = List::fold2r(ltmod.clone(), (std::sync::Arc::new(Mod::merge) as std::sync::Arc<dyn ::std::ops::Fn(Arc<DAE::Mod>, Arc<DAE::Mod>, ArcStr, bool) -> Result<Arc<DAE::Mod>> + 'static>), (name.clone()).clone(), true, openmodelica_frontend_types::DAE::Mod::interned_NOMOD())?;
                    let __pa0 = ::match_deref::match_deref! { match &(SCodeUtil::elementPrefixes(comp.clone())?) {
                        Deref @ SCode::Prefixes { finalPrefix: __pa0, .. } => __pa0.clone(),
                        _ => bail!("pattern mismatch"),
                    } };
...
pub(crate) fn redeclareType(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inMod: Arc<DAE::Mod>, mut inElement: Arc<SCode::Element>, mut inPrefix: DAE::Prefix, mut inState: ClassInf::State, mut inImpl: bool, mut inCmod: Arc<DAE::Mod>) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, Arc<SCode::Element>, Arc<DAE::Mod>)> {
    ...
    let mut outMod: Arc<DAE::Mod> = openmodelica_frontend_types::DAE::Mod::interned_NOMOD();
    let mut redecl_el: Arc<SCode::Element>;
    let mut r#mod: Arc<SCode::Mod> = Arc::new(SCode::Mod::NOMOD);
    let mut redecl_mod: Arc<DAE::Mod>;
    ...
    if !(Mod::isRedeclareMod(inMod.clone())) {
        outMod = Mod::merge(inMod, inCmod, (literal!("")).clone(), true)?;
    ...
    (outElement, outMod) = 'mc: {
        ...
        if let Ok((__v, __wb0, __wb1, __wb2, __wb3, __wb4, __wb5, __wb6, __wb7, __wb8, __wb9)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ SCode::Element::COMPONENT { .. }, Deref @ SCode::Element::COMPONENT { prefixes: Deref @ SCode::Prefixes { replaceablePrefix: repl, .. }, .. }) => {
                    ...
                    (outCache, outEnv, outIH) = updateComponentsInEnv(inCache.clone(), inEnv.clone(), inIH.clone(), inPrefix.clone(), openmodelica_frontend_types::DAE::Mod::interned_NOMOD(), crefs.clone(), inState.clone(), inImpl);
                    (outCache, m) = Mod::elabMod(outCache.clone(), outEnv.clone(), outIH.clone(), inPrefix.clone(), r#mod.clone(), inImpl, Mod::ModScope::COMPONENT { name: (redecl_name.clone()).clone() }, var_field!((*redecl_el).info, SCode::Element::COMPONENT).clone())?;
                    (outCache, old_m) = Mod::elabMod(outCache.clone(), outEnv.clone(), outIH.clone(), inPrefix.clone(), var_field!((*inElement).modifications, SCode::Element::COMPONENT).clone(), inImpl, Mod::ModScope::COMPONENT { name: (var_field!((*inElement).name, SCode::Element::COMPONENT).clone()).clone() }, var_field!((*inElement).info, SCode::Element::COMPONENT).clone())?;
        ...
        })() { cc_comps = __wb0; crefs = __wb1; m = __wb2; r#mod = __wb3; old_m = __wb4; outCache = __wb5; outElement = __wb6; outEnv = __wb7; outIH = __wb8; redecl_mod = __wb9; break 'mc __v; }
        if let Ok((__v, __wb0, __wb1, __wb2)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ SCode::Element::CLASS { .. }, Deref @ SCode::Element::CLASS { .. }) => {
                    ...
                    (outCache, outEnv, outIH) = updateComponentsInEnv(inCache.clone(), inEnv.clone(), inIH.clone(), inPrefix.clone(), inMod.clone(), list![Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (var_field!((*inElement).name, SCode::Element::CLASS).clone()).clone(), subscripts: metamodelica::nil() })], inState.clone(), inImpl);
        ...
        if let Ok((__v, __wb0, __wb1, __wb2, __wb3)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ SCode::Element::CLASS { .. }, Deref @ SCode::Element::COMPONENT { .. }) => {
                    ...
                    (outCache, outEnv, outIH) = updateComponentsInEnv(inCache.clone(), inEnv.clone(), inIH.clone(), inPrefix.clone(), inMod.clone(), list![Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (name.clone()).clone(), subscripts: metamodelica::nil() })], inState.clone(), inImpl);
                    Ok(((inElement.clone(), redecl_mod.clone()), name.clone(), outCache.clone(), outEnv.clone(), outIH.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { name = __wb0; outCache = __wb1; outEnv = __wb2; outIH = __wb3; break 'mc __v; }
        if let Ok((__v, __wb0, __wb1, __wb2, __wb3)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ SCode::Element::CLASS { .. }, Deref @ SCode::Element::COMPONENT { .. }) => {
                    let mut name: ArcStr = name.clone();
...
fn propagateRedeclCompAttr(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inOldComponent: Arc<SCode::Element>, mut inNewComponent: Arc<SCode::Element>) -> Result<(FCore::Cache, Arc<SCode::Element>)> {
    ...
    if SCodeUtil::isArrayComponent(inOldComponent.clone()) && !(SCodeUtil::isArrayComponent(inNewComponent.clone())) {
        (outCache, is_array) = Lookup::isArrayType(outCache, inEnv, SCodeUtil::getElementTypePath(inNewComponent.clone())?);
    }
...
fn updateComponentsInEnv(mut cache: FCore::Cache, mut env: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut pre: DAE::Prefix, mut r#mod: Arc<DAE::Mod>, mut crefs: Arc<metamodelica::List<Arc<Absyn::ComponentRef>>>, mut ci_state: ClassInf::State, mut r#impl: bool) -> (FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>) {
    ...
    if '__try0: {
        (outCache, outEnv, outIH, _) = updateComponentsInEnv2(cache.clone(), env.clone(), inIH.clone(), pre.clone(), r#mod.clone(), crefs.clone(), ci_state.clone(), r#impl, None, None);
        Ok::<(), anyhow::Error>(())
    }.is_err() {
    }
...
fn updateComponentInEnv(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut pre: DAE::Prefix, mut r#mod: Arc<DAE::Mod>, mut cref: Arc<Absyn::ComponentRef>, mut inCIState: ClassInf::State, mut r#impl: bool, mut inUpdatedComps: Option<(metamodelica::Array<Arc<metamodelica::List<(Arc<Absyn::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<Absyn::ComponentRef>, i32)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<Absyn::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<Absyn::ComponentRef>, Arc<Absyn::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<Absyn::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(i32) -> Result<ArcStr> + 'static>))>, mut currentCref: Option<Arc<Absyn::ComponentRef>>) -> (FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, Option<(metamodelica::Array<Arc<metamodelica::List<(Arc<Absyn::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<Absyn::ComponentRef>, i32)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<Absyn::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<Absyn::ComponentRef>, Arc<Absyn::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<Absyn::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(i32) -> Result<ArcStr> + 'static>))>) {
    let mut outCache: FCore::Cache;
    let mut outEnv: FCore::Graph;
    let mut outIH: Arc<metamodelica::List<InnerOuter::TopInstance>>;
    let mut outUpdatedComps: Option<(metamodelica::Array<Arc<metamodelica::List<(Arc<Absyn::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<Absyn::ComponentRef>, i32)>>), i32, (HashTable5::FuncHashCref, HashTable5::FuncCrefEqual, HashTable5::FuncCrefStr, HashTable5::FuncExpStr))>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, Deref @ DAE::Mod::REDECL { element: Deref @ SCode::Element::COMPONENT { name, prefixes: prefixes @ Deref @ SCode::Prefixes { visibility, .. }, attributes, modifications: smod, info, .. }, .. }) => {
                    ...
                    let mut crefs3: Arc<metamodelica::List<Arc<Absyn::ComponentRef>>>;
                    let mut crefs_1: Arc<metamodelica::List<Arc<Absyn::ComponentRef>>>;
                    let mut crefs_2: Arc<metamodelica::List<Arc<Absyn::ComponentRef>>>;
                    let mut cond: Option<Arc<Absyn::Exp>>;
                    let mut pf: Arc<SCode::Prefixes>;
                    let mut daeMod: Arc<DAE::Mod>;
                    ...
                    let false = (smod.clone() == openmodelica_frontend_types::SCode::Mod::interned_NOMOD()) else { bail!("pattern mismatch") };
                    ...
                    cache = __pa0.clone();
                    (cache, daeMod) = Mod::elabMod(cache.clone(), env.clone(), ih.clone(), pre.clone(), smod.clone(), r#impl, Mod::ModScope::COMPONENT { name: (name.clone()).clone() }, info.clone())?;
                    mods = daeMod.clone();
                    ...
                    let (__pa13, __pa14, __pa15, __pa16) = ::match_deref::match_deref! { match &(updateComponentsInEnv2(cache.clone(), env.clone(), ih.clone(), pre.clone(), openmodelica_frontend_types::DAE::Mod::interned_NOMOD(), crefs_2.clone(), ci_state.clone(), r#impl, Some(updatedComps.clone()), Some(cref.clone()))) {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, mods) => {
                    ...
                    updatedComps = BaseHashTable::add((cref.clone(), 0), updatedComps.clone())?;
                    let (__pa19, __pa20, __pa21, __pa22) = ::match_deref::match_deref! { match &(updateComponentsInEnv2(cache.clone(), env.clone(), ih.clone(), pre.clone(), mods.clone(), crefs_2.clone(), ci_state.clone(), r#impl, Some(updatedComps.clone()), Some(cref.clone()))) {
...
fn updateComponentInEnv2_dispatch(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inClsEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inPrefix: DAE::Prefix, mut inPath: Arc<Absyn::Path>, mut inName: ArcStr, mut inSubscripts: Arc<metamodelica::List<Arc<Absyn::Subscript>>>, mut inClass: Arc<SCode::Element>, mut inAttr: SCode::Attributes, mut inPrefixes: Arc<SCode::Prefixes>, mut inDAttr: Arc<DAE::Attributes>, mut inInfo: SourceInfo, mut inSMod: Arc<SCode::Mod>, mut inClsMod: Arc<DAE::Mod>, mut inMod: Arc<DAE::Mod>, mut inCref: Arc<Absyn::ComponentRef>, mut inState: ClassInf::State, mut inImpl: bool, mut inUpdatedComps: (metamodelica::Array<Arc<metamodelica::List<(Arc<Absyn::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<Absyn::ComponentRef>, i32)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<Absyn::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<Absyn::ComponentRef>, Arc<Absyn::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<Absyn::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(i32) -> Result<ArcStr> + 'static>))) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, (metamodelica::Array<Arc<metamodelica::List<(Arc<Absyn::ComponentRef>, i32)>>>, (i32, i32, metamodelica::Array<Option<(Arc<Absyn::ComponentRef>, i32)>>), i32, (Arc<dyn ::std::ops::Fn(Arc<Absyn::ComponentRef>) -> Result<i32> + 'static>, Arc<dyn ::std::ops::Fn(Arc<Absyn::ComponentRef>, Arc<Absyn::ComponentRef>) -> Result<bool> + 'static>, Arc<dyn ::std::ops::Fn(Arc<Absyn::ComponentRef>) -> Result<ArcStr> + 'static>, Arc<dyn ::std::ops::Fn(i32) -> Result<ArcStr> + 'static>)))> {
    ...
    if '__try0: {
        ...
        (outCache, mod1) = updateComponentInEnv3(outCache.clone(), outEnv.clone(), outIH.clone(), smod.clone(), inImpl, Mod::ModScope::COMPONENT { name: (inName.clone()).clone() }, inInfo.clone())?;
        ...
        (outCache, mod2) = Mod::updateMod(outCache.clone(), outEnv.clone(), outIH.clone(), openmodelica_frontend_types::DAE::Prefix::NOPRE, mod2.clone(), inImpl, inInfo.clone())?;
        ...
        (outCache, dims) = InstUtil::elabArraydim(outCache.clone(), outEnv.clone(), own_cref.clone(), inPath.clone(), inSubscripts.clone(), eq.clone(), inImpl, true, false, inPrefix.clone(), inInfo.clone(), metamodelica::nil())?;
...
fn updateComponentInEnv3(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inMod: Arc<SCode::Mod>, mut inImpl: bool, mut inModScope: Mod::ModScope, mut inInfo: SourceInfo) -> Result<(FCore::Cache, Arc<DAE::Mod>)> {
    ...
    (outCache, outMod) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ...
            (cache, r#mod) = Mod::elabMod(inCache.clone(), inEnv.clone(), inIH.clone(), openmodelica_frontend_types::DAE::Prefix::NOPRE, inMod.clone(), inImpl, inModScope.clone(), inInfo.clone())?;
...
pub fn makeFullyQualified(mut cache: FCore::Cache, mut inEnv: FCore::Graph, mut path: Arc<Absyn::Path>) -> Result<(FCore::Cache, Arc<Absyn::Path>)> {
    ...
    (cache, path) = (::match_deref::match_deref! { match &(path.clone()) {
        Deref @ Absyn::Path::IDENT { .. } => {
            (cache, path) = makeFullyQualifiedIdent(cache, inEnv, (var_field!((*path).name, Absyn::Path::IDENT).clone()).clone(), path)?;
            (cache, path)
        },
        Deref @ Absyn::Path::FULLYQUALIFIED { .. } => (cache, path),
...
fn makeFullyQualifiedFromQual(mut cache: FCore::Cache, mut inEnv: FCore::Graph, mut path: Arc<Absyn::Path>) -> (FCore::Cache, Arc<Absyn::Path>) {
    let mut cache: FCore::Cache = cache;
    let mut path: Arc<Absyn::Path> = path;
    (cache, path) = 'mc: {
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                _ => {
                    ...
                    let mut crPath: Arc<DAE::ComponentRef>;
                    let mut name: ArcStr;
                    let mut cache: FCore::Cache = cache.clone();
...
fn makeFullyQualifiedIdentCheckBuiltin(mut ident: ArcStr) -> Result<(Arc<Absyn::Path>, bool)> {
    ...
    path = (::match_deref::match_deref! { match &(ident) {
...
pub(crate) fn instList<Type_a: Clone + 'static + metamodelica::gc::MMTrace>(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inPrefix: DAE::Prefix, mut inSets: DAE::Connect::Sets, mut inState: ClassInf::State, mut instFunc: Arc<dyn ::std::ops::Fn(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, DAE::Prefix, DAE::Connect::Sets, ClassInf::State, Type_a, bool, bool, ConnectionGraph::ConnectionGraph) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, DAE::DAElist, DAE::Connect::Sets, ClassInf::State, ConnectionGraph::ConnectionGraph)> + 'static>, mut inTypeALst: Arc<metamodelica::List<Type_a>>, mut inImplicit: bool, mut unrollForLoops: bool, mut inGraph: ConnectionGraph::ConnectionGraph) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, DAE::DAElist, DAE::Connect::Sets, ClassInf::State, ConnectionGraph::ConnectionGraph)> {
    ...
    let mut outState: ClassInf::State;
    let mut outGraph: ConnectionGraph::ConnectionGraph;
    (outCache, outEnv, outIH, outDae, outSets, outState, outGraph) = (::match_deref::match_deref! { match &((inCache, inEnv, inIH, inPrefix, inSets, inState, inTypeALst, inImplicit, inGraph)) {
        ...
        (cache, env, ih, pre, csets, ci_state, Deref @ metamodelica::List::Cons { head: e, tail: es }, r#impl, graph) => {
            ...
            let mut ih = (*ih).clone();
            let mut graph = (*graph).clone();
            (cache, env_1, ih, dae1, csets_1, ci_state_1, graph) = instFunc(cache.clone(), env.clone(), ih.clone(), pre.clone(), csets.clone(), ci_state.clone(), e.clone(), r#impl.clone(), unrollForLoops, graph.clone())?;
            (cache, env_2, ih, dae2, csets_2, ci_state_2, graph) = instList(cache.clone(), env_1, ih.clone(), pre.clone(), csets_1, ci_state_1, instFunc.clone(), es.clone(), r#impl.clone(), unrollForLoops, graph.clone())?;
            dae = DAEUtil::joinDaes(dae1, dae2)?;
            (cache.clone(), env_2, ih.clone(), dae, csets_2, ci_state_2, graph.clone())
        },
        _ => bail!("match: no arm matched"),
    } });
...
fn instConstraints(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPrefix: DAE::Prefix, mut inState: ClassInf::State, mut inConstraints: Arc<metamodelica::List<SCode::ConstraintSection>>, mut inImpl: bool) -> Result<(FCore::Cache, FCore::Graph, DAE::DAElist, ClassInf::State)> {
    ...
    (outCache, outEnv, outDae, outState) = (::match_deref::match_deref! { match &(inConstraints) {
        Deref @ metamodelica::List::Nil => {
            (inCache, inEnv, DAE::emptyDae().clone(), inState)
        },
        Deref @ metamodelica::List::Cons { head: constr, tail: rest } => {
            ...
            let mut cache: FCore::Cache;
            let mut dae: DAE::DAElist;
            (cache, env1, constraints_1, ci_state) = InstSection::instConstraint(inCache, inEnv, inPrefix.clone(), inState, constr.clone(), inImpl)?;
            (cache, env2, constraints_2, ci_state) = instConstraints(cache, env1, inPrefix, ci_state, rest.clone(), inImpl)?;
...
fn instClassAttributes2(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inPrefix: DAE::Prefix, mut inAttrs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>, mut inImplicit: bool, mut inInfo: SourceInfo, mut inClsAttrs: DAE::DAElist) -> Result<(FCore::Cache, FCore::Graph, DAE::DAElist)> {
    '__tco: loop {
        ...
        (cache, env, pre, Deref @ metamodelica::List::Cons { head: na, tail: rest }, r#impl, clsAttrs) => {
            ...
            clsAttrs = insertClassAttribute(clsAttrs.clone(), (attrName).clone(), outExp)?;
...
fn insertClassAttribute(mut inAttrs: DAE::DAElist, mut attrName: ArcStr, mut inAttrExp: Arc<DAE::Exp>) -> Result<DAE::DAElist> {
    ...
    outAttrs = (::match_deref::match_deref! { match &((inAttrs, attrName)) {
        (attrs, Deref @ "objective") => {
            let mut startTimeE: Option<Arc<DAE::Exp>>;
            let mut finalTimeE: Option<Arc<DAE::Exp>>;
            let mut objectiveIntegrandE: Option<Arc<DAE::Exp>>;
            ...
            attrs = DAE::DAElist { elementLst: list![Arc::new(DAE::Element::CLASS_ATTRIBUTES { classAttrs: Arc::new(DAE::ClassAttributes { objetiveE: Some(inAttrExp), objectiveIntegrandE: objectiveIntegrandE, startTimeE: startTimeE, finalTimeE: finalTimeE }) })] };
            attrs.clone()
        },
        (attrs, Deref @ "objectiveIntegrand") => {
            let mut objectiveE: Option<Arc<DAE::Exp>>;
            let mut startTimeE: Option<Arc<DAE::Exp>>;
            let mut finalTimeE: Option<Arc<DAE::Exp>>;
            let mut attrs = (*attrs).clone();
            let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(attrs.clone()) {
            ...
            startTimeE = __pa1.clone();
            finalTimeE = __pa2.clone();
            attrs = DAE::DAElist { elementLst: list![Arc::new(DAE::Element::CLASS_ATTRIBUTES { classAttrs: Arc::new(DAE::ClassAttributes { objetiveE: objectiveE, objectiveIntegrandE: Some(inAttrExp), startTimeE: startTimeE, finalTimeE: finalTimeE }) })] };
            attrs.clone()
        },
        (attrs, Deref @ "startTime") => {
            ...
            finalTimeE = __pa2.clone();
            attrs = DAE::DAElist { elementLst: list![Arc::new(DAE::Element::CLASS_ATTRIBUTES { classAttrs: Arc::new(DAE::ClassAttributes { objetiveE: objectiveE, objectiveIntegrandE: objectiveIntegrandE, startTimeE: Some(inAttrExp), finalTimeE: finalTimeE }) })] };
            attrs.clone()
        },
        (attrs, Deref @ "finalTime") => {
            ...
            attrs = DAE::DAElist { elementLst: list![Arc::new(DAE::Element::CLASS_ATTRIBUTES { classAttrs: Arc::new(DAE::ClassAttributes { objetiveE: objectiveE, objectiveIntegrandE: objectiveIntegrandE, startTimeE: startTimeE, finalTimeE: Some(inAttrExp) }) })] };
            attrs.clone()
        },
        _ => {
            let true = (Flags::isSet(Flags::FAILTRACE.clone())?) else { bail!("pattern mismatch") };
...
fn modifyInstantiateClass(mut inMod: Arc<DAE::Mod>, mut path: Arc<Absyn::Path>) -> Result<(Arc<DAE::Mod>, Arc<DAE::Mod>)> {
    let mut omod1: Arc<DAE::Mod>;
    let mut omod2: Arc<DAE::Mod>;
    (omod1, omod2) = (::match_deref::match_deref! { match &(inMod.clone()) {
        Deref @ DAE::Mod::REDECL { element: Deref @ SCode::Element::CLASS { name: id, .. }, .. } => {
            if (id.clone() == AbsynUtil::pathString(path, (literal!(".")).clone(), true, false)?) {(inMod, openmodelica_frontend_types::DAE::Mod::interned_NOMOD())} else {(openmodelica_frontend_types::DAE::Mod::interned_NOMOD(), inMod)}
        },
        _ => {
...
fn removeSelfReferenceAndUpdate(mut inCache: FCore::Cache, mut inEnv: FCore::Graph, mut inIH: Arc<metamodelica::List<InnerOuter::TopInstance>>, mut inStore: UnitAbsyn::InstStore, mut inRefs: Arc<metamodelica::List<Arc<Absyn::ComponentRef>>>, mut inRef: Arc<Absyn::ComponentRef>, mut inPath: Arc<Absyn::Path>, mut inState: ClassInf::State, mut iattr: SCode::Attributes, mut inPrefixes: Arc<SCode::Prefixes>, mut r#impl: bool, mut inInstDims: Arc<metamodelica::List<Arc<metamodelica::List<Arc<DAE::Dimension>>>>>, mut pre: DAE::Prefix, mut mods: Arc<DAE::Mod>, mut scodeMod: Arc<SCode::Mod>, mut info: SourceInfo) -> Result<(FCore::Cache, FCore::Graph, Arc<metamodelica::List<InnerOuter::TopInstance>>, UnitAbsyn::InstStore, Arc<metamodelica::List<Arc<Absyn::ComponentRef>>>)> {
    ...
    let mut outIH: Arc<metamodelica::List<InnerOuter::TopInstance>>;
    let mut outStore: UnitAbsyn::InstStore;
    let mut o1: Arc<metamodelica::List<Arc<Absyn::ComponentRef>>>;
    (outCache, outEnv, outIH, outStore, o1) = 'mc: {
        let __mc_input = (inCache, inEnv, inIH, inStore, inRefs, inRef, inPath, inState, iattr, inInstDims);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, cl1, c1, _, _, _, _) => {
                    let mut cl2: Arc<metamodelica::List<Arc<Absyn::ComponentRef>>>;
                    let mut i1: i32;
                    let mut i2: i32;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cache, env, ih, store, cl1, c1 @ Deref @ Absyn::ComponentRef::CREF_IDENT { name: n, .. }, sty, state, attr @ SCode::Attributes { arrayDims: ad, connectorType: ct, parallelism: prl1, variability: var1, direction: dir, .. }, inst_dims) => {
                    ...
                    (cache, m) = Mod::elabMod(cache.clone(), env.clone(), ih.clone(), pre.clone(), smod.clone(), r#impl, Mod::ModScope::COMPONENT { name: (n.clone()).clone() }, info.clone())?;
                    ...
                    new_var = Arc::new(DAE::Var { name: (n.clone()).clone(), attributes: Arc::new(DAE::Attributes { connectorType: DAEUtil::toConnectorTypeNoState(ct.clone(), None), parallelism: prl1.clone(), variability: var1.clone(), direction: dir.clone(), innerOuter: io.clone(), visibility: vis.clone() }), ty: ty.clone(), binding: openmodelica_frontend_types::DAE::Binding::interned_UNBOUND(), bind_from_outside: false, constOfForIteratorRange: None });
...
fn makeFullyQualified2(mut env: FCore::Graph, mut name: ArcStr, mut cachedPath: Arc<Absyn::Path>) -> Result<Arc<Absyn::Path>> {
    ...
    let mut oscope: Option<Arc<Absyn::Path>>;
    oscope = FGraph::getScopePath(env)?;
    if isNone(oscope.clone()) {
        path = makeFullyQualified2Builtin((name).clone(), cachedPath);
    } else {
        ...
        scope = __pa0.clone();
        path = AbsynUtil::joinPaths(scope, (::match_deref::match_deref! { match &(cachedPath.clone()) {
        Deref @ Absyn::Path::IDENT { name: Deref @ "" } => Arc::new(Absyn::Path::IDENT { name: (name).clone() }),
        _ => cachedPath,
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } }))?;
    }
...
fn makeFullyQualified2Builtin(mut ident: ArcStr, mut cachedPath: Arc<Absyn::Path>) -> Arc<Absyn::Path> {
    ...
    path = (::match_deref::match_deref! { match &(ident.clone()) {
        ...
        Deref @ Absyn::Path::IDENT { name: Deref @ "" } => Arc::new(Absyn::Path::IDENT { name: (ident).clone() }),
        _ => cachedPath,
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } }),
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
...
pub(crate) fn getCachedInstance(mut cache: FCore::Cache, mut env: FCore::Graph, mut name: ArcStr, mut r#ref: metamodelica::Array<FCore::Node>) -> Result<(FCore::Cache, FCore::Graph)> {
    ...
    cls = __pa2.clone();
    prefix = __pa3.clone();
    env2 = FGraph::openScope(env.clone(), enc, (name).clone(), FGraph::restrictionToScopeType(res))?;
    match '__try5: {
...
fn generateCachePath(mut env: FCore::Graph, mut cls: Arc<SCode::Element>, mut prefix: DAE::Prefix, mut callScope: InstTypes::CallingScope) -> Result<Arc<Absyn::Path>> {
    ...
    let mut name: ArcStr;
    name = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*InstTypes::callingScopeStr(callScope)?); __mm_s.push_str(&*literal!("$")); __mm_s.push_str(&*SCodeDump::restrString(SCodeUtil::getClassRestriction(cls)?)?); __mm_s.push_str(&*literal!("$")); __mm_s.push_str(&*generatePrefixStr(prefix)); __mm_s.push_str(&*literal!("$")); ArcStr::from(__mm_s) }).clone();
    cachePath = AbsynUtil::joinPaths(Arc::new(Absyn::Path::IDENT { name: (name).clone() }), FGraph::getGraphName(env)?)?;
    Ok(cachePath)
}
...
fn showCacheInfo(mut inMsg: ArcStr, mut inPath: Arc<Absyn::Path>) -> Result<()> {
    if Flags::isSet(Flags::SHOW_INST_CACHE_INFO.clone())? {
        metamodelica::print(({ let mut __mm_s = String::new(); __mm_s.push_str(&*inMsg); __mm_s.push_str(&*AbsynUtil::pathString(inPath, (literal!(".")).clone(), true, false)?); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone());
    }
    Ok(())
}

fn instFunctionAnnotations(mut comments: Arc<metamodelica::List<Arc<SCode::Comment>>>, mut state: ClassInf::State) -> DAE::DAElist {
    ...
    dae = (::match_deref::match_deref! { match &(r#mod.clone()) {
        Deref @ SCode::Mod::NOMOD { .. } => if (isNone(comment.clone())) {dae} else {DAE::DAElist { elementLst: list![Arc::new(DAE::Element::COMMENT { cmt: Arc::new(SCode::Comment { annotation_: None, comment: comment }) })] }},
        _ => DAE::DAElist { elementLst: list![Arc::new(DAE::Element::COMMENT { cmt: Arc::new(SCode::Comment { annotation_: Some(Arc::new(SCode::Annotation { modification: r#mod })), comment: comment }) })] },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    dae
}
...
pub(crate) fn instClassType(mut cache: FCore::Cache, mut env: FCore::Graph, mut classElem: Arc<SCode::Element>) -> Result<(FCore::Cache, FCore::Graph, Arc<DAE::Type>)> {
    ...
    let mut env: FCore::Graph = env;
    let mut ty: Arc<DAE::Type>;
    (cache, env, _, _, _, _, ty, _, _, _) = instClass(cache, env, InnerOuter::emptyInstHierarchy().clone(), UnitAbsyn::noStore().clone(), openmodelica_frontend_types::DAE::Mod::interned_NOMOD(), openmodelica_frontend_types::DAE::Prefix::NOPRE, classElem, metamodelica::nil(), true, openmodelica_frontend_inst::InstTypes::CallingScope::TOP_CALL, ConnectionGraph::EMPTY().clone(), Connect::emptySet().clone())?;
    Ok((cache, env, ty))
}

fn checkInstanceRestriction(mut cdef: Arc<SCode::Element>, mut path: Arc<Absyn::Path>, mut relaxedFrontEnd: bool) -> Result<()> {
    if !(relaxedFrontEnd) && (SCodeUtil::isFunction(cdef.clone()) || SCodeUtil::isPackage(cdef.clone())) {
        Error::addSourceMessage(Error::INST_INVALID_RESTRICTION.clone(), list![(AbsynUtil::pathString(path, (literal!(".")).clone(), true, false)?).clone(), (SCodeDump::restrString(SCodeUtil::getClassRestriction(cdef.clone())?)?).clone()], SCodeUtil::elementInfo(cdef))?;
        bail!("fail");
    }
    Ok(())
}
