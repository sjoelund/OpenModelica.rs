...
pub(crate) fn refactorGraphicalAnnotation(mut wholeAST: Absyn::Program, mut classToRefactor: Arc<Absyn::Class>) -> Result<Arc<Absyn::Class>> {
    ...
    changedClass = (::match_deref::match_deref! { match &(classToRefactor.clone()) {
        _ => {
            ...
            c = refactorGraphAnnInClass(classToRefactor, wholeAST, Arc::new(Absyn::Path::IDENT { name: (literal!("")).clone() }))?;
            c
        },
...
fn refactorGraphAnnInClassDef(mut inDef: Arc<Absyn::ClassDef>, mut inProgram: Absyn::Program, mut classPath: Arc<Absyn::Path>, mut inClassEnv: Interactive::GraphicEnvCache) -> Arc<Absyn::ClassDef> {
    let mut outDef: Arc<Absyn::ClassDef>;
    outDef = 'mc: {
        let __mc_input = (inDef.clone(), inProgram, classPath, inClassEnv);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ Absyn::ClassDef::PARTS { typeVars, classAttrs, classParts: cp, ann, comment: cmt }, p, cPath, env) => {
                    let mut resultPart: Arc<metamodelica::List<Arc<Absyn::ClassPart>>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ Absyn::ClassDef::DERIVED { typeSpec: ts, attributes: attrs, arguments: args, comment: Some(Deref @ Absyn::Comment { annotation_: Some(Deref @ Absyn::Annotation { elementArgs: annList }), comment: cmt }) }, p, _, _) => {
                    let mut resAnnList: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>;
...
fn refactorGraphAnnInClassParts(mut inParts: Arc<metamodelica::List<Arc<Absyn::ClassPart>>>, mut inProgram: Absyn::Program, mut classPath: Arc<Absyn::Path>, mut env: Interactive::GraphicEnvCache) -> Result<Arc<metamodelica::List<Arc<Absyn::ClassPart>>>> {
    ...
    outParts = (::match_deref::match_deref! { match &((inParts, inProgram, classPath)) {
        (Deref @ metamodelica::List::Nil, _, _) => {
            metamodelica::nil()
        },
        (Deref @ metamodelica::List::Cons { head: firstPart, tail: restParts }, p, cPath) => {
            let mut resParts: Arc<metamodelica::List<Arc<Absyn::ClassPart>>>;
            ...
            resParts = refactorGraphAnnInClassParts(restParts.clone(), p.clone(), cPath.clone(), env)?;
...
fn refactorGraphAnnInClassPart(mut inPart: Arc<Absyn::ClassPart>, mut inProgram: Absyn::Program, mut classPath: Arc<Absyn::Path>, mut inClassEnv: Interactive::GraphicEnvCache) -> Arc<Absyn::ClassPart> {
    let mut outPart: Arc<Absyn::ClassPart>;
    outPart = 'mc: {
        let __mc_input = (inPart.clone(), inProgram, classPath, inClassEnv);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ Absyn::ClassPart::PUBLIC { contents: elContent }, p, cPath, env) => {
                    let mut resultElContent: Arc<metamodelica::List<Arc<Absyn::ElementItem>>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ Absyn::ClassPart::EQUATIONS { contents: eqContent }, p, cPath, env) => {
                    let mut resultEqContent: Arc<metamodelica::List<Arc<Absyn::EquationItem>>>;
                    resultEqContent = refactorGraphAnnInContentList(eqContent.clone(), (std::sync::Arc::new(fnptr!(refactorGraphAnnInEqItem, Arc<Absyn::EquationItem>, Absyn::Program, Arc<Absyn::Path>, Interactive::GraphicEnvCache)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<Absyn::EquationItem>, Absyn::Program, Arc<Absyn::Path>, Interactive::GraphicEnvCache) -> Result<Arc<Absyn::EquationItem>> + 'static>), p.clone(), cPath.clone(), env.clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ Absyn::ClassPart::ALGORITHMS { contents: algContent }, p, cPath, env) => {
                    let mut resultAlgContent: Arc<metamodelica::List<Arc<Absyn::AlgorithmItem>>>;
                    resultAlgContent = refactorGraphAnnInContentList(algContent.clone(), (std::sync::Arc::new(fnptr!(refactorGraphAnnInAlgItem, Arc<Absyn::AlgorithmItem>, Absyn::Program, Arc<Absyn::Path>, Interactive::GraphicEnvCache)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<Absyn::AlgorithmItem>, Absyn::Program, Arc<Absyn::Path>, Interactive::GraphicEnvCache) -> Result<Arc<Absyn::AlgorithmItem>> + 'static>), p.clone(), cPath.clone(), env.clone())?;
...
fn refactorGraphAnnInContentList<contentType: Clone + 'static + metamodelica::gc::MMTrace>(mut inList: Arc<metamodelica::List<contentType>>, mut refactorGraphAnnInItem: Arc<dyn ::std::ops::Fn(contentType, Absyn::Program, Arc<Absyn::Path>, Interactive::GraphicEnvCache) -> Result<contentType> + 'static>, mut inProgram: Absyn::Program, mut classPath: Arc<Absyn::Path>, mut inClassEnv: Interactive::GraphicEnvCache) -> Result<Arc<metamodelica::List<contentType>>> {
    ...
    outList = (::match_deref::match_deref! { match &((inList, inProgram, classPath, inClassEnv)) {
        (Deref @ metamodelica::List::Nil, _, _, _) => {
            metamodelica::nil()
        },
        (Deref @ metamodelica::List::Cons { head: firstItem, tail: restList }, p, cPath, env) => {
            let mut resList: Arc<metamodelica::List<contentType>>;
...
fn refactorGraphAnnInElItem(mut inItem: Arc<Absyn::ElementItem>, mut inProgram: Absyn::Program, mut classPath: Arc<Absyn::Path>, mut inClassEnv: Interactive::GraphicEnvCache) -> Result<Arc<Absyn::ElementItem>> {
    ...
    outItem = (::match_deref::match_deref! { match &((inItem, inProgram, classPath, inClassEnv)) {
        (Deref @ Absyn::ElementItem::ELEMENTITEM { element: el }, p, cPath, env) => {
            ...
            Arc::new(Absyn::ElementItem::ELEMENTITEM { element: resultElement })
        },
        _ => bail!("match: no arm matched"),
    } });
    Ok(outItem)
...
fn refactorGraphAnnInEqItem(mut inItem: Arc<Absyn::EquationItem>, mut inProgram: Absyn::Program, mut classPath: Arc<Absyn::Path>, mut inClassEnv: Interactive::GraphicEnvCache) -> Arc<Absyn::EquationItem> {
    let mut outItem: Arc<Absyn::EquationItem>;
    outItem = 'mc: {
        let __mc_input = (inItem.clone(), inProgram);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
fn refactorGraphAnnInElement(mut inElement: Arc<Absyn::Element>, mut inProgram: Absyn::Program, mut classPath: Arc<Absyn::Path>, mut inClassEnv: Interactive::GraphicEnvCache) -> Result<Arc<Absyn::Element>> {
    ...
    outElement = (::match_deref::match_deref! { match &((inElement, inProgram, classPath, inClassEnv)) {
        (Deref @ Absyn::Element::ELEMENT { finalPrefix: f, redeclareKeywords: rdk, innerOuter: io, specification: es, info: i, constrainClass: cc }, p, cPath, env) => {
            ...
            Arc::new(Absyn::Element::ELEMENT { finalPrefix: f.clone(), redeclareKeywords: rdk.clone(), innerOuter: io.clone(), specification: resultSpec, info: i.clone(), constrainClass: cc.clone() })
        },
        _ => bail!("match: no arm matched"),
    } });
    Ok(outElement)
...
fn refactorConstrainClass(mut inCC: Option<Arc<Absyn::ConstrainClass>>, mut inProgram: Absyn::Program, mut classPath: Arc<Absyn::Path>, mut inClassEnv: Interactive::GraphicEnvCache) -> Result<Option<Arc<Absyn::ConstrainClass>>> {
    ...
    outCC = (::match_deref::match_deref! { match &((inCC, inProgram, classPath, inClassEnv)) {
        (Some(Deref @ Absyn::ConstrainClass { elementSpec: es, comment: com }), p, cPath, env) => {
            let mut resultSpec: Arc<Absyn::ElementSpec>;
            resultSpec = refactorGraphAnnInElSpec(es.clone(), p.clone(), cPath.clone(), env.clone());
            Some(Arc::new(Absyn::ConstrainClass { elementSpec: resultSpec, comment: com.clone() }))
        },
        (None, _, _, _) => {
            None
        },
...
fn refactorGraphAnnInElSpec(mut inSpec: Arc<Absyn::ElementSpec>, mut inProgram: Absyn::Program, mut classPath: Arc<Absyn::Path>, mut inClassEnv: Interactive::GraphicEnvCache) -> Arc<Absyn::ElementSpec> {
    let mut outSpec: Arc<Absyn::ElementSpec>;
    outSpec = 'mc: {
        let __mc_input = (inSpec.clone(), inProgram, classPath, inClassEnv);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ Absyn::ElementSpec::COMPONENTS { attributes: at, typeSpec: Deref @ Absyn::TypeSpec::TPATH { path, arrayDim: z }, components: Deref @ metamodelica::List::Cons { head: firstComp, tail: restCompList } }, p, cPath, env) => {
                    let mut resultComp: Arc<Absyn::ComponentItem>;
                    let mut resCompList: Arc<metamodelica::List<Arc<Absyn::ComponentItem>>>;
                    let mut at = (*at).clone();
                    ...
                    Ok(Arc::new(Absyn::ElementSpec::COMPONENTS { attributes: at.clone(), typeSpec: Arc::new(Absyn::TypeSpec::TPATH { path: path.clone(), arrayDim: z.clone() }), components: metamodelica::cons(resultComp.clone(), resCompList.clone()) }))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
...
fn refactorGraphAnnInComponentItem(mut inCom: Arc<Absyn::ComponentItem>, mut classPath: Arc<Absyn::Path>, mut inPath: Arc<Absyn::Path>, mut inProgram: Absyn::Program, mut inClassEnv: Interactive::GraphicEnvCache) -> Arc<Absyn::ComponentItem> {
    let mut outCom: Arc<Absyn::ComponentItem>;
    outCom = 'mc: {
        let __mc_input = (inCom.clone(), classPath, inPath, inProgram, inClassEnv);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
fn transformComponentAnnList(mut inArgs: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>, mut inCon: Context, mut resultList: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>, mut classPath: Arc<Absyn::Path>, mut inPath: Arc<Absyn::Path>, mut inProgram: Absyn::Program, mut inClassEnv: Interactive::GraphicEnvCache) -> Result<Arc<metamodelica::List<Arc<Absyn::ElementArg>>>> {
    let mut outArgs: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>;
    outArgs = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { finalPrefix: fi, eachPrefix: e, path: Deref @ Absyn::Path::IDENT { name: Deref @ "extent" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::MATRIX { matrix: Deref @ metamodelica::List::Cons { head: Deref @ metamodelica::List::Cons { head: x1, tail: Deref @ metamodelica::List::Cons { head: y1, tail: Deref @ metamodelica::List::Nil } }, tail: Deref @ metamodelica::List::Cons { head: Deref @ metamodelica::List::Cons { head: x2, tail: Deref @ metamodelica::List::Cons { head: y2, tail: Deref @ metamodelica::List::Nil } }, tail: Deref @ metamodelica::List::Nil } } }, .. }, .. }), comment: com, info }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Component", tail: _ }, res, cPath, path, p, env) => {
                    let mut trans: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>;
                    ...
                    res = list![Arc::new(Absyn::ElementArg::MODIFICATION { finalPrefix: fi.clone(), eachPrefix: e.clone(), path: Arc::new(Absyn::Path::IDENT { name: (literal!("Placement")).clone() }), modification: Some(Arc::new(Absyn::Modification { elementArgLst: trans.clone(), eqMod: openmodelica_ast::Absyn::EqMod::interned_NOMOD() })), comment: com.clone(), info: info.clone() })];
                    Ok(res.clone())
                }
                _ => bail!("nomatch"),
            }}
...
fn getRestrictionInClass(mut inClass: Arc<Absyn::Class>) -> Result<Absyn::Restriction> {
    let mut outRestriction: Absyn::Restriction;
    outRestriction = (::match_deref::match_deref! { match &(inClass) {
...
fn getRotationDegree(mut inList: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>) -> Option<metamodelica::Real> {
    ...
    degrees = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "rotation" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp: ex, .. }, .. }), .. }, tail: _ } => {
...
fn getIconTransformation(mut ax1: Arc<Absyn::Exp>, mut ay1: Arc<Absyn::Exp>, mut ax2: Arc<Absyn::Exp>, mut ay2: Arc<Absyn::Exp>, mut inRotation: Option<metamodelica::Real>, mut classPath: Arc<Absyn::Path>, mut inPath: Arc<Absyn::Path>, mut inProg: Absyn::Program, mut inClassEnv: Interactive::GraphicEnvCache) -> Result<Arc<Absyn::ElementArg>> {
    let mut iconTrans: Arc<Absyn::ElementArg>;
    iconTrans = (::match_deref::match_deref! { match &((ax1, ay1, ax2, ay2, inRotation, classPath, inPath, inProg, inClassEnv)) {
        ...
        (x1, y1, x2, y2, Some(rot), cPath, path, p, env) => {
            ...
            Arc::new(Absyn::ElementArg::MODIFICATION { finalPrefix: false, eachPrefix: openmodelica_ast::Absyn::Each::NON_EACH, path: Arc::new(Absyn::Path::IDENT { name: (literal!("iconTransformation")).clone() }), modification: Some(Arc::new(Absyn::Modification { elementArgLst: list![x, y, scale, aspectRatio, flipHorizontal, flipVertical, rotation], eqMod: openmodelica_ast::Absyn::EqMod::interned_NOMOD() })), comment: None, info: Absyn::dummyInfo.clone() })
        },
        _ => bail!("match: no arm matched"),
    } });
    Ok(iconTrans)
...
fn getDiagramTransformation(mut ax1: Arc<Absyn::Exp>, mut ay1: Arc<Absyn::Exp>, mut ax2: Arc<Absyn::Exp>, mut ay2: Arc<Absyn::Exp>, mut inRotation: Option<metamodelica::Real>, mut classPath: Arc<Absyn::Path>, mut inPath: Arc<Absyn::Path>, mut inProg: Absyn::Program, mut inClassEnv: Interactive::GraphicEnvCache) -> Result<Arc<Absyn::ElementArg>> {
    ...
    trans = (::match_deref::match_deref! { match &((ax1, ay1, ax2, ay2, inRotation, classPath, inPath, inProg, inClassEnv)) {
        (x1, y1, x2, y2, None, cPath, path, p, env) => {
            let mut rcx1: metamodelica::Real;
            let mut rcy1: metamodelica::Real;
            let mut rcx2: metamodelica::Real;
        ...
        (x1, y1, x2, y2, Some(rot), cPath, path, p, env) => {
            ...
            flipVertical = getFlipAnn(ray1, ray2, (literal!("flipVertical")).clone());
            rotation = getRotationAnn(rot.clone());
            Arc::new(Absyn::ElementArg::MODIFICATION { finalPrefix: false, eachPrefix: openmodelica_ast::Absyn::Each::NON_EACH, path: Arc::new(Absyn::Path::IDENT { name: (literal!("transformation")).clone() }), modification: Some(Arc::new(Absyn::Modification { elementArgLst: list![x, y, scale, aspectRatio, flipHorizontal, flipVertical, rotation], eqMod: openmodelica_ast::Absyn::EqMod::interned_NOMOD() })), comment: None, info: Absyn::dummyInfo.clone() })
        },
        _ => bail!("match: no arm matched"),
    } });
    Ok(trans)
...
fn getFlipAnn(mut val1: metamodelica::Real, mut val2: metamodelica::Real, mut name: ArcStr) -> Arc<Absyn::ElementArg> {
    ...
    let mut value: bool;
    value = val1 > val2;
    flip = Arc::new(Absyn::ElementArg::MODIFICATION { finalPrefix: false, eachPrefix: openmodelica_ast::Absyn::Each::NON_EACH, path: Arc::new(Absyn::Path::IDENT { name: (name).clone() }), modification: Some(Arc::new(Absyn::Modification { elementArgLst: metamodelica::nil(), eqMod: Arc::new(Absyn::EqMod::EQMOD { exp: Arc::new(Absyn::Exp::BOOL { value: value }), info: Absyn::dummyInfo.clone() }) })), comment: None, info: Absyn::dummyInfo.clone() });
    flip
}
...
fn getRotationAnn(mut rot: metamodelica::Real) -> Arc<Absyn::ElementArg> {
    ...
    rotation = Arc::new(Absyn::ElementArg::MODIFICATION { finalPrefix: false, eachPrefix: openmodelica_ast::Absyn::Each::NON_EACH, path: Arc::new(Absyn::Path::IDENT { name: (literal!("rotation")).clone() }), modification: Some(Arc::new(Absyn::Modification { elementArgLst: metamodelica::nil(), eqMod: Arc::new(Absyn::EqMod::EQMOD { exp: Arc::new(Absyn::Exp::REAL { value: (s).clone() }), info: Absyn::dummyInfo.clone() }) })), comment: None, info: Absyn::dummyInfo.clone() });
    rotation
}
...
fn getCoordsInPath(mut classPath: Arc<Absyn::Path>, mut inPath: Arc<Absyn::Path>, mut inProgram: Absyn::Program, mut contextToGetCoordsFrom: Context, mut inClassEnv: Interactive::GraphicEnvCache) -> Result<(Arc<Absyn::Exp>, Arc<Absyn::Exp>, Arc<Absyn::Exp>, Arc<Absyn::Exp>)> {
    let mut posX1: Arc<Absyn::Exp>;
    let mut posY1: Arc<Absyn::Exp>;
    let mut posX2: Arc<Absyn::Exp>;
    let mut posY2: Arc<Absyn::Exp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (cPath, path, p, context, _) => {
                    let mut cdef: Arc<Absyn::Class>;
                    let mut x1: Arc<Absyn::Exp>;
                    let mut y1: Arc<Absyn::Exp>;
                    let mut x2: Arc<Absyn::Exp>;
                    let mut y2: Arc<Absyn::Exp>;
                    let mut fullPath: Arc<Absyn::Path>;
                    fullPath = fixPaths(cPath.clone(), path.clone());
                    cdef = ProgramUtil::getPathedClassInProgram(fullPath.clone(), p.clone(), false, false)?;
...
fn getCoordsInClass(mut inClass: Arc<Absyn::Class>, mut contextToGetCoordsFrom: Context) -> Result<(Arc<Absyn::Exp>, Arc<Absyn::Exp>, Arc<Absyn::Exp>, Arc<Absyn::Exp>)> {
    ...
    (x1, y1, x2, y2) = (::match_deref::match_deref! { match &((inClass, contextToGetCoordsFrom)) {
...
fn getCoordsInAnnList(mut inAnns: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>, mut contextToGetCoordsFrom: Context) -> Result<(Arc<Absyn::Exp>, Arc<Absyn::Exp>, Arc<Absyn::Exp>, Arc<Absyn::Exp>)> {
    ...
    (x1, y1, x2, y2) = 'mc: {
        ...
        if let Ok((__v, __wb0, __wb1, __wb2, __wb3)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "Coordsys" }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, .. }), .. }, tail: _ }, _) => {
        ...
        })() { x1 = __wb0; x2 = __wb1; y1 = __wb2; y2 = __wb3; break 'mc __v; }
        if let Ok((__v, __wb0, __wb1, __wb2, __wb3)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "Icon" }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, .. }), .. }, tail: _ }, Deref @ metamodelica::List::Cons { head: Deref @ "Icon", tail: _ }) => {
                    let mut x1: Arc<Absyn::Exp> = x1.clone();
                    let mut x2: Arc<Absyn::Exp> = x2.clone();
                    let mut y1: Arc<Absyn::Exp> = y1.clone();
                    let mut y2: Arc<Absyn::Exp> = y2.clone();
        ...
        if let Ok((__v, __wb0, __wb1, __wb2, __wb3)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "Diagram" }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, .. }), .. }, tail: _ }, Deref @ metamodelica::List::Cons { head: Deref @ "Diagram", tail: _ }) => {
                    let mut x1: Arc<Absyn::Exp> = x1.clone();
                    let mut x2: Arc<Absyn::Exp> = x2.clone();
                    let mut y1: Arc<Absyn::Exp> = y1.clone();
                    let mut y2: Arc<Absyn::Exp> = y2.clone();
...
fn getCoordsFromLayerArgs(mut inAnns: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>) -> Result<(Arc<Absyn::Exp>, Arc<Absyn::Exp>, Arc<Absyn::Exp>, Arc<Absyn::Exp>)> {
    ...
    (x1, y1, x2, y2) = 'mc: {
        ...
        if let Ok((__v, __wb0, __wb1, __wb2, __wb3)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "coordinateSystem" }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, .. }), .. }, tail: _ } => {
                    let mut x1: Arc<Absyn::Exp> = x1.clone();
                    let mut x2: Arc<Absyn::Exp> = x2.clone();
                    let mut y1: Arc<Absyn::Exp> = y1.clone();
                    let mut y2: Arc<Absyn::Exp> = y2.clone();
...
fn transformConnectAnnList(mut inArgs: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>, mut inCon: Context, mut resultList: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>, mut inProgram: Absyn::Program) -> Result<Arc<metamodelica::List<Arc<Absyn::ElementArg>>>> {
    ...
    outArgs = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { finalPrefix: fi, eachPrefix: e, path: Deref @ Absyn::Path::IDENT { name: Deref @ "points" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::MATRIX { matrix: expMatrix }, info }, .. }), comment: com, info: mod_info }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Connect", tail: _ }, res, p) => {
                    let mut expLst: Arc<metamodelica::List<Arc<Absyn::Exp>>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { finalPrefix: fi, eachPrefix: e, path: Deref @ Absyn::Path::IDENT { name: Deref @ "points" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::MATRIX { matrix: expMatrix }, info }, .. }), comment: com, info: mod_info }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Line", tail: _ }, res, p) => {
                    let mut expLst: Arc<metamodelica::List<Arc<Absyn::Exp>>>;
                    let mut res = (*res).clone();
                    expLst = List::map(expMatrix.clone(), (std::sync::Arc::new(fnptr!(matrixToArray, Arc<metamodelica::List<Arc<Absyn::Exp>>>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<metamodelica::List<Arc<Absyn::Exp>>>) -> Result<Arc<Absyn::Exp>> + 'static>))?;
                    res = transformConnectAnnList(rest.clone(), context.clone(), res.clone(), p.clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { finalPrefix: fi, eachPrefix: e, path: Deref @ Absyn::Path::IDENT { name: Deref @ "style" }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, eqMod }), comment: com, info: mod_info }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Connect", tail: _ }, res, p) => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "style" }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, .. }), .. }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Line", tail: _ }, res, p) => {
                    let mut args = (*args).clone();
                    let mut rest = (*rest).clone();
                    let mut res = (*res).clone();
                    args = cleanStyleAttrs(args.clone(), metamodelica::nil(), context.clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { finalPrefix: fi, eachPrefix: e, path: Deref @ Absyn::Path::IDENT { name: Deref @ "color" }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::INTEGER { value: x }, info } }), comment: com, info: mod_info }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Line", tail: _ }, res, p) => {
                    let mut color1: i32;
                    let mut color2: i32;
                    let mut color3: i32;
                    let mut res = (*res).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { finalPrefix: fi, eachPrefix: e, path: Deref @ Absyn::Path::IDENT { name: Deref @ "pattern" }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::INTEGER { value: x }, .. } }), comment: com, info: mod_info }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Line", tail: _ }, res, p) => {
                    let mut val: ArcStr;
                    let mut res = (*res).clone();
                    val = ((patternMapList.clone()).get(x.clone() + 1)?).clone();
                    res = transformConnectAnnList(rest.clone(), context.clone(), res.clone(), p.clone())?;
                    Ok(metamodelica::cons(Arc::new(Absyn::ElementArg::MODIFICATION { finalPrefix: fi.clone(), eachPrefix: e.clone(), path: Arc::new(Absyn::Path::IDENT { name: (literal!("pattern")).clone() }), modification: Some(Arc::new(Absyn::Modification { elementArgLst: args.clone(), eqMod: Arc::new(Absyn::EqMod::EQMOD { exp: Arc::new(Absyn::Exp::CREF { componentRef: Arc::new(Absyn::ComponentRef::CREF_QUAL { name: (literal!("LinePattern")).clone(), subscripts: metamodelica::nil(), componentRef: Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (val.clone()).clone(), subscripts: metamodelica::nil() }) }) }), info: Absyn::dummyInfo.clone() }) })), comment: com.clone(), info: mod_info.clone() }), res.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { finalPrefix: fi, eachPrefix: e, path: Deref @ Absyn::Path::IDENT { name: Deref @ "thickness" }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::INTEGER { value: x }, .. } }), comment: com, info: mod_info }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Line", tail: _ }, res, p) => {
                    let mut s: ArcStr;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { finalPrefix: fi, eachPrefix: e, path: Deref @ Absyn::Path::IDENT { name: Deref @ "smooth" }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, eqMod }), comment: com, info: mod_info }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Line", tail: _ }, res, p) => {
                    let mut res = (*res).clone();
                    res = transformConnectAnnList(rest.clone(), context.clone(), res.clone(), p.clone())?;
                    Ok(metamodelica::cons(Arc::new(Absyn::ElementArg::MODIFICATION { finalPrefix: fi.clone(), eachPrefix: e.clone(), path: Arc::new(Absyn::Path::IDENT { name: (literal!("smooth")).clone() }), modification: Some(Arc::new(Absyn::Modification { elementArgLst: args.clone(), eqMod: eqMod.clone() })), comment: com.clone(), info: mod_info.clone() }), res.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
...
fn transformClassAnnList(mut inArgs: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>, mut inCon: Context, mut resultList: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>, mut inProgram: Absyn::Program) -> Result<Arc<metamodelica::List<Arc<Absyn::ElementArg>>>> {
    ...
    outArgs = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { finalPrefix: fi, eachPrefix: e, path: Deref @ Absyn::Path::IDENT { name: Deref @ "Icon" }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, eqMod }), comment: com, info: mod_info }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Class", tail: c }, res, p) => {
                    let mut argRes: Arc<metamodelica::List<Arc<Absyn::Exp>>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { finalPrefix: fi, eachPrefix: e, path: Deref @ Absyn::Path::IDENT { name: Deref @ "Diagram" }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, eqMod }), comment: com, info: mod_info }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Class", tail: c }, res, p) => {
                    let mut argRes: Arc<metamodelica::List<Arc<Absyn::Exp>>>;
                    let mut coord: Arc<Absyn::ElementArg>;
                    let mut c = (*c).clone();
                    let mut res = (*res).clone();
                    ...
                    coord = getCoordSysAnn(listAppend(res.clone(), rest.clone()), p.clone())?;
                    res = transformClassAnnList(rest.clone(), context.clone(), res.clone(), p.clone())?;
                    Ok(metamodelica::cons(Arc::new(Absyn::ElementArg::MODIFICATION { finalPrefix: fi.clone(), eachPrefix: e.clone(), path: Arc::new(Absyn::Path::IDENT { name: (literal!("Diagram")).clone() }), modification: Some(Arc::new(Absyn::Modification { elementArgLst: list![coord.clone(), Arc::new(Absyn::ElementArg::MODIFICATION { finalPrefix: false, eachPrefix: openmodelica_ast::Absyn::Each::NON_EACH, path: Arc::new(Absyn::Path::IDENT { name: (literal!("graphics")).clone() }), modification: Some(Arc::new(Absyn::Modification { elementArgLst: metamodelica::nil(), eqMod: Arc::new(Absyn::EqMod::EQMOD { exp: Arc::new(Absyn::Exp::ARRAY { arrayExp: argRes.clone() }), info: Absyn::dummyInfo.clone() }) })), comment: None, info: mod_info.clone() })], eqMod: eqMod.clone() })), comment: com.clone(), info: mod_info.clone() }), res.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { finalPrefix: fi, eachPrefix: e, path: Deref @ Absyn::Path::IDENT { name: Deref @ "Coordsys" }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, eqMod }), comment: com, info: mod_info }, tail: rest }, context, res, p) => {
                    let mut res = (*res).clone();
                    let true = (isLayerAnnInList(listAppend(res.clone(), rest.clone()))) else { bail!("pattern mismatch") };
                    res = metamodelica::cons(Arc::new(Absyn::ElementArg::MODIFICATION { finalPrefix: fi.clone(), eachPrefix: e.clone(), path: Arc::new(Absyn::Path::IDENT { name: (literal!("Coordsys")).clone() }), modification: Some(Arc::new(Absyn::Modification { elementArgLst: args.clone(), eqMod: eqMod.clone() })), comment: com.clone(), info: mod_info.clone() }), res.clone());
                    res = transformClassAnnList(rest.clone(), context.clone(), res.clone(), p.clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { finalPrefix: fi, eachPrefix: e, path: Deref @ Absyn::Path::IDENT { name: Deref @ "extent" }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::MATRIX { matrix: Deref @ metamodelica::List::Cons { head: Deref @ metamodelica::List::Cons { head: x1, tail: Deref @ metamodelica::List::Cons { head: y1, tail: Deref @ metamodelica::List::Nil } }, tail: Deref @ metamodelica::List::Cons { head: Deref @ metamodelica::List::Cons { head: x2, tail: Deref @ metamodelica::List::Cons { head: y2, tail: Deref @ metamodelica::List::Nil } }, tail: Deref @ metamodelica::List::Nil } } }, info } }), comment: com, info: mod_info }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Coordsys", tail: _ }, res, p) => {
                    let mut res = (*res).clone();
                    res = metamodelica::cons(Arc::new(Absyn::ElementArg::MODIFICATION { finalPrefix: fi.clone(), eachPrefix: e.clone(), path: Arc::new(Absyn::Path::IDENT { name: (literal!("extent")).clone() }), modification: Some(Arc::new(Absyn::Modification { elementArgLst: args.clone(), eqMod: Arc::new(Absyn::EqMod::EQMOD { exp: Arc::new(Absyn::Exp::ARRAY { arrayExp: list![Arc::new(Absyn::Exp::ARRAY { arrayExp: list![x1.clone(), y1.clone()] }), Arc::new(Absyn::Exp::ARRAY { arrayExp: list![x2.clone(), y2.clone()] })] }), info: info.clone() }) })), comment: com.clone(), info: mod_info.clone() }), res.clone());
                    res = transformClassAnnList(rest.clone(), context.clone(), res.clone(), p.clone())?;
                    Ok(res.clone())
                }
                _ => bail!("nomatch"),
...
fn getCoordSysAnn(mut inArgs: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>, mut inProgram: Absyn::Program) -> Result<Arc<Absyn::ElementArg>> {
    ...
    coordSys = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { finalPrefix: fi, eachPrefix: e, path: Deref @ Absyn::Path::IDENT { name: Deref @ "Coordsys" }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, eqMod }), comment: com, info }, tail: _ }, p) => {
                    let mut args = (*args).clone();
                    args = transformClassAnnList(args.clone(), metamodelica::cons((literal!("Coordsys")).clone(), metamodelica::nil()), metamodelica::nil(), p.clone())?;
...
fn transAnnLstToCalls(mut inArgs: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>, mut inCon: Context) -> Result<Arc<metamodelica::List<Arc<Absyn::Exp>>>> {
    let mut outArgs: Arc<metamodelica::List<Arc<Absyn::Exp>>>;
    outArgs = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "Line" }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, .. }), .. }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Layer", tail: c }) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::Exp>>>;
                    let mut argRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    let mut c = (*c).clone();
                    ...
                    Ok(metamodelica::cons(Arc::new(Absyn::Exp::CALL { function_: Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (literal!("Line")).clone(), subscripts: metamodelica::nil() }), functionArgs: Arc::new(Absyn::FunctionArgs::FUNCTIONARGS { args: metamodelica::nil(), argNames: metamodelica::cons(Arc::new(Absyn::NamedArg { argName: (literal!("color")).clone(), argValue: Arc::new(Absyn::Exp::ARRAY { arrayExp: list![Arc::new(Absyn::Exp::INTEGER { value: 0 }), Arc::new(Absyn::Exp::INTEGER { value: 0 }), Arc::new(Absyn::Exp::INTEGER { value: 255 })] }) }), argRes.clone()) }), typeVars: metamodelica::nil() }), restRes.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: n }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, .. }), .. }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Layer", tail: c }) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::Exp>>>;
                    let mut argRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    let mut c = (*c).clone();
                    ...
                    restRes = transAnnLstToCalls(rest.clone(), context.clone())?;
                    Ok(metamodelica::cons(Arc::new(Absyn::Exp::CALL { function_: Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (n.clone()).clone(), subscripts: metamodelica::nil() }), functionArgs: Arc::new(Absyn::FunctionArgs::FUNCTIONARGS { args: metamodelica::nil(), argNames: metamodelica::cons(Arc::new(Absyn::NamedArg { argName: (literal!("lineColor")).clone(), argValue: Arc::new(Absyn::Exp::ARRAY { arrayExp: list![Arc::new(Absyn::Exp::INTEGER { value: 0 }), Arc::new(Absyn::Exp::INTEGER { value: 0 }), Arc::new(Absyn::Exp::INTEGER { value: 255 })] }) }), argRes.clone()) }), typeVars: metamodelica::nil() }), restRes.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: n }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, .. }), .. }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Layer", tail: c }) => {
                    ...
                    argRes = transAnnLstToNamedArgs(args.clone(), c.clone())?;
                    restRes = transAnnLstToCalls(rest.clone(), context.clone())?;
                    Ok(metamodelica::cons(Arc::new(Absyn::Exp::CALL { function_: Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (n.clone()).clone(), subscripts: metamodelica::nil() }), functionArgs: Arc::new(Absyn::FunctionArgs::FUNCTIONARGS { args: metamodelica::nil(), argNames: argRes.clone() }), typeVars: metamodelica::nil() }), restRes.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: _, tail: rest }, context) => {
                    let mut res: Arc<metamodelica::List<Arc<Absyn::Exp>>>;
...
fn nameArgWithName(mut narg: Arc<Absyn::NamedArg>, mut argName: ArcStr) -> Result<bool> {
    ...
    res = (::match_deref::match_deref! { match &(narg) {
        Deref @ Absyn::NamedArg { argName: name, argValue: _ } => {
            res = name.clone() == argName;
            res
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn transAnnLstToNamedArgs(mut inArgs: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>, mut inCon: Context) -> Result<Arc<metamodelica::List<Arc<Absyn::NamedArg>>>> {
    let mut outArgs: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
    outArgs = 'mc: {
        let __mc_input = (inArgs, inCon);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "extent" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::MATRIX { matrix: Deref @ metamodelica::List::Cons { head: Deref @ metamodelica::List::Cons { head: x1, tail: Deref @ metamodelica::List::Cons { head: y1, tail: Deref @ metamodelica::List::Nil } }, tail: Deref @ metamodelica::List::Cons { head: Deref @ metamodelica::List::Cons { head: x2, tail: Deref @ metamodelica::List::Cons { head: y2, tail: Deref @ metamodelica::List::Nil } }, tail: Deref @ metamodelica::List::Nil } } }, .. }, .. }), .. }, tail: rest }, context) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    restRes = transAnnLstToNamedArgs(rest.clone(), context.clone())?;
                    Ok(metamodelica::cons(Arc::new(Absyn::NamedArg { argName: (literal!("extent")).clone(), argValue: Arc::new(Absyn::Exp::ARRAY { arrayExp: list![Arc::new(Absyn::Exp::ARRAY { arrayExp: list![x1.clone(), y1.clone()] }), Arc::new(Absyn::Exp::ARRAY { arrayExp: list![x2.clone(), y2.clone()] })] }) }), restRes.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "style" }, modification: Some(Deref @ Absyn::Modification { elementArgLst: args, .. }), .. }, tail: rest }, context) => {
                    let mut res: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    let mut argRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    let mut args = (*args).clone();
                    restRes = transAnnLstToNamedArgs(rest.clone(), context.clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "color" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::INTEGER { value: x }, .. }, .. }), .. }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Text", tail: _ }) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    let mut color1: i32;
                    let mut color2: i32;
                    let mut color3: i32;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "color" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::INTEGER { value: x }, .. }, .. }), .. }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Line", tail: _ }) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    let mut color1: i32;
                    let mut color2: i32;
                    let mut color3: i32;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "color" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::INTEGER { value: x }, .. }, .. }), .. }, tail: rest }, context) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    let mut color1: i32;
                    let mut color2: i32;
                    let mut color3: i32;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "fillColor" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::INTEGER { value: x }, .. }, .. }), .. }, tail: rest }, context) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    let mut color1: i32;
                    let mut color2: i32;
                    let mut color3: i32;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "pattern" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::INTEGER { value: x }, .. }, .. }), .. }, tail: rest }, context) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    let mut val: ArcStr;
                    val = ((patternMapList.clone()).get(x.clone() + 1)?).clone();
                    restRes = transAnnLstToNamedArgs(rest.clone(), context.clone())?;
                    Ok(metamodelica::cons(Arc::new(Absyn::NamedArg { argName: (literal!("pattern")).clone(), argValue: Arc::new(Absyn::Exp::CREF { componentRef: Arc::new(Absyn::ComponentRef::CREF_QUAL { name: (literal!("LinePattern")).clone(), subscripts: metamodelica::nil(), componentRef: Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (val.clone()).clone(), subscripts: metamodelica::nil() }) }) }) }), restRes.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "fillPattern" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::INTEGER { value: x }, .. }, .. }), .. }, tail: rest }, context) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    let mut val: ArcStr;
                    val = ((fillPatternMapList.clone()).get(x.clone() + 1)?).clone();
                    restRes = transAnnLstToNamedArgs(rest.clone(), context.clone())?;
                    Ok(metamodelica::cons(Arc::new(Absyn::NamedArg { argName: (literal!("fillPattern")).clone(), argValue: Arc::new(Absyn::Exp::CREF { componentRef: Arc::new(Absyn::ComponentRef::CREF_QUAL { name: (literal!("FillPattern")).clone(), subscripts: metamodelica::nil(), componentRef: Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (val.clone()).clone(), subscripts: metamodelica::nil() }) }) }) }), restRes.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "thickness" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::INTEGER { value: x }, .. }, .. }), .. }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Line", tail: _ }) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    let mut s: ArcStr;
                    let mut thick: metamodelica::Real;
                    thick = (thicknessMapList.clone()).get(x.clone())?;
                    restRes = transAnnLstToNamedArgs(rest.clone(), context.clone())?;
                    s = (realString(thick.clone())).clone();
                    Ok(metamodelica::cons(Arc::new(Absyn::NamedArg { argName: (literal!("thickness")).clone(), argValue: Arc::new(Absyn::Exp::REAL { value: (s.clone()).clone() }) }), restRes.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "thickness" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::INTEGER { value: x }, .. }, .. }), .. }, tail: rest }, context) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    let mut s: ArcStr;
                    let mut thick: metamodelica::Real;
                    thick = (thicknessMapList.clone()).get(x.clone())?;
                    restRes = transAnnLstToNamedArgs(rest.clone(), context.clone())?;
                    s = (realString(thick.clone())).clone();
                    Ok(metamodelica::cons(Arc::new(Absyn::NamedArg { argName: (literal!("lineThickness")).clone(), argValue: Arc::new(Absyn::Exp::REAL { value: (s.clone()).clone() }) }), restRes.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "gradient" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::INTEGER { value: x }, .. }, .. }), .. }, tail: rest }, context) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    let mut val: ArcStr;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "smooth" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp, .. }, .. }), .. }, tail: rest }, context) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    restRes = transAnnLstToNamedArgs(rest.clone(), context.clone())?;
                    Ok(metamodelica::cons(Arc::new(Absyn::NamedArg { argName: (literal!("smooth")).clone(), argValue: exp.clone() }), restRes.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "arrow" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::INTEGER { value: x }, .. }, .. }), .. }, tail: rest }, context) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    let mut arrows: Arc<metamodelica::List<ArcStr>>;
                    let mut val1: ArcStr;
                    let mut val2: ArcStr;
                    ...
                    val2 = ((arrows.clone()).get(2)?).clone();
                    restRes = transAnnLstToNamedArgs(rest.clone(), context.clone())?;
                    Ok(metamodelica::cons(Arc::new(Absyn::NamedArg { argName: (literal!("arrow")).clone(), argValue: Arc::new(Absyn::Exp::ARRAY { arrayExp: list![Arc::new(Absyn::Exp::CREF { componentRef: Arc::new(Absyn::ComponentRef::CREF_QUAL { name: (literal!("Arrow")).clone(), subscripts: metamodelica::nil(), componentRef: Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (val1.clone()).clone(), subscripts: metamodelica::nil() }) }) }), Arc::new(Absyn::Exp::CREF { componentRef: Arc::new(Absyn::ComponentRef::CREF_QUAL { name: (literal!("Arrow")).clone(), subscripts: metamodelica::nil(), componentRef: Arc::new(Absyn::ComponentRef::CREF_IDENT { name: (val2.clone()).clone(), subscripts: metamodelica::nil() }) }) })] }) }), restRes.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "textStyle" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp, .. }, .. }), .. }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Text", tail: _ }) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    restRes = transAnnLstToNamedArgs(rest.clone(), context.clone())?;
                    Ok(metamodelica::cons(Arc::new(Absyn::NamedArg { argName: (literal!("textStyle")).clone(), argValue: exp.clone() }), restRes.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "font" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp, .. }, .. }), .. }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Text", tail: _ }) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    restRes = transAnnLstToNamedArgs(rest.clone(), context.clone())?;
                    Ok(metamodelica::cons(Arc::new(Absyn::NamedArg { argName: (literal!("font")).clone(), argValue: exp.clone() }), restRes.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "string" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp, .. }, .. }), .. }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Text", tail: _ }) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    restRes = transAnnLstToNamedArgs(rest.clone(), context.clone())?;
                    Ok(metamodelica::cons(Arc::new(Absyn::NamedArg { argName: (literal!("textString")).clone(), argValue: exp.clone() }), restRes.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "name" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp, .. }, .. }), .. }, tail: rest }, context @ Deref @ metamodelica::List::Cons { head: Deref @ "Bitmap", tail: _ }) => {
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    restRes = transAnnLstToNamedArgs(rest.clone(), context.clone())?;
                    Ok(metamodelica::cons(Arc::new(Absyn::NamedArg { argName: (literal!("fileName")).clone(), argValue: exp.clone() }), restRes.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "points" }, modification: Some(Deref @ Absyn::Modification { eqMod: Deref @ Absyn::EqMod::EQMOD { exp: Deref @ Absyn::Exp::MATRIX { matrix: expMatrix }, .. }, .. }), .. }, tail: rest }, context) => {
                    let mut expLst: Arc<metamodelica::List<Arc<Absyn::Exp>>>;
                    let mut restRes: Arc<metamodelica::List<Arc<Absyn::NamedArg>>>;
                    expLst = List::map(expMatrix.clone(), (std::sync::Arc::new(fnptr!(matrixToArray, Arc<metamodelica::List<Arc<Absyn::Exp>>>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<metamodelica::List<Arc<Absyn::Exp>>>) -> Result<Arc<Absyn::Exp>> + 'static>))?;
                    restRes = transAnnLstToNamedArgs(rest.clone(), context.clone())?;
...
fn cleanStyleAttrs2(mut inArgs: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>, mut inResultList: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>, mut inCon: Context) -> Result<Arc<metamodelica::List<Arc<Absyn::ElementArg>>>> {
    '__tco: loop {
        ...
        (Deref @ metamodelica::List::Cons { head: arg @ Deref @ Absyn::ElementArg::MODIFICATION { path: Deref @ Absyn::Path::IDENT { name: Deref @ "color" }, .. }, tail: rest }, resultList, context) => {
            let mut outList: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>;
...
fn isGradientInList(mut inArgs: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>) -> bool {
    '__tco: loop {
        ...
        Deref @ metamodelica::List::Cons { head: _, tail: rest } => {
            let mut res: bool;
            { inArgs = rest.clone(); continue '__tco; }
        },
        _ => unreachable!("tail-call lowered match: no arm matched"),
    } }
    }
...
fn removeFillPatternInList(mut inList: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>) -> Arc<metamodelica::List<Arc<Absyn::ElementArg>>> {
    ...
    outList = (::match_deref::match_deref! { match &(inList) {
        ...
        Deref @ metamodelica::List::Cons { head: arg, tail: rest } => {
            let mut lst: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>;
...
fn setDefaultFillColor(mut oldList: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>, mut transformedList: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>) -> (Arc<metamodelica::List<Arc<Absyn::ElementArg>>>, Arc<metamodelica::List<Arc<Absyn::ElementArg>>>) {
    let mut oList: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>;
    let mut tList: Arc<metamodelica::List<Arc<Absyn::ElementArg>>>;
    (oList, tList) = 'mc: {
...
fn matrixToArray(mut inLst: Arc<metamodelica::List<Arc<Absyn::Exp>>>) -> Arc<Absyn::Exp> {
    ...
    outExp = Arc::new(Absyn::Exp::ARRAY { arrayExp: inLst });
    outExp
}
...
fn addContext(mut inList: Arc<metamodelica::List<ArcStr>>, mut newCon: ArcStr) -> Arc<metamodelica::List<ArcStr>> {
    ...
    outList = (::match_deref::match_deref! { match &((inList, newCon)) {
        (strLst, r#str) => {
            metamodelica::cons((r#str.clone()).clone(), strLst.clone())
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    outList
