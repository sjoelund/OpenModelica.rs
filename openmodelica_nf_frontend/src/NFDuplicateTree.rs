...
pub fn fold<FT: Clone + 'static>(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry>, FT) -> Result<FT> + 'static>, mut inStartValue: FT) -> FT {
...
pub fn foldCond<FT: Clone + 'static>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry>, FT) -> Result<(FT, bool)> + 'static>, mut value: FT) -> FT {
...
pub fn fold_2<FT1: Clone + 'static, FT2: Clone + 'static>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry>, FT1, FT2) -> Result<(FT1, FT2)> + 'static>, mut foldArg1: FT1, mut foldArg2: FT2) -> (FT1, FT2) {
...
pub fn map(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry>) -> Result<Arc<Entry>> + 'static>) -> Arc<Tree> {
...
pub fn mapFold<FT: Clone + 'static>(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry>, FT) -> Result<(Arc<Entry>, FT)> + 'static>, mut inStartValue: FT) -> (Arc<Tree>, FT) {
...
pub fn update(mut tree: Arc<Tree>, mut key: Key, mut value: Value) -> Arc<Tree> {
    let mut outTree: Arc<Tree> = add(tree.clone(), (key.clone()).clone(), value.clone(), (std::sync::Arc::new(fnptr!(addConflictReplace, Arc<Entry>, Arc<Entry>, ArcStr)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<Entry>, Arc<Entry>, ArcStr) -> Result<Arc<Entry>> + 'static>)).unwrap();
    outTree
}
