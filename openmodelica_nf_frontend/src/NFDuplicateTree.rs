...
pub fn addList(mut tree: Arc<Tree>, mut inValues: Arc<metamodelica::List<(ArcStr, Arc<Entry>)>>, mut conflictFunc: ConflictFunc) -> Result<Arc<Tree>> {
...
pub fn addUpdate(mut tree: Arc<Tree>, mut key: Key, mut r#fn: Arc<dyn ::std::ops::Fn(Option<Arc<Entry>>) -> Result<Arc<Entry>> + 'static>) -> Result<Arc<Tree>> {
    pub type UpdateFn = fn(Option<Arc<Entry>>) -> Result<Value>;
...
pub fn fromList(mut inValues: Arc<metamodelica::List<(ArcStr, Arc<Entry>)>>, mut conflictFunc: ConflictFunc) -> Result<Arc<Tree>> {
...
pub fn update(mut tree: Arc<Tree>, mut key: Key, mut value: Value) -> Arc<Tree> {
    let mut outTree: Arc<Tree> = add(tree.clone(), (key.clone()).clone(), value.clone(), fnptr!(addConflictReplace, Arc<Entry>, Arc<Entry>, ArcStr)).unwrap();
    outTree
}
