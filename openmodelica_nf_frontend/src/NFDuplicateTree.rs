...
pub(crate) fn idExistsInEntry(mut id: Arc<NFLookupTree::Entry::Entry>, mut entry: Arc<Entry>) -> Result<bool> {
    let mut exists: bool;
    exists = NFLookupTree::Entry::isEqual(id.clone(), entry.entry.clone())? || List::any(entry.children.clone(), (std::sync::Arc::new({ let __pe_b0 = id; move |__pe_a1| idExistsInEntry(__pe_b0.clone(), __pe_a1) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<Entry>) -> Result<bool> + 'static>))?;
    Ok(exists)
}
...
pub(crate) fn add(mut inTree: Arc<Tree>, mut inKey: Key, mut inValue: Value, mut conflictFunc: Arc<dyn ::std::ops::Fn(Arc<Entry>, Arc<Entry>, ArcStr) -> Result<Arc<Entry>> + 'static>) -> Result<Arc<Tree>> {
    ...
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        ...
        Deref @ Tree::LEAF { .. } => {
            ...
            if key_comp.clone() == -1 {
                outTree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: Arc::new(Tree::LEAF { key: (inKey).clone(), value: inValue }), right: crate::NFDuplicateTree::Tree::interned_EMPTY() });
            } else if key_comp.clone() == 1 {
                outTree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: crate::NFDuplicateTree::Tree::interned_EMPTY(), right: Arc::new(Tree::LEAF { key: (inKey).clone(), value: inValue }) });
            } else {
...
pub(crate) fn addUpdate(mut tree: Arc<Tree>, mut key: Key, mut r#fn: Arc<dyn ::std::ops::Fn(Option<Arc<Entry>>) -> Result<Arc<Entry>> + 'static>) -> Result<Arc<Tree>> {
    ...
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        ...
        Deref @ Tree::LEAF { .. } => {
            ...
            if key_comp == -1 {
                new_tree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: Arc::new(Tree::LEAF { key: (key).clone(), value: r#fn(None)? }), right: crate::NFDuplicateTree::Tree::interned_EMPTY() });
            } else if key_comp == 1 {
                new_tree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: crate::NFDuplicateTree::Tree::interned_EMPTY(), right: Arc::new(Tree::LEAF { key: (key).clone(), value: r#fn(None)? }) });
            } else {
