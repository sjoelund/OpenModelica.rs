...
pub(crate) fn mapExp(mut eq: Arc<NFEquation>, mut func: Arc<dyn ::std::ops::Fn(Arc<Expression::NFExpression>) -> Result<Arc<Expression::NFExpression>> + 'static>) -> Result<Arc<NFEquation>> {
    ...
    eq = (::match_deref::match_deref! { match &(eq.clone()) {
        Deref @ EQUALITY { .. } => {
            ...
            if (referenceEq(&*(e1.clone()),&*(var_field!((*eq).lhs, NFEquation::EQUALITY).clone())) && referenceEq(&*(e2.clone()),&*(var_field!((*eq).rhs, NFEquation::EQUALITY).clone()))) {eq} else {Arc::new(NFEquation::EQUALITY { lhs: e1, rhs: e2, ty: var_field!((*eq).ty, NFEquation::EQUALITY).clone(), scope: var_field!((*eq).scope, NFEquation::EQUALITY).clone(), source: var_field!((*eq).source, NFEquation::EQUALITY).clone(), scalarizeMode: var_field!((*eq).scalarizeMode, NFEquation::EQUALITY).clone() })}
        },
        Deref @ CONNECT { .. } => {
            let mut e1: Arc<Expression::NFExpression>;
            let mut e2: Arc<Expression::NFExpression>;
            ...
            if (referenceEq(&*(e1.clone()),&*(var_field!((*eq).lhs, NFEquation::CONNECT).clone())) && referenceEq(&*(e2.clone()),&*(var_field!((*eq).rhs, NFEquation::CONNECT).clone()))) {eq} else {Arc::new(NFEquation::CONNECT { lhs: e1, rhs: e2, scope: var_field!((*eq).scope, NFEquation::CONNECT).clone(), source: var_field!((*eq).source, NFEquation::CONNECT).clone() })}
        },
        Deref @ FOR { .. } => {
        ...
        Deref @ ASSERT { .. } => {
            ...
            if (referenceEq(&*(e1.clone()),&*(var_field!((*eq).condition, NFEquation::ASSERT).clone())) && referenceEq(&*(e2.clone()),&*(var_field!((*eq).message, NFEquation::ASSERT).clone())) && referenceEq(&*(e3.clone()),&*(var_field!((*eq).level, NFEquation::ASSERT).clone()))) {eq} else {Arc::new(NFEquation::ASSERT { condition: e1, message: e2, level: e3, scope: var_field!((*eq).scope, NFEquation::ASSERT).clone(), source: var_field!((*eq).source, NFEquation::ASSERT).clone() })}
        },
        Deref @ TERMINATE { .. } => {
            let mut e1: Arc<Expression::NFExpression>;
            e1 = func(var_field!((*eq).message, NFEquation::TERMINATE).clone())?;
            if (referenceEq(&*(e1.clone()),&*(var_field!((*eq).message, NFEquation::TERMINATE).clone()))) {eq} else {Arc::new(NFEquation::TERMINATE { message: e1, scope: var_field!((*eq).scope, NFEquation::TERMINATE).clone(), source: var_field!((*eq).source, NFEquation::TERMINATE).clone() })}
        },
        Deref @ REINIT { .. } => {
            let mut e1: Arc<Expression::NFExpression>;
            let mut e2: Arc<Expression::NFExpression>;
            ...
            if (referenceEq(&*(e1.clone()),&*(var_field!((*eq).cref, NFEquation::REINIT).clone())) && referenceEq(&*(e2.clone()),&*(var_field!((*eq).reinitExp, NFEquation::REINIT).clone()))) {eq} else {Arc::new(NFEquation::REINIT { cref: e1, reinitExp: e2, scope: var_field!((*eq).scope, NFEquation::REINIT).clone(), source: var_field!((*eq).source, NFEquation::REINIT).clone() })}
        },
        Deref @ NORETCALL { .. } => {
            let mut e1: Arc<Expression::NFExpression>;
            e1 = func(var_field!((*eq).exp, NFEquation::NORETCALL).clone())?;
            if (referenceEq(&*(e1.clone()),&*(var_field!((*eq).exp, NFEquation::NORETCALL).clone()))) {eq} else {Arc::new(NFEquation::NORETCALL { exp: e1, scope: var_field!((*eq).scope, NFEquation::NORETCALL).clone(), source: var_field!((*eq).source, NFEquation::NORETCALL).clone() })}
        },
        _ => {
            eq
        },
