...
pub(crate) const fn ONE() -> Unit { Unit::UNIT { s: 0, m: 0, g: 0, A: 0, K: 0, mol: 0, cd: 0, factor: metamodelica::OrderedFloat(1e0_f64) } }
...
pub(crate) fn LU_COMPLEXUNITS() -> Arc<metamodelica::List<(ArcStr, Unit)>> { __LU_COMPLEXUNITS_TLS.with(|__t| __t.clone()) }
...
pub fn getKnownUnitsInverse() -> Result<UnitToStringTable> {
    ...
    let mut s: ArcStr;
    let mut ut: Unit;
    outKnownUnitsInverse = UnorderedMap::new((std::sync::Arc::new(hash) as std::sync::Arc<dyn ::std::ops::Fn(Unit) -> Result<i32> + 'static>), (std::sync::Arc::new(isEqual) as std::sync::Arc<dyn ::std::ops::Fn(Unit, Unit) -> Result<bool> + 'static>), Util::nextPrime((LU_COMPLEXUNITS().clone().len() as i32)));
    for mut unit in &*LU_COMPLEXUNITS().clone() {
        let mut unit = unit.clone();
        (s, ut) = unit.clone();
...
pub(crate) fn realAlmostEqRel(mut a: metamodelica::Real, mut b: metamodelica::Real, mut relTol: metamodelica::Real) -> Result<bool> {
    let mut c: bool;
    c = if (a == b) {true} else {relTol > metamodelica::real_div_checked((a - b).abs(), ((a).abs() + (b).abs()))?};
    Ok(c)
}
...
pub(crate) fn unit2string(mut unit: Unit) -> Result<ArcStr> {
    ...
    outString = ((match unit.clone() {
        Unit::UNIT { .. } => {
            ...
            s = (if (intEq(var_field!(unit.mol, Unit::UNIT).clone(), 0)) {literal!("")} else {s}).clone();
            ...
            s = (if (b && intNe(var_field!(unit.cd, Unit::UNIT).clone(), 0)) {literal!(" * ")} else {literal!("")}).clone();
            r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*r#str); __mm_s.push_str(&*s); ArcStr::from(__mm_s) }).clone();
            ...
            s = (if (intEq(var_field!(unit.cd, Unit::UNIT).clone(), 0)) {literal!("")} else {s}).clone();
            b = b || intNe(var_field!(unit.cd, Unit::UNIT).clone(), 0);
            r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*r#str); __mm_s.push_str(&*s); ArcStr::from(__mm_s) }).clone();
            s = (if (b && intNe(var_field!(unit.m, Unit::UNIT).clone(), 0)) {literal!(" * ")} else {literal!("")}).clone();
            r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*r#str); __mm_s.push_str(&*s); ArcStr::from(__mm_s) }).clone();
            ...
            s = (if (intEq(var_field!(unit.m, Unit::UNIT).clone(), 0)) {literal!("")} else {s}).clone();
            b = b || intNe(var_field!(unit.m, Unit::UNIT).clone(), 0);
            r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*r#str); __mm_s.push_str(&*s); ArcStr::from(__mm_s) }).clone();
            s = (if (b && intNe(var_field!(unit.s, Unit::UNIT).clone(), 0)) {literal!(" * ")} else {literal!("")}).clone();
            r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*r#str); __mm_s.push_str(&*s); ArcStr::from(__mm_s) }).clone();
            ...
            s = (if (intEq(var_field!(unit.s, Unit::UNIT).clone(), 0)) {literal!("")} else {s}).clone();
            b = b || intNe(var_field!(unit.s, Unit::UNIT).clone(), 0);
            r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*r#str); __mm_s.push_str(&*s); ArcStr::from(__mm_s) }).clone();
            s = (if (b && intNe(var_field!(unit.A, Unit::UNIT).clone(), 0)) {literal!(" * ")} else {literal!("")}).clone();
            r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*r#str); __mm_s.push_str(&*s); ArcStr::from(__mm_s) }).clone();
            ...
            s = (if (intEq(var_field!(unit.A, Unit::UNIT).clone(), 0)) {literal!("")} else {s}).clone();
            b = b || intNe(var_field!(unit.A, Unit::UNIT).clone(), 0);
            r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*r#str); __mm_s.push_str(&*s); ArcStr::from(__mm_s) }).clone();
            s = (if (b && intNe(var_field!(unit.K, Unit::UNIT).clone(), 0)) {literal!(" * ")} else {literal!("")}).clone();
            r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*r#str); __mm_s.push_str(&*s); ArcStr::from(__mm_s) }).clone();
            ...
            s = (if (intEq(var_field!(unit.K, Unit::UNIT).clone(), 0)) {literal!("")} else {s}).clone();
            b = b || intNe(var_field!(unit.K, Unit::UNIT).clone(), 0);
            r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*r#str); __mm_s.push_str(&*s); ArcStr::from(__mm_s) }).clone();
            s = (if (b && intNe(var_field!(unit.g, Unit::UNIT).clone(), 0)) {literal!(" * ")} else {literal!("")}).clone();
            r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*r#str); __mm_s.push_str(&*s); ArcStr::from(__mm_s) }).clone();
            ...
            s = (if (intEq(var_field!(unit.g, Unit::UNIT).clone(), 0)) {literal!("")} else {s}).clone();
            b = b || intNe(var_field!(unit.g, Unit::UNIT).clone(), 0);
            r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*r#str); __mm_s.push_str(&*s); ArcStr::from(__mm_s) }).clone();
            r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*r#str); __mm_s.push_str(&*if (b) {literal!("")} else {literal!("1")}); ArcStr::from(__mm_s) }).clone();
            r#str
        },
        Unit::MASTER { .. } => {
...
pub fn unitDiv(mut inUnit1: Unit, mut inUnit2: Unit) -> Result<Unit> {
    let mut outUnit: Unit;
    outUnit = (match (inUnit1.clone(), inUnit2.clone()) {
        (Unit::UNIT { .. }, Unit::UNIT { .. }) => Unit::UNIT { s: var_field!(inUnit1.s, Unit::UNIT).clone() - var_field!(inUnit2.s, Unit::UNIT).clone(), m: var_field!(inUnit1.m, Unit::UNIT).clone() - var_field!(inUnit2.m, Unit::UNIT).clone(), g: var_field!(inUnit1.g, Unit::UNIT).clone() - var_field!(inUnit2.g, Unit::UNIT).clone(), A: var_field!(inUnit1.A, Unit::UNIT).clone() - var_field!(inUnit2.A, Unit::UNIT).clone(), K: var_field!(inUnit1.K, Unit::UNIT).clone() - var_field!(inUnit2.K, Unit::UNIT).clone(), mol: var_field!(inUnit1.mol, Unit::UNIT).clone() - var_field!(inUnit2.mol, Unit::UNIT).clone(), cd: var_field!(inUnit1.cd, Unit::UNIT).clone() - var_field!(inUnit2.cd, Unit::UNIT).clone(), factor: metamodelica::real_div_checked(var_field!(inUnit1.factor, Unit::UNIT).clone(), var_field!(inUnit2.factor, Unit::UNIT).clone())? },
        _ => bail!("match: no arm matched"),
    });
    Ok(outUnit)
}
