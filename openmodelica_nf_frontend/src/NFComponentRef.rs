...
pub(crate) fn mergeSubscripts2(mut subscripts: Arc<metamodelica::List<Arc<Subscript::NFSubscript>>>, mut cref: Arc<NFComponentRef>, mut applyToScope: bool, mut backend: bool, mut reverse: bool) -> Result<(Arc<metamodelica::List<Arc<Subscript::NFSubscript>>>, Arc<NFComponentRef>)> {
    ...
    (subscripts, cref) = (::match_deref::match_deref! { match &(cref.clone()) {
        Deref @ CREF { subscripts: cref_subs, .. } if (applyToScope || var_field!((*cref).origin, NFComponentRef::CREF).clone() == Origin::CREF.clone()) => {
            ...
            (subscripts, Arc::new(NFComponentRef::CREF { node: var_field!((*cref).node, NFComponentRef::CREF).clone(), subscripts: cref_subs.clone(), ty: var_field!((*cref).ty, NFComponentRef::CREF).clone(), origin: var_field!((*cref).origin, NFComponentRef::CREF).clone(), restCref: rest_cref.clone() }))
        },
        _ => {
...
pub(crate) fn subscriptsAllWithWhole(mut cref: Arc<NFComponentRef>, mut accumSubs: Arc<metamodelica::List<Arc<metamodelica::List<Arc<Subscript::NFSubscript>>>>>) -> Result<Arc<metamodelica::List<Arc<metamodelica::List<Arc<Subscript::NFSubscript>>>>>> {
    '__tco: loop {
        ...
        Deref @ CREF { subscripts: Deref @ metamodelica::List::Nil, .. } => {
            ...
            { (cref, accumSubs) = (var_field!((*cref).restCref, NFComponentRef::CREF).clone(), metamodelica::cons(subs.clone(), accumSubs)); continue '__tco; }
        },
        Deref @ CREF { .. } => {
...
pub(crate) fn toString_impl(mut cref: Arc<NFComponentRef>, mut strl: Arc<metamodelica::List<ArcStr>>) -> Result<Arc<metamodelica::List<ArcStr>>> {
    ...
    strl = (::match_deref::match_deref! { match &(cref.clone()) {
        Deref @ CREF { .. } => {
            ...
            toString_impl(var_field!((*cref).restCref, NFComponentRef::CREF).clone(), metamodelica::cons((r#str.clone()).clone(), strl))?
        },
        Deref @ WILD { .. } => {
...
pub(crate) fn scalarize(mut cref: Arc<NFComponentRef>, mut resize: bool) -> Result<Arc<metamodelica::List<Arc<NFComponentRef>>>> {
    ...
    crefs = (::match_deref::match_deref! { match &(cref.clone()) {
        Deref @ CREF { ty: Deref @ Type::ARRAY { .. }, .. } => {
            ...
            dims = Type::arrayDims(var_field!((*cref).ty, NFComponentRef::CREF).clone());
            subs = Subscript::scalarizeList(var_field!((*cref).subscripts, NFComponentRef::CREF).clone(), dims.clone(), resize)?;
...
pub fn sizes(mut cref: Arc<NFComponentRef>, mut withComplex: bool, mut resize: bool, mut s_lst: Arc<metamodelica::List<i32>>) -> Result<Arc<metamodelica::List<i32>>> {
    ...
    s_lst = ({
        ...
        Deref @ CREF { .. } => {
            ...
            s_lst = listAppend(local_lst.clone(), s_lst);
