...
pub fn add(mut inTree: Arc<Tree>, mut inKey: Key, mut inValue: Value, mut conflictFunc: Arc<dyn ::std::ops::Fn(Arc<NFExpression::NFExpression>, Arc<NFExpression::NFExpression>, ArcStr) -> Result<Arc<NFExpression::NFExpression>> + 'static>) -> Result<Arc<Tree>> {
    ...
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        ...
        Deref @ Tree::NODE { key, .. } => {
            let mut value: Value = Arc::new(NFExpression::END);
        ...
        Deref @ Tree::LEAF { .. } => {
            ...
            if key_comp.clone() == -1 {
                outTree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: Arc::new(Tree::LEAF { key: (inKey.clone()).clone(), value: inValue.clone() }), right: Arc::new(crate::NFCallParameterTree::Tree::EMPTY) });
            } else if key_comp.clone() == 1 {
                outTree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: Arc::new(crate::NFCallParameterTree::Tree::EMPTY), right: Arc::new(Tree::LEAF { key: (inKey.clone()).clone(), value: inValue.clone() }) });
            } else {
...
pub fn addList(mut tree: Arc<Tree>, mut inValues: Arc<metamodelica::List<(ArcStr, Arc<NFExpression::NFExpression>)>>, mut conflictFunc: Arc<dyn ::std::ops::Fn(Arc<NFExpression::NFExpression>, Arc<NFExpression::NFExpression>, ArcStr) -> Result<Arc<NFExpression::NFExpression>> + 'static>) -> Result<Arc<Tree>> {
...
pub fn addUpdate(mut tree: Arc<Tree>, mut key: Key, mut r#fn: Arc<dyn ::std::ops::Fn(Option<Arc<NFExpression::NFExpression>>) -> Result<Arc<NFExpression::NFExpression>> + 'static>) -> Result<Arc<Tree>> {
    ...
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        ...
        Deref @ Tree::LEAF { .. } => {
            ...
            if key_comp.clone() == -1 {
                new_tree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: Arc::new(Tree::LEAF { key: (key.clone()).clone(), value: r#fn(None)? }), right: Arc::new(crate::NFCallParameterTree::Tree::EMPTY) });
            } else if key_comp.clone() == 1 {
                new_tree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: Arc::new(crate::NFCallParameterTree::Tree::EMPTY), right: Arc::new(Tree::LEAF { key: (key.clone()).clone(), value: r#fn(None)? }) });
            } else {
...
pub fn fold<FT: Clone + 'static>(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<NFExpression::NFExpression>, FT) -> Result<FT> + 'static>, mut inStartValue: FT) -> FT {
...
pub fn foldCond<FT: Clone + 'static>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<NFExpression::NFExpression>, FT) -> Result<(FT, bool)> + 'static>, mut value: FT) -> FT {
...
pub fn fold_2<FT1: Clone + 'static, FT2: Clone + 'static>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<NFExpression::NFExpression>, FT1, FT2) -> Result<(FT1, FT2)> + 'static>, mut foldArg1: FT1, mut foldArg2: FT2) -> (FT1, FT2) {
...
pub fn forEach(mut tree: Arc<Tree>, mut func: Arc<dyn ::std::ops::Fn(ArcStr, Arc<NFExpression::NFExpression>) -> Result<()> + 'static>) -> Result<()> {
...
pub fn fromList(mut inValues: Arc<metamodelica::List<(ArcStr, Arc<NFExpression::NFExpression>)>>, mut conflictFunc: Arc<dyn ::std::ops::Fn(Arc<NFExpression::NFExpression>, Arc<NFExpression::NFExpression>, ArcStr) -> Result<Arc<NFExpression::NFExpression>> + 'static>) -> Result<Arc<Tree>> {
    let mut tree: Arc<Tree> = Arc::new(crate::NFCallParameterTree::Tree::EMPTY);
    let mut key: Key = arcstr::literal!("");
    let mut value: Value = Arc::new(NFExpression::END);
    for mut t in &*inValues.clone() {
        let mut t = t.clone();
...
pub fn getOpt(mut tree: Arc<Tree>, mut key: Key) -> Option<Arc<NFExpression::NFExpression>> {
    let mut value: Option<Arc<NFExpression::NFExpression>> = None;
    let mut k: Key = arcstr::literal!("");
    k = ((::match_deref::match_deref! { match &(tree.clone()) {
        Deref @ Tree::NODE { .. } => var_field!((*tree).key, Tree::NODE).clone(),
        Deref @ Tree::LEAF { .. } => var_field!((*tree).key, Tree::LEAF).clone(),
...
pub fn join(mut tree: Arc<Tree>, mut treeToJoin: Arc<Tree>, mut conflictFunc: Arc<dyn ::std::ops::Fn(Arc<NFExpression::NFExpression>, Arc<NFExpression::NFExpression>, ArcStr) -> Result<Arc<NFExpression::NFExpression>> + 'static>) -> Result<Arc<Tree>> {
    let mut tree: Arc<Tree> = tree;
...
pub fn listValues(mut tree: Arc<Tree>, mut lst: Arc<metamodelica::List<Arc<NFExpression::NFExpression>>>) -> Arc<metamodelica::List<Arc<NFExpression::NFExpression>>> {
...
pub fn map(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<NFExpression::NFExpression>) -> Result<Arc<NFExpression::NFExpression>> + 'static>) -> Arc<Tree> {
    pub type MapFunc = std::sync::Arc<dyn ::std::ops::Fn(Key, Value) -> Result<Value> + 'static>;

    let mut outTree: Arc<Tree> = inTree.clone();
    outTree = (::match_deref::match_deref! { match &(outTree.clone()) {
        Deref @ Tree::NODE { value, key, .. } => {
            let mut new_value: Value = Arc::new(NFExpression::END);
...
pub fn mapFold<FT: Clone + 'static>(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<NFExpression::NFExpression>, FT) -> Result<(Arc<NFExpression::NFExpression>, FT)> + 'static>, mut inStartValue: FT) -> (Arc<Tree>, FT) {
...
pub fn new() -> Arc<Tree> {
    let mut outTree: Arc<Tree> = Arc::new(crate::NFCallParameterTree::Tree::EMPTY);
    outTree
}
...
fn rotateLeft(mut inNode: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    outNode = (::match_deref::match_deref! { match &(outNode.clone()) {
        ...
        Deref @ Tree::NODE { right: child @ Deref @ Tree::LEAF { .. }, .. } => {
            let mut node: Arc<Tree> = Arc::new(Tree::EMPTY);
            node = setTreeLeftRight(outNode.clone(), var_field!((*outNode).left, Tree::NODE).clone(), Arc::new(crate::NFCallParameterTree::Tree::EMPTY))?;
            setTreeLeftRight(child.clone(), node.clone(), Arc::new(crate::NFCallParameterTree::Tree::EMPTY))?
        },
        _ => {
            inNode.clone()
        },
...
fn rotateRight(mut inNode: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    outNode = (::match_deref::match_deref! { match &(outNode.clone()) {
        ...
        Deref @ Tree::NODE { left: child @ Deref @ Tree::LEAF { .. }, .. } => {
            let mut node: Arc<Tree> = Arc::new(Tree::EMPTY);
            node = setTreeLeftRight(outNode.clone(), Arc::new(crate::NFCallParameterTree::Tree::EMPTY), var_field!((*outNode).right, Tree::NODE).clone())?;
            setTreeLeftRight(child.clone(), Arc::new(crate::NFCallParameterTree::Tree::EMPTY), node.clone())?
        },
        _ => {
            inNode.clone()
        },
...
pub fn toList(mut inTree: Arc<Tree>, mut lst: Arc<metamodelica::List<(ArcStr, Arc<NFExpression::NFExpression>)>>) -> Arc<metamodelica::List<(ArcStr, Arc<NFExpression::NFExpression>)>> {
