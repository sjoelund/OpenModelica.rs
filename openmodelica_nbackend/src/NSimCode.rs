...
#[derive(Clone, Debug, Eq, metamodelica::MetaCmp, metamodelica::ReferenceEq)]
    ...
    pub fn create(mut bdae: Arc<BackendDAE::NBackendDAE>, mut name: Arc<Absyn::Path>, mut fileNamePrefix: ArcStr, mut simSettingsOpt: Option<OldSimCode::SimulationSettings>, mut program: Absyn::Program) -> Result<(Arc<SimCode>, Arc<AvlTreePathFunction::Tree>)> {
        ...
        Deref @ BackendDAE::MAIN { varData: varData @ Deref @ BVariable::VarData::VAR_DATA_SIM { .. }, eqData: eqData @ Deref @ BEquation::EqData::EQ_DATA_SIM { .. }, .. } => {
            let mut funcMap: Arc<UnorderedMap::UnorderedMap<Arc<Absyn::Path>, Arc<Function::Function>>>;
            let mut residual_vars: Arc<VariablePointers::VariablePointers>;
            let mut vars: Arc<SimVars::SimVars>;
            ...
            let mut clockedPartitions: Arc<metamodelica::List<Arc<SimPartition::NSimPartition>>>;
            let mut literals: Arc<metamodelica::List<Arc<Expression::NFExpression>>>;
            let mut externalFunctionIncludes: Arc<metamodelica::List<ArcStr>>;
            let mut generic_loop_calls: Arc<metamodelica::List<Arc<SimGenericCall::NSimGenericCall>>>;
            let mut independent: Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>;
            let mut nominal: Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>;
            let mut min: Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>;
            let mut max: Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>;
            let mut param: Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>;
            let mut no_ret: Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>;
            let mut event_clocks: Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>;
            let mut algorithms: Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>;
            let mut jac_blocks: Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>;
            let mut init: Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>;
            let mut init_0: Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>;
            let mut init_no_ret: Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>;
            let mut start: Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>;
            let mut ode: Arc<metamodelica::List<Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>>>;
            let mut algebraic: Arc<metamodelica::List<Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>>>;
            let mut linearLoops: Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>;
            let mut nonlinearLoops: Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>;
            let mut discreteVars: Arc<metamodelica::List<Arc<ComponentRef::NFComponentRef>>>;
            let mut extObjInfo: Arc<ExtObjInfo::ExtObjInfo>;
            let mut jacobians: Arc<metamodelica::List<Arc<SimJacobian::SimJacobian>>>;
            ...
            let mut daeModeData: Option<Arc<DaeModeData::DaeModeData>>;
            let mut jacA: Arc<SimJacobian::SimJacobian>;
            let mut jacB: Arc<SimJacobian::SimJacobian>;
            let mut jacC: Arc<SimJacobian::SimJacobian>;
            let mut jacD: Arc<SimJacobian::SimJacobian>;
            let mut jacF: Arc<SimJacobian::SimJacobian>;
            let mut jacH: Arc<SimJacobian::SimJacobian>;
            let mut jacAdjoint: Arc<SimJacobian::SimJacobian>;
            let mut jacLfg: Arc<SimJacobian::SimJacobian>;
            let mut jacMrf: Arc<SimJacobian::SimJacobian>;
            let mut jacR0: Arc<SimJacobian::SimJacobian>;
            let mut inlineEquations: Arc<metamodelica::List<Arc<SimStrongComponent::Block::Block>>>;
    ...
    pub fn convert(mut simCode: Arc<SimCode>) -> Result<OldSimCode::SimCode> {
        ...
        __acc.reverse()
    }), recordDecls: simCode.recordDecls.clone(), externalFunctionIncludes: simCode.externalFunctionIncludes.clone(), generic_loop_calls: ({
        ...
        __acc.reverse()
    }), localKnownVars: SimStrongComponent::Block::convertList(simCode.independent.clone())?, allEquations: SimStrongComponent::Block::convertList(simCode.allSim.clone())?, odeEquations: SimStrongComponent::Block::convertListList(simCode.ode.clone())?, algebraicEquations: SimStrongComponent::Block::convertListList(simCode.algebraic.clone())?, clockedPartitions: ({
...
#[derive(Clone, Debug, Eq, metamodelica::MetaCmp, metamodelica::ReferenceEq)]
    ...
    pub(crate) fn convert(mut modelInfo: Arc<ModelInfo>) -> Result<OldSimCode::ModelInfo> {
        ...
        oldModelInfo = OldSimCode::ModelInfo { name: modelInfo.name.clone(), description: (modelInfo.description.clone()).clone(), version: (modelInfo.version.clone()).clone(), author: (modelInfo.author.clone()).clone(), license: (modelInfo.license.clone()).clone(), copyright: (modelInfo.copyright.clone()).clone(), directory: (modelInfo.directory.clone()).clone(), fileName: (modelInfo.fileName.clone()).clone(), varInfo: VarInfo::convert(modelInfo.varInfo.clone()), vars: SimVars::convert(modelInfo.vars.clone())?, functions: modelInfo.functions.clone(), labels: modelInfo.labels.clone(), resourcePaths: modelInfo.resourcePaths.clone(), sortedClasses: modelInfo.sortedClasses.clone(), nClocks: modelInfo.nClocks.clone(), nSubClocks: modelInfo.nSubClocks.clone(), nSpatialDistributions: modelInfo.nSpatialDistributions.clone(), hasLargeLinearEquationSystems: modelInfo.hasLargeLinearEquationSystems.clone(), linearSystems: SimStrongComponent::Block::convertList(modelInfo.linearLoops.clone())?, nonLinearSystems: SimStrongComponent::Block::convertList(modelInfo.nonlinearLoops.clone())?, unitDefinitions: metamodelica::nil() };
        Ok(oldModelInfo)
    }
