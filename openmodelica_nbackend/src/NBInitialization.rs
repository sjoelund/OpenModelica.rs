...
pub(crate) fn createStartEquation(mut var: Pointer::Pointer<Arc<Variable::NFVariable>>, mut ptr_start_vars: Pointer::Pointer<Arc<metamodelica::List<Pointer::Pointer<Arc<Variable::NFVariable>>>>>, mut ptr_start_vars_init: Pointer::Pointer<Arc<metamodelica::List<Pointer::Pointer<Arc<Variable::NFVariable>>>>>, mut ptr_start_eqs: Pointer::Pointer<Arc<metamodelica::List<Pointer::Pointer<Arc<Equation::Equation>>>>>, mut idx: Pointer::Pointer<i32>, mut algorithm_outputs: Arc<UnorderedSet::UnorderedSet<Arc<ComponentRef::NFComponentRef>>>) -> Result<()> {
    if !(UnorderedSet::contains(BVariable::getVarName(var.clone()), algorithm_outputs)?) {
        ...
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
            ...
            start_eq = BEquation::Equation::makeAssignment(Expression::fromCref(name, false)?, start_exp, idx, (arcstr::literal!(BEquation::START_STR)).clone(), crate::NBEquation::Iterator::interned_EMPTY(), BEquation::default(kind, true, None, None))?;
        ...
        Some(e) if (!(Expression::isLiteralXML(e.clone())?)) => {
            ...
            kind = if (BVariable::isContinuous(var.clone(), true)?) {EquationKind::CONTINUOUS.clone()} else {EquationKind::DISCRETE.clone()};
            start_eq = BEquation::Equation::makeAssignment(Expression::fromCref(start_name, false)?, e.clone(), idx, (arcstr::literal!(BEquation::START_STR)).clone(), crate::NBEquation::Iterator::interned_EMPTY(), BEquation::default(kind, true, None, None))?;
            Pointer::update(ptr_start_eqs.clone(), metamodelica::cons(start_eq, Pointer::access(ptr_start_eqs)));
...
pub(crate) fn createStartExpressionSlice(mut exp: Arc<Expression::NFExpression>, mut var_slice: Arc<Slice::NBSlice<Pointer::Pointer<Arc<Variable::NFVariable>>>>, mut var_ptr: Pointer::Pointer<Arc<Variable::NFVariable>>, mut name: Arc<ComponentRef::NFComponentRef>) -> Result<(Arc<Expression::NFExpression>, Pointer::Pointer<Arc<Variable::NFVariable>>, Arc<ComponentRef::NFComponentRef>, Pointer::Pointer<Arc<Variable::NFVariable>>, Arc<ComponentRef::NFComponentRef>, Arc<Iterator::Iterator>)> {
    ...
    (start_exp, iterator) = (::match_deref::match_deref! { match &(exp.clone()) {
        Deref @ Expression::CALL { call: array_constructor @ Deref @ Call::TYPED_ARRAY_CONSTRUCTOR { .. } } => {
            ...
            (Expression::map(var_field!((**array_constructor).exp, Call::NFCall::TYPED_ARRAY_CONSTRUCTOR).clone(), (std::sync::Arc::new({ let __pe_b1 = replacements; move |__pe_a0| Replacements::applySimpleExp(__pe_a0, __pe_b1.clone()) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<Expression::NFExpression>) -> Result<Arc<Expression::NFExpression>> + 'static>))?, iterator)
        },
        _ => {
    ...
    eqn = (::match_deref::match_deref! { match &(eqn.clone()) {
        ...
        Deref @ BEquation::Equation::WHEN_EQUATION { .. } => {
            ...
            if !(stmts.clone().is_empty()) {
                ...
                new_eqn = BEquation::Equation::setResidualVar(new_eqn, BEquation::Equation::getResidualVar(Pointer::create(eqn))?)?;
            } else {
