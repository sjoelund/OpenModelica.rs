...
pub(crate) fn main(mut bdae: Arc<BackendDAE::NBackendDAE>) -> Result<Arc<BackendDAE::NBackendDAE>> {
    ...
    let mut modules: Arc<metamodelica::List<(Module::wrapper, ArcStr)>>;
    let mut clocks: Arc<metamodelica::List<(ArcStr, metamodelica::Real)>>;
    ...
    match '__try0: {
        ...
        Deref @ BackendDAE::MAIN { varData: varData @ Deref @ BVariable::VarData::VAR_DATA_SIM { variables: __esc_variables, initials: __esc_initialVars, .. }, eqData: eqData @ Deref @ BEquation::EqData::EQ_DATA_SIM { equations: __esc_equations, initials: __esc_initialEqs, .. }, .. } => {
            ...
            let mut clonedEqns: Arc<EquationPointers::EquationPointers>;
            let mut clonedVars: Arc<VariablePointers::VariablePointers>;
        ...
        modules = list![((std::sync::Arc::new({ let __pe_b1 = true; move |__pe_a0| BackendDAE::simplify(__pe_a0, __pe_b1.clone()) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::NBackendDAE>) -> Result<Arc<BackendDAE::NBackendDAE>> + 'static>), literal!("Simplify")), ((std::sync::Arc::new({ let __pe_b1 = list![openmodelica_frontend_types::DAE::InlineType::NORM_INLINE, openmodelica_frontend_types::DAE::InlineType::BUILTIN_EARLY_INLINE, openmodelica_frontend_types::DAE::InlineType::EARLY_INLINE, openmodelica_frontend_types::DAE::InlineType::DEFAULT_INLINE]; let __pe_b2 = true; move |__pe_a0| Inline::main(__pe_a0, __pe_b1.clone(), __pe_b2.clone()) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::NBackendDAE>) -> Result<Arc<BackendDAE::NBackendDAE>> + 'static>), literal!("Inline")), ((std::sync::Arc::new({ let __pe_b1 = BPartition::Kind::INI.clone(); move |__pe_a0| Partitioning::main(__pe_a0, __pe_b1.clone()) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::NBackendDAE>) -> Result<Arc<BackendDAE::NBackendDAE>> + 'static>), literal!("Partitioning")), ((std::sync::Arc::new(cleanup) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::NBackendDAE>) -> Result<Arc<BackendDAE::NBackendDAE>> + 'static>), literal!("Cleanup")), ((std::sync::Arc::new({ let __pe_b1 = BPartition::Kind::INI.clone(); move |__pe_a0| Causalize::main(__pe_a0, __pe_b1.clone()) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::NBackendDAE>) -> Result<Arc<BackendDAE::NBackendDAE>> + 'static>), literal!("Causalize")), ((std::sync::Arc::new({ let __pe_b1 = BPartition::Kind::INI.clone(); move |__pe_a0| Tearing::main(__pe_a0, __pe_b1.clone()) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<BackendDAE::NBackendDAE>) -> Result<Arc<BackendDAE::NBackendDAE>> + 'static>), literal!("Tearing"))];
        ...
        if unwrap_break_err!(Flags::isSet(Flags::DUMP_BACKEND_CLOCKS.clone()), '__try0) {
            if !(clocks.clone().is_empty()) {
                metamodelica::print((unwrap_break_err!(StringUtil::headline_4((literal!("Initialization Backend Clocks:")).clone()), '__try0)).clone());
...
pub(crate) fn createStartEquations(mut states: Arc<VariablePointers::VariablePointers>, mut variables: Arc<VariablePointers::VariablePointers>, mut initialVars: Arc<VariablePointers::VariablePointers>, mut equations: Arc<EquationPointers::EquationPointers>, mut initialEqs: Arc<EquationPointers::EquationPointers>, mut idx: Pointer::Pointer<i32>, mut algorithm_outputs: Arc<UnorderedSet::UnorderedSet<Arc<ComponentRef::NFComponentRef>>>, mut r#str: ArcStr) -> Result<(Arc<VariablePointers::VariablePointers>, Arc<VariablePointers::VariablePointers>, Arc<EquationPointers::EquationPointers>, Arc<EquationPointers::EquationPointers>)> {
    ...
    let mut start_eqs: Arc<metamodelica::List<Pointer::Pointer<Arc<Equation::Equation>>>>;
    BVariable::VariablePointers::mapPtr(states, (std::sync::Arc::new({ let __pe_b1 = ptr_start_vars.clone(); let __pe_b2 = ptr_start_vars_init.clone(); let __pe_b3 = ptr_start_eqs.clone(); let __pe_b4 = idx; let __pe_b5 = algorithm_outputs; move |__pe_a0| createStartEquation(__pe_a0, __pe_b1.clone(), __pe_b2.clone(), __pe_b3.clone(), __pe_b4.clone(), __pe_b5.clone()) }) as std::sync::Arc<dyn ::std::ops::Fn(Pointer::Pointer<Arc<Variable::NFVariable>>) -> Result<()> + 'static>))?;
    ...
    if !(UnorderedSet::contains(BVariable::getVarName(var.clone()), algorithm_outputs)?) {
        ...
        Deref @ Variable::VARIABLE { .. } if (BVariable::isArray(var.clone())) => {
            if BVariable::isFixed(var.clone())? {
                createStartEquationSlice(Arc::new(Slice::NBSlice { t: var.clone(), indices: metamodelica::nil() }), ptr_start_vars, ptr_start_eqs, idx, BVariable::isFixed(var.clone())?)?;
            } else {
                createStartEquationSlice(Arc::new(Slice::NBSlice { t: var.clone(), indices: metamodelica::nil() }), ptr_start_vars_init, ptr_start_eqs, idx, BVariable::isFixed(var.clone())?)?;
            }
            ()
        },
        ...
        _ => {
            ...
            Pointer::update(ptr_start_vars.clone(), metamodelica::cons(start_var.clone(), Pointer::access(ptr_start_vars)));
        ...
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
            ...
            start_eq = BEquation::Equation::makeAssignment(Expression::fromCref(name.clone(), false)?, start_exp.clone(), idx, (arcstr::literal!(BEquation::START_STR)).clone(), crate::NBEquation::Iterator::interned_EMPTY(), BEquation::default(kind.clone(), true, None, None))?;
            Pointer::update(ptr_start_eqs.clone(), metamodelica::cons(start_eq.clone(), Pointer::access(ptr_start_eqs)));
            ()
        },
        ...
        Some(e) if (!(Expression::isLiteralXML(e.clone())?)) => {
            ...
            kind = if (BVariable::isContinuous(var.clone(), true)?) {EquationKind::CONTINUOUS.clone()} else {EquationKind::DISCRETE.clone()};
            start_eq = BEquation::Equation::makeAssignment(Expression::fromCref(start_name.clone(), false)?, e.clone(), idx, (arcstr::literal!(BEquation::START_STR)).clone(), crate::NBEquation::Iterator::interned_EMPTY(), BEquation::default(kind.clone(), true, None, None))?;
            Pointer::update(ptr_start_eqs.clone(), metamodelica::cons(start_eq.clone(), Pointer::access(ptr_start_eqs)));
            Pointer::update(ptr_start_vars_init.clone(), metamodelica::cons(start_var.clone(), Pointer::access(ptr_start_vars_init)));
            ()
        },
        _ => {
            ()
...
pub(crate) fn createWhenReplacementEquation(mut tpl: (Arc<ComponentRef::NFComponentRef>, Arc<Iterator::Iterator>), mut ptr_start_eqs: Pointer::Pointer<Arc<metamodelica::List<Pointer::Pointer<Arc<Equation::Equation>>>>>, mut idx: Pointer::Pointer<i32>) -> Result<()> {
    ...
    let mut var_pre: Option<Pointer::Pointer<Arc<Variable::NFVariable>>>;
    let mut pre: Arc<ComponentRef::NFComponentRef>;
    let mut kind: EquationKind;
    ...
    if isSome(var_pre.clone()) {
        ...
        pre = ComponentRef::copySubscripts(cref.clone(), pre)?;
        kind = if (BVariable::isContinuous(var_ptr, true)?) {EquationKind::CONTINUOUS.clone()} else {EquationKind::DISCRETE.clone()};
        eq = BEquation::Equation::makeAssignment(Expression::fromCref(cref, true)?, Expression::fromCref(pre, true)?, idx, (arcstr::literal!(BEquation::START_STR)).clone(), iter, BEquation::default(kind, true, None, None))?;
        Pointer::update(ptr_start_eqs.clone(), metamodelica::cons(eq, Pointer::access(ptr_start_eqs)));
    } else {
...
pub(crate) fn createStartVar(mut var_ptr: Pointer::Pointer<Arc<Variable::NFVariable>>, mut name: Arc<ComponentRef::NFComponentRef>, mut subscripts: Arc<metamodelica::List<Arc<Subscript::NFSubscript>>>) -> Result<(Pointer::Pointer<Arc<Variable::NFVariable>>, Arc<ComponentRef::NFComponentRef>, Pointer::Pointer<Arc<Variable::NFVariable>>, Arc<ComponentRef::NFComponentRef>)> {
    ...
    start_var = (match BVariable::getParent(var_ptr.clone()) {
        ...
        _ => {
            ...
            start_var = BVariable::setParent(start_var, start_parent.clone());
            start_var
        },
        _ => {
            start_var
...
pub(crate) fn createParameterEquations(mut parameters: Arc<VariablePointers::VariablePointers>, mut equations: Arc<EquationPointers::EquationPointers>, mut initialEqs: Arc<EquationPointers::EquationPointers>, mut initialVars: Arc<VariablePointers::VariablePointers>, mut new_iters: Arc<UnorderedSet::UnorderedSet<Pointer::Pointer<Arc<Variable::NFVariable>>>>, mut idx: Pointer::Pointer<i32>, mut r#str: ArcStr) -> Result<(Arc<EquationPointers::EquationPointers>, Arc<EquationPointers::EquationPointers>, Arc<VariablePointers::VariablePointers>)> {
    ...
    equations = BEquation::EquationPointers::addList(parameter_eqs.clone(), equations)?;
    initialEqs = BEquation::EquationPointers::addList(parameter_eqs.clone(), initialEqs)?;
...
pub(crate) fn createParameterEquation(mut var: Pointer::Pointer<Arc<Variable::NFVariable>>, mut new_iters: Arc<UnorderedSet::UnorderedSet<Pointer::Pointer<Arc<Variable::NFVariable>>>>, mut idx: Pointer::Pointer<i32>, mut parameter_eqs: Arc<metamodelica::List<Pointer::Pointer<Arc<Equation::Equation>>>>, mut initial_param_vars: Arc<metamodelica::List<Pointer::Pointer<Arc<Variable::NFVariable>>>>) -> Result<(Arc<metamodelica::List<Pointer::Pointer<Arc<Equation::Equation>>>>, Arc<metamodelica::List<Pointer::Pointer<Arc<Variable::NFVariable>>>>)> {
    ...
    if BVariable::isKnownRecord(var.clone()) {
        if !(BVariable::hasEvaluableBinding(var.clone())?) && (BVariable::isBound(var.clone()) || BVariable::hasStartAttr(var.clone())) {
            ...
            parameter_eqs = metamodelica::cons(BEquation::Equation::generateBindingEquation(var, idx, true, new_iters)?, parameter_eqs);
        } else {
            for mut c_var in &*BVariable::getRecordChildren(var) {
...
pub(crate) fn createStartEquationSlice(mut var_slice: Arc<Slice::NBSlice<Pointer::Pointer<Arc<Variable::NFVariable>>>>, mut ptr_start_vars: Pointer::Pointer<Arc<metamodelica::List<Pointer::Pointer<Arc<Variable::NFVariable>>>>>, mut ptr_start_eqs: Pointer::Pointer<Arc<metamodelica::List<Pointer::Pointer<Arc<Equation::Equation>>>>>, mut idx: Pointer::Pointer<i32>, mut fixed: bool) -> Result<()> {
    ...
    let mut sliced_eqn: Arc<metamodelica::List<Pointer::Pointer<Arc<Equation::Equation>>>>;
    ...
    if fixed {
        ...
        start_eq = Some(BEquation::Equation::makeAssignment(Expression::fromCref(name, true)?, start_exp, idx, (arcstr::literal!(BEquation::START_STR)).clone(), iterator, BEquation::default(kind, true, None, None))?);
    } else {
    ...
    if isSome(start_eq.clone()) {
        if !(var_slice.indices.clone().is_empty()) {
            ...
            Pointer::update(ptr_start_eqs.clone(), listAppend(Pointer::access(ptr_start_eqs), sliced_eqn));
        } else {
            Pointer::update(ptr_start_eqs.clone(), metamodelica::cons(Util::getOption(start_eq)?, Pointer::access(ptr_start_eqs)));
        }
    }
    Ok(())
}
...
pub(crate) fn createStartExpressionSlice(mut exp: Arc<Expression::NFExpression>, mut var_slice: Arc<Slice::NBSlice<Pointer::Pointer<Arc<Variable::NFVariable>>>>, mut var_ptr: Pointer::Pointer<Arc<Variable::NFVariable>>, mut name: Arc<ComponentRef::NFComponentRef>) -> Result<(Arc<Expression::NFExpression>, Pointer::Pointer<Arc<Variable::NFVariable>>, Arc<ComponentRef::NFComponentRef>, Pointer::Pointer<Arc<Variable::NFVariable>>, Arc<ComponentRef::NFComponentRef>, Arc<Iterator::Iterator>)> {
    ...
    (start_exp, iterator) = (::match_deref::match_deref! { match &(exp.clone()) {
        Deref @ Expression::CALL { call: array_constructor @ Deref @ Call::TYPED_ARRAY_CONSTRUCTOR { .. } } => {
            ...
            let mut old_iter: Arc<InstNode::InstNode>;
            let mut new_iter: Arc<ComponentRef::NFComponentRef>;
            (var_ptr, name, start_var, start_cref, _, frames, iterator) = createIteratedStartCref(var_ptr, name, (var_field!((**array_constructor).iters, Call::NFCall::TYPED_ARRAY_CONSTRUCTOR).clone().len() as i32))?;
            ...
            (Expression::map(var_field!((**array_constructor).exp, Call::NFCall::TYPED_ARRAY_CONSTRUCTOR).clone(), (std::sync::Arc::new({ let __pe_b1 = replacements.clone(); move |__pe_a0| Replacements::applySimpleExp(__pe_a0, __pe_b1.clone()) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<Expression::NFExpression>) -> Result<Arc<Expression::NFExpression>> + 'static>))?, iterator)
        },
        _ => {
            ...
            if Slice::isFull(var_slice) {
                ...
                start_exp = Expression::applySubscripts(subscripts.clone(), exp, true)?;
            }
            (start_exp, iterator)
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
...
fn createIteratedStartCref(mut var_ptr: Pointer::Pointer<Arc<Variable::NFVariable>>, mut name: Arc<ComponentRef::NFComponentRef>, mut num_dim: i32) -> Result<(Pointer::Pointer<Arc<Variable::NFVariable>>, Arc<ComponentRef::NFComponentRef>, Pointer::Pointer<Arc<Variable::NFVariable>>, Arc<ComponentRef::NFComponentRef>, Arc<metamodelica::List<Arc<Subscript::NFSubscript>>>, Arc<metamodelica::List<(Arc<ComponentRef::NFComponentRef>, Arc<Expression::NFExpression>, Option<Arc<Iterator::Iterator>>)>>, Arc<Iterator::Iterator>)> {
    ...
    let mut iterators: Arc<metamodelica::List<Arc<InstNode::InstNode>>>;
    let mut ranges: Arc<metamodelica::List<Arc<Expression::NFExpression>>>;
    let mut iter_crefs: Arc<metamodelica::List<Arc<ComponentRef::NFComponentRef>>>;
    dims = Type::arrayDims(ComponentRef::getSubscriptedType(name.clone(), false)?);
    dims = if (num_dim == 0) {dims} else {List::firstN(dims, num_dim)?};
    (iterators, ranges, subscripts) = Flatten::makeIterators(name.clone(), dims)?;
    iter_crefs = ({
    ...
    frames = List::zip3(iter_crefs.clone(), ranges, List::fill(None, (iter_crefs.len() as i32)));
...
pub(crate) fn createPreEquation(mut var_ptr: Pointer::Pointer<Arc<Variable::NFVariable>>, mut ptr_pre_eqs: Pointer::Pointer<Arc<metamodelica::List<Pointer::Pointer<Arc<Equation::Equation>>>>>, mut idx: Pointer::Pointer<i32>) -> Result<()> {
    ...
    if !(BVariable::isPrevious(var_ptr.clone())) {
        ...
        if isSome(pre.clone()) {
            kind = if (BVariable::isContinuous(var_ptr.clone(), true)?) {EquationKind::CONTINUOUS.clone()} else {EquationKind::DISCRETE.clone()};
            pre_eq = BEquation::Equation::makeAssignment(Expression::fromCref(BVariable::getVarName(var_ptr), false)?, Expression::fromCref(BVariable::getVarName(Util::getOption(pre)?), false)?, idx, (arcstr::literal!(BEquation::PRE_STR)).clone(), crate::NBEquation::Iterator::interned_EMPTY(), BEquation::default(kind, true, None, None))?;
            Pointer::update(ptr_pre_eqs.clone(), metamodelica::cons(pre_eq, Pointer::access(ptr_pre_eqs)));
        }
    }
    Ok(())
}
...
pub(crate) fn createPreEquationSlice(mut var_slice: Arc<Slice::NBSlice<Pointer::Pointer<Arc<Variable::NFVariable>>>>, mut ptr_pre_eqs: Pointer::Pointer<Arc<metamodelica::List<Pointer::Pointer<Arc<Equation::Equation>>>>>, mut idx: Pointer::Pointer<i32>) -> Result<()> {
    ...
    let mut pre: Option<Pointer::Pointer<Arc<Variable::NFVariable>>>;
    let mut name: Arc<ComponentRef::NFComponentRef>;
    let mut pre_name: Arc<ComponentRef::NFComponentRef>;
    let mut dims: Arc<metamodelica::List<Arc<Dimension::NFDimension>>>;
    let mut iterators: Arc<metamodelica::List<Arc<InstNode::InstNode>>>;
    let mut ranges: Arc<metamodelica::List<Arc<Expression::NFExpression>>>;
    let mut subscripts: Arc<metamodelica::List<Arc<Subscript::NFSubscript>>>;
    ...
    if !(BVariable::isPrevious(var_ptr.clone())) {
        ...
        __acc.reverse()
    }), ranges.clone(), List::fill(None, (ranges.len() as i32)));
            ...
            kind = if (BVariable::isContinuous(var_ptr, true)?) {EquationKind::CONTINUOUS.clone()} else {EquationKind::DISCRETE.clone()};
            pre_eq = BEquation::Equation::makeAssignment(Expression::fromCref(name, true)?, Expression::fromCref(pre_name, false)?, idx, (arcstr::literal!(BEquation::PRE_STR)).clone(), BEquation::Iterator::fromFrames(frames), BEquation::default(kind, true, None, None))?;
            if !(var_slice.indices.clone().is_empty()) {
                (sliced_eqn, _) = BEquation::Equation::slice(pre_eq, var_slice.indices.clone())?;
                Pointer::update(ptr_pre_eqs.clone(), listAppend(Pointer::access(ptr_pre_eqs), sliced_eqn));
            } else {
                Pointer::update(ptr_pre_eqs.clone(), metamodelica::cons(pre_eq, Pointer::access(ptr_pre_eqs)));
            }
        }
...
pub(crate) fn cleanup(mut bdae: Arc<BackendDAE::NBackendDAE>) -> Result<Arc<BackendDAE::NBackendDAE>> {
    ...
    bdae = (::match_deref::match_deref! { match &(bdae.clone()) {
        ...
        for mut par in (var_field!((*bdae).ode, BackendDAE::NBackendDAE::MAIN).clone()).into_iter().cloned() {
            let __x = BPartition::Partition::mapEqn(par.clone(), (std::sync::Arc::new({ let __pe_b1 = BPartition::Partition::getKind(par.clone()); move |__pe_a0| cleanupInitialCall(__pe_a0, __pe_b1.clone()) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<Equation::Equation>) -> Result<Arc<Equation::Equation>> + 'static>))?;
            __acc = cons(__x, __acc);
        }
        __acc.reverse()
    }),
        ...
        let mut __acc: Arc<metamodelica::List<Arc<Partition::Partition>>> = metamodelica::nil();
        for mut par in (var_field!((*bdae).ode, BackendDAE::NBackendDAE::MAIN).clone()).into_iter().cloned() {
            let __x = BPartition::Partition::mapExp(par.clone(), (std::sync::Arc::new({ let __pe_b1 = BPartition::Partition::getKind(par.clone()); move |__pe_a0| cleanupHomotopy(__pe_a0, __pe_b1.clone()) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<Expression::NFExpression>) -> Result<Arc<Expression::NFExpression>> + 'static>))?;
            __acc = cons(__x, __acc);
        }
        __acc.reverse()
    }),
        ...
        for mut par in (var_field!((*bdae).init, BackendDAE::NBackendDAE::MAIN).clone()).into_iter().cloned() {
            let __x = BPartition::Partition::mapExp(par.clone(), (std::sync::Arc::new({ let __pe_b1 = hasHom.clone(); move |__pe_a0| containsLambda0(__pe_a0, __pe_b1.clone()) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<Expression::NFExpression>) -> Result<Arc<Expression::NFExpression>> + 'static>))?;
            __acc = cons(__x, __acc);
        }
        __acc.reverse()
    }));
...
pub(crate) fn cleanupInitialCall(mut eq: Arc<Equation::Equation>, mut kind: BPartition::Kind) -> Result<Arc<Equation::Equation>> {
    fn cleanupInitialCallExp(mut exp: Arc<Expression::NFExpression>, mut kind: BPartition::Kind, mut simplify: Pointer::Pointer<bool>) -> Result<Arc<Expression::NFExpression>> {
        ...
        if Expression::isCallNamed(exp.clone(), (literal!("initial")).clone())? {
            exp = Arc::new(Expression::NFExpression::BOOLEAN { value: kind == BPartition::Kind::INI.clone() || kind == BPartition::Kind::INI_0.clone() });
            ...
            exp = Arc::new(Expression::NFExpression::BOOLEAN { value: kind == BPartition::Kind::INI_0.clone() });
            Pointer::update(simplify, true);
        }
        Ok(exp)
    }
...
pub(crate) fn cleanupHomotopy(mut exp: Arc<Expression::NFExpression>, mut kind: BPartition::Kind) -> Result<Arc<Expression::NFExpression>> {
    ...
    exp = (::match_deref::match_deref! { match &(exp.clone()) {
        Deref @ Expression::CALL { .. } if (Call::isNamed(var_field!((*exp).call, Expression::NFExpression::CALL).clone(), (literal!("homotopy")).clone())?) => (match kind {
...
pub(crate) fn minimizeHomotopySystem(mut bdae: Arc<BackendDAE::NBackendDAE>) -> Result<Arc<BackendDAE::NBackendDAE>> {
    ...
    bdae = (::match_deref::match_deref! { match &(bdae.clone()) {
        ...
        let mut __acc: Arc<metamodelica::List<Arc<Partition::Partition>>> = metamodelica::nil();
        for mut par in (var_field!((*bdae).init, BackendDAE::NBackendDAE::MAIN).clone()).into_iter().cloned() {
            let __x = BPartition::Partition::mapStrongComponents(par.clone(), (std::sync::Arc::new({ let __pe_b1 = true; move |__pe_a0| Ok(StrongComponent::setHomotopy(__pe_a0, __pe_b1.clone())) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<StrongComponent::NBStrongComponent>) -> Result<Arc<StrongComponent::NBStrongComponent>> + 'static>))?;
            __acc = cons(__x, __acc);
        }
        __acc.reverse()
    }));
...
pub(crate) fn removeWhenEquation(mut eqn: Arc<Equation::Equation>, mut iter: Arc<Iterator::Iterator>, mut cref_map: Arc<UnorderedMap::UnorderedMap<Arc<ComponentRef::NFComponentRef>, Arc<Iterator::Iterator>>>) -> Result<Arc<Equation::Equation>> {
    ...
    eqn = (::match_deref::match_deref! { match &(eqn.clone()) {
        ...
        __acc.reverse()
            ...
            if (List::all(var_field!((*eqn).body, Equation::Equation::FOR_EQUATION).clone(), (std::sync::Arc::new(fnptr!(BEquation::Equation::isDummy, Arc<Equation::Equation>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<Equation::Equation>) -> Result<bool> + 'static>))?) {crate::NBEquation::Equation::interned_DUMMY_EQUATION()} else {eqn}
        },
        Deref @ BEquation::Equation::WHEN_EQUATION { .. } => {
            ...
            if !(stmts.clone().is_empty()) {
                ...
                new_eqn = BEquation::Equation::setResidualVar(new_eqn.clone(), BEquation::Equation::getResidualVar(Pointer::create(eqn))?)?;
            } else {
        ...
        Deref @ BEquation::Equation::IF_EQUATION { .. } => {
            assign_variant_field!(eqn => Equation::Equation::IF_EQUATION; body = removeWhenEquationIfBody(var_field!((*eqn).body, Equation::Equation::IF_EQUATION).clone(), iter, cref_map)?);
            ...
            if (var_field!((*eqn).size, Equation::Equation::IF_EQUATION).clone() > 0) {eqn} else {crate::NBEquation::Equation::interned_DUMMY_EQUATION()}
        },
...
pub(crate) fn removeWhenEquationIfBody(mut body: Arc<IfEquationBody::IfEquationBody>, mut iter: Arc<Iterator::Iterator>, mut cref_map: Arc<UnorderedMap::UnorderedMap<Arc<ComponentRef::NFComponentRef>, Arc<Iterator::Iterator>>>) -> Result<Arc<IfEquationBody::IfEquationBody>> {
    ...
    assign_field!(
        ...
        body.else_if = Util::applyOption(body.else_if.clone(), (std::sync::Arc::new({ let __pe_b1 = iter; let __pe_b2 = cref_map; move |__pe_a0| removeWhenEquationIfBody(__pe_a0, __pe_b1.clone(), __pe_b2.clone()) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<IfEquationBody::IfEquationBody>) -> Result<Arc<IfEquationBody::IfEquationBody>> + 'static>))?
    );
    Ok(body)
}
...
pub(crate) fn removeConditionEquation(mut stmt: Arc<Statement::NFStatement>, mut condition_set: Arc<UnorderedSet::UnorderedSet<Arc<Expression::NFExpression>>>, mut tail_stmts_ptr: Pointer::Pointer<Arc<metamodelica::List<Arc<Statement::NFStatement>>>>) -> Result<Arc<metamodelica::List<Arc<Statement::NFStatement>>>> {
    ...
    out_stmts = (::match_deref::match_deref! { match &(stmt.clone()) {
        Deref @ Statement::ASSIGNMENT { .. } if (UnorderedSet::contains(var_field!((*stmt).lhs, Statement::NFStatement::ASSIGNMENT).clone(), condition_set.clone())?) => {
            ...
            let mut post_cref: Arc<ComponentRef::NFComponentRef>;
            let mut tail_stmts: Arc<metamodelica::List<Arc<Statement::NFStatement>>>;
            ...
            if UnorderedSet::isEmpty(pre_set.clone()) {
                ...
                tail_stmts = metamodelica::cons(stmt.clone(), Pointer::access(tail_stmts_ptr.clone()));
                for mut pre_cref in &*UnorderedSet::toList(pre_set.clone()) {
                    ...
                    tail_stmts = metamodelica::cons(Arc::new(Statement::NFStatement::ASSIGNMENT { lhs: Expression::fromCref(pre_cref.clone(), false)?, rhs: Expression::fromCref(post_cref.clone(), false)?, ty: ComponentRef::getSubscriptedType(pre_cref.clone(), false)?, source: DAE::emptyElementSource().clone() }), tail_stmts.clone());
                }
                Pointer::update(tail_stmts_ptr, tail_stmts.clone());
            }
            out_stmts
        },
        _ => {
...
pub(crate) fn findPreVars(mut exp: Arc<Expression::NFExpression>, mut pre_set: Arc<UnorderedSet::UnorderedSet<Arc<ComponentRef::NFComponentRef>>>) -> Result<Arc<Expression::NFExpression>> {
    ...
    let () = (::match_deref::match_deref! { match &(exp.clone()) {
        Deref @ Expression::CREF { .. } if (BVariable::isPrevious(BVariable::getVarPointer(var_field!((*exp).cref, Expression::NFExpression::CREF).clone(), metamodelica::sourceInfo!("NBackEnd/Modules/1_Main/NBInitialization.mo"))?)) => {
            UnorderedSet::add(var_field!((*exp).cref, Expression::NFExpression::CREF).clone(), pre_set)?;
            ()
        },
        _ => (),
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
...
pub(crate) fn collectAlgorithmOutputs(mut eqn: Arc<Equation::Equation>, mut outputs: Arc<UnorderedSet::UnorderedSet<Arc<ComponentRef::NFComponentRef>>>) -> Result<Arc<Equation::Equation>> {
    ...
    let () = (::match_deref::match_deref! { match &(eqn.clone()) {
        Deref @ BEquation::Equation::ALGORITHM { alg, .. } => {
            let mut out_crefs: Arc<metamodelica::List<Arc<ComponentRef::NFComponentRef>>>;
