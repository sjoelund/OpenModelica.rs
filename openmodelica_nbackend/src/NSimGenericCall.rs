    ...
    call = (::match_deref::match_deref! { match &(call.clone()) {
        Deref @ SINGLE_GENERIC_CALL { .. } => {
            assign_variant_field!(call => NSimGenericCall::SINGLE_GENERIC_CALL;
                lhs = func(var_field!((*call).lhs, NSimGenericCall::SINGLE_GENERIC_CALL).clone())?,
...
pub(crate) fn fromIdentifier(mut ident_tpl: (Arc<Identifier::Identifier>, i32)) -> Result<Arc<NSimGenericCall>> {
    ...
    let (__pa0, __pa1, __pa2) = ::match_deref::match_deref! { match &(ident_tpl) {
...
pub(crate) fn convert(mut call: Arc<NSimGenericCall>) -> Result<OldSimCode::SimGenericCall> {
    ...
    old_call = (::match_deref::match_deref! { match &(call.clone()) {
        ...
        _ => {
            Error::addMessage(Error::INTERNAL_ERROR.clone(), list![({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("NSimGenericCall.convert")); __mm_s.push_str(&*literal!(" failed for incorrect call: ")); __mm_s.push_str(&*toString(call)?); ArcStr::from(__mm_s) }).clone()])?;
            bail!("fail")
        },
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
pub mod SimIterator {
    ...
    pub(crate) fn toString(mut iter: Arc<SimIterator>) -> Result<ArcStr> {
        ...
        __acc.reverse()
    }), (std::sync::Arc::new(ComponentRef::toString) as std::sync::Arc<dyn ::std::ops::Fn(Arc<ComponentRef::NFComponentRef>) -> Result<ArcStr> + 'static>), (literal!("")).clone(), (literal!("(")).clone(), (literal!(", ")).clone(), (literal!(")")).clone(), false, 0)?;
            Ok(r#str)
        }
    ...
    pub(crate) fn fromIterator(mut iter: Arc<Iterator::Iterator>) -> Result<Arc<metamodelica::List<Arc<SimIterator>>>> {
        ...
        Deref @ Expression::RANGE { .. } => {
            ...
            metamodelica::cons(Arc::new(SimIterator::SIM_ITERATOR_RANGE { name: name.clone(), start: var_field!((*range).start, Expression::NFExpression::RANGE).clone(), step: step.clone(), stop: var_field!((*range).stop, Expression::NFExpression::RANGE).clone(), size: size.clone(), sub_iter: sub_iter.clone() }), sim_iter.clone())
        },
        ...
        __acc.reverse()
            ...
            sub_iter = if (isSome(map.clone())) {subIterators(Util::getOption(map.clone())?)?} else {metamodelica::nil()};
            metamodelica::cons(Arc::new(SimIterator::SIM_ITERATOR_LIST { name: name.clone(), lst: lst.clone(), size: (lst.clone().len() as i32), sub_iter: sub_iter.clone() }), sim_iter.clone())
        },
        _ => {
    ...
    pub(crate) fn subIterators(mut iter: Arc<Iterator::Iterator>) -> Result<Arc<metamodelica::List<(Arc<ComponentRef::NFComponentRef>, metamodelica::Array<Arc<Expression::NFExpression>>)>>> {
        ...
        Deref @ Expression::ARRAY { .. } => metamodelica::cons((name.clone(), var_field!((*range).elements, Expression::NFExpression::ARRAY).clone()), sub_iter.clone()),
        _ => {
...
pub mod SimBranch {
    ...
    pub(crate) fn toString(mut branch: Arc<SimBranch>) -> Result<ArcStr> {
        ...
        Deref @ SIM_BRANCH_STMT { .. } => {
            ...
            r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*r#str); __mm_s.push_str(&*List::toString(var_field!((*branch).body, SimBranch::SIM_BRANCH_STMT).clone(), (std::sync::Arc::new({ let __pe_b1 = (literal!("")).clone(); move |__pe_a0| Statement::toString(__pe_a0, __pe_b1.clone()) }) as std::sync::Arc<dyn ::std::ops::Fn(Arc<Statement::NFStatement>) -> Result<ArcStr> + 'static>), (literal!("\t  ")).clone(), (literal!("\t  ")).clone(), (literal!("\n")).clone(), (literal!("")).clone(), true, 0)?); ArcStr::from(__mm_s) }).clone();
            r#str
        },
    ...
    pub(crate) fn fromIfBody(mut if_body: Arc<IfEquationBody::IfEquationBody>) -> Result<Arc<metamodelica::List<Arc<SimBranch>>>> {
        ...
        for mut eqn in &*if_body.then_eqns.clone().reverse() {
            ...
            body = metamodelica::cons((Util::getOption(Equation::getLHS(Pointer::access(eqn.clone()))?)?, Util::getOption(Equation::getRHS(Pointer::access(eqn.clone()))?)?), body.clone());
        }
        branch = Arc::new(SimBranch::SIM_BRANCH { condition: if_body.condition.clone(), body: body });
    ...
    pub(crate) fn convert(mut branch: Arc<SimBranch>) -> Result<OldSimCode::SimBranch> {
        ...
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
            ...
            OldSimCode::SimBranch::SIM_BRANCH { condition: old_condition, body: old_body }
        },
        Deref @ SIM_BRANCH_STMT { .. } => {
        ...
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
            ...
            OldSimCode::SimBranch::SIM_BRANCH_STMT { condition: old_condition, body: ConvertDAE::convertStatements(var_field!((*branch).body, SimBranch::SIM_BRANCH_STMT).clone())? }
        },
        _ => bail!("fail"),
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
