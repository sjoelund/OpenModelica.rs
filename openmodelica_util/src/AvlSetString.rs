...
pub fn add(mut inTree: Arc<Tree>, mut inKey: Key) -> Result<Arc<Tree>> {
    ...
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        ...
        Deref @ Tree::LEAF { key } => {
            ...
            key_comp = keyCompare((inKey.clone()).clone(), (key.clone()).clone());
            if key_comp.clone() == -1 {
                outTree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), height: 2, left: Arc::new(Tree::LEAF { key: (inKey).clone() }), right: crate::AvlSetString::Tree::interned_EMPTY() });
            } else if key_comp.clone() == 1 {
                outTree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), height: 2, left: crate::AvlSetString::Tree::interned_EMPTY(), right: Arc::new(Tree::LEAF { key: (inKey).clone() }) });
            } else {
                outTree = tree;
            }
            outTree.clone()
...
pub(crate) fn intersection(mut tree1: Arc<Tree>, mut tree2: Arc<Tree>) -> Result<(Arc<Tree>, Arc<Tree>, Arc<Tree>)> {
    let mut intersect: Arc<Tree> = crate::AvlSetString::Tree::interned_EMPTY();
    let mut rest1: Arc<Tree> = crate::AvlSetString::Tree::interned_EMPTY();
    let mut rest2: Arc<Tree> = crate::AvlSetString::Tree::interned_EMPTY();
    let mut keylist1: Arc<metamodelica::List<ArcStr>>;
    let mut keylist2: Arc<metamodelica::List<ArcStr>>;
    let mut k1: Key;
    let mut k2: Key;
    let mut key_comp: i32;
...
pub fn join(mut tree: Arc<Tree>, mut treeToJoin: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    tree = (::match_deref::match_deref! { match &(treeToJoin.clone()) {
        ...
        Deref @ Tree::NODE { .. } => {
            tree = add(tree, (var_field!((*treeToJoin).key, Tree::NODE).clone()).clone())?;
            tree = join(tree, var_field!((*treeToJoin).left, Tree::NODE).clone())?;
            tree = join(tree, var_field!((*treeToJoin).right, Tree::NODE).clone())?;
            tree
        },
        Deref @ Tree::LEAF { .. } => add(tree, (var_field!((*treeToJoin).key, Tree::LEAF).clone()).clone())?,
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    Ok(tree)
}
...
pub fn new() -> Arc<Tree> {
    let mut outTree: Arc<Tree> = crate::AvlSetString::Tree::interned_EMPTY();
    outTree
}
...
fn rotateLeft(mut inNode: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    outNode = (::match_deref::match_deref! { match &(outNode.clone()) {
        ...
        Deref @ Tree::NODE { right: child @ Deref @ Tree::LEAF { .. }, .. } => {
            let mut node: Arc<Tree>;
            node = setTreeLeftRight(outNode.clone(), var_field!((*outNode).left, Tree::NODE).clone(), crate::AvlSetString::Tree::interned_EMPTY())?;
            setTreeLeftRight(child.clone(), node.clone(), crate::AvlSetString::Tree::interned_EMPTY())?
        },
        _ => {
            inNode
        },
...
fn rotateRight(mut inNode: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    outNode = (::match_deref::match_deref! { match &(outNode.clone()) {
        ...
        Deref @ Tree::NODE { left: child @ Deref @ Tree::LEAF { .. }, .. } => {
            let mut node: Arc<Tree>;
            node = setTreeLeftRight(outNode.clone(), crate::AvlSetString::Tree::interned_EMPTY(), var_field!((*outNode).right, Tree::NODE).clone())?;
            setTreeLeftRight(child.clone(), crate::AvlSetString::Tree::interned_EMPTY(), node.clone())?
        },
        _ => {
            inNode
        },
...
pub(crate) fn setTreeLeftRight(mut orig: Arc<Tree>, mut left: Arc<Tree>, mut right: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    res = (::match_deref::match_deref! { match &((orig.clone(), left.clone(), right.clone())) {
        ...
        (Deref @ Tree::NODE { .. }, _, _) => if (referenceEqOrEmpty(var_field!((*orig).left, Tree::NODE).clone(), left.clone()) && referenceEqOrEmpty(var_field!((*orig).right, Tree::NODE).clone(), right.clone())) {orig} else {Arc::new(Tree::NODE { key: (var_field!((*orig).key, Tree::NODE).clone()).clone(), height: std::cmp::max(height(left.clone()), height(right.clone())) + 1, left: left, right: right })},
        (Deref @ Tree::LEAF { .. }, _, _) => Arc::new(Tree::NODE { key: (var_field!((*orig).key, Tree::LEAF).clone()).clone(), height: std::cmp::max(height(left.clone()), height(right.clone())) + 1, left: left, right: right }),
        _ => bail!("match: no arm matched"),
    } });
    Ok(res)
}
