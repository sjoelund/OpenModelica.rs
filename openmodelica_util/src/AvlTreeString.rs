...
pub fn addList(mut tree: Arc<Tree>, mut inValues: Arc<metamodelica::List<(ArcStr, i32)>>, mut conflictFunc: ConflictFunc) -> Result<Arc<Tree>> {
...
pub fn addUpdate(mut tree: Arc<Tree>, mut key: Key, mut r#fn: Arc<dyn ::std::ops::Fn(Option<i32>) -> Result<i32> + 'static>) -> Result<Arc<Tree>> {
    pub type UpdateFn = fn(Option<i32>) -> Result<Value>;
...
pub fn fromList(mut inValues: Arc<metamodelica::List<(ArcStr, i32)>>, mut conflictFunc: ConflictFunc) -> Result<Arc<Tree>> {
...
pub fn update(mut tree: Arc<Tree>, mut key: Key, mut value: Value) -> Arc<Tree> {
    let mut outTree: Arc<Tree> = add(tree.clone(), (key.clone()).clone(), value.clone(), fnptr!(addConflictReplace, i32, i32, ArcStr)).unwrap();
    outTree
}
