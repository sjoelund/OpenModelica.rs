...
pub fn add(mut inTree: Arc<Tree>, mut inKey: Key, mut inValue: Value, mut conflictFunc: ConflictFunc) -> Result<Arc<Tree>> {
    ...
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        ...
        Deref @ Tree::NODE { key, .. } => {
            ...
            if key_comp.clone() == -1 {
                assign_variant_field!(tree => Tree::NODE; left = add(var_field!((*tree).left, Tree::NODE).clone(), (inKey.clone()).clone(), (inValue.clone()).clone(), conflictFunc)?);
            } else if key_comp.clone() == 1 {
                assign_variant_field!(tree => Tree::NODE; right = add(var_field!((*tree).right, Tree::NODE).clone(), (inKey.clone()).clone(), (inValue.clone()).clone(), conflictFunc)?);
            } else {
...
pub fn addList(mut tree: Arc<Tree>, mut inValues: Arc<metamodelica::List<(ArcStr, ArcStr)>>, mut conflictFunc: ConflictFunc) -> Result<Arc<Tree>> {
    ...
    for mut t in &*inValues.clone() {
        let mut t = t.clone();
        (key, value) = t.clone();
        tree = add(tree.clone(), (key.clone()).clone(), (value.clone()).clone(), conflictFunc)?;
    }
    Ok(tree)
}
...
pub fn addUpdate(mut tree: Arc<Tree>, mut key: Key, mut r#fn: Arc<dyn ::std::ops::Fn(Option<ArcStr>) -> Result<ArcStr> + 'static>) -> Result<Arc<Tree>> {
    pub type UpdateFn = fn(Option<ArcStr>) -> Result<Value>;
...
pub fn fromList(mut inValues: Arc<metamodelica::List<(ArcStr, ArcStr)>>, mut conflictFunc: ConflictFunc) -> Result<Arc<Tree>> {
...
pub fn join(mut tree: Arc<Tree>, mut treeToJoin: Arc<Tree>, mut conflictFunc: ConflictFunc) -> Result<Arc<Tree>> {
    ...
    tree = (::match_deref::match_deref! { match &(treeToJoin.clone()) {
        ...
        Deref @ Tree::NODE { .. } => {
            tree = add(tree.clone(), (var_field!((*treeToJoin).key, Tree::NODE).clone()).clone(), (var_field!((*treeToJoin).value, Tree::NODE).clone()).clone(), conflictFunc)?;
        ...
        Deref @ Tree::LEAF { .. } => add(tree.clone(), (var_field!((*treeToJoin).key, Tree::LEAF).clone()).clone(), (var_field!((*treeToJoin).value, Tree::LEAF).clone()).clone(), conflictFunc)?,
        _ => bail!("match: no arm matched"),
    } });
    Ok(tree)
}
...
pub fn update(mut tree: Arc<Tree>, mut key: Key, mut value: Value) -> Arc<Tree> {
    let mut outTree: Arc<Tree> = add(tree.clone(), (key.clone()).clone(), (value.clone()).clone(), fnptr!(addConflictReplace, ArcStr, ArcStr, ArcStr)).unwrap();
    outTree
}
