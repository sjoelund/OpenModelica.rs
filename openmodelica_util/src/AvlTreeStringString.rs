    ...
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        ...
        Deref @ Tree::NODE { key, .. } => {
            ...
            if key_comp.clone() == -1 {
                ...
                value = (conflictFunc((inValue).clone(), (var_field!((*tree).value, Tree::NODE).clone()).clone(), (key.clone()).clone())?).clone();
        ...
        Deref @ Tree::LEAF { .. } => {
            ...
            if key_comp.clone() == -1 {
                outTree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: (var_field!((*tree).value, Tree::LEAF).clone()).clone(), height: 2, left: Arc::new(Tree::LEAF { key: (inKey).clone(), value: (inValue).clone() }), right: crate::AvlTreeStringString::Tree::interned_EMPTY() });
            } else if key_comp.clone() == 1 {
                outTree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: (var_field!((*tree).value, Tree::LEAF).clone()).clone(), height: 2, left: crate::AvlTreeStringString::Tree::interned_EMPTY(), right: Arc::new(Tree::LEAF { key: (inKey).clone(), value: (inValue).clone() }) });
            } else {
                value = (conflictFunc((inValue).clone(), (var_field!((*tree).value, Tree::LEAF).clone()).clone(), (var_field!((*tree).key, Tree::LEAF).clone()).clone())?).clone();
...
pub(crate) fn addUpdate(mut tree: Arc<Tree>, mut key: Key, mut r#fn: Arc<dyn ::std::ops::Fn(Option<ArcStr>) -> Result<ArcStr> + 'static>) -> Result<Arc<Tree>> {
    ...
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        ...
        Deref @ Tree::LEAF { .. } => {
            ...
            if key_comp == -1 {
                new_tree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: (var_field!((*tree).value, Tree::LEAF).clone()).clone(), height: 2, left: Arc::new(Tree::LEAF { key: (key).clone(), value: (r#fn(None)?).clone() }), right: crate::AvlTreeStringString::Tree::interned_EMPTY() });
            } else if key_comp == 1 {
                new_tree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: (var_field!((*tree).value, Tree::LEAF).clone()).clone(), height: 2, left: crate::AvlTreeStringString::Tree::interned_EMPTY(), right: Arc::new(Tree::LEAF { key: (key).clone(), value: (r#fn(None)?).clone() }) });
            } else {
...
pub(crate) fn fold<FT: Clone + 'static + metamodelica::gc::MMTrace>(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, ArcStr, FT) -> Result<FT> + 'static>, mut inStartValue: FT) -> Result<FT> {
    ...
    outResult = (::match_deref::match_deref! { match &(inTree.clone()) {
        Deref @ Tree::NODE { key, value, .. } => {
            ...
            outResult = inFunc((key.clone()).clone(), (value.clone()).clone(), outResult)?;
...
pub(crate) fn foldCond<FT: Clone + 'static + metamodelica::gc::MMTrace>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(ArcStr, ArcStr, FT) -> Result<(FT, bool)> + 'static>, mut value: FT) -> Result<FT> {
    ...
    value = (::match_deref::match_deref! { match &(tree.clone()) {
        Deref @ Tree::NODE { .. } => {
            let mut c: bool;
            (value, c) = foldFunc((var_field!((*tree).key, Tree::NODE).clone()).clone(), (var_field!((*tree).value, Tree::NODE).clone()).clone(), value)?;
            if c.clone() {
        ...
        Deref @ Tree::LEAF { .. } => {
            let mut c: bool;
            (value, c) = foldFunc((var_field!((*tree).key, Tree::LEAF).clone()).clone(), (var_field!((*tree).value, Tree::LEAF).clone()).clone(), value)?;
            value
        },
        _ => {
            value
...
pub(crate) fn fold_2<FT1: Clone + 'static + metamodelica::gc::MMTrace, FT2: Clone + 'static + metamodelica::gc::MMTrace>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(ArcStr, ArcStr, FT1, FT2) -> Result<(FT1, FT2)> + 'static>, mut foldArg1: FT1, mut foldArg2: FT2) -> Result<(FT1, FT2)> {
    ...
    let () = (::match_deref::match_deref! { match &(tree.clone()) {
        Deref @ Tree::NODE { .. } => {
            ...
            (foldArg1, foldArg2) = foldFunc((var_field!((*tree).key, Tree::NODE).clone()).clone(), (var_field!((*tree).value, Tree::NODE).clone()).clone(), foldArg1, foldArg2)?;
            (foldArg1, foldArg2) = fold_2(var_field!((*tree).right, Tree::NODE).clone(), foldFunc.clone(), foldArg1, foldArg2)?;
            ()
        },
        Deref @ Tree::LEAF { .. } => {
            (foldArg1, foldArg2) = foldFunc((var_field!((*tree).key, Tree::LEAF).clone()).clone(), (var_field!((*tree).value, Tree::LEAF).clone()).clone(), foldArg1, foldArg2)?;
            ()
        },
        _ => (),
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
...
pub(crate) fn join(mut tree: Arc<Tree>, mut treeToJoin: Arc<Tree>, mut conflictFunc: Arc<dyn ::std::ops::Fn(ArcStr, ArcStr, ArcStr) -> Result<ArcStr> + 'static>) -> Result<Arc<Tree>> {
    ...
    tree = (::match_deref::match_deref! { match &(treeToJoin.clone()) {
        ...
        Deref @ Tree::NODE { .. } => {
            tree = add(tree, (var_field!((*treeToJoin).key, Tree::NODE).clone()).clone(), (var_field!((*treeToJoin).value, Tree::NODE).clone()).clone(), conflictFunc.clone())?;
        ...
        Deref @ Tree::LEAF { .. } => add(tree, (var_field!((*treeToJoin).key, Tree::LEAF).clone()).clone(), (var_field!((*treeToJoin).value, Tree::LEAF).clone()).clone(), conflictFunc.clone())?,
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    Ok(tree)
}
...
pub(crate) fn mapFold<FT: Clone + 'static + metamodelica::gc::MMTrace>(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, ArcStr, FT) -> Result<(ArcStr, FT)> + 'static>, mut inStartValue: FT) -> Result<(Arc<Tree>, FT)> {
    ...
    outTree = (::match_deref::match_deref! { match &(outTree.clone()) {
        Deref @ Tree::NODE { key, value, .. } => {
            ...
            (new_value, outResult) = inFunc((key.clone()).clone(), (value.clone()).clone(), outResult)?;
...
pub(crate) fn setTreeLeftRight(mut orig: Arc<Tree>, mut left: Arc<Tree>, mut right: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    res = (::match_deref::match_deref! { match &((orig.clone(), left.clone(), right.clone())) {
        ...
        (Deref @ Tree::NODE { .. }, _, _) => if (referenceEqOrEmpty(var_field!((*orig).left, Tree::NODE).clone(), left.clone()) && referenceEqOrEmpty(var_field!((*orig).right, Tree::NODE).clone(), right.clone())) {orig} else {Arc::new(Tree::NODE { key: (var_field!((*orig).key, Tree::NODE).clone()).clone(), value: (var_field!((*orig).value, Tree::NODE).clone()).clone(), height: std::cmp::max(height(left.clone()), height(right.clone())) + 1, left: left, right: right })},
        (Deref @ Tree::LEAF { .. }, _, _) => Arc::new(Tree::NODE { key: (var_field!((*orig).key, Tree::LEAF).clone()).clone(), value: (var_field!((*orig).value, Tree::LEAF).clone()).clone(), height: std::cmp::max(height(left.clone()), height(right.clone())) + 1, left: left, right: right }),
        _ => bail!("match: no arm matched"),
    } });
    Ok(res)
}
