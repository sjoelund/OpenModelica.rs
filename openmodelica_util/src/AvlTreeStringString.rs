...
pub fn add(mut inTree: Arc<Tree>, mut inKey: Key, mut inValue: Value, mut conflictFunc: Arc<dyn ::std::ops::Fn(ArcStr, ArcStr, ArcStr) -> Result<ArcStr> + 'static>) -> Result<Arc<Tree>> {
    let mut tree: Arc<Tree> = inTree.clone();
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        Deref @ Tree::EMPTY { .. } => {
            Arc::new(Tree::LEAF { key: (inKey.clone()).clone(), value: (inValue.clone()).clone() })
        },
        Deref @ Tree::NODE { key, .. } => {
            let mut value: Value;
            ...
            if key_comp.clone() == -1 {
                assign_variant_field!(tree => Tree::NODE; left = add(var_field!((*tree).left, Tree::NODE).clone(), (inKey.clone()).clone(), (inValue.clone()).clone(), conflictFunc.clone())?);
            } else if key_comp.clone() == 1 {
                assign_variant_field!(tree => Tree::NODE; right = add(var_field!((*tree).right, Tree::NODE).clone(), (inKey.clone()).clone(), (inValue.clone()).clone(), conflictFunc.clone())?);
            } else {
                value = (conflictFunc((inValue.clone()).clone(), (var_field!((*tree).value, Tree::NODE).clone()).clone(), (key.clone()).clone())?).clone();
        ...
        Deref @ Tree::LEAF { .. } => {
            ...
            if key_comp.clone() == -1 {
                outTree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: (var_field!((*tree).value, Tree::LEAF).clone()).clone(), height: 2, left: Arc::new(Tree::LEAF { key: (inKey.clone()).clone(), value: (inValue.clone()).clone() }), right: crate::AvlTreeStringString::Tree::interned_EMPTY() });
            } else if key_comp.clone() == 1 {
                outTree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: (var_field!((*tree).value, Tree::LEAF).clone()).clone(), height: 2, left: crate::AvlTreeStringString::Tree::interned_EMPTY(), right: Arc::new(Tree::LEAF { key: (inKey.clone()).clone(), value: (inValue.clone()).clone() }) });
            } else {
                value = (conflictFunc((inValue.clone()).clone(), (var_field!((*tree).value, Tree::LEAF).clone()).clone(), (var_field!((*tree).key, Tree::LEAF).clone()).clone())?).clone();
...
pub(crate) fn addList(mut tree: Arc<Tree>, mut inValues: Arc<metamodelica::List<(ArcStr, ArcStr)>>, mut conflictFunc: Arc<dyn ::std::ops::Fn(ArcStr, ArcStr, ArcStr) -> Result<ArcStr> + 'static>) -> Result<Arc<Tree>> {
    ...
    for mut t in &*inValues.clone() {
        let mut t = t.clone();
        (key, value) = t.clone();
        tree = add(tree.clone(), (key.clone()).clone(), (value.clone()).clone(), conflictFunc.clone())?;
    }
    Ok(tree)
}
...
pub(crate) fn addUpdate(mut tree: Arc<Tree>, mut key: Key, mut r#fn: Arc<dyn ::std::ops::Fn(Option<ArcStr>) -> Result<ArcStr> + 'static>) -> Result<Arc<Tree>> {
    ...
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        Deref @ Tree::EMPTY { .. } => Arc::new(Tree::LEAF { key: (key.clone()).clone(), value: (r#fn(None)?).clone() }),
        Deref @ Tree::NODE { .. } => {
        ...
        Deref @ Tree::LEAF { .. } => {
            ...
            if key_comp.clone() == -1 {
                new_tree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: (var_field!((*tree).value, Tree::LEAF).clone()).clone(), height: 2, left: Arc::new(Tree::LEAF { key: (key.clone()).clone(), value: (r#fn(None)?).clone() }), right: crate::AvlTreeStringString::Tree::interned_EMPTY() });
            } else if key_comp.clone() == 1 {
                new_tree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: (var_field!((*tree).value, Tree::LEAF).clone()).clone(), height: 2, left: crate::AvlTreeStringString::Tree::interned_EMPTY(), right: Arc::new(Tree::LEAF { key: (key.clone()).clone(), value: (r#fn(None)?).clone() }) });
            } else {
...
pub(crate) fn fold<FT: Clone + 'static + metamodelica::gc::MMTrace>(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, ArcStr, FT) -> Result<FT> + 'static>, mut inStartValue: FT) -> Result<FT> {
    ...
    outResult = (::match_deref::match_deref! { match &(inTree.clone()) {
        Deref @ Tree::NODE { key, value, .. } => {
            ...
            outResult = inFunc((key.clone()).clone(), (value.clone()).clone(), outResult.clone())?;
...
pub(crate) fn foldCond<FT: Clone + 'static + metamodelica::gc::MMTrace>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(ArcStr, ArcStr, FT) -> Result<(FT, bool)> + 'static>, mut value: FT) -> Result<FT> {
    ...
    value = (::match_deref::match_deref! { match &(tree.clone()) {
        Deref @ Tree::NODE { .. } => {
            let mut c: bool;
            (value, c) = foldFunc((var_field!((*tree).key, Tree::NODE).clone()).clone(), (var_field!((*tree).value, Tree::NODE).clone()).clone(), value.clone())?;
            if c.clone() {
        ...
        Deref @ Tree::LEAF { .. } => {
            let mut c: bool;
            (value, c) = foldFunc((var_field!((*tree).key, Tree::LEAF).clone()).clone(), (var_field!((*tree).value, Tree::LEAF).clone()).clone(), value.clone())?;
            value.clone()
        },
        _ => {
            value.clone()
...
pub(crate) fn fold_2<FT1: Clone + 'static + metamodelica::gc::MMTrace, FT2: Clone + 'static + metamodelica::gc::MMTrace>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(ArcStr, ArcStr, FT1, FT2) -> Result<(FT1, FT2)> + 'static>, mut foldArg1: FT1, mut foldArg2: FT2) -> Result<(FT1, FT2)> {
    ...
    let () = (::match_deref::match_deref! { match &(tree.clone()) {
        Deref @ Tree::NODE { .. } => {
            ...
            (foldArg1, foldArg2) = foldFunc((var_field!((*tree).key, Tree::NODE).clone()).clone(), (var_field!((*tree).value, Tree::NODE).clone()).clone(), foldArg1.clone(), foldArg2.clone())?;
            (foldArg1, foldArg2) = fold_2(var_field!((*tree).right, Tree::NODE).clone(), foldFunc.clone(), foldArg1.clone(), foldArg2.clone())?;
            ()
        },
        Deref @ Tree::LEAF { .. } => {
            (foldArg1, foldArg2) = foldFunc((var_field!((*tree).key, Tree::LEAF).clone()).clone(), (var_field!((*tree).value, Tree::LEAF).clone()).clone(), foldArg1.clone(), foldArg2.clone())?;
            ()
        },
        _ => (),
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
...
pub(crate) fn forEach(mut tree: Arc<Tree>, mut func: Arc<dyn ::std::ops::Fn(ArcStr, ArcStr) -> Result<()> + 'static>) -> Result<()> {
    ...
    let () = (::match_deref::match_deref! { match &(tree.clone()) {
        Deref @ Tree::NODE { .. } => {
            ...
            func((var_field!((*tree).key, Tree::NODE).clone()).clone(), (var_field!((*tree).value, Tree::NODE).clone()).clone())?;
            forEach(var_field!((*tree).right, Tree::NODE).clone(), func.clone())?;
            ()
        },
        Deref @ Tree::LEAF { .. } => {
            func((var_field!((*tree).key, Tree::LEAF).clone()).clone(), (var_field!((*tree).value, Tree::LEAF).clone()).clone())?;
            ()
        },
        Deref @ Tree::EMPTY { .. } => (),
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
...
pub(crate) fn fromList(mut inValues: Arc<metamodelica::List<(ArcStr, ArcStr)>>, mut conflictFunc: Arc<dyn ::std::ops::Fn(ArcStr, ArcStr, ArcStr) -> Result<ArcStr> + 'static>) -> Result<Arc<Tree>> {
    let mut tree: Arc<Tree> = crate::AvlTreeStringString::Tree::interned_EMPTY();
    let mut key: Key;
    let mut value: Value;
    for mut t in &*inValues.clone() {
        let mut t = t.clone();
...
pub(crate) fn getOpt(mut tree: Arc<Tree>, mut key: Key) -> Option<ArcStr> {
    '__tco: loop {
        let mut k: Key;
        k = ((::match_deref::match_deref! { match &(tree.clone()) {
        Deref @ Tree::NODE { .. } => var_field!((*tree).key, Tree::NODE).clone(),
        ...
        (0, Deref @ Tree::LEAF { .. }) => return Some((var_field!((*tree).value, Tree::LEAF).clone()).clone()),
        (0, Deref @ Tree::NODE { .. }) => return Some((var_field!((*tree).value, Tree::NODE).clone()).clone()),
...
pub(crate) fn join(mut tree: Arc<Tree>, mut treeToJoin: Arc<Tree>, mut conflictFunc: Arc<dyn ::std::ops::Fn(ArcStr, ArcStr, ArcStr) -> Result<ArcStr> + 'static>) -> Result<Arc<Tree>> {
    let mut tree: Arc<Tree> = tree;
    tree = (::match_deref::match_deref! { match &(treeToJoin.clone()) {
        Deref @ Tree::EMPTY { .. } => tree.clone(),
        Deref @ Tree::NODE { .. } => {
            tree = add(tree.clone(), (var_field!((*treeToJoin).key, Tree::NODE).clone()).clone(), (var_field!((*treeToJoin).value, Tree::NODE).clone()).clone(), conflictFunc.clone())?;
        ...
        Deref @ Tree::LEAF { .. } => add(tree.clone(), (var_field!((*treeToJoin).key, Tree::LEAF).clone()).clone(), (var_field!((*treeToJoin).value, Tree::LEAF).clone()).clone(), conflictFunc.clone())?,
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    Ok(tree)
}
...
pub(crate) fn map(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, ArcStr) -> Result<ArcStr> + 'static>) -> Result<Arc<Tree>> {
    ...
    outTree = (::match_deref::match_deref! { match &(outTree.clone()) {
        Deref @ Tree::NODE { key, value, .. } => {
            ...
            new_value = (inFunc((key.clone()).clone(), (value.clone()).clone())?).clone();
            ...
            if !(referenceEq(&*(new_left.clone()),&*(var_field!((*outTree).left, Tree::NODE).clone()))) || !(referenceEq(&*(value.clone()),&*(new_value.clone()))) || !(referenceEq(&*(new_right.clone()),&*(var_field!((*outTree).right, Tree::NODE).clone()))) {
                outTree = Arc::new(Tree::NODE { key: (key.clone()).clone(), value: (new_value.clone()).clone(), height: var_field!((*outTree).height, Tree::NODE).clone(), left: new_left.clone(), right: new_right.clone() });
            }
            outTree.clone()
        },
        Deref @ Tree::LEAF { key, value } => {
...
pub(crate) fn mapFold<FT: Clone + 'static + metamodelica::gc::MMTrace>(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, ArcStr, FT) -> Result<(ArcStr, FT)> + 'static>, mut inStartValue: FT) -> Result<(Arc<Tree>, FT)> {
    ...
    outTree = (::match_deref::match_deref! { match &(outTree.clone()) {
        Deref @ Tree::NODE { key, value, .. } => {
            ...
            (new_value, outResult) = inFunc((key.clone()).clone(), (value.clone()).clone(), outResult.clone())?;
...
pub(crate) fn new() -> Arc<Tree> {
    let mut outTree: Arc<Tree> = crate::AvlTreeStringString::Tree::interned_EMPTY();
    outTree
}
...
pub(crate) fn printNodeStr(mut inNode: Arc<Tree>) -> Result<ArcStr> {
    ...
    outString = ((::match_deref::match_deref! { match &(inNode.clone()) {
        Deref @ Tree::NODE { .. } => { let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("(")); __mm_s.push_str(&*keyStr((var_field!((*inNode).key, Tree::NODE).clone()).clone())); __mm_s.push_str(&*literal!(", ")); __mm_s.push_str(&*valueStr((var_field!((*inNode).value, Tree::NODE).clone()).clone())); __mm_s.push_str(&*literal!(")")); ArcStr::from(__mm_s) },
        Deref @ Tree::LEAF { .. } => { let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("(")); __mm_s.push_str(&*keyStr((var_field!((*inNode).key, Tree::LEAF).clone()).clone())); __mm_s.push_str(&*literal!(", ")); __mm_s.push_str(&*valueStr((var_field!((*inNode).value, Tree::LEAF).clone()).clone())); __mm_s.push_str(&*literal!(")")); ArcStr::from(__mm_s) },
        _ => bail!("match: no arm matched"),
    } })).clone();
    Ok(outString)
}
...
fn rotateLeft(mut inNode: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    outNode = (::match_deref::match_deref! { match &(outNode.clone()) {
        ...
        Deref @ Tree::NODE { right: child @ Deref @ Tree::LEAF { .. }, .. } => {
            let mut node: Arc<Tree>;
            node = setTreeLeftRight(outNode.clone(), var_field!((*outNode).left, Tree::NODE).clone(), crate::AvlTreeStringString::Tree::interned_EMPTY())?;
            setTreeLeftRight(child.clone(), node.clone(), crate::AvlTreeStringString::Tree::interned_EMPTY())?
        },
        _ => {
            inNode.clone()
        },
...
fn rotateRight(mut inNode: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    outNode = (::match_deref::match_deref! { match &(outNode.clone()) {
        ...
        Deref @ Tree::NODE { left: child @ Deref @ Tree::LEAF { .. }, .. } => {
            let mut node: Arc<Tree>;
            node = setTreeLeftRight(outNode.clone(), crate::AvlTreeStringString::Tree::interned_EMPTY(), var_field!((*outNode).right, Tree::NODE).clone())?;
            setTreeLeftRight(child.clone(), crate::AvlTreeStringString::Tree::interned_EMPTY(), node.clone())?
        },
        _ => {
            inNode.clone()
        },
...
pub(crate) fn setTreeLeftRight(mut orig: Arc<Tree>, mut left: Arc<Tree>, mut right: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    res = (::match_deref::match_deref! { match &((orig.clone(), left.clone(), right.clone())) {
        (Deref @ Tree::NODE { .. }, Deref @ Tree::EMPTY { .. }, Deref @ Tree::EMPTY { .. }) => Arc::new(Tree::LEAF { key: (var_field!((*orig).key, Tree::NODE).clone()).clone(), value: (var_field!((*orig).value, Tree::NODE).clone()).clone() }),
        (Deref @ Tree::LEAF { .. }, Deref @ Tree::EMPTY { .. }, Deref @ Tree::EMPTY { .. }) => orig.clone(),
        (Deref @ Tree::NODE { .. }, _, _) => if (referenceEqOrEmpty(var_field!((*orig).left, Tree::NODE).clone(), left.clone()) && referenceEqOrEmpty(var_field!((*orig).right, Tree::NODE).clone(), right.clone())) {orig.clone()} else {Arc::new(Tree::NODE { key: (var_field!((*orig).key, Tree::NODE).clone()).clone(), value: (var_field!((*orig).value, Tree::NODE).clone()).clone(), height: std::cmp::max(height(left.clone()), height(right.clone())) + 1, left: left.clone(), right: right.clone() })},
        (Deref @ Tree::LEAF { .. }, _, _) => Arc::new(Tree::NODE { key: (var_field!((*orig).key, Tree::LEAF).clone()).clone(), value: (var_field!((*orig).value, Tree::LEAF).clone()).clone(), height: std::cmp::max(height(left.clone()), height(right.clone())) + 1, left: left.clone(), right: right.clone() }),
        _ => bail!("match: no arm matched"),
    } });
    Ok(res)
}
...
pub(crate) fn toList(mut inTree: Arc<Tree>, mut lst: Arc<metamodelica::List<(ArcStr, ArcStr)>>) -> Arc<metamodelica::List<(ArcStr, ArcStr)>> {
    let mut lst: Arc<metamodelica::List<(ArcStr, ArcStr)>> = lst;
    lst = (::match_deref::match_deref! { match &(inTree.clone()) {
        Deref @ Tree::NODE { key, value, .. } => {
...
pub(crate) fn update(mut tree: Arc<Tree>, mut key: Key, mut value: Value) -> Result<Arc<Tree>> {
    let mut outTree: Arc<Tree> = add(tree.clone(), (key.clone()).clone(), (value.clone()).clone(), (std::sync::Arc::new(fnptr!(addConflictReplace, ArcStr, ArcStr, ArcStr)) as std::sync::Arc<dyn ::std::ops::Fn(ArcStr, ArcStr, ArcStr) -> Result<ArcStr> + 'static>))?;
    Ok(outTree)
}
