...
pub mod Entry {
    ...
    pub fn index(mut entry: Arc<Entry>) -> Result<i32> {
        let mut index: i32;
        index = (::match_deref::match_deref! { match &(entry.clone()) {
        Deref @ CLASS { .. } => var_field!((*entry).index, Entry::CLASS).clone(),
        Deref @ COMPONENT { .. } => var_field!((*entry).index, Entry::COMPONENT).clone(),
        Deref @ IMPORT { .. } => var_field!((*entry).index, Entry::IMPORT).clone(),
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
        Ok(index)
    }

    pub fn isEqual(mut entry1: Arc<Entry>, mut entry2: Arc<Entry>) -> Result<bool> {
        let mut isEqual: bool = index(entry1.clone())? == index(entry2.clone())?;
        Ok(isEqual)
    }

    pub fn isImport(mut entry: Arc<Entry>) -> bool {
        let mut isImport: bool;
        isImport = (::match_deref::match_deref! { match &(entry.clone()) {
        Deref @ IMPORT { .. } => true,
        _ => false,
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
pub(crate) fn valueStr(mut inValue: Value) -> Result<ArcStr> {
    let mut outString: ArcStr;
    outString = ((::match_deref::match_deref! { match &(inValue.clone()) {
        Deref @ Entry::CLASS { .. } => { let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("class ")); __mm_s.push_str(&*ArcStr::from(::std::format!("{}", var_field!((*inValue).index, Entry::Entry::CLASS).clone()))); ArcStr::from(__mm_s) },
        Deref @ Entry::COMPONENT { .. } => { let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("comp ")); __mm_s.push_str(&*ArcStr::from(::std::format!("{}", var_field!((*inValue).index, Entry::Entry::COMPONENT).clone()))); ArcStr::from(__mm_s) },
...
pub type Value = Arc<Entry::Entry>;
...
pub fn add(mut inTree: Arc<Tree>, mut inKey: Key, mut inValue: Value, mut conflictFunc: Arc<dyn ::std::ops::Fn(Arc<Entry::Entry>, Arc<Entry::Entry>, ArcStr) -> Result<Arc<Entry::Entry>> + 'static>) -> Result<Arc<Tree>> {
    ...
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        ...
        Deref @ Tree::LEAF { .. } => {
            ...
            if key_comp.clone() == -1 {
                outTree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: Arc::new(Tree::LEAF { key: (inKey.clone()).clone(), value: inValue.clone() }), right: crate::NFLookupTree::Tree::interned_EMPTY() });
            } else if key_comp.clone() == 1 {
                outTree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: crate::NFLookupTree::Tree::interned_EMPTY(), right: Arc::new(Tree::LEAF { key: (inKey.clone()).clone(), value: inValue.clone() }) });
            } else {
...
pub(crate) fn addList(mut tree: Arc<Tree>, mut inValues: Arc<metamodelica::List<(ArcStr, Arc<Entry::Entry>)>>, mut conflictFunc: Arc<dyn ::std::ops::Fn(Arc<Entry::Entry>, Arc<Entry::Entry>, ArcStr) -> Result<Arc<Entry::Entry>> + 'static>) -> Result<Arc<Tree>> {
...
pub(crate) fn addUpdate(mut tree: Arc<Tree>, mut key: Key, mut r#fn: Arc<dyn ::std::ops::Fn(Option<Arc<Entry::Entry>>) -> Result<Arc<Entry::Entry>> + 'static>) -> Result<Arc<Tree>> {
    ...
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        ...
        Deref @ Tree::LEAF { .. } => {
            ...
            if key_comp.clone() == -1 {
                new_tree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: Arc::new(Tree::LEAF { key: (key.clone()).clone(), value: r#fn(None)? }), right: crate::NFLookupTree::Tree::interned_EMPTY() });
            } else if key_comp.clone() == 1 {
                new_tree = Arc::new(Tree::NODE { key: (var_field!((*tree).key, Tree::LEAF).clone()).clone(), value: var_field!((*tree).value, Tree::LEAF).clone(), height: 2, left: crate::NFLookupTree::Tree::interned_EMPTY(), right: Arc::new(Tree::LEAF { key: (key.clone()).clone(), value: r#fn(None)? }) });
            } else {
...
pub fn fold<FT: Clone + 'static + metamodelica::gc::MMTrace>(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry::Entry>, FT) -> Result<FT> + 'static>, mut inStartValue: FT) -> Result<FT> {
...
pub(crate) fn foldCond<FT: Clone + 'static + metamodelica::gc::MMTrace>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry::Entry>, FT) -> Result<(FT, bool)> + 'static>, mut value: FT) -> Result<FT> {
...
pub(crate) fn fold_2<FT1: Clone + 'static + metamodelica::gc::MMTrace, FT2: Clone + 'static + metamodelica::gc::MMTrace>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry::Entry>, FT1, FT2) -> Result<(FT1, FT2)> + 'static>, mut foldArg1: FT1, mut foldArg2: FT2) -> Result<(FT1, FT2)> {
...
pub(crate) fn forEach(mut tree: Arc<Tree>, mut func: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry::Entry>) -> Result<()> + 'static>) -> Result<()> {
...
pub(crate) fn fromList(mut inValues: Arc<metamodelica::List<(ArcStr, Arc<Entry::Entry>)>>, mut conflictFunc: Arc<dyn ::std::ops::Fn(Arc<Entry::Entry>, Arc<Entry::Entry>, ArcStr) -> Result<Arc<Entry::Entry>> + 'static>) -> Result<Arc<Tree>> {
    let mut tree: Arc<Tree> = crate::NFLookupTree::Tree::interned_EMPTY();
    let mut key: Key;
    let mut value: Value;
    for mut t in &*inValues.clone() {
        let mut t = t.clone();
...
pub fn getOpt(mut tree: Arc<Tree>, mut key: Key) -> Option<Arc<Entry::Entry>> {
    '__tco: loop {
        let mut k: Key;
        k = ((::match_deref::match_deref! { match &(tree.clone()) {
        Deref @ Tree::NODE { .. } => var_field!((*tree).key, Tree::NODE).clone(),
...
pub(crate) fn join(mut tree: Arc<Tree>, mut treeToJoin: Arc<Tree>, mut conflictFunc: Arc<dyn ::std::ops::Fn(Arc<Entry::Entry>, Arc<Entry::Entry>, ArcStr) -> Result<Arc<Entry::Entry>> + 'static>) -> Result<Arc<Tree>> {
    let mut tree: Arc<Tree> = tree;
...
pub(crate) fn listValues(mut tree: Arc<Tree>, mut lst: Arc<metamodelica::List<Arc<Entry::Entry>>>) -> Arc<metamodelica::List<Arc<Entry::Entry>>> {
    let mut lst: Arc<metamodelica::List<Arc<Entry::Entry>>> = lst;
    lst = (::match_deref::match_deref! { match &(tree.clone()) {
...
pub fn map(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry::Entry>) -> Result<Arc<Entry::Entry>> + 'static>) -> Result<Arc<Tree>> {
...
pub(crate) fn mapFold<FT: Clone + 'static + metamodelica::gc::MMTrace>(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry::Entry>, FT) -> Result<(Arc<Entry::Entry>, FT)> + 'static>, mut inStartValue: FT) -> Result<(Arc<Tree>, FT)> {
...
pub fn new() -> Arc<Tree> {
    let mut outTree: Arc<Tree> = crate::NFLookupTree::Tree::interned_EMPTY();
    outTree
}
...
fn rotateLeft(mut inNode: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    outNode = (::match_deref::match_deref! { match &(outNode.clone()) {
        ...
        Deref @ Tree::NODE { right: child @ Deref @ Tree::LEAF { .. }, .. } => {
            let mut node: Arc<Tree>;
            node = setTreeLeftRight(outNode.clone(), var_field!((*outNode).left, Tree::NODE).clone(), crate::NFLookupTree::Tree::interned_EMPTY())?;
            setTreeLeftRight(child.clone(), node.clone(), crate::NFLookupTree::Tree::interned_EMPTY())?
        },
        _ => {
            inNode.clone()
        },
...
fn rotateRight(mut inNode: Arc<Tree>) -> Result<Arc<Tree>> {
    ...
    outNode = (::match_deref::match_deref! { match &(outNode.clone()) {
        ...
        Deref @ Tree::NODE { left: child @ Deref @ Tree::LEAF { .. }, .. } => {
            let mut node: Arc<Tree>;
            node = setTreeLeftRight(outNode.clone(), crate::NFLookupTree::Tree::interned_EMPTY(), var_field!((*outNode).right, Tree::NODE).clone())?;
            setTreeLeftRight(child.clone(), crate::NFLookupTree::Tree::interned_EMPTY(), node.clone())?
        },
        _ => {
            inNode.clone()
        },
...
pub(crate) fn toList(mut inTree: Arc<Tree>, mut lst: Arc<metamodelica::List<(ArcStr, Arc<Entry::Entry>)>>) -> Arc<metamodelica::List<(ArcStr, Arc<Entry::Entry>)>> {
    let mut lst: Arc<metamodelica::List<(ArcStr, Arc<Entry::Entry>)>> = lst;
    lst = (::match_deref::match_deref! { match &(inTree.clone()) {
        Deref @ Tree::NODE { key, value, .. } => {
...
pub(crate) fn update(mut tree: Arc<Tree>, mut key: Key, mut value: Value) -> Result<Arc<Tree>> {
    let mut outTree: Arc<Tree> = add(tree.clone(), (key.clone()).clone(), value.clone(), (std::sync::Arc::new(fnptr!(addConflictReplace, Arc<Entry::Entry>, Arc<Entry::Entry>, ArcStr)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<Entry::Entry>, Arc<Entry::Entry>, ArcStr) -> Result<Arc<Entry::Entry>> + 'static>))?;
    Ok(outTree)
}
