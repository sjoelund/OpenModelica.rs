...
pub fn add(mut inTree: Arc<Tree>, mut inKey: Key, mut inValue: Value, mut conflictFunc: Arc<dyn ::std::ops::Fn(Arc<Entry::Entry>, Arc<Entry::Entry>, ArcStr) -> Result<Arc<Entry::Entry>> + 'static>) -> Result<Arc<Tree>> {
    ...
    tree = (::match_deref::match_deref! { match &(tree.clone()) {
        ...
        Deref @ Tree::NODE { key, .. } => {
            let mut value: Value = Arc::new(<Entry::Entry as ::std::default::Default>::default());
...
pub fn foldCond<FT: Clone + 'static + metamodelica::gc::MMTrace>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry::Entry>, FT) -> Result<(FT, bool)> + 'static>, mut value: FT) -> Result<FT> {
...
pub fn fold_2<FT1: Clone + 'static + metamodelica::gc::MMTrace, FT2: Clone + 'static + metamodelica::gc::MMTrace>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry::Entry>, FT1, FT2) -> Result<(FT1, FT2)> + 'static>, mut foldArg1: FT1, mut foldArg2: FT2) -> Result<(FT1, FT2)> {
...
pub fn listValues(mut tree: Arc<Tree>, mut lst: Arc<metamodelica::List<Arc<Entry::Entry>>>) -> Arc<metamodelica::List<Arc<Entry::Entry>>> {
...
pub fn map(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry::Entry>) -> Result<Arc<Entry::Entry>> + 'static>) -> Result<Arc<Tree>> {
    ...
    outTree = (::match_deref::match_deref! { match &(outTree.clone()) {
        Deref @ Tree::NODE { key, value, .. } => {
            let mut new_value: Value = Arc::new(<Entry::Entry as ::std::default::Default>::default());
...
pub fn mapFold<FT: Clone + 'static + metamodelica::gc::MMTrace>(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry::Entry>, FT) -> Result<(Arc<Entry::Entry>, FT)> + 'static>, mut inStartValue: FT) -> Result<(Arc<Tree>, FT)> {
...
pub fn toList(mut inTree: Arc<Tree>, mut lst: Arc<metamodelica::List<(ArcStr, Arc<Entry::Entry>)>>) -> Arc<metamodelica::List<(ArcStr, Arc<Entry::Entry>)>> {
