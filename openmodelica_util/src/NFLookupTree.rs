...
pub fn fold<FT: Clone + 'static>(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry::Entry>, FT) -> Result<FT> + 'static>, mut inStartValue: FT) -> Result<FT> {
...
pub fn foldCond<FT: Clone + 'static>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry::Entry>, FT) -> Result<(FT, bool)> + 'static>, mut value: FT) -> Result<FT> {
...
pub fn fold_2<FT1: Clone + 'static, FT2: Clone + 'static>(mut tree: Arc<Tree>, mut foldFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry::Entry>, FT1, FT2) -> Result<(FT1, FT2)> + 'static>, mut foldArg1: FT1, mut foldArg2: FT2) -> Result<(FT1, FT2)> {
...
pub fn getOpt(mut tree: Arc<Tree>, mut key: Key) -> Option<Arc<Entry::Entry>> {
    let mut value: Option<Arc<Entry::Entry>> = None;
    let mut k: Key = arcstr::literal!("");
    k = ((::match_deref::match_deref! { match &(tree.clone()) {
        Deref @ Tree::NODE { .. } => var_field!((*tree).key, Tree::NODE).clone(),
        Deref @ Tree::LEAF { .. } => var_field!((*tree).key, Tree::LEAF).clone(),
...
pub fn mapFold<FT: Clone + 'static>(mut inTree: Arc<Tree>, mut inFunc: Arc<dyn ::std::ops::Fn(ArcStr, Arc<Entry::Entry>, FT) -> Result<(Arc<Entry::Entry>, FT)> + 'static>, mut inStartValue: FT) -> Result<(Arc<Tree>, FT)> {
