...
pub fn saveFlags(mut inFlags: Flags::Flag) -> () {
    { let __v = inFlags; crate::Globals::flagsIndex.with(|__root| *__root.borrow_mut() = __v) };
    ()
}
...
pub fn getConfigOptionsStringList(mut inFlag: Flags::ConfigFlag) -> Result<(Arc<metamodelica::List<ArcStr>>, Arc<metamodelica::List<ArcStr>>)> {
    let mut outOptions: Arc<metamodelica::List<ArcStr>>;
    let mut outComments: Arc<metamodelica::List<ArcStr>>;
    (outOptions, outComments) = (match inFlag {
        Flags::ConfigFlag { validOptions: Some(Flags::ValidOptions::STRING_DESC_OPTION { options: mut options }), .. } => {
            (List::map(options.clone(), std::sync::Arc::new(fnptr!(Util::tuple21, _)))?, List::map(options.clone(), std::sync::Arc::new(fnptr!(Util::tuple22, _)))?)
        },
...
fn updateDebugFlagArray(mut inFlags: metamodelica::Array<bool>, mut inValue: bool, mut inFlag: Flags::DebugFlag) -> Result<(metamodelica::Array<bool>, bool)> {
    ...
    let mut outOldValue: bool;
    let mut index: i32;
    let Flags::DEBUG_FLAG { index: __pa0, .. } = (inFlag) else { bail!("pattern mismatch") };
    index = __pa0.clone();
...
pub fn readArgs(mut inArgs: Arc<metamodelica::List<ArcStr>>) -> Result<Arc<metamodelica::List<ArcStr>>> {
    ...
    Error::assertionOrAddSourceMessage(numError == Error::getNumErrorMessages(), Error::UTF8_COMMAND_LINE_ARGS.clone(), metamodelica::nil(), Util::dummyInfo.clone())?;
    saveFlags(flags);
...
fn readArg(mut inArg: ArcStr, mut inFlags: Flags::Flag, mut restArgs: Arc<metamodelica::List<ArcStr>>, mut nonFlags: Arc<metamodelica::List<ArcStr>>) -> Result<(Arc<metamodelica::List<ArcStr>>, Arc<metamodelica::List<ArcStr>>)> {
    ...
    if flagtype.clone() == literal!("+") {
        if len == 1 {
            parseFlag((inArg).clone(), crate::Flags::Flag::NO_FLAGS, restArgs.clone(), (literal!("")).clone())?;
        } else {
            restArgs = parseFlag(substring((inArg).clone(), 2, len)?, inFlags, restArgs, (flagtype).clone())?;
        }
    } else if flagtype.clone() == literal!("-") {
        if len == 1 {
            parseFlag((inArg).clone(), crate::Flags::Flag::NO_FLAGS, restArgs.clone(), (literal!("")).clone())?;
        } else if len == 2 {
            restArgs = parseFlag(substring((inArg).clone(), 2, 2)?, inFlags, restArgs, (flagtype).clone())?;
        } else if stringGetStringChar((inArg.clone()).clone(), 2)? == literal!("-") {
            if len < 4 || stringGetStringChar((inArg.clone()).clone(), 4)? == literal!("=") {
                parseFlag((inArg).clone(), crate::Flags::Flag::NO_FLAGS, restArgs.clone(), (literal!("")).clone())?;
            } else {
                restArgs = parseFlag(substring((inArg).clone(), 3, len)?, inFlags, restArgs, (literal!("--")).clone())?;
            }
        } else {
        ...
        nonFlags = metamodelica::cons((inArg).clone(), nonFlags);
    }
...
fn parseFlag(mut inFlag: ArcStr, mut inFlags: Flags::Flag, mut restArgs: Arc<metamodelica::List<ArcStr>>, mut inFlagPrefix: ArcStr) -> Result<Arc<metamodelica::List<ArcStr>>> {
    ...
    missing_value = stringEmpty((value.clone()).clone()) && !(StringUtil::endsWith((inFlag).clone(), (literal!("=")).clone()));
    restArgs = parseConfigFlag((flag).clone(), (value).clone(), inFlags, restArgs, (inFlagPrefix).clone(), missing_value)?;
    Ok(restArgs)
}
...
fn parseConfigFlag(mut inFlag: ArcStr, mut inValue: ArcStr, mut inFlags: Flags::Flag, mut restArgs: Arc<metamodelica::List<ArcStr>>, mut inFlagPrefix: ArcStr, mut missingValue: bool) -> Result<Arc<metamodelica::List<ArcStr>>> {
    ...
    let mut value: ArcStr;
    config_flag = lookupConfigFlag((inFlag).clone(), (inFlagPrefix).clone())?;
    if missingValue && flagRequiresValue(config_flag.clone()) && !(restArgs.clone().is_empty()) && !(StringUtil::startsWith((listHead(restArgs.clone())?).clone(), (literal!("-")).clone())) {
        value = (listHead(restArgs.clone())?).clone();
        restArgs = listRest(restArgs)?;
    } else {
        value = (inValue).clone();
    }
    evaluateConfigFlag(config_flag, (value).clone(), inFlags)?;
    Ok(restArgs)
}
...
fn evaluateConfigFlag(mut inFlag: Flags::ConfigFlag, mut inValue: ArcStr, mut inFlags: Flags::Flag) -> Result<()> {
    let () = (match (inFlag.clone(), inFlags) {
        (Flags::ConfigFlag { index: 1, .. }, Flags::Flag::FLAGS { debugFlags: mut debug_flags, .. }) => {
            List::map1_0(splitCSV((inValue).clone()), (std::sync::Arc::new(setDebugFlag) as std::sync::Arc<dyn ::std::ops::Fn(ArcStr, metamodelica::Array<bool>) -> Result<()> + 'static>), debug_flags.clone())?;
            ()
        },
        ...
        (_, _) if (configFlagEq(inFlag.clone(), Flags::PRE_OPT_MODULES_ADD.clone())?) => {
            setAdditionalOptModules(Flags::PRE_OPT_MODULES_ADD.clone(), Flags::PRE_OPT_MODULES_SUB.clone(), splitCSV((inValue).clone()))?;
            ()
        },
        (_, _) if (configFlagEq(inFlag.clone(), Flags::PRE_OPT_MODULES_SUB.clone())?) => {
            setAdditionalOptModules(Flags::PRE_OPT_MODULES_SUB.clone(), Flags::PRE_OPT_MODULES_ADD.clone(), splitCSV((inValue).clone()))?;
            ()
        },
        (_, _) if (configFlagEq(inFlag.clone(), Flags::POST_OPT_MODULES_ADD.clone())?) => {
            setAdditionalOptModules(Flags::POST_OPT_MODULES_ADD.clone(), Flags::POST_OPT_MODULES_SUB.clone(), splitCSV((inValue).clone()))?;
            ()
        },
        (_, _) if (configFlagEq(inFlag.clone(), Flags::POST_OPT_MODULES_SUB.clone())?) => {
            setAdditionalOptModules(Flags::POST_OPT_MODULES_SUB.clone(), Flags::POST_OPT_MODULES_ADD.clone(), splitCSV((inValue).clone()))?;
            ()
        },
        (_, _) if (configFlagEq(inFlag.clone(), Flags::INIT_OPT_MODULES_ADD.clone())?) => {
            setAdditionalOptModules(Flags::INIT_OPT_MODULES_ADD.clone(), Flags::INIT_OPT_MODULES_SUB.clone(), splitCSV((inValue).clone()))?;
            ()
        },
        (_, _) if (configFlagEq(inFlag.clone(), Flags::INIT_OPT_MODULES_SUB.clone())?) => {
            setAdditionalOptModules(Flags::INIT_OPT_MODULES_SUB.clone(), Flags::INIT_OPT_MODULES_ADD.clone(), splitCSV((inValue).clone()))?;
            ()
        },
        (_, Flags::Flag::FLAGS { configFlags: mut config_flags, .. }) => {
            setConfigFlag(inFlag.clone(), config_flags.clone(), (inValue).clone())?;
            ()
        },
        _ => bail!("match: no arm matched"),
    });
...
fn setDebugFlag(mut inFlag: ArcStr, mut inFlags: metamodelica::Array<bool>) -> Result<()> {
    ...
    negated = neg1 || neg2;
    flag_str = (if (negated) {StringUtil::rest((inFlag).clone())?} else {inFlag}).clone();
    flag_str = (if (neg2) {StringUtil::rest((flag_str).clone())?} else {flag_str}).clone();
    setDebugFlag2((flag_str).clone(), !(negated), inFlags.clone())?;
    Ok(())
}
...
fn matchDebugFlag(mut inFlagName: ArcStr, mut inFlag: Flags::DebugFlag) -> Result<bool> {
    ...
    let mut name: ArcStr;
    let Flags::DEBUG_FLAG { name: __pa0, .. } = (inFlag) else { bail!("pattern mismatch") };
    name = __pa0.clone();
    outMatches = stringEq((inFlagName).clone(), (name).clone());
    Ok(outMatches)
}
...
fn matchConfigFlag(mut inFlagName: ArcStr, mut inFlag: Flags::ConfigFlag) -> Result<bool> {
    ...
    let Flags::CONFIG_FLAG { name: __pa0, shortname: __pa1, .. } = (inFlag) else { bail!("pattern mismatch") };
    name = __pa0.clone();
    opt_shortname = __pa1.clone();
    shortname = (Util::getOptionOrDefault(opt_shortname, (literal!("")).clone())).clone();
    outMatches = stringEq((inFlagName.clone()).clone(), (shortname).clone()) || stringEq((System::tolower((inFlagName).clone())).clone(), (System::tolower((name).clone())).clone());
    Ok(outMatches)
}
...
fn setConfigFlag(mut inFlag: Flags::ConfigFlag, mut inConfigData: metamodelica::Array<Flags::FlagData>, mut inValue: ArcStr) -> Result<()> {
    ...
    let Flags::CONFIG_FLAG { name: __pa0, defaultValue: __pa1, validOptions: __pa2, .. } = (inFlag.clone()) else { bail!("pattern mismatch") };
    name = __pa0.clone();
    default_value = __pa1.clone();
    validOptions = __pa2.clone();
    data = stringFlagData((inValue).clone(), default_value, validOptions, (name).clone())?;
...
fn stringFlagData(mut inValue: ArcStr, mut inExpectedType: Flags::FlagData, mut validOptions: Option<Flags::ValidOptions>, mut inName: ArcStr) -> Result<Flags::FlagData> {
    ...
    outValue = 'mc: {
        let __mc_input = (inValue.clone(), inExpectedType.clone(), validOptions);
        if let Ok(__v) = (|| -> Result<_> {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, Flags::FlagData::STRING_FLAG { .. }, Some(options)) => {
                    let mut flags: Arc<metamodelica::List<ArcStr>>;
...
fn configFlagsIsEqualIndex(mut inFlag1: Flags::ConfigFlag, mut inFlag2: Flags::ConfigFlag) -> Result<bool> {
    ...
    let mut index1: i32;
    let mut index2: i32;
    let Flags::CONFIG_FLAG { index: __pa0, .. } = (inFlag1) else { bail!("pattern mismatch") };
    index1 = __pa0.clone();
    let Flags::CONFIG_FLAG { index: __pa1, .. } = (inFlag2) else { bail!("pattern mismatch") };
    index2 = __pa1.clone();
...
fn handleDeprecatedFlags() -> Result<()> {
    let mut remaining_flags: Arc<metamodelica::List<ArcStr>>;
...
fn applySideEffects(mut inFlag: Flags::ConfigFlag, mut inValue: Flags::FlagData) -> () {
    let () = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            let _ = __mc_input.clone() else { bail!("nomatch") };
            let mut corba_objid_path: ArcStr;
...
pub(crate) fn setConfigValue(mut inFlag: Flags::ConfigFlag, mut inValue: Flags::FlagData) -> Result<()> {
    ...
    let mut flags: Flags::Flag;
    flags = loadFlags(true)?;
    let Flags::FLAGS { debugFlags: __pa0, configFlags: __pa1 } = (flags) else { bail!("pattern mismatch") };
    debug_flags = __pa0.clone();
    config_flags = __pa1.clone();
...
pub fn setConfigString(mut inFlag: Flags::ConfigFlag, mut inValue: ArcStr) -> Result<()> {
    setConfigValue(inFlag, Flags::FlagData::STRING_FLAG { data: (inValue).clone() })?;
    Ok(())
}
...
pub fn appendConfigStringList(mut flag: Flags::ConfigFlag, mut value: ArcStr) -> Result<Arc<metamodelica::List<ArcStr>>> {
    let mut oldValues: Arc<metamodelica::List<ArcStr>>;
    oldValues = Flags::getConfigStringList(flag.clone())?;
    if !(listMember((value.clone()).clone(), oldValues.clone())) {
        setConfigStringList(flag, metamodelica::cons((value).clone(), oldValues.clone()))?;
    }
    Ok(oldValues)
}

pub fn setConfigEnum(mut inFlag: Flags::ConfigFlag, mut inValue: i32) -> Result<()> {
    let mut valid_values: Arc<metamodelica::List<(ArcStr, i32)>>;
    ...
    setConfigValue(inFlag, Flags::FlagData::ENUM_FLAG { data: inValue, validValues: valid_values })?;
    Ok(())
}
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Deref @ "topics", tail: Deref @ metamodelica::List::Nil } => {
                    ...
                    let mut topics: Arc<metamodelica::List<(ArcStr, ArcStr)>>;
                    let mut help: ArcStr = help.clone();
                    topics = list![(literal!("omc"), literal!("The command-line options available for omc.")), (literal!("debug"), literal!("Flags that enable debugging, diagnostics, and research prototypes.")), (literal!("optmodules"), literal!("Flags that determine which symbolic methods are used to produce the causalized equation system.")), (literal!("simulation"), literal!("The command-line options available for simulation executables generated by OpenModelica.")), (literal!("<flagname>"), literal!("Displays option descriptions for multi-option flag <flagname>.")), (literal!("topics"), literal!("This help-text."))];
                    r#str = (literal!("The available topics (help(\"topics\")) are as follows:\n")).clone();
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Deref @ "simulation", tail: Deref @ metamodelica::List::Nil } => {
                    let mut help: ArcStr = help.clone();
                    help = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("The simulation executable takes the following flags:\n\n")); __mm_s.push_str(&*System::getSimulationHelpText(true, false)); ArcStr::from(__mm_s) }).clone();
                    Ok((help.clone(), help.clone()))
                }
                _ => bail!("nomatch"),
            }}
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Deref @ "simulation-sphinxoutput", tail: Deref @ metamodelica::List::Nil } => {
                    let mut help: ArcStr = help.clone();
                    help = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("The simulation executable takes the following flags:\n\n")); __mm_s.push_str(&*System::getSimulationHelpText(true, true)); ArcStr::from(__mm_s) }).clone();
                    Ok((help.clone(), help.clone()))
                }
                _ => bail!("nomatch"),
            }}
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Deref @ "debug", tail: Deref @ metamodelica::List::Nil } => {
                    ...
                    let mut str2: ArcStr;
                    let mut help: ArcStr = help.clone();
                    str1 = (literal!("The debug flag takes a comma-separated list of flags which are used by the\ncompiler for debugging or experimental purposes.\nFlags prefixed with \"-\" or \"no\" will be disabled.\n")).clone();
                    str2 = (literal!("The available flags are (+ are enabled by default, - are disabled):\n\n")).clone();
                    strs = ({
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Deref @ "optmodules", tail: Deref @ metamodelica::List::Nil } => {
                    ...
                    let mut str1: ArcStr;
                    let mut str1a: ArcStr;
                    let mut str1b: ArcStr;
                    let mut str2: ArcStr;
                    let mut str3: ArcStr;
                    let mut str3a: ArcStr;
                    let mut str3b: ArcStr;
                    let mut str4: ArcStr;
                    let mut str5: ArcStr;
                    let mut str5a: ArcStr;
                    let mut str5b: ArcStr;
                    let mut str6: ArcStr;
                    let mut str7: ArcStr;
                    let mut str7a: ArcStr;
                    let mut str7b: ArcStr;
                    let mut str8: ArcStr;
                    let mut str9: ArcStr;
                    let mut str9a: ArcStr;
                    let mut str9b: ArcStr;
                    let mut str10: ArcStr;
                    let mut help: ArcStr = help.clone();
                    str1 = (literal!("The --preOptModules flag sets the optimization modules which are used before the\nmatching and index reduction in the back end. These modules are specified as a comma-separated list.")).clone();
                    ...
                    data = __pa0.clone();
                    str1a = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("The modules used by default are:")); __mm_s.push_str(&*literal!("\n--preOptModules=")); __mm_s.push_str(&*stringDelimitList(data.clone(), (literal!(",")).clone())); ArcStr::from(__mm_s) }).clone();
                    str1b = (literal!("The valid modules are:")).clone();
                    str2 = (printFlagValidOptionsDesc(Flags::PRE_OPT_MODULES.clone())?).clone();
                    str3 = (literal!("The --matchingAlgorithm sets the method that is used for the matching algorithm, after the pre optimization modules.")).clone();
                    ...
                    str3a = __pa1.clone();
                    str3a = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("The method used by default is:")); __mm_s.push_str(&*literal!("\n--matchingAlgorithm=")); __mm_s.push_str(&*str3a.clone()); ArcStr::from(__mm_s) }).clone();
                    str3b = (literal!("The valid methods are:")).clone();
                    str4 = (printFlagValidOptionsDesc(Flags::MATCHING_ALGORITHM.clone())?).clone();
                    str5 = (literal!("The --indexReductionMethod sets the method that is used for the index reduction, after the pre optimization modules.")).clone();
                    ...
                    str5a = __pa2.clone();
                    str5a = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("The method used by default is:")); __mm_s.push_str(&*literal!("\n--indexReductionMethod=")); __mm_s.push_str(&*str5a.clone()); ArcStr::from(__mm_s) }).clone();
                    str5b = (literal!("The valid methods are:")).clone();
                    str6 = (printFlagValidOptionsDesc(Flags::INDEX_REDUCTION_METHOD.clone())?).clone();
                    str7 = (literal!("The --initOptModules then sets the optimization modules which are used after the index reduction to optimize the system for initialization, specified as a comma-separated list.")).clone();
                    ...
                    data = __pa3.clone();
                    str7a = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("The modules used by default are:")); __mm_s.push_str(&*literal!("\n--initOptModules=")); __mm_s.push_str(&*stringDelimitList(data.clone(), (literal!(",")).clone())); ArcStr::from(__mm_s) }).clone();
                    str7b = (literal!("The valid modules are:")).clone();
                    str8 = (printFlagValidOptionsDesc(Flags::INIT_OPT_MODULES.clone())?).clone();
                    str9 = (literal!("The --postOptModules then sets the optimization modules which are used after the index reduction to optimize the system for simulation, specified as a comma-separated list.")).clone();
                    ...
                    data = __pa4.clone();
                    str9a = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("The modules used by default are:")); __mm_s.push_str(&*literal!("\n--postOptModules=")); __mm_s.push_str(&*stringDelimitList(data.clone(), (literal!(",")).clone())); ArcStr::from(__mm_s) }).clone();
                    str9b = (literal!("The valid modules are:")).clone();
        ...
        if let Ok((__v, __wb0)) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: r#str, tail: Deref @ metamodelica::List::Nil } => {
                    ...
                    let mut help: ArcStr = help.clone();
                    let ref __pa2 @ Flags::CONFIG_FLAG { name: ref __pa0, description: ref __pa1, .. } = (List::getMemberOnTrue((r#str.clone()).clone(), allConfigFlags.clone(), (std::sync::Arc::new(matchConfigFlag) as std::sync::Arc<dyn ::std::ops::Fn(ArcStr, Flags::ConfigFlag) -> Result<bool> + 'static>))?) else { bail!("pattern mismatch") };
                    name = __pa0.clone();
                    desc = __pa1.clone();
                    ...
                    str2 = stringAppendList(StringUtil::wordWrap((desc.clone()).clone(), System::getTerminalWidth(), (literal!("\n")).clone(), metamodelica::OrderedFloat(0.3_f64))?);
...
pub fn getValidOptionsAndDescription(mut flagName: ArcStr) -> Result<(Arc<metamodelica::List<ArcStr>>, ArcStr, Arc<metamodelica::List<ArcStr>>)> {
    let mut validStrings: Arc<metamodelica::List<ArcStr>>;
    let mut mainDescriptionStr: ArcStr;
    let mut descriptions: Arc<metamodelica::List<ArcStr>>;
    let mut validOptions: Flags::ValidOptions;
    let mut mainDescription: ArcStr;
    let (__pa0, __pa1) = ::match_deref::match_deref! { match &(List::getMemberOnTrue((flagName).clone(), allConfigFlags.clone(), (std::sync::Arc::new(matchConfigFlag) as std::sync::Arc<dyn ::std::ops::Fn(ArcStr, Flags::ConfigFlag) -> Result<bool> + 'static>))?) {
        Flags::ConfigFlag { description: __pa0, validOptions: Some(__pa1), .. } => (__pa0.clone(), __pa1.clone()),
        _ => bail!("pattern mismatch"),
    } };
    mainDescription = __pa0.clone();
    validOptions = __pa1.clone();
    mainDescriptionStr = (mainDescription).clone();
...
fn getValidOptionsAndDescription2(mut validOptions: Flags::ValidOptions) -> Result<(Arc<metamodelica::List<ArcStr>>, Arc<metamodelica::List<ArcStr>>)> {
    ...
    (validStrings, descriptions) = (match validOptions {
        ...
        Flags::ValidOptions::STRING_DESC_OPTION { options: mut options } => {
            ...
            descriptions = List::map(options.clone(), std::sync::Arc::new(fnptr!(Util::tuple22, _)))?;
            (validStrings.clone(), descriptions)
        },
    });
...
fn compareDebugFlags(mut flag1: Flags::DebugFlag, mut flag2: Flags::DebugFlag) -> Result<bool> {
    ...
    let Flags::DEBUG_FLAG { name: __pa0, .. } = (flag1) else { bail!("pattern mismatch") };
    name1 = __pa0.clone();
    let Flags::DEBUG_FLAG { name: __pa1, .. } = (flag2) else { bail!("pattern mismatch") };
    name2 = __pa1.clone();
    b = stringCompare((name1).clone(), (name2).clone()) > 0;
    Ok(b)
}
...
fn makeTopicString(mut topic: (ArcStr, ArcStr)) -> Result<ArcStr> {
    ...
    let mut str2: ArcStr;
    (str1, str2) = topic;
    str1 = (Util::stringPadRight((str1).clone(), 13, (literal!(" ")).clone())).clone();
    r#str = stringAppendList(StringUtil::wordWrap(({ let mut __mm_s = String::new(); __mm_s.push_str(&*str1); __mm_s.push_str(&*str2); ArcStr::from(__mm_s) }).clone(), System::getTerminalWidth(), (literal!("\n               ")).clone(), metamodelica::OrderedFloat(0.3_f64))?);
    Ok(r#str)
}
...
pub fn printUsage() -> Result<ArcStr> {
    ...
    Print::printBuf((literal!("Distributed under OMSC-PL and GPL, see www.openmodelica.org\n\n")).clone())?;
    Print::printBuf((literal!("Usage: omc [Options] (Model.mo | Script.mos) [Libraries | .mo-files]\n* Libraries: Fully qualified names of libraries to load before processing Model or Script.\n             The libraries should be separated by spaces: Lib1 Lib2 ... LibN.\n")).clone())?;
    Print::printBuf((literal!("\n* Options:\n")).clone())?;
    ...
    Print::printBuf((literal!("  omc Model.mo             will produce flattened Model on standard output.\n")).clone())?;
    Print::printBuf((literal!("  omc -s Model.mo          will produce simulation code for the model:\n")).clone())?;
    Print::printBuf((literal!("                            * Model.c           The model C code.\n")).clone())?;
    Print::printBuf((literal!("                            * Model_functions.c The model functions C code.\n")).clone())?;
    Print::printBuf((literal!("                            * Model.makefile    The makefile to compile the model.\n")).clone())?;
    Print::printBuf((literal!("                            * Model_init.xml    The initial values.\n")).clone())?;
    Print::printBuf((literal!("  omc Script.mos           will run the commands from Script.mos.\n")).clone())?;
    Print::printBuf((literal!("  omc Model.mo Modelica    will first load the Modelica library and then produce\n                            flattened Model on standard output.\n")).clone())?;
    Print::printBuf((literal!("  omc Model1.mo Model2.mo  will load both Model1.mo and Model2.mo, and produce\n                            flattened Model1 on standard output.\n")).clone())?;
    ...
    Print::printBuf((literal!("Documentation is available in the built-in package OpenModelica.Scripting or\nonline <https://build.openmodelica.org/Documentation/OpenModelica.Scripting.html>.\n")).clone())?;
...
pub(crate) fn printUsageSphinxAll() -> Result<ArcStr> {
    ...
    Print::printBuf((({
        ...
        for mut e in (1..=((s).clone().len() as i32)).into_iter() {
            let __x = literal!("=");
            __acc.push_str(&__x);
        }
        ArcStr::from(__acc)
    ...
    Print::printBuf((literal!("\n\n")).clone())?;
    Print::printBuf((literal!("The debug flag takes a comma-separated list of flags which are used by the\ncompiler for debugging or experimental purposes.\nFlags prefixed with \"-\" or \"no\" will be disabled.\n")).clone())?;
    Print::printBuf((literal!("The available flags are (+ are enabled by default, - are disabled):\n\n")).clone())?;
    ...
    s = (literal!("Flags for Optimization Modules")).clone();
    Print::printBuf((s.clone()).clone())?;
    Print::printBuf((literal!("\n")).clone())?;
    Print::printBuf((({
        let mut __acc = String::new();
    ...
    Print::printBuf((literal!("The :ref:`--preOptModules <omcflag-preOptModules>` flag sets the optimization modules which are used before the\nmatching and index reduction in the back end. These modules are specified as a comma-separated list.")).clone())?;
    Print::printBuf((literal!("\n\n")).clone())?;
    Print::printBuf((literal!("The :ref:`--matchingAlgorithm <omcflag-matchingAlgorithm>` sets the method that is used for the matching algorithm, after the pre optimization modules.")).clone())?;
    Print::printBuf((literal!("\n\n")).clone())?;
    Print::printBuf((literal!("The :ref:`--indexReductionMethod <omcflag-indexReductionMethod>` sets the method that is used for the index reduction, after the pre optimization modules.")).clone())?;
    Print::printBuf((literal!("\n\n")).clone())?;
    Print::printBuf((literal!("The :ref:`--initOptModules <omcflag-initOptModules>` then sets the optimization modules which are used after the index reduction to optimize the system for initialization, specified as a comma-separated list.")).clone())?;
    Print::printBuf((literal!("\n\n")).clone())?;
    Print::printBuf((literal!("The :ref:`--postOptModules <omcflag-postOptModules>` then sets the optimization modules which are used after the index reduction to optimize the system for simulation, specified as a comma-separated list.")).clone())?;
    Print::printBuf((literal!("\n\n")).clone())?;
    usage = (Print::getString()?).clone();
    Print::clearBuf();
    Ok(usage)
...
fn printConfigFlagName(mut inFlag: Flags::ConfigFlag, mut sphinx: bool) -> Result<(ArcStr, ArcStr)> {
    ...
    (outString, longName) = (match inFlag {
        Flags::ConfigFlag { name: mut name, shortname: Some(mut shortname), .. } => {
            shortname = (if (sphinx) {{ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("-")); __mm_s.push_str(&*shortname.clone()); ArcStr::from(__mm_s) }} else {Util::stringPadLeft(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("-")); __mm_s.push_str(&*shortname.clone()); ArcStr::from(__mm_s) }).clone(), 4, (literal!(" ")).clone())}).clone();
            (stringAppendList(list![(shortname.clone()).clone(), (literal!(", --")).clone(), (name.clone()).clone()]), name.clone())
        },
        Flags::ConfigFlag { name: mut name, shortname: None, .. } => {
            ({ let mut __mm_s = String::new(); __mm_s.push_str(&*if (sphinx) {literal!("--")} else {literal!("      --")}); __mm_s.push_str(&*name.clone()); ArcStr::from(__mm_s) }, name.clone())
        },
        _ => bail!("match: no arm matched"),
    });
...
fn printValidOptions(mut inFlag: Flags::ConfigFlag) -> Result<ArcStr> {
    ...
    outString = ((match inFlag {
        ...
        Flags::ConfigFlag { validOptions: Some(Flags::ValidOptions::STRING_DESC_OPTION { options: ref descl }), .. } => {
            let mut opt_str: ArcStr;
            opt_str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("\n")); __mm_s.push_str(&*arcstr::literal!(descriptionIndent)); __mm_s.push_str(&*literal!("   ")); __mm_s.push_str(&*literal!("Valid options:")); __mm_s.push_str(&*literal!("\n")); __mm_s.push_str(&*stringAppendList(({
        let mut __acc: Arc<metamodelica::List<ArcStr>> = metamodelica::nil();
...
fn printValidOptionsSphinx(mut inFlag: Flags::ConfigFlag) -> Result<ArcStr> {
    ...
    outString = ((match inFlag.clone() {
        ...
        Flags::ConfigFlag { validOptions: Some(Flags::ValidOptions::STRING_OPTION { options: ref strl }), .. } => {
            let mut opt_str: ArcStr;
            opt_str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("\n")); __mm_s.push_str(&*defaultFlagSphinx(inFlag.defaultValue.clone())); __mm_s.push_str(&*literal!(" ")); __mm_s.push_str(&*literal!("Valid options")); __mm_s.push_str(&*literal!(":\n\n")); __mm_s.push_str(&*({
        let mut __acc = String::new();
...
fn defaultFlagSphinx(mut flag: Flags::FlagData) -> ArcStr {
    ...
    r#str = ((::match_deref::match_deref! { match &(flag.clone()) {
        ...
        Flags::FlagData::STRING_FLAG { .. } => {
            { let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("String (default")); __mm_s.push_str(&*literal!(" ")); __mm_s.push_str(&*var_field!(flag.data, Flags::FlagData::STRING_FLAG).clone()); __mm_s.push_str(&*literal!(").")); ArcStr::from(__mm_s) }
        },
        Flags::FlagData::STRING_LIST_FLAG { data: Deref @ metamodelica::List::Nil } => {
            literal!("String list (default *empty*).")
        },
        Flags::FlagData::STRING_LIST_FLAG { .. } => {
            { let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("String list (default")); __mm_s.push_str(&*literal!(" ")); __mm_s.push_str(&*stringDelimitList(var_field!(flag.data, Flags::FlagData::STRING_LIST_FLAG).clone(), (literal!(",")).clone())); __mm_s.push_str(&*literal!(").")); ArcStr::from(__mm_s) }
        },
...
fn printFlagOptionDescShort(mut inOption: (ArcStr, ArcStr), mut sphinx: bool) -> ArcStr {
    ...
    let mut name: ArcStr;
    (name, _) = inOption;
    outString = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*if (sphinx) {literal!("* ")} else {{ let mut __mm_s = String::new(); __mm_s.push_str(&*arcstr::literal!(descriptionIndent)); __mm_s.push_str(&*literal!("    * ")); ArcStr::from(__mm_s) }}); __mm_s.push_str(&*name); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone();
    outString
}

fn printFlagValidOptionsDesc(mut inFlag: Flags::ConfigFlag) -> Result<ArcStr> {
    let mut outString: ArcStr;
    let mut options: Arc<metamodelica::List<(ArcStr, ArcStr)>>;
    let __pa0 = ::match_deref::match_deref! { match &(inFlag) {
    ...
    outString = (({
        let mut __acc = String::new();
        for mut o in (options).into_iter().cloned() {
...
fn sphinxMathMode(mut s: ArcStr) -> Result<ArcStr> {
    ...
    if i == 4 {
        ...
        s2 = __pa1.clone();
        s3 = __pa2.clone();
        o = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*s1); __mm_s.push_str(&*literal!(" :math:`")); __mm_s.push_str(&*s2); __mm_s.push_str(&*literal!("` ")); __mm_s.push_str(&*s3); ArcStr::from(__mm_s) }).clone();
    }
    Ok(o)
}
...
fn removeSphinxMathMode(mut s: ArcStr) -> Result<ArcStr> {
    ...
    if i == 4 {
        o = (removeSphinxMathMode(stringAppendList(listRest(strs)?))?).clone();
    }
    Ok(o)
}
...
fn printFlagOptionDesc(mut inOption: (ArcStr, ArcStr), mut sphinx: bool) -> Result<ArcStr> {
    ...
    if sphinx {
        desc_str = (({
        let mut __acc = String::new();
        for mut s in (System::strtok((desc_str).clone(), (literal!("\n")).clone())).into_iter().cloned() {
            let __x = System::trim((s.clone()).clone(), (literal!(" \u{c}\n\r\t\u{b}")).clone());
            __acc.push_str(&__x);
        }
        ArcStr::from(__acc)
    })).clone();
        outString = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("* ")); __mm_s.push_str(&*name); __mm_s.push_str(&*literal!(" (")); __mm_s.push_str(&*desc_str); __mm_s.push_str(&*literal!(")\n")); ArcStr::from(__mm_s) }).clone();
    } else {
        r#str = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*Util::stringPadRight(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!(" * ")); __mm_s.push_str(&*name); __mm_s.push_str(&*literal!(" ")); ArcStr::from(__mm_s) }).clone(), 30, (literal!(" ")).clone())); __mm_s.push_str(&*removeSphinxMathMode((desc_str).clone())?); ArcStr::from(__mm_s) }).clone();
        outString = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*stringDelimitList(StringUtil::wordWrap((r#str).clone(), System::getTerminalWidth(), ({ let mut __mm_s = String::new(); __mm_s.push_str(&*arcstr::literal!(descriptionIndent)); __mm_s.push_str(&*literal!("    ")); ArcStr::from(__mm_s) }).clone(), metamodelica::OrderedFloat(0.3_f64))?, (literal!("\n")).clone())); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone();
    }
    Ok(outString)
}
...
fn printDebugFlag(mut inFlag: Flags::DebugFlag, mut sphinx: bool) -> Result<ArcStr> {
    ...
    let Flags::DEBUG_FLAG { default: __pa0, name: __pa1, description: __pa2, .. } = (inFlag) else { bail!("pattern mismatch") };
    default = __pa0.clone();
    name = __pa1.clone();
    desc = __pa2.clone();
    desc_str = (desc).clone();
    if sphinx {
        ...
        outString = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("\n.. _omcflag-debug-")); __mm_s.push_str(&*name.clone()); __mm_s.push_str(&*literal!(":\n\n")); __mm_s.push_str(&*literal!(":ref:`")); __mm_s.push_str(&*name.clone()); __mm_s.push_str(&*literal!(" <omcflag-debug-")); __mm_s.push_str(&*name); __mm_s.push_str(&*literal!(">`")); __mm_s.push_str(&*literal!(" (default: ")); __mm_s.push_str(&*if (default) {literal!("on")} else {literal!("off")}); __mm_s.push_str(&*literal!(")\n  ")); __mm_s.push_str(&*desc_str); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone();
    } else {
        outString = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*Util::stringPadRight(({ let mut __mm_s = String::new(); __mm_s.push_str(&*if (default) {literal!(" + ")} else {literal!(" - ")}); __mm_s.push_str(&*name); __mm_s.push_str(&*literal!(" ")); ArcStr::from(__mm_s) }).clone(), 26, (literal!(" ")).clone())); __mm_s.push_str(&*removeSphinxMathMode((desc_str).clone())?); ArcStr::from(__mm_s) }).clone();
        outString = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*stringDelimitList(StringUtil::wordWrap((outString).clone(), System::getTerminalWidth(), (arcstr::literal!(descriptionIndent)).clone(), metamodelica::OrderedFloat(0.3_f64))?, (literal!("\n")).clone())); __mm_s.push_str(&*literal!("\n")); ArcStr::from(__mm_s) }).clone();
    }
    Ok(outString)
}
...
pub fn configFlagName(mut inFlag: Flags::ConfigFlag) -> Result<ArcStr> {
    let mut name: ArcStr;
    let Flags::CONFIG_FLAG { name: __pa0, .. } = (inFlag) else { bail!("pattern mismatch") };
    name = __pa0.clone();
    Ok(name)
}
...
pub(crate) fn flagDataEq(mut data1: Flags::FlagData, mut data2: Flags::FlagData) -> Result<bool> {
    ...
    eq = (match (data1.clone(), data2.clone()) {
        ...
        (Flags::FlagData::ENUM_FLAG { .. }, Flags::FlagData::ENUM_FLAG { .. }) => metamodelica::ReferenceEq::reference_eq(&*(var_field!(data1.validValues, Flags::FlagData::ENUM_FLAG).clone()), &*(var_field!(data2.validValues, Flags::FlagData::ENUM_FLAG).clone())) && var_field!(data1.data, Flags::FlagData::ENUM_FLAG).clone() == var_field!(data2.data, Flags::FlagData::ENUM_FLAG).clone(),
        _ => false,
    });
    Ok(eq)
}
...
pub fn unparseFlags() -> Result<Arc<metamodelica::List<ArcStr>>> {
    ...
    let mut fvalue: bool;
    if let Ok(Flags::FLAGS { debugFlags: __pa0, configFlags: __pa1 }) = loadFlags(false) {
        debug_flags = __pa0.clone();
        config_flags = __pa1.clone();
    } else {
    ...
    for mut f in &*allConfigFlags.clone() {
        ...
        Some(mut __esc_name) => {
            ...
            { let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("-")); __mm_s.push_str(&*name.clone()); ArcStr::from(__mm_s) }
        },
    ...
    for mut f in &*allDebugFlags.clone() {
        ...
        if f.default.clone() != fvalue {
            name = (if (fvalue) {f.name.clone()} else {{ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("no")); __mm_s.push_str(&*f.name.clone()); ArcStr::from(__mm_s) }}).clone();
            strl = metamodelica::cons((name.clone()).clone(), strl.clone());
        }
    }
    if !(strl.clone().is_empty()) {
        flagStrings = metamodelica::cons(({ let mut __mm_s = String::new(); __mm_s.push_str(&*literal!("-d=")); __mm_s.push_str(&*stringDelimitList(strl, (literal!(",")).clone())); ArcStr::from(__mm_s) }).clone(), flagStrings);
    }
    Ok(flagStrings)
}
