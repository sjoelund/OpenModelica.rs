...
impl metamodelica::gc::MMTrace for ExpressionBase {
    fn mm_accept(&self, __mmv: &mut dyn metamodelica::gc::MMVisitor) -> Result<(), ()> {
        match self {
            ...
            ExpressionBase::CONDITION { isNot, lhsExp, rhsValue, trueBranch, elseBranch } => {
            ...
            ExpressionBase::MAP { argExp, ofBinding, mapExp, hasIndexIdentOpt } => {
...
impl metamodelica::gc::MMTrace for TypeInfo {
    fn mm_accept(&self, __mmv: &mut dyn metamodelica::gc::MMVisitor) -> Result<(), ()> {
        match self {
            ...
            TypeInfo::TI_FUN_TYPE { inArgs, outArgs, tyVars } => {
...
impl metamodelica::gc::MMTrace for MMDeclaration {
    fn mm_accept(&self, __mmv: &mut dyn metamodelica::gc::MMVisitor) -> Result<(), ()> {
        match self {
            ...
            MMDeclaration::MM_FUN { isPublic, name, inArgs, outArgs, locals, statements, genInfoOpt } => {
                metamodelica::gc::MMTrace::mm_accept(isPublic, __mmv)?;
                metamodelica::gc::MMTrace::mm_accept(name, __mmv)?;
                metamodelica::gc::MMTrace::mm_accept(inArgs, __mmv)?;
                metamodelica::gc::MMTrace::mm_accept(outArgs, __mmv)?;
...
impl metamodelica::gc::MMTrace for MMExp {
    fn mm_accept(&self, __mmv: &mut dyn metamodelica::gc::MMVisitor) -> Result<(), ()> {
        match self {
            ...
            MMExp::MM_FOR_LOOP { idxName, arrName, eltName, statements } => {
            ...
            MMExp::MM_LIST_FOR_LOOP { eltName, listName, matchLocals, matchCases } => {
                metamodelica::gc::MMTrace::mm_accept(eltName, __mmv)?;
...
impl metamodelica::gc::MMTrace for Scope {
    fn mm_accept(&self, __mmv: &mut dyn metamodelica::gc::MMVisitor) -> Result<(), ()> {
        match self {
            ...
            Scope::CASE_SCOPE { mExp, mType, localNames, accLocals, extArgs, matchArgName, hasImplicitScope } => {
            ...
            Scope::LET_SCOPE { ident, idType, freshIdent, isUsed } => {
                metamodelica::gc::MMTrace::mm_accept(ident, __mmv)?;
...
pub(crate) fn transformAST(mut inTplPackage: TemplPackage) -> Result<MMPackage> {
    ...
    outMMPackage = (match inTplPackage.clone() {
        _ => {
            ...
            let mut templateDefs: Arc<metamodelica::List<(ArcStr, TemplateDef)>>;
            let mut mmDeclarations: Arc<metamodelica::List<MMDeclaration>>;
            let mut tp: TemplPackage;
            let mut astDefs: Arc<metamodelica::List<ASTDef>>;
...
pub(crate) fn importDeclarations(mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<Arc<metamodelica::List<MMDeclaration>>> {
    ...
    for mut astDef in &*inASTDefs {
        ...
        let ASTDef { importPackage: __pa0, isDefault: __pa1, .. } = (astDef.clone()) else { bail!("pattern mismatch") };
        importPackage = __pa0.clone();
        isDefault = __pa1.clone();
        outMMDecls = metamodelica::cons(MMDeclaration::MM_IMPORT { isPublic: isDefault, packageName: importPackage.clone() }, outMMDecls.clone());
    }
    Ok(outMMDecls)
}

pub(crate) fn transformTemplateDefs(mut inTemplateDefsRest: Arc<metamodelica::List<(ArcStr, TemplateDef)>>, mut inTplPackage: TemplPackage, mut inAccMMDecls: Arc<metamodelica::List<MMDeclaration>>) -> Result<Arc<metamodelica::List<MMDeclaration>>> {
    '__tco: loop {
        ...
        (Deref @ metamodelica::List::Cons { head: (tplname, TemplateDef::STR_TOKEN_DEF { value: stvalue }), tail: restTDefs }, tplPackage, accMMDecls) => {
            let mut mmDecls: Arc<metamodelica::List<MMDeclaration>>;
        ...
        (Deref @ metamodelica::List::Cons { head: (tplname, TemplateDef::TEMPLATE_DEF { args: targs, exp: texp, .. }), tail: restTDefs }, tplPackage, accMMDecls) => {
            ...
            let mut stmts: Arc<metamodelica::List<Arc<MMExp>>>;
...
pub(crate) fn inlineLastFunIfSingleCall(mut inInArgs: TypedIdents, mut inOutArgs: TypedIdents, mut inStmts: Arc<metamodelica::List<Arc<MMExp>>>, mut inLocals: TypedIdents, mut inAccMMDecls: Arc<metamodelica::List<MMDeclaration>>) -> Result<(Arc<metamodelica::List<Arc<MMExp>>>, TypedIdents, Arc<metamodelica::List<MMDeclaration>>)> {
    let mut outStmts: Arc<metamodelica::List<Arc<MMExp>>>;
    let mut outLocals: TypedIdents;
    let mut outMMDecls: Arc<metamodelica::List<MMDeclaration>>;
...
pub(crate) fn encodeIdent(mut inIdent: Ident, mut prefix: Ident) -> Result<Ident> {
    ...
    outIdent = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*prefix); __mm_s.push_str(&*encodeIdentNoPrefix((inIdent).clone())?); ArcStr::from(__mm_s) }).clone();
    Ok(outIdent)
}
...
pub(crate) fn encodePathIdent(mut inPath: Arc<PathIdent>, mut prefix: Ident) -> Result<Ident> {
    let mut outEncIdent: Ident;
    outEncIdent = (encodeIdent((pathIdentString(inPath)?).clone(), (prefix).clone())?).clone();
    Ok(outEncIdent)
}

pub(crate) fn encodeTypedIdent(mut inTypedIdent: (ArcStr, Arc<TypeSignature>), mut prefix: Ident) -> Result<(ArcStr, Arc<TypeSignature>)> {
...
pub(crate) fn addOutPrefixes(mut inStmts: Arc<metamodelica::List<Arc<MMExp>>>, mut inTextArgs: TypedIdents, mut inTranslatedTextArgs: Arc<metamodelica::List<(ArcStr, ArcStr)>>) -> Result<Arc<metamodelica::List<Arc<MMExp>>>> {
    let mut outStmts: Arc<metamodelica::List<Arc<MMExp>>>;
    outStmts = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: Deref @ MMExp::MM_ASSIGN { lhsArgs: largs, rhs }, tail: stmts }, txtargs, trIdents) => {
...
pub(crate) fn addOutPrefixesRhs(mut inStmt: Arc<MMExp>, mut inTranslatedTextArgs: Arc<metamodelica::List<(ArcStr, ArcStr)>>) -> Arc<MMExp> {
    let mut outStmt: Arc<MMExp>;
    outStmt = 'mc: {
        let __mc_input = (inStmt.clone(), inTranslatedTextArgs);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
pub(crate) fn addOutPrefixesLhs(mut inLhsArgs: Arc<metamodelica::List<ArcStr>>, mut inTextArgs: TypedIdents, mut inTranslatedTextArgs: Arc<metamodelica::List<(ArcStr, ArcStr)>>) -> Result<(Arc<metamodelica::List<ArcStr>>, Arc<metamodelica::List<(ArcStr, ArcStr)>>)> {
    let mut outLhsArgs: Arc<metamodelica::List<ArcStr>>;
    let mut outTranslatedTextArgs: Arc<metamodelica::List<(ArcStr, ArcStr)>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: ident, tail: largs }, txtargs, trIdents) => {
                    ...
                    (largs, trIdents) = addOutPrefixesLhs(largs.clone(), txtargs.clone(), trIdents.clone())?;
                    Ok((metamodelica::cons((ident.clone()).clone(), largs.clone()), trIdents.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
...
pub(crate) fn addOutTextAssigns(mut inTextArgs: TypedIdents, mut inTranslatedTextArgs: Arc<metamodelica::List<(ArcStr, ArcStr)>>) -> Arc<metamodelica::List<Arc<MMExp>>> {
    let mut outStmts: Arc<metamodelica::List<Arc<MMExp>>> = metamodelica::nil();
    let mut outident: ArcStr;
    let mut id: (ArcStr, Arc<TypeSignature>) = (arcstr::literal!(""), Arc::new(TypeSignature::BOOLEAN_TYPE));
    let mut ident: Ident;
    for mut id in &*inTextArgs {
        ...
        if '__try0: {
            ...
            outident = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*arcstr::literal!(outPrefix)); __mm_s.push_str(&*ident.clone()); ArcStr::from(__mm_s) }).clone();
            outStmts = metamodelica::cons(Arc::new(MMExp::MM_ASSIGN { lhsArgs: list![(outident.clone()).clone()], rhs: Arc::new(MMExp::MM_IDENT { ident: Arc::new(PathIdent::IDENT { ident: (ident.clone()).clone() }) }) }), outStmts.clone());
        }
    }
...
pub(crate) fn isAssignedIdent(mut inStatementList: Arc<metamodelica::List<Arc<MMExp>>>, mut inIdent: Ident) -> Result<bool> {
    let mut outIsAssigned: bool;
    let mut largs: Arc<metamodelica::List<ArcStr>>;
...
pub(crate) fn statementsFromExp(mut inExp: Expression, mut inMMEscOptions: Arc<metamodelica::List<(ArcStr, (Arc<MMExp>, Arc<TypeSignature>))>>, mut inStmts: Arc<metamodelica::List<Arc<MMExp>>>, mut inInText: Ident, mut inOutText: Ident, mut inLocals: TypedIdents, mut inScopeEnv: ScopeEnv, mut inTplPackage: TemplPackage, mut inAccMMDecls: Arc<metamodelica::List<MMDeclaration>>) -> Result<(Arc<metamodelica::List<Arc<MMExp>>>, TypedIdents, ScopeEnv, Arc<metamodelica::List<MMDeclaration>>, Ident)> {
    let mut outStmts: Arc<metamodelica::List<Arc<MMExp>>>;
    let mut outLocals: TypedIdents;
    ...
    (outStmts, outLocals, outScopeEnv, outMMDecls, outInText) = 'mc: {
        let __mc_input = (inExp, inMMEscOptions, inStmts, inInText, inOutText, inLocals, inScopeEnv, inTplPackage, inAccMMDecls);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                ((Deref @ ExpressionBase::BOUND_VALUE { boundPath: path }, sinfo), mmopts, stmts, intxt, outtxt, locals, scEnv, tplPackage @ TemplPackage { astDefs, .. }, accMMDecls) => {
                    ...
                    let mut exptype: Arc<TypeSignature>;
                    let mut stmts = (*stmts).clone();
                    let mut intxt = (*intxt).clone();
                    let mut locals = (*locals).clone();
                    let mut scEnv = (*scEnv).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                ((Deref @ ExpressionBase::FUN_CALL { name: fname, args: explst }, sinfo), mmopts, stmts, intxt, outtxt, locals, scEnv, tplPackage @ TemplPackage { astDefs, .. }, accMMDecls) => {
                    let mut stmt: Arc<MMExp>;
                    let mut mmexp: Arc<MMExp>;
                    let mut tyVars: Arc<metamodelica::List<ArcStr>>;
                    let mut iargs: TypedIdents;
                    let mut oargs: TypedIdents;
                    let mut rettype: Arc<TypeSignature>;
                    let mut argvals: Arc<metamodelica::List<(Arc<MMExp>, Arc<TypeSignature>, SourceInfo)>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                ((Deref @ ExpressionBase::CONDITION { isNot: isnot, lhsExp: exp, rhsValue: rhsval, trueBranch: tbranch, elseBranch: ebranch }, sinfo), mmopts, stmts, intxt, outtxt, locals, scEnv, tplPackage @ TemplPackage { astDefs, .. }, accMMDecls) => {
                    ...
                    let mut argval: (Arc<MMExp>, Arc<TypeSignature>, SourceInfo);
                    let mut argvals: Arc<metamodelica::List<(Arc<MMExp>, Arc<TypeSignature>, SourceInfo)>>;
                    let mut mcases: Arc<metamodelica::List<(Arc<MatchingExp>, (Arc<ExpressionBase>, SourceInfo))>>;
                    let mut stmts = (*stmts).clone();
                    let mut intxt = (*intxt).clone();
                    let mut locals = (*locals).clone();
                    let mut scEnv = (*scEnv).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                ((Deref @ ExpressionBase::MAP { argExp: argexp, ofBinding: ofbind, mapExp: mapexp, hasIndexIdentOpt: idxNmOpt }, _), mmopts, stmts, intxt, outtxt, locals, scEnv, tplPackage, accMMDecls) => {
                    let mut argvals: Arc<metamodelica::List<(Arc<MMExp>, Arc<TypeSignature>, SourceInfo)>>;
                    let mut explst: Arc<metamodelica::List<(Arc<ExpressionBase>, SourceInfo)>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                ((Deref @ ExpressionBase::ESCAPED { exp, options: opts }, _), mmopts, stmts, intxt, outtxt, locals, scEnv, tplPackage, accMMDecls) => {
                    let mut popstmts: Arc<metamodelica::List<Arc<MMExp>>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                ((Deref @ ExpressionBase::INDENTATION { width: n, items: explst }, _), mmopts, stmts, intxt, outtxt, locals, scEnv, tplPackage, accMMDecls) => {
                    ...
                    (stmts, locals, scEnv, accMMDecls, _) = statementsFromExpList(explst.clone(), metamodelica::cons(stmt.clone(), stmts.clone()), (outtxt.clone()).clone(), (outtxt.clone()).clone(), locals.clone(), scEnv.clone(), tplPackage.clone(), accMMDecls.clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                ((Deref @ ExpressionBase::LET { letExp: (Deref @ ExpressionBase::TEXT_ADD { name: ident, exp: txtexp }, sinfo2), exp }, _), mmopts, stmts, intxt, outtxt, locals, scEnv, tplPackage, accMMDecls) => {
                    ...
                    scEnv = metamodelica::cons(Scope::RECURSIVE_SCOPE { recIdent: (ident.clone()).clone(), freshIdent: (encIdent.clone()).clone() }, scEnv.clone());
...
pub(crate) fn statementsFromExpList(mut inExpLst: Arc<metamodelica::List<(Arc<ExpressionBase>, SourceInfo)>>, mut inStmts: Arc<metamodelica::List<Arc<MMExp>>>, mut inInText: Ident, mut inOutText: Ident, mut inLocals: TypedIdents, mut inScopeEnv: ScopeEnv, mut inTplPackage: TemplPackage, mut inAccMMDecls: Arc<metamodelica::List<MMDeclaration>>) -> Result<(Arc<metamodelica::List<Arc<MMExp>>>, TypedIdents, ScopeEnv, Arc<metamodelica::List<MMDeclaration>>, Ident)> {
    ...
    let mut outInText: Ident;
    (outStmts, outLocals, outScopeEnv, outMMDecls, outInText) = 'mc: {
        let __mc_input = (inExpLst, inStmts, inInText, inOutText, inLocals, inScopeEnv, inTplPackage, inAccMMDecls);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
pub(crate) fn warnIfSomeOptions(mut inMMEscOptions: Arc<metamodelica::List<(ArcStr, (Arc<MMExp>, Arc<TypeSignature>))>>) -> Result<()> {
    let () = 'mc: {
...
pub(crate) fn statementsFromEscOptions(mut inOptions: Arc<metamodelica::List<(ArcStr, Option<(Arc<ExpressionBase>, SourceInfo)>)>>, mut inAccMMEscOptions: Arc<metamodelica::List<(ArcStr, (Arc<MMExp>, Arc<TypeSignature>))>>, mut inStmts: Arc<metamodelica::List<Arc<MMExp>>>, mut inLocals: TypedIdents, mut inScopeEnv: ScopeEnv, mut inTplPackage: TemplPackage, mut inAccMMDecls: Arc<metamodelica::List<MMDeclaration>>) -> Result<(Arc<metamodelica::List<(ArcStr, (Arc<MMExp>, Arc<TypeSignature>))>>, Arc<metamodelica::List<Arc<MMExp>>>, TypedIdents, ScopeEnv, Arc<metamodelica::List<MMDeclaration>>)> {
    let mut outAccMMEscOptions: Arc<metamodelica::List<(ArcStr, (Arc<MMExp>, Arc<TypeSignature>))>>;
    let mut outStmts: Arc<metamodelica::List<Arc<MMExp>>>;
    let mut outLocals: TypedIdents;
    let mut outScopeEnv: ScopeEnv;
    let mut outMMDecls: Arc<metamodelica::List<MMDeclaration>>;
    (outAccMMEscOptions, outStmts, outLocals, outScopeEnv, outMMDecls) = 'mc: {
        let __mc_input = (inOptions, inAccMMEscOptions, inStmts, inLocals, inScopeEnv, inTplPackage, inAccMMDecls);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: (optid, None), tail: opts }, accMMEscOpts, stmts, locals, scEnv, tplPackage, accMMDecls) => {
                    ...
                    (accMMEscOpts, stmts, locals, scEnv, accMMDecls) = statementsFromEscOptions(opts.clone(), metamodelica::cons((optid.clone(), defoptval.clone()), accMMEscOpts.clone()), stmts.clone(), locals.clone(), scEnv.clone(), tplPackage.clone(), accMMDecls.clone())?;
                    Ok((accMMEscOpts.clone(), stmts.clone(), locals.clone(), scEnv.clone(), accMMDecls.clone()))
                }
                _ => bail!("nomatch"),
            }}
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: (optid, Some(optexp)), tail: opts }, accMMEscOpts, stmts, locals, scEnv, tplPackage @ TemplPackage { astDefs: astdefs, .. }, accMMDecls) => {
                    ...
                    let mut exptype: Arc<TypeSignature>;
                    let mut opttype: Arc<TypeSignature>;
                    ...
                    (accMMEscOpts, stmts, locals, scEnv, accMMDecls) = statementsFromEscOptions(opts.clone(), metamodelica::cons((optid.clone(), (mmarg.clone(), opttype.clone())), accMMEscOpts.clone()), stmts.clone(), locals.clone(), scEnv.clone(), tplPackage.clone(), accMMDecls.clone())?;
                    Ok((accMMEscOpts.clone(), stmts.clone(), locals.clone(), scEnv.clone(), accMMDecls.clone()))
                }
                _ => bail!("nomatch"),
            }}
...
pub(crate) fn getExpListForMap(mut inExp: Expression) -> Arc<metamodelica::List<(Arc<ExpressionBase>, SourceInfo)>> {
    let mut outExpsForMap: Arc<metamodelica::List<(Arc<ExpressionBase>, SourceInfo)>>;
...
pub(crate) fn pushPopBlock(mut inMMEscOptions: Arc<metamodelica::List<(ArcStr, (Arc<MMExp>, Arc<TypeSignature>))>>, mut inOptionIdent: Ident, mut inBlockTypeIdent: Ident, mut inStmts: Arc<metamodelica::List<Arc<MMExp>>>, mut inPopBlockStmts: Arc<metamodelica::List<Arc<MMExp>>>, mut inInText: Ident, mut inOutText: Ident) -> Result<(Arc<metamodelica::List<(ArcStr, (Arc<MMExp>, Arc<TypeSignature>))>>, Arc<metamodelica::List<Arc<MMExp>>>, Arc<metamodelica::List<Arc<MMExp>>>, Ident)> {
    let mut outMMEscOptions: Arc<metamodelica::List<(ArcStr, (Arc<MMExp>, Arc<TypeSignature>))>>;
    let mut outStmts: Arc<metamodelica::List<Arc<MMExp>>>;
    let mut outPopBlockStmts: Arc<metamodelica::List<Arc<MMExp>>>;
    let mut outInText: Ident;
    (outMMEscOptions, outStmts, outPopBlockStmts, outInText) = 'mc: {
        let __mc_input = (inMMEscOptions, inOptionIdent, inBlockTypeIdent, inStmts, inPopBlockStmts, inInText, inOutText);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
*/
pub(crate) fn statementsFromArg(mut inExp: Expression, mut inStmts: Arc<metamodelica::List<Arc<MMExp>>>, mut inLocals: TypedIdents, mut inScopeEnv: ScopeEnv, mut inTplPackage: TemplPackage, mut inAccMMDecls: Arc<metamodelica::List<MMDeclaration>>) -> Result<((Arc<MMExp>, Arc<TypeSignature>, SourceInfo), Arc<metamodelica::List<Arc<MMExp>>>, TypedIdents, ScopeEnv, Arc<metamodelica::List<MMDeclaration>>)> {
    ...
    (outArgValue, outStmts, outLocals, outScopeEnv, outMMDecls) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                ((Deref @ ExpressionBase::FUN_CALL { name: Deref @ PathIdent::IDENT { ident: Deref @ "sourceInfo" }, args: Deref @ metamodelica::List::Nil }, sinfo @ SourceInfo { fileName, lineNumberStart, columnNumberStart, .. }), stmts, locals, scEnv, _, accMMDecls) => {
                    let mut mmexp: Arc<MMExp>;
                    let mut fname: Arc<PathIdent>;
                    let mut rettype: Arc<TypeSignature>;
...
pub(crate) fn tplStatement(mut inFunName: Ident, mut inArgs: Arc<metamodelica::List<Arc<MMExp>>>, mut inInText: Ident, mut inOutArg: Ident) -> Arc<MMExp> {
    let mut outStmt: Arc<MMExp>;
    outStmt = Arc::new(MMExp::MM_ASSIGN { lhsArgs: list![(inOutArg).clone()], rhs: Arc::new(MMExp::MM_FN_CALL { fnName: Arc::new(PathIdent::PATH_IDENT { ident: (literal!("Tpl")).clone(), path: Arc::new(PathIdent::IDENT { ident: (inFunName).clone() }) }), args: metamodelica::cons(Arc::new(MMExp::MM_IDENT { ident: Arc::new(PathIdent::IDENT { ident: (inInText).clone() }) }), inArgs) }) });
    outStmt
}

pub(crate) fn pushBlockStatement(mut inBlockType: Ident, mut inArg: Arc<MMExp>, mut inInText: Ident, mut inOutArg: Ident) -> Arc<MMExp> {
    let mut outStmt: Arc<MMExp>;
    outStmt = Arc::new(MMExp::MM_ASSIGN { lhsArgs: list![(inOutArg).clone()], rhs: Arc::new(MMExp::MM_FN_CALL { fnName: Arc::new(PathIdent::PATH_IDENT { ident: (literal!("Tpl")).clone(), path: Arc::new(PathIdent::IDENT { ident: (literal!("pushBlock")).clone() }) }), args: list![Arc::new(MMExp::MM_IDENT { ident: Arc::new(PathIdent::IDENT { ident: (inInText).clone() }) }), Arc::new(MMExp::MM_FN_CALL { fnName: Arc::new(PathIdent::PATH_IDENT { ident: (literal!("Tpl")).clone(), path: Arc::new(PathIdent::IDENT { ident: (inBlockType).clone() }) }), args: list![inArg] })] }) });
    outStmt
}

pub(crate) fn addWriteCallFromMMExp(mut inHasRetValue: bool, mut inMMExp: Arc<MMExp>, mut inType: Arc<TypeSignature>, mut inSourceInfo: SourceInfo, mut inMMEscOptions: Arc<metamodelica::List<(ArcStr, (Arc<MMExp>, Arc<TypeSignature>))>>, mut inStmts: Arc<metamodelica::List<Arc<MMExp>>>, mut inInText: Ident, mut inOutText: Ident, mut inLocals: TypedIdents, mut inScopeEnv: ScopeEnv, mut inTplPackage: TemplPackage, mut inAccMMDecls: Arc<metamodelica::List<MMDeclaration>>) -> Result<(Arc<metamodelica::List<Arc<MMExp>>>, TypedIdents, ScopeEnv, Arc<metamodelica::List<MMDeclaration>>, Ident)> {
    let mut outStmts: Arc<metamodelica::List<Arc<MMExp>>>;
    let mut outLocals: TypedIdents;
    let mut outScopeEnv: ScopeEnv;
    let mut outMMDecls: Arc<metamodelica::List<MMDeclaration>>;
    let mut outInText: Ident;
    (outStmts, outLocals, outScopeEnv, outMMDecls, outInText) = 'mc: {
        let __mc_input = (inHasRetValue, inMMExp, inType, inMMEscOptions, inStmts, inInText, inOutText, inLocals, inScopeEnv, inTplPackage, inAccMMDecls);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
//no fail
pub(crate) fn mmExpToString(mut inMMExp: Arc<MMExp>, mut inType: Arc<TypeSignature>, mut inSourceInfo: SourceInfo) -> Result<Arc<MMExp>> {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (argvals, fname, Deref @ metamodelica::List::Cons { head: iarg, tail: iargs }, Deref @ metamodelica::List::Cons { head: oarg, tail: Deref @ metamodelica::List::Nil }, tyVars, intxt, outtxt, locals, tplPackage @ TemplPackage { astDefs, .. }) => {
                    let mut mmargs: Arc<metamodelica::List<Arc<MMExp>>>;
                    let mut mmexp: Arc<MMExp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (argvals, fname, Deref @ metamodelica::List::Cons { head: iarg, tail: iargs }, Deref @ metamodelica::List::Cons { head: oarg, tail: oargs @ Deref @ metamodelica::List::Cons { head: _, tail: _ } }, tyVars, intxt, outtxt, locals, tplPackage @ TemplPackage { astDefs, .. }) => {
                    let mut mmargs: Arc<metamodelica::List<Arc<MMExp>>>;
                    let mut lhsArgs: Arc<metamodelica::List<ArcStr>>;
                    let mut mmexp: Arc<MMExp>;
                    let mut mmtxt: Arc<MMExp>;
                    areTextInOutArgs(iarg.clone(), oarg.clone(), tplPackage.clone())?;
                    (mmargs, _) = typeAdaptMMArgsForFun(argvals.clone(), iargs.clone(), tyVars.clone(), metamodelica::nil(), astDefs.clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (argvals, fname, iargs, oargs, _, _, _, _, _) => {
                    let mut errArgVals: Arc<metamodelica::List<(Arc<MMExp>, Arc<TypeSignature>)>>;
                    let mut r#str: ArcStr;
...
pub(crate) fn areTextInOutArgs(mut inInArg: (ArcStr, Arc<TypeSignature>), mut inOutArg: (ArcStr, Arc<TypeSignature>), mut inTplPackage: TemplPackage) -> Result<()> {
    let () = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                ((inid, itype), (outid, otype), TemplPackage { astDefs: astdefs, .. }) => {
                    let mut inlst: Arc<metamodelica::List<ArcStr>>;
                    let mut outlst: Arc<metamodelica::List<ArcStr>>;
...
pub(crate) fn typeAdaptMMArgsForFun(mut inArgValues: Arc<metamodelica::List<(Arc<MMExp>, Arc<TypeSignature>, SourceInfo)>>, mut inInArgs: TypedIdents, mut inTypeVars: Arc<metamodelica::List<ArcStr>>, mut inSetTypeVars: TypedIdents, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<(Arc<metamodelica::List<Arc<MMExp>>>, TypedIdents)> {
    let mut outMMArguments: Arc<metamodelica::List<Arc<MMExp>>>;
...
pub(crate) fn typeAdaptMMArg(mut inMMArg: Arc<MMExp>, mut inArgType: Arc<TypeSignature>, mut inSourceInfo: SourceInfo, mut errorWhenFail: bool, mut inTargetType: Arc<TypeSignature>, mut inTypeVars: Arc<metamodelica::List<ArcStr>>, mut inSetTypeVars: TypedIdents, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<(Arc<MMExp>, TypedIdents)> {
    let mut outMMArg: Arc<MMExp>;
    let mut outSetTypeVars: TypedIdents;
    (outMMArg, outSetTypeVars) = 'mc: {
        let __mc_input = (inMMArg, inArgType, inSourceInfo, errorWhenFail, inTargetType, inTypeVars, inSetTypeVars, inASTDefs);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
pub(crate) fn typeAdaptMMOption(mut inMMArg: Arc<MMExp>, mut inArgType: Arc<TypeSignature>, mut sinfo: SourceInfo, mut inTargetType: Arc<TypeSignature>, mut inStmts: Arc<metamodelica::List<Arc<MMExp>>>, mut inLocals: TypedIdents, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<(Arc<MMExp>, Arc<metamodelica::List<Arc<MMExp>>>, TypedIdents)> {
    let mut outMMArg: Arc<MMExp>;
    let mut outStmts: Arc<metamodelica::List<Arc<MMExp>>>;
    let mut outLocals: TypedIdents;
...
pub(crate) fn mmEnsureNonFunctionArg(mut inMMArg: Arc<MMExp>, mut inTargetType: Arc<TypeSignature>, mut inStmts: Arc<metamodelica::List<Arc<MMExp>>>, mut inLocals: TypedIdents) -> Result<(Arc<MMExp>, Arc<metamodelica::List<Arc<MMExp>>>, TypedIdents)> {
    let mut outMMArg: Arc<MMExp>;
    let mut outStmts: Arc<metamodelica::List<Arc<MMExp>>>;
    let mut outLocals: TypedIdents;
    (outMMArg, outStmts, outLocals) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (mmarg @ Deref @ MMExp::MM_FN_CALL { .. }, targettype, stmts, locals) => {
                    ...
                    stmts = metamodelica::cons(Arc::new(MMExp::MM_ASSIGN { lhsArgs: list![(retval.clone()).clone()], rhs: mmarg.clone() }), stmts.clone());
...
pub(crate) fn elabOutTextArgs(mut inMMArguments: Arc<metamodelica::List<Arc<MMExp>>>, mut inInArgs: TypedIdents, mut inOutArgs: TypedIdents, mut inTplPackage: TemplPackage) -> Result<Arc<metamodelica::List<ArcStr>>> {
    let mut outLhsArgs: Arc<metamodelica::List<ArcStr>>;
    outLhsArgs = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: exp @ Deref @ MMExp::MM_IDENT { ident: Deref @ PathIdent::IDENT { ident: txtarg } }, tail: mmargs }, Deref @ metamodelica::List::Cons { head: _, tail: iargs }, Deref @ metamodelica::List::Cons { head: _, tail: oargs }, tplPackage) => {
                    ...
                    lhsArgs = elabOutTextArgs(mmargs.clone(), iargs.clone(), oargs.clone(), tplPackage.clone())?;
                    Ok(metamodelica::cons((txtarg.clone()).clone(), lhsArgs.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: _, tail: mmargs }, Deref @ metamodelica::List::Cons { head: _, tail: iargs }, Deref @ metamodelica::List::Cons { head: _, tail: oargs }, tplPackage) => {
                    let mut lhsArgs: Arc<metamodelica::List<ArcStr>>;
                    lhsArgs = elabOutTextArgs(mmargs.clone(), iargs.clone(), oargs.clone(), tplPackage.clone())?;
                    Ok(metamodelica::cons((literal!("_")).clone(), lhsArgs.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
...
pub(crate) fn statementsFromMapExp(mut inIsFirstArgToMap: bool, mut inArgValuesToMap: Arc<metamodelica::List<(Arc<MMExp>, Arc<TypeSignature>, SourceInfo)>>, mut inMapContext: MapContext, mut inStmts: Arc<metamodelica::List<Arc<MMExp>>>, mut inInText: Ident, mut inOutText: Ident, mut inLocals: TypedIdents, mut inScopeEnv: ScopeEnv, mut inTplPackage: TemplPackage, mut inAccMMDecls: Arc<metamodelica::List<MMDeclaration>>) -> Result<(Arc<metamodelica::List<Arc<MMExp>>>, TypedIdents, ScopeEnv, Arc<metamodelica::List<MMDeclaration>>, Ident)> {
    ...
    let mut outInText: Ident;
    (outStmts, outLocals, outScopeEnv, outMMDecls, outInText) = 'mc: {
        let __mc_input = (inIsFirstArgToMap, inArgValuesToMap, inMapContext, inStmts, inInText, inOutText, inLocals, inScopeEnv, inTplPackage, inAccMMDecls);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (isfirst, Deref @ metamodelica::List::Cons { head: argtomap @ (_, argtype, _), tail: restargs }, MapContext { ofBinding: ofbind, mapExp: mapexp @ (_, sinfo), iterMMExpOptions: iopts, hasIndexIdentOpt, useIter: useiter }, stmts, intxt, outtxt, locals, scEnv, tplPackage @ TemplPackage { astDefs, .. }, accMMDecls) => {
                    let mut mapstmts: Arc<metamodelica::List<Arc<MMExp>>>;
                    let mut stmt: Arc<MMExp>;
                    let mut oftype: Arc<TypeSignature>;
                    let mut fname: Ident;
                    ...
                    let mut mapctx: MapContext;
                    let mut extargvals: Arc<metamodelica::List<(Arc<MMExp>, Arc<TypeSignature>, SourceInfo)>>;
                    let mut ofbindEnc: Arc<MatchingExp>;
                    let mut mexp: Arc<MatchingExp>;
                    let mut mmmcMatched: MMMatchCase;
                    let mut isUsed: bool;
                    let mut mmFun: MMDeclaration;
                    let mut mmmcases: Arc<metamodelica::List<(Arc<metamodelica::List<Arc<MatchingExp>>>, Arc<metamodelica::List<Arc<MMExp>>>)>>;
                    let mut assignedIdents: Arc<metamodelica::List<ArcStr>>;
                    let mut localNames: Arc<metamodelica::List<(ArcStr, ArcStr)>>;
                    ...
                    (stmts, locals, scEnv, accMMDecls, intxt) = statementsFromMapExp(false, restargs.clone(), mapctx.clone(), metamodelica::cons(stmt.clone(), stmts.clone()), (intxt.clone()).clone(), (outtxt.clone()).clone(), locals.clone(), scEnv.clone(), tplPackage.clone(), metamodelica::cons(mmFun.clone(), accMMDecls.clone()))?;
                    Ok((stmts.clone(), locals.clone(), scEnv.clone(), accMMDecls.clone(), intxt.clone()))
                }
                _ => bail!("nomatch"),
            }}
        ...
        Deref @ MatchingExp::BIND_MATCH { bindIdent: __esc_eltName } => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (isfirst, Deref @ metamodelica::List::Cons { head: argtomap @ (_, argtype, _), tail: restargs }, MapContext { ofBinding: ofbind, mapExp: mapexp @ (_, sinfo), iterMMExpOptions: iopts, hasIndexIdentOpt, useIter: useiter }, stmts, intxt, outtxt, locals, scEnv, tplPackage @ TemplPackage { astDefs, .. }, accMMDecls) => {
                    ...
                    let mut isUsed: bool;
                    let mut mmFun: MMDeclaration;
                    let mut elabcases: Arc<metamodelica::List<(Arc<MatchingExp>, Arc<metamodelica::List<(ArcStr, Arc<TypeSignature>)>>, Arc<metamodelica::List<Arc<MMExp>>>)>>;
                    let mut mmmcases: Arc<metamodelica::List<(Arc<metamodelica::List<Arc<MatchingExp>>>, Arc<metamodelica::List<Arc<MMExp>>>)>>;
                    let mut assignedIdents: Arc<metamodelica::List<ArcStr>>;
                    let mut localNames: Arc<metamodelica::List<(ArcStr, ArcStr)>>;
                    let mut useiter = (*useiter).clone();
...
pub(crate) fn intersectInOutArgs(mut inList1: TypedIdents, mut inList2: TypedIdents) -> Result<(Arc<metamodelica::List<(ArcStr, Arc<TypeSignature>)>>, Arc<metamodelica::List<(ArcStr, Arc<TypeSignature>)>>, Arc<metamodelica::List<(ArcStr, Arc<TypeSignature>)>>)> {
    pub(crate) fn areTypedIdentsEqual(mut inTypedIdent1: (ArcStr, Arc<TypeSignature>), mut inTypedIdent2: (ArcStr, Arc<TypeSignature>)) -> bool {
        let mut equal: bool;
        let mut ident1: Ident;
        let mut ident2: Ident;
        (ident1, _) = inTypedIdent1;
        (ident2, _) = inTypedIdent2;
        equal = stringEq((ident1).clone(), (ident2).clone());
        equal
    }
...
*/
pub(crate) fn shouldUseIterFunctions(mut inIsFirstArgToMap: bool, mut inUseIterLast: bool, mut inIsListArgToMap: bool, mut wasIndexVarUsed: bool, mut inIterOptions: Arc<metamodelica::List<(ArcStr, (Arc<MMExp>, Arc<TypeSignature>))>>, mut inRestArgValsToMap: Arc<metamodelica::List<(Arc<MMExp>, Arc<TypeSignature>, SourceInfo)>>) -> bool {
    let mut outUseIterFuns: bool;
    outUseIterFuns = 'mc: {
        let __mc_input = (inIsFirstArgToMap, inUseIterLast, inIsListArgToMap, wasIndexVarUsed, inIterOptions, inRestArgValsToMap);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
...
*/
pub(crate) fn addGetIndex(mut wasIndexUsed: bool, mut inLocalIdxValIdent: Ident, mut inStmts: Arc<metamodelica::List<Arc<MMExp>>>, mut inInText: Ident, mut inLocals: TypedIdents) -> Result<(Arc<metamodelica::List<Arc<MMExp>>>, TypedIdents)> {
    let mut outStmts: Arc<metamodelica::List<Arc<MMExp>>>;
    let mut outLocals: TypedIdents;
...
pub(crate) fn addPushIter(mut inDoAddPushIter: bool, mut inMMEscOptions: Arc<metamodelica::List<(ArcStr, (Arc<MMExp>, Arc<TypeSignature>))>>, mut inStmts: Arc<metamodelica::List<Arc<MMExp>>>, mut inInText: Ident, mut inOutText: Ident) -> Result<(Arc<metamodelica::List<Arc<MMExp>>>, Ident)> {
    let mut outStmts: Arc<metamodelica::List<Arc<MMExp>>>;
    let mut outInText: Ident;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (true, opts, stmts, intxt, outtxt) => {
                    let mut mmopts: Arc<metamodelica::List<Arc<MMExp>>>;
                    let mut stmt: Arc<MMExp>;
...
pub(crate) fn makeMMExpOptions(mut inMMEscOptions: Arc<metamodelica::List<(ArcStr, (Arc<MMExp>, Arc<TypeSignature>))>>, mut inSpecifiedMMEscOptions: Arc<metamodelica::List<(ArcStr, (Arc<MMExp>, Arc<TypeSignature>))>>) -> Result<(Arc<metamodelica::List<Arc<MMExp>>>, Arc<metamodelica::List<(ArcStr, (Arc<MMExp>, Arc<TypeSignature>))>>)> {
    let mut outMMExpOpts: Arc<metamodelica::List<Arc<MMExp>>>;
    let mut outRestSpecifiedMMExpOpts: Arc<metamodelica::List<(ArcStr, (Arc<MMExp>, Arc<TypeSignature>))>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: (optid, _), tail: rest }, specopts) => {
                    let mut mexpOpts: Arc<metamodelica::List<Arc<MMExp>>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (argval @ (mmexp, exptype, _), mcases, scEnv, tplPackage, accMMDecls) => {
                    ...
                    let (__pa0, __pa1, __pa2, __pa3, __pa4, __pa5, __pa6) = ::match_deref::match_deref! { match &(elabMatchCases((mmexp.clone(), exptype.clone()), (implicitValueName.clone()).clone(), mcases.clone(), hasImplicitLookup, metamodelica::nil(), metamodelica::nil(), metamodelica::cons(Scope::FUN_SCOPE { args: metamodelica::nil(), localArgs: metamodelica::nil() }, scEnv.clone()), tplPackage.clone(), accMMDecls.clone())?) {
                    ...
                    mmFun = MMDeclaration::MM_FUN { isPublic: false, name: (fname.clone()).clone(), inArgs: iargs.clone(), outArgs: oargs.clone(), locals: metamodelica::cons(imlicitTxtArg.clone(), funLocals.clone()), statements: list![Arc::new(MMExp::MM_MATCH { matchCases: mmmcases.clone() })], genInfoOpt: crate::TplAbsyn::GenInfo::GI_MATCH_FUN };
                    ...
                    Ok((argvals.clone(), Arc::new(PathIdent::IDENT { ident: (fname.clone()).clone() }), iargs.clone(), oargs.clone(), scEnv.clone(), metamodelica::cons(mmFun.clone(), accMMDecls.clone())))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
...
//no fail
pub(crate) fn alignExtArgsToScopeEnv(mut inExtraArgs: TypedIdents, mut inEncExtraArgs: TypedIdents, mut inScopeEnv: ScopeEnv) -> (TypedIdents, TypedIdents) {
    ...
    (outExtraArgs, outEncExtraArgs) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (extargs, encExtargs, Deref @ metamodelica::List::Cons { head: Scope::FUN_SCOPE { args: fargs, localArgs }, tail: _ }) => {
...
//no fail
pub(crate) fn makeMMArgValue(mut inTypedIdent: (ArcStr, Arc<TypeSignature>)) -> (Arc<MMExp>, Arc<TypeSignature>, SourceInfo) {
    let mut outArgValue: (Arc<MMExp>, Arc<TypeSignature>, SourceInfo);
...
pub(crate) fn isText(mut inArg: (ArcStr, Arc<TypeSignature>)) -> bool {
    let mut outB: bool;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (argval @ (_, exptype), Deref @ metamodelica::List::Cons { head: (mexp, exp), tail: mcases }, locals, accCaseLocals, scEnv, tplPackage @ TemplPackage { astDefs: astdefs, .. }, accMMDecls) => {
                    ...
                    (stmts, locals, scEnv, accMMDecls, _) = statementsFromExp(exp.clone(), metamodelica::nil(), metamodelica::nil(), (arcstr::literal!(imlicitTxt)).clone(), (arcstr::literal!(imlicitTxt)).clone(), locals.clone(), metamodelica::cons(Scope::CASE_SCOPE { mExp: mexp.clone(), mType: exptype.clone(), localNames: metamodelica::nil(), accLocals: accCaseLocals.clone(), extArgs: metamodelica::nil(), matchArgName: (inImplicitValueName.clone()).clone(), hasImplicitScope: hasImplicitLookup }, scEnv.clone()), tplPackage.clone(), accMMDecls.clone())?;
                    ...
                    (elabcases, locals, scEnv, accMMDecls, assignedIdents) = elabMatchCases(argval.clone(), (inImplicitValueName.clone()).clone(), mcases.clone(), hasImplicitLookup, locals.clone(), accCaseLocals.clone(), scEnv.clone(), tplPackage.clone(), accMMDecls.clone())?;
...
pub(crate) fn getAssignedIdents(mut inStatements: Arc<metamodelica::List<Arc<MMExp>>>, mut inAssignedIdents: Arc<metamodelica::List<ArcStr>>) -> Result<Arc<metamodelica::List<ArcStr>>> {
    let mut outAssignedIdents: Arc<metamodelica::List<ArcStr>>;
...
*/
//fail and error
pub(crate) fn typeCheckMatchingExp(mut inMatchingExp: Arc<MatchingExp>, mut inMType: Arc<TypeSignature>, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<Arc<MatchingExp>> {
    let mut outTransformedMatchingExp: Arc<MatchingExp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ MatchingExp::TUPLE_MATCH { tupleArgs: mexpLst }, mtype, astDefs) => {
                    let mut otLst: Arc<metamodelica::List<Arc<TypeSignature>>>;
...
pub(crate) fn typeCheckMatchingExpRecord(mut inFieldMatchings: Arc<metamodelica::List<(ArcStr, Arc<MatchingExp>)>>, mut fields: TypedIdents, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<Arc<metamodelica::List<(ArcStr, Arc<MatchingExp>)>>> {
    let mut outTransformedMatchingExp: Arc<metamodelica::List<(ArcStr, Arc<MatchingExp>)>>;
    outTransformedMatchingExp = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: (ident, mexp), tail: fms }, astDefs) => {
                    let mut mtype: Arc<TypeSignature>;
                    let mut mexp = (*mexp).clone();
                    let mut fms = (*fms).clone();
...
pub(crate) fn typeCheckMatchingExpList(mut inMatchingExpLst: Arc<metamodelica::List<Arc<MatchingExp>>>, mut inTypeLst: Arc<metamodelica::List<Arc<TypeSignature>>>, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<Arc<metamodelica::List<Arc<MatchingExp>>>> {
    let mut outTransformedMatchingExp: Arc<metamodelica::List<Arc<MatchingExp>>>;
    outTransformedMatchingExp = (::match_deref::match_deref! { match &((inMatchingExpLst, inTypeLst, inASTDefs)) {
...
pub(crate) fn eliminateWildAs(mut inMatchingExp: Arc<MatchingExp>) -> Arc<MatchingExp> {
    let mut outRewrittenMatchingExp: Arc<MatchingExp>;
...
pub(crate) fn rewriteMatchExpByLocalNames(mut inMatchingExp: Arc<MatchingExp>, mut inMType: Arc<TypeSignature>, mut inLocalNames: Arc<metamodelica::List<(ArcStr, ArcStr)>>, mut inUsedLocals: TypedIdents, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<(Arc<MatchingExp>, TypedIdents)> {
    let mut outRewrittenMatchingExp: Arc<MatchingExp>;
    ...
    (outRewrittenMatchingExp, outUsedLocals) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ MatchingExp::RECORD_MATCH { tagName: tagpath, fieldMatchings: Deref @ metamodelica::List::Nil }, mtype, usedLocals, astDefs) => {
                    let mut fldId: Ident;
                    let mut tagpath = (*tagpath).clone();
                    let mut mtype = (*mtype).clone();
                    mtype = deAliasedType(mtype.clone(), astDefs.clone());
...
pub(crate) fn rewriteMatchExpByLocalNamesRecord(mut inFieldMatchings: Arc<metamodelica::List<(ArcStr, Arc<MatchingExp>)>>, mut fields: TypedIdents, mut inLocalNames: Arc<metamodelica::List<(ArcStr, ArcStr)>>, mut inUsedLocals: TypedIdents, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<(Arc<metamodelica::List<(ArcStr, Arc<MatchingExp>)>>, TypedIdents)> {
    let mut outRewrittenMatchingExp: Arc<metamodelica::List<(ArcStr, Arc<MatchingExp>)>>;
    let mut outUsedLocals: TypedIdents;
    (outRewrittenMatchingExp, outUsedLocals) = 'mc: {
...
pub(crate) fn rewriteMatchExpByLocalNamesList(mut inMatchingExpLst: Arc<metamodelica::List<Arc<MatchingExp>>>, mut inTypeLst: Arc<metamodelica::List<Arc<TypeSignature>>>, mut inLocalNames: Arc<metamodelica::List<(ArcStr, ArcStr)>>, mut inUsedLocals: TypedIdents, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<(Arc<metamodelica::List<Arc<MatchingExp>>>, TypedIdents)> {
    let mut outRewrittenMatchingExp: Arc<metamodelica::List<Arc<MatchingExp>>>;
    let mut outUsedLocals: TypedIdents;
    (outRewrittenMatchingExp, outUsedLocals) = 'mc: {
...
pub(crate) fn addLocalValue(mut inIdent: Ident, mut inMType: Arc<TypeSignature>, mut inLocals: TypedIdents) -> Result<TypedIdents> {
    let mut outLocals: TypedIdents;
    outLocals = 'mc: {
...
pub(crate) fn makeMMMatchCase(mut inElabCase: (Arc<MatchingExp>, Arc<metamodelica::List<(ArcStr, Arc<TypeSignature>)>>, Arc<metamodelica::List<Arc<MMExp>>>), mut inExtraArgs: TypedIdents, mut inOutArgs: TypedIdents) -> Result<MMMatchCase> {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                ((mexp, caseargs, stmts), extargs, oargs) => {
                    ...
                    let mut mexpLst: Arc<metamodelica::List<Arc<MatchingExp>>>;
...
pub(crate) fn makeExtraArgBinding(mut inExtraArg: (ArcStr, Arc<TypeSignature>), mut inCaseArgs: TypedIdents, mut inOutArgs: TypedIdents) -> Result<Arc<MatchingExp>> {
    let mut outExtraArgBinding: Arc<MatchingExp>;
...
pub(crate) fn addRestElabCase(mut inElabCases: Arc<metamodelica::List<(Arc<MatchingExp>, Arc<metamodelica::List<(ArcStr, Arc<TypeSignature>)>>, Arc<metamodelica::List<Arc<MMExp>>>)>>) -> Arc<metamodelica::List<(Arc<MatchingExp>, Arc<metamodelica::List<(ArcStr, Arc<TypeSignature>)>>, Arc<metamodelica::List<Arc<MMExp>>>)>> {
...
pub(crate) fn isAlwaysMatched(mut inMatchingExp: Arc<MatchingExp>) -> Result<()> {
    let () = (::match_deref::match_deref! { match &(inMatchingExp) {
...
pub(crate) fn isAlwaysMatchedBool(mut inMatchingExp: Arc<MatchingExp>) -> bool {
...
pub(crate) fn adaptTextToString(mut inArgValue: (Arc<MMExp>, Arc<TypeSignature>, SourceInfo), mut inArgExp: Expression, mut inStmts: Arc<metamodelica::List<Arc<MMExp>>>, mut inLocals: TypedIdents, mut inTplPackage: TemplPackage) -> Result<((Arc<MMExp>, Arc<TypeSignature>, SourceInfo), Expression, Arc<metamodelica::List<Arc<MMExp>>>, TypedIdents)> {
    let mut outArgValue: (Arc<MMExp>, Arc<TypeSignature>, SourceInfo);
...
pub(crate) fn casesForTrueFalseCondition(mut inIsNot: bool, mut inNotMatchingExp: Arc<MatchingExp>, mut inTrueBranch: Expression, mut inElseBranchOpt: Option<(Arc<ExpressionBase>, SourceInfo)>) -> Result<Arc<metamodelica::List<(Arc<MatchingExp>, (Arc<ExpressionBase>, SourceInfo))>>> {
    let mut outMCases: Arc<metamodelica::List<(Arc<MatchingExp>, (Arc<ExpressionBase>, SourceInfo))>>;
    outMCases = 'mc: {
...
pub(crate) fn getElseBranch(mut inElseBranchOpt: Option<(Arc<ExpressionBase>, SourceInfo)>) -> Result<Expression> {
...
pub(crate) fn resolveBoundPath(mut inPath: Arc<PathIdent>, mut inScopeEnv: ScopeEnv, mut inTplPackage: TemplPackage) -> Result<(Arc<MMExp>, Arc<TypeSignature>, ScopeEnv)> {
    let mut outMMExp: Arc<MMExp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (path, scEnv, TemplPackage { astDefs, .. }) => {
                    ...
                    let mut idtype: Arc<TypeSignature>;
                    let mut typepckgOpt: Option<Arc<PathIdent>>;
...
pub(crate) fn checkResolvedType(mut inPath: Arc<PathIdent>, mut inType: Arc<TypeSignature>, mut inUnresolvedMsg: ArcStr, mut inInfo: SourceInfo) -> () {
    let () = 'mc: {
...
pub(crate) fn resolvePathInScopeEnv(mut inIdent: Ident, mut inPath: Arc<PathIdent>, mut canDoImplicitLookup: bool, mut inScopeEnv: ScopeEnv, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<(Ident, Arc<TypeSignature>, ScopeEnv)> {
    ...
    (outLocalIdent, outType, outScopeEnv) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (ident, path, _, Deref @ metamodelica::List::Cons { head: scope @ Scope::RECURSIVE_SCOPE { recIdent: letIdent, .. }, tail: restEnv }, astdefs) => {
                    ...
                    (ident, idtype, restEnv) = resolvePathInScopeEnv((ident.clone()).clone(), path.clone(), canDoImplicitLookup, restEnv.clone(), astdefs.clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (ident, _, _, Deref @ metamodelica::List::Cons { head: Scope::LET_SCOPE { ident: letIdent, idType: idtype, freshIdent, .. }, tail: restEnv }, _) => {
                    let true = (stringEq((ident.clone()).clone(), (letIdent.clone()).clone())) else { bail!("pattern mismatch") };
                    Ok((freshIdent.clone(), idtype.clone(), metamodelica::cons(Scope::LET_SCOPE { ident: (letIdent.clone()).clone(), idType: idtype.clone(), freshIdent: (freshIdent.clone()).clone(), isUsed: true }, restEnv.clone())))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (ident, path, _, Deref @ metamodelica::List::Cons { head: Scope::FUN_SCOPE { args: fargs, localArgs }, tail: restEnv }, astdefs) => {
                    let mut localIdent: Ident;
                    let mut idtype: Arc<TypeSignature>;
                    let mut ident = (*ident).clone();
                    let mut fargs = (*fargs).clone();
                    ...
                    let mut restEnv = (*restEnv).clone();
                    (localIdent, idtype, restEnv) = resolvePathInScopeEnv((ident.clone()).clone(), path.clone(), canDoImplicitLookup, restEnv.clone(), astdefs.clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (ident, path, _, Deref @ metamodelica::List::Cons { head: Scope::CASE_SCOPE { mExp: mexp, mType: mtype, localNames, accLocals, extArgs: extargs, matchArgName, hasImplicitScope }, tail: restEnv }, astdefs) => {
                    let mut encident: Ident;
                    let mut idtype: Arc<TypeSignature>;
                    let mut mexp = (*mexp).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (ident, path, true, Deref @ metamodelica::List::Cons { head: Scope::CASE_SCOPE { mExp: mexp, mType: mtype, localNames, accLocals, extArgs: extargs, matchArgName, hasImplicitScope: true }, tail: restEnv }, astdefs) => {
                    let mut encident: Ident;
                    let mut idtype: Arc<TypeSignature>;
                    let mut mexp = (*mexp).clone();
                    let mut localNames = (*localNames).clone();
                    ...
                    (encident, localNames, accLocals) = updateLocalsForMatchingExp((ident.clone()).clone(), (encident.clone()).clone(), 0, idtype.clone(), localNames.clone(), accLocals.clone())?;
                    Ok((encident.clone(), idtype.clone(), metamodelica::cons(Scope::CASE_SCOPE { mExp: mexp.clone(), mType: mtype.clone(), localNames: localNames.clone(), accLocals: accLocals.clone(), extArgs: extargs.clone(), matchArgName: (matchArgName.clone()).clone(), hasImplicitScope: true }, restEnv.clone())))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (ident, _, _, Deref @ metamodelica::List::Cons { head: Scope::CASE_SCOPE { mExp: mexp, mType: mtype, localNames, accLocals, extArgs: extargs, matchArgName, hasImplicitScope }, tail: restEnv }, _) => {
                    let mut encident: Ident;
                    let mut ident = (*ident).clone();
                    let mut mexp = (*mexp).clone();
                    let mut localNames = (*localNames).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (ident, path, _, Deref @ metamodelica::List::Cons { head: Scope::CASE_SCOPE { mExp: mexp, mType: mtype, localNames, accLocals, extArgs: extargs, matchArgName, hasImplicitScope }, tail: restEnv }, astdefs) => {
                    ...
                    let mut restEnv = (*restEnv).clone();
                    (encident, idtype, restEnv) = resolvePathInScopeEnv((ident.clone()).clone(), path.clone(), canDoImplicitLookup && !(hasImplicitScope.clone()), restEnv.clone(), astdefs.clone())?;
...
pub(crate) fn addPostfixToIdent(mut inIdent: Ident, mut inPostfix: i32) -> Ident {
    ...
    outPostfixedIdent = ((match (inIdent.clone(), inPostfix) {
        ...
        (mut ident, _) => {
            ident = ({ let mut __mm_s = String::new(); __mm_s.push_str(&*ident.clone()); __mm_s.push_str(&*literal!("_")); __mm_s.push_str(&*intString(inPostfix)); ArcStr::from(__mm_s) }).clone();
            ident.clone()
        },
    })).clone();
    outPostfixedIdent
...
pub(crate) fn updateLocalsForMatchingExp(mut inIdent: Ident, mut inEncIdent: Ident, mut inPostfix: i32, mut inType: Arc<TypeSignature>, mut inLocalNames: Arc<metamodelica::List<(ArcStr, ArcStr)>>, mut inLocals: TypedIdents) -> Result<(Ident, Arc<metamodelica::List<(ArcStr, ArcStr)>>, TypedIdents)> {
    let mut outLocalIdent: Ident;
    let mut outLocalNames: Arc<metamodelica::List<(ArcStr, ArcStr)>>;
    let mut outLocals: TypedIdents;
    (outLocalIdent, outLocalNames, outLocals) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (ident, localNames, locals) => {
                    let mut encIdent: Ident;
                    encIdent = (addPostfixToIdent((inEncIdent.clone()).clone(), inPostfix)).clone();
                    if '__try0: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (ident, localNames, locals) => {
                    let mut encIdent: Ident;
                    let mut loctype: Arc<TypeSignature>;
                    encIdent = (addPostfixToIdent((inEncIdent.clone()).clone(), inPostfix)).clone();
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (ident, localNames, locals) => {
                    ...
                    (encIdent, localNames, locals) = updateLocalsForMatchingExp((ident.clone()).clone(), (inEncIdent.clone()).clone(), inPostfix + 1, inType.clone(), localNames.clone(), locals.clone())?;
                    Ok((encIdent.clone(), localNames.clone(), locals.clone()))
                }
                _ => bail!("nomatch"),
            }}
...
pub(crate) fn usedInImmediateLetScope(mut inIdent: Ident, mut inFreshIdent: Ident, mut inScopeEnv: ScopeEnv) -> bool {
    let mut outIsUsed: bool;
    outIsUsed = 'mc: {
        let __mc_input = inScopeEnv;
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Scope::LET_SCOPE { ident: letIdent, freshIdent, .. }, tail: _ } => {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: Scope::RECURSIVE_SCOPE { recIdent: letIdent, freshIdent }, tail: _ } => {
                    let true = (stringEq((inIdent.clone()).clone(), (letIdent.clone()).clone())) else { bail!("pattern mismatch") };
                    let true = (stringEq((inFreshIdent.clone()).clone(), (freshIdent.clone()).clone())) else { bail!("pattern mismatch") };
                    Ok(true)
                }
...
pub(crate) fn updateLocalsForLetExp(mut inIdent: Ident, mut inEncIdent: Ident, mut inPostfix: i32, mut inType: Arc<TypeSignature>, mut inLocals: TypedIdents, mut inScopeEnv: ScopeEnv) -> Result<(Ident, TypedIdents)> {
    let mut outLocalIdent: Ident;
    let mut outLocals: TypedIdents;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                _ => {
                    ...
                    let false = (loctype.clone() == inType.clone()) else { bail!("pattern mismatch") };
                    (encIdent, locals) = updateLocalsForLetExp((inIdent.clone()).clone(), (inEncIdent.clone()).clone(), inPostfix + 1, inType.clone(), inLocals.clone(), inScopeEnv.clone())?;
...
pub(crate) fn lookupUpdateMatchingExp(mut inIdent: Ident, mut inPathIdent: Arc<PathIdent>, mut inMatchingExp: Arc<MatchingExp>, mut inMType: Arc<TypeSignature>, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<(Arc<TypeSignature>, Arc<MatchingExp>)> {
    let mut outValueType: Arc<TypeSignature>;
    let mut outMatchingExp: Arc<MatchingExp>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (inid, Deref @ PathIdent::PATH_IDENT { ident: id, path }, Deref @ MatchingExp::BIND_AS_MATCH { bindIdent: bid, matchingExp: mexp }, mtype, astDefs) => {
                    let mut valtype: Arc<TypeSignature>;
                    let mut mexp = (*mexp).clone();
                    let true = (stringEq((id.clone()).clone(), (bid.clone()).clone())) else { bail!("pattern mismatch") };
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (inid, path, Deref @ MatchingExp::TUPLE_MATCH { tupleArgs: mexpLst }, mtype, astDefs) => {
                    let mut valtype: Arc<TypeSignature>;
                    let mut mtypeLst: Arc<metamodelica::List<Arc<TypeSignature>>>;
                    let mut mexpLst = (*mexpLst).clone();
                    let __pa0 = ::match_deref::match_deref! { match &(deAliasedType(mtype.clone(), astDefs.clone())) {
                        Deref @ TypeSignature::TUPLE_TYPE { ofTypes: __pa0 } => __pa0.clone(),
                        _ => bail!("pattern mismatch"),
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (inid, path, Deref @ MatchingExp::LIST_CONS_MATCH { head: mexp, rest: restmexp }, mtype, astDefs) => {
                    let mut otype: Arc<TypeSignature>;
                    let mut valtype: Arc<TypeSignature>;
                    let mut mexp = (*mexp).clone();
                    let mut restmexp = (*restmexp).clone();
                    let __pa0 = ::match_deref::match_deref! { match &(deAliasedType(mtype.clone(), astDefs.clone())) {
...
pub(crate) fn lookupUpdateMExpDotPath(mut inIdent: Ident, mut inPathIdent: Arc<PathIdent>, mut inMatchingExp: Arc<MatchingExp>, mut inMType: Arc<TypeSignature>, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<(Arc<TypeSignature>, Arc<MatchingExp>)> {
    let mut outValueType: Arc<TypeSignature>;
    let mut outMatchingExp: Arc<MatchingExp>;
    (outValueType, outMatchingExp) = 'mc: {
...
pub(crate) fn updateFieldMatchingsForField(mut inIdent: Ident, mut inField: Ident, mut inFieldMatchings: Arc<metamodelica::List<(ArcStr, Arc<MatchingExp>)>>) -> Result<Arc<metamodelica::List<(ArcStr, Arc<MatchingExp>)>>> {
    let mut outFieldMatchings: Arc<metamodelica::List<(ArcStr, Arc<MatchingExp>)>>;
...
pub(crate) fn makeBindAs(mut inIdent: Ident, mut inMExp: Arc<MatchingExp>) -> Result<Arc<MatchingExp>> {
    let mut outMExp: Arc<MatchingExp>;
    outMExp = 'mc: {
...
pub(crate) fn lookupUpdateMExpRecord(mut inIdent: Ident, mut inPathIdent: Arc<PathIdent>, mut inFieldMatchings: Arc<metamodelica::List<(ArcStr, Arc<MatchingExp>)>>, mut inFields: TypedIdents, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<(Arc<TypeSignature>, Arc<metamodelica::List<(ArcStr, Arc<MatchingExp>)>>)> {
    let mut outValueType: Arc<TypeSignature>;
    let mut outFieldMatchings: Arc<metamodelica::List<(ArcStr, Arc<MatchingExp>)>>;
    (outValueType, outFieldMatchings) = 'mc: {
...
pub(crate) fn lookupUpdateMExpList(mut inIdent: Ident, mut inPathIdent: Arc<PathIdent>, mut inMExpList: Arc<metamodelica::List<Arc<MatchingExp>>>, mut inMTypeList: Arc<metamodelica::List<Arc<TypeSignature>>>, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<(Arc<TypeSignature>, Arc<metamodelica::List<Arc<MatchingExp>>>)> {
    let mut outValueType: Arc<TypeSignature>;
    let mut outMExpList: Arc<metamodelica::List<Arc<MatchingExp>>>;
...
pub(crate) fn getFieldsForRecord(mut inMType: Arc<TypeSignature>, mut inTagPath: Arc<PathIdent>, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<(TypedIdents, Arc<PathIdent>)> {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ TypeSignature::NAMED_TYPE { name: typepath }, tagpath, astDefs) => {
                    ...
                    let mut typepckg: Arc<PathIdent>;
                    let mut typepckgOpt: Option<Arc<PathIdent>>;
                    let mut tagpckgOpt: Option<Arc<PathIdent>>;
...
pub(crate) fn splitPackageAndIdent(mut inTypePathIdent: Arc<PathIdent>) -> Result<(Option<Arc<PathIdent>>, Ident)> {
    let mut outPackagePath: Option<Arc<PathIdent>>;
...
pub(crate) fn makePathIdent(mut inPackage: Arc<PathIdent>, mut inIdent: Ident) -> Result<Arc<PathIdent>> {
...
pub(crate) fn getTypeInfo(mut inTypePackageOpt: Option<Arc<PathIdent>>, mut inTypeIdent: Ident, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<(Arc<PathIdent>, TypeInfo)> {
    ...
    (outTypePackage, outTypeInfo) = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (None, typeident, Deref @ metamodelica::List::Cons { head: ASTDef { importPackage: importckg, isDefault: true, types: typeLst }, tail: _ }) => {
                    let mut typeinfo: TypeInfo;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Some(typepckg), typeident, Deref @ metamodelica::List::Cons { head: ASTDef { importPackage: importckg, types: typeLst, .. }, tail: _ }) => {
                    let mut typeinfo: TypeInfo;
...
fn deAliasedType(mut inType: Arc<TypeSignature>, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Arc<TypeSignature> {
    let mut outType: Arc<TypeSignature>;
    outType = 'mc: {
        let __mc_input = (inType.clone(), inASTDefs);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ TypeSignature::NAMED_TYPE { name: typepath }, astDefs) => {
                    let mut dt: Arc<TypeSignature>;
                    let mut typeident: Ident;
                    let mut typepckgOpt: Option<Arc<PathIdent>>;
                    (typepckgOpt, typeident) = splitPackageAndIdent(typepath.clone())?;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ TypeSignature::NAMED_TYPE { name: Deref @ PathIdent::IDENT { ident: tid } }, tyConcrete, Deref @ metamodelica::List::Cons { head: _, tail: _ }, setTyVars, astDefs) => {
                    let mut ty: Arc<TypeSignature>;
                    let mut tyConcreteDA: Arc<TypeSignature>;
...
fn typesEqualConcrete(mut inTypeA: Arc<TypeSignature>, mut inTypeB: Arc<TypeSignature>, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<()> {
    let () = 'mc: {
        let __mc_input = (inTypeA, inTypeB, inASTDefs);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ TypeSignature::NAMED_TYPE { name: Deref @ PathIdent::IDENT { ident: tid } }, Deref @ metamodelica::List::Cons { head: _, tail: _ }, setTyVars) => {
                    let mut tyConcrete: Arc<TypeSignature>;
...
pub(crate) fn getFunSignature(mut inFunName: Arc<PathIdent>, mut inSourceInfo: SourceInfo, mut inTplPackage: TemplPackage) -> Result<(Arc<PathIdent>, TypedIdents, TypedIdents, Arc<metamodelica::List<ArcStr>>)> {
    let mut outPath: Arc<PathIdent>;
    let mut outInArgs: TypedIdents;
    let mut outOutArgs: TypedIdents;
    let mut outTypeVars: Arc<metamodelica::List<ArcStr>>;
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (fname, TemplPackage { astDefs, .. }) => {
                    ...
                    let mut funpckgOpt: Option<Arc<PathIdent>>;
...
pub(crate) fn checkPackageOpt(mut inPackage: Arc<PathIdent>, mut inPackageOpt: Option<Arc<PathIdent>>) -> Result<()> {
    let () = 'mc: {
        let __mc_input = (inPackage, inPackageOpt);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (_, None) => {
                    Ok(())
...
pub(crate) fn getFields(mut inTagIdent: Ident, mut inTypeInfo: TypeInfo, mut inTypeIdent: Ident) -> Result<TypedIdents> {
    let mut outFields: TypedIdents;
    outFields = 'mc: {
...
pub(crate) fn isRecordTag(mut inTagIdent: Ident, mut inTypeInfo: TypeInfo, mut inTypeIdent: Ident) -> Result<()> {
    let () = (match (inTagIdent, inTypeInfo, inTypeIdent) {
...
pub(crate) fn fullyQualifyASTDefs(mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<Arc<metamodelica::List<ASTDef>>> {
    let mut outFullyQualifiedASTDefs: Arc<metamodelica::List<ASTDef>>;
    outFullyQualifiedASTDefs = 'mc: {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: ASTDef { importPackage: importckg, isDefault: isdefault, types: typeLst }, tail: restAstDefs } => {
                    ...
                    Ok(metamodelica::cons(ASTDef { importPackage: importckg.clone(), isDefault: isdefault.clone(), types: typeLst.clone() }, restAstDefs.clone()))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ metamodelica::List::Cons { head: ASTDef { importPackage: importckg, types: typeLst, .. }, tail: _ } => {
                    let mut typeLst = (*typeLst).clone();
                    let true = (Flags::isSet(Flags::FAILTRACE.clone())?) else { bail!("pattern mismatch") };
                    if '__try0: {
...
pub(crate) fn fullyQualifyAstTypeInfo(mut inASTTypeInfo: TypeInfo, mut inImportPackage: Arc<PathIdent>) -> Result<TypeInfo> {
...
pub(crate) fn fullyQualifyAstTypedIdents(mut inASTDefTypedIdents: TypedIdents, mut inImportPackage: Arc<PathIdent>, mut inTypeVars: Arc<metamodelica::List<ArcStr>>) -> Result<TypedIdents> {
    let mut outASTDefTypedIdents: TypedIdents;
    outASTDefTypedIdents = listMap2Tuple22(inASTDefTypedIdents, (std::sync::Arc::new(fnptr!(fullyQualifyAstTypeSignature, Arc<TypeSignature>, Arc<PathIdent>, Arc<metamodelica::List<ArcStr>>)) as std::sync::Arc<dyn ::std::ops::Fn(Arc<TypeSignature>, Arc<PathIdent>, Arc<metamodelica::List<ArcStr>>) -> Result<Arc<TypeSignature>> + 'static>), inImportPackage, inTypeVars)?;
    Ok(outASTDefTypedIdents)
}

pub(crate) fn fullyQualifyAstTypeSignature(mut inASTDefTypeSignature: Arc<TypeSignature>, mut inImportPackage: Arc<PathIdent>, mut inTypeVars: Arc<metamodelica::List<ArcStr>>) -> Arc<TypeSignature> {
...
pub(crate) fn convertNameTypeIfIntrinsic(mut inNameOfType: Arc<PathIdent>) -> Arc<TypeSignature> {
    let mut outTypeSignature: Arc<TypeSignature>;
    outTypeSignature = (::match_deref::match_deref! { match &(inNameOfType.clone()) {
        ...
        _ => Arc::new(TypeSignature::NAMED_TYPE { name: inNameOfType }),
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    outTypeSignature
}

pub(crate) fn fullyQualifyTemplateDef(mut inTemplateDef: TemplateDef, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Result<TemplateDef> {
...
pub(crate) fn fullyQualifyTemplateTypeSignature(mut inTemplateTypeSignature: Arc<TypeSignature>, mut inASTDefs: Arc<metamodelica::List<ASTDef>>) -> Arc<TypeSignature> {
    let mut outFullyQualifiedTypeSignature: Arc<TypeSignature>;
    outFullyQualifiedTypeSignature = 'mc: {
        let __mc_input = (inTemplateTypeSignature.clone(), inASTDefs);
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (Deref @ metamodelica::List::Cons { head: h, tail: rest }, itemA) => {
                    ...
                    Ok((itemB.clone(), metamodelica::cons(h.clone(), rest.clone())))
                }
                _ => bail!("nomatch"),
            }}
        })() { break 'mc __v; }
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                (lstAl, Deref @ metamodelica::List::Cons { head: (a, _), tail: lstBy }) => {
                    let mut b: Type_b;
                    let mut lst: Arc<metamodelica::List<(Type_a, Type_b)>>;
...
fn listMap1Tuple22<Type_a: Clone + 'static + metamodelica::gc::MMTrace, Type_b: Clone + 'static + metamodelica::gc::MMTrace, Type_d: Clone + 'static + metamodelica::gc::MMTrace, Type_c: Clone + 'static + metamodelica::gc::MMTrace>(mut inList: Arc<metamodelica::List<(Type_a, Type_b)>>, mut inFun_Tbd_to_Tc: Arc<dyn ::std::ops::Fn(Type_b, Type_d) -> Result<Type_c> + 'static>, mut inExtraArg: Type_d) -> Result<Arc<metamodelica::List<(Type_a, Type_c)>>> {
    ...
    outList = (::match_deref::match_deref! { match &((inList, inFun_Tbd_to_Tc.clone(), inExtraArg)) {
        (Deref @ metamodelica::List::Nil, _, _) => {
            metamodelica::nil()
        },
...
fn listMap2Tuple22<Type_a: Clone + 'static + metamodelica::gc::MMTrace, Type_b: Clone + 'static + metamodelica::gc::MMTrace, Type_d: Clone + 'static + metamodelica::gc::MMTrace, Type_e: Clone + 'static + metamodelica::gc::MMTrace, Type_c: Clone + 'static + metamodelica::gc::MMTrace>(mut inList: Arc<metamodelica::List<(Type_a, Type_b)>>, mut inFun_Tbde_to_Tc: Arc<dyn ::std::ops::Fn(Type_b, Type_d, Type_e) -> Result<Type_c> + 'static>, mut inExtraArg: Type_d, mut inExtraArg2: Type_e) -> Result<Arc<metamodelica::List<(Type_a, Type_c)>>> {
    ...
    let mut outList: Arc<metamodelica::List<(Type_a, Type_c)>>;
    outList = (::match_deref::match_deref! { match &((inList, inFun_Tbde_to_Tc.clone(), inExtraArg, inExtraArg2)) {
...
pub(crate) fn addSusanError(mut inErrMsg: ArcStr, mut inInfo: SourceInfo) -> Result<()> {
    ...
    Error::addSourceMessage(Error::SUSAN_ERROR.clone(), list![(inErrMsg).clone()], inInfo)?;
    Ok(())
}

fn addSusanNotification(mut inErrMsg: ArcStr, mut inInfo: SourceInfo) -> Result<()> {
    Error::addSourceMessage(Error::SUSAN_NOTIFY.clone(), list![(inErrMsg).clone()], inInfo)?;
    Ok(())
}

pub(crate) fn canBeEscapedUnquoted(mut inStringList: Arc<metamodelica::List<ArcStr>>) -> bool {
...
pub(crate) fn canBeOnOneLine(mut inStringList: Arc<metamodelica::List<ArcStr>>) -> bool {
    let mut outCanBeOnOneLine: bool;
    outCanBeOnOneLine = (inStringList.clone().len() as i32) <= 4 && (stringAppendList(inStringList).len() as i32) <= 10;
    outCanBeOnOneLine
}

pub(crate) fn pathIdentString(mut inPathIndent: Arc<PathIdent>) -> Result<ArcStr> {
    let mut outPathIdentString: ArcStr;
    outPathIdentString = ('mc: {
        let __mc_input = inPathIndent;
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
        ...
        if let Ok(__v) = (|| -> Result<_> {
            ::match_deref::match_deref! { match &__mc_input {
                Deref @ PathIdent::PATH_IDENT { ident, path } => {
                    let mut ident = (*ident).clone();
...
pub(crate) fn typeSignatureString(mut inTS: Arc<TypeSignature>) -> Result<ArcStr> {
    ...
    outStr = (Tpl::textString(txt)?).clone();
    Ok(outStr)
}

pub(crate) fn mmExpString(mut inMMExp: Arc<MMExp>) -> Result<ArcStr> {
    let mut outStr: ArcStr;
    let mut txt: Tpl::Text;
    txt = TplCodegen::mmExp(eTxt.clone(), inMMExp, (literal!("=")).clone())?;
    outStr = (Tpl::textString(txt)?).clone();
    Ok(outStr)
}

pub(crate) fn stmtsString(mut inStmts: Arc<metamodelica::List<Arc<MMExp>>>) -> Result<ArcStr> {
    let mut outStr: ArcStr;
    let mut txt: Tpl::Text;
...
pub(crate) fn removeUnusedImports(mut pkg: MMPackage) -> Result<MMPackage> {
    ...
    let mut set: Arc<AvlSetString::Tree>;
    ...
    pkg.mmDeclarations = ({
        ...
        for mut elt in (pkg.mmDeclarations.clone()).into_iter().cloned() {
            if !((match elt.clone() {
        MMDeclaration::MM_IMPORT { packageName: ref __esc_name, .. } => {
...
fn addTypeSignatureToSet(mut set: Arc<AvlSetString::Tree>, mut sig: Arc<TypeSignature>) -> Result<Arc<AvlSetString::Tree>> {
    ...
    let mut name: Arc<PathIdent> = Arc::new(<PathIdent as ::std::default::Default>::default());
    set = (::match_deref::match_deref! { match &(sig) {
        Deref @ TypeSignature::LIST_TYPE { ofType: __esc_sig2 } => {
            sig2 = (*__esc_sig2).clone();
            addTypeSignatureToSet(set, sig2.clone())?
        },
        Deref @ TypeSignature::ARRAY_TYPE { ofType: __esc_sig2 } => {
            sig2 = (*__esc_sig2).clone();
            addTypeSignatureToSet(set, sig2.clone())?
        },
        Deref @ TypeSignature::OPTION_TYPE { ofType: __esc_sig2 } => {
            sig2 = (*__esc_sig2).clone();
            addTypeSignatureToSet(set, sig2.clone())?
        },
        Deref @ TypeSignature::TUPLE_TYPE { ofTypes: __esc_sigs } => {
            sigs = (*__esc_sigs).clone();
            List::foldr(sigs.clone(), (std::sync::Arc::new(addTypeSignatureToSet) as std::sync::Arc<dyn ::std::ops::Fn(Arc<AvlSetString::Tree>, Arc<TypeSignature>) -> Result<Arc<AvlSetString::Tree>> + 'static>), set)?
        },
        Deref @ TypeSignature::NAMED_TYPE { name: __esc_name } => {
            name = (*__esc_name).clone();
            addPathIdentToSet(set, name.clone())?
        },
        _ => set,
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
...
fn addPathIdentToSet(mut set: Arc<AvlSetString::Tree>, mut name: Arc<PathIdent>) -> Result<Arc<AvlSetString::Tree>> {
    ...
    set = (::match_deref::match_deref! { match &(name.clone()) {
        Deref @ PathIdent::IDENT { .. } => AvlSetString::add(set, (var_field!((*name).ident, PathIdent::IDENT).clone()).clone())?,
        Deref @ PathIdent::PATH_IDENT { .. } => AvlSetString::add(set, (var_field!((*name).ident, PathIdent::PATH_IDENT).clone()).clone())?,
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    Ok(set)
}
...
fn addExpToSet(mut set: Arc<AvlSetString::Tree>, mut exp: Arc<MMExp>) -> Result<Arc<AvlSetString::Tree>> {
    ...
    set = (::match_deref::match_deref! { match &(exp.clone()) {
        Deref @ MMExp::MM_ASSIGN { .. } => addExpToSet(set, var_field!((*exp).rhs, MMExp::MM_ASSIGN).clone())?,
        Deref @ MMExp::MM_FN_CALL { .. } => List::foldr(var_field!((*exp).args, MMExp::MM_FN_CALL).clone(), (std::sync::Arc::new(addExpToSet) as std::sync::Arc<dyn ::std::ops::Fn(Arc<AvlSetString::Tree>, Arc<MMExp>) -> Result<Arc<AvlSetString::Tree>> + 'static>), addPathIdentToSet(set, var_field!((*exp).fnName, MMExp::MM_FN_CALL).clone())?)?,
        Deref @ MMExp::MM_IDENT { .. } => addPathIdentToSet(set, var_field!((*exp).ident, MMExp::MM_IDENT).clone())?,
        Deref @ MMExp::MM_MATCH { .. } => List::foldr(var_field!((*exp).matchCases, MMExp::MM_MATCH).clone(), (std::sync::Arc::new(addMatchCaseToSet) as std::sync::Arc<dyn ::std::ops::Fn(Arc<AvlSetString::Tree>, (Arc<metamodelica::List<Arc<MatchingExp>>>, Arc<metamodelica::List<Arc<MMExp>>>)) -> Result<Arc<AvlSetString::Tree>> + 'static>), set)?,
        Deref @ MMExp::MM_LIST_FOR_LOOP { .. } => List::foldr(var_field!((*exp).matchCases, MMExp::MM_LIST_FOR_LOOP).clone(), (std::sync::Arc::new(addMatchCaseToSet) as std::sync::Arc<dyn ::std::ops::Fn(Arc<AvlSetString::Tree>, (Arc<metamodelica::List<Arc<MatchingExp>>>, Arc<metamodelica::List<Arc<MMExp>>>)) -> Result<Arc<AvlSetString::Tree>> + 'static>), set)?,
        _ => set,
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    Ok(set)
...
fn addMatchCaseToSet(mut set: Arc<AvlSetString::Tree>, mut c: MMMatchCase) -> Result<Arc<AvlSetString::Tree>> {
    let mut set: Arc<AvlSetString::Tree> = set;
    let mut mexps: Arc<metamodelica::List<Arc<MatchingExp>>>;
    let mut exps: Arc<metamodelica::List<Arc<MMExp>>>;
    (mexps, exps) = c;
    set = List::foldr(exps, (std::sync::Arc::new(addExpToSet) as std::sync::Arc<dyn ::std::ops::Fn(Arc<AvlSetString::Tree>, Arc<MMExp>) -> Result<Arc<AvlSetString::Tree>> + 'static>), set)?;
    set = List::foldr(mexps, (std::sync::Arc::new(addMatchingExpToSet) as std::sync::Arc<dyn ::std::ops::Fn(Arc<AvlSetString::Tree>, Arc<MatchingExp>) -> Result<Arc<AvlSetString::Tree>> + 'static>), set)?;
    Ok(set)
}
...
fn addMatchingExpToSet(mut set: Arc<AvlSetString::Tree>, mut exp: Arc<MatchingExp>) -> Result<Arc<AvlSetString::Tree>> {
    ...
    set = (::match_deref::match_deref! { match &(exp.clone()) {
        Deref @ MatchingExp::BIND_AS_MATCH { .. } => addMatchingExpToSet(set, var_field!((*exp).matchingExp, MatchingExp::BIND_AS_MATCH).clone())?,
        Deref @ MatchingExp::RECORD_MATCH { .. } => {
            set = addPathIdentToSet(set, var_field!((*exp).tagName, MatchingExp::RECORD_MATCH).clone())?;
        ...
        Deref @ MatchingExp::SOME_MATCH { .. } => addMatchingExpToSet(set, var_field!((*exp).value, MatchingExp::SOME_MATCH).clone())?,
        Deref @ MatchingExp::TUPLE_MATCH { .. } => List::foldr(var_field!((*exp).tupleArgs, MatchingExp::TUPLE_MATCH).clone(), (std::sync::Arc::new(addMatchingExpToSet) as std::sync::Arc<dyn ::std::ops::Fn(Arc<AvlSetString::Tree>, Arc<MatchingExp>) -> Result<Arc<AvlSetString::Tree>> + 'static>), set)?,
        Deref @ MatchingExp::LIST_MATCH { .. } => List::foldr(var_field!((*exp).listElts, MatchingExp::LIST_MATCH).clone(), (std::sync::Arc::new(addMatchingExpToSet) as std::sync::Arc<dyn ::std::ops::Fn(Arc<AvlSetString::Tree>, Arc<MatchingExp>) -> Result<Arc<AvlSetString::Tree>> + 'static>), set)?,
        Deref @ MatchingExp::LIST_CONS_MATCH { .. } => addMatchingExpToSet(addMatchingExpToSet(set, var_field!((*exp).head, MatchingExp::LIST_CONS_MATCH).clone())?, var_field!((*exp).rest, MatchingExp::LIST_CONS_MATCH).clone())?,
        _ => set,
        _ => unreachable!("match_deref! exhaustiveness placeholder"),
    } });
    Ok(set)
